...
namespace MMAP
{
    ...
    void MapBuilder::discoverTiles()
    {
        ...
        for (TileList::iterator itr = m_tiles.begin(); itr != m_tiles.end(); ++itr)
        {
            ...
            files.clear();
            char l_Buffer[4096];
            sprintf(l_Buffer, "%04u_*.vmtile", mapID);
        ...
        dtNavMesh* navMesh)
    {
        ...
        config.maxVertsPerPoly = DT_VERTS_PER_POLYGON;      ///< ok
        config.cs = BASE_UNIT_DIM;
        config.ch = BASE_UNIT_DIM;
        config.walkableSlopeAngle = m_maxWalkableAngle;     ///< ok
        config.tileSize = VERTEX_PER_TILE;
        config.walkableRadius = m_bigBaseUnit ? 1 : 2;      ///< ok
        ...
        config.detailSampleDist = config.cs * 6.f;      ///< ok
        config.detailSampleMaxError = config.ch * 1.f;      ///< ok
