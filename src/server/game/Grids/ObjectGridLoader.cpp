...
void AddObjectHelper(CellCoord &cell, EventObjectMapType& m, uint32 &count, Map* map, EventObject *obj)
{
    obj->AddToGrid(m);
    ObjectGridLoader::SetObjectCell(obj, cell);
    obj->AddToWorld();
...
void ObjectGridLoader::Visit(EventObjectMapType &m)
{
    CellCoord cellCoord = i_cell.GetCellCoord();
    CellObjectGuids const& cell_guids = sObjectMgr->GetCellObjectGuids(i_map->GetId(), i_map->GetSpawnMode(), cellCoord.GetId());
    LoadHelper(cell_guids.eventobject, cellCoord, m, i_eventobject, i_map);
}
...
void ObjectGridLoader::Visit(AreaTriggerMapType &m)
{
    CellCoord cellCoord = i_cell.GetCellCoord();
    CellObjectGuids const& cell_guids = sObjectMgr->GetCellObjectGuids(i_map->GetId(), i_map->GetSpawnMode(), cellCoord.GetId());
    LoadHelper(cell_guids.areatriggers, cellCoord, m, i_areatrigger, i_map);
}
...
template void ObjectGridUnloader::Visit(EventObjectMapType &);
...
template void ObjectGridCleaner::Visit<EventObject>(EventObjectMapType &);
