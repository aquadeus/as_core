...
struct CellArea
{
    ...
    bool operator== (CellArea const& p_CellArea) const
    {
        return (low_bound == p_CellArea.low_bound
            && high_bound == p_CellArea.high_bound);
    }

    bool operator!= (CellArea const& p_CellArea) const
    {
        return !(*this == p_CellArea);
    }
...
struct Cell
{
    ...
    template<class T, class CONTAINER> void VisitWithNearbies(CellCoord const&, TypeContainerVisitor<T, CONTAINER>& visitor, Map &, WorldObject const&, float) const;
    template<class T, class CONTAINER> void VisitWithNearbies(CellCoord const&, TypeContainerVisitor<T, CONTAINER>& visitor, Map &, float, float, float) const;
...
private:
    ...
    template<class T, class CONTAINER> void VisitCircleWithNearbies(TypeContainerVisitor<T, CONTAINER> &, Map &, CellCoord const&, CellCoord const&) const;
};
