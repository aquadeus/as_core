...
template<class T, class CONTAINER>
inline void Cell::VisitWithNearbies(CellCoord const& standing_cell, TypeContainerVisitor<T, CONTAINER>& visitor, Map& map, float radius, float x_off, float y_off) const
{
    if (!standing_cell.IsCoordValid())
        return;
    ...
    if (radius <= 0.0f)
    {
        map.VisitWithNearbies(*this, visitor);
        return;
    }
    //lets limit the upper value for search radius
    if (radius > SIZE_OF_GRIDS)
    ...
    if ((area.high_bound.x_coord > (area.low_bound.x_coord + 4)) && (area.high_bound.y_coord > (area.low_bound.y_coord + 4)))
    {
        VisitCircleWithNearbies(visitor, map, area.low_bound, area.high_bound);
        return;
    }
    ...
    for (uint32 x = area.low_bound.x_coord; x <= area.high_bound.x_coord; ++x)
    {
        for (uint32 y = area.low_bound.y_coord; y <= area.high_bound.y_coord; ++y)
        {
            ...
            if (cellCoord != standing_cell)
            {
                Cell r_zone(cellCoord);
                r_zone.data.Part.nocreate = this->data.Part.nocreate;
                map.VisitWithNearbies(r_zone, visitor);
            }
        }
    }
}
...
template<class T, class CONTAINER>
inline void Cell::VisitWithNearbies(CellCoord const& standing_cell, TypeContainerVisitor<T, CONTAINER>& visitor, Map& map, WorldObject const& obj, float radius) const
{
    //we should increase search radius by object's radius, otherwise
    //we could have problems with huge creatures, which won't attack nearest players etc
    VisitWithNearbies(standing_cell, visitor, map, radius + obj.GetObjectSize(), obj.GetPositionX(), obj.GetPositionY());
}
...
template<class T, class CONTAINER>
inline void Cell::VisitCircleWithNearbies(TypeContainerVisitor<T, CONTAINER>& visitor, Map& map, CellCoord const& begin_cell, CellCoord const& end_cell) const
{
    //here is an algorithm for 'filling' circum-squared octagon
    uint32 x_shift = (uint32)ceilf((end_cell.x_coord - begin_cell.x_coord) * 0.3f - 0.5f);
    //lets calculate x_start/x_end coords for central strip...
    ...
    for (uint32 step = 1; step <= (x_start - begin_cell.x_coord); ++step)
    {
        ...
        for (uint32 y = y_start; y >= y_end; --y)
        {
            ...
            Cell r_zone_left(cellCoord_left);
            r_zone_left.data.Part.nocreate = this->data.Part.nocreate;
            map.VisitWithNearbies(r_zone_left, visitor);

            //right trapezoid cell visit
            CellCoord cellCoord_right(x_end + step, y);
            Cell r_zone_right(cellCoord_right);
            r_zone_right.data.Part.nocreate = this->data.Part.nocreate;
            map.VisitWithNearbies(r_zone_right, visitor);
        }
    }
}
#endif
