...
static DumpTable g_DumpTables[] =
{
    ...
    { "character_artifact",                  DTT_CHAR_TABLE , {}},
    { "character_artifact_powers",           DTT_CHAR_TABLE , {}},
    ...
    //{ "character_pvp_info",                  DTT_CHAR_TABLE , {}},
    ...
    { "character_garrison_broken_isles",     DTT_GARR_CONTINENT  , {}},
    { "character_garrison_draenor",          DTT_GARR_CONTINENT  , {}},
    { "character_garrison_building",         DTT_GARR_B     , {}},
    { "character_garrison_draenor_building", DTT_GARR_B     , {}},
    ...
    { "item_instance_gems",                  DTT_ITEM_TABLE , {}},
    { "item_instance_transmog",              DTT_ITEM_TABLE , {}},
    { "mail",                                DTT_MAIL       , {}},
...
std::string CreateDumpString(uint32 type, char const* tableName, QueryResult result, std::vector<std::string> const& columns, uint32 p_Expansion)
{
    if (!tableName || !result)
        return "";

    if (p_Expansion && (std::string(tableName) == "account_achievement"
        || std::string(tableName) == "account_achievement_progress"
        || std::string(tableName) == "character_aura"
        || std::string(tableName) == "character_aura_effect"))
        return "";

    std::ostringstream ss;

    if (p_Expansion && std::string(tableName) == "character_garrison_draenor")
        ss << "REPLACE INTO " << _TABLE_SIM_ << tableName << _TABLE_SIM_;
    else
    ...
    for (int32 l_I = 0; l_I < (int32)columns.size(); l_I++)
    {
        if (p_Expansion &&
            ...
            || (std::string(tableName) == "item_instance" && columns[l_I] == "challengeKeyIsCharded")))
        {
...
bool PlayerDumpWriter::DumpTable(std::string& dump, uint32 guid, uint32 account, char const*tableFrom, char const*tableTo, DumpTableType type, std::vector<std::string> const& columns, uint32 p_Expansion, uint32& p_GarrisonID)
{
    GUIDs const* guids = NULL;
    char const* fieldname = NULL;

    std::string l_SrcTable = tableFrom;
    ...
    if (type == DTT_ACC_SPELL || type == DTT_ACC_ACH || type == DTT_ACC_ACH_PRO)
        return true;
    ...
    do
    {
        ...
        else if (strcmp(fieldname, "garrison_id") == 0 || type == DTT_GARR_CONTINENT)
            wherestr = GenerateWhereStr(fieldname, p_GarrisonID);
        else
            // not set case, get single guid string
            wherestr = GenerateWhereStr(fieldname, guid);

        QueryResult result = CharacterDatabase.PQuery("SELECT * FROM %s WHERE %s", l_SrcTable.c_str(), wherestr.c_str());
        if (!result)
            return true;
        ...
        do
        {
            ...
            dump += CreateDumpString(type, tableTo, result, columns, p_Expansion);
            dump += "\n";
        }
...
bool PlayerDumpWriter::GetDump(uint32 guid, uint32 account, std::string &dump, uint32 p_Expansion)
{
    dump = "";
    ...
    {
        if (!std::strcmp(g_DumpTables[i].name, "character_garrison_building"))
            continue;

        if (!DumpTable(dump, guid, account, g_DumpTables[i].name, g_DumpTables[i].name, g_DumpTables[i].type, g_DumpTables[i].columns, p_Expansion, l_GarrisonID))
            return false;
    }
...
DumpReturn PlayerDumpWriter::WriteDump(const std::string& file, uint32 guid, uint32 account, uint32 p_Expansion)
{
    ...
    std::string dump;
    if (!GetDump(guid, account, dump, p_Expansion))
...
DumpReturn PlayerDumpReader::LoadDump(const std::string& p_File, uint32 p_Account, std::string p_Name, uint32& p_Guid, bool p_OnlyBoundedItems, uint32 p_AtLogin, uint32 p_TransactionID, uint32 p_StartRealm /*= 0*/)
{
    bool l_IsCharacterAuction = p_Name.find("#characterauction-") != std::string::npos;

    uint32 charcount = AccountMgr::GetCharactersCount(p_Account);
    if (!l_IsCharacterAuction && charcount >= sWorld->getIntConfig(CONFIG_CHARACTERS_PER_REALM))
    ...
    while (!feof(fin))
    {
        ...
        if (p_TransactionID != 0 && (l_TableType == DTT_PET || l_TableType == DTT_PET_TABLE) && l_Class == Classes::CLASS_WARLOCK)
            continue;

        if (l_IsCharacterAuction && l_TableName == "item_instance_transmog")
            continue;
        ...
        switch (l_TableType)
        {
            case DTT_CHARACTER:
            {
                ...
                if (getnth(l_Line, l_Index).empty())
                {
                ...
                l_Index = GetFieldIndexFromColumn("class", l_Columns) + 1;
                l_Class = std::atoi(getnth(l_Line, l_Index).c_str());

                /// Copy from older extension
                /// Convert player bytes to legion
                if (p_AtLogin & AtLoginFlags::AT_LOGIN_DELETE_INVALID_SPELL)
                {
                    l_Index = GetFieldIndexFromColumn("playerBytes2", l_Columns) + 1;
                    uint32 l_OldPlayerBytes2 = std::atoi(getnth(l_Line, l_Index).c_str());

                    uint8 l_FacialStyle  = (l_OldPlayerBytes2) & 0xFF;
                    uint8 l_BankBagSlots = (l_OldPlayerBytes2 >> 16) & 0xFF;

                    uint32 l_PlayerBytes2 = l_FacialStyle << 24;
                    uint32 l_PlayerBytes3 = l_BankBagSlots << 8 | std::atoi(getnth(l_Line, GetFieldIndexFromColumn("gender", l_Columns) + 1).c_str()) << 16;

                    if (!changenth(l_Line, l_Index, std::to_string(l_PlayerBytes2).c_str()))
                    {
                        sLog->outAshran("LoadDump: DUMP_FILE_BROKEN [characters.playerBytes2]");
                        ROLLBACK(DUMP_FILE_BROKEN);
                    }

                    /// Insert it there to append a update for the playerBytes3 (the field doesn't exist in pervious extension so we have to use a update query)
                    l_AllowedAppend = false;
                    fixNULLfields(l_Line);
                    l_CharTransaction->Append(l_Line.c_str());

                    std::ostringstream l_UpdateQuery;
                    l_UpdateQuery << "UPDATE characters SET playerBytes3 = " << l_PlayerBytes3 << " WHERE guid = " << newguid;
                    l_CharTransaction->Append(l_UpdateQuery.str().c_str());
                }
            ...
            case DTT_GARR_MAIN:
            {
                std::string l_Level                     = getnth(l_Line, GetFieldIndexFromColumn("level", l_Columns) + 1);
                std::string l_BlueRecipes               = getnth(l_Line, GetFieldIndexFromColumn("blue_recipes", l_Columns) + 1);
                std::string l_Specializations           = getnth(l_Line, GetFieldIndexFromColumn("specializations", l_Columns) + 1);
                std::string l_NumFollowerActiv          = getnth(l_Line, GetFieldIndexFromColumn("num_follower_activation", l_Columns) + 1);
                std::string l_NumFollowerActivRegenTime = getnth(l_Line, GetFieldIndexFromColumn("num_follower_activation_regen_timestamp", l_Columns) + 1);
                std::string l_CacheLastUsage            = getnth(l_Line, GetFieldIndexFromColumn("cache_last_usage", l_Columns) + 1);

                std::ostringstream l_Query;
                l_Query << "INSERT INTO character_garrison (id, character_guid,  draenor_num_follower_activation, draenor_num_follower_activation_regen_timestamp) VALUES (";
                l_Query << l_NewGarrisonID << "," << newguid << "," << l_NumFollowerActiv << "," << l_NumFollowerActivRegenTime << ");";
                l_CharTransaction->Append(l_Query.str().c_str());

                l_Query.str("");
                l_Query.clear();
                l_Query << "INSERT INTO character_garrison_draenor (id, level, blue_recipes, specializations, cache_last_usage) VALUES (";
                l_Query << l_NewGarrisonID << "," << l_Level << ", '" << l_BlueRecipes << "','" << l_Specializations << "'," << l_CacheLastUsage << ");";
                l_CharTransaction->Append(l_Query.str().c_str());
            ...
            case DTT_GARR_CONTINENT:
            {
                ...
                if (!changenth(l_Line, l_Index, l_NewGarrisonID))
                {
                    sLog->outAshran("LoadDump: DUMP_FILE_BROKEN [DTT_GARR_CONTINENT]");
                    ROLLBACK(DUMP_FILE_BROKEN);
                }
                break;
            }
            ...
            case DTT_INVENTORY:
            {
                ...
                if (p_StartRealm && (p_StartRealm != g_RealmID))
                {
                    if (!(atoi(getnth(l_Line, l_Index).c_str())))
                    {
                        uint32 l_SlotIndex = GetFieldIndexFromColumn("slot", l_Columns) + 1;
                        uint32 l_Slot;
                        if ((l_Slot = atoi(getnth(l_Line, l_SlotIndex).c_str())) >= 39)
                        {
                            char l_NewSlot[5];
                            snprintf(l_NewSlot, 20, "%u", l_Slot + 8);
                            if (!changenth(l_Line, l_SlotIndex, l_NewSlot))
                            {
                                sLog->outAshran("LoadDump: DUMP_FILE_BROKEN [16.5]");
                                ROLLBACK(DUMP_FILE_BROKEN);
                            }
                        }
                    }
            ...
            case DTT_ITEM_TABLE:
            {
                ...
                uint32 l_Index = GetFieldIndexFromColumn("itemGuid", l_Columns) + 1;
                if (!changeGuid(l_Line, l_Index, items, sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM)))
                {
            ...
            case DTT_PET_TABLE: // pet_aura, pet_spell, pet_spell_cooldown
            {
                ...
                if (l_Line.find("pet_spell_cooldown") != std::string::npos)
                {
                    l_AllowedAppend = false;
                    break;
                }
            ...
            default:
                ...
                {
                    sLog->outExtChat("#jarvis", "danger", true, "Transfer to realm [%u] on account [%u] failed. Reason: Unknow table", g_RealmID, p_Account);
                    ROLLBACK(DUMP_FILE_BROKEN);
                }
                break;
        }
        ...
        if (l_AllowedAppend)
        {
            ...
            if (l_TableType == DTT_ACC_SPELL || l_TableType == DTT_ACC_ACH || l_TableType == DTT_ACC_ACH_PRO)
            {
                boost::replace_all(l_Line, "account_achievement", "account_legion_achievement");
                l_AccTransaction->Append(l_Line.c_str());
            }
            else
            {
                boost::replace_all(l_Line, "character_garrison_building", "character_garrison_draenor_building");
                boost::replace_all(l_Line, "character_garrison_daily_tavern_data", "character_garrison_draenor_daily_tavern_data");
                boost::replace_all(l_Line, "character_garrison_weekly_tavern_data", "character_garrison_draenor_weekly_tavern_data");
                l_CharTransaction->Append(l_Line.c_str());
            }
        }
    }
    ...
    if (p_TransactionID)
    {
        LoginDatabase.CommitTransaction(l_AccTransaction);

        MS::Utilities::CallBackPtr l_CallBack = std::make_shared<MS::Utilities::Callback>([p_TransactionID, p_Account](bool p_Success) -> void
        {
            if (!p_Success)
            {
                sLog->outExtChat("#jarvis", "danger", true, "Transfer to realm [%u] on account [%u] failed. Reason: Character transaction fail", g_RealmID, p_Account);
                LoginDatabase.PQuery("UPDATE webshop_delivery_interexp_transfer SET error = %u, state = 0, nb_attempt = nb_attempt + 1 WHERE id = %u", (uint32)DUMP_FILE_BROKEN, p_TransactionID);
                return;
            }

            LoginDatabase.PQuery("UPDATE webshop_delivery_interexp_transfer SET state = 2 WHERE id = %u", p_TransactionID);
        });

        CharacterDatabase.CommitTransaction(l_CharTransaction, sWorld->GetQueryCallbackMgr(), l_CallBack);
    }
    else
    {
