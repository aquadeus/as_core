...
bool WordFilterMgr::NormalizeWord(std::string& utf8String)
{
    // remove spaces
...
void WordFilterMgr::LoadLetterAnalogs()
{
    ...
    do
    {
        ...
        std::wstring _wanalogs;
        Utf8toWStr(analogs, _wanalogs);

        m_letterAnalogs[letter] = _wanalogs;
...
void WordFilterMgr::ConvertLettersToAnalogs(std::wstring& text)
{
    // named icons not available in 3.3.5, only rt*
    std::vector<std::wstring> icons =
    {
        L"{star}", L"{rt1}",
        L"{sircle}", L"{coin}", L"{rt2}",
        L"{diamond}", L"{rt3}",
        L"{triangle}", L"{rt4}",
        L"{moon}", L"{rt5}",
        L"{square}", L"{rt6}",
    ...
    for (std::wstring const& icon : icons)
    {
        std::wstring::size_type pos = text.find(icon);
        while (pos != std::wstring::npos)
        {
            text.erase(pos, icon.size());
            pos = text.find(icon, pos + 1);
        }
    }

    // todo: to change to STL algorithms (for optimize).
    for (std::wstring::iterator sit = text.begin(); sit != text.end(); ++sit)
        for (LetterAnalogMap::const_iterator mit = m_letterAnalogs.begin(); mit != m_letterAnalogs.end(); ++mit)
            if (mit->second.find(*sit) != std::wstring::npos)
            {
                *sit = mit->first;
                break;
            }
...
std::string WordFilterMgr::FindBadWord(const std::string& text)
{
    ...
    std::wstring _wtext;
    Utf8toWStr(_text, _wtext);

    ConvertLettersToAnalogs(_wtext);

    WStrToUtf8(_wtext, _text);
...
bool WordFilterMgr::AddBadWord(const std::string& badWord, bool toDB)
{
    ...
    std::wstring _wtext;
    Utf8toWStr(convertedBadWord, _wtext);

    ConvertLettersToAnalogs(_wtext);

    WStrToUtf8(_wtext, convertedBadWord);
