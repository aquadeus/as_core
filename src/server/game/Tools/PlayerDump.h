...
enum DumpTableType
{
    ...
    DTT_GARR_CONTINENT  ///< Garrison continent data tables
};
...
public:
    ...
    bool GetDump(uint32 guid, uint32 account, std::string& dump, uint32 p_Expansion);
    DumpReturn WriteDump(const std::string& file, uint32 guid, uint32 account, uint32 p_Expansion);

private:
    typedef std::set<uint32> GUIDs;

    bool DumpTable(std::string& dump, uint32 guid, uint32 account, char const*tableFrom, char const*tableTo, DumpTableType type, std::vector<std::string> const& columns, uint32 p_Expansion, uint32& p_GarrisonID);
...
public:
    ...
    DumpReturn LoadDump(const std::string& p_File, uint32 p_Account, std::string p_Name, uint32& p_Guid, bool p_OnlyBoundedItems = false, uint32 p_AtLogin = 1, uint32 p_TransactionId = 0, uint32 p_StartRealm = 0);
};
