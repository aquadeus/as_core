...
namespace MS { namespace Garrison
{
    ...
    void ShipmentManager::Init()
    {
        ...
        for (uint32 l_I = 0; l_I < sCharShipmentStore.GetNumRows(); ++l_I)
        {
            ...
            if (!l_ContainerEntry || (std::string(l_ContainerEntry->Name->Get(sWorld->GetDefaultDb2Locale())).empty() && std::string(l_ContainerEntry->Description->Get(sWorld->GetDefaultDb2Locale())).empty()))
                continue;
    ...
    uint32 ShipmentManager::GetShipmentIDForBuilding(uint32 p_BuildingID, Player* p_Target, bool p_ForStartWorkOrder)
    {
        ...
        if (l_BuildingType == GDraenor::Building::Type::TradingPost)
