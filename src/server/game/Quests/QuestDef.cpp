...
Quest::Quest(Field* p_QuestRecord)
{
    ...
    MaxScalingLevel         = p_QuestRecord[l_Index++].GetInt32();
    ...
    InProgressPhaseID       = p_QuestRecord[l_Index++].GetUInt32();
    CompletedPhaseID        = p_QuestRecord[l_Index++].GetUInt32();
    SuggestedPlayers        = p_QuestRecord[l_Index++].GetUInt8();
    LimitTime               = p_QuestRecord[l_Index++].GetUInt32();
    AreaGroupId             = p_QuestRecord[l_Index++].GetUInt32();
    RewardID                = p_QuestRecord[l_Index++].GetUInt32();
    Expansion               = p_QuestRecord[l_Index++].GetInt32();
    RequiredTeam            = p_QuestRecord[l_Index++].GetInt8();
    RequiredClasses         = p_QuestRecord[l_Index++].GetInt32();
    RequiredRaces           = p_QuestRecord[l_Index++].GetInt64();
    ...
    RewardXPMultiplier      = p_QuestRecord[l_Index++].GetFloat();
    RewardMoney             = p_QuestRecord[l_Index++].GetInt32();
    RewardMoneyDifficulty   = p_QuestRecord[l_Index++].GetUInt8();
    RewardMoneyMultiplier   = p_QuestRecord[l_Index++].GetFloat();
    RewardMoneyMaxLevel     = p_QuestRecord[l_Index++].GetUInt32();

    for (uint32 l_I = 0; l_I < QUEST_REWARD_SPELL_COUNT; ++l_I)
        RewardSpells[l_I] = p_QuestRecord[l_Index++].GetUInt32();
    ...
    RewardArtifactXP        = p_QuestRecord[l_Index++].GetUInt32();
    RewardArtifactXPMultiplier = p_QuestRecord[l_Index++].GetFloat();
    RewardArtifactXPDifficulty = p_QuestRecord[l_Index++].GetUInt32();
    ...
    RewardTitleId           = p_QuestRecord[l_Index++].GetUInt16();
    ...
    for (int i = 0; i < QUEST_REPUTATIONS_COUNT; ++i)
        RewardFactionCapIn[i] = p_QuestRecord[l_Index++].GetInt32();
    ...
    RewardArtifactXP = 0;
    RewardArtifactXPMultiplier = 0.0f;
    RewardArtifactXPDifficulty = 0;
...
uint32 Quest::XPValue(Player* player) const
{
    if (player)
    {
        int32 quest_level = Level;

        /// Quest scale with the player level
        if (quest_level == -1)
            quest_level = std::min((uint32)player->getLevel(), MaxScalingLevel);
...
uint32 Quest::MoneyValue(uint8 playerLevel) const
{
    ...
    /// Quest scale with the player level
    if (quest_level == -1)
        quest_level = std::min((uint32)playerLevel, MaxScalingLevel);

    if (QuestMoneyRewardEntry const* money = sQuestMoneyRewardStore.LookupEntry(quest_level))
...
uint32 Quest::GetRewMoneyMaxLevel() const
{
    ...
    if (HasFlag(QUEST_FLAGS_NO_MONEY_FROM_EXPERIENCE))
        return 0;
...
{
    return sWorld->getBoolConfig(CONFIG_QUEST_IGNORE_AUTO_COMPLETE) ? false : (Method == QUEST_METHOD_AUTO_COMPLETE/* || HasFlag(QUEST_FLAGS_AUTOCOMPLETE)*/);
}
...
uint32 Quest::CalculateHonorGain() const
{
    return RewardHonor;
}
