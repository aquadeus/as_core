...
namespace MS
{
    namespace Battlegrounds
    {
        ...
        {
        public:
            static void Status(WorldPacket* p_Data, Battleground* p_BG, Player* p_Player, uint8 p_QueueSlot, uint8 p_StatusID, uint32 p_Time1, uint32 p_Time2, uint8 /*p_Arenatype*/, bool p_IsSkirmish, bool p_IsRandom = false, bool p_AsGroup = false)
            {
                ...
                bool l_RegisteredMatch = p_BG && ((p_BG->isArena() && !p_BG->IsSkirmish()) || p_BG->IsRatedBG());
                ...
                if (p_IsRandom && p_BG)
                {
                    l_BGQueueID &= ~p_BG->GetTypeID();
                    l_BGQueueID |= BattlegroundTypeId::BATTLEGROUND_RB;
                }
                ...
                switch (p_StatusID)
                {
                    ...
                    case STATUS_WAIT_QUEUE:
                    {
                        ...
                        *p_Data << uint32(GetMSTimeDiffToNow(p_Time2));                           ///< Wait Time
                        ...
                        p_Data->WriteBit(p_AsGroup);                                              ///< As Group
            ...
            {
                int32 l_TheoricCount = 0;

                std::set<uint64> l_PlayerGuids;

                for (uint8 l_I = 0; l_I < TeamsCount::Value; ++l_I)
                {
                    if (GroupPtr l_Group = p_BG->GetBGRaid(l_I))
                    {
                        l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Member)
                        {
                            l_PlayerGuids.insert(l_Member->guid);
                            ++l_TheoricCount;
                        });
                    }
                }
                ...
                p_Data->Initialize(SMSG_PVP_LOG_DATA, 5 * 1024);
                ...
                for (Battleground::BattlegroundScoreMap::const_iterator l_It = l_ScoreBeginIT; l_It != p_BG->GetPlayerScoresEnd();)
                {
                    ...
                    l_PlayerGuids.erase(l_ScoreBeginIT->first);
                ...
                bool l_IsAlliance        = false;

                p_Data->WriteBit(l_HasRatings);                             ///< HaveArenaData
                p_Data->WriteBit(p_BG->GetStatus() == STATUS_WAIT_LEAVE);   ///< If Ended
                p_Data->FlushBits();

                *p_Data << uint32(l_TheoricCount);

                *p_Data << uint8(l_HordePlayerCount ? l_HordePlayerCount : (l_TheoricCount - l_PlayersCount));
                *p_Data << uint8(l_AlliancePlayerCount ? l_AlliancePlayerCount : (l_TheoricCount - l_PlayersCount));
                ...
                if (l_HasRatings)
                {
                    ...
                    for (int8 l_I = (TeamsCount::Value - 1); l_I >= 0 ; --l_I)
                    {
                ...
                for (Battleground::BattlegroundScoreMap::const_iterator l_It = l_ScoreBeginIT; l_It != p_BG->GetPlayerScoresEnd();)
                {
                    ...
                    switch (p_BG->GetTypeID(true))
                    {
                        case BATTLEGROUND_RB:
                            switch (p_BG->GetMapId())
                            {
                                ...
                                case 566:
                                    ...
                                    break;
                                case 529:
                                    l_Buffer << uint32(((BattlegroundABScore*)l_ScoreBeginIT->second)->BasesAssaulted);       ///< bases assaulted
                                ...
                                case 607:
                                    ...
                                    break;
                                case 628:                                                                                     ///< IC
                                    l_Buffer << uint32(((BattlegroundICScore*)l_ScoreBeginIT->second)->BasesAssaulted);       ///< bases assaulted
                                ...
                                case 726:
                                    ...
                                    break;
                                case 761:
                                    l_Buffer << uint32(((BattlegroundBFGScore*)l_ScoreBeginIT->second)->BasesAssaulted);      ///< bases assaulted
                                    l_Buffer << uint32(((BattlegroundBFGScore*)l_ScoreBeginIT->second)->BasesDefended);       ///< bases defended
                                    break;
                                case 1803:
                                    l_Buffer << uint32(((BattlegroundSSScore*)l_ScoreBeginIT->second)->MinedAzerite);         ///< Mined Azerite Nodes in Points value
                                    break;
                            }
                            break;
                        ...
                        case BATTLEGROUND_SEETHING_SHORE:
                            l_Buffer << uint32(((BattlegroundSSScore*)l_ScoreBeginIT->second)->MinedAzerite);                 ///< Azerite mined
                            break;
                        default:
                            break;
                    }
                    ...
                    *p_Data << int32(l_Player->GetActiveSpecializationID());                        ///< PrimaryTalentTree
                    *p_Data << int32(l_Player->getGender());                                        ///< Gender
                    *p_Data << int32(l_Player->getRace());                                          ///< Race
                    *p_Data << uint32(l_Player->GetPrestige());                                     ///< Prestige rang

                    p_Data->append(l_Buffer);                                                       ///< Stats

                    l_IsAlliance = ((l_IsArena && !l_IsSkirmish) || l_IsRatedBg) ? p_BG->GetPlayerTeam(l_Player->GetGUID()) == ALLIANCE : l_Player->GetTeam() == ALLIANCE;

                    p_Data->WriteBit(l_IsAlliance);                                                 ///< Faction
                    ...
                    if (l_HasHonor)
                    {
                        ...
                        *p_Data << uint32(l_ScoreBeginIT->second->BonusHonor);                      ///< Contribution Points
                    }
                ...
                /// Force the build of scoreboard of dodging players
                if (!l_PlayerGuids.empty())
                {
                    for (uint64 const& l_Guid : l_PlayerGuids)
                    {
                        Player* l_Player = ObjectAccessor::FindPlayerInOrOutOfWorld(l_Guid);
                        if (!l_Player)
                            continue;

                        uint32 l_LooserTeam = l_Player->GetBGTeam();

                        p_Data->appendPackGUID(l_Guid);                                                 ///< Player GUID

                        *p_Data << uint32(0);                                                           ///< Killing Blows
                        *p_Data << uint32(0);                                                           ///< Damage done
                        *p_Data << uint32(0);                                                           ///< Healing Done
                        *p_Data << uint32(0);                                                           ///< Stats count
                        *p_Data << int32(l_Player->GetActiveSpecializationID());                        ///< PrimaryTalentTree
                        *p_Data << uint32(0);                                                           ///< PrimaryTalentTreeNameIndex
                        *p_Data << int32(l_Player->getGender());                                        ///< Gender
                        *p_Data << int32(l_Player->getRace());                                          ///< Race
                        *p_Data << uint32(l_Player->GetPrestige());                                     ///< Prestige rang
                        ...
                        if (l_HasHonor)
                        {
                            *p_Data << uint32(0);                                                       ///< Honor Kills
                            *p_Data << uint32(0);                                                       ///< Deaths
                            *p_Data << uint32(0);                                                       ///< Contribution Points
                        }
                        ...
                        if (l_HasRatingChange)
                            *p_Data << int32(p_BG->GetArenaTeamRatingChangeByIndex(p_BG->GetTeamIndexByTeamId(l_LooserTeam)));
            ...
            static void List(WorldPacket* p_Data, uint64 p_GUID, Player* p_Player, BattlegroundTypeId p_BGTypeID, uint32 p_BattlemasterListID, bool p_HasWinToday)
            {
                if (!p_Player)
                    return;
                ...
                *p_Data << uint8(l_Bracket ? l_Bracket->m_MinLevel : MAX_LEVEL); ///< MinLevel
                *p_Data << uint8(l_Bracket ? l_Bracket->m_MaxLevel : MAX_LEVEL); ///< MaxLevel
                *p_Data << uint32(l_BattleFields.size());

                for (uint32 l_BattleField : l_BattleFields)
                    *p_Data << uint32(l_BattleField);

                p_Data->WriteBit(1);                    ///< Enabled
                p_Data->WriteBit(p_HasWinToday);
                p_Data->FlushBits();
            }
