...
void Battleground::Update(uint32 diff)
{
    ...
    switch (GetStatus())
    {
        case STATUS_WAIT_JOIN:
            ...
            {
                _ProcessJoin(diff);
                //CheckSafePositions(diff);  disable, use check for every bg in PostUpdateImpl
            }
            break;
...
void Battleground::CheckSafePositions(uint32 p_Diff)
{
    float l_MaxDist = GetStartMaxDist();
    if (!l_MaxDist)
        return;

    m_ValidStartPositionTimer += p_Diff;
    ...
    if (m_ValidStartPositionTimer >= CHECK_PLAYER_POSITION_INVERVAL)
    {
        ...
        for (BattlegroundPlayerMap::const_iterator l_Iter = GetPlayers().begin(); l_Iter != GetPlayers().end(); ++l_Iter)
        {
            if (Player* l_Player = ObjectAccessor::FindPlayer(l_Iter->first))
            {
                ...
                GetTeamStartLoc(l_Player->GetBGTeam(), l_X, l_Y, l_Z, l_O);
                if (l_Pos.GetExactDistSq(l_X, l_Y, l_Z) > (l_MaxDist * l_MaxDist))
                {
                    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground::CheckSafePositions: Sending %s back to start location (map: %u) (possible exploit)", l_Player->GetName(), GetMapId());
                    l_Player->TeleportTo(GetMapId(), l_X, l_Y, l_Z, l_O);
                }
            }
        }
    }
...
void Battleground::RemoveFromInterRealm(uint64 p_PlayerGuid)
{
    if (Player* l_Player = sObjectAccessor->FindPlayerInOrOutOfWorld(p_PlayerGuid))
    {
        ...
        {
            if ((!isArena() && GetStatus() == STATUS_IN_PROGRESS) || GetStatus() == STATUS_WAIT_JOIN)
                l_Player->CastSpell(l_Player, MS::Battlegrounds::Spells::DeserterBuff, true);
        }

        l_Player->RemoveFromGroup();
        ...
        l_Player->SaveToDB(false, false, l_CallBack);
    }
 }
#endif
...
inline void Battleground::_ProcessRessurect(uint32 diff)
{
    ...
    else if (m_LastResurrectTime > 500)    // Resurrect players only half a second later, to see spirit heal effect on NPC
    {
        for (std::vector<uint64>::const_iterator itr = m_ResurrectQueue.begin(); itr != m_ResurrectQueue.end(); ++itr)
        {
            ...
            if (!player->HasAura(155228)) ///Lone Wolf disable it
                player->CastSpell(player, 6962, true);

            player->CastSpell(player, SPELL_SPIRIT_HEAL_MANA, true);
            player->GetMap()->ConvertCorpseToBones(*itr);
        }
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        if (isArena())
        {
            for (BattlegroundPlayerMap::const_iterator l_Iterator = GetPlayers().begin(); l_Iterator != GetPlayers().end(); ++l_Iterator)
            {
                ...
                {
                    l_Player->ModifyAuraState(AURA_STATE_PVP_RAID_PREPARE, false);

                    /// May cause players to be invisible in some cases (update object fail from the client)
                    UpdateData l_UpdateData(GetMapId());
                    WorldPacket l_Packet;

                    l_Player->BuildValuesUpdateBlockForPlayer(&l_UpdateData, l_Player);
                    if (l_UpdateData.HasData() && l_UpdateData.BuildPacket(&l_Packet))
            ...
            for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
            {
                if (Player* l_Player = ObjectAccessor::FindPlayer(itr->first))
                {
                    ...
                    l_Player->SetByteValue(PLAYER_FIELD_ARENA_FACTION, PLAYER_BYTES_4_OFFSET_ARENA_FACTION, l_Player->GetBGTeam());
                    l_Player->RemoveAurasDueToSpell(SPELL_ARENA_PREPARATION);
                    l_Player->ResetAllPowers();

                    WorldPacket team_packet(SMSG_ARENA_PREP_OPPONENT_SPECIALIZATIONS);
...
inline void Battleground::_ProcessLeave(uint32 diff)
{
    ...
    if (m_EndTime <= 0)
    {
        ...
        if (m_InvitedHorde != 0 || m_InvitedAlliance != 0)
        {
            sLog->outAshran("Battleground::_ProcessLeave: All invited count wasn't at 0");
            m_InvitedHorde = 0;
            m_InvitedAlliance = 0;
        }
    }
...
void Battleground::RewardHonorToTeam(uint32 p_Honor, uint32 TeamID, MS::Battlegrounds::RewardCurrencyType::Type p_RewardCurrencyType /* = None */)
{
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        if (Player* l_Player = _GetPlayerForTeam(TeamID, itr, "RewardHonorToTeam"))
            UpdatePlayerScore(l_Player, nullptr, SCORE_BONUS_HONOR, p_Honor, true, p_RewardCurrencyType);
    }
}

void Battleground::RewardTeamWithAdditionalRewards(bool p_Winner, uint32 TeamID)
{
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        if (Player* l_Player = _GetPlayerForTeam(TeamID, itr, "RewardTeamWithAdditionalRewards"))
            RewardPlayerWithOtherRewards(l_Player, p_Winner);
    }
}
...
{
    GroupPtr winner_team              = nullptr;
    GroupPtr loser_team               = nullptr;
    ...
    if (IsRatedBG())
    {
        loser_matchmaker_rating = GetArenaMatchmakerRating(GetOtherTeam(p_Winner), (uint8)BattlegroundBracketType::Battleground10v10);
        winner_matchmaker_rating = GetArenaMatchmakerRating(p_Winner, (uint8)BattlegroundBracketType::Battleground10v10);
        ...
        if (!isArena())
        {
            ...
            {
                uint32 l_Team = l_Itr->second.Team;
                ...
                {
                    uint8 l_Bracket      = (uint8)BattlegroundBracketType::Battleground10v10;
                    int32 l_RatingChange = 0;
                    int32 l_MMRating_mod = 0;

                    if (l_Player->GetArenaPersonalRating(l_Bracket) < l_Player->GetArenaMatchMakerRating(l_Bracket))
                    {
                        l_RatingChange = Arena::GetRatingMod(l_Player->GetArenaPersonalRating(l_Bracket), l_Team == p_Winner ? loser_matchmaker_rating : winner_matchmaker_rating, l_Team == p_Winner, true);
                        l_Player->SetArenaPersonalRating(l_Bracket, std::max(0, (int)l_Player->GetArenaPersonalRating(l_Bracket) + l_RatingChange));
                    }

                    if (l_Team == p_Winner)
                    {
                        l_MMRating_mod = Arena::GetMatchmakerRatingMod(winner_matchmaker_rating, loser_matchmaker_rating, true);
                        l_Player->SetArenaMatchMakerRating(l_Bracket, l_Player->GetArenaMatchMakerRating(l_Bracket) + l_MMRating_mod);

                        l_Player->IncrementWeekWins(l_Bracket);
                        l_Player->IncrementSeasonWins(l_Bracket);
                        l_Player->IncrementWeekGames(l_Bracket);
                        l_Player->IncrementSeasonGames(l_Bracket);
                    }
                    else
                    {
                        l_MMRating_mod = Arena::GetMatchmakerRatingMod(loser_matchmaker_rating, winner_matchmaker_rating, false);
                        l_Player->SetArenaMatchMakerRating(l_Bracket, std::max(0, (int)l_Player->GetArenaMatchMakerRating(l_Bracket) + l_MMRating_mod));
                        l_Player->IncrementWeekGames(l_Bracket);
                        l_Player->IncrementSeasonGames(l_Bracket);
                    }

                    for (auto const& l_Itr : PlayerScores)
                    {
                        if (l_Itr.first == l_Player->GetObjectGuid())
                        {
                            l_Itr.second->m_RatingChange = l_RatingChange;
                            l_Itr.second->m_MmrRatingChange = l_MMRating_mod;
                        }
                    }
                }
                ...
                {
                    uint32 l_GuildID = l_Group->GetGroupGuildID();
                    if (l_Group->IsGuildGroup() && l_GuildID == l_Player->GetGuildId())
                    {
                        if (Guild* l_Guild = l_Player->GetGuild())
                        {
                            l_Guild->CompleteGuildChallenge(ChallengeRatedBG, l_Player);
                            break;
                        }
                    }
                }
    ...
    {
        uint8 slot = (uint8)GetBattlegroundBracketType();
        ...
        if (GetWinner() != 3)
        {
            WinnerRatingChanges(winner_matchmaker_rating, loser_matchmaker_rating, winner_change, winner_matchmaker_change, slot, p_Winner);
            LooserRatingChanges(loser_matchmaker_rating, winner_matchmaker_rating, loser_change, loser_matchmaker_change, slot, p_Winner);
        ...
        /// Handle arena rating decay system
        /*if (slot == uint8(BattlegroundBracketType::Arena2v2) || slot == uint8(BattlegroundBracketType::Arena3v3))
        {
            if (winner_team)
                winner_team->HandleArenaDecay(slot);

            if (loser_team)
                loser_team->HandleArenaDecay(slot);
        }*/
    }
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (itr->second.OfflineRemoveTime)
        {
            ...
            {
                if (l_Team == p_Winner)
                    winner_team->OfflineMemberLost(itr->first, loser_matchmaker_rating, (uint8)GetBattlegroundBracketType(), winner_matchmaker_change);
                else
                    loser_team->OfflineMemberLost(itr->first, winner_matchmaker_rating, (uint8)GetBattlegroundBracketType(), loser_matchmaker_change);
            }
            continue;
        }
        ...
        if (winner_team && loser_team && winner_team != loser_team && GetWinner() != 3)
        {
            if (isArena() && !IsSkirmish() && !IsWargame())
            {
                ...
                if (l_Team == p_Winner)
                {
                    ...
                    l_Player->UpdateCriteria(CRITERIA_TYPE_WIN_RATED_ARENA, rating ? rating : 1);
                    l_Player->UpdateCriteria(CRITERIA_TYPE_WIN_ARENA, GetMapId());
                }
                else
                {
                    // Member lost
                    // Arena lost => reset the win_rated_arena having the "no_lose" condition
                    l_Player->ResetCriteria(CRITERIA_TYPE_WIN_RATED_ARENA, CRITERIA_CONDITION_NO_LOSE);
                }
            }
            else if (IsRatedBG())
                l_Player->UpdateCriteria(CRITERIA_TYPE_WIN_RATED_BATTLEGROUND, GetMapId());
        }
        ...
        if (IsRatedBG() && winner_team && loser_team && winner_team != loser_team && GetWinner() != 3)
        {
            ...
            if (l_Player->GetArenaPersonalRating(l_Bracket) < l_Player->GetArenaMatchMakerRating(l_Bracket))
            {
                int32 l_RatingChange = Arena::GetRatingMod(l_Player->GetArenaPersonalRating(l_Bracket), l_Team == p_Winner ? loser_matchmaker_rating : winner_matchmaker_rating, l_Team == p_Winner, true);
                l_Player->SetArenaPersonalRating(l_Bracket, std::max(0, (int)l_Player->GetArenaPersonalRating(l_Bracket) + l_RatingChange));
            }
            ...
            if (l_Team == p_Winner)
            {
                ...
                l_Player->SetArenaMatchMakerRating(l_Bracket, l_Player->GetArenaMatchMakerRating(l_Bracket) + MMRating_mod);

                l_Player->IncrementDayWins(l_Bracket);
                l_Player->IncrementWeekWins(l_Bracket);
                l_Player->IncrementSeasonWins(l_Bracket);
                l_Player->IncrementDayGames(l_Bracket);
                l_Player->IncrementWeekGames(l_Bracket);
                l_Player->IncrementSeasonGames(l_Bracket);
            }
            else
            {
                ...
                l_Player->SetArenaMatchMakerRating(l_Bracket, std::max(0, (int)l_Player->GetArenaMatchMakerRating(l_Bracket) + MMRating_mod));
                l_Player->IncrementDayGames(l_Bracket);
                l_Player->IncrementWeekGames(l_Bracket);
                l_Player->IncrementSeasonGames(l_Bracket);
            }
        ...
        if (l_Team == p_Winner)
        {
            l_Player->UpdateCriteria(CRITERIA_TYPE_WIN_BG, 1);

            // 44173 45173 quests completion
            if (!isArena())
            {
                l_Player->QuestObjectiveSatisfy(53035, 1, QUEST_OBJECTIVE_TYPE_CRITERIA_TREE, l_Player->GetGUID());
                l_Player->QuestObjectiveSatisfy(55665, 1, QUEST_OBJECTIVE_TYPE_CRITERIA_TREE, l_Player->GetGUID());
            }
            /// The Arena Calls completion
            if (isArena() && IsSkirmish())
                l_Player->QuestObjectiveSatisfy(53033, 1, QUEST_OBJECTIVE_TYPE_CRITERIA_TREE, l_Player->GetGUID());
            ...
            if (!guildAwarded)
            {
                ...
                if (uint32 guildId = GetBgMap()->GetOwnerGuildId(l_Player->GetTeam()))
                    ...
                    {
                        guild->GetAchievementMgr().UpdateCriteria(CRITERIA_TYPE_WIN_BG, 1, 0, 0, nullptr, l_Player);
                        if (isArena() && !IsSkirmish() && winner_team && loser_team && winner_team != loser_team)
                            guild->GetAchievementMgr().UpdateCriteria(CRITERIA_TYPE_WIN_RATED_ARENA, std::max<uint32>(winner_team->GetRating((uint8)GetBattlegroundBracketType()), 1), 0, 0, nullptr, l_Player);
                    }
            }
#endif
        }
        ...
        if (!IsWargame())
            l_Player->UpdateCriteria(CRITERIA_TYPE_COMPLETE_BATTLEGROUND, 1);
...
uint32 Battleground::GetBonusHonorFromKill(uint32 kills) const
{
    ...
    uint32 maxLevel = std::min(GetMaxLevel(), (uint32)MAX_LEVEL);
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
    ...
    if (player)
    {
        if (!isArena() && GetStatus() == STATUS_IN_PROGRESS)
            player->CastSpell(player, MS::Battlegrounds::Spells::DeserterBuff, true);

        if (player->HasAuraType(SPELL_AURA_SPIRIT_OF_REDEMPTION))
            player->RemoveAurasByType(SPELL_AURA_MOD_SHAPESHIFT);

        if (player->HasAuraType(SPELL_AURA_SWITCH_TEAM))
            player->RemoveAurasByType(SPELL_AURA_SWITCH_TEAM);

        if (player->HasAuraType(SPELL_AURA_MOD_FACTION))
        ...
        ChrSpecializationsEntry const* l_SpecEntry = sChrSpecializationsStore.LookupEntry(player->GetBGLastActiveSpec());

        if (l_SpecEntry)
            player->ActivateTalentGroup(l_SpecEntry);
    }
    ...
    if (participant) // if the player was a match participant, remove auras, calc rating, update queue
    {
        if (player)
        {
            ...
            if (isArena())
            {
                ...
                if (!IsSkirmish() && GetStatus() && GetStatus() != STATUS_WAIT_LEAVE)
                {
                    //left a rated match while the encounter was in progress, consider as loser
                    GroupPtr winner_group = GetBgRaid(GetOtherTeam(team));
                    GroupPtr loser_group = GetBgRaid(team);
        ...
        // removing offline participant
        {
            ...
            {
                //left a rated match while the encounter was in progress, consider as loser
                GroupPtr others_group = GetBgRaid(GetOtherTeam(team));
                GroupPtr players_group = GetBgRaid(team);
                if (others_group && players_group)
                    players_group->OfflineMemberLost(guid, GetArenaMatchmakerRating(GetOtherTeam(team), (uint8)GetBattlegroundBracketType()), (uint8)GetBattlegroundBracketType());
            }
        }
    ...
    if (player)
    {
        ...
        player->SetByteValue(PLAYER_FIELD_ARENA_FACTION, PLAYER_BYTES_4_OFFSET_ARENA_FACTION, 0);
...
void Battleground::BuildArenaOpponentSpecializations(WorldPacket* p_Packet, uint32 p_Team)
{
    ...
    *p_Packet << uint32(l_OpponentCount);
    ...
    for (BattlegroundPlayerMap::iterator l_It = m_Players.begin(); l_It != m_Players.end(); ++l_It)
    {
        ...
        {
            *p_Packet << uint32(l_Player->GetActiveSpecializationID());
            *p_Packet << uint32(0);
...
void Battleground::SendArenaCrowdControlSpells(Player* p_Requester, Player* p_Requested)
{
    PvpTalentEntry const* l_TalentEntry = sPvpTalentStore.LookupEntry(p_Requested->GetPvPTalentAtTier(0));
    if (!l_TalentEntry)
        return;

    WorldPacket l_Data(Opcodes::SMSG_ARENA_CROWD_CONTROL_SPELLS, 13);

    l_Data.appendPackGUID(p_Requested->GetGUID());
    l_Data << uint32(l_TalentEntry->m_SpellID);

    p_Requester->GetSession()->SendPacket(&l_Data);
}
...
void Battleground::AddPlayer(Player* player)
{
    ...
    ///< Clear debuffs such Cheated Death, Demonic Gateway etc... on enter BG or arena
    player->CastSpell(player, 171099);
    ...
    if (isArena())
    {
        ...
        player->SetByteValue(PLAYER_FIELD_ARENA_FACTION, PLAYER_BYTES_4_OFFSET_ARENA_FACTION, player->GetBGTeam());
        ...
        team_packet.Initialize(SMSG_ARENA_PREP_OPPONENT_SPECIALIZATIONS);
    ...
    else
    {
        ...
        if (player->HasAura(MercenarySpells::MercenaryContractHorde))
        {
            player->CastSpell(player, MercenarySpells::MercenaryHorde1, true);
            player->CastSpell(player, MercenarySpells::MercenaryHorde2, true);
            player->RemoveAura(MercenarySpells::MercenaryContractHorde);
        }
        else if (player->HasAura(MercenarySpells::MercenaryContractAlliance))
        {
            player->CastSpell(player, MercenarySpells::MercenaryAlliance1, true);
            player->CastSpell(player, MercenarySpells::MercenaryAlliance2, true);
            player->RemoveAura(MercenarySpells::MercenaryContractAlliance);
        }
    }

    std::vector<CriteriaTypes> l_CriteriaTypesToReset =
    {
        CRITERIA_TYPE_KILL_CREATURE, CRITERIA_TYPE_WIN_BG, CRITERIA_TYPE_DAMAGE_DONE, CRITERIA_TYPE_BE_SPELL_TARGET,
        CRITERIA_TYPE_CAST_SPELL, CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, CRITERIA_TYPE_HONORABLE_KILL_AT_AREA,
        CRITERIA_TYPE_HONORABLE_KILL, CRITERIA_TYPE_HEALING_DONE, CRITERIA_TYPE_GET_KILLING_BLOWS, CRITERIA_TYPE_SPECIAL_PVP_KILL,
        CRITERIA_TYPE_BE_SPELL_TARGET2
    };

    for (auto l_Type : l_CriteriaTypesToReset)
        player->ResetCriteria(l_Type, CRITERIA_CONDITION_BG_MAP, GetMapId(), true);
...
PlayerData::PlayerData(Player* p_Player, uint32 p_TeamID) :
    GuidLow(p_Player->GetGUIDLow()), Name(p_Player->GetName()), TeamID(p_TeamID)
{
    memset(&Rating, 0, sizeof (Rating));
    memset(&MMR, 0, sizeof (MMR));

    for (uint8 l_I = 0; l_I < uint8(BattlegroundBracketType::MaxRated); ++l_I)
    {
        Rating[l_I] = p_Player->GetArenaPersonalRating(l_I);
        MMR[l_I] = p_Player->GetArenaMatchMakerRating(l_I);
    }

#ifdef CROSS
    RealmID = p_Player->GetSession()->GetInterRealmNumber();
#else
    RealmID = g_RealmID;
#endif
}

void Battleground::RegisterPlayer(Player* p_Player, uint32 p_Team)
{
    m_PlayerData[p_Player->GetGUID()] = PlayerData(p_Player, p_Team);
}

PlayerData const* Battleground::GetPlayerData(uint64 p_Guid)
{
    auto const& l_Iter = m_PlayerData.find(p_Guid);
    if (l_Iter == m_PlayerData.end())
        return nullptr;

    return &l_Iter->second;
}

void Battleground::WinnerRatingChanges(uint32 p_OwnMMR, uint32 p_OpponentMMR, int32& p_RatingChange, int32& p_MMRChange, uint8 p_Bracket, uint32 p_Winner)
{
    /// Called when the team has won
    /// Change in Matchmaker rating
    p_MMRChange = Arena::GetMatchmakerRatingMod(p_OwnMMR, p_OpponentMMR, true);

    uint32 l_TeamRating = GetTeamPersonalRating(p_Winner, p_Bracket);
    uint32 l_OpponentRating = GetTeamPersonalRating(GetOtherTeam(p_Winner), p_Bracket);

    for (auto const& l_Iter : m_PlayerData)
    {
        if (l_Iter.second.TeamID != p_Winner)
            continue;

        uint32 l_PersonalRating = l_Iter.second.GetPersonalRating(p_Bracket);

        /// Weighting team rating and personal rating at each 50%
        uint32 l_RatingUsed = CalculatePct(l_TeamRating, 50) + CalculatePct(l_PersonalRating, 50);

        /// In 2v2, if player is above 2200 rating and is above his team mate rating by 700 or more, only uses his personal rating
        if (p_Bracket == uint8(BattlegroundBracketType::Arena2v2) && l_PersonalRating >= 2200)
        {
            uint32 l_MateRating = 0;

            /// Get personal rating of the team mate
            for (auto const& l_OtherIter : m_PlayerData)
            {
                if (l_OtherIter.second.TeamID != p_Winner || l_OtherIter.first == l_Iter.first)
                    continue;

                l_MateRating = l_OtherIter.second.GetPersonalRating(p_Bracket);
            }

            if (l_PersonalRating >= (l_MateRating + 700))
                l_RatingUsed = l_PersonalRating;
        }

        /// Change in Team Rating
        p_RatingChange = Arena::GetRatingMod(l_RatingUsed, l_OpponentRating, true);

        if (l_PersonalRating < 1000)
            p_RatingChange = 96;

        if (Player* l_Player = ObjectAccessor::FindPlayer(l_Iter.first))
        {
            if (l_Player->GetBattleground())
            {
                for (auto const& l_Score : l_Player->GetBattleground()->GetPlayerScores())
                {
                    if (l_Score.first == l_Iter.first)
                    {
                        l_Score.second->m_RatingChange = p_RatingChange;
                        l_Score.second->m_MmrRatingChange = p_MMRChange;
                    }
                }
            }

            l_Player->SetArenaPersonalRating(p_Bracket, l_Player->GetArenaPersonalRating(p_Bracket) + p_RatingChange);
            l_Player->SetArenaMatchMakerRating(p_Bracket, l_Player->GetArenaMatchMakerRating(p_Bracket) + p_MMRChange);

            l_Player->IncrementWeekWins(p_Bracket);
            l_Player->IncrementSeasonWins(p_Bracket);
            l_Player->IncrementWeekGames(p_Bracket);
            l_Player->IncrementSeasonGames(p_Bracket);
        }
    }
}

void Battleground::LooserRatingChanges(uint32 p_OwnMMR, uint32 p_OpponentMMR, int32& p_RatingChange, int32& p_MMRChange, uint8 p_Bracket, uint32 p_Winner, uint64 p_OnlyGuid /*= 0*/)
{
    /// Called when the team has lost
    /// Change in Matchmaker Rating
    p_MMRChange = Arena::GetMatchmakerRatingMod(p_OwnMMR, p_OpponentMMR, false);

    uint32 l_TeamRating = GetTeamPersonalRating(GetOtherTeam(p_Winner), p_Bracket);
    uint32 l_OpponentRating = GetTeamPersonalRating(p_Winner, p_Bracket);

    for (auto const& l_Iter : m_PlayerData)
    {
        if (l_Iter.second.TeamID != GetOtherTeam(p_Winner))
            continue;

        /// Handling for a specific player (dodging queue or leaving before end of match)
        if (p_OnlyGuid != 0 && l_Iter.first != p_OnlyGuid)
            continue;
        ...
        if (p_Bracket == uint8(BattlegroundBracketType::Arena2v2) && l_PersonalRating >= 2200)
        {
            ...
            for (auto const& l_OtherIter : m_PlayerData)
            {
                if (l_OtherIter.second.TeamID != GetOtherTeam(p_Winner) || l_OtherIter.first == l_Iter.first)
                    continue;
        ...
        /// Change in Team Rating
        p_RatingChange = Arena::GetRatingMod(l_RatingUsed, l_OpponentRating, false);

        if (Player* l_Player = ObjectAccessor::FindPlayerInOrOutOfWorld(l_Iter.first))
        {
            if (l_Player->GetBattleground())
            {
                for (auto const& l_Score : l_Player->GetBattleground()->GetPlayerScores())
            ...
            l_Player->SetArenaPersonalRating(p_Bracket, std::max(0, (int)l_Player->GetArenaPersonalRating(p_Bracket) + p_RatingChange));
            l_Player->SetArenaMatchMakerRating(p_Bracket, std::max(0, (int)l_Player->GetArenaMatchMakerRating(p_Bracket) + p_MMRChange));
...
void Battleground::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardCurrencyType)
{
    ...
    switch (p_Type)
    {
        ...
        case SCORE_HONORABLE_KILLS:                         // Honorable kills
        {
            ...
            /// BONUS_HONOR is called from Player::RewardHonor
        ...
        case SCORE_BONUS_HONOR:                             // Honor bonus
            ...
            if (p_RewardCurrencyType == MS::Battlegrounds::RewardCurrencyType::BattlegroundWin && !p_Value && p_DoAddHonor && (!isBattleground() || p_Source->IsInRandomBG() || IsRatedBG()))
            {
                BattlegroundBracketType l_Bracket = GetBattlegroundBracketType();
                if (l_Bracket != BattlegroundBracketType::Max)
                {
                    p_Value = g_HonorRewardPerBracket[(uint8)l_Bracket];

                    if (!p_Source->HasWinInBracket(l_Bracket, true))
                        p_Value *= 2;
                }
...
BattlegroundBracketType Battleground::GetBattlegroundBracketType() const
{
    if (IsRatedBG())
        return BattlegroundBracketType::Battleground10v10;

    if (IsSkirmish())
        return BattlegroundBracketType::ArenaSkirmish;

    if (GetArenaType() == ArenaType::Arena1v1)
        return BattlegroundBracketType::Arena1v1;

    if (GetArenaType() == ArenaType::Arena2v2)
        return BattlegroundBracketType::Arena2v2;

    if (GetArenaType() == ArenaType::Arena3v3)
        return BattlegroundBracketType::Arena3v3;

    return BattlegroundBracketType::RandomBattleground;
}

void Battleground::RewardPlayerWithOtherRewards(Player* p_Source, bool p_Winner)
{
    if (IsWargame() || !p_Source->IsMaxLevel())
        return;

    if (p_Winner)
    {
        /// Since 7.1, a piece of gear is rewarded at every victorious battleground / rated battleground
        /// In arena, there is a chance to get a piece of gear. The chance is higher in rated arena than in skrimish
        /// The ilvl of the reward is based on your current ilvl, like the world quest
        bool l_RewardGear = isBattleground() || roll_chance_i(20);

        if (l_RewardGear)
        {
            if (ItemTemplate const* l_GearReward = sObjectMgr->GetItemRewardForPvp(p_Source->GetActiveSpecializationID(), IsRated() ? MS::Battlegrounds::RewardGearType::Rated : MS::Battlegrounds::RewardGearType::Regular, p_Source, InventoryType::INVTYPE_ROBE, PvpSeasons::CurrentSeason))
            {
                ItemPosCountVec sDest;
                InventoryResult msg = p_Source->CanStoreNewItem(INVENTORY_SLOT_BAG_0, NULL_SLOT, sDest, l_GearReward->ItemId, 1);

                if (msg == EQUIP_ERR_OK)
                {
                    std::vector<uint32> l_ItemBonuses;
                    std::vector<uint32> l_Dummy;
                    Item::GenerateItemBonus(l_GearReward->ItemId, static_cast<ItemContext>(p_Source->GetWorldQuestManager()->GenerateItemContext(MS::WorldQuest::WorldQuestType::DailyPvP, l_GearReward, l_Dummy, GetBattlegroundBracketType())), l_ItemBonuses, true, p_Source);

                    if (Item* l_AddedItem = p_Source->StoreNewItem(sDest, l_GearReward->ItemId, true, Item::GenerateItemRandomPropertyId(l_GearReward->ItemId), l_ItemBonuses))
                    {
                        p_Source->SendNewItem(l_AddedItem, 1, true, false);
                        p_Source->SendDisplayToast(l_GearReward->ItemId, 1, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false, l_AddedItem->GetAllItemBonuses());
                    }
                }
            }
        }

        /// Victorious battleground/arena, rated or not should also give a artifact power item.
        /// You get a better item for the first of the day for every bracket
        /// Taken from Blizzard_PVPUI.lua
        static std::unordered_map<uint8, std::array<uint32, 2>> g_ArtifactPowerRewards =
        {
            { (uint8)BattlegroundBracketType::Arena1v1,            { {143714, 138865} } },
            { (uint8)BattlegroundBracketType::Arena2v2,            { {143714, 138865} } },
            { (uint8)BattlegroundBracketType::Arena3v3,            { {143715, 143677} } },
            { (uint8)BattlegroundBracketType::RandomBattleground,  { {143680, 138880} } },
            { (uint8)BattlegroundBracketType::ArenaSkirmish,       { {143713, 138864} } },
            { (uint8)BattlegroundBracketType::Battleground10v10,   { {143716, 138881} } }
        };

        BattlegroundBracketType l_Bracket = GetBattlegroundBracketType();
        if (l_Bracket != BattlegroundBracketType::Max && (!isBattleground() || p_Source->IsInRandomBG() || IsRatedBG()))
        {
            uint32 l_ArtifactItemId = g_ArtifactPowerRewards[(uint8)l_Bracket][p_Source->HasWinInBracket(l_Bracket, true)];

            p_Source->AddItem(l_ArtifactItemId, 1);
            p_Source->SendDisplayToast(l_ArtifactItemId, 1, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
        }

        /// First random daily battleground win awards 30 Wakening Essences
        if (l_Bracket == BattlegroundBracketType::RandomBattleground && !p_Source->HasWinInBracket(l_Bracket, true) && p_Source->IsInRandomBG())
        {
            p_Source->ModifyCurrency(CurrencyTypes::CURRENCY_TYPE_WAKENING_ESSENCE, 30);
            p_Source->SendDisplayToast(CurrencyTypes::CURRENCY_TYPE_WAKENING_ESSENCE, 30, 0, DISPLAY_TOAST_METHOD_CURRENCY_OR_GOLD, TOAST_TYPE_NEW_CURRENCY, false, false);
        }

        /// Strongbox - Only available in random bg, rated bg & rated arena 2v2 according to SMSG_REQUEST_PVP_REWARDS_RESPONSE
        if ((isBattleground() && p_Source->IsInRandomBG()) || (!IsSkirmish() && (GetArenaType() == ArenaType::Arena2v2 || GetArenaType() == ArenaType::Arena1v1)) || IsRatedBG())
        {
            p_Source->AddItem(p_Source->GetTeamId() == TeamId::TEAM_HORDE ? 138469: 138474, 1);
            p_Source->SendDisplayToast(138474, 1, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
        }

        if (isArena())
        {
            if ((p_Source->GetCharacterWorldStateValue(CharacterWorldStates::LastArenaLegendaryAttemptTimestamp) + 900) < time(nullptr))
            {
                p_Source->CompleteLegendaryActivity(LegendaryActivity::ArenaWon);
                p_Source->SetCharacterWorldState(CharacterWorldStates::LastArenaLegendaryAttemptTimestamp, time(nullptr));
            }
        }
        else
            p_Source->CompleteLegendaryActivity(LegendaryActivity::BattlegroundWon);
    }
    else
    {
        /// Strongbox - Only available in random bg, rated bg & rated arena 2v2 according to SMSG_REQUEST_PVP_REWARDS_RESPONSE
        if ((isBattleground() && p_Source->IsInRandomBG()) || (!IsSkirmish() && (GetArenaType() == ArenaType::Arena2v2 || GetArenaType() == ArenaType::Arena1v1)) || IsRatedBG())
        {
            if (roll_chance_i(IsSkirmish() ? 15 : 30))
            {
                p_Source->AddItem(p_Source->GetNativeTeamId() == TeamId::TEAM_HORDE ? 138469 : 138474, 1);
                p_Source->SendDisplayToast(138474, 1, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
            }
        }
    }

    uint8 l_Bracket = static_cast<uint8>(GetBattlegroundBracketType());

    if (((isBattleground() && p_Source->IsInRandomBG()) && !IsRatedBG()) || IsSkirmish())
    {
        p_Source->IncrementDayGames(l_Bracket);
        p_Source->IncrementWeekGames(l_Bracket);
        p_Source->IncrementSeasonGames(l_Bracket);

        if (p_Winner)
        {
            p_Source->IncrementDayWins(l_Bracket);
            p_Source->IncrementWeekWins(l_Bracket);
            p_Source->IncrementSeasonWins(l_Bracket);
        }
    }
}
void Battleground::RepopPlayerAtGraveyard(uint64 p_GUID)
{
...
bool Battleground::AddObject(uint32 p_Type, uint32 p_Entry, Position p_Pos, Position p_Rotation /*= { }*/, uint32 p_RespawnTime /*= 0*/)
{
    return AddObject(p_Type, p_Entry, p_Pos.m_positionX, p_Pos.m_positionY, p_Pos.m_positionZ, p_Pos.m_orientation, p_Rotation.m_positionX, p_Rotation.m_positionY, p_Rotation.m_positionZ, p_Rotation.m_orientation, p_RespawnTime);
}
...
bool Battleground::IsDoorClosed(uint32 p_Type) const
{
    if (GameObject* l_Obj = GetBgMap()->GetGameObject(BgObjects[p_Type]))
    {
        if (l_Obj->getLootState() == LootState::GO_READY && l_Obj->GetGoState() == GOState::GO_STATE_READY)
            return true;
    }
...
bool Battleground::AddSpiritGuide(uint32 type, float x, float y, float z, float o, uint32 team)
{
    ...
    if (Creature* creature = AddCreature(entry, type, team, x, y, z, o))
    {
        ...
        creature->AddChannelObject(Guid64To128(creature->GetGUID()));
        // aura
...
void Battleground::SendWarningToAll(int32 entry, ...)
{
    ...
    const char *format = sObjectMgr->GetTrinityStringForDB2Locale(entry);
...
{
    // FIXME: now we have different DB2 locales and need localized message for each target client
    return sObjectMgr->GetTrinityStringForDB2Locale(entry);
}
...
void Battleground::HandleKillPlayer(Player* victim, Player* killer)
{
    ...
    if (killer)
    {
        ...
        UpdatePlayerScore(killer, victim, SCORE_HONORABLE_KILLS, 1, !IsWargame());
        ...
        for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
        {
            ...
            if (creditedPlayer->GetTeam() == killer->GetTeam() && creditedPlayer->IsAtGroupRewardDistance(victim))
                UpdatePlayerScore(creditedPlayer, victim, SCORE_HONORABLE_KILLS, 1, !IsWargame());
        }
    }
...
uint32 Battleground::GetAlivePlayersCountByTeam(uint32 Team) const
{
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        if (itr->second.Team == Team)
        {
            ...
            if (player && player->isAlive() && !player->HasAura(27827) && !player->HasAura(215769))
                ++count;
        }
    }
    return count;
...
void Battleground::SetBgRaid(uint32 p_TeamID, GroupPtr p_BgRaid)
{
    uint32 l_BgIndex = p_TeamID == ALLIANCE ? BG_TEAM_ALLIANCE : BG_TEAM_HORDE;

    if (m_BgRaids[l_BgIndex])
        m_BgRaids[l_BgIndex]->SetBattlegroundGroup(nullptr);

    if (p_BgRaid)
        p_BgRaid->SetBattlegroundGroup(this);

    m_BgRaids[l_BgIndex] = p_BgRaid;
}
...
{
    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground::CheckAchievementCriteriaMeet: No implementation for criteria %u", criteriaId);
    return false;
}
...
void Battleground::RewardXPAtKill(Player* killer, Player* victim)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_BG_XP_FOR_KILL) && killer && victim)
    {
        ...
        killer->RewardPlayerAndGroupAtKill(victim, true, l_RewardedPlayers);
    }
}
...
uint32 Battleground::GetArenaMatchmakerRating(uint32 Team, uint8 slot)
{
    ...
    for (auto const& l_Iter : m_PlayerData)
    {
        if (l_Iter.second.TeamID != Team)
            continue;

        MMR += l_Iter.second.GetMatchMakerRating(slot);
        ++count;
    }
...
uint32 Battleground::GetTeamPersonalRating(uint32 p_Team, uint8 p_Bracket)
{
    uint32 l_Rating = 0;
    uint32 l_Count = 0;
    ...
    for (auto const& l_Iter : m_PlayerData)
    {
        if (l_Iter.second.TeamID != p_Team)
            continue;

        l_Rating += std::max(uint32(1500), l_Iter.second.GetPersonalRating(p_Bracket));
        ++l_Count;
    }
...
void Battleground::AwardTeams(uint32 p_Winner)
{
    ...
    /// Prevent exploit when all other team logged out before the match is started.
    auto l_Loser_team = GetBgRaid(GetOtherTeam(p_Winner));
    auto l_Winner_team = GetBgRaid(p_Winner);
    if (!l_Winner_team || !l_Loser_team)
        return;

    RewardHonorToTeam(0, p_Winner, MS::Battlegrounds::RewardCurrencyType::BattlegroundWin);
    RewardTeamWithAdditionalRewards(true, p_Winner);
    RewardTeamWithAdditionalRewards(false, GetOtherTeam(p_Winner));
}
...
void Battleground::ApplyDampeningIfNeeded()
{
    ...
    if (GetArenaType() == ArenaType::Arena2v2)
    {
        ...
        l_Timer = 6 * TimeConstants::MINUTE;

    if (GetElapsedTime() >= (l_Timer * IN_MILLISECONDS))
    {
        for (BattlegroundPlayerMap::iterator l_Iter = m_Players.begin(); l_Iter != m_Players.end(); ++l_Iter)
        {
            if (Player* l_Player = _GetPlayer(l_Iter, "ApplyDampeningIfNeeded"))
            {
                ...
                {
                    Aura* l_Aura = l_Player->AddAura(110310, l_Player);
                    if (l_Aura && GetArenaType() == ArenaType::Arena2v2 && IsRated())
                    {
                        if (AuraEffect* l_Effect = l_Aura->GetEffect(EFFECT_0))
                            l_Effect->ChangeAmount(20);
                    }
                    else if (l_Aura)
                    {
                        if (AuraEffect* l_Effect = l_Aura->GetEffect(EFFECT_0))
                            l_Effect->ChangeAmount(1);
                    }
                }
            }
        }
