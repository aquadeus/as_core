...
enum BG_SA_Objects
{
    ...
    BG_SA_RIGHT_FLAG,
    BG_SA_LEFT_FLAG,
    BG_SA_REJUVENATION_EAST,
    BG_SA_REJUVENATION_WEST,
    BG_SA_BOMB,
...
class BattlegroundSA : public Battleground
{
    public:
        ...
        virtual void GetTeamStartLoc(uint32 TeamID, float &X, float &Y, float &Z, float &O) const override;
        ...
        virtual void FillInitialWorldStates(ByteBuffer& data) override;
        /// Called when a player deal damage to building (door)
        virtual void EventPlayerDamagedGO(Player* player, GameObject* go, uint32 eventType) override;
        /// Called when a player kill a unit in bg
        virtual void HandleKillUnit(Creature* creature, Player* killer) override;
        ...
        virtual void EventPlayerUsedGO(Player* Source, GameObject* object) override;
        ...
        uint32 GetTeamScore(uint32 /*p_TeamID*/) const override { return 2; }
        uint32 GetMaxScore() const override { return 3; }
        bool IsScoreIncremental() const override { return true; }

        uint32 GetZoneId() override { return BATTLEGROUND_SA_ZONEID; }
