...
#define BG_KT_EVENT_UNK1            998
#define BG_KT_OBJECTIVE_ENTRY       419
#define BG_KT_CENTER_CRITERIA_TIMER_MAX (90 * IN_MILLISECONDS)
...
enum BG_KT_Objects
{
    ...
    BG_KT_OBJECT_BERSERK_NORTH  = 6,
    BG_KT_OBJECT_BERSERK_SOUTH  = 7,
...
enum BG_KT_SpellId
{
    ...
    BG_KT_SPELL_ACHIEVEMENT     = 128768    // For 'Powerball' achievement
};
...
class BattlegroundKT : public Battleground
{
    ...
    public:
        ...
        virtual void EventPlayerClickedOnFlag(Player* source, GameObject* target_obj) override { EventPlayerClickedOnOrb(source, target_obj); }
        ...
        uint32 GetZoneId() override { return BATTLEGROUND_KT_ZONEID; }
        bool IsOrbKeeper(uint64 p_Guid) const
        {
            for (uint8 i = 0; i < MAX_ORBS; ++i)
            {
                if (m_OrbKeepers[i] == p_Guid)
                    return true;
            }
            return false;
        }
    private:
        ...
        std::set<uint64> const GetFlagPickersGUID(int32 p_Team) const override
        {
        ...
        std::map<uint64, time_t> m_playersInCenterTimers;
