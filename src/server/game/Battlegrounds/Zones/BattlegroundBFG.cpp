    ...
    if (GetStatus() == STATUS_WAIT_JOIN)
    {
        if (m_CheatersCheckTimer <= diff)
        {
            ...
            for (auto l_Pair : l_Players)
            {
                ...
                if (l_Player->GetBGTeam() == ALLIANCE && l_Player->GetPositionX() > 922.0f)
                {
                    l_Player->TeleportTo(GetMapId(), 900.09f, 1340.91f, 27.52f, l_Player->GetOrientation(), 0);
                }
                else if (l_Player->GetBGTeam() == HORDE && l_Player->GetPositionX() < 1393.0f)
                {
                    l_Player->TeleportTo(GetMapId(), 1406.15f, 983.60f, 7.45f, l_Player->GetOrientation(), 0);
                }
            }
...
void BattlegroundBFG::HandleAreaTrigger(Player * /*Source*/, uint32 /*Trigger*/, bool /*p_Enter*/)
{
...
void BattlegroundBFG::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardType)
{
    ...
    switch (p_Type)
    {
        case SCORE_BASES_ASSAULTED:
            ...
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, BG_OBJECTIVE_ASSAULT_BASE);
            break;
        case SCORE_BASES_DEFENDED:
            ((BattlegroundBFGScore*)itr->second)->BasesDefended += p_Value;
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, BG_OBJECTIVE_DEFEND_BASE);
            break;
        default:
