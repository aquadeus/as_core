...
void BattlegroundEY::StartingEventOpenDoors()
{
    ...
    // Achievement: Flurry
    StartCriteriaTimer(CRITERIA_TIMED_TYPE_EVENT, EY_EVENT_START_BATTLE);
}
...
void BattlegroundEY::HandleAreaTrigger(Player* Source, uint32 Trigger, bool /*p_Enter*/)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundEY::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardType)
{
    ...
    switch (p_Type)
    {
        case SCORE_FLAG_CAPTURES:                           // flags captured
            ...
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, EY_OBJECTIVE_CAPTURE_FLAG);
            break;
        default:
