...
void BattlegroundWS::PostUpdateImpl(uint32 diff)
{
    ...
    for (BattlegroundPlayerMap::const_iterator l_Itr = GetPlayers().begin(); l_Itr != GetPlayers().end(); ++l_Itr)
    {
        ...
        WorldLocation l_AllianceFlagSpawn(489, 1539.98f, 1481.36f, 352.659f);
        if (l_Player->GetDistance(l_AllianceFlagSpawn) < 5.0f)
        {
            if (_flagState[BG_TEAM_HORDE] && !_flagState[BG_TEAM_ALLIANCE])
            {
                if (GetFlagPickerGUID(BG_TEAM_HORDE) == l_Player->GetGUID())
                    EventPlayerCapturedFlag(l_Player);
            }
        }

        WorldLocation l_HordeFlagSpawn(489, 918.496f, 1434.04, 346.054);
        if (l_Player->GetDistance(l_HordeFlagSpawn) < 5.0f)
        {
            if (_flagState[BG_TEAM_ALLIANCE] && !_flagState[BG_TEAM_HORDE])
            {
                if (GetFlagPickerGUID(BG_TEAM_ALLIANCE) == l_Player->GetGUID())
                    EventPlayerCapturedFlag(l_Player);
            }
        }
    }
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (_bothFlagsKept)
        {
            ...
            /// After 1 minute, then every 30 seconds, add a new focused stack
            if ((_flagDebuffState == 0 && _flagSpellForceTimer >= 1 * MINUTE * IN_MILLISECONDS) ||
                (_flagDebuffState < 5 && _flagDebuffState > 0 && _flagSpellForceTimer >= 30 * IN_MILLISECONDS))
            {
            ...
            /// After 5 stacks, then every 30 seconds, add a new brutal stack
            else if (_flagDebuffState == 5 && _flagSpellForceTimer >= 30 * IN_MILLISECONDS)
            {
                if (Player* player = ObjectAccessor::FindPlayer(m_FlagKeepers[0]))
                {
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
        ...
        && BgObjects[BG_WS_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        Source->StartCriteriaTimer(CRITERIA_TIMED_TYPE_SPELL_TARGET, BG_WS_SPELL_SILVERWING_FLAG_PICKED);
        ...
        && BgObjects[BG_WS_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        ...
        Source->StartCriteriaTimer(CRITERIA_TIMED_TYPE_SPELL_TARGET, BG_WS_SPELL_WARSONG_FLAG_PICKED);
    ...
    if (GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_GROUND && Source->IsWithinDistInMap(target_obj, 10) && target_obj->GetGOInfo()->entry == BG_OBJECT_A_FLAG_GROUND_WS_ENTRY)
    {
        ...
        else
        {
            ...
            /// 7.1.5 - Build 23420: Forced removal of Sandstone Drake and Obsidian Nightwing mounts, since they are not considered "mounts" but "transforms" by the game
            Source->RemoveAura(93326);
            Source->RemoveAura(121820);
        }
...
void BattlegroundWS::HandleAreaTrigger(Player* Source, uint32 Trigger, bool p_Enter)
{
}
...
void BattlegroundWS::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardType)
{
    ...
    switch (p_Type)
    {
        case SCORE_FLAG_CAPTURES:                           // flags captured
            ...
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, WS_OBJECTIVE_CAPTURE_FLAG);
            break;
        case SCORE_FLAG_RETURNS:                            // flags returned
            ((BattlegroundWGScore*)itr->second)->FlagReturns += p_Value;
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, WS_OBJECTIVE_RETURN_FLAG);
            break;
        default:
