...
void BattlegroundTP::PostUpdateImpl(uint32 diff)
{
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (m_BothFlagsKept)
        {
            ...
            if ((m_FlagDebuffState == 0 && m_FlagSpellForceTimer >= 1 * MINUTE * IN_MILLISECONDS) ||
                (m_FlagDebuffState < 5 && m_FlagDebuffState > 0 && m_FlagSpellForceTimer >= 30 * IN_MILLISECONDS))
            {
            ...
            else if (m_FlagDebuffState == 5 && m_FlagSpellForceTimer >= 30 * IN_MILLISECONDS)
            {
                if (Player* player = ObjectAccessor::FindPlayer(_flagKeepers[0]))
                {
...
void BattlegroundTP::EventPlayerClickedOnFlag(Player *Source, GameObject* target_obj)
{
        ...
        && this->BgObjects[BG_TP_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
        ...
        Source->StartCriteriaTimer(CRITERIA_TIMED_TYPE_SPELL_TARGET, BG_TP_SPELL_ALLIANCE_FLAG_PICKED);
        ...
        && this->BgObjects[BG_TP_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
        ...
        Source->StartCriteriaTimer(CRITERIA_TIMED_TYPE_SPELL_TARGET, BG_TP_SPELL_HORDE_FLAG_PICKED);
...
void BattlegroundTP::HandleAreaTrigger(Player *Source, uint32 Trigger, bool /*p_Enter*/)
{
...
void BattlegroundTP::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardType)
{
    ...
    switch (p_Type)
    {
        case SCORE_FLAG_CAPTURES:                           // flags captured
            ...
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, TP_OBJECTIVE_CAPTURE_FLAG);
            break;
        case SCORE_FLAG_RETURNS:                            // flags returned
            ((BattlegroundTPScore*)itr->second)->FlagReturns += p_Value;
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, TP_OBJECTIVE_RETURN_FLAG);
            break;
        default:
