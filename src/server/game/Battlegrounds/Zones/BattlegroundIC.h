...
class BattlegroundIC : public Battleground
{
    public:
        ...
        virtual void HandlePlayerResurrect(Player* player) override;

        uint32 GetNodeState(uint8 nodeType) { return (uint8)nodePoint[nodeType].nodeState; }

        uint32 GetTeamScore(uint32 p_TeamID) const override { return factionReinforcements[GetTeamIndexByTeamId(p_TeamID)]; }
        uint32 GetMaxScore() const override { return MAX_REINFORCEMENTS; }
        bool IsScoreIncremental() const override { return false; }

        virtual bool IsAllNodesControlledByTeam(uint32 team) const override;
        uint32 GetZoneId() override { return BATTLEGROUND_IC_ZONEID; }
    private:
