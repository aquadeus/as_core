...
void BattlegroundIC::HandleAreaTrigger(Player* /*Source*/, uint32 /*Trigger*/, bool /*p_Enter*/)
{
    ...
    for (uint8 i = 0; i < MAX_NODE_TYPES; i++)
    {
        if (nodePoint[i].gameobject_entry == target_obj->GetEntry())
        {
                ...
                // If we are going to spawn the definitve faction banner, we dont need the timer anymore
            {
                ...
                player->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, 246, 1);
            }
