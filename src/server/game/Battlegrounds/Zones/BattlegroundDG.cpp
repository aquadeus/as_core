...
bool BattlegroundDG::SetupBattleground()
{
    ...
    // Buffs
    if (   !AddObject(BG_DG_OBJECT_BUFF_NORTH, BG_OBJECTID_REGENBUFF_ENTRY, BG_DG_BuffPositions[0][0], BG_DG_BuffPositions[0][1], BG_DG_BuffPositions[0][2], BG_DG_BuffPositions[0][3], 0, 0, std::sin(BG_DG_BuffPositions[0][3] / 2), std::cos(BG_DG_BuffPositions[0][3] / 2), BUFF_RESPAWN_TIME)
        || !AddObject(BG_DG_OBJECT_BUFF_SOUTH, BG_OBJECTID_REGENBUFF_ENTRY, BG_DG_BuffPositions[1][0], BG_DG_BuffPositions[1][1], BG_DG_BuffPositions[1][2], BG_DG_BuffPositions[1][3], 0, 0, std::sin(BG_DG_BuffPositions[1][3] / 2), std::cos(BG_DG_BuffPositions[1][3] / 2), BUFF_RESPAWN_TIME)
        || !AddObject(BG_DG_OBJECT_BUFF_EAST, BG_OBJECTID_BERSERKERBUFF_ENTRY, BG_DG_BuffPositions[2][0], BG_DG_BuffPositions[2][1], BG_DG_BuffPositions[2][2], BG_DG_BuffPositions[2][3], 0, 0, std::sin(BG_DG_BuffPositions[2][3] / 2), std::cos(BG_DG_BuffPositions[2][3] / 2), BUFF_RESPAWN_TIME)
        || !AddObject(BG_DG_OBJECT_BUFF_WEST, BG_OBJECTID_BERSERKERBUFF_ENTRY, BG_DG_BuffPositions[3][0], BG_DG_BuffPositions[3][1], BG_DG_BuffPositions[3][2], BG_DG_BuffPositions[3][3], 0, 0, std::sin(BG_DG_BuffPositions[3][3] / 2), std::cos(BG_DG_BuffPositions[3][3] / 2), BUFF_RESPAWN_TIME))
    {
...
void BattlegroundDG::EventPlayerClickedOnFlag(Player* p_Source, Unit* /*p_Flag*/)
{
    ...
    if (!p_Source->CanUseBattlegroundObject())
        return;
...
void BattlegroundDG::HandleAreaTrigger(Player* p_Player, uint32 p_TriggerID, bool p_Enter)
{
    switch (p_TriggerID)
    {
        case 9012: // Alliance cart spawn
            if (_flagState[TEAM_HORDE] == BG_DG_CART_STATE_ON_PLAYER && _flagState[TEAM_ALLIANCE] == BG_DG_CART_STATE_ON_BASE)
            {
        ...
        case 9013: // Horde cart spawn
            if (_flagState[TEAM_ALLIANCE] == BG_DG_CART_STATE_ON_PLAYER && _flagState[TEAM_HORDE] == BG_DG_CART_STATE_ON_BASE)
            {
        ...
        case 9303: // flying => should tp outside the mine when triggered
            ...
            break;
        default:
            Battleground::HandleAreaTrigger(p_Player, p_TriggerID, p_Enter);
            break;
    }
}
...
void BattlegroundDG::RespawnFlagAfterDrop(uint32 p_Team)
{
    ...
    _UpdateTeamScore(p_Team, _flagGold[p_Team]);
    _flagGold[p_Team] = 0;
...
class spell_mine_cart : public SpellScriptLoader
{
    public:
        ...
        class spell_mine_cart_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
               AfterEffectRemove += AuraEffectRemoveFn(spell_mine_cart_AuraScript::HandleOnRemove, EFFECT_4, SPELL_AURA_LINKED_2, AURA_EFFECT_HANDLE_REAL);
            }
        };
