...
enum BG_SM_Spells
{
    ...
    BG_SM_END_OF_THE_LINE_SPELL     = 128720, // Achievement

    BG_SM_MINE_CART_COURIER_1       = 128702, // Achievement, East to South criteria
    BG_SM_MINE_CART_COURIER_2       = 128703, // Achievement, East to North criteria
    BG_SM_MINE_CART_COURIER_3       = 128706, // Achievement, North to West criteria
    BG_SM_MINE_CART_COURIER_4       = 128705, // Achievement, North to East criteria
    BG_SM_MINE_CART_COURIER_5       = 128704, // Achievement, South criteria

    BG_SM_FIVE_FIVE_FIVE_SPELL      = 128650, // Achievement
};
...
enum BG_SM_Moving_Paths
{
    SM_MOVING_UNKNOWN,
    SM_MOVING_EAST_TO_SOUTH,
    SM_MOVING_EAST_TO_NORTH,
    SM_MOVING_NORTH_TO_WEST,
    SM_MOVING_NORTH_TO_EAST,
    SM_MOVING_TO_SOUTH
};
...
#define SM_PLAYER_TO_CART_DISTANCE      22.0f
#define SM_DEPOT_DISTANCE               6.0f
#define SM_ACHIEVEMENT_DISTANCE         26.0f
#define BG_SM_EVENT_UNK1                727
#define BG_SM_CART_OBJECTIVE_ENTRY      428
...
class BattlegroundSM : public Battleground
{
    public:
        ...
        bool IsPlayerCartKeeper(Player* p_Player, BG_SM_MineCarts p_Cart) const;
        bool IsPlayerAnyCartKeeper(Player* p_Player) const;

        /* Battleground Events */
        void EventReopenDepot(uint32 p_Diff);

        uint32 GetZoneId() override { return BATTLEGROUND_SSM_ZONEID; }
    ...
    private:
        ...
        bool IsCartNearDepot(Creature* p_Cart, SMDepots p_Depot, float p_Distance = SM_DEPOT_DISTANCE) const;
        bool IsCartNearAnyDepot(Creature* p_Cart, float p_Distance = SM_DEPOT_DISTANCE) const;

        /* Scorekeeping */
        uint32 GetTeamScore(uint32 p_TeamID) const override { return m_TeamScores[GetTeamIndexByTeamId(p_TeamID)]; }
        uint32 GetMaxScore() const override { return BG_SM_MAX_TEAM_SCORE; }
        ...
        uint32 GetMineCartTeamKeeper(uint8 p_MineCart) const;
        ...
        int32 m_DepotCloseTimer[SM_DEPOT_MAX];
        uint32 m_MineCartAddPointsTimer;
        int32 m_TrackSwitchClickTimer[SM_MAX_TRACK_SWITCH];
        bool m_Depot[SM_DEPOT_MAX]; // 0 = Waterfall, 1 = Lava, 2 = Diamond, 3 = Troll
        ...
        bool m_MineCartTakenNearDepot[SM_MINE_CART_MAX];
        BG_SM_Moving_Paths m_MineCartMovingPaths[SM_MINE_CART_MAX];
        ...
        std::unordered_map<uint64 /*guid*/, uint32 /*counter*/> m_PlayerCapturesWithoutDying;
};
