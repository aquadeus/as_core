...
void BattlegroundSM::Reset()
{
    ...
    for (uint8 l_Iter = 0; l_Iter < SM_MINE_CART_MAX; ++l_Iter)
    {
        ...
        m_MineCartTakenNearDepot[l_Iter] = false;
        m_MineCartMovingPaths[l_Iter] = BG_SM_Moving_Paths::SM_MOVING_UNKNOWN;
    }
    ...
    memset(m_PathDone, 0, sizeof(m_PathDone));
...
void BattlegroundSM::PostUpdateImpl(uint32 p_Diff)
{
    ...
    if (GetStatus() == STATUS_WAIT_JOIN)
    {
        if (m_CheatersCheckTimer <= p_Diff)
        {
            ...
            Position l_AlliancePos = { 851.53f, 134.97f, 328.99f, 0.0f };
            Position l_HordePos = { 630.02f, 229.97f, 328.99f, 0.0f };
            for (auto l_Pair : l_Players)
            {
                ...
                if (l_Player->GetBGTeam() == ALLIANCE && l_Player->GetExactDist(&l_AlliancePos) > 35.0f)
                {
                ...
                else if (l_Player->GetBGTeam() == HORDE && l_Player->GetExactDist(&l_HordePos) > 35.0f)
                {
...
bool BattlegroundSM::IsCartNearDepot(Creature* p_Cart, SMDepots p_Depot, float p_Distance) const
{
    if (p_Cart == nullptr)
        return false;

    return p_Cart->GetExactDist2d(BG_SM_DepotPos[p_Depot][0], BG_SM_DepotPos[p_Depot][1]) <= p_Distance;
}

bool BattlegroundSM::IsCartNearAnyDepot(Creature* p_Cart, float p_Distance) const
{
    for (uint32 l_I = 0; l_I < SM_DEPOT_MAX; ++l_I)
    {
        if (IsCartNearDepot(p_Cart, (SMDepots)l_I, p_Distance))
            return true;
    }
    return false;
}
...
void BattlegroundSM::StartingEventOpenDoors()
{
    ...
    StartCriteriaTimer(CRITERIA_TIMED_TYPE_EVENT, BG_SM_EVENT_UNK1);
}
    ...
    if (m_MineCartCheckTimer <= 0)
    {
        for (BattlegroundPlayerMap::const_iterator l_Iter = GetPlayers().begin(); l_Iter != GetPlayers().end(); ++l_Iter)
        {
            if (Player* l_Player = ObjectAccessor::FindPlayer(l_Iter->first))
            {
                ...
                {
                    if (Creature* l_MineCart = l_Player->FindNearestCreature(NPC_MINE_CART_1, SM_PLAYER_TO_CART_DISTANCE, true))
                    {
                        ...
                        if (m_MineCartsProgressBar[BG_SM_MINE_CART_1 - 1] > BG_SM_PROGRESS_BAR_NEUTRAL)
                        {
                            ...
                            if (!l_MineCart->HasAura(BG_SM_CONTROL_VISUAL_ALLIANCE))
                            {
                                ...
                                m_MineCartTakenNearDepot[BG_SM_MINE_CART_1 - 1] = IsCartNearAnyDepot(l_MineCart, SM_ACHIEVEMENT_DISTANCE);
                            }
                        }
                    ...
                    else if (Creature* l_MineCart = l_Player->FindNearestCreature(NPC_MINE_CART_2, SM_PLAYER_TO_CART_DISTANCE, true))
                    {
                        UpdateWorldStateForPlayer(SM_DISPLAY_PROGRESS_BAR, BG_SM_PROGRESS_BAR_SHOW, l_Player);
                        ...
                        if (m_MineCartsProgressBar[BG_SM_MINE_CART_2 - 1] > BG_SM_PROGRESS_BAR_NEUTRAL)
                        {
                            ...
                            if (!l_MineCart->HasAura(BG_SM_CONTROL_VISUAL_ALLIANCE))
                            {
                                l_MineCart->CastSpell(l_MineCart, BG_SM_CONTROL_VISUAL_ALLIANCE, true);
                                SendMessageToAll(LANG_BG_SM_ALLIANCE_CONTROLLED_MINE_CART, CHAT_MSG_BG_SYSTEM_ALLIANCE);
                                m_MineCartTakenNearDepot[BG_SM_MINE_CART_2 - 1] = IsCartNearAnyDepot(l_MineCart, SM_ACHIEVEMENT_DISTANCE);
                            }
                        }
                    ...
                    else if (Creature* l_MineCart = l_Player->FindNearestCreature(NPC_MINE_CART_3, SM_PLAYER_TO_CART_DISTANCE, true))
                    {
                        UpdateWorldStateForPlayer(SM_DISPLAY_PROGRESS_BAR, BG_SM_PROGRESS_BAR_SHOW, l_Player);
                        ...
                        if (m_MineCartsProgressBar[BG_SM_MINE_CART_3 - 1] > BG_SM_PROGRESS_BAR_NEUTRAL)
                        {
                            ...
                            if (!l_MineCart->HasAura(BG_SM_CONTROL_VISUAL_ALLIANCE))
                            {
                                l_MineCart->CastSpell(l_MineCart, BG_SM_CONTROL_VISUAL_ALLIANCE, true);
                                SendMessageToAll(LANG_BG_SM_ALLIANCE_CONTROLLED_MINE_CART, CHAT_MSG_BG_SYSTEM_ALLIANCE);
                                m_MineCartTakenNearDepot[BG_SM_MINE_CART_3 - 1] = IsCartNearAnyDepot(l_MineCart, SM_ACHIEVEMENT_DISTANCE);
                            }
                        }
...
void BattlegroundSM::CheckMineCartNearDepot()
{
    if (!m_MineCartNearDepot[BG_SM_MINE_CART_1 - 1])
    {
        ...
        {
            if (IsCartNearDepot(l_MineCart, SM_LAVA_DEPOT))
            {
            ...
            if (IsCartNearDepot(l_MineCart, SM_DIAMOND_DEPOT))
            {
    ...
    if (!m_MineCartNearDepot[BG_SM_MINE_CART_2 - 1])
    {
        ...
        {
            if (IsCartNearDepot(l_MineCart, SM_WATERFALL_DEPOT))
            {
    ...
    if (!m_MineCartNearDepot[BG_SM_MINE_CART_3 - 1])
    {
        if (Creature* l_MineCart = GetBGCreature(NPC_SM_MINE_CART_3))
        {
            ...
            if (IsCartNearDepot(l_MineCart, SM_TROLL_DEPOT))
            {
...
void BattlegroundSM::EventTeamCapturedMineCart(uint32 p_Team, uint8 p_MineCart)
{
    ...
    for (BattlegroundPlayerMap::const_iterator l_Iter = GetPlayers().begin(); l_Iter != GetPlayers().end(); ++l_Iter)
    {
        if (Player* l_Player = ObjectAccessor::FindPlayer(l_Iter->first))
        {
            bool l_IsNearCart1 = l_Player->FindNearestCreature(NPC_MINE_CART_1, SM_PLAYER_TO_CART_DISTANCE, true);
            bool l_IsNearCart2 = !l_IsNearCart1 && l_Player->FindNearestCreature(NPC_MINE_CART_2, SM_PLAYER_TO_CART_DISTANCE, true);
            bool l_IsNearCart3 = !l_IsNearCart2 && l_Player->FindNearestCreature(NPC_MINE_CART_3, SM_PLAYER_TO_CART_DISTANCE, true);
            if (l_Player->GetBGTeam() == p_Team)
                if (l_IsNearCart1 || l_IsNearCart2 || l_IsNearCart3) ///< ??? Why is there this check? Why the player, who doesn't capture the cart, is rewarded if he's just near another cart?
                {
                    UpdatePlayerScore(l_Player, nullptr, SCORE_CART_CONTROLLED, 1);
                    l_Player->RewardHonor(l_Player, 1, irand(10, 12));

                    /// Achievements block
                    /// Update criterias for players captured the cart only

                    bool l_RewardEndOfLineAchievement = false;
                    uint32 l_MineCartCourierSpell = 0;
                    bool l_RewardObjective = false;
                    switch (p_MineCart)
                    {
                        case BG_SM_MINE_CART_1:
                            l_RewardEndOfLineAchievement = l_IsNearCart1 && m_MineCartTakenNearDepot[BG_SM_MINE_CART_1 - 1];
                            if (l_IsNearCart1 && m_MineCartMovingPaths[BG_SM_MINE_CART_1 - 1] == BG_SM_Moving_Paths::SM_MOVING_EAST_TO_SOUTH)
                                l_MineCartCourierSpell = BG_SM_MINE_CART_COURIER_1;
                            else if (l_IsNearCart1 && m_MineCartMovingPaths[BG_SM_MINE_CART_1 - 1] == BG_SM_Moving_Paths::SM_MOVING_EAST_TO_NORTH)
                                l_MineCartCourierSpell = BG_SM_MINE_CART_COURIER_2;
                            if (l_IsNearCart1)
                                m_PlayerCapturesWithoutDying[l_Player->GetGUID()]++;
                            l_RewardObjective = l_IsNearCart1;
                            break;
                        case BG_SM_MINE_CART_2:
                            l_RewardEndOfLineAchievement = l_IsNearCart2 && m_MineCartTakenNearDepot[BG_SM_MINE_CART_2 - 1];
                            if (l_IsNearCart2 && m_MineCartMovingPaths[BG_SM_MINE_CART_2 - 1] == BG_SM_Moving_Paths::SM_MOVING_TO_SOUTH)
                                l_MineCartCourierSpell = BG_SM_MINE_CART_COURIER_5;
                            if (l_IsNearCart2)
                                m_PlayerCapturesWithoutDying[l_Player->GetGUID()]++;
                            l_RewardObjective = l_IsNearCart2;
                            break;
                        case BG_SM_MINE_CART_3:
                            l_RewardEndOfLineAchievement = l_IsNearCart3 && m_MineCartTakenNearDepot[BG_SM_MINE_CART_3 - 1];
                            if (l_IsNearCart3 && m_MineCartMovingPaths[BG_SM_MINE_CART_3 - 1] == BG_SM_Moving_Paths::SM_MOVING_NORTH_TO_WEST)
                                l_MineCartCourierSpell = BG_SM_MINE_CART_COURIER_3;
                            else if (l_IsNearCart3 && m_MineCartMovingPaths[BG_SM_MINE_CART_3 - 1] == BG_SM_Moving_Paths::SM_MOVING_NORTH_TO_EAST)
                                l_MineCartCourierSpell = BG_SM_MINE_CART_COURIER_4;
                            if (l_IsNearCart3)
                                m_PlayerCapturesWithoutDying[l_Player->GetGUID()]++;
                            l_RewardObjective = l_IsNearCart3;
                            break;
                        default:
                            break;
                    }
                    if (l_RewardEndOfLineAchievement)
                        l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_BE_SPELL_TARGET, BG_SM_Spells::BG_SM_END_OF_THE_LINE_SPELL);
                    if (l_MineCartCourierSpell)
                        l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_BE_SPELL_TARGET, l_MineCartCourierSpell);
                    if (m_PlayerCapturesWithoutDying[l_Player->GetGUID()] == 5)
                        l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_BE_SPELL_TARGET, BG_SM_Spells::BG_SM_FIVE_FIVE_FIVE_SPELL);
                    if (l_RewardObjective)
                        l_Player->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, BG_SM_CART_OBJECTIVE_ENTRY, 1);
...
uint32 BattlegroundSM::GetMineCartTeamKeeper(uint8 p_MineCart) const
{
...
void BattlegroundSM::ResetDepotsAndMineCarts(uint8 p_Depot, uint8 p_MineCart)
{
    ...
    m_MineCartTakenNearDepot[p_MineCart - 1] = false;
    m_MineCartMovingPaths[p_MineCart - 1] = BG_SM_Moving_Paths::SM_MOVING_UNKNOWN;
}
...
void BattlegroundSM::MineCartsMoves()
{
            ...
            if (l_MineCart->GetExactDist2d(717.169312f, 114.258339f) < 0.5f) // East pos
            {
                if (Creature* l_Track = GetBgMap()->GetCreature(BgCreatures[SM_TRACK_SWITCH_EAST]))
                {
                    if (l_Track->HasAura(BG_SM_TRACK_SWITCH_OPENED))
                    {
                        ...
                        m_MineCartMovingPaths[BG_SM_MINE_CART_1 - 1] = BG_SM_Moving_Paths::SM_MOVING_EAST_TO_SOUTH;
                    }
                        ...
                        l_Track->HasAura(BG_SM_TRACK_SWITCH_CLOSED))
                    {
                        ...
                        m_PathDone[SM_EAST_PATH][1] = true;
                        m_MineCartMovingPaths[BG_SM_MINE_CART_1 - 1] = BG_SM_Moving_Paths::SM_MOVING_EAST_TO_NORTH;
                    }
                }
            }
        }
    ...
    if (!m_WaterfallPathDone)
    {
        if (Creature* l_MineCart = GetBGCreature(NPC_SM_MINE_CART_2))
        {
            ...
            m_MineCartMovingPaths[BG_SM_MINE_CART_2 - 1] = BG_SM_Moving_Paths::SM_MOVING_TO_SOUTH;
        }
    }
            ...
            if (l_MineCart->GetExactDist2d(834.727234f, 299.809753f) < 0.5f) // North pos
            {
                if (Creature* l_Track = GetBgMap()->GetCreature(BgCreatures[SM_TRACK_SWITCH_NORTH]))
                {
                        ...
                        l_Track->HasAura(BG_SM_TRACK_SWITCH_CLOSED))
                    {
                        ...
                        m_MineCartMovingPaths[BG_SM_MINE_CART_3 - 1] = BG_SM_Moving_Paths::SM_MOVING_NORTH_TO_WEST;
                    }
                    else
                    {
                        l_MineCart->GetMotionMaster()->Clear(true); // n -> e
                        l_MineCart->GetMotionMaster()->MovePath(NPC_MINE_CART_3 * 100, false);
                        m_PathDone[SM_NORTH_PATH][1] = true;
                        m_MineCartMovingPaths[BG_SM_MINE_CART_3 - 1] = BG_SM_Moving_Paths::SM_MOVING_NORTH_TO_EAST;
                    }
                }
            }
        }
...
void BattlegroundSM::HandleKillPlayer(Player* p_Player, Player* p_Killer)
{
    ...
    m_PlayerCapturesWithoutDying[p_Player->GetGUID()] = 0;
}
...
bool BattlegroundSM::IsPlayerCartKeeper(Player* p_Player, BG_SM_MineCarts p_Cart) const
{
    uint32 l_CartEntry = 0;
    switch (p_Cart)
    {
        case BG_SM_MineCarts::BG_SM_MINE_CART_1:
            l_CartEntry = NPC_MINE_CART_1;
            break;
        case BG_SM_MineCarts::BG_SM_MINE_CART_2:
            l_CartEntry = NPC_MINE_CART_2;
            break;
        case BG_SM_MineCarts::BG_SM_MINE_CART_3:
            l_CartEntry = NPC_MINE_CART_3;
            break;
    }
    ...
    bool l_IsNearCart = p_Player->FindNearestCreature(l_CartEntry, SM_PLAYER_TO_CART_DISTANCE, true);
    return l_IsNearCart && GetMineCartTeamKeeper(p_Cart) == p_Player->GetTeam();
}

bool BattlegroundSM::IsPlayerAnyCartKeeper(Player* p_Player) const
{
    bool l_IsCart1 = IsPlayerCartKeeper(p_Player, BG_SM_MineCarts::BG_SM_MINE_CART_1);
    bool l_IsCart2 = !l_IsCart1 && IsPlayerCartKeeper(p_Player, BG_SM_MineCarts::BG_SM_MINE_CART_2);
    bool l_IsCart3 = !l_IsCart2 && IsPlayerCartKeeper(p_Player, BG_SM_MineCarts::BG_SM_MINE_CART_3);

    return l_IsCart1 || l_IsCart2 || l_IsCart3;
}