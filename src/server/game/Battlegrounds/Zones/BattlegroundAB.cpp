...
void BattlegroundAB::HandleAreaTrigger(Player* Source, uint32 Trigger, bool /*p_Enter*/)
{
    if (GetStatus() != STATUS_IN_PROGRESS)
        return;
...
void BattlegroundAB::UpdatePlayerScore(Player* p_Source, Player* p_Victim, uint32 p_Type, uint32 p_Value, bool p_DoAddHonor, MS::Battlegrounds::RewardCurrencyType::Type p_RewardType)
{
    ...
    switch (p_Type)
    {
        case SCORE_BASES_ASSAULTED:
            ...
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, AB_OBJECTIVE_ASSAULT_BASE);
            break;
        case SCORE_BASES_DEFENDED:
            ((BattlegroundABScore*)itr->second)->BasesDefended += p_Value;
            p_Source->UpdateCriteria(CRITERIA_TYPE_BG_OBJECTIVE_CAPTURE, AB_OBJECTIVE_DEFEND_BASE);
            break;
        default:
