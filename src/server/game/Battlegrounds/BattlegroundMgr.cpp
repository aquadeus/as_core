...
namespace MS
{
    namespace Battlegrounds
    {
        ...
        BattlegroundMgr::~BattlegroundMgr()
        {
            /// Prevent ASAN crash at shutdown
        ...
        void BattlegroundMgr::LogArenaMatch(Battleground* p_Arena, uint32 p_Winner, int32 p_WinnerRatingChange, int32 p_WinnerMmrChange, int32 p_LoserRatingChange, int32 p_LoserMmrChange)
        {
            ...
            auto const& l_Scores = p_Arena->GetPlayerScores();

            for (auto const& l_Itr : l_Scores)
            {
                if (Player* l_Player = ObjectAccessor::FindPlayer(l_Itr.first))
                {
                    ...
                    uint32 l_Rating      = l_Player->GetArenaPersonalRating((uint8)p_Arena->GetBattlegroundBracketType());
                    uint32 l_MMR         = l_Player->GetArenaMatchMakerRating((uint8)p_Arena->GetBattlegroundBracketType());
                ...
                else if (PlayerData const* l_Data = p_Arena->GetPlayerData(l_Itr.first))
                {
                    uint32 l_PlayerGuid     = l_Data->GuidLow;
                    uint32 l_Realm          = l_Data->RealmID;
                    bool   l_Winner         = l_Data->TeamID == p_Winner;
                    uint32 l_Damage         = 0;
                    uint32 l_Heal           = 0;
                    uint32 l_Kill           = 0;
                    uint32 l_Death          = 0;
                    uint32 l_Rating         = l_Data->GetPersonalRating((uint8)p_Arena->GetBattlegroundBracketType());
                    uint32 l_MMR            = l_Data->GetMatchMakerRating((uint8)p_Arena->GetBattlegroundBracketType());
                    int32 l_RatingChange    = l_Data->TeamID == p_Winner ? p_WinnerRatingChange : p_LoserRatingChange;
                    int32 l_MmrChange       = l_Data->TeamID == p_Winner ? p_WinnerMmrChange : p_LoserMmrChange;
            ...
            for (auto const& l_Data : p_Arena->GetPlayerData())
            {
                if (l_Scores.find(l_Data.first) != l_Scores.end())
                    continue;

                uint32 l_PlayerGuid     = l_Data.second.GuidLow;
                uint32 l_Realm          = l_Data.second.RealmID;
                bool   l_Winner         = l_Data.second.TeamID == p_Winner;
                uint32 l_Damage         = 0;
                uint32 l_Heal           = 0;
                uint32 l_Kill           = 0;
                uint32 l_Death          = 0;
                uint32 l_Rating         = l_Data.second.GetPersonalRating((uint8)p_Arena->GetBattlegroundBracketType());
                uint32 l_MMR            = l_Data.second.GetMatchMakerRating((uint8)p_Arena->GetBattlegroundBracketType());
                int32 l_RatingChange    = l_Data.second.TeamID == p_Winner ? p_WinnerRatingChange : p_LoserRatingChange;
                int32 l_MmrChange       = l_Data.second.TeamID == p_Winner ? p_WinnerMmrChange : p_LoserMmrChange;
...
# endif
        ...
        Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundType::Type p_BgType, Bracket const* p_BracketEntry, uint8 p_ArenaType, bool p_IsSkirmish, bool p_IsWargame, bool p_UseTournamentRules, bool p_RatedBg)
        {
            ...
            switch (p_BgType)
            {
                ...
                case BattlegroundType::BladeEdgeArena:
                    l_Battleground = new BattlegroundBEA(*(BattlegroundBEA*)l_BattlegroundTemplate);
                    break;
                ...
                case BattlegroundType::SeethingShore:
                    l_Battleground = new BattlegroundSS(*(BattlegroundSS*)l_BattlegroundTemplate);
                    break;
                case BattlegroundType::BlackrookHoldArena:
                    l_Battleground = new BattlegroundBRH(*(BattlegroundBRH*)l_BattlegroundTemplate);
                    break;
                case BattlegroundType::AshamanesFallArena:
                    l_Battleground = new BattlegroundAF(*(BattlegroundAF*)l_BattlegroundTemplate);
                    break;
                default:
        ...
        uint32 BattlegroundMgr::CreateBattlegroundTemplate(CreateBattlegroundData& data)
        {
            ...
            switch (data.bgTypeId)
            {
                ...
                case BATTLEGROUND_BRH:
                    l_Bg = new BattlegroundBRH;
                    break;
                case BATTLEGROUND_AF:
                    l_Bg = new BattlegroundAF;
                    break;
                case BATTLEGROUND_SEETHING_SHORE:
                    l_Bg = new BattlegroundSS;
                    break;
                default:
        ...
        void BattlegroundMgr::CreateInitialBattlegrounds()
        {
            ...
            do
            {
                ...
                if (!l_Bl)
                {
                    sLog->outError(LOG_FILTER_BATTLEGROUND, "Battleground ID %u not found in BattlemasterList.db2. Battleground not created.", l_BgTypeId);
                    continue;
                }
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Table `battleground_template` for id %u have non-existed WorldSafeLocs.db2 id %u in field `AllianceStartLoc`. BG not created.", l_Data.bgTypeId, l_StartId);
                    continue;
                }
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Table `battleground_template` for id %u have non-existed WorldSafeLocs.db2 id %u in field `HordeStartLoc`. BG not created.", l_Data.bgTypeId, l_StartId);
                    continue;
                }
