    ...
    namespace Battlegrounds
    {
        ...
        bool BattlegroundInvitationsMgr::InviteGroupToBG(GroupQueueInfo* p_GroupInfo, Battleground* p_Bg, uint32 p_Side)
        {
            ...
            if (!p_GroupInfo->m_IsInvitedToBGInstanceGUID)
            {
                ...
                p_GroupInfo->m_RemoveInviteTime = getMSTime() + (p_Bg->isArena() ? INVITE_ACCEPT_WAIT_TIME_ARENA : INVITE_ACCEPT_WAIT_TIME_BG);
                ...
                for (auto l_Itr = std::begin(p_GroupInfo->m_Players); l_Itr != std::end(p_GroupInfo->m_Players); ++l_Itr)
                {
                    ...
                    p_Bg->RegisterPlayer(l_Player, p_GroupInfo->m_Team);
                    ...
                    m_Events.AddEvent(l_RemoveEvent, m_Events.CalculateTime(p_Bg->isArena() ? INVITE_ACCEPT_WAIT_TIME_ARENA : INVITE_ACCEPT_WAIT_TIME_BG));
                    ...
                    /// Send status packet.
                    WorldPacket data;
                    PacketFactory::Status(&data, p_Bg, l_Player, l_QueueSlot, STATUS_WAIT_JOIN, (p_Bg->isArena() ? INVITE_ACCEPT_WAIT_TIME_ARENA : INVITE_ACCEPT_WAIT_TIME_BG), l_Player->GetBattlegroundQueueJoinTime(p_GroupInfo->m_BgTypeId), p_GroupInfo->m_ArenaType, p_GroupInfo->m_IsSkirmish, p_GroupInfo->m_IsRandom);
        ...
        void BattlegroundInvitationsMgr::RemovePlayer(uint64 p_Guid, bool p_DecreaseInvitedCount, BattlegroundType::Type p_Type)
        {
            ...
            if (l_Group->m_IsInvitedToBGInstanceGUID && l_Group->m_IsRatedBG && IsArena(l_Group->m_BgTypeId) && p_DecreaseInvitedCount)
            {
                if (Player* player = ObjectAccessor::FindPlayerInOrOutOfWorld(p_Guid))
                {
                    ...
                    uint8 slot = (uint8)GetBracketTypeByArenaType(l_Group->m_ArenaType);
        ...
        bool BGQueueRemoveEvent::Execute(uint64 /*p_Time*/, uint32 /*p_Time*/)
        {
            ...
            l_Player->SetWargameInvite(false);

            if (m_BgInstanceGUID == l_Player->GetBattlegroundId())
                return true;

            BattlegroundType::Type l_Type = GetSchedulerType(m_BgTypeId);
            Battleground* l_Bg = sBattlegroundMgr->GetBattleground(m_BgInstanceGUID, l_Type);
            /// Battleground can be deleted already when we are removing queue info
            /// Bg pointer can be NULL! so use it carefully!

            uint32 l_QueueSlot = l_Player->GetBattlegroundQueueIndex(l_Type);
            if (l_QueueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES) /// Player is in queue, or in Battleground.
            {
                ...
                if (l_InvitationsMgr.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
                {
                    ...
                    /// When queue for rated 2v2 & 3v3 pops up, and ready to enter, if you do not enter, you should lose rating
                    if (l_Bg && l_Bg->IsRated() && l_Bg->isArena())
                    {
                        if (PlayerData const* l_Data = l_Bg->GetPlayerData(l_Player->GetObjectGuid()))
                        {
                            uint32 l_LooserTeam = l_Data->TeamID;
                            uint32 l_WinnerTeam = l_Bg->GetOtherTeam(l_LooserTeam);
                            uint32 l_LooserMMR  = l_Bg->GetArenaMatchmakerRating(l_LooserTeam, uint8(l_Bg->GetBattlegroundBracketType()));
                            uint32 l_WinnerMMR  = l_Bg->GetArenaMatchmakerRating(l_WinnerTeam, uint8(l_Bg->GetBattlegroundBracketType()));
                            ...
                            if (GroupPtr l_Group = l_Player->GetGroup())
                                l_Bg->SetBgRaid(l_LooserTeam, l_Group);

                            l_Bg->LooserRatingChanges(l_LooserMMR, l_WinnerMMR, l_LooserChange, l_LooserMMRChange, uint8(l_Bg->GetBattlegroundBracketType()), l_WinnerTeam, l_Player->GetObjectGuid());

                            l_Bg->SetArenaMatchmakerRating(l_LooserTeam, l_LooserMMR + l_LooserMMRChange);
                            l_Bg->SetArenaTeamRatingChangeForTeam(l_LooserTeam, l_LooserChange);
                        }
                    }

                    l_Player->RemoveBattlegroundQueueId(l_Type);
#ifndef CROSS
                    if (l_Bg)
                        l_Player->HandleDeserterAfterLeaveQueue(l_LooserChange, l_LooserMMRChange, uint8(l_Bg->GetBattlegroundBracketType()));
#else
                    if (l_Bg)
                    {
                        if (InterRealmClient* l_Client = l_Player->GetSession()->GetInterRealmClient())
                            l_Client->SendBattlegroundLeaveQueue(l_Player->GetGUID(), l_LooserChange, l_LooserMMRChange, uint8(l_Bg->GetBattlegroundBracketType()));
                    }
#endif // !CROSS
