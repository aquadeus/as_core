...
enum BattlegroundAreaIds
{
    ...
    BATTLEGROUND_BRH_ZONEID = 8151,
    //BATTLEGROUND_NA2_ZONEID = 7822,
    BATTLEGROUND_AF_ZONEID  = 8008,
    BATTLEGROUND_SS_ZONEID  = 9136
};
...
namespace MS
{
    namespace Battlegrounds
    {
        ...
        enum class RewardGearType  : uint32
        {
            Regular         = 0,
            Rated           = 1,
            Elite           = 2
        ...
        namespace BattlegroundType
        {
            enum Type
            {
                ...
                RuinsOfLordaeron     = 25,
                NagrandArena         = 26,
                BlackrookHoldArena   = 27,
                //NagrandArena2        = 26,
                AshamanesFallArena   = 28,
                LastRandomArena      = 29,
                /// End iterators.
                End,
                RatedBg15v15,
            ...
            {
                return (p_Type == RatedBg10v10 || p_Type == RatedBg15v15 || p_Type == RatedBg25v25) || (p_Type >= Arena1v1 && p_Type <= Arena5v5);
            }
            ...
            inline ArenaType GetArenaType(Type p_Type) ///< GetArenaType is unused
            {
                switch (p_Type)
                {
                    case BattlegroundType::Arena1v1:
                    case BattlegroundType::ArenaSkirmish1v1:
                        return ArenaType::Arena1v1;
        ...
        namespace
        {
            static std::map<uint32, BattlegroundType::Type> k_MapIdToBattlegroundType =
            {
                ...
                { 1803, BattlegroundType::SeethingShore         }
            };
        ...
        inline BattlegroundTypeId GetIdFromType(BattlegroundType::Type p_Type) ///< GetIdFromType is unused
        {
            switch (p_Type)
            {
                ...
                case BattlegroundType::SeethingShore:
                ...
                case BattlegroundType::BlackrookHoldArena:
                    return BATTLEGROUND_BRH;
                /*case BattlegroundType::NagrandArena2:
                    return BATTLEGROUND_NA2;*/
                case BattlegroundType::AshamanesFallArena:
                    return BATTLEGROUND_AF;
                default:
        ...
        inline BattlegroundType::Type GetTypeFromId(BattlegroundTypeId p_BgTypeId, uint8 p_ArenaType, bool p_IsSkirmish = false) ///< GetTypeFromId is unused
        {
            switch (p_BgTypeId)
            {
                ...
                case BATTLEGROUND_SEETHING_SHORE:
                    return BattlegroundType::SeethingShore;
                ...
                case BATTLEGROUND_TYPE_NONE:
                {
                    switch (p_ArenaType)
                    {
                        case ArenaType::Arena1v1:
                            return p_IsSkirmish ? BattlegroundType::ArenaSkirmish1v1 : BattlegroundType::Arena1v1;
        ...
        inline BattlegroundType::Type GetSchedulerType(BattlegroundTypeId p_BgTypeId) ///< GetSchedulerType is unused
        {
            switch (p_BgTypeId)
            {
                ...
                case BATTLEGROUND_BRH:
                    return BattlegroundType::BlackrookHoldArena;
                /*case BATTLEGROUND_NA2:
                    return BattlegroundType::NagrandArena2;*/
                case BATTLEGROUND_AF:
                    return BattlegroundType::AshamanesFallArena;
                default:
                    return BattlegroundType::None;
            }
        }
        ...
        inline bool IsArenaType(BattlegroundType::Type p_BgType) ///< IsArenaType is unused
        {
            return (p_BgType == BattlegroundType::AllArenas ||
                ...
                p_BgType == BattlegroundType::BlackrookHoldArena ||
                //p_BgType == BattlegroundType::NagrandArena2 ||
                p_BgType == BattlegroundType::AshamanesFallArena);
        }
...
{
    CHECK_PLAYER_POSITION_INVERVAL  = 1000,                 ///< Milliseconds
    RESURRECTION_INTERVAL           = 30000,                ///< Milliseconds
    //REMIND_INTERVAL                 = 10000,                ///< Milliseconds
    INVITATION_REMIND_TIME          = 20000,                ///< Milliseconds
    INVITE_ACCEPT_WAIT_TIME_BG      = 90000,                ///< Milliseconds
    INVITE_ACCEPT_WAIT_TIME_ARENA   = 30000,                ///< Milliseconds
    TIME_AUTOCLOSE_BATTLEGROUND     = 120000,               ///< Milliseconds
    MAX_OFFLINE_TIME                = 300,                  ///< Seconds
    RESPAWN_ONE_DAY                 = 86400,                ///< Seconds
    RESPAWN_IMMEDIATELY             = 0,                    ///< Seconds
    BUFF_RESPAWN_TIME               = 180,                  ///< Seconds
    BATTLEGROUND_COUNTDOWN_MAX      = 120000,               ///< Milliseconds
    ARENA_COUNTDOWN_MAX             = 60000                 ///< Milliseconds
};
...
static std::unordered_map<uint8, uint32> g_HonorRewardPerBracket =
{
    ...
    { (uint8)BattlegroundBracketType::Arena1v1,           100 },   ///< Custom
    { (uint8)BattlegroundBracketType::RandomBattleground, 300 }
};
...
class BattlegroundScore
{
    public:
        BattlegroundScore() : KillingBlows(0), Deaths(0), HonorableKills(0),
            BonusHonor(0), DamageDone(0), HealingDone(0), RatingChange(0), m_RatingChange(0), m_MmrRatingChange(0)
        {}
        virtual ~BattlegroundScore() {}                     //virtual destructor is used when deleting score from scores map

        int32 GetRatingChange() const { return m_RatingChange; }
        int32 GetMmrRatingChange() const { return m_MmrRatingChange; }
        ...
        uint32 HealingDone;
        int32 RatingChange;
        int32 m_RatingChange;
        int32 m_MmrRatingChange;
};
...
{
    PlayerData();
    PlayerData(Player* p_Player, uint32 p_TeamID);

    uint32 GetPersonalRating(uint8 p_Bracket) const
    {
        if (p_Bracket >= uint8(BattlegroundBracketType::MaxRated))
            return 0;

        return Rating[p_Bracket];
    }

    uint32 GetMatchMakerRating(uint8 p_Bracket) const
    {
        if (p_Bracket >= uint8(BattlegroundBracketType::MaxRated))
            return 0;

        return MMR[p_Bracket];
    }
    ...
    uint32 RealmID = 0;
    uint32 TeamID = 0;
    std::array<uint32, uint8(BattlegroundBracketType::MaxRated)> Rating;
    std::array<uint32, uint8(BattlegroundBracketType::MaxRated)> MMR;
};

using PlayerDataMap = std::unordered_map<uint64, PlayerData>;
...
class Battleground
{
    public:
        ...
        BattlegroundScoreMap const& GetPlayerScores() { return PlayerScores; }
        ...
        void CheckSafePositions(uint32 p_Diff);
        ...
        void RewardTeamWithAdditionalRewards(bool p_Winner, uint32 TeamID);
        ...
        // Raid Group
        GroupPtr GetBgRaid(uint32 TeamID) const { return TeamID == ALLIANCE ? m_BgRaids[BG_TEAM_ALLIANCE] : m_BgRaids[BG_TEAM_HORDE]; }
        GroupPtr GetBGRaid(uint8 p_Index) const { return p_Index < MS::Battlegrounds::TeamsCount::Value ? m_BgRaids[p_Index] : nullptr; }
        void SetBgRaid(uint32 TeamID, GroupPtr bg_raid);
        ...
        void RewardPlayerWithOtherRewards(Player* p_Source, bool p_Winner);
        ...
        uint32 GetTeamPersonalRating(uint32 p_Team, uint8 p_Bracket);
        ...
        virtual void HandleAreaTrigger(Player* /*Source*/, uint32 /*Trigger*/, bool /*p_Enter*/) {}
        ...
        void RegisterPlayer(Player* p_Player, uint32 p_Team);

        PlayerData const* GetPlayerData(uint64 p_Guid);
        PlayerDataMap const& GetPlayerData() { return m_PlayerData; }

        void WinnerRatingChanges(uint32 p_OwnMMR, uint32 p_OpponentMMR, int32& p_RatingChange, int32& p_MMRChange, uint8 p_Bracket, uint32 p_Winner);
        void LooserRatingChanges(uint32 p_OwnMMR, uint32 p_OpponentMMR, int32& p_RatingChange, int32& p_MMRChange, uint8 p_Bracket, uint32 p_Winner, uint64 p_OnlyGuid = 0);

        virtual void RepopPlayerAtGraveyard(uint64 p_GUID);
        ...
        bool AddObject(uint32 p_Type, uint32 p_Entry, Position p_Pos, Position p_Rotation /*= { }*/, uint32 p_RespawnTime /*= 0*/);
        ...
        bool IsDoorClosed(uint32 p_Type) const;
        ...
        BattlegroundBracketType GetBattlegroundBracketType() const;

        bool IsRated() const
        {
            return IsRatedBG() || (isArena() && !IsSkirmish());
        }

        void SendArenaCrowdControlSpells(Player* p_Requester, Player* p_Requested);
    ...
    private:
        ...
        // Raid Group
        GroupPtr m_BgRaids[MS::Battlegrounds::TeamsCount::Value];                   // 0 - alliance, 1 - horde
        ...
        std::unordered_map<uint64, PlayerData> m_PlayerData;
};
...
inline BattlegroundBracketType GetBracketTypeByArenaType(ArenaType p_ArenaType)
{
    switch (p_ArenaType)
    {
        case ArenaType::Arena2v2:
        ...
        case ArenaType::Arena5v5:
        default:
            return BattlegroundBracketType::Max;
    }
