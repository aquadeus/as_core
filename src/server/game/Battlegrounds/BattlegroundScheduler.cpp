    ...
    namespace Battlegrounds
    {
        ...
        namespace
        {
            ...
            static bool IsEligibleForBattleground(GroupQueueInfo const* p_Group, BattlegroundType::Type p_Type, Bracket::Id p_BracketId, Battleground const* p_Template)
            {
                ...
                bool l_AllowHealer = true;
                /// Skirmishes have changed in Legion. Healers will only be placed in 3v3 matches while DPS and Tanks may find themselves in either 2v2 or 3v3 matches (blizz note)
                if (BattlegroundType::IsArena(p_Type) && !BattlegroundType::IsRated(p_Type) &&
                    (BattlegroundType::GetArenaType(p_Type) == ArenaType::Arena2v2 || BattlegroundType::GetArenaType(p_Type) == ArenaType::Arena1v1))
                    l_AllowHealer = false;
                ...
                for (auto l_Plrs : p_Group->m_Players)
                {
                    ...
                    if (!l_AllowHealer && l_Player->GetRoleForGroup() == ROLE_HEALER)
                        return false;
                }
            ...
            static bool AreMatching(GroupQueueInfo const* p_A, GroupQueueInfo const* p_B)
            {
                ...
                float l_MmrASteps = floor(float((getMSTime() - p_A->m_JoinTime) / 60));
                float l_MmrBSteps = floor(float((getMSTime() - p_B->m_JoinTime) / 60));

                uint32 l_MmrAMaxDiff = l_MmrASteps * 150;
                uint32 l_MmrBMaxDiff = l_MmrBSteps * 150;

                uint32 l_DiscardTime = sWorld->GetGameTime() - (sBattlegroundMgr->GetRatingDiscardTimer() / TimeConstants::IN_MILLISECONDS);
        ...
        GroupQueueInfo* BattlegroundScheduler::AddGroup(Player* p_Leader, GroupPtr p_Group, BattlegroundType::Type p_BgTypeId, uint32 p_BlackWishes[2], MS::Battlegrounds::Bracket const*  p_BracketEntry, ArenaType p_ArenaType, bool p_IsRatedBG, uint32 p_ArenaRating, uint32 p_MatchmakerRating, bool p_IsSkirmish)
        {
            ...
            switch (p_BgTypeId)
            {
                ...
                case BattlegroundType::ArenaSkirmish1v1:
                    l_GroupQueue->m_WantedBGs = BattlegroundMasks::ArenaSkirmish1v1;
                    break;
                case BattlegroundType::ArenaSkirmish2v2:
                    l_GroupQueue->m_WantedBGs = BattlegroundMasks::ArenaSkirmish2v2 | BattlegroundMasks::ArenaSkirmish3v3;
                    break;
                default:
                ...
                p_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
                {
                    ...
                    if (l_GroupQueue->m_Team != l_Member->GetTeam())
                    {
                        if (l_Member->GetTeam() == ALLIANCE)
                            l_Member->CastSpell(l_Member, MercenarySpells::MercenaryContractHorde);
                        else
                            l_Member->CastSpell(l_Member, MercenarySpells::MercenaryContractAlliance);
                    }
...
#pragma region Internal
                ...
                for (GroupQueueInfo* l_Group : m_QueuedGroups[p_BracketId][l_Team])
                {
                    ...
                    for (std::size_t i = 0; i < BattlegroundType::Max * 2; i += 2)
                    {
                        ...
                        else if (BattlegroundType::IsRated(l_BgType))
                        {
                            ...
                            if (l_NumberOfPlayersRequired > l_Group->m_Players.size() && !sBattlegroundMgr->isTesting())
                                continue;
        ...
        bool BattlegroundScheduler::TryCreateBattleground(BattlegroundType::Type p_DecidedBg, std::vector<std::list<GroupQueueInfo*>>& p_PotentialGroups, std::size_t p_BracketId)
        {
                ...
                for (GroupQueueInfo* l_Group : l_Groups)
                {
                    ...
                    if (l_Previous && AreMatching(l_Previous, l_Group))
                    {
                        ...
                        if (BattlegroundType::IsArena(p_DecidedBg))
                            l_RatedBg = static_cast<BattlegroundType::Type>(urand(BattlegroundType::FirstRandomArena, BattlegroundType::BlackrookHoldArena));
                        else
                            l_RatedBg = static_cast<BattlegroundType::Type>(urand(BattlegroundType::AlteracValley, BattlegroundType::BattleForGilneas));

                        Battleground* l_Template = nullptr;

                        /// We get the battleground template.
                        uint8 l_Count = 50;
                        do
                        {
                            l_Template = sBattlegroundMgr->GetBattlegroundTemplate(l_RatedBg);
                            ...
                            --l_Count;
                        } while (l_Template == nullptr && l_Count > 0);
            ...
            {
                BattlegroundType::Type l_Arena = static_cast<BattlegroundType::Type>(urand(BattlegroundType::FirstRandomArena, BattlegroundType::BlackrookHoldArena));
                ...
                do
                {
                    l_Template = sBattlegroundMgr->GetBattlegroundTemplate(l_Arena);
                    if (l_Template != nullptr)
                        break;

                    l_Arena = static_cast<BattlegroundType::Type>(urand(BattlegroundType::FirstRandomArena, BattlegroundType::BlackrookHoldArena));
