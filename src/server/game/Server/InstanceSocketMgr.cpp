...
/**
* This is a helper class to RealmSocketMgr, that manages
...
InstanceSocketMgr::InstanceSocketMgr() :
    m_NetThreads(0),
    m_NetThreadsCount(0),
    m_SockOutKBuff(-1),
...
int
InstanceSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
...
int
InstanceSocketMgr::StartNetwork (ACE_UINT16 port, const char* address)
{
...
int
InstanceSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            sLog->outError(LOG_FILTER_GENERAL, "InstanceSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            sLog->outError(LOG_FILTER_GENERAL, "InstanceSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
