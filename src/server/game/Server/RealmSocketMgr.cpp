...
int
RealmSocketMgr::StartReactiveIO (ACE_UINT16 port, const char* address)
{
...
int
RealmSocketMgr::StartNetwork (ACE_UINT16 port, const char* address)
{
...
int
RealmSocketMgr::OnSocketOpen (WorldSocket* sock)
{
    ...
    if (m_SockOutKBuff >= 0)
    {
        ...
        {
            sLog->outError(LOG_FILTER_GENERAL, "RealmSocketMgr::OnSocketOpen set_option SO_SNDBUF");
            return -1;
        }
    }
    ...
    if (m_UseNoDelay)
    {
        if (sock->peer().set_option (ACE_IPPROTO_TCP,
            ...
            sizeof (int)) == -1)
        {
            sLog->outError(LOG_FILTER_GENERAL, "RealmSocketMgr::OnSocketOpen: peer().set_option TCP_NODELAY errno = %s", ACE_OS::strerror (errno));
            return -1;
        }
    }
