...
int IRSocket::Handle_TunneledPacket(WorldPacket* packet)
{
    ...
    *packet >> opcodeId;
    ...
    {
        if (WorldSession* l_Session = l_Player->GetSession())
        {
            if (!l_Session->m_IsInDestructor)
            {
                l_Session->m_IsSendingPacketFromIRSocket = true;
                l_Session->SendPacket(packet, false, true);
                l_Session->m_IsSendingPacketFromIRSocket = false;
            }
        }
    }
