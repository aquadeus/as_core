...
class LoginQueryHolder : public SQLQueryHolder
{
    ...
    public:
        LoginQueryHolder(uint32 accountId, uint64 guid, uint32 realmId, std::function<void()> p_Callback = [](){})
            : m_accountId(accountId), m_guid(guid), m_realmId(realmId), m_Callback(p_Callback) { }
...
{
    DB2_REPLY_SPARSE                        = 0x919BE54E,   // Hash of ItemSparse.db2
...
namespace AccountCustomFlags
{
    enum
    {
        NoChatLocaleFiltering = 0x01,
        FreePremadeWoTLK      = 0x02,
        FreePremadeCataclysm  = 0x04,
        FreePremadeMoP        = 0x08,
        FreePremadeWoD        = 0x10,
        FreePremadeLegion     = 0x20,
    };
}
...
class CharacterCreateInfo
{
    ...
    public:
        CharacterCreateInfo(std::string name, uint8 race, uint8 cclass, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 outfitId, uint8 tattoos,
        uint8 hornStyle, uint8 blindfolds, uint32 templateId,
        ...
        TemplateId(templateId), Data(data), CharCount(0), Tattoos(tattoos), Blindfolds(blindfolds), HornStyle(hornStyle)
#else /* CROSS */
    public:
        CharacterCreateInfo(std::string name, uint8 race, uint8 cclass, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 outfitId, uint8 tattoos, uint8 hornStyle, uint8 blindfolds, uint32 templateId) :
            Name(name), Race(race), Class(cclass), Gender(gender), Skin(skin), Face(face), HairStyle(hairStyle), HairColor(hairColor), FacialHair(facialHair), OutfitId(outfitId), Tattoos(tattoos), Blindfolds(blindfolds), HornStyle(hornStyle),
...
class WorldSession
{
    public:
#ifndef CROSS
        WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, bool p_IsAnimator, bool ispremium, uint8 premiumType, uint8 expansion, time_t mute_time, LocaleConstant locale,
            uint32 recruiter, bool isARecruiter, uint32 p_ServiceFlags, uint32 p_CustomFlags, uint32 p_LastMacAddr);
#else /* CROSS */
        ...
        void SetPlayerLoading(uint64 val) { m_playerLoading = val; }
        ...
        uint64 PlayerLoading() const { return m_playerLoading; }
        bool PlayerLogout() const { return m_playerLogout; }
        bool PlayerLogoutWithSave() const { return m_playerLogout && m_playerSave; }
        bool PlayerRecentlyLoggedOut() const { return m_playerRecentlyLogout; }

#ifndef CROSS
        void AddInstanceConnection(WorldSocket* sock) { m_Socket[CONNECTION_TYPE_INSTANCE] = sock; }
#endif // !CROSS
        ...
        void SendPackets(std::vector<WorldPacket*>& p_Packets, ConnectionType p_ConnIdx);
        ...
        void SendSetPhaseShift(const std::set<uint32> & p_PhaseIds, const std::set<uint32>& p_TerrainSwaps, const std::set<uint32>& p_InactiveTerrainSwap, const std::set<uint32>& p_UIWorldMapAreaIDSwaps = {});
        void SendQueryTimeResponse();

        static void WriteAuthResponse(WorldPacket& p_Data, uint8 code, bool queued, uint32 queuePos = 0, uint32 p_WaitTime = 0);
        void SendClientCacheVersion(uint32 version);

        void SendAvailableHotfixes();

        AccountTypes GetSecurity() const { return _security; }
        bool IsAnimator() const { return m_IsAnimator; }
        ...
        void LoadTutorialsData(PreparedQueryResult p_Result);
        ...
        void SendAuctionCommandResult(AuctionEntryPtr p_Auction, uint32 p_Action, uint32 p_Error, uint32 p_BidError = 0);
        void SendAuctionOutBidNotification(AuctionEntryPtr p_AuctionEntry, Item const* p_Item);
        void SendAuctionClosedNotification(AuctionEntryPtr p_AuctionEntry, float p_MailDelay, bool p_Sold, Item const* p_Item);
        void SendAuctionWonNotification(AuctionEntryPtr p_AuctionEntry, Item const* p_Item);
        void SendAuctionOwnerBidNotification(AuctionEntryPtr p_AuctionEntry, Item const* p_Item);
        ...
        void BuildPartyMemberStatsChangedPacket(Player* p_Player, WorldPacket* p_Data, uint32 p_Mask, bool p_Ennemy = false, uint64 p_TargetGUID = 0);
        ...
        // Locales
        LocaleConstant GetSessionDb2Locale() const { return m_sessionDbcLocale; }
        ...
        void FinializePlayerLoading(LoginQueryHolder* p_CharacterHolder, LoginDBQueryHolder* p_LoginHolder);
        ...
        uint32 GetMacAddr() const { return m_LastMacAddr; }

        std::atomic_bool m_IsInDestructor;
        std::atomic_bool m_IsSendingPacketFromIRSocket;
    ...
    public:                                                 // opcodes handlers
        ...
        void HandleInspectPVPStatsOpcode(WorldPacket& recvPacket);
        ...
        void HandleReplicateItems(WorldPacket& p_RecvData);
        ...
        void SendCalendarRaidLockout(InstanceSavePtr save, bool add);
        void SendCalendarRaidLockoutUpdated(InstanceSavePtr save);
        ...
        void HandleSuspendTokenResponse(WorldPacket& p_RecvData);
        ...
        void HandleInstanceLockResponseOpcode(WorldPacket& p_RecvData);
        ...
        void HandleChangeBagSlotFlagOpcode(WorldPacket& p_RecvPacket);
        void HandleChangeBankBagSlotFlagOpcode(WorldPacket& p_RecvPacket);
        void HandleSetBackpackAutoSortDisable(WorldPacket& p_RecvPacket);
        ...
        void HandleGossipTalkOpcode(WorldPacket& recvPacket);
        ...
        void HandleLearnPvPTalents(WorldPacket& p_RecvPacket);
        ...
        void HandleQueryQuestRewards(WorldPacket& recvPacket);
        ...
        void SendCorpseLocation(uint64 p_Guid);
        ...
        void HandleSceneCompleteOpcode(WorldPacket& p_Packet);
        ...
        void HandlePrestige(WorldPacket& p_RecvData);
        void HandleRequestCrowdControlSpell(WorldPacket& p_RecvData);
        ...
        void SendLfgListJoinResult(LFGListEntryPtr const p_Entry, uint8 p_Error);
        void SendLfgSearchResponse(uint32 p_ActivityCategory, uint32 p_ActivitySubCategory, std::string p_FilterString);
        void SendLfgListApplyForGroupResult(LFGListEntryPtr const p_LFGEntry, LFGListEntry::LFGListApplicationEntryPtr const p_Application);
        void SendLfgListApplicantGroupInviteResponse(LFGListEntry::LFGListApplicationEntryPtr const p_Applicant);

        static void BuildLfgListRideTicket(WorldPacket* p_Data, LFGListEntryPtr const p_Entry);
        static void BuildLfgListApplicationRideTicket(WorldPacket* p_Data, LFGListEntry::LFGListApplicationEntryPtr const p_Entry);
        static void BuildLfgListEntryInfo(WorldPacket* p_Data, LFGListEntryPtr const p_Entry);
        static void BuildLfgListJoinRequest(WorldPacket* p_Data, LFGListEntryPtr const p_Entry);
        static void BuildLfgListQueueUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, bool p_Listed);
        static LFGListEntryPtr ReadLfgListRideTicketInfo(WorldPacket* p_Data, uint32* l_ID);
        static LFGListEntry::LFGListApplicationEntryPtr ReadLfgListApplicantRideTicketInfo(WorldPacket* p_Data, LFGListEntryPtr p_ReferenceEntry, uint32* l_ID);
        static LFGListEntry::LFGListApplicationEntryPtr ReadLfgListApplicantRideTicketInfo(WorldPacket* p_Data, uint32* l_ID);
        static void BuildLfgListApplicantListUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, LFGListEntry::LFGListApplicationEntryPtr const p_Applicant);
        static void BuildLfgListApplicantListUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, std::list<LFGListEntry::LFGListApplicationEntryPtr> p_ApplicantList);
        static void BuilfLfgListApplicantGroupInvite(WorldPacket* p_Data, LFGListEntry::LFGListApplicationEntryPtr const p_Applicant);
        ...
        void HandleGuildBankSwapItemsLegacy(WorldPacket& recvData);

        void HandleGuildBankMoveItemsPlayerBank(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsBankPlayer(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsBankBank(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsPlayerBankCount(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsBankPlayerCount(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsBankPlayerAuto(WorldPacket& p_RecvData);
        void HandleGuildBankMoveItemsBankBankCount(WorldPacket& p_RecvData);
        void HandleGuildBankMergeItemsPlayerBank(WorldPacket& p_RecvData);
        void HandleGuildBankMergeItemsBankPlayer(WorldPacket& p_RecvData);
        void HandleGuildBankMergeItemsBankBank(WorldPacket& p_RecvData);
        void HandleGuildBankSwapItemsBankPlayer(WorldPacket& p_RecvData);
        void HandleGuildBankSwapItemsBankBank(WorldPacket& p_RecvData);
        ...
        /// Barber
        void HandleAlterAppearance(WorldPacket& p_RecvData);
        void SendBarberShopResult(BarberShopResult p_Result);

        void SendCharCustomize(ResponseCodes p_Error, uint64 p_Guid);
        void SendCharFactionChange(ResponseCodes p_Error, uint64 p_Guid);
        ...
        void HandleRequestWorldQUestUpdate(WorldPacket& p_RecvData);
        void HandleQuestPOIQuery(WorldPacket& recvData);
        void HandleEjectPassenger(WorldPacket& data);
        void HandleEnterPlayerVehicle(WorldPacket& data);
        void HandleDBQueryBulk(WorldPacket& recvPacket);
        void HandleHotfixRequest(WorldPacket& p_RecvPacket);
        ...
        void HandleContributionGetState(WorldPacket& p_RecvPacket);
        void HandleContributionContribute(WorldPacket& p_RecvPacket);
        ...
        /// Artifact
        void HandleArtifactLearnTrait(WorldPacket& p_RecvData);
        void HandleArtifactConfirmRespec(WorldPacket& p_RecvData);
        void HandleArtifactSetAppearance(WorldPacket& p_RecvData);
        void HandleArtifactAddRelicTalent(WorldPacket& p_RecvData);
        void HandleArtifactAttunePreviewRelic(WorldPacket& p_RecvData);
        void HandleArtifactAttuneSocketedRelic(WorldPacket& p_RecvData);
        void SendArtifactRespec(uint64 p_Guid);
        void SendOpenArtifactForge(uint64 p_ItemGUID, uint64 p_ForgeGUID);

        /// Challenge
        void HandleGetChallengeModeRewards(WorldPacket& p_Packet);
        void HandleChallengeModeRequestMapStats(WorldPacket& p_Packet);
        void HandleRequestLeaders(WorldPacket& p_Packet);
        void HandleStartChallengeMode(WorldPacket& p_Packet);
        void HandleResetChallengeMode(WorldPacket& p_Packet);

        /// Area POI
        void HandleRequestAreaPOIUpdate(WorldPacket& recvPacket);

        //////////////////////////////////////////////////////////////////////////
        /// Garrison
        //////////////////////////////////////////////////////////////////////////
        void HandleGetGarrisonInfoOpcode(WorldPacket& p_RecvData);
        void HandleRequestGarrisonRequestBlueprintAndSpecializationDataOpcode(WorldPacket& p_RecvData);
        ...
        void SendGetGarrisonInfoResponse();

        void HandleGarrisonGetBuildingLandmarkOpcode(WorldPacket& p_RecvData);
        ...
        void HandleGarrisonResearchTalent(WorldPacket& p_RecvData);
        ...
        void HandleGarrisonRemoveFollowerOpcode(WorldPacket& p_RecvData);
        void HandleDisplayPlayerChoiceResponse(WorldPacket& p_Packet);

        void SendGarrisonOpenArchitect(uint64 p_CreatureGUID);
        void SendGarrisonOpenMissionNpc(uint64 p_CreatureGUID);
        void SendGarrisonSetMissionNpc(uint64 p_CreatureGUID, uint32 p_FollowerType);
        ...
        /// ToyBox
        //////////////////////////////////////////////////////////////////////////
        void HandleAddToyOpcode(WorldPacket& p_RecvData);
        void HandleUseToyOpcode(WorldPacket& p_RecvData);

        void HandleQueryScenarioPOI(WorldPacket& p_RecvData);
        ...
        /// Opens the adventure map
        void SendAdventureMapOpen(uint64 p_GUID);
        void HandleAdventureMapPOIEnabledQuery(WorldPacket& p_RecvData);
        void HandleAdventureMapStartQuest(WorldPacket& p_RecvData);
...
#ifdef CROSS
        /// Cross related
        void LoadCharacter(std::function<void()> p_Callback = [](){});
        ...
        void SetDummySession() { m_IsDummySession = true; }
        bool IsDummySession() { return m_IsDummySession; }
        ...
        uint32 GetLoyaltyEventCount(LoyaltyEvent p_Event) const { return m_LoyaltyEvents[(uint32)p_Event]; }
        void   AddLoyaltyPoints(uint32 p_Count, std::string p_Reason);
        void   AddFirestormPoints(uint32 p_Count, std::string p_Reason);
        void   FetchFirestormPoints(std::function<void(PreparedQueryResult)> p_Callback);
        void   SaveCosmeticPurchase(CosmeticEntry p_CosmeticEntry);
        void   SetLastClaim(time_t p_ClaimTime) { m_LastClaim = p_ClaimTime; }
        void   CompleteLoyaltyEvent(LoyaltyEvent p_Event);

        std::list<uint32> m_LastSpellCasted;

        FunctionProcessor m_Functions;

        void ProcessQueryCallbacks();

        bool HasRecentSpellPacket() { return m_HasRecentSpellPacket; }

        void SaveCosmeticLootboxPurchase(uint32 p_Amount, uint32 p_Cost);
        void BuildLootBoxShopEntryData(std::stringstream& l_ShopEntryData, uint32 p_Amount);
        ...
        void SetforceExit(bool force = true)
        {
            forceExit = force;
            if (forceExit && !forceExitTime)
                forceExitTime = getMSTime();
        }
        bool IsforceExit() { return forceExit; }

        void SetTrustedState() { m_Trusted = true; }
        bool IsTrusted() const { return m_Trusted; }

        void SetChangelogReceived();
        bool HasReceivedChangelog();
    ...
    private:
        ...
        bool m_IsInWorldUpdateSession;
        bool m_HasRecentSpellPacket;
        ...
        std::list<Channel*> m_ChannelsSave;
#endif
    ...
    private:
        ...
        uint64 m_RealGUID;
#else
        WorldSocket* m_Socket[MAX_CONNECTION_TYPES];
#endif
        bool m_CanResumCommms;

        uint32 forceExitTime;
        bool forceExit;
        uint32 expireTime;
        ...
        uint64 m_playerLoading;                             // code processed in LoginPlayer
        ...
        uint32 recruiterId;
        bool isRecruiter;
        ACE_Based::LockedQueue<WorldPacket*, ACE_Null_Mutex> m_RecvQueue;
        moodycamel::ConcurrentQueue<WorldPacket*> m_ConcurrentRecvQueue;
        ...
        time_t m_TimeLastAddFriend;
        time_t m_TimeLastAddIgnoreFriend;
        time_t m_TimeLastGarrisonInfo;
        time_t m_TimeLastMoveBankGuildItems;
        time_t m_TimeLastLogout;
        ...
        uint32 m_LastMacAddr;

        bool m_Trusted;

        bool m_ChangelogReceived;

        bool m_IsDummySession;

        bool m_IsUpdating;

        std::queue<std::pair<uint32_t, uint32_t>> m_HotfixQueue;
};
#endif
/// @}
