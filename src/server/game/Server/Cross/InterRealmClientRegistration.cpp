...
void SkipPlayerBlock(WorldPacket* packet)
{
    ...
    *packet >> temp;      // playerName
    ...
    packet->read_skip((uint8)BattlegroundBracketType::Max * 4 * 9);    ///< PlayerArenaInfo block
}
...
void ReadOrSkipArenaInfoBlock(WorldPacket& packet, Player* player = NULL)
{
    if (player == NULL)
    {
        packet.read_skip((uint8)BattlegroundBracketType::Max * 4 * 9);
        return;
    }
...
Player* InterRealmClient::CreatePlayer(WorldPacket& packet, uint64 playerGuid)
{
    ...
    packet >> guildLevel;
    packet >> bg_status;
    /// @TODO cross createInfo
    CharacterCreateInfo createInfo(playerName, playerRace, playerClass, playerGender, playerBytes1 & 0xFF, (playerBytes1 >> 8) & 0xFF, (playerBytes1 >> 16) & 0xFF, (playerBytes1 >> 24) & 0xFF, playerBytes2 & 0xFF, 0, 0, 0 ,0, 0);

    Player* player = new Player(session);

    sLog->outInfo(LOG_FILTER_POINTERS, "Pointer 0x%" PRIxPTR " -> Player Guid: %u", std::uintptr_t(player), GUID_LOPART(playerGuid));

    if (!player->Create(GUID_LOPART(playerGuid), &createInfo))
    {
        printf("Can't create player %lu\r\n", playerGuid);
        packet.read_skip((uint8)BattlegroundBracketType::Max * 4 * 9);
        delete session;
        delete player;
        return NULL;
    ...
    sRecruitAFriendMgr->AddEvent(eRecruitAFriendMgrEvents::EventPlayerLogin, player->GetRealGUID());
...
void InterRealmClient::Handle_AppearRequest(WorldPacket& p_Packet)
{
    uint64 l_PlayerGuid;
    uint64 l_TargetGuid;

    p_Packet >> l_PlayerGuid;
    p_Packet >> l_TargetGuid;
    ...
    if (sWorld->HasPlayer(l_PlayerGuid))
    {
        sLog->outError(LOG_FILTER_INTERREALM, "Failed to appear because player is already at the cross-server.");
        p_Packet.rfinish();
        return;
    }

    Player* l_TargetPlayer = sObjectAccessor->FindPlayer(l_TargetGuid);
    if (!l_TargetPlayer || l_TargetPlayer->IsNeedRemove())
    {
        sLog->outError(LOG_FILTER_INTERREALM, "Failed to appear because targeted player is not at the cross-server.");
        p_Packet.rfinish();
        return;
    }

    if (!l_TargetPlayer->GetBattlegroundId() || !l_TargetPlayer->GetBattleground())
    {
        sLog->outError(LOG_FILTER_INTERREALM, "Failed to appear because targeted player is not at the arena.");
        p_Packet.rfinish();
        return;
    }

    Player* l_Player = CreatePlayer(p_Packet, l_PlayerGuid);
    if (!l_Player)
    {
    ...
    uint32 l_BgZoneId = l_TargetPlayer->GetBattleground()->GetZoneId();

    sObjectAccessor->AddObject(l_Player);
    m_Players[l_Player->GetRealGUID()] = l_Player;
    sWorld->AddPlayer(l_Player);

    uint32 l_InterRealmClientId = GetRealmId();

    l_Player->GetSession()->LoadCharacter([l_TargetGuid, l_PlayerGuid, l_BgZoneId, l_InterRealmClientId]() -> void
    {
        InterRealmClient* l_IRClient = sInterRealmMgr->GetClientByRealmNumber(l_InterRealmClientId);
        if (!l_IRClient)
            return;

        WorldPacket l_Data(IR_SMSG_APPEAR_REQUEST_RESP);
        l_Data << uint64(l_PlayerGuid);
        l_Data << uint64(l_TargetGuid);
        l_Data << uint8(IR_REG_OK);
        l_Data << uint32(l_BgZoneId);
        l_IRClient->SendPacket(&l_Data);
    });
}
...
void InterRealmClient::Handle_BattlefieldPort(WorldPacket& packet)
{
    ...
    printf("Receive IR_CMSG_BATTLEFIELD_PORT\r\n");
    ...
    switch (l_Action)
    {
        ...
        case 1:
        {
            ...
            l_Player->SetInRandomBG(l_GroupQueueInfo.m_IsRandom);

            uint32 l_InvitedToBGInstanceGUID = l_GroupQueueInfo.m_IsInvitedToBGInstanceGUID;
            uint32 l_ZoneId = l_BG->GetZoneId();
            bool   l_RandomBG = l_GroupQueueInfo.m_IsRandom;

            WorldSession* l_Session = l_Player->GetSession();

            auto l_Callback = [l_PlayerGuid, l_InvitedToBGInstanceGUID, l_BGQueueTypeID, l_ZoneId, l_RandomBG, l_Session]() -> void
            {
                CharacterPortData l_PortData;
                l_PortData.playerGuid = l_PlayerGuid;
                l_PortData.IsInvitedToBGInstanceGUID = l_InvitedToBGInstanceGUID;
                l_PortData.bgTypeId = l_BGQueueTypeID;
                l_PortData.bgZoneId = l_ZoneId ? l_ZoneId : 1;

                Player* l_Player = sObjectAccessor->FindPlayerInOrOutOfWorld(l_PlayerGuid);
                if (l_Player)
                    l_Player->SetInRandomBG(l_RandomBG);

                l_Session->GetInterRealmClient()->SendBattlefieldPort(l_PortData);
            };

            l_Player->GetSession()->LoadCharacter(l_Callback);
...
{
    printf("Received packet IR_CMSG_BATTLEFIELD_ENTER\r\n");
    ...
    if (Player* l_Player = sObjectAccessor->FindPlayerInOrOutOfWorld(l_PlayerGuid))
    {
        ...
        printf("IR_CMSG_BATTLEFIELD_ENTER: Teleport player to bg\r\n");
...
{
    printf("Send IR_SMSG_BATTLEFIELD_PORT_RESP\r\n");
