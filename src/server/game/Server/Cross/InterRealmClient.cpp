...
{
    sLog->outDebug(LOG_FILTER_INTERREALM, "Received packet CMSG_HELLO");
    ...
    sLog->outDebug(LOG_FILTER_INTERREALM, "Send packet SMSG_HELLO");
...
void InterRealmClient::Handle_WhoAmI(WorldPacket& packet)
{
    ...
    sLog->outDebug(LOG_FILTER_INTERREALM, "Received packet CMSG_WHO_AM_I");
    ...
    InterRealmDatabase.SetConnectionInfo(config->connection_info, 10, 8);

    if (InterRealmDatabase.Open() || !InterRealmDatabase.PrepareStatements())
    {
        printf("Database connection was failed for InterRealmClient %u.\r\n", m_realmId);
        pckt << uint8(1);
        SendPacket(&pckt);
        m_isNeedClose = true;
        return;
    ...
    printf("Database connection was successful. %lu\r\n", time(nullptr));
...
void InterRealmClient::Handle_CheckPlayers(WorldPacket& packet)
{
    ...
    if (num_players > 0)
    {
        for (uint16 i = 0; i < num_players; ++i)
        {
            uint64 playerGuid;
            packet >> playerGuid;
            sLog->outDebug(LOG_FILTER_INTERREALM, "Connection lost with player %lu", playerGuid);
...
void InterRealmClient::SendInvitedForBattlegroundQueueType(uint64 playerGUID, uint32 p_QueueID, uint32 bgQueueTypeId)
{
    WorldPacket pckt(IR_SMSG_BATTLEGROUND_INVITED, 16);
    pckt << uint64(playerGUID);
    pckt << uint32(p_QueueID);
    pckt << uint32(bgQueueTypeId);
    SendPacket(&pckt);
}

void InterRealmClient::SendBattlegroundLeaveQueue(uint64 p_PlayerGUID, int32 p_RatingChange /*= 0*/, int32 p_MMRChange /*= 0*/, uint8 p_Slot /*= 0*/, bool p_Deserter /*= true*/)
{
    WorldPacket pckt(IR_SMSG_BATTLEGROUND_LEAVE_QUEUE, 18);
    pckt << uint64(p_PlayerGUID);
    pckt << int32(p_RatingChange);
    pckt << int32(p_MMRChange);
    pckt << uint8(p_Slot);
    pckt << bool(p_Deserter);
    SendPacket(&pckt);
}
...
void InterRealmClient::Handle_BattlegroundListQuery(WorldPacket& packet)
{
    ...
    WorldPacket pckt;
    MS::Battlegrounds::PacketFactory::List(&pckt, _playerGuid, sObjectAccessor->FindPlayer(_playerGuid), BattlegroundTypeId(_bgTypeId), 0, false);
...
void InterRealmClient::Handle_SummonPlayer(WorldPacket& recvPacket)
{
    ...
    Player* pPlayer = sObjectAccessor->FindPlayerInOrOutOfWorld(l_PlayerGuid);
    if (!pPlayer)
        return;
    ...
    pPlayer->SetBattlegroundEntryPoint(l_WorldLocation);

    if (Battleground* bg = pPlayer->GetBattleground())
    {
        bg->RemovePlayerAtLeave(l_PlayerGuid, true, true);
        bg->RemoveFromInterRealm(l_PlayerGuid);
    }
}
...
void InterRealmClient::Handle_RAFEvent(WorldPacket& p_Packet)
{
