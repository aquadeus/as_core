    ...
    if (IsWarTime())
    {
        ...
        if (player->GetTeamId() < 2)
        {
            if (m_PlayersInWar[player->GetTeamId()].find(player->GetGUID()) != m_PlayersInWar[player->GetTeamId()].end())
            {
                ...
                if (GroupPtr group = player->GetGroup()) // Remove the player from the raid group
...
// ******************* Group System *******************
// ****************************************************
GroupPtr Battlefield::GetFreeBfRaid(TeamId TeamId)
{
    for (GuidSet::const_iterator itr = m_Groups[TeamId].begin(); itr != m_Groups[TeamId].end(); ++itr)
        if (GroupPtr group = sGroupMgr->GetGroupByGUID(*itr))
            if (!group->IsFull())
                return group;
...
{
    //Get map object
    uint32 l_Zone = sMapMgr->GetZoneId(m_MapId, x, y, z);
    Map* map = const_cast < Map * >(sMapMgr->CreateBaseMap(m_MapId, l_Zone));
    if (!map)
    {
...
{
    // Get map object
    uint32 l_Zone = sMapMgr->GetZoneId(571, x, y, z);
    Map* map = const_cast<Map*>(sMapMgr->CreateBaseMap(571, l_Zone)); // *vomits*
    if (!map)
        return 0;
