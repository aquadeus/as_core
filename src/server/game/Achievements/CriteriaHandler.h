...
struct CriteriaProgress
{
    ...
    uint64 PlayerGUID;                                  // GUID of the player that last updated the criteria
    bool Changed = false;
};
...
{                                                 // value1         value2        comment
    ...
    // REUSE
    CRITERIA_DATA_TYPE_S_KNOWN_TITLE        = 23, // TitleId                      known (pvp) title, values from db2
...
public:
    ...
    void RemoveCriteriaProgress(Criteria const* criteria, bool p_Timed = false);
...
protected:
    ...
    virtual void SendCriteriaProgressRemoved(uint32 p_ID, Criteria const* p_Criteria = nullptr, CriteriaProgress const* p_Progress = nullptr) = 0;
...
public:
    ...
    void LoadAccountCriteriaList();

    bool IsAccountCriteria(uint32 p_CriteriaID)
    {
        return _AccountCriteriaID.find(p_CriteriaID) != _AccountCriteriaID.end();
    }
...
private:
    void LoadAccountCriteriaList(AchievementEntry const* p_Achievement);
    void LoadAccountCriteriaList(CriteriaTree const* p_Tree);
...
private:
    ...
    std::set<uint32> _AccountCriteriaID;
