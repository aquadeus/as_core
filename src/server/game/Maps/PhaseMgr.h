...
struct PhaseInfo
{
    PhaseInfo() : phasemask(0), terrainswapmap(0), phaseId(0), uiworldmaparea(0) {}
...
struct PhaseData
{
    PhaseData(Player* _player) : _PhasemaskThroughDefinitions(0), _PhasemaskThroughAuras(0), player(_player) {}
    ...
    void ClearActivePhasesIDs() { m_ActivePhasesIDs.clear(); }
    void RemoveActivePhaseID(uint32 p_PhaseID) { m_ActivePhasesIDs.erase(p_PhaseID); }
    void AddActivePhaseID(uint32 p_PhaseID) { m_ActivePhasesIDs.insert(p_PhaseID); }
    std::set<uint32> const& GetActivesPhases() const { return m_ActivePhasesIDs; }

private:
    Player* player;
    std::list<PhaseDefinition const*> activePhaseDefinitions;
    PhaseInfoContainer spellPhaseInfo;
    std::set<uint32> m_ActivePhasesIDs;
};
...
public:
    ...
    static std::map<uint32, uint32> g_PhaseIdToBitIndex;
    static uint32 g_LastBitMapped;
    inline static uint32 GetPhaseBitIndex(uint32 p_PhaseId);
    static uint32 GetLastPhaseBit() { return g_LastBitMapped; }
    ...
    std::set<uint32> const& GetActivesPhases() const { return phaseData.GetActivesPhases(); }
