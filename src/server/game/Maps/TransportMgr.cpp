...
void TransportMgr::LoadTransportTemplates()
{
    ...
    do
    {
        ...
        if (transport.inInstance && !transport.mapsUsed.empty())
...
void TransportMgr::GeneratePath(GameObjectTemplate const* goInfo, TransportTemplate* transport)
{
    ...
    if (cyclic)
    {
        std::vector<KeyFrame> realFrames;
        for (uint32 l_I = 1; l_I < keyFrames.size() - 2; ++l_I)
            realFrames.push_back(keyFrames[l_I]);

        keyFrames = realFrames;
    }
...
*/
    ...
    if (cyclic)
    {
        ...
        spline->init_cyclic_spline(&splinePath[1], splinePath.size() - 3, Movement::SplineBase::ModeCatmullrom, 0);
        spline->initLengths();
        for (size_t i = 0; i < keyFrames.size(); ++i)
        {
            ...
            if (i)
                ...
                keyFrames[i].DistFromPrev = spline->length(spline->last() - 2, spline->last() - 1);
    ...
    {
        if (neverStop)
            keyFrames[i].TimeTo = (keyFrames[i].DistUntilStop / speed);
        else
        {
    ...
    for (size_t i = 0; i < keyFrames.size(); ++i)
    {
        int32 j = (i + lastStop + keyFrames.size()) % keyFrames.size();
        if (keyFrames[j].IsStopFrame() || j == lastStop)
    ...
    for (size_t i = 1; i < keyFrames.size(); ++i)
    {
        ...
        if (keyFrames[i].IsStopFrame())
        {
            ...
            curPathTime += static_cast<float>(keyFrames[i].Node->Delay);
    ...
    if (cyclic)
        keyFrames.back().NextArriveTime = keyFrames.back().ArriveTime + uint32(keyFrames.back().TimeTo * IN_MILLISECONDS);
    else
        keyFrames.back().NextArriveTime = keyFrames.back().DepartureTime;

    transport->pathTime = keyFrames.back().NextArriveTime;
...
Transport* TransportMgr::CreateTransport(uint32 entry, uint32 guid /*= 0*/, Map* map /*= NULL*/)
{
    ...
    trans->SetMap(map ? map : sMapMgr->CreateBaseMap(mapId, sMapMgr->GetZoneId(mapId, x, y, z)));
