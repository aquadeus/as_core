...
MapInstanced::MapInstanced(uint32 id, time_t expiry) : Map(id, expiry, 0, DifficultyNormal, 0)
{
...
void MapInstanced::Update(const uint32 m, const uint32 t)
{
...
Map* MapInstanced::CreateInstanceForPlayer(const uint32 mapId, Player* player)
{
    ...
    {
        Difficulty l_Diff           = player->GetDifficultyID(GetEntry());
        InstancePlayerBindPtr pBind = player->GetBoundInstance(GetId(), l_Diff);
        InstanceSavePtr pSave       = pBind ? pBind->save : NULL;

        bool l_IsGarrisonTransfet = i_mapEntry && (i_mapEntry->Flags & MapFlags::MAP_FLAG_GARRISON) != 0;

        // the player's permanent player bind is taken into consideration first
        // then the player's group bind and finally the solo bind.
        if (!pBind || !pBind->perm || l_Diff == Difficulty::DifficultyRaidNormal || l_Diff == Difficulty::DifficultyRaidHeroic)
        {
            InstanceGroupBindPtr groupBind = NULL;
        ...
        else
        {
            ...
            if (player->GetAppearZoneInstanceId())
                newInstanceId = player->GetAppearZoneInstanceId();
...
InstanceMap* MapInstanced::CreateInstance(uint32 InstanceId, InstanceSavePtr save, Difficulty difficulty)
{
    // load/create a map
    ...
    m_InstancedMapsLock.lock();
    m_InstancedMaps[InstanceId] = map;
    m_InstancedMapsLock.unlock();
