...
uint32 const MinNewsItemLevel[MAX_EXPANSION] = { 61, 90, 200, 353, 470, 640, 760 };
...
class Guild
{
    private:
        ...
        class GuildNewsLog
        {
            public:
                ...
                void AddNewEvent(GuildNews eventType, time_t date, uint64 playerGuid, uint32 flags, uint32 data, Item* p_Item = nullptr);
        ...
        typedef std::shared_ptr<RankInfo> RankInfoPtr;
        ...
        typedef std::shared_ptr<BankTab> BankTabPtr;
        ...
        class BankMoveItemData : public MoveItemData
        {
            ...
            private:
                Item* _StoreItem(SQLTransaction& trans, BankTabPtr pTab, Item* pItem, ItemPosCount& pos, bool clone) const;
                bool _ReserveSpace(uint8 slotId, Item* pItem, Item* pItemDest, uint32& count);
                void CanStoreItemInTab(Item* pItem, uint8 skipSlotId, bool merge, uint32& count);
        };

        typedef ACE_Based::LockedMap<uint32, MemberPtr> Members;
        typedef ACE_Based::LockedVector<BankTabPtr> BankTabs;

    public:

        typedef ACE_Based::LockedVector<RankInfoPtr> Ranks;
        ...
        void CompleteGuildChallenge(int32 p_Type, Player* p_ReferencePlayer);
        void SendItemAcquired(Player* p_Player, Item* p_Item) const;
        ...
        {
            m_members.safe_foreach(true, [&](Members::iterator& itr)
            {
        ...
        inline MemberPtr GetMember(uint64 guid)
        {
            Members::iterator itr = m_members.find(GUID_LOPART(guid));
            return itr != m_members.end() ? itr->second : nullptr;
        }

        void SetBankLoaded() { m_BankLoaded = true; }
        void SetInLoading(bool p_Value) { m_InLoading = p_Value; }

        struct GuildBankRequest
        {
            GuildBankRequest() {}
            GuildBankRequest(uint32 p_AccountId, uint8 p_TabID, bool p_WithContent, bool p_WithTabInfo) :
                AccountId(p_AccountId), TabID(p_TabID), WithContent(p_WithContent), WithTabInfo(p_WithTabInfo) {}
        ...
        ACE_Based::LockedQueue<GuildBankRequest, ACE_Thread_Mutex> m_SessionToSendBank;
    ...
    private:
        ...
        inline const RankInfoPtr GetRankInfo(uint32 rankId) const { return rankId < _GetRanksSize() ? m_ranks[rankId] : nullptr; }
        inline RankInfoPtr GetRankInfo(uint32 rankId) { return rankId < _GetRanksSize() ? m_ranks[rankId] : nullptr; }
        ...
        inline BankTabPtr GetBankTab(uint8 tabId) { return tabId < m_bankTabs.size() ? m_bankTabs[tabId] : nullptr; }
        inline const BankTabPtr GetBankTab(uint8 tabId) const { return tabId < m_bankTabs.size() ? m_bankTabs[tabId] : nullptr; }

        inline const MemberPtr GetMember(uint64 guid) const
        {
            Members::const_iterator itr = m_members.find(GUID_LOPART(guid));
            return itr != m_members.end() ? itr->second : NULL;
        }

        inline MemberPtr GetMember(WorldSession* session, const std::string& name)
        {
            MemberPtr l_Result = nullptr;

            m_members.safe_foreach(false, [&](Members::iterator& itr)
            {
                if (itr->second->GetName() == name)
                {
                    l_Result = itr->second;
                    itr = m_members.getSource().end();
                }
                else
                    itr++;
            });
        ...
        void _SetLeaderGUID(MemberPtr pLeader);
