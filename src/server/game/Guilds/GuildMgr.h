...
public:
    ...
    std::map<uint32, GuildReward> const& GetGuildRewards() const { return m_GuildRewards; }
    GuildReward const* GetGuildReward(uint32 p_ItemID) const
    {
        auto l_Iter = m_GuildRewards.find(p_ItemID);
        if (l_Iter == m_GuildRewards.end())
            return nullptr;

        return &l_Iter->second;
    }

    bool IsGuildReward(uint32 p_ItemID) const
    {
        auto l_Iter = m_GuildRewards.find(p_ItemID);
        if (l_Iter == m_GuildRewards.end())
            return false;
...
protected:
    ...
    std::map<uint32, GuildReward> m_GuildRewards;
#endif /* not CROSS */
