...
inline void Guild::LogHolder::WritePacket(WorldPacket& p_Data) const
{
    ...
    if (p_Data.GetOpcode() == SMSG_GUILD_BANK_LOG_QUERY_RESULT)
    {
        l_Check = p_Data.WriteBit(false);
        p_Data.FlushBits();
    }
...
{
    uint8 slotId     = fields[ItemInstanceField::End + 3].GetUInt8();
...
Item* Guild::BankMoveItemData::StoreItem(SQLTransaction& trans, Item* pItem)
{
    ...
    BankTabPtr pTab = m_pGuild->GetBankTab(m_container);
    if (!pTab)
        return NULL;
...
void Guild::BankMoveItemData::LogAction(MoveItemData* pFrom) const
{
    ...
    if (!pFrom->IsBank() && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE) && !AccountMgr::IsPlayerAccount(m_pPlayer->GetSession()->GetSecurity()))       // TODO: move to scripts
        sLog->outCommand(m_pPlayer->GetSession()->GetAccountId(), "", m_pPlayer->GetGUIDLow(), m_pPlayer->GetName(), 0, "", 0, "",
                        ...
                        m_pPlayer->GetName(), m_pPlayer->GetSession()->GetAccountId(), pFrom->GetItem()->GetTemplate()->Name1->Get(sWorld->GetDefaultDb2Locale()), pFrom->GetItem()->GetEntry(), pFrom->GetItem()->GetCount(), m_pGuild->GetId());
}

Item* Guild::BankMoveItemData::_StoreItem(SQLTransaction& trans, BankTabPtr pTab, Item* pItem, ItemPosCount& pos, bool clone) const
{
...
Guild::Guild() : m_id(0), m_leaderGuid(0), m_createdDate(0), m_accountsNumber(0), m_bankMoney(0), m_eventLog(NULL),
    m_achievementMgr(this), _newsLog(this), m_BankLoaded(false), m_InLoading(false)
{
...
bool Guild::Create(Player* p_Leader, const std::string & p_Name)
{
    ...
    if (!sObjectMgr->CheckFakeColorProtection(p_Name, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return false;
...
{
    if (m_name == name || name.empty() || utf8length(name) > 24 || sObjectMgr->IsReservedName(name) || !ObjectMgr::IsValidCharterName(name))
        return false;
    ...
    WorldPacket l_Data(SMSG_GUILD_NAME_CHANGED, 40);
    l_Data.appendPackGUID(GetGUID());
    l_Data.WriteBits(name.length(), 7);
    l_Data.FlushBits();
    l_Data.WriteString(name);
    BroadcastPacket(&l_Data);

    return true;
}

thread_local WorldPacket k_RosterBuffer;

void Guild::HandleRoster(WorldSession* p_Session)
{
    k_RosterBuffer.reserve(2048 * 2048);
    k_RosterBuffer.clear();

    bool l_CanViewOffNote = _HasRankRight(p_Session->GetPlayer(), GR_RIGHT_VIEWOFFNOTE);

    uint32 weeklyRepCap = uint32(sWorld->getIntConfig(CONFIG_GUILD_WEEKLY_REP_CAP)); ///< weekyRepCap is never read 01/18/16

    k_RosterBuffer.Initialize(SMSG_GUILD_ROSTER);

    k_RosterBuffer << uint32(m_accountsNumber);
    k_RosterBuffer << uint32(MS::Utilities::WowTime::Encode(m_createdDate));
    k_RosterBuffer << uint32(0);                                      ///< Guild Flags
    k_RosterBuffer << uint32(m_members.size());
    k_RosterBuffer.WriteBits(m_motd.length(), 10);                                                              ///< Welcome Text
    k_RosterBuffer.WriteBits(m_info.length(), 11);                                                              ///< Info Text
    k_RosterBuffer.FlushBits();

    m_members.safe_foreach(true, [&](Guild::Members::iterator& itr)
    {
        MemberPtr l_Member = itr->second;
        ...
        /// Avoid bad zone ID
        if (!sAreaTableStore.LookupEntry(l_ZoneId))
            l_ZoneId = 4395;    ///< Dalaran center

        k_RosterBuffer.appendPackGUID(l_Member->GetGUID());
        k_RosterBuffer << uint32(l_Member->GetRankId());
        k_RosterBuffer << uint32(l_ZoneId);
        k_RosterBuffer << uint32(l_Player ? l_Player->GetAchievementPoints() : 0);
        k_RosterBuffer << uint32(l_Player ? l_Player->GetReputation(REP_GUILD) : 0);
        k_RosterBuffer << float(l_Player ? 0.0f : float(::time(NULL) - l_Member->GetLogoutTime()) / DAY);       ///< Last Save
        ...
        for (int l_I = 0; l_I < 2; ++l_I)
        {
            ...
            if (l_ProfessionID)
            {
                k_RosterBuffer << uint32(l_ProfessionID);                                                       ///< Db ID
                k_RosterBuffer << uint32(l_Player->GetSkillValue(l_ProfessionID));                              ///< Rank
                k_RosterBuffer << uint32(l_Player->GetSkillStep(l_ProfessionID));                               ///< Step
            }
            else
            {
                k_RosterBuffer << uint32(0);                                                                    ///< Db ID
                k_RosterBuffer << uint32(0);                                                                    ///< Rank
                k_RosterBuffer << uint32(0);                                                                    ///< Step
            }
        }

        k_RosterBuffer << uint32(g_RealmID);                                                                    ///< Virtual Realm Address
        k_RosterBuffer << uint8(l_Flags);                                                                       ///< Status
        k_RosterBuffer << uint8(l_Member->GetLevel());                                                          ///< Level
        k_RosterBuffer << uint8(l_Member->GetClass());                                                          ///< Class ID
        k_RosterBuffer << uint8(l_Player ? l_Player->getGender() : 0);                                          ///< Gender

        k_RosterBuffer.WriteBits(l_Member->GetName().length(), 6);                                              ///< Name
        k_RosterBuffer.WriteBits(l_Member->GetPublicNote().length(), 8);                                        ///< Note
        k_RosterBuffer.WriteBits(l_CanViewOffNote ? l_Member->GetOfficerNote().length() : 0, 8);                ///< Officer Note
        k_RosterBuffer.WriteBit(false);                                                                         ///< @TODO Has Authenticator
        k_RosterBuffer.WriteBit(false);                                                                         ///< @TODO Can Scroll of Ressurect
        k_RosterBuffer.FlushBits();

        k_RosterBuffer.WriteString(l_Member->GetName());                                                        ///< Name
        k_RosterBuffer.WriteString(l_Member->GetPublicNote());                                                  ///< Note
        k_RosterBuffer.WriteString(l_CanViewOffNote ? l_Member->GetOfficerNote() : "");                         ///< Officer Note
    });

    k_RosterBuffer.WriteString(m_motd);                                                                         ///< Welcome Text
    k_RosterBuffer.WriteString(m_info);                                                                         ///< Info Text

    p_Session->SendPacket(&k_RosterBuffer);
...
void Guild::HandleQuery(WorldSession* session)
{
    ...
    {
        RankInfoPtr l_Rank = m_ranks[i];

        l_Data << uint32(l_Rank->GetId());
        l_Data << uint32(i);

        l_Data.WriteBits(l_Rank->GetName().size(), 7);
        l_Data.FlushBits();

        l_Data.WriteString(l_Rank->GetName());
    }
...
void Guild::HandleGuildRanks(WorldSession* p_Session) const
{
    ...
    {
        RankInfoPtr l_RankInfo = GetRankInfo(l_I);
...
void Guild::HandleSetEmblem(WorldSession* session, const EmblemInfo& emblemInfo)
{
    ...
    {
        player->ModifyMoney(-int64(EMBLEM_PRICE), "Guild::HandleSetEmblem");
        ...
        /// Update guild emblem client-side
        m_members.safe_foreach(true, [&](Guild::Members::iterator& itr)
        {
            if (Player* player = itr->second->FindPlayer())
                player->SetUInt32Value(PLAYER_FIELD_GUILD_TIME_STAMP, getMSTime());
        });

        m_achievementMgr.UpdateCriteria(CRITERIA_TYPE_BUY_GUILD_TABARD, 1, 0, 0, NULL, player);
    }
}
...
void Guild::HandleSetLeader(WorldSession* p_Session, const std::string & p_Name)
{
    ...
    else if (MemberPtr l_OldLeader = GetMember(player->GetGUID()))
    {
        /// New leader must be a member of guild
        if (MemberPtr l_NewLeader = GetMember(p_Session, p_Name))
        {
...
{
    if (MemberPtr l_OldLeader = GetMember(GetLeaderGUID()))
    {
        /// New leader must be a member of guild
        if (MemberPtr l_NewLeader = GetMember(newLeaderGuid))
        {
            _SetLeaderGUID(l_NewLeader);
            l_OldLeader->ChangeRank(GR_OFFICER);
...
{
    if (BankTabPtr pTab = GetBankTab(p_TabID))
    {
...
void Guild::HandleSetMemberNote(WorldSession* session, std::string const& note, uint64 guid, bool isPublic)
{
    ...
    else if (MemberPtr member = GetMember(guid))
    {
        if (isPublic)
...
void Guild::HandleSetRankInfo(WorldSession* p_Session, uint32 p_RankID, const std::string& p_Name, uint32 p_Rights, uint32 p_MoneyPerDay, GuildBankRightsAndSlotsVec p_RightsAndSlots)
{
    ...
    else if (RankInfoPtr l_RankInfo = GetRankInfo(p_RankID))
    {
...
void Guild::HandleBuyBankTab(WorldSession* p_Session, uint8 p_TabID)
{
    ...
    p_Player->ModifyMoney(-int64(p_TabCost), "Guild::HandleBuyBankTab");
...
void Guild::HandleSpellEffectBuyBankTab(WorldSession* p_Session, uint8 p_TabID)
{
    ...
    GetAchievementMgr().UpdateCriteria(CRITERIA_TYPE_BUY_GUILD_BANK_SLOTS, 1, 0, 0, NULL, p_Player);
}

#define GUILD_MEMBER_CAP 1000
...
void Guild::HandleInviteMember(WorldSession* p_Session, const std::string& p_Name)
{
    ...
    if (m_members.size() >= GUILD_MEMBER_CAP)
    {
        ChatHandler(l_Player).PSendSysMessage("Your guild is full. You can't have more than 1000 members!");
        return;
    }
    ...
    if (!sObjectMgr->CheckFakeColorProtection(m_name, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void Guild::HandleLeaveMember(WorldSession* session)
{
    ...
    if (disband)
    {
        /// Really delete the guild pointer 10 seconds after the clear from all containers so even if some others map threads still using it in the current update, it won't be corrupt and crash
        sGarbageCollector->Add(this);
    }
    else
        HandleRoster(session);
...
void Guild::HandleRemoveMember(WorldSession* session, uint64 guid)
{
    ...
    MemberPtr member = GetMember(guid);
...
void Guild::HandleSetMemberRank(WorldSession* session, uint64 targetGuid, uint64 setterGuid, uint32 rank)
{
    ...
    if (MemberPtr member = GetMember(targetGuid))
    {
...
void Guild::HandleSwapRanks(WorldSession* p_Session, uint32 p_RankID, bool p_Up) ///< p_Session is unused
{
    ...
    if (!l_Player)
    {
        SendCommandResult(p_Session, GuildCommandType::GUILD_CREATE_S, GuildCommandError::ERR_GUILD_PERMISSIONS);
        return;
    }

    MemberPtr l_Member = GetMember(l_Player->GetGUID());
    if (!l_Member || !l_Member->IsRank(GuildDefaultRanks::GR_GUILDMASTER))
    {
        SendCommandResult(p_Session, GuildCommandType::GUILD_CREATE_S, GuildCommandError::ERR_GUILD_PERMISSIONS);
        return;
    }

    RankInfoPtr l_FirstRank = nullptr;
    RankInfoPtr l_SecondRank = nullptr;
...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint64 amount, bool cashFlow /*=false*/)
{
    ...
    player->ModifyMoney(-int64(amount), "Guild::HandleMemberDepositMoney");
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint64 amount, bool repair)
{
    ...
    {
        if (MemberPtr member = GetMember(player->GetGUID()))
    ...
    if (!repair)
    {
        player->ModifyMoney(amount, "Guild::HandleMemberWithdrawMoney " + std::to_string(m_id));
        player->SaveGoldToDB(trans);
    }
    // Log guild bank event
...
void Guild::HandleMemberLogout(WorldSession* p_Session)
{
    ...
    if (MemberPtr l_Member = GetMember(l_Player->GetGUID()))
    {
...
void Guild::HandleGuildPartyRequest(WorldSession* p_Session)
{
    ...
    bool l_GuildGroup = l_Group->IsGuildGroup();

    WorldPacket l_Data(Opcodes::SMSG_GUILD_PARTY_STATE, 13);

    l_Data.WriteBit(l_GuildGroup);
    l_Data.FlushBits();

    if (l_GuildGroup)
    {
        l_Data << uint32(l_Group->CountGuildMembers());
        l_Data << uint32(l_Group->CountNeededGuildMembers());
        l_Data << float(1.0f);  ///< Guild XP multiplier
    }
    else
    {
...
void Guild::LoadBank()
{
    if (m_BankLoaded || m_InLoading)
        return;

    m_InLoading = true;

    uint32 l_GuildId = m_id;

    PreparedStatement* l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_BANK_EVENT);
    l_Stmt->setUInt32(0, m_id);
    CharacterDatabase.AsyncQuery(l_Stmt, sWorld->GetQueryCallbackMgr(), [l_GuildId](PreparedQueryResult const& p_Result)
    {
        if (p_Result)
        {
            Guild* l_Guild = sGuildMgr->GetGuildById(l_GuildId);
            if (!l_Guild)
                return;
            ...
            do
            {
                ...
                l_Guild->LoadBankEventLogFromDB(fields);
            } while (p_Result->NextRow());
        }

        PreparedStatement* l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_BANK_TAB);
        l_Stmt->setUInt32(0, l_GuildId);
        CharacterDatabase.AsyncQuery(l_Stmt, sWorld->GetQueryCallbackMgr(), [l_GuildId](PreparedQueryResult const& p_Result)
        {
            if (p_Result)
            {
                ...
                {
                    Field* fields = p_Result->Fetch();
                    l_Guild->LoadBankTabFromDB(fields);
                } while (p_Result->NextRow());
            }

            PreparedStatement* l_Statement = CharacterDatabase.GetPreparedStatement(CHAR_SEL_GUILD_BANK_ITEMS);
            l_Statement->setUInt32(0, l_GuildId);
            CharacterDatabase.AsyncQuery(l_Statement, sWorld->GetQueryCallbackMgr(), [l_GuildId](PreparedQueryResult const& p_Result)
            {
                Guild* l_Guild = sGuildMgr->GetGuildById(l_GuildId);
                if (!l_Guild)
                    return;
                ...
                if (p_Result)
                {
                    ...
                    {
                        Field* fields = p_Result->Fetch();
                        l_Guild->LoadBankItemFromDB(fields);
                    } while (p_Result->NextRow());
                }

                l_Guild->SetBankLoaded();

                GuildBankRequest l_BankRequest;
                while (l_Guild->m_SessionToSendBank.next(l_BankRequest))
                {
                    WorldSession* l_Session = sWorld->FindSession(l_BankRequest.AccountId);
                    if (!l_Session)
                        continue;

                    l_Guild->SendBankList(l_Session, l_BankRequest.TabID, l_BankRequest.WithContent, l_BankRequest.WithTabInfo);
                }
            });
        });
    });
...
void Guild::SendBankList(WorldSession* p_Session, uint8 p_TabID, bool p_WithContent, bool p_WithTabInfo)
{
    if (!m_BankLoaded)
    {
        m_SessionToSendBank.add(GuildBankRequest(p_Session->GetAccountId(), p_TabID, p_WithContent, p_WithTabInfo));
        LoadBank();
        return;
    }
    ...
    BankTabPtr const l_CurrTab = GetBankTab(p_TabID);
    ...
    {
        if (BankTabPtr const l_BankTab = GetBankTab(p_TabID))
        {
    ...
    if (p_WithTabInfo)
    {
        for (uint8 l_TabID = 0; l_TabID < GetPurchasedTabsSize(); ++l_TabID)
        {
            ...
            l_Data.WriteBits(m_bankTabs[l_TabID]->GetIcon().length() / 2, 8);                   ///< Icon
            l_Data.WriteBit(m_bankTabs[l_TabID]->GetIcon().length() % 2);                       ///< Icon
            l_Data.FlushBits();
    ...
    if (p_WithContent && _MemberHasTabRights(p_Session->GetPlayer()->GetGUID(), p_TabID, GuildBankRights::GUILD_BANK_RIGHT_VIEW_TAB))
    {
        if (BankTabPtr const l_Tab = GetBankTab(p_TabID))
        {
            for (uint8 l_SlotID = 0; l_SlotID < GuildMisc::GUILD_BANK_MAX_SLOTS; ++l_SlotID)
            {
                if (Item* l_TabItem = l_Tab->GetItem(l_SlotID))
                {
                    ...
                    size_t l_GemCountPos = l_Data.bitwpos();
                    l_Data.WriteBits(0, 2);                                 ///< Enchant count
                    ...
                    uint32 l_GemCount = 0;
                    for (ItemDynamicFieldGems const& l_GemData : l_TabItem->GetGems())
                    {
                        if (sObjectMgr->GetItemTemplate(l_GemData.ItemId))
                        {
                            ...
                            l_Data << uint8(l_GemCount);
                            Item::BuildDynamicItemDatas(l_Data, l_GemData.ItemId, l_Bonuses);
                            ++l_GemCount;
                        }
                    }

                    l_Data.PutBits(l_GemCountPos, l_GemCount, 2);
                }
            }
        }
    }
...
{
     if (BankTabPtr const tab = GetBankTab(tabId))
...
void Guild::SendLoginInfo(WorldSession* p_Session)
{
    ...
    GetAchievementMgr().SendAllData(p_Session->GetPlayer());
...
void Guild::SendGuildRecipes(WorldSession* p_Session) const
{
    ...
    {
        packet.ReadUInt32("Skill Id", i);        // iterate all SkillLineAbility.db2 rows:
...
void Guild::SendMemberLeave(WorldSession* p_Session, uint64 p_PlayerGuid, bool p_Kicked)
{
    ...
    MemberPtr l_Member = GetMember(p_PlayerGuid);
...
bool Guild::LoadFromDB(Field* p_Fields)
{
    ...
    for (uint8 i = 0; i < l_PurchasedTabs; ++i)
        m_bankTabs[i] = std::make_shared<BankTab>(m_id, i);
...
{
    RankInfoPtr rankInfo = std::make_shared<RankInfo>(m_id);
...
bool Guild::LoadMemberFromDB(Field* fields)
{
    ...
    MemberPtr member =  std::make_shared<Member>(m_id, MAKE_NEW_GUID(lowguid, 0, HIGHGUID_PLAYER), fields[2].GetUInt8());
    ...
    sWorld->UpdateCharacterInfoGuild(lowguid, GetId());
...
{
    uint8 tabId = fields[ItemInstanceField::End + 2].GetUInt8();
    if (tabId >= GetPurchasedTabsSize())
    {
        sLog->outError(LOG_FILTER_GUILD, "Invalid tab for item (GUID: %u, id: #%u) in guild bank, skipped.",
            fields[ItemInstanceField::Guid].GetUInt32(), fields[ItemInstanceField::ItemEntry].GetUInt32());
        return false;
    }
...
bool Guild::Validate()
{
    ...
    else
    {
        ...
        {
            RankInfoPtr rankInfo = GetRankInfo(rankId);
    ...
    MemberPtr pLeader = GetMember(m_leaderGuid);
    if (!pLeader)
    {
...
bool Guild::AddMember(uint64 p_Guid, uint8 p_RankID)
{
    ...
    else if (CharacterInfo const* l_CharInfo = sWorld->GetCharacterInfo(l_LowGuid))
    {
        if (l_CharInfo->GuildId)
            return false;
    }
    ...
    MemberPtr l_Member = std::make_shared<Member>(m_id, p_Guid, p_RankID);
    ...
    else
    {
        ...
        CharacterDatabase.AsyncQuery(l_Stmt, sWorld->GetQueryCallbackMgr(), [l_GuildId, l_LowGuid](PreparedQueryResult const& p_Result)
        {
            if (!p_Result)
                return;
            ...
            MemberPtr l_Member = l_Guild->GetMember(l_LowGuid);
            if (!l_Member)
                return;
    ...
    sWorld->UpdateCharacterInfoGuild(l_LowGuid, GetId());
    ...
    if (l_Player)
    {
        l_Player->AddCriticalOperation([l_Player, this, p_RankID]() -> void
        {
...
void Guild::DeleteMember(uint64 p_Guid, bool p_IsDisbanding, bool p_IsKicked, bool p_CanDeleteGuild)
{
    ...
    {
        MemberPtr l_OldLeader = NULL;
        MemberPtr l_NewLeader = NULL;

        m_members.safe_foreach(true, [&](Guild::Members::iterator& l_I)
        {
    ...
    sWorld->UpdateCharacterInfoGuild(l_LowGuid, 0);
...
bool Guild::ChangeMemberRank(uint64 p_GUID, uint8 p_NewRank)
{
    ...
    {
        if (MemberPtr l_Member = GetMember(p_GUID))
        {
...
void Guild::AutoStoreItemInInventory(Player* p_Player, uint8 p_TabID, uint8 p_SlotID, uint32 p_Amount)
{
    ...
    if (BankTabPtr l_Tab = GetBankTab(p_TabID))
        l_Tab->SetItem(l_Transaction, p_SlotID, NULL);

    _DecreaseMemberRemainingSlots(l_Transaction, p_Player->GetGUID(), p_TabID);
    _LogBankEvent(l_Transaction, GUILD_BANK_LOG_WITHDRAW_ITEM, p_TabID, p_Player->GetGUIDLow(), l_Item->GetEntry(), p_Amount, 0);
...
bool Guild::_CreateNewBankTab()
{
    ...
    m_bankTabs.push_back(std::make_shared<BankTab>(m_id, tabId));
...
void Guild::_CreateRank(const std::string& p_Name, uint32 p_Rights)
{
    ...
    RankInfoPtr p_Info = std::make_shared<RankInfo>(m_id, p_NewRankId, p_Name, p_Rights, 0);
...
bool Guild::_IsLeader(Player* player) const
{
    if (player->GetGUID() == m_leaderGuid)
        return true;
    if (const MemberPtr member = GetMember(player->GetGUID()))
...
void Guild::_SetLeaderGUID(MemberPtr pLeader)
{
    if (!pLeader)
        return;
...
{
    if (RankInfoPtr rankInfo = GetRankInfo(rankId))
    {
        m_members.safe_foreach(true, [&](Guild::Members::iterator& itr)
        {
            if (itr->second->IsRank(rankId))
...
{
    if (const MemberPtr member = GetMember(guid))
...
inline void Guild::_DecreaseMemberRemainingSlots(SQLTransaction& trans, uint64 guid, uint8 tabId)
{
    ...
    if (uint32 remainingSlots = _GetMemberRemainingSlots(guid, tabId))
    {
        ...
        {
            if (MemberPtr member = GetMember(guid))
...
{
    if (const BankTabPtr tab = GetBankTab(tabId))
...
{
    if (BankTabPtr pTab = GetBankTab(tabId))
...
{
    MemberPtr l_Member = GetMember(p_OtherGUID);
    ASSERT(l_Member);
...
void Guild::CompleteGuildChallenge(int32 p_ChallengeType, Player* p_ReferencePlayer)
{
    if (p_ChallengeType == GuildChallengeType::ChallengeNone || p_ChallengeType >= GuildChallengeType::ChallengeMax)
        return;
    ...
    // Achievements
    UpdateCriteria(CRITERIA_TYPE_COMPLETE_GUILD_CHALLENGE, 1, 0, 0, NULL, p_ReferencePlayer);
    UpdateCriteria(CRITERIA_TYPE_COMPLETE_GUILD_CHALLENGE_TYPE, p_ChallengeType, 0, 0, NULL, p_ReferencePlayer);
}

void Guild::SendItemAcquired(Player* p_Player, Item* p_Item) const
{
    WorldPacket l_Data(SMSG_GUILD_BROADCAST_LOOTED_ITEM);
    ...
    l_Data.WriteBits(p_Player->GetNameLength(), 6); ///< Unk string
    l_Data.FlushBits();
...
void Guild::GuildNewsLog::AddNewEvent(GuildNews eventType, time_t date, uint64 playerGuid, uint32 flags, uint32 data, Item* p_Item /* = nullptr */)
{
    uint32 id = _newsLog.empty() ? 1 : _newsLog.rbegin()->first + 1;
    ...
    if (p_Item)
    {
        log.Item.ItemGUID = p_Item->GetGUIDLow();
        log.Item.ItemID = p_Item->GetTemplate()->ItemId;
        log.Item.RandomProperty = p_Item->GetItemRandomPropertyId();
        log.Item.RandomSuffix = p_Item->GetItemSuffixFactor();
        log.Item.Bonuses = p_Item->GetAllItemBonuses();
    }
    else
    {
        log.Item.ItemGUID = 0;
        log.Item.ItemID = 0;
        log.Item.RandomProperty = 0;
        log.Item.RandomSuffix = 0;
    }
...
void Guild::GuildNewsLog::LoadFromDB(PreparedQueryResult result)
{
    ...
    do
    {
        ...
        if (log.EventType == GuildNews::GUILD_NEWS_ITEM_LOOTED || log.EventType == GuildNews::GUILD_NEWS_ITEM_CRAFTED || log.EventType == GuildNews::GUILD_NEWS_ITEM_PURCHASED)
        {
            log.Item.ItemGUID = log.Data;
            log.Item.ItemID = fields[6].GetUInt32();
            log.Item.RandomProperty = fields[7].GetUInt16();
            log.Item.RandomSuffix = GenerateEnchSuffixFactor(log.Item.ItemID);

            Tokenizer l_BonusTokens(fields[8].GetString(), ' ');

            for (auto const& token : l_BonusTokens)
                log.Item.Bonuses.push_back(atoi(token));
        }
        else
        {
            log.Item.ItemGUID = 0;
...
void Guild::GuildNewsLog::BuildNewsData(uint32 p_ID, GuildNewsEntry& p_GuildNews, WorldPacket& p_Data)
{
    ...
    p_Data.WriteBit(p_GuildNews.Item.ItemGUID);                 ///< Has item loot data
    p_Data.FlushBits();

    if (p_GuildNews.Item.ItemGUID)
    {
        p_Data << uint32(p_GuildNews.Item.ItemID);              ///< Item ID
        p_Data << uint32(p_GuildNews.Item.RandomProperty);      ///< Random Properties Seed
        p_Data << uint32(p_GuildNews.Item.RandomSuffix);        ///< Random Properties ID

        p_Data.WriteBit(p_GuildNews.Item.Bonuses.size() != 0);  ///< Has Item Bonuses
        p_Data.WriteBit(false);                                 ///< Has Modifications
        p_Data.FlushBits();

        /// Item bonuses
        if (p_GuildNews.Item.Bonuses.size())
        {
            p_Data << uint8(0);                                 ///< Context
            p_Data << uint32(p_GuildNews.Item.Bonuses.size());

            for (auto& l_BonusId : p_GuildNews.Item.Bonuses)
                p_Data << uint32(l_BonusId);
        }
    };
}
...
void Guild::GuildNewsLog::BuildNewsData(WorldPacket& p_Data)
{
    ...
    constexpr int l_MaxNewsCount = 200;

    p_Data << uint32(_newsLog.size() > l_MaxNewsCount ? l_MaxNewsCount : _newsLog.size());

    int l_Count = 0;
    for (GuildNewsLogMap::const_reverse_iterator l_It = _newsLog.rbegin(); l_It != _newsLog.rend() && l_Count < l_MaxNewsCount; ++l_It, ++l_Count)
    {
        ...
        p_Data.WriteBit(l_It->second.Item.ItemGUID);            ///< Has item loot dataif (p_GuildNews.Item.ItemGUID)
        p_Data.FlushBits();

        if (l_It->second.Item.ItemGUID)
        {
            GuildNewsEntry::ItemInfo const& l_Item =  l_It->second.Item;

            p_Data << uint32(l_Item.ItemID);                    ///< Item ID
            p_Data << uint32(l_Item.RandomProperty);            ///< Random Properties Seed
            p_Data << uint32(l_Item.RandomSuffix);              ///< Random Properties ID

            p_Data.WriteBit(l_Item.Bonuses.size() != 0);        ///< Has Item Bonuses
            p_Data.WriteBit(false);                             ///< Has Modifications
            p_Data.FlushBits();

            /// Item bonuses
            if (l_Item.Bonuses.size())
            {
                p_Data << uint8(0);                             ///< Context
                p_Data << uint32(l_Item.Bonuses.size());

                for (auto& l_BonusId : l_Item.Bonuses)
                    p_Data << uint32(l_BonusId);
            }
        };
    }
