...
namespace Vignette
{
    ...
    */
    inline VignetteEntry const* GetVignetteEntryFromWorldObject(WorldObject const* p_Target) ///<  Unused function 'GetVignetteEntryFromWorldObject'
    {
        ...
        if (p_Target->GetTypeId() == TYPEID_UNIT)
            l_VignetteId = p_Target->ToCreature()->GetVignetteID();
    ...
    */
    inline uint32 GetTrackingQuestIdFromWorldObject(WorldObject const* p_Target)
    {
        uint32 l_TrackingQuest = 0;

        if (p_Target->GetTypeId() == TYPEID_UNIT)
            l_TrackingQuest = p_Target->ToCreature()->GetTrackingQuestID();
        if (p_Target->GetTypeId() == TYPEID_GAMEOBJECT)
    ...
    */
    inline Vignette::Type GetDefaultVignetteTypeFromWorldObject(WorldObject const* p_Target)
    {
