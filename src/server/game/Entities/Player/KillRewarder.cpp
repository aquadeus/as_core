...
KillRewarder::KillRewarder(Player* killer, Unit* victim, bool isBattleGround, std::set<uint64> p_RewardedPlayers) :
// 1. Initialize internal variables to default values.
_killer(killer), _victim(victim), _group(killer->GetGroup()),
_groupRate(1.0f), _maxNotGrayMember(NULL), _count(0), _sumLevel(0), _xp(0),
_isFullXP(false), _maxLevel(0), _isBattleGround(isBattleGround), _isPvP(false), m_RewardedMembers(p_RewardedPlayers)
{
...
inline void KillRewarder::_InitGroupData()
{
    ...
    {
        // 2. In case when player is in group, initialize variables necessary for group calculations:
        _group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
        {
...
inline void KillRewarder::_RewardXP(Player* p_Player, float p_Rate)
{
    ...
    if (l_Xp)
    {
        ...
        l_Xp *= p_Player->GetTotalAuraMultiplier(SPELL_AURA_MOD_XP_PCT);
        l_Xp *= p_Player->GetTotalAuraMultiplierByMiscValue(SPELL_AURA_MOD_XP_FROM_CREATURE_TYPE, int32(_victim->GetCreatureType()));
...
inline void KillRewarder::_RewardKillCredit(Player* player)
{
    ...
    if (!_group || player->isAlive() || !player->GetCorpse())
    {
        if (_victim->GetTypeId() == TYPEID_UNIT)
        {
            ...
            player->UpdateCriteria(CRITERIA_TYPE_KILL_CREATURE_TYPE, _victim->ToCreature()->GetCreatureType(), 1, 0, _victim);

#ifndef CROSS
            if (Guild* l_Guild = player->GetGuild())
                l_Guild->GetAchievementMgr().UpdateCriteria(CRITERIA_TYPE_KILL_CREATURE_TYPE_GUILD, _victim->ToCreature()->GetCreatureType(), 1, 0, _victim, player);
#endif
        }
    }
}
...
void KillRewarder::_RewardPlayer(Player* player, bool isDungeon)
{
    ...
    if (!_isPvP || _isBattleGround)
    {
        ...
        {
            // 4.2. Give XP.
            if (!(_victim->getLevelForTarget(player) > player->getLevel() && (_victim->getLevelForTarget(player) - player->getLevel()) > 6 && _group))
...
{
    if (std::find(m_RewardedMembers.begin(), m_RewardedMembers.end(), _killer->GetGUID()) != m_RewardedMembers.end())
        return;
    ...
    if (Creature* victim = _victim->ToCreature())
    {
        ...
        if (uint32 guildId = victim->GetMap()->GetOwnerGuildId())
        {
            ...
            if (Guild* guild = sGuildMgr->GetGuildById(guildId))
                guild->GetAchievementMgr().UpdateCriteria(CRITERIA_TYPE_KILL_CREATURE, victim->GetEntry(), 1, 0, victim, _killer);
#endif
        }

        // Update group scenario/challenge criterias
        if (Scenario* progress = sScenarioMgr->GetScenario(victim->GetMap()->GetScenarioGuid()))
