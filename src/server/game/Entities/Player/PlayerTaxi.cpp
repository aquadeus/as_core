...
void PlayerTaxi::AppendTaximaskTo(ByteBuffer & p_DataDest, bool p_All)
{
    if (p_All)
    {
        for (uint32 l_I = 0; l_I < TaxiMaskSize; ++l_I)
            p_DataDest << uint8(sTaxiNodesMask[l_I] & (sArgusTaxiMask[l_I] ^ 0xFF));
    }
    else    ///< All existed nodes
    {
        for (uint32 l_I = 0; l_I < TaxiMaskSize; ++l_I)
            p_DataDest << uint8(m_TaxiMask[l_I] & (sArgusTaxiMask[l_I] ^ 0xFF));
    }
}

void PlayerTaxi::AppendArgusTaxiMaskTo(ByteBuffer & p_DataDest, bool p_All, bool p_Second)
{
    for (uint32 l_I = 0; l_I < TaxiMaskSize; ++l_I)
    {
        /// Hackfix for argus Taxi
        if (l_I == 248 && p_Second)
        {
            p_DataDest << uint8(((p_All ? sTaxiNodesMask[l_I] : m_TaxiMask[l_I]) & sArgusTaxiMask[l_I]) | 0x7); ///< Hidden taxi nodes needed for argus
            continue;
        }

        p_DataDest << uint8((p_All ? sTaxiNodesMask[l_I] : m_TaxiMask[l_I]) & sArgusTaxiMask[l_I]);
    }
}
