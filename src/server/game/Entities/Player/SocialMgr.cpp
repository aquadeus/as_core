...
PlayerSocial::~PlayerSocial()
{
    m_PlayerSocialMapLock.acquire();
    m_playerSocialMap.clear();
    m_PlayerSocialMapLock.release();
}
...
uint32 PlayerSocial::GetNumberOfSocialsWithFlag(SocialFlag flag)
{
    ...
    ACE_Guard<ACE_Recursive_Thread_Mutex> l_Guard(m_PlayerSocialMapLock);
...
void PlayerSocial::RemoveFromSocialList(uint32 friendGuid, bool ignore)
{
    ...
    if (itr->second.Flags == 0)
    {
        ...
        sLog->outAshran("removing friends in PlayerSocial::RemoveFromSocialList account %u removed character %u (flag %u)", m_AccountID, friendGuid, flag);
    }
    else
    {
...
void PlayerSocial::SetFriendNote(uint32 friendGuid, std::string note)
{
    ...
    m_PlayerSocialMapLock.acquire_write();
...
void SocialMgr::GetFriendInfo(Player* player, uint32 friendGUID, FriendInfo &friendInfo)
{
    ...
    player->GetSocial()->m_PlayerSocialMapLock.acquire();
    ...
    player->GetSocial()->m_PlayerSocialMapLock.release();
...
void SocialMgr::BroadcastToFriendListers(Player* player, WorldPacket* packet)
{
    ...
    m_SocialMapLock.acquire_read();
    for (SocialMap::iterator itr = m_socialMap.begin(); itr != m_socialMap.end(); ++itr)
    {
        itr->second.m_PlayerSocialMapLock.acquire();
        ...
        itr->second.m_PlayerSocialMapLock.release();
    }
    m_SocialMapLock.release();
}

PlayerSocial* SocialMgr::LoadFromDB(PreparedQueryResult result, uint32 guid, uint32 account_id)
{
    m_SocialMapLock.acquire_write();
    ...
    ACE_Guard<ACE_Recursive_Thread_Mutex> l_Guard(social->m_PlayerSocialMapLock);
