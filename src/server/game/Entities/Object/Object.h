...
struct TempSummonGroupKey
{
    ...
    bool operator==(TempSummonGroupKey const& p_Key) const
    {
        return _summonerEntry == p_Key._summonerEntry &&
            _summonerType == p_Key._summonerType &&
            _summonGroup == p_Key._summonGroup;
    }
...
typedef std::set<uint64> GuidList;
typedef std::list<uint64> RealGuidList;
...
class Object
{
    public:
        ...
        uint32 GetRealGUIDHigh() const { return GetGUIDHigh(); }
#endif
        ...
        void SetGuidValue(uint16 p_Index, Guid128 p_Guid);
        ...
        void ModifyDynamicValue(uint16 p_Index, uint32 p_OldValue, uint32 p_NewValue);
        ...
        EventObject* ToEventObject() { if (GetTypeId() == TypeID::TYPEID_EVENTOBJECT) return reinterpret_cast<EventObject*>(this); else return nullptr; }
        EventObject const* ToEventObject() const { if (GetTypeId() == TypeID::TYPEID_EVENTOBJECT) return reinterpret_cast<EventObject const*>(this); else return nullptr; }
    ...
    protected:
        ...
        TempSummonMap m_TempSummonList;
...
//public:
    ...
    bool operator!() const
    {
        return (m_positionX == 0.0f && m_positionY == 0.0f && m_positionZ == 0.0f && m_positionH == 0.0f && m_orientation == 0.0f);
    }
    ...
    virtual void Relocate(const Position &pos)
        { m_positionX = pos.m_positionX; m_positionY = pos.m_positionY; m_positionZ = pos.m_positionZ; SetOrientation(pos.m_orientation); m_positionH = pos.m_positionH; }
    ...
    void PositionToVector(G3D::Vector3& pos) const {  pos.x = m_positionX; pos.y = m_positionY; pos.z = GetPositionZH(); }
    ...
    bool HasInLine(WorldObject const* target, float width, bool p_ObjectSize = true) const;
    bool IsInPolygon(std::vector<G3D::Vector2> const & p_Polygon) const;
    ...
    void GetRandomNearPosition(Position& p_Pos, float p_Radius, WorldObject* p_RefObject)
    {
        GetPosition(&p_Pos);
        MovePosition(p_Pos, p_Radius * (float)rand_norm(), (float)rand_norm() * static_cast<float>(2 * M_PI), p_RefObject);
    }

    void MovePosition(Position& p_Pos, float p_Dist, float p_Angle, WorldObject* p_RefObject);

    bool IsInBack(WorldObject const* p_Target, float p_Arc = M_PI) const;
};
...
class GridObject
{
    public:
        ...
        virtual void AddToGrid(GridRefManager<T>& m) { ASSERT(!IsInGrid()); _gridRef.link(&m, (T*)this); }

        void AddNearbyToGrid(GridRefManager<T>& m)
        {
            GridReference<T>* l_NewGridRef = new GridReference<T>();
            (*l_NewGridRef).link(&m, (T*)this);
            _nearbiesGridRef.push_back(l_NewGridRef);
        }
        ...
        void RemoveNearbyFromGrids()
        {
            for (auto l_Itr = _nearbiesGridRef.begin(); l_Itr != _nearbiesGridRef.end();)
            {
                (*const_cast<Reference<GridRefManager<T>, T>*>((Reference<GridRefManager<T>, T> const*)(&(**l_Itr)))).unlink();
                delete *l_Itr;
                l_Itr = _nearbiesGridRef.erase(l_Itr);
            }
        }

    private:
        GridReference<T> _gridRef;
        std::list<GridReference<T>*> _nearbiesGridRef;
};
...
class WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        void GetNearestPosition(Position &pos, float dist, float angle);
        ...
        virtual float GetObjectSize() const;

        bool HasInLineCheckingStrafe(WorldObject const * target, float width) const;
        ...
        std::set<uint32> const& GetPhases() const { return m_Phases; }
        void ConvertPhaseMaskToPhaseId();
        bool HasInExcludePhaseList(uint32 phase) const;
        bool SetInExcludePhase(uint32 id, bool update, bool apply);
        std::set<uint32> const& GetExcludePhases() const { return m_ExcludePhases; }
        ...
        bool IsWithinDist3d(Position const* p_Pos, float p_Dist, WorldObject const* p_Target, bool p_IgnoreCasterSize, bool p_IgnoreTargetSize = false) const
        {
            float l_CasterSize = p_IgnoreCasterSize ? 0.0f : GetObjectSize();
            float l_TargetSize = (p_Target == nullptr || p_IgnoreTargetSize) ? 0.0f : p_Target->GetObjectSize();

            return IsInDist(p_Pos, p_Dist + l_CasterSize + l_TargetSize);
        }
        ...
        bool IsInAxe(const WorldLocation* obj1, const WorldLocation* obj2, float size = 0) const;
        ...
        virtual void SendMessageToSet(WorldPacket* p_Data, bool p_Self);
		...
		void PlayDirectMusic(uint32 musicId, Player* target = NULL);
        ...
        bool canSeeOrDetect(WorldObject const* p_Object, bool p_IgnoreStealth = false, bool p_DistanceCheck = false, bool p_IgnoreInvisibility = false) const;
        ...
        void SetCurrMap(Map* p_Map) { m_currMap = p_Map; } ///< NEVER USE THIS FUNCTION
        ...
        void ClearZoneScript() { m_zoneScript = nullptr; }

        TempSummon* SummonCreature(uint32 id, const Position &pos, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, uint32 vehId = 0, uint64 viewerGuid = 0, std::list<uint64>* viewersList = nullptr, SummonPropertiesEntry const* p_SumProp = nullptr, bool p_Big = false, bool p_ActiveObject = false) const;
        ...
        TempSummon* SummonCreature(uint32 p_ID, SpellDestination const* p_Dest, TempSummonType p_Type = TEMPSUMMON_MANUAL_DESPAWN, uint32 p_DespawnTime = 0, uint64 p_Viewer = 0, std::list<uint64>* p_Viewers = nullptr);
        GameObject* SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime, uint64 viewerGuid = 0, std::list<uint64>* viewersList = NULL, uint32 p_AnimProgress = 100, uint32 p_GoHealth = 0, bool p_GarrisonPlotObject = false, bool p_Active = false, uint32 p_CustomFlags = 0, bool p_IsActiveObject = false);
        ...
        GameObject* SummonGameObject(uint32 p_Entry, Position const& p_Pos, Position p_Rotation = Position(), uint32 p_RespTime = 0, uint64 p_ViewerGuid = 0, std::list<uint64>* p_ViewerList = nullptr, uint32 p_AnimProgress = 100, uint32 p_GoHealth = 0, bool p_GarrisonPlotObject = false, bool p_Active = false, uint32 p_CustomFlags = 0, bool p_IsActiveObject = false);
        Creature*   SummonTrigger(float x, float y, float z, float ang, uint32 dur, CreatureAI* (*GetAI)(Creature*) = NULL);
        void SummonCreatureGroup(uint8 group, std::list<TempSummon*>* list = nullptr);
        void SummonCreatureGroupDespawn(uint8 group, std::list<TempSummon*>* list = NULL);

        Creature*   FindNearestCreature(uint32 entry, float range, bool alive = true) const;
        Creature*   FindDetectableNearestCreature(uint32 entry, float range, bool alive = true) const;
        GameObject* FindNearestGameObject(uint32 entry, float range) const;
        GameObject* FindNearestGameObject(float p_Range) const;
        GameObject* FindNearestGameObjectOfType(GameobjectTypes type, float range) const;
        Player*     FindNearestPlayer(float range);
        Player*     FindNearestDeadPlayer(float p_Range);
        ...
        void GetPlayerListInGrid(std::list<Player*>& lList, float fMaxSearchRange, bool p_Self = false, bool p_AliveCheck = true) const;
        ...
        void GetAttackableUnitListInRange(std::list<Unit*> &list, float fMaxSearchRange, bool p_ObjectSize = true) const;
        virtual void GetAttackableUnitListInPositionRange(std::list<Unit*> &list, Position const p_Position, float fMaxSearchRange, bool p_ObjectSize = true, bool p_AliveCheck = true) const;
        ...
        bool HideForSomePlayers() const { return !m_HideForGuid.empty(); }
        void AddToHideList(uint64 p_Guid, uint32 p_HideTime = 0) { m_HideForGuid[p_Guid] = p_HideTime; }
        bool ShouldHideFor(uint64 p_Guid) const { return m_HideForGuid.find(p_Guid) != m_HideForGuid.end(); };
        void RemoveFromHideList(uint64 p_Guid) { m_HideForGuid.erase(p_Guid); }

        void GenerateMinMaxGold(uint32& p_MinGold, uint32& p_MaxGold);

        bool IsBig() const { return m_IsBig; }
        void SetBig(bool p_Apply) { m_IsBig = p_Apply; }
    ...
    protected:
        ...
        std::mutex m_PacketsToBroadcastLock;
        std::vector<WorldPacket*> m_PacketsToBroadcast;

        std::unordered_map<uint64, uint32> m_HideForGuid;
    ...
    private:
        ...
        std::set<uint32> m_Phases;
        std::set<uint32> m_ExcludePhases;
        ...
        bool CanDetect(WorldObject const* p_Object, bool p_IgnoreStealth, bool p_IgnoreInvisibility = false) const;
...
{
    template<class T>
    void RandomResizeList(T &list, uint32 _size)
    {
    ...
    void RandomResizeList(T &list, Predicate& predicate, uint32 size)
    {
        //! First use predicate filter
        T listCopy;
        for (typename T::iterator itr = list.begin(); itr != list.end(); ++itr)
        {
            if (predicate(*itr))
    ...
    class ObjectDistanceOrderPredPlayer
    {
        public:
            ObjectDistanceOrderPredPlayer(const WorldObject* pRefObj, bool ascending = true) : m_refObj(pRefObj), m_ascending(ascending) {}
            bool operator()(const WorldObject* pLeft, const WorldObject* pRight) const
            {
                if ((pRight->IsPlayer() || pLeft->IsPlayer()) && !(pRight->IsPlayer() && pLeft->IsPlayer()))
                    return pLeft->IsPlayer();
