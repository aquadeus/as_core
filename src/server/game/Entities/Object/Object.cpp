...
uint32 GuidHigh2TypeId(uint32 guid_hi)
{
    switch (guid_hi)
    {
        ...
        case HIGHGUID_EVENTOBJECT:  return TYPEID_EVENTOBJECT;
    }
...
void WorldObject::CleanBeforeGC()
{
    // this may happen because there are many !create/delete
    if (IsWorldObject() && m_currMap)
    {
...
thread_local ByteBuffer tl_Buffer;
...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    /// Hackfix for Thermal Anvil - Not sent to client on retail
    if (GetEntry() == 184633)
        return;

    tl_Buffer.reserve(100 * 1024);
    ...
    if (target == this)                                      // building packet for yourself
    {
        ...
        if (GetTypeId() == TYPEID_PLAYER)
            flags |= UPDATEFLAG_PLAYER_CREATE_INFO;
    }
    ...
    tl_Buffer << uint8(updateType);
    tl_Buffer.append(GetPackGUID());
    tl_Buffer << uint8(m_objectTypeId);

    BuildMovementUpdate(&tl_Buffer, flags);
    BuildValuesUpdate(updateType, &tl_Buffer, target);
    BuildDynamicValuesUpdate(updateType, &tl_Buffer, target);

    data->AddUpdateBlock(tl_Buffer);
}

thread_local UpdateData  tl_UpdateData_SendUpdateToPlayer;
thread_local WorldPacket tl_WorldPacket_SendUpdateToPlayer;

void Object::SendUpdateToPlayer(Player* player)
{
    tl_UpdateData_SendUpdateToPlayer.Clear();
    tl_UpdateData_SendUpdateToPlayer.SetMap(player->GetMapId());
    tl_WorldPacket_SendUpdateToPlayer.reserve(10 * 1024);
    tl_WorldPacket_SendUpdateToPlayer.clear();

    // send create update to player
    BuildCreateUpdateBlockForPlayer(&tl_UpdateData_SendUpdateToPlayer, player);

    if (tl_UpdateData_SendUpdateToPlayer.BuildPacket(&tl_WorldPacket_SendUpdateToPlayer))
        player->GetSession()->SendPacket(&tl_WorldPacket_SendUpdateToPlayer);
}

thread_local ByteBuffer UpdateValue_Buffer;

void Object::BuildValuesUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    UpdateValue_Buffer.reserve(50 * 1024);
    UpdateValue_Buffer.clear();

    UpdateValue_Buffer << uint8(UPDATETYPE_VALUES);
    UpdateValue_Buffer.append(GetPackGUID());

    BuildValuesUpdate(UPDATETYPE_VALUES, &UpdateValue_Buffer, target);
    BuildDynamicValuesUpdate(UPDATETYPE_VALUES, &UpdateValue_Buffer, target);

    data->AddUpdateBlock(UpdateValue_Buffer);
}
...
void Object::BuildMovementUpdate(ByteBuffer* p_Data, uint32 p_Flags) const
{
    ...
    const WorldObject   * l_WorldObject = GetTypeId() == TypeID::TYPEID_ITEM ? nullptr : (const WorldObject*)this;
    ...
    p_Data->WriteBit(p_Flags & UPDATEFLAG_SMOOTH_PHASING);          ///< Has smooth phasing
    p_Data->WriteBit(p_Flags & UPDATEFLAG_THIS_IS_YOU);             ///< Self
    p_Data->WriteBit(p_Flags & UPDATEFLAG_SCENE_OBJECT);            ///< Scene Object
    p_Data->WriteBit(p_Flags & UPDATEFLAG_PLAYER_CREATE_INFO);      ///< Has player create info
    p_Data->FlushBits();
    ...
    if (p_Flags & UPDATEFLAG_HAS_MOVEMENT_UPDATE)
    {
        ...
        /// Movement
        {
            ...
            *p_Data << float(l_Unit->GetPositionZ());                       ///< Mover position Z
            ...
            *p_Data << uint32(0);                                           ///< Move Index
            ...
            for (uint32 l_Frame = 0; l_Frame < l_RemoveForcesCount; l_Frame++)
                p_Data->appendPackGUID(0);                                  ///< Remove force IDs

            p_Data->WriteBits(l_MovementFlags, 30);                         ///< Movement flags
            p_Data->WriteBits(l_ExtraMovementFlags, 18);                    ///< Extra movement flags
        ...
        if (l_HasSpline)
        {
            ...
            *p_Data << uint32(l_Spline->GetId());                           ///< Move spline ID
            ...
            if (!l_Spline->Finalized())
            {
                ...
                *p_Data << uint32(l_Spline->splineflags.raw());             ///< Spline flags
                ...
                *p_Data << float(1.0f);                                     ///< DurationModNext
                ...
                p_Data->WriteBits(l_Spline->facing.type, 2);                ///< Final facing computation
                ...
                p_Data->WriteBits(l_Spline->getPath().size(), 16);          ///< Path node count
                p_Data->WriteBits(uint8(l_Spline->spline.mode()), 2);       ///< Spline mode
                p_Data->WriteBit(l_HasFilterKeys);                          ///< HasSplineFilter
                p_Data->WriteBit(0);                                        ///< HasSpellEffectExtraData
                p_Data->FlushBits();
    ...
    *p_Data << uint32(l_FrameCount);                                ///< Transport frame count
    ...
    {
        *p_Data << uint32(l_Unit->GetVehicleKit()->GetVehicleInfo()->ID);   ///< Vehicle ID
    ...
    if (p_Flags & UPDATEFLAG_HAS_AREATRIGGER)
    {
        ...
        if (t->GetLiveTime())
            *p_Data << uint32(t->GetLiveTime());       // Elapsed Time Ms
        else
            *p_Data << uint32(1);                                         // Elapsed Time Ms

        *p_Data << t->GetAreaTriggerInfo().RollPitchYaw1X;
        *p_Data << t->GetAreaTriggerInfo().RollPitchYaw1Y;
        *p_Data << t->GetAreaTriggerInfo().RollPitchYaw1Z;

        p_Data->WriteBit(t->GetAreaTriggerInfo().HasAbsoluteOrientation); // HasAbsoluteOrientation
        p_Data->WriteBit(t->GetAreaTriggerInfo().HasDynamicShape);        // HasDynamicShape
        p_Data->WriteBit(t->GetAreaTriggerInfo().HasAttached);            // HasAttached is have transport, not complete
        p_Data->WriteBit(t->GetAreaTriggerInfo().HasFaceMovementDir);     // HasFaceMovementDir
        p_Data->WriteBit(t->GetAreaTriggerInfo().HasFollowsTerrain);      // HasFollowsTerrain
        p_Data->WriteBit(false);                                          // unkbit
        p_Data->WriteBit(t->HasTargetRollPitchYaw());                     // HasTargetRollPitchYaw
        p_Data->WriteBit(t->GetAreaTriggerInfo().ScaleCurveID);           // HasScaleCurveID
        p_Data->WriteBit(t->GetAreaTriggerInfo().MorphCurveID);           // HasMorphCurveID
        p_Data->WriteBit(t->GetAreaTriggerInfo().FacingCurveID);          // HasFacingCurveID
        p_Data->WriteBit(t->GetAreaTriggerInfo().MoveCurveID);            // hasMoveCurveID
        p_Data->WriteBit(t->GetAreaTriggerInfo().ShapeID1);               // unkbit4C
        p_Data->WriteBit(t->GetAreaTriggerInfo().HasUnkBit50);            // unkbit50
        p_Data->WriteBit(t->GetAreaTriggerInfo().ShapeID2);               // unkbit58
        p_Data->WriteBit(t->GetVisualScale() != 0.0f);                    // HasAreaTriggerSphere
        p_Data->WriteBit(t->GetAreaTriggerInfo().hasAreaTriggerBox);      // HasAreaTriggerBox
        p_Data->WriteBit(t->HasPolygon());                                // AreaTriggerPolygon
        p_Data->WriteBit(t->GetAreaTriggerCylinder());                    // areaTriggerCylinder
        p_Data->WriteBit(t->HasSpline());                                 // areaTriggerSpline
        p_Data->WriteBit(t->HasCircleData());                             // areaTriggerCircle

        if (t->GetAreaTriggerInfo().HasUnkBit50)
            p_Data->WriteBit(t->GetAreaTriggerInfo().UnkBit50);

        if (t->HasSpline())
            *p_Data << t->GetSplineInfo();

        if (t->HasTargetRollPitchYaw())
        {
            *p_Data << t->GetAreaTriggerInfo().TargetRollPitchYawX;
            *p_Data << t->GetAreaTriggerInfo().TargetRollPitchYawY;
            *p_Data << t->GetAreaTriggerInfo().TargetRollPitchYawZ;
        }

        if (t->GetAreaTriggerInfo().ScaleCurveID)
            *p_Data << uint32(t->GetAreaTriggerInfo().ScaleCurveID);

        if (t->GetAreaTriggerInfo().MorphCurveID)
            *p_Data << uint32(t->GetAreaTriggerInfo().MorphCurveID);

        if (t->GetAreaTriggerInfo().FacingCurveID)
            *p_Data << uint32(t->GetAreaTriggerInfo().FacingCurveID);

        if (t->GetAreaTriggerInfo().MoveCurveID)
            *p_Data << uint32(t->GetAreaTriggerInfo().MoveCurveID);

        if (t->GetAreaTriggerInfo().ShapeID1)
            *p_Data << uint32(t->GetAreaTriggerInfo().ShapeID1);

        if (t->GetAreaTriggerInfo().ShapeID2)
            *p_Data << uint32(t->GetAreaTriggerInfo().ShapeID2);

        if (t->GetVisualScale() != 0.0f)                                        // areaTriggerSphere
        {
            *p_Data << t->GetVisualScale();                                 // Radius
            *p_Data << t->GetVisualScale(true);                             // RadiusTarget
        }
        ...
        if (t->GetAreaTriggerInfo().hasAreaTriggerBox)
        {
            ...
            *p_Data << t->GetAreaTriggerInfo().Radius;                    // Radius
            *p_Data << t->GetAreaTriggerInfo().Polygon.Height;            // Height
            *p_Data << t->GetAreaTriggerInfo().LocationZOffset;           // LocationZOffset
            *p_Data << t->GetAreaTriggerInfo().RadiusTarget;              // RadiusTarget
            *p_Data << t->GetAreaTriggerInfo().Polygon.HeightTarget;      // HeightTarget
            *p_Data << t->GetAreaTriggerInfo().LocationZOffsetTarget;     // LocationZOffsetTarget
        }

        if (t->HasPolygon())
            *p_Data << t->GetAreaTriggerInfo().Polygon;
        ...
        if (t->GetAreaTriggerCylinder())                                // areaTriggerCylinder
        {
            ...
            *p_Data << t->GetAreaTriggerInfo().Polygon.Height;            // Height
            *p_Data << t->GetAreaTriggerInfo().Polygon.HeightTarget;      // HeightTarget
            *p_Data << t->GetAreaTriggerInfo().LocationZOffset;           // Float4
            *p_Data << t->GetAreaTriggerInfo().LocationZOffsetTarget;     // LocationZOffsetTarget
        }

         if (t->HasCircleData())
            *p_Data << *const_cast<AreaTriggerCircle*>(t->GetCircleData());
    }
    ...
    if (p_Flags & UPDATEFLAG_PLAYER_CREATE_INFO)
    {
        bool l_HasRuneState = ToUnit()->getClass() == Classes::CLASS_DEATH_KNIGHT;

        p_Data->WriteBit(0);                    ///< HasSceneInstanceIDs
        p_Data->WriteBit(l_HasRuneState);       ///< HasRuneState
        p_Data->FlushBits();
        ...
        */
        ...
        if (l_HasRuneState)
        {
            Player const* l_DeathKnight = ToUnit()->ToPlayer();
            *p_Data << uint8(l_DeathKnight->GetRunesState());    ///<   RechargingRuneMask
            *p_Data << uint8(l_DeathKnight->GetRunesState());    ///<   UsableRuneMask
            *p_Data << uint32(ToUnit()->GetMaxPower(POWER_RUNES));

            for (uint32 l_I = 0; l_I < ToUnit()->GetMaxPower(POWER_RUNES); ++l_I)
            {
                ...
                *p_Data << uint8(l_CooldownPkt);
            }
        }
    }
}

thread_local ByteBuffer tl_FieldBuffer;
...
void Object::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    tl_FieldBuffer.reserve(50 * 1024);
    tl_FieldBuffer.clear();
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            ((updateType == UPDATETYPE_VALUES ? _changesMask.GetBit(index) : m_uint32Values[index]) && (flags[index] & visibleFlag)))
        {
            ...
            tl_FieldBuffer << m_uint32Values[index];
    ...
    *data << uint8(updateMask.GetBlockCount());
    ...
    data->append(tl_FieldBuffer);
}

thread_local ByteBuffer tl_DynamicFieldBuffer;
thread_local ByteBuffer tl_DynamicLoopFieldBuffer;
...
void Object::BuildDynamicValuesUpdate(uint8 p_UpdateType, ByteBuffer* p_Data, Player* p_Target) const
{
    ...
    tl_DynamicFieldBuffer.reserve(10 * 1024);
    tl_DynamicLoopFieldBuffer.reserve(10 * 1024);
    tl_DynamicFieldBuffer.clear();
    ...
    {
        tl_DynamicLoopFieldBuffer.clear();
            ...
            ((p_UpdateType == UPDATETYPE_VALUES ? _dynamicChangesMask.GetBit(l_Index) : !l_Values.empty()) && (l_Flags[l_Index] & l_VisibleFlags)))
        {
            ...
            for (std::size_t l_Iter = 0; l_Iter < l_Values.size(); ++l_Iter)
            {
                if (p_UpdateType != UPDATETYPE_VALUES || _dynamicChangesArrayMask[l_Index].GetBit(l_Iter))
                {
                    ...
                    tl_DynamicLoopFieldBuffer << uint32(l_Values[l_Iter]);           ///< DynamicValue
                }
            }

            tl_DynamicFieldBuffer << uint16(l_ArrayMask.GetBlockCount());    ///< TabCount
            l_ArrayMask.AppendToPacket(&tl_DynamicFieldBuffer);             ///< TabMask (Completed fields)
            tl_DynamicFieldBuffer.append(tl_DynamicLoopFieldBuffer);
        }
    }

    *p_Data << uint8(l_UpdateMask.GetBlockCount());                 ///< MaskSize
    l_UpdateMask.AppendToPacket(p_Data);                            ///< Mask
    p_Data->append(tl_DynamicFieldBuffer);
}
...
void Object::SetGuidValue(uint16 p_Index, Guid128 p_Guid)
{
    ASSERT(p_Index + 3 < m_valuesCount || PrintIndexError(p_Index, true));

    bool l_Changed = false;

    if (m_uint32Values[p_Index] != PAIR64_LOPART(p_Guid.GetLow()))
    {
        m_uint32Values[p_Index] = PAIR64_LOPART(p_Guid.GetLow());
        _changesMask.SetBit(p_Index);
        l_Changed = true;
    }

    if (m_uint32Values[p_Index + 1] != PAIR64_HIPART(p_Guid.GetLow()))
    {
        m_uint32Values[p_Index + 1] = PAIR64_HIPART(p_Guid.GetLow());
        _changesMask.SetBit(p_Index + 1);
        l_Changed = true;
    }

    if (m_uint32Values[p_Index + 2] != PAIR64_LOPART(p_Guid.GetHi()))
    {
        m_uint32Values[p_Index + 2] = PAIR64_LOPART(p_Guid.GetHi());
        _changesMask.SetBit(p_Index + 2);
        l_Changed = true;
    }

    if (m_uint32Values[p_Index + 3] != PAIR64_HIPART(p_Guid.GetHi()))
    {
        m_uint32Values[p_Index + 3] = PAIR64_HIPART(p_Guid.GetHi());
        _changesMask.SetBit(p_Index + 3);
        l_Changed = true;
    }
...
void Object::SetUInt32Value(uint16 index, uint32 value)
{
    ...
    if (index == PLAYER_FIELD_PRESTIGE && value > 18)
    {
        ACE_Stack_Trace l_StackTrace;
        sLog->outAshran("Player field prestige override %u %s", value, l_StackTrace.c_str());
    }
...
uint32 Object::GetDynamicValue(uint16 index, uint16 secondIndex) const
{
    ...
    if (secondIndex >= _dynamicValues[index].size())
        return 0;
...
void Object::RemoveDynamicValue(uint16 index, uint32 value)
{
    ...
    std::vector<uint32>& l_Values = _dynamicValues[index];
    for (size_t l_I = 0; l_I < l_Values.size(); ++l_I)
    {
        if (l_Values[l_I] == value)
        {
            l_Values[l_I] = 0;

            _dynamicChangesMask.SetBit(index);
            _dynamicChangesArrayMask[index].SetBit(l_I);
        }
    }
...
void Object::ModifyDynamicValue(uint16 p_Index, uint32 p_OldValue, uint32 p_NewValue)
{
    std::vector<uint32>& l_Values = _dynamicValues[p_Index];
    for (size_t l_I = 0; l_I < l_Values.size(); ++l_I)
    {
        if (l_Values[l_I] == p_OldValue)
        {
            l_Values[l_I] = p_NewValue;

            _dynamicChangesMask.SetBit(p_Index);
            _dynamicChangesArrayMask[p_Index].SetBit(l_I);
        }
    }
...
bool Position::HasInLine(WorldObject const* target, float width, bool p_ObjectSize /*= true*/) const
{
...
bool Position::IsInPolygon(std::vector<G3D::Vector2> const & p_Polygon) const
{
    if (p_Polygon.size() < 3)
        return false;

    uint32 l_Count = uint32(p_Polygon.size());
    uint32 l_J = 0;
    bool l_Ok = false;
    ...
    {
        G3D::Vector2 l_Point = { m_positionX, m_positionY };

        G3D::Vector2 const & l_Curr = p_Polygon.at(l_I);
        G3D::Vector2 const & l_Prev = p_Polygon.at(l_J);

        if (((l_Curr.y > l_Point.y) != (l_Prev.y > l_Point.y)) &&
            (l_Point.x < (l_Prev.x - l_Curr.x) * (l_Point.y - l_Curr.y) / (l_Prev.y - l_Curr.y) + l_Curr.x))
        {
            l_Ok = !l_Ok;
        }
    }
...
void Position::MovePosition(Position& p_Pos, float p_Dist, float p_Angle, WorldObject* p_RefObject)
{
    if (!p_RefObject)
        return;

    p_Angle += GetOrientation();
    float l_DestX, l_DestY, l_DestZ, l_Ground, l_Floor;
    l_DestX = p_Pos.m_positionX + p_Dist * std::cos(p_Angle);
    l_DestY = p_Pos.m_positionY + p_Dist * std::sin(p_Angle);

    /// Prevent invalid coordinates here, position is unchanged
    if (!JadeCore::IsValidMapCoord(l_DestX, l_DestY, p_Pos.m_positionZ))
    {
        sLog->outFatal(LOG_FILTER_GENERAL, "Position::MovePosition invalid coordinates X: %f and Y: %f were passed!", l_DestX, l_DestY);
        return;
    }

    l_Ground = p_RefObject->GetMap()->GetHeight(p_RefObject->GetPhaseMask(), l_DestX, l_DestY, MAX_HEIGHT, true);
    l_Floor = p_RefObject->GetMap()->GetHeight(p_RefObject->GetPhaseMask(), l_DestX, l_DestY, p_Pos.m_positionZ, true);
    l_DestZ = fabs(l_Ground - p_Pos.m_positionZ) <= fabs(l_Floor - p_Pos.m_positionZ) ? l_Ground : l_Floor;

    float l_Step = p_Dist / 10.0f;

    for (uint8 l_J = 0; l_J < 10; ++l_J)
    {
        /// Do not allow too big z changes
        if (fabs(p_Pos.m_positionZ - l_DestZ) > 6)
        {
            l_DestX -= l_Step * std::cos(p_Angle);
            l_DestY -= l_Step * std::sin(p_Angle);
            l_Ground = p_RefObject->GetMap()->GetHeight(p_RefObject->GetPhaseMask(), l_DestX, l_DestY, MAX_HEIGHT, true);
            l_Floor = p_RefObject->GetMap()->GetHeight(p_RefObject->GetPhaseMask(), l_DestX, l_DestY, p_Pos.m_positionZ, true);
            l_DestZ = fabs(l_Ground - p_Pos.m_positionZ) <= fabs(l_Floor - p_Pos.m_positionZ) ? l_Ground : l_Floor;
        }
        /// We have correct destz now
        else if (p_RefObject->IsWithinLOS(l_DestX, l_DestY, l_DestZ))
        {
            p_Pos.Relocate(l_DestX, l_DestY, l_DestZ);
            break;
        }
    }

    JadeCore::NormalizeMapCoord(p_Pos.m_positionX);
    JadeCore::NormalizeMapCoord(p_Pos.m_positionY);

    p_Pos.SetOrientation(GetOrientation());
}

bool Position::IsInBack(WorldObject const* p_Target, float p_Arc /*= M_PI*/) const
{
    return !HasInArc(2.0f * M_PI - p_Arc, p_Target);
}
...
m_transport(NULL), m_currMap(NULL), m_InstanceId(0), m_IsBig(false),
...
{
    if (Map* l_Map = GetMap())
        return (l_Map->IsDungeon() || l_Map->IsScenario()) ? ((InstanceMap*)l_Map)->GetInstanceScript() : nullptr;

    return nullptr;
}

InstanceScript const* WorldObject::GetInstanceScript() const
{
    if (Map* l_Map = GetMap())
        return (l_Map->IsDungeon() || l_Map->IsScenario()) ? ((InstanceMap*)l_Map)->GetInstanceScript() : nullptr;
...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj) const
{
    ...
    {
        if (l_Instance->IsLineOfSightDisabled())
            return true;
    }

    if (obj->ToCreature() && obj->ToCreature()->IsAIEnabled)
    {
        if (obj->ToCreature()->AI()->CanBeTargetedOutOfLOS() || obj->ToCreature()->AI()->CanBeTargetedOutOfLOSXYZ(GetPositionX(), GetPositionY(), GetPositionZ()))
            return true;
    }

    if (ToCreature() && ToCreature()->IsAIEnabled)
    {
        if (ToCreature()->AI()->CanTargetOutOfLOS() || ToCreature()->AI()->CanTargetOutOfLOSXYZ(obj->GetPositionX(), obj->GetPositionY(), obj->GetPositionZ()))
            return true;
    }

    return obj->IsWithinLOS(ox, oy, oz);
}

bool WorldObject::IsWithinLOS(float ox, float oy, float oz) const
{
    /// Hack fix for Antoran High Command
    if (IsPlayer() && ToPlayer()->GetVehicle())
    {
        switch (ToPlayer()->GetVehicle()->GetCreatureEntry())
        {
            case 122554:
            case 123020:
            case 123013:
    ...
    // Hack fix for Kin'garoth
    if (GetMapId() == 1712 && GetAreaId() == 8638 && GetDistance2d(-10574.7002f, 8098.3901f) <= 63.0f)
        return true;
    ...
    {
        if (l_Creature->IsAIEnabled && (l_Creature->AI()->CanTargetOutOfLOS() || l_Creature->AI()->CanTargetOutOfLOSXYZ(ox, oy, oz)))
            return true;
    }

    if (IsInWorld())
        return GetMap()->isInLineOfSight(GetPositionX(), GetPositionY(), GetPositionZH()+2.0f, ox, oy, oz+2.0f, GetPhaseMask());
...
bool WorldObject::HasInLineCheckingStrafe(WorldObject const* target, float width) const
{
    if (!HasInArc(M_PI, target))
        return false;
    ...
    /// Calculate realative angle considering strafe
    float l_Orientation = m_orientation;

    if (m_movementInfo.GetMovementFlags() & MOVEMENTFLAG_STRAFE_LEFT)
        l_Orientation += static_cast<float>(M_PI) / 2.0f;
    if(m_movementInfo.GetMovementFlags() & MOVEMENTFLAG_STRAFE_RIGHT)
        l_Orientation -= static_cast<float>(M_PI) / 2.0f;

    float angle = GetAngle(target) - l_Orientation;

    return fabs(sin(angle)) * GetExactDist2d(target->GetPositionX(), target->GetPositionY()) < width;
}

float GameObject::GetObjectSize() const
{
    if (auto l_GameObjectTemplate = GetGOInfo())
        return l_GameObjectTemplate->size;

    return DEFAULT_WORLD_OBJECT_SIZE;
}

float Unit::GetObjectSize() const
{
...
void Position::GetSinCos(const float x, const float y, float &vsin, float &vcos, Unit* p_Caster /*= nullptr*/) const
{
    ...
    if (fabs(dx) < 0.001f && fabs(dy) < 0.001f)
    {
        ...
        {
            if (this == p_Caster)
                l_Angle = NormalizeOrientation(m_orientation - M_PI);
            else
...
void Position::SimplePosXYRelocationByAngle(Position &pos, float dist, float angle, bool relative /*= false*/) const
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!JadeCore::IsValidMapCoord(pos.m_positionX, pos.m_positionY))
    {
        pos.Relocate(this);
...
void Position::SimplePosXYRelocationByAngle(float &x, float &y, float &z, float dist, float angle, bool relative) const
{
    ...
    // Prevent invalid coordinates here, position is unchanged
    if (!JadeCore::IsValidMapCoord(x, y))
    {
        x = m_positionX;
        y = m_positionY;
        z = m_positionZ;
...
bool WorldObject::IsInAxe(const WorldLocation* obj1, const WorldLocation* obj2, float size) const
{
    if (!obj1 || !obj2)
        return false;
...
void WorldObject::GetAttackableUnitListInRange(std::list<Unit*> &list, float fMaxSearchRange, bool p_ObjectSize /*= true*/) const
{
    CellCoord p(JadeCore::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    JadeCore::AnyUnitInObjectRangeCheck u_check(this, fMaxSearchRange, p_ObjectSize);
...
void WorldObject::GetAttackableUnitListInPositionRange(std::list<Unit*> &list, Position const p_Position, float fMaxSearchRange, bool p_ObjectSize /*= true*/, bool p_AliveCheck /*= true*/) const
{
    CellCoord p(JadeCore::ComputeCellCoord(GetPositionX(), GetPositionY()));
    Cell cell(p);
    cell.SetNoCreate();

    JadeCore::AnyUnitInPositionRangeCheck u_check(p_Position, this, fMaxSearchRange, p_ObjectSize, p_AliveCheck);
    JadeCore::UnitListSearcher<JadeCore::AnyUnitInPositionRangeCheck> searcher(this, list, u_check);

    TypeContainerVisitor<JadeCore::UnitListSearcher<JadeCore::AnyUnitInPositionRangeCheck>, WorldTypeMapContainer > world_unit_searcher(searcher);
    TypeContainerVisitor<JadeCore::UnitListSearcher<JadeCore::AnyUnitInPositionRangeCheck>, GridTypeMapContainer >  grid_unit_searcher(searcher);

    cell.Visit(p, world_unit_searcher, *GetMap(), *this, fMaxSearchRange);
    cell.Visit(p, grid_unit_searcher, *GetMap(), *this, fMaxSearchRange);
}

void WorldObject::GetAreatriggerListInRange(std::list<AreaTrigger*>& p_List, float p_Range) const
{
...
{
    if (PathGenerator::NeedToUseShortcut(GetPosition().AsVector3(), G3D::Vector3(x, y, z), GetMapId()))
        return;

    float l_Offset = GetPositionH() < 2.0f ? 2.0f : 0.0f;
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            if (!ToCreature()->CanFly())
            {
                ...
                float ground_z = z;
                float max_z = canSwim
                    ? GetMap()->GetWaterOrGroundLevel(x, y, z + l_Offset, &ground_z, !ToUnit()->HasAuraType(SPELL_AURA_WATER_WALK))
                    : ((ground_z = GetMap()->GetHeight(GetPhaseMask(), x, y, z + l_Offset, true)));
                max_z += GetPositionH();
                ...
                if (max_z > INVALID_HEIGHT)
                {
                    if (z > max_z)
                    {
                        if (!PathGenerator::NeedToUseShortcut(G3D::Vector3(m_positionX, m_positionY, m_positionZ), G3D::Vector3(x, y, m_positionZ), GetMapId()))
                            z = max_z;
                    }
                    else if (z < ground_z)
                        z = ground_z;
            ...
            {
                float ground_z = GetMap()->GetHeight(GetPhaseMask(), x, y, z + l_Offset, true);
                ground_z += GetPositionH();
                if (z < ground_z)
                    z = ground_z;
            }
        ...
        case TYPEID_PLAYER:
        {
            ...
            {
                float ground_z = z;
                float max_z = GetMap()->GetWaterOrGroundLevel(x, y, z + l_Offset, &ground_z, !ToUnit()->HasAuraType(SPELL_AURA_WATER_WALK));
                max_z += GetPositionH();
                ground_z += GetPositionH();
                if (max_z > INVALID_HEIGHT)
                {
...
{
    if (ToPlayer())
        return GetMap()->GetVisibilityRange(GetZoneId(), GetAreaId());
    else if (ToCreature())
...
float WorldObject::GetSightRange(const WorldObject* target) const
{
    if (ToUnit())
    {
        if (Player const* l_ThisPlayer = ToPlayer())
        {
            ...
            else
            {
                if (Map* map = GetMap())
                {
                    switch (map->GetId())
                    {
                        ...
                        {
                            if (GetAreaId() == 5893) ///< Maelstrom
                                return 500.0f;

                            return map->GetVisibilityRange(l_ThisPlayer->GetZoneId(), l_ThisPlayer->GetAreaId());
                        }
                        case 1520: ///< The Emerald Nightmare
                        {
                            if (GetAreaId() == 8043) ///< Rift of Aln
                                return 500.0f;
...
bool WorldObject::canSeeOrDetect(WorldObject const* p_Object, bool p_IgnoreStealth /*= false*/, bool p_DistanceCheck /*= false*/, bool p_IgnoreInvisibility /*= false*/) const
{
    if (this == p_Object)
        return true;

    Player const* thisPlayer = ToPlayer();
    Player const* objPlayer  = p_Object->ToPlayer();

    if (thisPlayer != nullptr && p_Object->MustBeVisibleOnlyForSomePlayers())
    {
        if (!p_Object->IsPlayerInPersonnalVisibilityList(thisPlayer->GetGUID()))
            return false;
    }

    /// Make sure the client always receive the arena frame packet before the update_object to create the opponent
    if (thisPlayer && objPlayer && thisPlayer->InArena())
    {
        if (thisPlayer->GetBGTeam() != objPlayer->GetBGTeam() && thisPlayer->GetBattleground() && thisPlayer->GetBattleground()->GetStatus() <= STATUS_WAIT_JOIN)
            return false;
    }

    if (p_Object->HideForSomePlayers())
    {
        if (p_Object->ShouldHideFor(GetGUID()))
            return false;
    }

    if (thisPlayer)
    {
        GameObject const* l_Gameobject = p_Object->ToGameObject();
        if (l_Gameobject)
        {
            if (uint32 l_TrackingQuestId = l_Gameobject->GetGOInfo()->GetTrackingQuestId())
            {
                uint32 l_QuestBit = GetQuestUniqueBitFlag(l_TrackingQuestId);
                if (thisPlayer->IsQuestBitFlaged(l_QuestBit))
                    return false;
            }
        }
    }

    if (p_Object->IsNeverVisible() || CanNeverSee(p_Object) || p_Object->CanNeverSee2(this))
        return false;

    if (p_Object->IsAlwaysVisibleFor(this) || CanAlwaysSee(p_Object))
        return true;
    ...
    if (p_DistanceCheck)
    {
        ...
        if (thisPlayer)
        {
            if (thisPlayer->isDead() && thisPlayer->GetHealth() > 0 && // Cheap way to check for ghost state
                !(p_Object->m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GHOST) & m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GHOST) & GHOST_VISIBILITY_GHOST))
            {
                if (Corpse* corpse = thisPlayer->GetCorpse())
                {
                    corpseCheck = true;
                    if (corpse->IsWithinDist(thisPlayer, GetSightRange(p_Object), false))
                        if (corpse->IsWithinDist(p_Object, GetSightRange(p_Object), false))
        ...
        if (!corpseCheck && !viewpoint->IsWithinDist(p_Object, GetSightRange(p_Object), false))
            return false;
    }

    // GM visibility off or hidden NPC
    if (!p_Object->m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GM))
    {
        ...
        return m_serverSideVisibilityDetect.GetValue(SERVERSIDE_VISIBILITY_GM) >= p_Object->m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GM);

    if (Unit const* l_Unit = ToUnit())
    {
        Unit::AuraEffectList const& l_List = l_Unit->GetAuraEffectsByType(AuraType::SPELL_AURA_FOG_OF_WAR);
        for (Unit::AuraEffectList::const_iterator l_Iter = l_List.begin(); l_Iter != l_List.end(); ++l_Iter)
        {
            if (l_Unit->GetDistance(p_Object) > float((*l_Iter)->GetMiscValue()))
                return false;
        }
    }
    ...
    if (!corpseVisibility && !(p_Object->m_serverSideVisibility.GetValue(SERVERSIDE_VISIBILITY_GHOST) & m_serverSideVisibilityDetect.GetValue(SERVERSIDE_VISIBILITY_GHOST)))
    {
        // Alive players can see dead players in some cases, but other objects can't do that
        if (Player const* thisPlayer = ToPlayer())
        {
            if (Player const* objPlayer = p_Object->ToPlayer())
            {
    ...
    if (p_Object->IsInvisibleDueToDespawn())
        return false;

    if (!CanDetect(p_Object, p_IgnoreStealth, p_IgnoreInvisibility))
        return false;

    if (Creature const* l_Creature = p_Object->ToCreature())
    {
        if ((l_Creature->IsAIEnabled && !l_Creature->AI()->CanBeSeenByPlayers()) || l_Creature->HasAura(7870))
            return false;
    }
    else if (Creature const* l_Creature = ToCreature())
    {
        if (l_Creature->IsAIEnabled && !l_Creature->AI()->CanBeSeenByPlayers())
            return false;
    }

    if (thisPlayer && thisPlayer->HasLockedObjects() && thisPlayer->IsObjectInvisibleForPlayer(p_Object))
        return false;

    return true;
}

bool WorldObject::CanDetect(WorldObject const* p_Object, bool p_IgnoreStealth, bool p_IgnoreInvisibility /*= false*/) const
{
    ...
    if (p_Object->IsAlwaysDetectableFor(seer))
        return true;

    if (Creature const* l_Creature = p_Object->ToCreature())
    {
        if (l_Creature->IsAIEnabled && l_Creature->AI()->CanBeSeenForced())
            return true;
    }

    if (!p_IgnoreInvisibility && !seer->CanDetectInvisibilityOf(p_Object))
        return false;

    if (!p_IgnoreStealth && !seer->CanDetectStealthOf(p_Object))
        return false;
...
bool WorldObject::CanDetectStealthOf(WorldObject const* obj) const
{
    ...
    else
    {
        ...
        ///< Stealth & Shroud of concealment should let rogue and his party/raid members be non-detectible within so longer distance
        if (obj->ToUnit() && (obj->ToUnit()->HasAura(115834) || obj->ToUnit()->HasAura(114018) || obj->ToUnit()->HasAura(1784) || obj->ToUnit()->HasAura(115191) || obj->ToUnit()->HasAura(131361)))
            visibilityRange = CalculatePct(visibilityRange, 50);
...
void WorldObject::GenerateMinMaxGold(uint32& p_MinGold, uint32& p_MaxGold)
{
    if (p_MinGold && p_MaxGold)
        return;

    if (ToCreature() && !ToCreature()->isWorldBoss())
        return;

    /// TODO: Create generic calculation for gameobjects if needed.
    if (!ToCreature())
    {
        switch (GetEntry())
        {
            case 254168: ///< Cenarius (Emerald Nightmare)
            case 260526: ///< Odyn (Trial of Valor)
            case 266483: ///< Gul'dan (The Nighthold)
            case 276503: ///< Eonar (Antorus)
            case 277355: ///< Argus (Antorus)
                break;
            default:
                return;
        }
    ...
    {
        case Difficulty::DifficultyNone:
        {
            /// Handle only creatures
            if (!ToCreature())
                break;

            /// Only for World Bosses
            if (!ToCreature()->GetCreatureTemplate() || !ToCreature()->GetCreatureTemplate()->VignetteID)
                break;

            /// Roll between 25 and 75 gold for WB
            p_MinGold = 25 * MoneyConstants::GOLD;
            p_MaxGold = 75 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyNormal:
        {
            /// Roll between 10 and 20 gold for Mythic bosses
            p_MinGold = 10 * MoneyConstants::GOLD;
            p_MaxGold = 20 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyHeroic:
        {
            /// Roll between 20 and 30 gold for Heroic bosses
            p_MinGold = 20 * MoneyConstants::GOLD;
            p_MaxGold = 30 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyMythic:
        {
            /// Roll between 30 and 40 gold for Mythic bosses
            p_MinGold = 30 * MoneyConstants::GOLD;
            p_MaxGold = 40 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyRaidLFR:
        {
            /// Roll between 25 and 50 gold for LFR bosses
            p_MinGold = 25 * MoneyConstants::GOLD;
            p_MaxGold = 50 * MoneyConstants::GOLD;
            break;
            break;
        }
        case Difficulty::DifficultyRaidNormal:
        {
            /// Roll between 50 and 75 gold for Normal bosses
            p_MinGold = 50 * MoneyConstants::GOLD;
            p_MaxGold = 75 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyRaidHeroic:
        {
            /// Roll between 75 and 100 gold for Heroic bosses
            p_MinGold = 75 * MoneyConstants::GOLD;
            p_MaxGold = 100 * MoneyConstants::GOLD;
            break;
        }
        case Difficulty::DifficultyRaidMythic:
        {
            /// Roll between 100 and 150 gold for Mythic bosses
            p_MinGold = 100 * MoneyConstants::GOLD;
            p_MaxGold = 150 * MoneyConstants::GOLD;
            break;
        }
        default:
            break;
...
void WorldObject::SendMessageToSet(WorldPacket* p_Data, bool p_Self)
{
    if (!IsInWorld())
        return;
    ...
    if (sWorld->getBoolConfig(CONFIG_ENABLE_BROADCAST_MESSAGE_BATCH) && WorldPacket::GetCurrentPacketID() != -1)
    {
        WorldPacket::AddMapPacket(PacketSendingInfo(this, *p_Data, p_Self, true));
        return;
    }

    if (p_Self && IsPlayer())
        ToPlayer()->GetSession()->SendPacket(p_Data);

    float l_Distance = GetVisibilityRange();
    JadeCore::MessageDistDeliverer l_Notifier(this, p_Data, l_Distance);
    VisitNearbyWorldObject(l_Distance, l_Notifier);
}
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= NULL*/, uint32 duration /*= 0*/, Unit* summoner /*= NULL*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/, uint64 viewerGuid /*= 0*/, std::list<uint64>* viewersList /*= NULL*/, bool p_Big /*= false*/, bool p_ActiveObject /*= false*/, uint64 summonerGuid /*= 0*/)
{
            ...
            case SUMMON_CATEGORY_UNK:
            {
                switch (properties->Type)
                {
                    ...
                    case SUMMON_TYPE_MINIPET:
                        mask = UNIT_MASK_MINION;
                        break;
                    default:///< Mirror Image, Summon Gargoyle, Void Tendril, Wild Mushroom,             Earth And Fire Spirits,                        Shadowy Reflection  Portal to Mardum(Imp Portal)
                        if ((properties->Flags & 512) || entry == 65282 || entry == 47649 || entry == eMonkPets::Fire ||  entry == eMonkPets::Earth || entry == 97055 || entry == 111748 || entry == 99937)
                            mask = UNIT_MASK_GUARDIAN;
                        break;
                }
                break;
    ...
    // Fix Serpent Jade Statue, Sturdy Ox Statue, Sneaky Snake (Cobra Commander) - is Guardian
    if (entry == 60849 || entry == 61146 || entry == 121661)
        mask = UNIT_MASK_GUARDIAN;

    /// Fix Storm Scarab - 51113 (is Temp Summon instead of Guardian)
    /// Xavius: Dream Simulacrum player's clone
    if (entry == 51113 || entry == 104096)
        mask = UNIT_MASK_MINION;
    ...
    summon->SetBig(p_Big);
    summon->setActive(p_ActiveObject);
    ...
    Player* p_PlayerSummoner = nullptr;
    if (summoner)
    {
        for (uint32 l_ExcludePhase : summoner->GetExcludePhases())
            summon->SetInExcludePhase(l_ExcludePhase, false, false);
        for (uint32 l_ExcludePhase : summoner->GetExcludePhases())
            summon->SetInExcludePhase(l_ExcludePhase, false, true);

        p_PlayerSummoner = summoner->ToPlayer();

        summon->SetGuidValue(UNIT_FIELD_DEMON_CREATOR, summoner->GetGUID());

        if (IS_PLAYER_GUID(summoner->GetGuidValue(UNIT_FIELD_DEMON_CREATOR)))
            summon->SetGuidValue(UNIT_FIELD_DEMON_CREATOR, summoner->GetGuidValue(UNIT_FIELD_DEMON_CREATOR));

        uint32 l_I = summoner->GetPhases().size();
        for (uint32 l_Phase : summoner->GetPhases())
            summon->SetInPhase(l_Phase, !(--l_I), true);
    }

    summon->SetHomePosition(pos);

    summon->InitStats(duration, p_PlayerSummoner);
    ...
    summon->SetSummonerObjectGuid(summonerGuid);
    summon->InitSummon();
...
GameObject* Map::SummonGameObject(uint32 p_Entry, Position const& p_Position, uint32 p_CustomFlags /*= 0*/, bool p_ActiveObject /*= false*/)
{
    GameObject* l_GameObject = new GameObject;

    l_GameObject->SetCustomFlags(p_CustomFlags);
    l_GameObject->setActive(p_ActiveObject);

    if (!l_GameObject->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT), p_Entry, this,
        PHASEMASK_NORMAL, p_Position.GetPositionX(), p_Position.GetPositionY(), p_Position.GetPositionZ(), p_Position.GetOrientation(),
        0, 0, 0, 0, 100, GO_STATE_READY))
    {
        l_GameObject->CleanBeforeGC();
        sGarbageCollector->Add(l_GameObject);
        return nullptr;
    }

    if (!AddToMap(l_GameObject))
        return nullptr;
    ...
    if (Map* map = GetMap())
    {
        ...
        else if (!map->IsBattlegroundOrArena())
        {
            if (Battlefield* bf = sBattlefieldMgr->GetBattlefieldToZoneId(GetZoneId(true)))
                m_zoneScript = bf;
            else if (ZoneScript* l_OutdoorPvP = sOutdoorPvPMgr->GetZoneScript(GetZoneId(true)))
                m_zoneScript = l_OutdoorPvP;
            else if (ZoneScript* l_OutdoorPvE = sOutdoorPvEMgr->GetZoneScript(GetMap()))
                m_zoneScript = l_OutdoorPvE;
            else
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position &pos, TempSummonType spwtype /*= TEMPSUMMON_MANUAL_DESPAW*/, uint32 duration /*= 0*/, uint32 vehId /*= 0*/, uint64 viewerGuid /*= 0*/, std::list<uint64>* viewersList /*= nullptr*/, SummonPropertiesEntry const* p_SumProp /*= nullptr*/, bool p_Big /*= false*/, bool p_ActiveObject /*= false*/) const
{
    if (Map* map = GetMap())
    {
        if (TempSummon* summon = map->SummonCreature(entry, pos, p_SumProp, duration, isType(TYPEMASK_UNIT) ? (Unit*)this : NULL, 0, vehId, viewerGuid, viewersList, p_Big, p_ActiveObject, GetGUID()))
        {
            summon->SetTempSummonType(spwtype);
            if (GameObject const* l_GameObject = ToGameObject())
            {
                if (summon->IsAIEnabled)
                    summon->AI()->IsSummonedByGob(l_GameObject);
            }
            return summon;
        }
    }
...
void Player::SummonPet(uint32 entry, float x, float y, float z, float ang, PetType petType, uint32 duration, PetSlot slotID, bool stampeded, std::function<void(Pet*, bool)> p_Callback, bool p_ByPass)
{
    ...
    if (petType == SUMMON_PET && !p_ByPass)
    {
        ...
        l_Database->AsyncQuery(l_PetStatement, m_QueryCallbackMgr, [entry, x, y, z, ang, petType, duration, l_LoadPetSlotID, slotID, stampeded, p_Callback, l_Pet, currentPet, l_PlayerGUID, l_RealmID](PreparedQueryResult p_Result) -> void
        {
            Player* l_Player = sObjectAccessor->FindPlayer(l_PlayerGUID);
            if (!l_Player)
            {
                l_Pet->CleanBeforeGC();
                sGarbageCollector->Add(l_Pet);
                return;
            }
            ...
            l_Player->GetQueryCallbackMgr()->AddQueryHolderCallback(QueryHolderCallback(l_QueryHolderResultFuture, [entry, x, y, z, ang, petType, duration, l_LoadPetSlotID, slotID, stampeded, p_Callback, l_Pet, currentPet, l_PlayerGUID](SQLQueryHolder* p_QueryHolder) -> void
            {
                Player* l_Player = sObjectAccessor->FindPlayer(l_PlayerGUID);
                if (!l_Player || l_Player != l_Pet->GetOwner() || !p_QueryHolder)
                {
                    l_Pet->CleanBeforeGC();
                    sGarbageCollector->Add(l_Pet);
                    return;
                ...
                l_Pet->LoadPetFromDB(l_Player, entry, 0, currentPet, l_LoadPetSlotID, stampeded, (PetQueryHolder*)p_QueryHolder, [entry, x, y, z, ang, petType, duration, slotID, l_LoadPetSlotID, stampeded, p_Callback, l_PlayerGUID](Pet* p_Pet, bool p_Result) -> void
                {
                    ...
                    if (p_Pet->GetOwner() && p_Pet->GetOwner()->getClass() == CLASS_WARLOCK)
                    {
                        ...
                        if (p_Pet->GetOwner()->getLevel() >= 56)
                        {
                            ...
                            bp = p_Pet->GetCommandDemonSpellByEntry(p_Pet->GetEntry());
                    ...
                    if (p_Pet->IsPetGhoulOrAbomination())
    ...
    if (!l_Pet->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_PET), map, GetPhaseMask(), l_Phases, entry))
    {
        l_Pet->CleanBeforeGC();
        sGarbageCollector->Add(l_Pet);
        p_Callback(nullptr, false);
...
GameObject* WorldObject::SummonGameObject(uint32 entry, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 respawnTime, uint64 viewerGuid /*= 0*/,
    std::list<uint64>* viewersList /*= nullptr*/, uint32 p_AnimProgress /*= 100*/, uint32 p_GoHealth /*= 0*/, bool p_GarrisonPlotObject /*= false*/, bool p_Active /*= false*/, uint32 p_CustomFlags /*= 0*/, bool p_IsActiveObject /*= false*/)
{
    if (!IsInWorld())
        return NULL;
    ...
    if (p_CustomFlags)
        go->SetCustomFlags(p_CustomFlags);
    if (p_IsActiveObject)
        go->setActive(p_IsActiveObject);
    ...
    for (uint32 l_ExcludePhase : GetExcludePhases())
        go->SetInExcludePhase(l_ExcludePhase, false, false);
    for (uint32 l_ExcludePhase : GetExcludePhases())
        go->SetInExcludePhase(l_ExcludePhase, false, true);
...
GameObject* WorldObject::SummonGameObject(uint32 p_Entry, Position const& p_Pos, Position p_Rotation, uint32 p_RespTime /*= 0*/, uint64 p_ViewerGuid /*= 0*/,
    std::list<uint64>* p_ViewerList /*= nullptr*/, uint32 p_AnimProgress /*= 100*/, uint32 p_GoHealth /*= 0*/, bool p_GarrisonPlotObject /*= false*/, bool p_Active /*= false*/, uint32 p_CustomFlags /*= 0*/, bool p_IsActiveObject /*= false*/)
{
    return SummonGameObject(p_Entry, p_Pos.m_positionX, p_Pos.m_positionY, p_Pos.m_positionZ, p_Pos.m_orientation, p_Rotation.m_positionX, p_Rotation.m_positionY, p_Rotation.m_positionZ,
        p_Rotation.m_orientation, p_RespTime, p_ViewerGuid, p_ViewerList, p_AnimProgress, p_GoHealth, p_GarrisonPlotObject, p_Active, p_CustomFlags, p_IsActiveObject);
}
        ...
        switch (itr->actionType)
        {
            case SUMMON_ACTION_TYPE_DEFAULT:
            {
                if (TempSummon* summon = SummonCreature(itr->entry, itr->pos, itr->type, itr->time))
                {
                    ...
                    tempSummonGroupList[groupKey].insert(summon->GetGUID());
                }
                break;
            }
            case SUMMON_ACTION_TYPE_ROUND_HOME_POS:
            {
                ...
                for (uint8 i = 0; i < itr->count; ++i)
                {
                    ...
                    if (TempSummon* summon = SummonCreature(itr->entry, posHome, itr->type, itr->time))
                    {
                        if (list)
                            list->push_back(summon);
            ...
            case SUMMON_ACTION_TYPE_ROUND_SUMMONER:
            {
                ...
                for (uint8 i = 0; i < itr->count; ++i)
                {
                    ...
                    if (TempSummon* summon = SummonCreature(itr->entry, pos, itr->type, itr->time))
                    {
                        if (list)
                            list->push_back(summon);
...
Creature* WorldObject::FindDetectableNearestCreature(uint32 entry, float range, bool alive) const
{
    Creature* creature = NULL;
    JadeCore::NearestCreatureEntryWithLiveStateInObjectRangeCanBeSeenCheck checker(*this, entry, alive, range);
    JadeCore::CreatureLastSearcher<JadeCore::NearestCreatureEntryWithLiveStateInObjectRangeCanBeSeenCheck> searcher(this, creature, checker);
    VisitNearbyObject(range, searcher);
    return creature;
}
...
Player* WorldObject::FindNearestPlayer(float range)
{
    ...
    JadeCore::NearestPlayerInObjectRangeCheck check(this, range);
    JadeCore::PlayerLastSearcher<JadeCore::NearestPlayerInObjectRangeCheck> searcher(this, player, check);
    VisitNearbyWorldObject(range, searcher);
    return player;
}

Player* WorldObject::FindNearestDeadPlayer(float p_Range)
{
    Player* l_Player = nullptr;
    JadeCore::NearestDeadPlayerInObjectRangeCheck l_Check(this, p_Range);
    JadeCore::PlayerLastSearcher<JadeCore::NearestDeadPlayerInObjectRangeCheck> l_Searcher(this, l_Player, l_Check);
    VisitNearbyWorldObject(p_Range, l_Searcher);
    return l_Player;
}
...
void WorldObject::GetPlayerListInGrid(std::list<Player*>& playerList, float maxSearchRange, bool p_Self /*= false*/, bool p_AliveCheck /*= true*/) const
{
    JadeCore::AnyPlayerInObjectRangeCheck checker(this, maxSearchRange, p_AliveCheck, p_Self);
...
void WorldObject::GetNearPoint(WorldObject const* p_Searcher, float &p_InOutX, float &p_InOutY, float &p_InOutZ, float p_SearcherSize, float p_Distance2D, float p_AbsAngle) const
{
    ...
    p_InOutZ = GetPositionZH() + (p_Searcher != nullptr ? p_Searcher->GetPositionH() : 0.0f);

    /// Should "searcher" be used instead of "this" when updating z coordinate ?
    if (!p_Searcher)
        UpdateAllowedPositionZ(p_InOutX, p_InOutY, p_InOutZ);
    else if (!p_Searcher->ToCreature())
        p_Searcher->UpdateAllowedPositionZ(p_InOutX, p_InOutY, p_InOutZ);
...
void WorldObject::MovePosition(Position &pos, float dist, float angle)
{
    ...
    PositionToVector(l_Pos1);
    pos.PositionToVector(l_Pos2);

    bool l_NeedToUseShortcut = false;
    if (PathGenerator::NeedToUseShortcut(l_Pos1, l_Pos2, GetMapId()))
    {
        destz = m_positionZ;
        l_NeedToUseShortcut = true;
    }
    else
    {
    ...
    JadeCore::NormalizeMapCoord(pos.m_positionX);
    JadeCore::NormalizeMapCoord(pos.m_positionY);
    if (!l_NeedToUseShortcut || fabs(destz) > 10000.0f)
        UpdateGroundPositionZ(pos.m_positionX, pos.m_positionY, pos.m_positionZ);
    pos.SetOrientation(GetOrientation());
}

void WorldObject::GetNearestPosition(Position &pos, float dist, float angle)
{
    GetPosition(&pos);
    float destx, desty, destz, ground, floor;
    ...
    while(!JadeCore::IsValidMapCoord(destx, desty, pos.m_positionZ) && dist > 0.0f);
...
void WorldObject::ConvertPhaseMaskToPhaseId()
{
    static const uint32 k_PhaseMaskToPhaseId = 88899900;
    for (int l_I = 1; l_I < 32; l_I++)
        SetInPhase(k_PhaseMaskToPhaseId + l_I, false, (m_phaseMask & uint32(1 << (l_I - 1))) != 0);
}

bool WorldObject::SetInPhase(uint32 id, bool update, bool apply)
{
    Map* l_ThreadLocalMap = Map::GetCurrentMapThread();
    if (m_currMap && l_ThreadLocalMap && l_ThreadLocalMap != m_currMap)
    {
        sLog->outExtChat("#jarvis-log", "danger", true, "Threadguard triggered for Unit (GUID = %u) (Map (Id : %u) modifying Map (Id : %u))", GetGUIDLow(), l_ThreadLocalMap->GetId(), m_currMap->GetId());
...
bool WorldObject::SetInExcludePhase(uint32 id, bool update, bool apply)
{
    if (id)
    {
        if (apply)
        {
            if (HasInExcludePhaseList(id)) // do not run the updates if we are already in this phase
                return false;

            m_ExcludePhases.insert(id);
        }
        else
        {
            if (!HasInExcludePhaseList(id)) // do not run the updates if we are not in this phase
                return false;

            m_ExcludePhases.erase(id);
        }
    }
...
void FillWithPhaseMgr(WorldObject const* p_Obj, std::set<uint32>& p_Phases)
{
    if (p_Obj->GetTypeId() == TYPEID_PLAYER)
    {
        std::set<uint32> const& l_ActivePhases = p_Obj->ToPlayer()->GetPhaseMgr().GetActivesPhases();
        if (l_ActivePhases.size())
            p_Phases.insert(l_ActivePhases.begin(), l_ActivePhases.end());
    }
}

bool WorldObject::InSamePhase(WorldObject const* p_Target) const
{
    /// GMs can see everything no matter what
    if (Player const* l_Player = p_Target->IsPlayer() ? p_Target->ToPlayer() : this->ToPlayer())
    {
    ...
    /// Excluding phases
    /// Ugly but efficient
    /// Description: if either this or the target is a player or a player's summon then check if excluded phases are intersecting.
    if (((
          ((m_objectType & TypeMask::TYPEMASK_UNIT) && (IsPlayer() || IS_PLAYER_GUID(GetGuidValue(EUnitFields::UNIT_FIELD_DEMON_CREATOR)))) ||
          ((m_objectType & TypeMask::TYPEMASK_GAMEOBJECT) && (IsPlayer() || IS_PLAYER_GUID(GetGuidValue(EGameObjectFields::GAMEOBJECT_FIELD_CREATED_BY))))
        )
        ^
        (
          ((p_Target->m_objectType & TypeMask::TYPEMASK_UNIT) && (p_Target->IsPlayer() || IS_PLAYER_GUID(p_Target->GetGuidValue(EUnitFields::UNIT_FIELD_DEMON_CREATOR)))) ||
          ((p_Target->m_objectType & TypeMask::TYPEMASK_GAMEOBJECT) && (p_Target->IsPlayer() || IS_PLAYER_GUID(p_Target->GetGuidValue(EGameObjectFields::GAMEOBJECT_FIELD_CREATED_BY))))
        ))
        && Containers::Intersects(m_ExcludePhases.begin(), m_ExcludePhases.end(), p_Target->m_ExcludePhases.begin(), p_Target->m_ExcludePhases.end()))
        return false;

    /// This check will cover 99% of the case and is very fast
    if (m_phaseMask & p_Target->m_phaseMask)
        return true;

    return Containers::Intersects(m_Phases.begin(), m_Phases.end(), p_Target->m_Phases.begin(), p_Target->m_Phases.end());
}

bool WorldObject::HasInPhaseList(uint32 phase) const
{
    return m_Phases.find(phase) != m_Phases.end();
}

bool WorldObject::HasInExcludePhaseList(uint32 phase) const
{
    return m_ExcludePhases.find(phase) != m_ExcludePhases.end();
}
...
void WorldObject::PlayDirectMusic(uint32 musicId, Player * p_Target /*= NULL*/)
{
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    for (std::list<Player*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
    {
        ...
        if (isType(TYPEMASK_PLAYER) && player->PreventClientDestroyFor(GetGUID()))
            continue;

        DestroyForPlayer(player);

        player->m_clientGUIDs_lock.acquire_write();
        player->m_clientGUIDs.erase(GetGUID());
        player->m_clientGUIDs_lock.release();
...
{
    if ((ToGameObject() && (ToGameObject()->IsTransport() || ToGameObject()->GetGoType() == GameobjectTypes::GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING)) || IsBig())
    {
...
void WorldObject::ForceSendUpdate()
{
    UpdateDataMapType l_UpdateData;
    BuildUpdate(l_UpdateData);

    WorldPacket l_Packet;
    for (UpdateDataMapType::iterator l_Iter = l_UpdateData.begin(); l_Iter != l_UpdateData.end(); ++l_Iter)
    {
        l_Iter->second.BuildPacket(&l_Packet);
        l_Iter->first->GetSession()->SendPacket(&l_Packet);
        l_Packet.clear();
    }
}
...
void WorldObject::RemoveFromObjectUpdate()
{
    ...
    if (Unit* l_Unit = ToUnit())
        GetMap()->RemoveUpdateVisibilityObject(l_Unit);
}

TempSummon* WorldObject::SummonCreature(uint32 p_ID, SpellDestination const* p_Dest, TempSummonType p_Type, uint32 p_DespawnTime, uint64 p_Viewer, std::list<uint64>* p_Viewers)
{
    return SummonCreature(p_ID, p_Dest->_position.m_positionX, p_Dest->_position.m_positionY, p_Dest->_position.m_positionZ, p_Dest->_position.m_orientation, p_Type, p_DespawnTime, p_Viewer, p_Viewers);
}

void WorldObject::SetVisible(bool on)
{
    if (!on)
