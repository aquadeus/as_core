...
void GossipMenu::AddMenuItem(int32 menuItemId, uint8 icon, uint32 p_BroadcastTextId, LocaleConstant p_Locale, uint32 sender, uint32 action, std::string const& boxMessage, uint32 boxMoney, bool coded /*= false*/)
{
    ASSERT(_menuItems.size() <= GOSSIP_MAX_MENU_ITEMS);
    ...
    std::string message;
    if (BroadcastTextEntry const* l_BroadcastText = sBroadcastTextStore.LookupEntry(p_BroadcastTextId))
        message = GetBroadcastTextValue(l_BroadcastText, p_Locale, GENDER_MALE);
...
void GossipMenu::AddMenuItem(uint32 menuId, uint32 menuItemId, uint32 sender, uint32 action)
{
    ...
    for (GossipMenuItemsContainer::const_iterator itr = bounds.first; itr != bounds.second; ++itr)
    {
        ...
        if (localeConstant >= 0)
        {
            ...
            else if (GossipMenuItemsLocale const* gossipMenuLocale = sObjectMgr->GetGossipMenuItemsLocale(MAKE_PAIR64(menuId, menuItemId)))
...
void PlayerMenu::ClearGossipMenu()
{
    _gossipMenu.ClearMenu();
}
...
void PlayerMenu::SendGossipMenu(uint32 l_TextID, uint64 objectGUID) const
{
    _session->GetPlayer()->SetInteractionStatus(objectGUID, InteractionStatus::Gossip);

    WorldPacket l_Data(SMSG_GOSSIP_MESSAGE, 2 * 1024);

    l_Data.appendPackGUID(objectGUID);                        ///< GossipGUID

    if (l_TextID == DEFAULT_GOSSIP_MESSAGE && !_gossipMenu.GetMenuId())
        l_Data << uint32(DEFAULT_GREETINGS_GOSSIP);           ///< default greeting ID
    else
        l_Data << uint32(_gossipMenu.GetMenuId());            ///< GossipID

    l_Data << uint32(0);                                      ///< FriendshipFactionID
    l_Data << uint32(l_TextID);                               ///< TextID
    l_Data << uint32(_gossipMenu.GetMenuItems().size());      ///< GossipOptions
    l_Data << uint32(_questMenu.GetMenuItemCount());          ///< GossipText
    ...
    for (GossipMenuItemContainer::const_iterator l_It = _gossipMenu.GetMenuItems().begin(); l_It != _gossipMenu.GetMenuItems().end(); ++l_It)
    {
        ...
        l_Data << uint8(l_Item.IsCoded);                      ///< BoxCoded
        l_Data << uint32(l_Item.BoxMoney);                    ///< BoxMoney
    ...
    for (uint32 l_It = 0; l_It < _questMenu.GetMenuItemCount(); ++l_It)
    {
        ...
        uint32 l_QuestType = l_Player ? l_Player->GetQuestStatus(l_QuestID) : 0;

        if (l_QuestType == QUEST_STATUS_COMPLETE || l_QuestType == QUEST_STATUS_INCOMPLETE)
        {
        ...
        else if (l_QuestType == QUEST_STATE_NONE)
            l_QuestType = 2;

        l_Data << uint32(l_QuestID);
        l_Data << uint32(l_QuestType);                        ///< QuestType
        l_Data << int32(l_Quest->GetQuestLevel());            ///< QuestLevel
        l_Data << int32(l_Quest->GetQuestMaxScalingLevel());  ///< QuestMaxScalingLevel
        l_Data << uint32(l_Quest->GetFlags());                ///< QuestFlags
        l_Data << uint32(l_Quest->GetFlags2());               ///< QuestFlagsEx

        l_Data.WriteBit(l_Quest->IsRepeatable());             ///< Repeatable
...
void PlayerMenu::SendQuestGiverQuestList(QEmote eEmote, const std::string& Title, uint64 npcGUID)
{
    ...
    for (uint32 count = 0; count < _questMenu.GetMenuItemCount(); ++count)
    {
        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(questID))
        {
            ...
            data << int32(quest->GetQuestMaxScalingLevel());
...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* quest, uint64 npcGUID) const
{
    ...
    data.appendPackGUID(guid);                                                                  ///< QuestGiverGUID
    data.appendPackGUID(guid2);                                                                 ///< InformUnit
    ...
    data << uint32(0);                                                                          ///< @todo legion: QuestStartItemID
...

    {
        ...
        data << uint32(quest->MoneyValue(_session->GetPlayer()->getLevel()));
        data << uint32(quest->XPValue(_session->GetPlayer()) * QuestXpRate);
        data << uint64(quest->GetRewArtifactXP());
        data << uint32(0);                                                                      ///< @todo Legion: NYI ArtifactCategoryID
        ...
        for (uint32 i = 0; i < QUEST_REPUTATIONS_COUNT; ++i)
        {
            ...
            data << uint32(quest->RewardFactionCapIn[i]);
        }

        for (uint32 l_I = 0; l_I < QUEST_REWARD_SPELL_COUNT; ++l_I)
            data << uint32(quest->RewardSpells[l_I]);
        ...
        data << uint32(0);                                                                      ///< @todo Legion: RewardID

        data.WriteBit(false);                                                                   ///< @todo Legion: IsBoostSpell
        data.FlushBits();
    }
...
void PlayerMenu::SendQuestQueryResponse(Quest const* p_Quest) const
{
    ...
    uint32 l_PlayerLevel = _session->GetPlayer() ? _session->GetPlayer()->getLevel() : 1;
    ...
    int64 l_MoneyReward = 0;
    if (!l_HideItemReward)
        l_MoneyReward = p_Quest->RewardMoney < 0 ? p_Quest->RewardMoney : p_Quest->MoneyValue(_session->GetPlayer()->getLevel());
    ...
    l_Data << uint32(p_Quest->GetQuestMaxScalingLevel());                                       ///< Quest Max Scaling Level
    ...
    l_Data << uint32(p_Quest->GetXPId());                                                       ///< RewardXPDifficulty
    l_Data << float(p_Quest->GetXPMultiplier());                                                ///< RewardXPMultiplier
    l_Data << int32(l_MoneyReward);                                                             ///< Reward Money
    l_Data << uint32(p_Quest->GetMoneyDifficulty());                                            ///< Reward Money Difficulty
    l_Data << float(p_Quest->GetMoneyMultiplier());                                             ///< Reward Money Multiplier
    l_Data << uint32(p_Quest->GetRewMoneyMaxLevel());                                           ///< RewardBonusMoney
    l_Data << uint32(p_Quest->RewardSpells[0]);                                                 ///< Reward Display Spell, this spell will display (icon) (casted if RewSpellCast == 0)
    l_Data << uint32(p_Quest->RewardSpells[1]);                                                 ///< Reward Display Spell, this spell will display (icon) (casted if RewSpellCast == 0)
    l_Data << uint32(p_Quest->RewardSpells[2]);                                                 ///< Reward Display Spell, this spell will display (icon) (casted if RewSpellCast == 0)
    ...
    l_Data << uint32(p_Quest->GetRewArtifactXPDifficulty());                                    ///< Reward Artifact XP Difficulty
    l_Data << float(p_Quest->GetRewArtifactXPMultilier());                                      ///< Reward Artifact XP Multilpier
    l_Data << uint32(p_Quest->GetRewArtifactXP());                                              ///< Reward Artifact XP
    ...
    l_Data << uint32(0);                                                                        ///< RewardArenaPoints
    ...
    for (uint32 l_I = 0; l_I < QUEST_REPUTATIONS_COUNT; ++l_I)
    {
        ...
        l_Data << uint32(p_Quest->RewardFactionCapIn[l_I]);                                     ///< Reward Faction Cap In
    }
    ...
    l_Data << uint32(p_Quest->GetAreaGroupId());                                                ///< AreaGroupID
    l_Data << uint32(p_Quest->GetLimitTime());                                                  ///< Time Allowed
    l_Data << uint32(p_Quest->QuestObjectives.size());                                          ///< Objective Count
    l_Data << uint64(p_Quest->GetRequiredRaces());                                              ///< AllowableRaces
    l_Data << uint32(p_Quest->QuestRewardID());                                                 ///< QuestRewardID
    l_Data << uint32(p_Quest->GetExpansion());                                                  ///< Expansion
    ...
    for (auto const& l_Objective : p_Quest->QuestObjectives)
    {
        ...
        l_Data << uint32(l_Objective.Flags2);                                                   ///< Flags2
        l_Data << float(l_Objective.ProgressBarWeight);                                         ///< ProgressBarWeight
...
void PlayerMenu::SendQuestGiverOfferReward(Quest const* p_Quest, uint64 p_NpcGUID, bool p_EnableNext) const
{
    ...
    l_Data << uint32(p_Quest->MoneyValue(_session->GetPlayer()->getLevel()));
    l_Data << uint32(p_Quest->XPValue(_session->GetPlayer()) * l_QuestXpRate);
    l_Data << uint64(p_Quest->GetRewArtifactXP());
    l_Data << uint32(0); ///< questRewards.ArtifactCategoryID from TC NYI
    l_Data << uint32(p_Quest->GetRewHonorAddition());
    ...
    for (uint32 l_I = 0; l_I < QUEST_REPUTATIONS_COUNT; ++l_I)
    {
        ...
        l_Data << uint32(p_Quest->RewardFactionCapIn[l_I]);
    }

    for (int32 l_I = 0; l_I < QUEST_REWARD_SPELL_COUNT; ++l_I)
        l_Data << uint32(p_Quest->RewardSpells[l_I]);
    ...
    l_Data << uint32(0);                        ///< questRewards.RewardID from TC NYI
...
void PlayerMenu::SendQuestGiverRequestItems(Quest const* quest, uint64 npcGUID, bool canComplete, bool /*closeOnCancel*/) const
{
    ...
    for (auto const& l_Objective : quest->QuestObjectives)
    {
        if (l_Objective.Type == QUEST_OBJECTIVE_TYPE_ITEM)
        {
            ...
            l_RequiredItemData << uint32(0);                    ///< RewardID
