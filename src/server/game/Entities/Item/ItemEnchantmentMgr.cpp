...
uint32 CalculateEnchantmentBonus(SpellItemEnchantmentEntry const* p_SpellItemEnchantement, uint32 p_Amount, uint8 p_Index, uint8 p_CharacterLevel)
{
    ...
    if (p_SpellItemEnchantement->m_ScalingClass)
    {
        ...
        int32 l_Unk = (p_SpellItemEnchantement->Flags & 0x20) != 0 ? 1 : 60;
        ...
        SpellScalingTableEntry const* l_GtScaling = g_SpellScalingTable.LookupEntry(l_FinalLevel);

        if (l_GtScaling)
            l_Amount = round(GetSpellScalingColumnForClass(l_GtScaling, l_ScalingClassIndex) * p_SpellItemEnchantement->m_EffectScalingPoints[p_Index]);
    }
