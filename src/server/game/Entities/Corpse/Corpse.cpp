...
{
    SetMap(map);
    /// @note: When updating to 128 send mapid as owner
...
bool Corpse::LoadCorpseFromDB(uint32 guid, Field* fields)
{
    ...
    if (CharacterInfo const* l_CharacterInfo = sWorld->GetCharacterInfo(GetGuidValue(CORPSE_FIELD_OWNER)))
        SetUInt32Value(CORPSE_FIELD_FACTION_TEMPLATE, sChrRacesStore[l_CharacterInfo->Race]->FactionID);
    ...
    uint32 l_ZoneId = sMapMgr->GetZoneId(mapId, posX, posY, posZ);
    if (Map* l_Map = sMapMgr->CreateBaseMap(mapId, l_ZoneId))
        l_Map->AddCorpse(this);
