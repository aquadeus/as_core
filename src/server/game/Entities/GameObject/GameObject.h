...
struct GameObjectTemplate
{
    ...
    uint32 GetLootId() const
    {
        switch (type)
        {
            ...
            case GAMEOBJECT_TYPE_GATHERING_NODE:        return entry;
    ...
    {
        uint32 l_Vignette = GetVignetteId();

        if (VignetteEntry const* l_VignetteEntry = sVignetteStore.LookupEntry(l_Vignette))
        {
            if (l_VignetteEntry->QuestID)
                return l_VignetteEntry->QuestID;
        }
...
/*          case GAMEOBJECT_TYPE_CAPTURE_POINT:             ///< not sure about this one (2018-08-28)
    ...
    bool IsChallengeChest() const
    {
        return entry == 252680 || entry == 252681 || entry == 252673;
    }

    uint8 GetChallengeChestLevelBonus() const
    {
        switch (entry)
        {
            case 252681:
...
class GameObject : public WorldObject, public GridObject<GameObject>, public MapObject
{
    public:
        ...
        bool Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit = 0, uint32 p_GoHealth = 0, uint32 p_PhaseID = 0);
        ...
        bool LoadGameObjectFromDB(uint32 guid, Map* map, bool addToMap = true, bool p_GenerateNewGuid = true);
        void DeleteFromDB();
        ...
        std::unordered_map<uint64, Loot> m_LootContainers;
        ...
        void CastSpell(Position const p_Position, uint32 p_SpellID);
        ...
        void SetCustomFlags(uint32 p_CustomFlags) { m_CustomFlags = p_CustomFlags; }

        void SetRotationQuat(float p_QX, float p_QY, float p_QZ, float p_QW);
        void SetRotationQuat(Position const* p_Position)
        {
            SetRotationQuat(p_Position->m_positionX, p_Position->m_positionY, p_Position->m_positionZ, p_Position->m_orientation);
        }

        void SendGameObjectActivateAnimKit(uint32 p_AnimKitID, bool p_Maintain = true, Player* p_Target = nullptr);
        void SetLastLootTime(uint32 p_Time) { m_LastLootTime = p_Time; }
        uint32 GetLastLootTime() const { return m_LastLootTime; }

        /// Event handler
        EventProcessor m_Events;

        void AddDelayedEvent(std::function<void()> p_Lambda, uint32 p_Delay)
        {
        ...
        void SetTransport(Transport* p_Transport) override
        {
            WorldObject::SetTransport(p_Transport);
            if (p_Transport)
    ...
    protected:
        ...
        uint32      m_LastLootTime;
        bool        m_spawnedByDefault;
        time_t      m_cooldownTime;                         // used as internal reaction delay time store (not state change reaction).
                                                            // For traps this: spell casting cooldown, for doors/buttons: reset time.
        std::list<uint32> m_SkillupList;

        uint64 m_ritualOwner;                              // used for GAMEOBJECT_TYPE_RITUAL where GO is not summoned (no owner)
