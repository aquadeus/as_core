...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map* map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit, uint32 p_GoHealth, uint32 p_PhaseID)
{
    {
    ...
    SetBig(m_CustomFlags & eGoBCustomFlags::CustomFlagBig);
    ...
    if (p_PhaseID)
        SetInPhase(p_PhaseID, true, true);
    ...
    if (m_model && GetMap() && GetMap()->ContainsGameObjectModel(*m_model))
    {
        GetMap()->RemoveGameObjectModel(*m_model);
        delete m_model;
        m_model = nullptr;
    ...
    if (m_model && m_model->isMapObject())
        SetFlag(GAMEOBJECT_FIELD_FLAGS, GO_FLAG_MAP_OBJECT);
    ...
    m_CachedZoneId = GetZoneId();

    switch (goinfo->type)
    {
        case GAMEOBJECT_TYPE_DESTRUCTIBLE_BUILDING:
            /// Get the values somehow, no longer in gameobject_template
            m_goValue->Building.Health = 20000; ///< goinfo->destructibleBuilding.intactNumHits + goinfo->destructibleBuilding.InteriorVisible;
        ...
        case GAMEOBJECT_TYPE_PHASEABLE_MO:
            SetByteValue(GAMEOBJECT_FIELD_FLAGS, 1, m_goInfo->phaseableMO.AreaNameSet & 0xF);
            break;
        default:
    ...
    sLog->outInfo(LOG_FILTER_POINTERS, "Pointer 0x%" PRIxPTR " -> GameObject entry %u Guid: %u", std::uintptr_t(this), GetEntry(), GetDBTableGUIDLow());
...
{
    ASSERT(GetGOInfo()->type == GAMEOBJECT_TYPE_TRANSPORT || GetGOInfo()->type == GAMEOBJECT_TYPE_MAP_OBJ_TRANSPORT);
    if (m_goValue->Transport.AnimationInfo)
...
void GameObject::SetTransportState(GOState state, uint32 stopFrame /*= 0*/)
{
    ...
    {
        ASSERT(uint8(state) < GO_STATE_TRANSPORT_STOPPED + MAX_GO_STATE_TRANSPORT_STOP_FRAMES);
...
uint32 CalculateDynamicResapawnTime(GameObject* p_GameObject)
{
    uint32 l_RespawnTime = p_GameObject->GetRespawnDelay();
    if (!sWorld->getBoolConfig(CONFIG_CREATURE_RESPAWN_DYNAMIC) || p_GameObject->GetMap()->Instanceable())
        return l_RespawnTime;

    float l_Radius = sWorld->getIntConfig(CONFIG_CREATURE_RESPAWN_SEARCHER_RANGE);

    std::list<Player*> l_Player;
    JadeCore::AnyUnitInObjectRangeCheck l_Check(p_GameObject, l_Radius);
    JadeCore::PlayerListSearcher<JadeCore::AnyUnitInObjectRangeCheck> searcher(p_GameObject, l_Player, l_Check);
    p_GameObject->VisitNearbyObject(l_Radius, searcher);
...
void GameObject::Update(uint32 diff)
{
    ...
    if (GetGoType() == GAMEOBJECT_TYPE_TRANSPORT || GetGoType() == GAMEOBJECT_TYPE_MAP_OBJ_TRANSPORT)
    {
    ...
    if (AreaTriggerStruct const* l_AreaTriggerPoint = sObjectMgr->GetGameObjectAreaTriggerTeleport(GetEntry(), GetMapId(), m_CachedZoneId))
    {
        std::list<Player*> l_Targets;
        JadeCore::AnyPlayerInObjectRangeCheck l_Check(this, l_AreaTriggerPoint->radius, false);
        JadeCore::PlayerListSearcher<JadeCore::AnyPlayerInObjectRangeCheck> l_Searcher(this, l_Targets, l_Check);
        VisitNearbyWorldObject(l_AreaTriggerPoint->radius, l_Searcher);

        for (std::list<Player*>::const_iterator l_Itr = l_Targets.begin(); l_Itr != l_Targets.end(); ++l_Itr)
            (*l_Itr)->TeleportTo(l_AreaTriggerPoint->target_mapId, l_AreaTriggerPoint->target_X, l_AreaTriggerPoint->target_Y, l_AreaTriggerPoint->target_Z, l_AreaTriggerPoint->target_Orientation);
    }
    ...
    switch (m_lootState)
    {
        ...
        case GO_READY:
        {
            ...
            if (isSpawned())
            {
                ...
                else if (goInfo->type == GAMEOBJECT_TYPE_GATHERING_NODE)
                {
                    if (GetLastLootTime() && GetLastLootTime() + 30 < time(nullptr))
                    {
                        SetLootState(GO_JUST_DEACTIVATED);
                    }
                }
        ...
        case GO_ACTIVATED:
        {
            switch (GetGoType())
            {
                ...
                case GAMEOBJECT_TYPE_GATHERING_NODE:
                    /// Prevent the gameobject to be stuck in GO_ACTIVATED loot state if the player open it but don't loot it.
                    if (m_LastLootTime && !GetMap()->Instanceable() && (time(nullptr) - m_LastLootTime) > 60)
                    {
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            m_respawnTime = time(NULL) + CalculateDynamicResapawnTime(this);
...
bool GameObject::LoadGameObjectFromDB(uint32 guid, Map* map, bool addToMap, bool p_GenerateNewGuid)
{
    ...
    if (!Create(guid, entry, map, phaseMask, x, y, z, ang, rotation0, rotation1, rotation2, rotation3, animprogress, go_state, artKit, 0, data->PhaseID))
        return false;
...
{
    /// Make artifact chest in hub glow
    if (sWorld->getBoolConfig(CONFIG_TOURNAMENT_ENABLE))
    {
        if (GetEntry() == 934221 || GetEntry() == 934220)
            return true;
    }
    ...
    switch (GetGoType())
    {
        ...
        {
            if (GetGOInfo()->generic.questID == -1 || target->GetQuestStatus(GetGOInfo()->generic.questID) == QUEST_STATUS_INCOMPLETE) ///< Comparison of integers of different signs: 'uint32' (aka 'unsigned int') and 'int'
...
void GameObject::Use(Unit* p_User)
{
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_RITUAL:              //18
        {
            ...
            Player* l_RitualOwner = sObjectAccessor->FindPlayer(m_ritualOwner);

            // ritual owner is set for GO's without owner (not summoned)
            if (!m_ritualOwner && !owner)
            {
                m_ritualOwner = player->GetGUID();
                l_RitualOwner = player;
            }
            ...
            {
                if (player != l_RitualOwner && (info->ritual.castersGrouped && !player->IsInSameRaidWith(l_RitualOwner)))
                    return;
            ...
            {
                spellCaster = l_RitualOwner ? l_RitualOwner : spellCaster;
                if (l_RitualOwner)
                    spellCaster = l_RitualOwner;
        ...
        case GAMEOBJECT_TYPE_MEETINGSTONE:                  //23
        {
            ...
            Player* l_TargetPlayer = ObjectAccessor::FindPlayer(l_Player->GetSelection());

            if (l_Owner && l_Owner->IsPlayer())
            {
                // accept only use by player from same raid as owner, except caster itself
                if (!l_TargetPlayer || l_TargetPlayer == l_Player || !l_TargetPlayer->IsInSameRaidWith(l_Owner->ToPlayer()))
                    return;
            }
            else
            {
                // accept only use by player from same raid as caster, except caster itself
                if (!l_TargetPlayer || l_TargetPlayer == l_Player || !l_TargetPlayer->IsInSameRaidWith(l_Player))
                    return;
            }

            //required lvl checks!
            uint8 level = l_Player->getLevel();
            if (level < info->meetingStone.minLevel)
                return;
            level = l_TargetPlayer->getLevel();
            if (level < info->meetingStone.minLevel)
                return;
        ...
        case GAMEOBJECT_TYPE_FLAGSTAND:                     // 24
        {
            ...
            if (player->CanUseBattlegroundObject())
            {
                ...
                player->RemoveAurasByType(SPELL_AURA_MECHANIC_IMMUNITY);
        ...
        case GAMEOBJECT_TYPE_FISHINGHOLE:                   // 25
        {
            ...
            Loot* l_Loot = &m_LootContainers[player->GetGUID()];
            l_Loot->clear();
            l_Loot->Context             = GetMap()->GetLootItemContext(player);
            l_Loot->SpawnMode           = GetMap()->GetSpawnMode();
            l_Loot->m_PersonalLootFilter = true;
            l_Loot->SetSource(GetGUID());

            l_Loot->FillLoot(GetGOInfo()->GetLootId(), LootTemplates_Gameobject, player, true);

            player->SendLoot(GetGUID(), LOOT_FISHINGHOLE);
            player->UpdateCriteria(CRITERIA_TYPE_FISH_IN_GAMEOBJECT, GetGOInfo()->entry);
            player->UpdateCriteria(CRITERIA_TYPE_CATCH_FROM_POOL, GetGOInfo()->entry);
            return;
        }
        ...
        case GAMEOBJECT_TYPE_ARTIFACT_FORGE:
        {
            ...
            Item* l_Item = l_Artifact->ToItem();

            if (!l_Item)
                return;

            l_Player->GetSession()->SendOpenArtifactForge(l_Item->GetGUID(), GetGUID());
            return;
        }
        ...
        case GAMEOBJECT_TYPE_UI_LINK:
        {
            ...
            WorldPacket l_Data(SMSG_GAME_OBJECT_UI_ACTION);
            l_Data.appendPackGUID(GetGUID());
            l_Data << uint32(eUILink::ObliterumForge);
            l_Player->GetSession()->SendPacket(&l_Data);
            return;
        }
        default:
    ...
    if (!spellInfo)
    {
        if (p_User->GetTypeId() != TYPEID_PLAYER || !sOutdoorPvPMgr->HandleCustomSpell(p_User->ToPlayer(), spellId, this))
            ...
            sLog->outDebug(LOG_FILTER_OUTDOORPVP, "WORLD: %u non-db2 spell was handled by OutdoorPvP", spellId);
        return;
    }
...
void GameObject::CastSpell(Position const p_Position, uint32 p_SpellID)
{
    ...
    if (Unit* l_Owner = GetOwner())
    {
        ...
        l_Trigger->CastSpell(p_Position, p_SpellID, true, 0, nullptr, l_Owner->GetGUID());
    }
    else
    {
...
void GameObject::ModifyHealth(int32 change, Unit* attackerOrHealer /*= NULL*/, uint32 spellId /*= 0*/)
{
    ...
    else if (GetGOValue()->Building.Health <= GetGOInfo()->destructibleBuilding.InteriorVisible)
...
void GameObject::SetDestructibleState(GameObjectDestructibleState state, Player* eventInvoker /*= NULL*/, bool setHealth /*= false*/)
{
    ...
    switch (state)
    {
        ...
        case GO_DESTRUCTIBLE_DAMAGED:
        {
            ...
            if (setHealth)
            {
                m_goValue->Building.Health = m_goInfo->destructibleBuilding.InteriorVisible;
...
void GameObject::SetGoState(GOState state)
{
    ...
    if (InstanceScript* l_Instance = GetInstanceScript())
        l_Instance->OnGameObjectStateChanged(this, state);
...
void GameObject::UpdateModel()
{
    ...
    ApplyModFlag(GAMEOBJECT_FIELD_FLAGS, GO_FLAG_MAP_OBJECT, m_model && m_model->isMapObject());
}
...
bool GameObject::IsLootAllowedFor(Player const* p_Player) const
{
    ...
    GroupPtr const playerGroup = p_Player->GetGroup();
    ...
    for (uint16 index = 0; index < m_valuesCount; ++index)
    {
            ...
            (index == GAMEOBJECT_FIELD_FLAGS && forcedFlags) || index == OBJECT_FIELD_DYNAMIC_FLAGS || index == GAMEOBJECT_FIELD_PERCENT_HEALTH || (index == GAMEOBJECT_FIELD_LEVEL && IsTransport()))
        {
            ...
            if (index == OBJECT_FIELD_DYNAMIC_FLAGS)
            {
                ...
                switch (GetGoType())
                {
                    ...
                    case GAMEOBJECT_TYPE_CHALLENGE_MODE_REWARD:
                    {
                        if (sChallengeMgr->HasOploteLoot(target->GetGUID()))
                            dynFlags |= GO_DYNFLAG_LO_CHALLENGE_AVAILABLE;
                        else if (sChallengeMgr->HasRecordLastWeek(target->GetGUID()))
                            dynFlags |= GO_DYNFLAG_LO_CHALLENGE_CLAIMED;
                        else
                            dynFlags |= GO_DYNFLAG_LO_CHALLENGE_NO_LOOTS;
                ...
                if (AI())
                    AI()->OnBuildDynFlags(dynFlags, target);
            ...
            else if (index == GAMEOBJECT_FIELD_DISPLAY_ID)
            {
                ...
                if (AI())
                    AI()->OnBuildDisplayId(modelID, target);
