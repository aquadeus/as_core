...
void OutdoorPvPMgr::HandlePlayerUpdateArea(Player* p_Player, uint32 p_OldAreaID, uint32 p_NewAreaID)
{
    OutdoorPvPMap::iterator l_Itr = m_OutdoorPvPMap.find(p_Player->GetZoneId());
    if (l_Itr == m_OutdoorPvPMap.end())
        return;

    l_Itr->second->HandlePlayerUpdateArea(p_Player, p_OldAreaID, p_NewAreaID);
}
...
void OutdoorPvPMgr::HandleLootReleased(Player* p_Player, uint64 p_GUID)
{
    for (OutdoorPvPSet::iterator l_Itr = m_OutdoorPvPSet.begin(); l_Itr != m_OutdoorPvPSet.end(); ++l_Itr)
        (*l_Itr)->HandleLootReleased(p_Player, p_GUID);
}
