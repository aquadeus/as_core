...
class OPvPCapturePoint
{
    public:
        ...
        virtual int32 HandleLootReleased(Player* p_Player, uint64 p_GUID);
...
class OutdoorPvP : public ZoneScript
{
    ...
    public:
        ...
        virtual bool HandleAreaTrigger(Player* player, uint32 trigger, bool p_Enter);
        ...
        /// Called when a player just looted from an object
        ///  Be careful, check type before declaring entity, might be creature, gob, corpse...
        virtual void HandleLootReleased(Player* p_Player, uint64 p_GUID);
        ...
        /// Don't forget to check returned value, can be invalid.
        Creature* GetCreatureFromMap(uint64 p_GUID, uint32 p_MapID, uint32 p_InstanceID = 0);
        /// Don't forget to check returned value, can be invalid.
        GameObject* GetGameObjectFromMap(uint64 p_GUID, uint32 p_MapID, uint32 p_InstanceID = 0);
    ...
    protected:
        ...
        virtual void HandlePlayerUpdateArea(Player* /*p_Player*/, uint32 /*p_OldAreaID*/, uint32 /*p_NewAreaID*/) { }
        ...
        void RegisterZone(uint32 p_MapId, uint32 p_ZoneId);

        virtual bool HasPlayer(Player* player) const;
        ...
        bool AddCreature(uint32 p_Type, uint32 p_Entry, uint32 p_Team, uint32 p_MapID, Position p_Pos, uint32 p_SpawnTime = 0)
        {
            return AddCreature(p_Type, p_Entry, p_Team, p_MapID, p_Pos.m_positionX, p_Pos.m_positionY, p_Pos.m_positionZ, p_Pos.m_orientation, p_SpawnTime);
        }
        ...
        bool DelCreatureWithGUID(uint64 p_GUID);
        ...
        bool AddObject(uint32 p_Type, uint32 p_Entry, uint32 p_MapID, Position p_Pos, Position p_Rotation)
        {
            return AddObject(p_Type, p_Entry, p_MapID, p_Pos.m_positionX, p_Pos.m_positionY, p_Pos.m_positionZ, p_Pos.m_orientation, p_Rotation.m_positionX, p_Rotation.m_positionY, p_Rotation.m_positionZ, p_Rotation.m_orientation);
        }
