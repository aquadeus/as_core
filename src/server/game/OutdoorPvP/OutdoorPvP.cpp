...
OutdoorPvP::OutdoorPvP() : m_sendUpdate(true), m_LastResurectTimer(0), m_MapId(0)
{
}
...
Creature* OutdoorPvP::GetCreatureFromMap(uint64 p_GUID, uint32 p_MapID, uint32 p_InstanceID /*= 0*/)
{
    if (Map* l_Map = sMapMgr->FindMap(p_MapID, p_InstanceID))
        return l_Map->GetCreature(p_GUID);

    return nullptr;
}

GameObject* OutdoorPvP::GetGameObjectFromMap(uint64 p_GUID, uint32 p_MapID, uint32 p_InstanceID /*= 0*/)
{
    if (Map* l_Map = sMapMgr->FindMap(p_MapID, p_InstanceID))
        return l_Map->GetGameObject(p_GUID);
...
void OutdoorPvP::HandleLootReleased(Player* p_Player, uint64 p_GUID)
{
    for (OPvPCapturePointMap::iterator itr = m_capturePoints.begin(); itr != m_capturePoints.end(); ++itr)
        itr->second->HandleLootReleased(p_Player, p_GUID);
}
...
int32 OPvPCapturePoint::HandleLootReleased(Player* p_Player, uint64 p_GUID)
{
    switch (uint32 HighGUID = GUID_HIPART(p_GUID))
    {
        case HIGHGUID_GAMEOBJECT:
        {
            std::map<uint64, uint32>::iterator itr = m_ObjectTypes.find(p_GUID);
        ...
        case HIGHGUID_UNIT:
        {
            std::map<uint64, uint32>::iterator itr = m_CreatureTypes.find(p_GUID);
...
bool OutdoorPvP::HandleAreaTrigger(Player* /*player*/, uint32 /*trigger*/, bool /*p_Enter*/)
{
    return false;
}
...
void OutdoorPvP::RegisterZone(uint32 p_MapId, uint32 p_ZoneId)
{
    m_MapId = p_MapId;
    sOutdoorPvPMgr->AddZone(p_ZoneId, this);
}
...
void OutdoorPvP::AddAreaTrigger(uint32 p_Entry, uint32 p_PhaseMask, uint32 p_SpellVisualID, Position const& p_Pos, uint32 p_Duration, Map* p_Map)
{
    ...
    //if (!l_AreaTrigger->CreateAreaTrigger(sObjectMgr->GenerateLowGuid(HIGHGUID_AREATRIGGER), p_Entry, nullptr, nullptr, p_Pos, p_Pos))
...
bool OutdoorPvP::DelCreatureWithGUID(uint64 p_GUID)
{
    if (!m_CreatureTypes[p_GUID])
    {
        sLog->outDebug(LOG_FILTER_OUTDOORPVP, "OutdoorPvP::DelCreature, creature guid " UI64FMTD " was already deleted", p_GUID);
        return false;
    }

    Creature* l_Creature = HashMapHolder<Creature>::Find(p_GUID);
    if (!l_Creature)
    {
        // Can happen when closing the core
        m_CreatureTypes[p_GUID] = 0;
        return false;
    }

    sLog->outDebug(LOG_FILTER_OUTDOORPVP, "OutdoorPvP::DelCreature, deleting creature guid " UI64FMTD " ", p_GUID);
    ...
    l_Creature->AddObjectToRemoveList();
    sObjectMgr->DeleteCreatureData(p_GUID);
    m_Creatures[m_CreatureTypes[p_GUID]] = 0;
    m_CreatureTypes[p_GUID] = 0;
    return true;
}
...
void OutdoorGraveyard::Resurrect()
{
    ...
    for (GuidSet::const_iterator l_Iter = m_ResurrectQueue.begin(); l_Iter != m_ResurrectQueue.end(); ++l_Iter)
    {
        ...
        l_Player->GetMap()->ConvertCorpseToBones(l_Player->GetGUID());
    }
