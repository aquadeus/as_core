...
namespace Battlepay
{
    ...
    struct Product
    {
        ...
        std::map<LocaleConstant, std::string> CustomLocaleData;

        /// Custom fields
        std::pair<uint32, uint32>   CustomData;
        bool                        HasSpecialCustomData;
        bool                        HasSpecialLocaleData;
        /*
         * Serialize the product data to json
         * Needed for the website purchase history
         */

        void SetSpecialCustomData(uint32 p_TypeCosmetic, uint32 p_CosmeticID)
        {
            HasSpecialCustomData = true;
            CustomData = {p_TypeCosmetic, p_CosmeticID};
        }

        void SetSpecialLocaleData(std::map<LocaleConstant, std::string> p_Data)
        {
            HasSpecialLocaleData = true;
            CustomLocaleData = p_Data;
        }
