...
namespace Battlepay
{
    ...
    void Manager::SavePurchase(WorldSession* p_Session, Battlepay::Purchase* p_Purchase)
    {
        ...
        l_Statement->setUInt32(3, p_Session->GetPlayer() ? p_Session->GetPlayer()->GetGUIDLow() : 0);
        l_Statement->setString(4, l_Product.Serialize());
        l_Statement->setUInt32(5, 1);
        l_Statement->setUInt32(6, p_Purchase->CurrentPrice);
        l_Statement->setString(7, p_Session->GetRemoteAddress());

        uint32 l_SessionID = p_Session->GetAccountId();

        WebDatabase.AsyncQuery(l_Statement, sWorld->GetQueryCallbackMgr(), [l_SessionID](PreparedQueryResult p_Result) -> void
        {
    ...
    {
        if (WorldSession* l_Session = sWorld->FindSession(p_AccountId))
        {
    ...
    std::string Product::Serialize() const
    {
        ...
        l_Node["Expansion"]    = std::to_string(Expansion::EXPANSION_LEGION);    ///< Legion
        l_Node["Type"]         = std::to_string(WebsiteType);
        l_Node["Quantity"]     = std::to_string(1);
        l_Node["IngameShop"]   = 1;

        if (HasSpecialCustomData)
        {
            EasyJSon::Node<std::string> l_CustomDataNode;
            l_CustomDataNode["TypeCosmetic"] = CustomData.first;
            l_CustomDataNode["CosmeticID"] = CustomData.second;
            l_Node["CustomData"] = l_CustomDataNode;
        }
        else
        ...
        std::map<uint32, uint32> l_IngameToWebsiteLocale =
        {
            ...
            { LocaleConstant::LOCALE_ptPT, 7 }
        };

        if (!HasSpecialLocaleData)
        {
        ...
        else
        {
            ...
            {
                std::string l_Value = "";
                auto l_SecondItr = CustomLocaleData.find(static_cast<LocaleConstant>(l_Itr.first));
                if (l_SecondItr != CustomLocaleData.end())
                    l_Value = (*l_SecondItr).second;
