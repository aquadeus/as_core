...
void GameEventMgr::Initialize()
{
    ...
    if (result)
    {
        ...
        uint16 maxEventId = fields[0].GetUInt16();
        ...
        m_GameEventSpawnedCreatureGuids.resize(maxEventId * 2 - 1);
        mGameEventGameobjectGuids.resize(maxEventId * 2 - 1);
        m_GameEventSpawnedGobGuids.resize(maxEventId * 2 - 1);
...
void GameEventMgr::GameEventSpawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventCreatureGuids element %i (size: %zu)",
    ...
    for (GuidList::iterator itr = mGameEventCreatureGuids[internal_event_id].begin(); itr != mGameEventCreatureGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            sMapMgr->AddTaskToAllMapsWithId(data->mapid, [data, itr, this, internal_event_id]()->void
            {
                ...
                if (!l_Map->Instanceable() && l_Map->IsGridLoaded(data->posX, data->posY))
                {
                    ...
                    if (!creature->LoadCreatureFromDB(*itr, l_Map))
                        sGarbageCollector->Add(creature);
                    else
                    {
                        if (internal_event_id < int32(m_GameEventSpawnedCreatureGuids.size()))
                        {
                            m_SpawnedCreatureGuidsLock.lock();
                            /// Use or create an iterator to this instance ID
                            auto& l_GuidSet = m_GameEventSpawnedCreatureGuids[internal_event_id][l_Map->GetInstanceId()];

                            /// Register real spawned guid, will be useful to despawn the Gobs on every instances of a same map
                            l_GuidSet.insert(creature->GetGUID());
                            m_SpawnedCreatureGuidsLock.unlock();
                        }
                    }
                }
            });
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventGameobjectGuids element %i (size: %zu)",
    ...
    for (GuidList::iterator itr = mGameEventGameobjectGuids[internal_event_id].begin(); itr != mGameEventGameobjectGuids[internal_event_id].end(); ++itr)
    {
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            sMapMgr->AddTaskToAllMapsWithId(data->mapid, [data, itr, this, internal_event_id, event_id]()->void
            {
                Map* l_Map = Map::GetCurrentMapThread();

                /// Don't spawn Gurubashi chest on Tournament if there are less than 15 players
                if (event_id == 16 && sWorld->getBoolConfig(CONFIG_TOURNAMENT_ENABLE) && l_Map->GetPlayerCount() < 15)
                    return;
                ...
                if (!l_Map->Instanceable() && l_Map->IsGridLoaded(data->posX, data->posY))
                {
                    ...
                    if (!pGameobject->LoadGameObjectFromDB(*itr, l_Map, false))
                        sGarbageCollector->Add(pGameobject);
                    else
                    {
                        if (pGameobject->isSpawnedByDefault())
                            l_Map->AddToMap(pGameobject);

                        if (internal_event_id < int32(m_GameEventSpawnedGobGuids.size()))
                        {
                            m_SpawnedGobGuidsLock.lock();
                            /// Use or create an iterator to this instance ID
                            auto& l_GuidSet = m_GameEventSpawnedGobGuids[internal_event_id][l_Map->GetInstanceId()];

                            /// Register real spawned guid, will be useful to despawn the Gobs on every instances of a same map
                            l_GuidSet.insert(pGameobject->GetGUID());
                            m_SpawnedGobGuidsLock.unlock();
                        }
                    }
                }
            });
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventSpawn attempt access to out of range mGameEventPoolIds element %u (size: %zu)",
...
void GameEventMgr::GameEventUnspawn(int16 event_id)
{
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventCreatureGuids.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventCreatureGuids element %i (size: %zu)",
            internal_event_id, mGameEventCreatureGuids.size());
        return;
    }
        ...
        if (CreatureData const* data = sObjectMgr->GetCreatureData(*itr))
        {
            ...
            if (internal_event_id < int32(m_GameEventSpawnedCreatureGuids.size()))
            {
                sMapMgr->AddTaskToAllMapsWithId(data->mapid, [data, itr, this, internal_event_id, event_id]()->void
                {
                    ...
                    if (m_GameEventSpawnedCreatureGuids[internal_event_id].find(l_Map->GetInstanceId()) == m_GameEventSpawnedCreatureGuids[internal_event_id].end())
                        return;

                    m_SpawnedCreatureGuidsLock.lock();
                    auto& l_GuidSet = m_GameEventSpawnedCreatureGuids[internal_event_id][l_Map->GetInstanceId()];

                    /// Despawn and un-register every spawned gobs on every instances of a same map
                    for (uint64 const& l_Guid : l_GuidSet)
                    {
                        if (Creature* l_Creature = l_Map->GetCreature(l_Guid))
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventGameobjectGuids.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventGameobjectGuids element %i (size: %zu)",
            internal_event_id, mGameEventGameobjectGuids.size());
        return;
    }
        ...
        if (GameObjectData const* data = sObjectMgr->GetGOData(*itr))
        {
            ...
            if (internal_event_id < int32(m_GameEventSpawnedGobGuids.size()))
            {
                sMapMgr->AddTaskToAllMapsWithId(data->mapid, [data, itr, this, internal_event_id, event_id]()->void
                {
                    ...
                    if (m_GameEventSpawnedGobGuids[internal_event_id].find(l_Map->GetInstanceId()) == m_GameEventSpawnedGobGuids[internal_event_id].end())
                        return;
                    ...
                    for (uint64 const& l_Guid : l_GuidSet)
                    {
                        if (GameObject* l_Gob = l_Map->GetGameObject(l_Guid))
    ...
    if (internal_event_id < 0 || internal_event_id >= int32(mGameEventPoolIds.size()))
    {
        sLog->outError(LOG_FILTER_GAMEEVENTS, "GameEventMgr::GameEventUnspawn attempt access to out of range mGameEventPoolIds element %u (size: %zu)", internal_event_id, mGameEventPoolIds.size());
        return;
    }
...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
    ...
    sMapMgr->AddTaskToAllMaps([activate, event_id]()->void
    {
        Map* l_Map = Map::GetCurrentMapThread();
        if (!l_Map)
            return;
        ...
        /// Copy is intended
        auto l_WorldObjects = l_Map->GetObjectsOnMap();
        for (WorldObject* l_WorldObject : l_WorldObjects)
        {
            ...
            if (l_WorldObject->GetTypeId() == TYPEID_UNIT)
            {
                ...
                if (!l_Creature || !l_Creature->IsInWorld() || !l_Creature->AI())
                    continue;

                l_Creature->AddDelayedEvent([l_Creature, activate, event_id]() -> void
                {
                    if (l_Creature->AI())
                        l_Creature->AI()->sOnGameEvent(activate, event_id);
                });
            }

            if (l_WorldObject->GetTypeId() == TYPEID_GAMEOBJECT)
            {
                GameObject* l_GameObject = l_WorldObject->ToGameObject();
                if (!l_GameObject || !l_GameObject->IsInWorld() || !l_GameObject->AI())
                    continue;

                l_GameObject->AddDelayedEvent([l_GameObject, activate, event_id]() -> void
                {
                    if (l_GameObject->AI())
                        l_GameObject->AI()->OnGameEvent(activate, event_id);
                }, 0);
            }
        }
    });
