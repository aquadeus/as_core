    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            ...
            case SCRIPT_COMMAND_CAST_SPELL:
            {
                ...
                if (!uSource || !uSource->isType(TYPEMASK_UNIT) || !uSource->IsInWorld())
                {
                ...
                if (!uTarget || !uTarget->isType(TYPEMASK_UNIT) || !uTarget->IsInWorld())
                {
                ...
                /// We can't cast a spell with a caster / target that isn't in our map
                /// This can cause crashs because of mutli threaded map updates
                if (uSource->GetMap() != this || uTarget->GetMap() != this)
                    break;
            ...
            case SCRIPT_COMMAND_SET_ANIMKIT:
            {
                // Source must be Creature.
                if (Creature* l_Creature = _GetScriptCreature(source, true, step.script))
                    l_Creature->SetAIAnimKitId(step.script->SetAnimKit.AnimKitID);
                break;
            }
            default:
