...
void DoScriptText(int32 p_ItemTextEntry, WorldObject* p_Source, Unit* p_Target)
{
    ...
    {
        if (sSoundKitStore.LookupEntry(l_TextDatas->uiSoundId))
...
#define FOR_SCRIPTS_NO_RETN(T, C, E) \
    for (SCR_REG_ITR(T) C = SCR_REG_LST(T).begin(); \
        C != SCR_REG_LST(T).end(); ++C)
#define FOREACH_SCRIPT_NO_RETN(T) \
    FOR_SCRIPTS_NO_RETN(T, itr, end) \
    itr->second
...
void ScriptMgr::Unload()
{
    ...
    ScriptRegistry<OutdoorPvEMapScript>::Clear();
    ScriptRegistry<CommandScript>::Clear();
    ScriptRegistry<WeatherScript>::Clear();
    ScriptRegistry<SceneTriggerScript>::Clear();
    ScriptRegistry<EventObjectScript>::Clear();
#ifndef CROSS
...
void ScriptMgr::InitScriptEntity(AreaTrigger* p_AreaTrigger)
{
    ...
    AreaTriggerEntityScript* l_AreaTriggerScript = ScriptRegistry<AreaTriggerEntityScript>::GetScriptById(p_AreaTrigger->GetAreaTriggerInfo().ScriptId);
...
void ScriptMgr::BeforeCreateAreaTriggerEntity(AreaTrigger* p_AreaTrigger)
{
    ASSERT(p_AreaTrigger);
    ...
    l_Script->BeforeCreate(p_AreaTrigger);
}
...
void ScriptMgr::OnSetCreatePositionEntity(AreaTrigger* p_AreaTrigger, Unit* p_Caster, Position& p_SourcePosition, Position& p_DestinationPosition, std::vector<G3D::Vector3>& p_PathToLinearDestination)
{
    ASSERT(p_AreaTrigger);
...
bool ScriptMgr::OnAreaTriggerBounce(AreaTrigger* p_AreaTrigger)
{
    if (!p_AreaTrigger->GetScript())
        return true;

    return p_AreaTrigger->GetScript()->OnAreaTriggerBounce(p_AreaTrigger);
}

bool ScriptMgr::OnSpellAreaTriggerAddTarget(AreaTrigger* p_AreaTrigger, Unit* p_Target)
{
    (p_AreaTrigger);
    ...
    return p_AreaTrigger->GetScript()->OnAddTarget(p_AreaTrigger, p_Target);
}

bool ScriptMgr::OnSpellAreaTriggerRemoveTarget(AreaTrigger* p_AreaTrigger, Unit* p_Target)
{
    ASSERT(p_AreaTrigger);
    ...
    return p_AreaTrigger->GetScript()->OnRemoveTarget(p_AreaTrigger, p_Target);
}

void ScriptMgr::OnMoveAreaTrigger(AreaTrigger* p_AreaTrigger, uint32 p_ArrivalTime)
{
    ASSERT(p_AreaTrigger);
    ...
    p_AreaTrigger->GetScript()->OnMoveAreaTrigger(p_AreaTrigger, p_ArrivalTime);
}
...
bool ScriptMgr::OnSceneTrigger(Player* p_Player, SpellScene const* p_Trigger, const std::string& p_Triggername)
{
    ASSERT(p_Player);
    ASSERT(p_Trigger);

    GET_SCRIPT_RET(SceneTriggerScript, p_Trigger->scriptID, l_Script, false);
    return l_Script->OnTrigger(p_Player, p_Trigger, p_Triggername);
}

bool ScriptMgr::OnEventObject(Player* p_Player, EventObject* p_Event)
{
    ASSERT(p_Player);
    ASSERT(p_Event);

    GET_SCRIPT_RET(EventObjectScript, p_Event->GetScriptId(), l_Script, false);
    return l_Script->OnTrigger(p_Player, p_Event);
}
...
void ScriptMgr::OnCloseGossip(Player* p_Player, Creature* p_Creature)
{
    ASSERT(p_Player);
    ASSERT(p_Creature);

    if (CreatureScript* l_TempScript = p_Creature->GetCreatureScript())
        l_TempScript->OnCloseGossip(p_Player, p_Creature);
}
...
AreaTriggerAI* ScriptMgr::GetAreaTriggerAI(AreaTrigger* areatrigger)
{
    ...
    GET_SCRIPT_RET(AreaTriggerScript, sObjectMgr->GetAreaTriggerScriptId(areatrigger->GetCustomEntry()), tmpscript, nullptr);
...
void ScriptMgr::OnGroupAddMember(GroupPtr p_Group, uint64 p_GUID)
{
    ASSERT(p_Group);
...
void ScriptMgr::OnGroupInviteMember(GroupPtr p_Group, uint64 p_GUID)
{
    ASSERT(p_Group);
...
void ScriptMgr::OnGroupRemoveMember(GroupPtr p_Group, uint64 p_GUID, RemoveMethod p_Method, uint64 p_KickerGUID, const char* p_Reason)
{
    ASSERT(p_Group);
...
void ScriptMgr::OnGroupChangeLeader(GroupPtr p_Group, uint64 p_NewLeaderGUID, uint64 p_OldLeaderGUID)
{
    ASSERT(p_Group);
...
/// @p_Group : Group Instance
void ScriptMgr::OnGroupDisband(GroupPtr p_Group)
{
    ASSERT(p_Group);
...
/// Called when a player selects a gossip item in the creature's gossip menu.
/// @p_Player   : Source player instance
/// @p_Item     : Target item instance
/// @p_Sender   : Sender menu
/// @p_Action   : Action
bool ScriptMgr::OnGossipSelect(Player* p_Player, Item* p_Item, uint32 p_Sender, uint32 p_Action)
{
    ASSERT(p_Player);
    ASSERT(p_Item);

    GET_SCRIPT_RET(ItemScript, p_Item->GetScriptId(), tmpscript, false);
    return tmpscript->OnGossipSelect(p_Player, p_Item, p_Sender, p_Action);
}
...
bool ScriptMgr::OnItemCreate(Player* p_Player, const ItemTemplate* p_ItemTemplate, Item* p_Item)
{
    ASSERT(p_Player);
    ASSERT(p_ItemTemplate);
    ASSERT(p_Item);
    ...
    GET_SCRIPT_RET(ItemScript, p_ItemTemplate->ScriptId, tmpscript, false);
    return tmpscript->OnCreate(p_Player, p_Item);
}
...
void ScriptMgr::OnPlayerEnterMap(Map* p_Map, Player* p_Player)
{
    ...
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnMapEnter(p_Player);
    FOREACH_SCRIPT_NO_RETN(ContributionScript)->OnPlayerEnterMap(p_Player, p_Map);
...
void ScriptMgr::OnPlayerLeaveMap(Map* p_Map, Player* p_Player)
{
    ...
    FOREACH_SCRIPT(PlayerScript)->OnMapLeave(p_Player);
}
...
/// Called when reactive socket I/O is started (RealmSocketMgr/InstanceSocketMgr).
...
void ScriptMgr::OnPacketReceive(WorldPacket p_Packet, WorldSession* p_Session)
{
    FOREACH_SCRIPT(ServerScript)->OnPacketReceive(p_Packet, p_Session);
}
...
void ScriptMgr::OnAuctionAdd(AuctionHouseObject* p_AuctionHouseObject, AuctionEntryPtr p_Entry)
{
...
void ScriptMgr::OnAuctionRemove(AuctionHouseObject* p_AuctionHouseObject, AuctionEntryPtr p_Entry)
{
    ASSERT(p_AuctionHouseObject);
    ASSERT(p_Entry);
...
void ScriptMgr::OnAuctionSuccessful(AuctionHouseObject* p_AuctionHouseObject, AuctionEntryPtr p_Entry)
{
    ASSERT(p_AuctionHouseObject);
    ASSERT(p_Entry);
...
void ScriptMgr::OnAuctionExpire(AuctionHouseObject* p_AuctionHouseObject, AuctionEntryPtr p_Entry)
{
    ASSERT(p_AuctionHouseObject);
    ASSERT(p_Entry);
...
void ScriptMgr::OnBaseGainCalculation(uint32 & p_Gain, uint8 p_PlayerLevel, uint8 p_MobLevel)
{
    FOREACH_SCRIPT(FormulaScript)->OnBaseGainCalculation(p_Gain, p_PlayerLevel, p_MobLevel);
}
...
/// @p_Player : Player
/// @p_Time : Time since last Update
void ScriptMgr::OnUpdate(Player* p_Player, uint32 p_Time)
{
    FOREACH_SCRIPT(PlayerScript)->OnUpdate(p_Player, p_Time);
}
...
/// @p_Killer : Killer instance
/// @p_Killed : Killed instance
void ScriptMgr::OnKill(Player* p_Killer, Unit* p_Killed, bool p_BeforeAurasRemoved /*= false*/)
{
    FOREACH_SCRIPT(PlayerScript)->OnKill(p_Killer, p_Killed, p_BeforeAurasRemoved);
}

/// Called when a Unit is killed
/// @p_Killed : Unit instance
/// @p_Attacker : Unit instance
void ScriptMgr::OnKillReward(Unit* p_Killed, Unit* p_Attacker)
{
    FOREACH_SCRIPT(PlayerScript)->OnKillReward(p_Killed, p_Attacker);
}
...
/// Called when melee hit is redirected
/// @p_newVictim : New victim
/// @p_oldVictim : Old victim
/// @p_RedirectAura : Aura which redirected
void ScriptMgr::OnMeleeHitRedirect(Unit* p_NewVictim, Unit* p_OldVictim, Aura* p_RedirectAura)
{
    FOREACH_SCRIPT(PlayerScript)->OnMeleeHitRedirect(p_NewVictim, p_OldVictim, p_RedirectAura);
}

/// Called when a player finishes a non-taxi move spline
/// @p_Player : Player instance
void ScriptMgr::OnMoveSplineDone(Player* p_Player)
{
    FOREACH_SCRIPT(PlayerScript)->OnMoveSplineDone(p_Player);
}

/// Called when a Player has finished casting a spell (after cast time is complete)
/// @p_Player : Player instance
/// @p_Spell : Spell instance
/// @ok : check if spell has really finished casting or if it was interrupted, cancelled, etc
void ScriptMgr::OnSpellFinished(Player* p_Player, Spell* p_Spell, bool ok)
{
    FOREACH_SCRIPT(PlayerScript)->OnSpellFinished(p_Player, p_Spell, ok);
}

/// Called when a player summons a pet
/// @p_Player : Player instance
void ScriptMgr::OnSummonPet(Player* p_Player)
{
    FOREACH_SCRIPT(PlayerScript)->OnSummonPet(p_Player);
}

/// Called when a player's minions is removed
/// @p_Player : Player instance
/// @p_Minion : Removed Minion
void ScriptMgr::OnMinionDespawn(Player* p_Player, Minion* p_Minion)
{
    FOREACH_SCRIPT(PlayerScript)->OnMinionDespawn(p_Player, p_Minion);
}
...
/// Called when the player consume or restore a spell Charge
/// @p_Player : Player instance
/// @p_CategoryChargeEntryID : ID of the entry of the modified categorycharge
void ScriptMgr::OnModifyCharges(Player* p_Player, uint32 p_CategoryChargeEntryID)
{
    FOREACH_SCRIPT(PlayerScript)->OnModifyCharges(p_Player, p_CategoryChargeEntryID);
}

/// Called when specialisation is modify (SetSpecializationId)
/// @p_Player : Player instance
/// @p_NewSpec  : New Specialisation
void ScriptMgr::OnModifySpec(Player* p_Player, int32 p_OldSpec, int32 p_NewSpec)
{
    FOREACH_SCRIPT(PlayerScript)->OnModifySpec(p_Player, p_OldSpec, p_NewSpec);
}

/// Called when a player kills another player
/// @p_Player : Player instance
/// @p_Value  : New value
void ScriptMgr::OnModifyHealth(Player* p_Player, int64 p_Value, int64 p_oldValue)
{
    FOREACH_SCRIPT(PlayerScript)->OnModifyHealth(p_Player, p_Value, p_oldValue);
}
...
/// @p_Player : Player
/// @p_SpellID : SpellID
void ScriptMgr::OnTeleport(Player* p_Player, const SpellInfo*p_SpellInfo, bool p_WorldPort)
{
    FOREACH_SCRIPT(PlayerScript)->OnTeleport(p_Player, p_SpellInfo, p_WorldPort);
}
...
void ScriptMgr::OnPlayerChat(Player* p_Player, uint32 p_Type, uint32 p_Lang, std::string & p_Message, GroupPtr p_Group)
{
...
/// Called when a player choses a response from a PlayerChoice
/// @p_Player       : Player that chooses a response
/// @p_ChoiceID     : Id of the Playerchoice
/// @p_ResponseID   : Id of the response chosen by the player
void ScriptMgr::OnPlayerChoiceResponse(Player* p_Player, uint32 p_ChoiceID, uint32 p_ResponseID)
{
    FOREACH_SCRIPT(PlayerScript)->OnPlayerChoiceResponse(p_Player, p_ChoiceID, p_ResponseID);
}
...
/// @p_Player    : Player instance
/// @p_Spell     : Casted spell
void ScriptMgr::OnSpellInterrupt(Player* p_Player, Spell* p_Spell, Spell* p_InterruptingSpell, uint32 p_InterruptingSpellID)
{
    FOREACH_SCRIPT(PlayerScript)->OnSpellInterrupt(p_Player, p_Spell, p_InterruptingSpell, p_InterruptingSpellID);
}
...
/// @p_Spell     : Casted spell
/// @p_SkipCheck : Skipped checks
void ScriptMgr::AfterPlayerSpellCast(Player* p_Player, Spell* p_Spell, bool p_SkipCheck)
{
    FOREACH_SCRIPT(PlayerScript)->AfterSpellCast(p_Player, p_Spell, p_SkipCheck);
}
...
/// When the player unlearns a spell
/// @p_Player  : Player instance
/// @p_SpellID : Learned spell ID
void ScriptMgr::OnPlayerSpellRemoved(Player* p_Player, uint32 p_SpellID)
{
    FOREACH_SCRIPT(PlayerScript)->OnSpellRemoved(p_Player, p_SpellID);
}
...
{
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnLogin(p_Player);
    FOREACH_SCRIPT_NO_RETN(POIScript)->OnPlayerLogin(p_Player);
    FOREACH_SCRIPT_NO_RETN(ContributionScript)->OnPlayerLogin(p_Player);
}
...
void ScriptMgr::OnPlayerBindToInstance(Player* p_Player, Difficulty p_Difficulty, uint32 p_MapID, bool p_Permanent, uint8 p_ExtendState)
{
    FOREACH_SCRIPT(PlayerScript)->OnBindToInstance(p_Player, p_Difficulty, p_MapID, p_Permanent, p_ExtendState);
}
...
{
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnUpdateZone(p_Player, p_NewZoneID, p_OldZoneID, p_NewAreaID);
    FOREACH_SCRIPT_NO_RETN(POIScript)->OnPlayerUpdateZone(p_Player, p_NewZoneID, p_OldZoneID);
}

/// Called when a player updates his movement
/// @p_Player : Player instance
void ScriptMgr::OnPlayerUpdateMovement(Player* p_Player, uint16 p_OpcodeID)
{
    FOREACH_SCRIPT(PlayerScript)->OnUpdateMovement(p_Player, p_OpcodeID);
}
...
{
    FOREACH_SCRIPT_NO_RETN(POIScript)->OnQuestAccept(p_Player, p_Quest);
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnQuestAccept(p_Player, p_Quest);
}
...
{
    FOREACH_SCRIPT_NO_RETN(POIScript)->OnQuestReward(p_Player, p_Quest);
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnQuestReward(p_Player, p_Quest);
}
...
/// Called when a player leaves a mount
/// @p_Player : Player instance
void ScriptMgr::OnPlayerDismount(Player* p_Player)
{
    FOREACH_SCRIPT(PlayerScript)->OnDismount(p_Player);
}
...
/// @p_Player          : Player instance
/// @p_MovieId         : MovieId
void ScriptMgr::OnMovieComplete(Player* p_Player, uint32 p_MovieId)
{
    FOREACH_SCRIPT(PlayerScript)->OnMovieComplete(p_Player, p_MovieId);
}
...
/// @p_Player          : Player instance
/// @p_Origin          : Unit damaging the player
/// @p_DamageEffectTyp : Damage type
/// @p_Damage          : Amount of damage taken
void ScriptMgr::OnPlayerTakeDamage(Player* p_Player, Unit* p_Origin, DamageEffectType p_DamageEffectType, uint32 p_Damage, SpellSchoolMask p_SchoolMask, CleanDamage const* p_CleanDamage)
{
    FOREACH_SCRIPT(PlayerScript)->OnTakeDamage(p_Player, p_Origin, p_DamageEffectType, p_Damage, p_SchoolMask, p_CleanDamage);
}

/// Called when a player deal damage
/// @p_Player          : Player instance
/// @p_Target          : Target Unit instance
/// @p_DamageEffectTyp : Damage type
/// @p_Damage          : Amount of damage taken
void ScriptMgr::OnPlayerDealDamage(Player* p_Player, Unit* p_Target, DamageEffectType p_DamageEffectType, uint32 p_Damage, SpellSchoolMask p_SchoolMask, CleanDamage const* p_CleanDamage)
{
    FOREACH_SCRIPT(PlayerScript)->OnDealDamage(p_Player, p_Target, p_DamageEffectType, p_Damage, p_SchoolMask, p_CleanDamage);
}
...
void ScriptMgr::OnPlayerBracketWin(Player* p_Player, BattlegroundBracketType p_Bracket)
{
    FOREACH_SCRIPT(PlayerScript)->OnBracketWin(p_Player, p_Bracket);
}

void ScriptMgr::OnPlayerModifyCurrency(Player* p_Player, uint32 p_CurrencyID, int32& p_Count, bool p_IgnoreMultipliers)
{
    FOREACH_SCRIPT(PlayerScript)->OnModifyCurrency(p_Player, p_CurrencyID, p_Count, p_IgnoreMultipliers);
}

void ScriptMgr::OnPlayerAfterUnequipItem(Player* p_Player, Item const* p_Item)
{
    FOREACH_SCRIPT(PlayerScript)->OnAfterUnequipItem(p_Player, p_Item);
}

void ScriptMgr::OnAddRelicTalent(Player* p_Player)
{
    FOREACH_SCRIPT(PlayerScript)->OnAddRelicTalent(p_Player);
}

void ScriptMgr::OnActivateArgusTaxi(Player* p_Player, uint32 p_TaxiNodeId)
{
    FOREACH_SCRIPT(PlayerScript)->OnActivateArgusTaxi(p_Player, p_TaxiNodeId);
}

void ScriptMgr::OnGarrisonMissionComplete(Player* p_Player, uint32 p_MissionId, bool p_Succeeded)
{
    FOREACH_SCRIPT(PlayerScript)->OnGarrisonMissionComplete(p_Player, p_MissionId, p_Succeeded);
}

void ScriptMgr::OnAchievementComplete(Player* p_Player, AchievementEntry const* p_Achievement)
{
    FOREACH_SCRIPT(PlayerScript)->OnAchievementComplete(p_Player, p_Achievement);
}

/// Called when some phase update
/// Called only from accept/reward quest functions
/// @p_Player : Player instance
/// @p_Quest  : QuestId
/// @p_Accept : Define accept or reward
void ScriptMgr::OnPhaseUpdate(Player* p_Player, uint32 p_Quest, bool p_Accept)
{
    FOREACH_SCRIPT_NO_RETN(PlayerScript)->OnPhaseUpdate(p_Player, p_Quest, p_Accept);
}
...
/// Should return a fully valid OutdoorPvE object for the map ID.
/// @p_MapId : Map Id
OutdoorPvEScript* ScriptMgr::CreateOutdoorPvE(Map* p_Map)
{
    ASSERT(p_Map);

    if (uint32 l_ScriptId = sOutdoorPvEMgr->GetScriptId(p_Map->GetId()))
    {
        GET_SCRIPT_RET(OutdoorPvEMapScript, l_ScriptId, tmpscript, NULL);
        return tmpscript->GetOutdoorPvEScript(p_Map);
    }
...
void ScriptMgr::OnCreateBaseMap(Map* p_Map, uint32 p_ZoneID)
{
    sOutdoorPvEMgr->CreateOutdoorPvEScript(p_Map);

    FOREACH_SCRIPT(POIScript)->OnCreateBaseMap(p_Map, p_ZoneID);
}

/// Called just after any worldobject is created
/// @p_WorldObject  : WorldObject that have just been created
void ScriptMgr::OnWorldObjectCreate(WorldObject const* p_WorldObject)
{
    FOREACH_SCRIPT_NO_RETN(POIScript)->OnWorldObjectCreate(p_WorldObject);
    FOREACH_SCRIPT_NO_RETN(ContributionScript)->OnWorldObjectCreate(p_WorldObject);
}
...
void ScriptMgr::OnUpdateWorldQuests(Player* p_Player, uint32 p_OldAreaID, uint32 p_NewAreaID)
{
    FOREACH_SCRIPT(POIScript)->OnUpdateWorldQuests(p_Player, p_OldAreaID, p_NewAreaID);
}

/// Called when a POI appears for everyone
/// @p_AreaPOI      : POI that just appeared
void ScriptMgr::OnPOIAppears(AreaPOI const& p_AreaPOI)
{
    FOREACH_SCRIPT(POIScript)->OnPOIAppears(p_AreaPOI);
}

/// Called when a POI disappears for everyone
/// @p_AreaPOI      : POI that just appeared
void ScriptMgr::OnPOIDisappears(AreaPOI const& p_AreaPOI)
{
    FOREACH_SCRIPT(POIScript)->OnPOIDisappears(p_AreaPOI);
}

void ScriptMgr::OnNextPOISelect(uint32 p_GroupId, std::set<uint32>& p_AvailableNextPOIs)
{
    FOREACH_SCRIPT(POIScript)->OnNextPOISelect(p_GroupId, p_AvailableNextPOIs);
}
...
void ScriptMgr::OnContributionChangeState(Contribution const* p_Contribution)
{
    FOREACH_SCRIPT(ContributionScript)->OnChangeState(p_Contribution);
}

/// Called when ContriutionMgr Updates
void ScriptMgr::UpdateContributions(uint32 const p_GameTime)
{
    FOREACH_SCRIPT(ContributionScript)->Update(p_GameTime);
}

/// Called when command .reload quest_template is used
void ScriptMgr::OnReloadQuestTemplates()
{
    FOREACH_SCRIPT(ContributionScript)->OnReloadQuestTemplates();
}

/// Called when a player contributes
/// @p_Player           : Contributing Player
/// @p_p_Contribution   : Concerned contribution
void ScriptMgr::OnPlayerContribute(Player* p_Player, Contribution const* p_Contribution)
{
    FOREACH_SCRIPT(ContributionScript)->OnPlayerContribute(p_Player, p_Contribution);
}
...
/// @p_Player      : Player instance
bool ScriptMgr::EvalPlayerConditionScript(uint32 p_ConditionID, PlayerConditionEntry const* p_Condition, Unit const* p_Unit)
{
    ...
    return l_Script->OnConditionCheck(p_ConditionID, p_Condition, p_Unit);
}
...
/// Constructor
/// @p_Name : Script p_Name
OutdoorPvEMapScript::OutdoorPvEMapScript(const char* p_Name, uint32 p_MapID)
    : ScriptObjectImpl(p_Name), MapScript<Map>(p_MapID)
{
    if (GetEntry() && !GetEntry()->IsWorldMap())
        sLog->outError(LOG_FILTER_TSCR, "WorldMapScript for map %u is invalid.", p_MapID);

    ScriptRegistry<OutdoorPvEMapScript>::AddScript(this);
}
...
/// Constructor
/// @p_Name : Script p_Name
SceneTriggerScript::SceneTriggerScript(const char* p_Name)
    : ScriptObjectImpl(p_Name)
{
    ScriptRegistry<SceneTriggerScript>::AddScript(this);
}

EventObjectScript::EventObjectScript(const char* p_Name)
    : ScriptObject(p_Name)
{
...
/// Constructor
/// @p_Name : Script p_Name
POIScript::POIScript(const char* p_Name)
    : ScriptObjectImpl(p_Name)
{
    ScriptRegistry<POIScript>::AddScript(this);
}

/// Constructor
/// @p_Name : Script p_Name
ContributionScript::ContributionScript(const char* p_Name)
    : ScriptObjectImpl(p_Name)
{
    ScriptRegistry<ContributionScript>::AddScript(this);
}
...
template class ScriptRegistry<OutdoorPvEMapScript>;
