...
void BattlePet::Load(Field* p_Fields)
{
    ...
    if (sBattlePetAbilitiesBySpecies.find(Species) != sBattlePetAbilitiesBySpecies.end())
    {
        for (BattlePetSpeciesXAbilityEntry const* l_SpeciesXAbilityInfo : sBattlePetAbilitiesBySpecies[Species])
        {
...
void BattlePet::AddToPlayer(Player* p_Player, SQLTransaction& p_Transaction)
{
    ...
    p_Player->UpdateCriteria(CRITERIA_TYPE_OWN_BATTLE_PET_COUNT, 1);

    if (BattlePetSpeciesEntry const* l_SpeciesInfo = sBattlePetSpeciesStore.LookupEntry(Species))
    {
        p_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_OWN_PET, l_SpeciesInfo->CreatureID);
        p_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_OWN_BATTLE_PET, l_SpeciesInfo->ID);
    }
}
...
void BattlePet::AddToPlayer(Player* p_Player)
{
    ...
    LoginDatabase.CommitTransaction(l_Transaction, p_Player->GetQueryCallbackMgr(), l_CallBack);

    p_Player->UpdateCriteria(CRITERIA_TYPE_OWN_BATTLE_PET_COUNT, 1);

    if (BattlePetSpeciesEntry const* l_SpeciesInfo = sBattlePetSpeciesStore.LookupEntry(Species))
    {
        p_Player->UpdateCriteria(CRITERIA_TYPE_OWN_PET, l_SpeciesInfo->ID);
        p_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_OWN_BATTLE_PET, l_SpeciesInfo->ID);
    }
}
...
{
    BattlePetXPEntry const* l_Entry = g_BattlePetXPStore.LookupEntry(Level);

    if (!l_Entry)
        return 0;

    return l_Entry->xp * l_Entry->wins;
}
/// Get xp earn
...
void PetBattle::Begin()
{
    ...
    for (size_t l_CurrentTeamID = 0; l_CurrentTeamID < MAX_PETBATTLE_TEAM; ++l_CurrentTeamID)
    {
        ...
        if (l_Player)
            l_Player->AddCriticalOperation([l_Player]()->void { l_Player->UnsummonCurrentBattlePetIfAny(true); });
    }
...
void PetBattle::Finish(uint32 p_WinnerTeamID, bool p_Aborted)
{
    ...
    for (size_t l_CurrentTeamID = 0; l_CurrentTeamID < MAX_PETBATTLE_TEAM; ++l_CurrentTeamID)
    {
        if (Teams[l_CurrentTeamID]->PlayerGuid)
        {
            ...
            for (size_t l_CurrentPetID = 0; l_CurrentPetID < Teams[l_CurrentTeamID]->TeamPetCount; ++l_CurrentPetID)
            {
                ...
                if (p_WinnerTeamID == l_CurrentTeamID && BattleType == PETBATTLE_TYPE_PVE && l_PetsCanGetXP.find(l_CurrentPet->ID) != l_PetsCanGetXP.end())
                {
                    ...
                    if ((l_CurrentPet->XP + l_XpEarn) >= l_XpToNextLevel)
                    {
                        ...
                        if (l_SpeciesInfo)
                            l_Player->UpdateCriteria(CRITERIA_TYPE_LEVEL_BATTLE_PET, l_CurrentPet->Level, l_SpeciesInfo->PetType);

                        if (l_CurrentPet->Level >= 3)
                        {
                            /// Quest progress for 12 x Level Up!
                            l_Player->AddCriticalOperation([l_Player]()->void { l_Player->KilledMonsterCredit(65876); });
                        }
                    }
                    else
            ...
            if (p_WinnerTeamID == l_CurrentTeamID)
                l_Player->UpdateCriteria(CRITERIA_TYPE_WIN_PET_BATTLE, 1, 0, 0, l_Player);

            if (BattleType == PETBATTLE_TYPE_PVE && PveBattleType == PVE_PETBATTLE_WILD)
            {
                /// Quest progress for 12 x Learning the Ropes
                l_Player->AddCriticalOperation([l_Player]()->void { l_Player->KilledMonsterCredit(65355); });
            }
            ...
            if (BattleType == PETBATTLE_TYPE_PVE && l_CurrentTeamID == p_WinnerTeamID)
            {
                ...
                for (size_t l_CurrentPetID = 0; l_CurrentPetID < Teams[l_OtherTeamID]->TeamPetCount; ++l_CurrentPetID)
                {
                    ...
                    if (l_SpeciesInfo && l_SpeciesInfo->CreatureID == 111158) ///< Blind Rat, specific Dalaran Sewers case.
                        l_Player->AddCriticalOperation([l_Player]()->void { l_Player->ModifyCurrency(1149, 75, false); });
                }
            }
            ...
            if (BattleType == PETBATTLE_TYPE_PVE && l_CurrentTeamID == p_WinnerTeamID && Teams[l_CurrentTeamID]->CapturedPet != PETBATTLE_NULL_ID)
            {
                ...
                {
                    if (AchievementEntry const* l_Achievement = sAchievementStore.LookupEntry(6571))
                        l_Player->AddCriticalOperation([l_Player, l_Achievement]()->void { l_Player->CompletedAchievement(l_Achievement); });
                }
                ...
                if (l_SpeciesInfo)
                {
                    l_Player->UpdateCriteria(CRITERIA_TYPE_CAPTURE_BATTLE_PET, l_SpeciesInfo->CreatureID);
                    l_Player->UpdateCriteria(CRITERIA_TYPE_OWN_BATTLE_PET, l_SpeciesInfo->ID);
                    l_Player->UpdateCriteria(CRITERIA_TYPE_CAPTURE_BATTLE_PET, 1, Pets[Teams[l_CurrentTeamID]->CapturedPet]->Quality);
                }

                /// Quest progress for 12 x Got One!
                l_Player->AddCriticalOperation([l_Player]()->void { l_Player->KilledMonsterCredit(65356); });
            }
            ...
            if (BattleType == PETBATTLE_TYPE_PVE && PveBattleType == PVE_PETBATTLE_TRAINER && l_CurrentTeamID == p_WinnerTeamID)
            {
                ...
                if (l_Trainer)
                {
                    ...
                    l_Player->AddCriticalOperation([l_Player, l_TrainerGuid, l_TrainerEntry]()->void { l_Player->QuestObjectiveSatisfy(l_TrainerEntry, 1, QUEST_OBJECTIVE_TYPE_PET_TRAINER_DEFEAT, l_TrainerGuid); });
                }
            }
        }
    }
    ...
    for (size_t l_CurrentTeamID = 0; l_CurrentTeamID < MAX_PETBATTLE_TEAM; ++l_CurrentTeamID)
    {
        if (Teams[l_CurrentTeamID]->PlayerGuid)
        {
            ...
            auto l_BattlePetType = BattleType;
            l_Player->AddCriticalOperation([l_Player, l_BattlePetType]()->void
            {
                ...
                if (l_BattlePetType == PETBATTLE_TYPE_PVP_MATCHMAKING)
        ...
        else
        {
            if (BattleType == PETBATTLE_TYPE_PVE && PveBattleType == PVE_PETBATTLE_WILD)
            {
                for (size_t l_CurrentPetID = 0; l_CurrentPetID < Teams[l_CurrentTeamID]->TeamPetCount; ++l_CurrentPetID)
                {
                    ...
                    l_WildPet->GetMap()->AddTask([l_WildPet, p_WinnerTeamID]()->void
                    {
                ...
                case LFBState::LFB_STATE_PROPOSAL:
                {
                    ...
                    if (l_Left->ProposalAnswer == LFBAnswer::LFB_ANSWER_AGREE && l_Right->ProposalAnswer == LFBAnswer::LFB_ANSWER_AGREE)
                    {
                        if (l_LeftPlayer && l_RightPlayer)
                        {
                            ...
                            float l_LeftPlayerOrientation = l_RightPlayer->GetOrientation() - M_PI;
                            l_LeftPlayer->AddCriticalOperation([l_LeftPlayer, l_Location, l_LeftPlayerOrientation]()->void
                            {
                                ...
                                l_LeftPlayer->TeleportTo(l_Location.MapID, l_Location.Positions[PETBATTLE_TEAM_1].x + 0.01f, l_Location.Positions[PETBATTLE_TEAM_1].y + 0.01f, l_Location.Positions[PETBATTLE_TEAM_1].z + 0.01f, l_LeftPlayerOrientation);
                            });

                            float l_RightPlayerOrientation = l_LeftPlayer->GetOrientation() - M_PI;
                            l_RightPlayer->AddCriticalOperation([l_RightPlayer, l_Location, l_RightPlayerOrientation]()->void
                            {
                                ...
                                l_RightPlayer->TeleportTo(l_Location.MapID, l_Location.Positions[PETBATTLE_TEAM_2].x + 0.01f, l_Location.Positions[PETBATTLE_TEAM_2].y + 0.01f, l_Location.Positions[PETBATTLE_TEAM_2].z + 0.01f, l_RightPlayerOrientation);
                            });
