...
class ChatHandler
{
    public:
        ...
        virtual void SendSysMessage(const char *str, bool p_Inline = false);
        ...
        virtual LocaleConstant GetSessionDb2Locale() const;
        virtual int GetSessionDbLocaleIndex() const;

        bool HasLowerSecurity(Player* target, uint64 guid, bool strong = false);
        bool HasLowerSecurityAccount(WorldSession* target, uint32 account, bool strong = false);

        void SendGlobalGMSysMessage(const char *str, AccountTypes p_Rank = AccountTypes::SEC_GAMEMASTER);
        void SendGlobalFormatedGMSysMessage(const char* p_Format, ...);
        ...
        uint32    GetItemIDAndBonusesFromLink(char* p_Text, std::vector<uint32>& p_Bonuses);
        ...
        bool GetPlayerGroupAndGUIDByName(const char* cname, Player* &player, GroupPtr &group, uint64 &guid, bool offline = false);
        std::string extractPlayerNameFromLink(char* text);
        // select by arg (name/link) or in-game selection online/offline player
        bool extractPlayerTarget(char* args, Player** player, uint64* player_guid = 0, std::string* player_name = NULL);
    ...
    protected:
        ...
        static bool SetDataForCommandInTable(ChatCommand* table, const char* text, uint32 security, uint32 p_AnimatorSecurity, std::string const& help, std::string const& fullcommand);
...
class CliHandler : public ChatHandler
{
    public:
        ...
        void SendSysMessage(const char *str, bool m_Inline = false);
        std::string GetNameLink() const;
        bool needReportToTarget(Player* chr) const;
        LocaleConstant GetSessionDb2Locale() const;
