...
enum ChannelFlags
{
    ...
    CHANNEL_FLAG_100        = 0x100,
    CHANNEL_FLAG_200        = 0x200,
    CHANNEL_FLAG_400        = 0x400,
    CHANNEL_FLAG_1000       = 0x1000
...
class Channel
{
    struct PlayerInfo
    {
        ...
        uint8 flags;
        uint32 LocaleFilter;
        uint32 m_LastMessageTime;

        PlayerInfo()
            : LocaleFilter(0xFFFFFFFF), m_LastMessageTime(0)
        {
        ...
        void UpdateLastMessageTime() { m_LastMessageTime = getMSTime(); }
        bool CheckSlowMode(uint32 p_Interval) const { return (GetMSTimeDiffToNow(m_LastMessageTime) / TimeConstants::IN_MILLISECONDS) >= p_Interval; }

    };


    typedef std::unordered_map<uint64, PlayerInfo> PlayerList;
    PlayerList  m_Players;
    mutable ACE_Recursive_Thread_Mutex m_PlayersLock;
    ...
    struct ChatHistory
    {
        ...
        bool                Muted;
        time_t              MuteStart;
        time_t              LastWarningTime;
    };

    const std::array<uint32, 6> m_MuteTimes =
    {
        {
            0,
            0,
    ...
    std::unordered_map<uint64, ChatHistory> m_PlayersHistory;
    ...
    private:
        ...
        {
            ACE_Guard<ACE_Recursive_Thread_Mutex> l_Guard(m_PlayersLock);
            return banned.find(guid) != banned.end();
        }

        bool IsWorld() const;
        bool IsWorldLocal(LocaleConstant) const;
    ...
    public:
        ...
        uint32 GetSlowModeInterval() const { return m_SlowModeInterval; }
        bool IsLockedDown() const { return m_LockDown; }
        ...
        void SetSlowMode(uint32 p_Interval) { m_SlowModeInterval = p_Interval; }
        void SetLockedDown(bool p_Locked) { m_LockDown = p_Locked; }
        bool CheckPlayerSlowMode(uint64 p_Guid)
        {
            m_Players[p_Guid].UpdateLastMessageTime();
            return !m_SlowModeInterval || m_Players[p_Guid].CheckSlowMode(m_SlowModeInterval);
        }
};
#endif
