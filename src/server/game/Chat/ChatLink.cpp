...
// |color|Hglyph:glyph_slot_id:glyph_prop_id|h[%s]|h|r                    - client, at shift click in glyphs dialog, GlyphSlot.db2, GlyphProperties.db2
...
bool ItemChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    const char* suffixStrings = _suffix ? _suffix->Name->Get(LOCALE_enUS) : (_property ? _property->nameSuffix->Get(LOCALE_enUS) : NULL);

    bool res = (FormatName(LOCALE_enUS, (char*)suffixStrings) == buffer);
    if (!res)
    {
        for (uint8 index = LOCALE_koKR; index < TOTAL_LOCALES; ++index)
        {
            if (FormatName(index, (char*)suffixStrings) == buffer)
            {
                res = true;
                break;
            }
...
bool QuestChatLink::Initialize(std::istringstream& iss)
{
    ...
    // Validate quest
    m_Quest = sObjectMgr->GetQuestTemplate(questId);
    if (!m_Quest)
    {
    ...
    // Read min quest level
    if (!ReadInt32(iss, m_MinLevel))
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading min quest level", iss.str().c_str());
        return false;
    }
    ...
    // Read max scaling quest level
    if (!ReadInt32(iss, m_MaxScalingLevel))
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): sequence finished unexpectedly while reading max scaling quest level", iss.str().c_str());
        return false;
    }
...
bool QuestChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    bool res = (m_Quest->GetTitle() == buffer);
    if (!res)
        if (QuestLocale const* ql = sObjectMgr->GetQuestLocale(m_Quest->GetQuestId()))
    ...
    if (!res)
        sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): linked quest (id: %u) title wasn't found in any localization", context, m_Quest->GetQuestId());
    return res;
}
...
bool SpellChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (_spell->Attributes & SPELL_ATTR0_TRADESPELL)
    {
        ...
        if (!skillLine)
        {
            sLog->outDebug(LOG_FILTER_CHATSYS, "ChatHandler::isValidChatMessage('%s'): skill line not found for skill %u", context, skillInfo->SkillLine);
            return false;
        }

        uint32 skillLineNameLength = strlen(skillLine->DisplayName->Get(sWorld->GetDefaultDb2Locale()));
        if (skillLineNameLength > 0 && strncmp(skillLine->DisplayName->Get(sWorld->GetDefaultDb2Locale()), buffer, skillLineNameLength) == 0)
        {
    ...
    if (*_spell->SpellName->Get(sWorld->GetDefaultDb2Locale()) && strcmp(_spell->SpellName->Get(sWorld->GetDefaultDb2Locale()), buffer) == 0)
        return true;
...
bool AchievementChatLink::ValidateName(char* buffer, const char* context)
{
    ...
    if (strcmp(_achievement->TitleLang->Get(sWorld->GetDefaultDb2Locale()), buffer) == 0)
        return true;
...
bool LinkExtractor::IsValidMessage()
{
    ...
    while (!_iss.eof())
    {
        ...
        char commandChar;
        _iss.get(commandChar); ///< 03/11/2019 - Check send packet with fake byte. Example (hex: "7c 20 7c 63 6f 6c 6f 72", text: "| |color")
        ...
        switch (commandChar)
        {
            ...
            case 'H':
                ...
                /// Only Quest link is updated and need to be checked to prevent wow errors
                if (strcmp(buffer, "quest") == 0)
                {
                    l_ValidateLink = true;
                ...
                link->SetColor(color);
                if (l_ValidateLink && !link->Initialize(_iss))
                    return false;
                break;
            case 'h':
                ...
                if (*validSequenceIterator == 'h')
                {
                    ...
                    /// Disable title checking, we don't load all the locales
                    //if (l_ValidateLink && !link->ValidateName(buffer, _iss.str().c_str()))
                        //return false;
                }
                break;
            case 'r':
...
constexpr std::array<const char*, 8> l_Links =
{
    {
        "Hquest",
        "Hitem",
...
bool CheckDoubleLink(const char* p_Message)
{
    if (!strchr(p_Message, '|'))
        return true;

    uint32 l_Count = 0;
    for (auto l_Link : l_Links)
    {
        if (strstr(p_Message, l_Link)) ++l_Count;
        if (l_Count > 1) return false;
    }
