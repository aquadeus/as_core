    ...
    if (LoadCommandTable())
    {
        ...
        if (result)
        {
            do
            {
                ...
                SetDataForCommandInTable(commandTableCache, name.c_str(), fields[1].GetUInt8(), fields[2].GetUInt8(), fields[3].GetString(), name);
            }
...
bool ChatHandler::isAvailable(ChatCommand const& cmd) const
{
    ...
    return m_session->GetSecurity() >= (m_session->IsAnimator() ? AccountTypes(cmd.AnimatorSecurity) : AccountTypes(cmd.SecurityLevel));
}
...
bool ChatHandler::HasLowerSecurityAccount(WorldSession* target, uint32 target_account, bool strong)
{
    ...
    if (target)
        ...
        target_sec = sWorld->GetAccountSecurity(target_account);
    else
...
void ChatHandler::SendSysMessage(const char *str, bool p_Inline)
{
    WorldPacket data;
...
void ChatHandler::SendGlobalGMSysMessage(const char *str, AccountTypes p_Rank /*= SEC_GAMEMASTER*/)
{
    // Chat output
    WorldPacket data;
    ...
    {
        FillSystemMessageData(&data, line);
        sWorld->SendGlobalGMMessage(&data, nullptr, 0, p_Rank);
     }
    free(buf);
}

void ChatHandler::SendGlobalFormatedGMSysMessage(const char* p_Format, ...)
{
    va_list ap;
    char str[2048];
    va_start(ap, p_Format);
    vsnprintf(str, 2048, p_Format, ap);
    va_end(ap);
    SendGlobalGMSysMessage(&str[0]);
}
...
bool ChatHandler::SetDataForCommandInTable(ChatCommand* table, const char* text, uint32 security, uint32 p_AnimatorSecurity, std::string const& help, std::string const& fullcommand)
{
    std::string cmd = "";
    ...
    for (uint32 i = 0; table[i].Name != NULL; i++)
    {
        ...
        if (table[i].ChildCommands != NULL)
        {
            if (SetDataForCommandInTable(table[i].ChildCommands, text, security, p_AnimatorSecurity, help, fullcommand))
                return true;
            else if (*text)
                return false;
        ...
        if (table[i].AnimatorSecurity != p_AnimatorSecurity)
            sLog->outInfo(LOG_FILTER_GENERAL, "Table `command` overwrite for command '%s' default Animator Security (%u) by %u", fullcommand.c_str(), table[i].AnimatorSecurity, p_AnimatorSecurity);

        table[i].SecurityLevel = security;
        table[i].AnimatorSecurity = p_AnimatorSecurity;
...
*/
    ...
    if (!CheckDoubleLink(message))
    {
        sLog->outDebug(LOG_FILTER_CHATSYS, "CheckDoubleLink('%s'): double link has been found", message);
        return false;
    }
...
void ChatHandler::FillMessageData(WorldPacket* data, WorldSession* session, uint8 type, uint32 language, const char *channelName, uint64 target_guid, const char *message, Unit* speaker, const char* addonPrefix /*= NULL*/, uint32 achievementId /*= 0*/)
{
    ...
    auto l_Group = speakerPlayer ? speakerPlayer->GetGroup() : nullptr;
    if (l_Group)
        groupGuid = l_Group->GetGUID();
    ...
    *data << uint8(language);
    ...
    data->appendPackGUID(speakerPlayer ? speakerPlayer->GetSession()->GetBNetAccountGUID() : 0);
    data->appendPackGUID(target_guid);
...
uint32 ChatHandler::GetItemIDAndBonusesFromLink(char* p_Text, std::vector<uint32>& p_Bonuses)
{
    /// Skip empty
    if (!p_Text)
        return 0;
    ...
    /// Extract data from the item link
    std::string l_ValuesString = strtok(nullptr, "|");
    std::vector<std::string> l_ItemLinkValues;

    std::string l_Value = "";
    for (int i = 0; i < l_ValuesString.size(); i++)
    {
        if (l_ValuesString[i] == ':')
        {
            l_ItemLinkValues.push_back(l_Value);
            l_Value = "";
        }
        else
            l_Value += l_ValuesString[i];
    }

    /// Get item entry from extracted values
    uint32 l_ItemEntry = 0;
    if (!l_ItemLinkValues[ItemLinkIndex::ItemEntry].size())
        return 0;

    /// std::stoi can throw exception if the string we provide aren't number
    /// data of the item link come from the client, so we can't be sure of that
    try
    {
        l_ItemEntry = std::stoi(l_ItemLinkValues[ItemLinkIndex::ItemEntry]);

        /// Get bonus ids from extracted values
        if (l_ItemLinkValues[ItemLinkIndex::NumBonusIDs].size())
        {
            uint32 l_BonusCount = std::stoi(l_ItemLinkValues[ItemLinkIndex::NumBonusIDs]);
            uint32 l_End = ItemLinkIndex::BonusData + l_BonusCount;

            /// Invalid bonus count
            if (l_End >= l_ItemLinkValues.size())
                return 0;

            for (uint32 l_Idx = ItemLinkIndex::BonusData; l_Idx < l_End; l_Idx++)
            {
                if (!l_ItemLinkValues[l_Idx].size())    ///< Empty bonus
                    continue;

                uint32 l_BonusID = std::stoi(l_ItemLinkValues[l_Idx]);
                if (GetItemBonusesByID(l_BonusID))
                    p_Bonuses.push_back(l_BonusID);
            }
        }
    }
    catch (...)
...
LocaleConstant ChatHandler::GetSessionDb2Locale() const
{
    return m_session->GetSessionDb2Locale();
}
...
void CliHandler::SendSysMessage(const char *str, bool /*p_Inline*/)
{
...
bool ChatHandler::GetPlayerGroupAndGUIDByName(const char* cname, Player* &player, GroupPtr &group, uint64 &guid, bool offline)
{
    player  = NULL;
    guid = 0;
...
LocaleConstant CliHandler::GetSessionDb2Locale() const
{
    return sWorld->GetDefaultDb2Locale();
}
