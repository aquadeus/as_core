...
m_dungeonDifficulty(DifficultyNormal), m_raidDifficulty(DifficultyRaidNormal), m_LegacyRaidDifficuty(Difficulty10N),
    m_bgGroup(NULL), m_bfGroup(NULL), m_lootMethod(LootMethod::PERSONAL_LOOT), m_lootThreshold(ITEM_QUALITY_UNCOMMON), m_looterGuid(0),
    m_subGroupsCounts(NULL), m_guid(0), m_UpdateCount(0), m_maxEnchantingLevel(0), m_dbStoreId(0), m_readyCheckCount(0),
    m_membersInInstance(0), m_readyCheck(false), m_Team(0), m_IsDisbanding(false)
{
    ...
    m_RaidMarkers.resize(eRaidMarkersMisc::MaxRaidMarkers, std::make_shared<RaidMarker>());
}
...
Group::~Group()
{
    ...
    {
        m_boundInstances[i].safe_foreach(true, [&](BoundInstancesMap::iterator* p_Itr)
        {
            (*p_Itr)->second->save->RemoveGroup(shared_from_this());
        });
    }
...
bool Group::Create(Player* leader)
{
    ...
    m_lootMethod = LootMethod::PERSONAL_LOOT;
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        m_raidDifficulty        = isLFGGroup() ? (l_NewLFR ? Difficulty::DifficultyRaidLFR : Difficulty::DifficultyRaidTool) : leader->GetRaidDifficultyID();
        m_LegacyRaidDifficuty   = leader->GetLegacyRaidDifficultyID();
...
uint16 Group::GetPartyFlags() const
{
    return m_PartyFlags;
}
...
{
    MemberSlotPtr member = std::make_shared<MemberSlot>(MemberSlot());
...
{
    m_PartyFlags = PartyFlags(m_PartyFlags | PARTY_FLAG_LFG | PARTY_FLAG_UNK_4);

    sLFGMgr->SetLfgGroup(GUID_LOPART(GetGUID()), true);
...
void Group::ConvertToRaid()
{
    ...
    m_memberSlots.foreach([](Group::MemberSlotPtr l_Member)
    {
        if (Player* player = ObjectAccessor::FindPlayer(l_Member->guid))
...
void Group::LinkPlayer(Player* p_Player)
{
    m_memberSlots.foreach_until([p_Player](Group::MemberSlotPtr l_Member) -> bool
    {
        if (l_Member->guid == p_Player->GetGUID())
        {
            l_Member->player = p_Player;
            return true;
        }
...
void Group::UnlinkPlayer(Player* p_Player)
{
    uint32 l_Count = 0;
    m_memberSlots.foreach_until([p_Player, &l_Count](Group::MemberSlotPtr l_Member) -> bool
    {
        if (l_Member->guid == p_Player->GetGUID())
        {
            l_Member->player = nullptr;
            l_Count++;
            return false;
        }
        else if (l_Member->player == p_Player)
        {
            sLog->outExtChat("#jarvis", "", false, "Group::UnlinkPlayer - guid doesn't match but pointer do [" UI64FMTD ", " UI64FMTD "]", p_Player->GetGUID(), l_Member->guid);
            l_Member->player = nullptr;
            l_Count++;
            return false;
        }
    ...
    if (l_Count > 1)
        sLog->outExtChat("#jarvis", "", false, "Group::UnlinkPlayer - Player[" UI64FMTD "] is referenced %u time in the  m_memberSlots", p_Player->GetGUID(), l_Count);
}

bool Group::AddMember(Player* p_Player)
{
    std::lock_guard<std::recursive_mutex> l_Lock(m_DisbandingLock);

    if (m_IsDisbanding)
        return false;

    if (IsMember(p_Player->GetGUID()))
        return false;
    ...
    MemberSlotPtr l_Member = std::make_shared<MemberSlot>(MemberSlot());
    l_Member->player       = p_Player;
    ...
    l_Member->specID       = p_Player->GetActiveSpecializationID();
    ...
    if (p_Player->GetGroup() && (isBGGroup() || isBFGroup())) // if player is in group and he is being added to BG raid group, then call SetBattlegroundRaid()
        p_Player->SetBattlegroundOrBattlefieldRaid(shared_from_this(), l_SubGroup);
    else if (p_Player->GetGroup()) //if player is in bg raid and we are adding him to normal group, then call SetOriginalGroup()
        p_Player->SetOriginalGroup(shared_from_this(), l_SubGroup);
    else //if player is not in group, then call set group
        p_Player->SetGroup(shared_from_this(), l_SubGroup);

    // if the same group invites the player back, cancel the homebind timer
    InstanceGroupBindPtr l_Bind = GetBoundInstance(p_Player);
    if (l_Bind && l_Bind->save->GetInstanceId() == p_Player->GetInstanceId())
        p_Player->m_InstanceValid = true;

    if (sLFGListMgr->IsGroupQueued(shared_from_this()))
        sLFGListMgr->PlayerAddedToGroup(p_Player, shared_from_this());
    ...
    SendUpdate();
    sScriptMgr->OnGroupAddMember(shared_from_this(), p_Player->GetGUID());
    ...
    if (p_Player->IsInWorld() && p_Player->GetMap() != nullptr)
    {
        ...
        m_memberSlots.foreach([&](Group::MemberSlotPtr l_MemberItr) -> void
        {
            Player* l_Member = l_MemberItr->player;
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_MemberItr)
    {
        if (l_MemberItr->player == p_Player)
            return;

        if (Player* l_Member = l_MemberItr->player)
        {
    ...
    if (p_Player->GetMap() && p_Player->GetMap()->GetEntry()->instanceType == InstanceTypes::MAP_COMMON)
    {
        p_Player->AddMapTask(eTaskType::TaskSwitchToPhasedMap, [p_Player]() -> void
        {
            p_Player->SwitchToPhasedMap(p_Player->GetMapId());
        });
    }
...
bool Group::AddCreatureMember(Creature* p_Creature)
{
    /// Get first not-full group
    uint8 l_SubGroup = 0;
    if (m_subGroupsCounts)
    ...
    MemberSlotPtr l_Member = std::make_shared<MemberSlot>(MemberSlot());
    l_Member->player       = nullptr;
    l_Member->guid         = p_Creature->GetGUID();
    l_Member->name         = p_Creature->GetName();
    l_Member->group        = l_SubGroup;
    l_Member->flags        = 0;
    l_Member->roles        = 0;
    l_Member->playerClass  = p_Creature->getClass();
    l_Member->specID       = 0;
    ...
    /// Broadcast group members' fields to player
    m_memberSlots.foreach([p_Creature](Group::MemberSlotPtr l_MemberItr)
    {
        if (Player* l_Member = l_MemberItr->player)
        {
            if (l_Member->HaveAtClient(p_Creature))
            {
                UpdateData l_NewData(p_Creature->GetMapId());

                WorldPacket l_NewDataPacket;

                p_Creature->BuildValuesUpdateBlockForPlayer(&l_NewData, l_Member);
...
bool Group::RemoveMember(uint64 p_Guid, RemoveMethod const& p_Method /*= GROUP_REMOVEMETHOD_DEFAULT*/, uint64 p_Kicker /*= 0*/, char const* p_Reason /*= NULL*/)
{
    ...
    if (m_IsDisbanding)
    {
        ///sLog->outExtChat("#jarvis", "", false, "Trying to remove member in disbanding group");
        return false;
    }
    ...
    if (Player* l_Player = sObjectAccessor->FindPlayer(p_Guid))
        if (sLFGListMgr->IsGroupQueued(shared_from_this()))
            sLFGListMgr->PlayerRemoveFromGroup(l_Player, shared_from_this());

    sScriptMgr->OnGroupRemoveMember(shared_from_this(), p_Guid, p_Method, p_Kicker, p_Reason);

    /// LFG group vote kick handled in scripts
    if (isLFGGroup() && p_Method == GROUP_REMOVEMETHOD_KICK)
        return m_memberSlots.size();

    auto slot = _getMemberWSlot(p_Guid);

    /// Remove member and change leader (if need) only if strong more 2 members _before_ member remove (BG/BF allow 1 member group)
    if (sLFGListMgr->IsGroupQueued(shared_from_this()) || GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBFGroup()) ? 1u : 2u))
    {
        Player* l_Player = ObjectAccessor::GetObjectInOrOutOfWorld(p_Guid, (Player*)NULL);
        if (!l_Player && slot)
            l_Player = (slot)->player;
        ...
        if (l_Player)
        {
            ...
            {
                if (l_Player->GetOriginalGroup() == shared_from_this())
                {
                    l_Player->SetOriginalGroup(NULL);

                    if (l_Player->GetGroup() == shared_from_this())
                    {
                        sLog->outAshran("GetGroup() == GetOriginalGroup for player guid %u", l_Player->GetGUIDLow());
            ...
            uint64 l_PartyGUID = GetGUID();
            uint64 l_LeaderGUID = GetLeaderGUID();

            uint32 l_PlayerListCount = 0;
        ...
        {
            m_memberSlots.foreach_until([&](Group::MemberSlotPtr l_Member) -> bool
            {
                if (ObjectAccessor::FindPlayer(l_Member->guid))
                {
                    ChangeLeader(l_Member->guid);
                    return true;
                }
                return false;
            });
        ...
        if ((!sLFGListMgr->IsGroupQueued(shared_from_this()) || !l_OnlineMember) && l_OnlineMember < ((isLFGGroup() || isBGGroup()) ? 1u : 2u))
            Disband();
...
bool Group::RemoveCreatureMember(Creature* p_Creature)
{
    /// Update subgroups
    auto l_Slot = _getMemberWSlot(p_Creature->GetGUID());
    if (l_Slot)
    {
        SubGroupCounterDecrease(l_Slot->group);
        m_memberSlots.erase(l_Slot);
    }
...
{
    auto slot = _getMemberWSlot(newLeaderGuid);
    if (!slot)
        return;
    ...
    sScriptMgr->OnGroupChangeLeader(shared_from_this(), m_leaderGuid, newLeaderGuid);
    ...
    if (!isBGGroup() && !isBFGroup())
    {
        ...
        for (uint8 i = 0; i < Difficulty::MaxDifficulties; ++i)
        {
            m_boundInstances[i].safe_foreach(false, [&](BoundInstancesMap::iterator* p_Itr)
            {
                ...
                if ((*p_Itr)->second->perm && !sMapMgr->FindMap((*p_Itr)->first, (*p_Itr)->second->save->GetInstanceId()))
                {
#ifndef CROSS
                    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_GROUP_INSTANCE_PERM_BINDING);
                    stmt->setUInt32(0, m_dbStoreId);
                    stmt->setUInt32(1, (*p_Itr)->second->save->GetInstanceId());
                    trans->Append(stmt);
#endif
                    (*p_Itr)->second->save->RemoveGroup(shared_from_this());
                    m_boundInstances[i].getSource().erase((*p_Itr)++);
                }
                else
                    ++(*p_Itr);
             });
        ...
        Player::ConvertInstancesToGroup(newLeader, shared_from_this(), true);
#ifndef CROSS
...
void Group::Disband(bool hideDestroy /* = false */)
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        player = HashMapHolder<Player>::Find(l_Member->guid);
        if (!player)
            player = l_Member->player;
        ...
        if (!player->GetSession() || player->GetMapId() == MAPID_INVALID)
            return;
        ...
        else
        {
            ...
            l_Data.WriteBit(false); ///< HasLfgInfo
            l_Data.WriteBit(false); ///< HasLootSettings
            l_Data.WriteBit(false); ///< HasDifficultySettings
            l_Data.FlushBits();
    ...
    if (isLFGGroup())
        sLFGMgr->SetLfgGroup(GetGUID(), false);
}
...
void Group::SendLootStartRoll(uint32 p_CountDown, uint32 p_MapID, RollPtr p_Roll)
{
    for (Roll::PlayerVote::const_iterator l_Iter = p_Roll->playerVote.begin(); l_Iter != p_Roll->playerVote.end(); ++l_Iter)
    {
        Player* l_Player = ObjectAccessor::FindPlayer(l_Iter->first);
        if (!l_Player || !l_Player->GetSession())
            continue;

        uint8 l_RollVoteMask = p_Roll->rollVoteMask;

        if (ItemDisenchantLootEntry const* disenchant = p_Roll->GetItemDisenchantLoot(l_Player))
            if (m_maxEnchantingLevel >= disenchant->RequiredDisenchantSkill)
                l_RollVoteMask |= ROLL_FLAG_TYPE_DISENCHANT;

        WorldPacket l_Data(Opcodes::SMSG_LOOT_START_ROLL, 200);
        ...
        l_Data << uint8(l_RollVoteMask);                        ///< Roll type mask
        l_Data << uint8(GROUP_LOOT);                            ///< Maybe the number of players rolling for it???

        l_Data.WriteBits(LOOT_ITEM_TYPE_ITEM, 2);               ///< Type
        l_Data.WriteBits(LOOT_SLOT_TYPE_ROLL_ONGOING, 3);       ///< Ui Type
        ...
        if (l_Iter->second == NOT_EMITED_YET)
            l_Player->GetSession()->SendPacket(&l_Data);
    }
}

void Group::SendLootStartRollToPlayer(uint32 p_CountDown, uint32 p_MapID, Player* p_Player, bool /*p_CanNeed*/, RollPtr p_Roll) ///< p_CanNeed is unused
{
...
void Group::SendLootRoll(uint64 /*p_TargetGUID*/, uint64 targetGuid, uint8 p_RollNumber, uint8 rollType, RollPtr p_Roll) ///< p_TargetGUID is unused
{
...
void Group::SendLootRollWon(uint64 /*p_SourceGUID*/, uint64 p_TargetGUID, uint8 p_RollNumber, uint8 rollType, RollPtr p_Roll)
{
...
void Group::SendLootAllPassed(RollPtr p_Roll)
{
...
void Group::SendLooter(Creature* creature, Player* p_GroupLooter)
{
    ...
    uint64 l_LootGuid = MAKE_NEW_GUID(GUID_LOPART(creature->GetGUID()), 0, HIGHGUID_LOOT);

    l_Data.appendPackGUID(creature->GetGUID());   ///< Owner
    l_Data.appendPackGUID(l_LootGuid);            ///< LootObj
...
void Group::GroupLoot(Loot* loot, WorldObject* pLootedObject)
{
    ...
    for (i = loot->Items.begin(); i != loot->Items.end(); ++i, ++itemSlot)
    {
        ...
        if (item->Quality >= uint32(m_lootThreshold))
        {
            ...
            RollPtr r = std::make_shared<Roll>(Roll(newitemGUID, *i));
...
void Group::MasterLoot(Loot* loot, WorldObject* p_LootedObject)
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        Player* l_Looter = l_Member->player;

        if (!l_Looter || !l_Looter->IsInWorld())
            return;
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        ...
        if (!l_Looter->BossAlreadyLooted(p_LootedObject) && l_Looter->IsAtGroupRewardDistance(p_LootedObject))
    ...
    {
        Player* l_Looter = l_Member->player;
        if (l_Looter && l_Looter->GetGUID() == GetLooterGuid())
...
{
    uint64 newitemGUID = MAKE_NEW_GUID(sObjectMgr->GenerateLowGuid(HIGHGUID_ITEM), 0, HIGHGUID_ITEM);
    RollPtr r = std::make_shared<Roll>(Roll(newitemGUID, item));
    r->lootedGUID = guid;
...
void Group::CountTheRoll(Rolls::iterator p_RollIter)
{
    RollPtr l_Roll = *p_RollIter;

    /// Is loot already deleted ?
    if (!l_Roll->isValid())
    {
        RollId.erase(p_RollIter);
        return;
    }

    /// End of the roll
    if (l_Roll->totalNeed > 0)
    {
        if (!l_Roll->playerVote.empty())
        {
            uint8 l_MaxResult = 0;
            uint64 l_MaxGuid  = (*l_Roll->playerVote.begin()).first;
            Player* l_Player = nullptr;

            for (Roll::PlayerVote::const_iterator l_Iter = l_Roll->playerVote.begin(); l_Iter != l_Roll->playerVote.end(); ++l_Iter)
            {
                if (l_Iter->second != RollVote::NEED)
                    continue;

                uint8 l_Rand = urand(1, 100);
                SendLootRoll(0, l_Iter->first, l_Rand, RollType::ROLL_NEED, l_Roll);
                if (l_MaxResult < l_Rand)
                {
                    l_MaxGuid  = l_Iter->first;
                    l_MaxResult = l_Rand;
                }
            }

            SendLootRollWon(0, l_MaxGuid, l_MaxResult, RollType::ROLL_NEED, l_Roll);
            l_Player = ObjectAccessor::FindPlayer(l_MaxGuid);

            if (l_Player && l_Player->GetSession())
            {
                l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_ROLL_NEED_ON_LOOT, l_Roll->itemid, l_MaxResult);

                ItemPosCountVec l_Dest;
                LootItem* l_LootItem = &(l_Roll->itemSlot >= l_Roll->getLoot()->Items.size() ? l_Roll->getLoot()->QuestItems[l_Roll->itemSlot - l_Roll->getLoot()->Items.size()] : l_Roll->getLoot()->Items[l_Roll->itemSlot]);
                InventoryResult l_Msg = l_Player->CanStoreNewItem(InventorySlot::NULL_BAG, InventorySlot::NULL_SLOT, l_Dest, l_Roll->itemid, l_LootItem->count);
                if (l_Msg == InventoryResult::EQUIP_ERR_OK)
                {
                    l_LootItem->is_looted = true;

                    l_Roll->getLoot()->NotifyItemRemoved(l_Roll->itemSlot);
                    l_Roll->getLoot()->UnlootedCount--;

                    AllowedLooterSet l_Looters = l_LootItem->GetAllowedLooters();
                    std::vector<uint32> l_Bonuses = l_LootItem->itemBonuses;

                    Item* l_Item = l_Player->StoreNewItem(l_Dest, l_Roll->itemid, true, l_LootItem->randomPropertyId, l_Looters, l_Bonuses);
                }
                else
                {
                    l_LootItem->is_blocked = false;
                    l_Player->SendEquipError(l_Msg, nullptr, nullptr, l_Roll->itemid);
                }
            }
        }
    }
    else if (l_Roll->totalGreed > 0)
    {
        if (!l_Roll->playerVote.empty())
        {
            uint8 l_MaxResult = 0;
            uint64 l_MaxGuid = (*l_Roll->playerVote.begin()).first;
            Player* l_Player = nullptr;
            RollVote l_RollVote = RollVote::NOT_VALID;

            Roll::PlayerVote::iterator l_Iter;
            for (l_Iter = l_Roll->playerVote.begin(); l_Iter != l_Roll->playerVote.end(); ++l_Iter)
            {
                if (l_Iter->second != RollVote::GREED && l_Iter->second != RollVote::DISENCHANT)
                    continue;

                uint8 l_Random = urand(1, 100);
                SendLootRoll(0, l_Iter->first, l_Random, l_Iter->second, l_Roll);

                if (l_MaxResult < l_Random)
                {
                    l_MaxGuid  = l_Iter->first;
                    l_MaxResult = l_Random;
                    l_RollVote = l_Iter->second;
                }
            }

            SendLootRollWon(0, l_MaxGuid, l_MaxResult, l_RollVote, l_Roll);
            l_Player = ObjectAccessor::FindPlayer(l_MaxGuid);

            if (l_Player && l_Player->GetSession())
            {
                l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_ROLL_GREED_ON_LOOT, l_Roll->itemid, l_MaxResult);

                LootItem* l_LootItem = &(l_Roll->itemSlot >= l_Roll->getLoot()->Items.size() ? l_Roll->getLoot()->QuestItems[l_Roll->itemSlot - l_Roll->getLoot()->Items.size()] : l_Roll->getLoot()->Items[l_Roll->itemSlot]);

                if (l_RollVote == RollVote::GREED)
                {
                    ItemPosCountVec l_Dest;
                    InventoryResult l_Msg = l_Player->CanStoreNewItem(InventorySlot::NULL_BAG, InventorySlot::NULL_SLOT, l_Dest, l_Roll->itemid, l_LootItem->count);
                    if (l_Msg == InventoryResult::EQUIP_ERR_OK)
                ...
                else if (l_RollVote == RollVote::DISENCHANT)
                {
                    l_LootItem->is_looted = true;
                    ...
                    if (ItemDisenchantLootEntry const* disenchant = (l_Roll->GetItemDisenchantLoot(l_Player)))
                        if (ItemTemplate const* l_Proto = sObjectMgr->GetItemTemplate(l_Roll->itemid))
                            l_Player->AutoStoreLoot(disenchant->Id, LootTemplates_Disenchant, true);

                    /// Disenchant
                    l_Player->UpdateCriteria(CriteriaTypes::CRITERIA_TYPE_CAST_SPELL, 13262);
                }
            }
        }
    }
    ...
    {
        SendLootAllPassed(l_Roll);

        /// Remove is_blocked so that the item is lootable by all players
        if (LootItem* l_Item = &(l_Roll->itemSlot >= l_Roll->getLoot()->Items.size() ? l_Roll->getLoot()->QuestItems[l_Roll->itemSlot - l_Roll->getLoot()->Items.size()] : l_Roll->getLoot()->Items[l_Roll->itemSlot]))
        {
            l_Item->is_blocked = false;

            if (GetLootMethod() == LootMethod::MASTER_LOOT)
                l_Item->alreadyAskedForRoll = false;
        }
    }
...
void Group::SetTargetIcon(uint8 p_Symbol, uint64 p_WhoGuid, uint64 p_TargetGuid, uint8 p_PartyIndex)
{
    ...
    WorldPacket l_Data(Opcodes::SMSG_SEND_RAID_TARGET_UPDATE_SINGLE, (1 + 16 + 1 + 16));
...
void Group::SendUpdate()
{
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        SendUpdateToPlayer(l_Member->guid, l_Member);
    });
}

void Group::SendUpdateToPlayer(uint64 playerGUID, MemberSlotPtr slot)
{
    ...
    if (!slot)
    {
        ...
        if (!witr) // if there is no MemberSlot for such a player
            return;
    ...
    uint32 l_PlayerListCount    = GetMembersCount();

    /// A guild group must have at least 80% of members of the same guild
    if (IsGuildGroup())
        m_PartyFlags = PartyFlags(m_PartyFlags | PartyFlags::PARTY_FLAG_GUILD_GROUP);
    else
        m_PartyFlags = PartyFlags(m_PartyFlags & ~PartyFlags::PARTY_FLAG_GUILD_GROUP);
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_MemberItr) -> void
    {
        Player* l_Member       = HashMapHolder<Player>::Find(l_MemberItr->guid);
        ...
        l_Data.WriteBits(l_MemberItr->name.length(), 6);
        l_Data.WriteBit(0); ///< FromSocialQueue
        l_Data.FlushBits();

        l_Data.appendPackGUID(l_MemberItr->guid);
        l_Data << uint8(l_OnlineState);
        l_Data << uint8(l_MemberItr->group);
        l_Data << uint8(l_MemberItr->flags);
        l_Data << uint8(l_MemberItr->roles);
        l_Data << uint8(l_MemberItr->playerClass);
        l_Data.WriteString(l_MemberItr->name);
    });
...
void Group::UpdatePlayerOutOfRange(Player* player)
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        member = l_Member->player;
        if (member && !member->IsWithinDist(player, member->GetSightRange(), false))
        {
            WorldPacket data;
            player->GetSession()->BuildPartyMemberStatsChangedPacket(player, &data, player->GetGroupUpdateFlag(), false, member->GetGUID());
...
void Group::BroadcastAddonMessagePacket(WorldPacket* packet, const std::string& prefix, bool ignorePlayersInBGRaid, int group, uint64 ignore)
{
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        ...
        if (WorldSession* session = player->GetSession())
            if (session && (group == -1 || player->GetSubGroup() == group))
...
void Group::BroadcastPacket(WorldPacket* packet, bool ignorePlayersInBGRaid, int group, uint64 ignore)
{
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        ...
        if (player->GetSession() && (group == -1 || player->GetSubGroup() == group))
...
void Group::OfflineReadyCheck()
{
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        Player* l_Player = ObjectAccessor::FindPlayer(l_Member->guid);
        ...
        if (!l_Player || !l_Player->GetSession())
        {
            ...
            l_Response.appendPackGUID(l_Member->guid);
...
void Group::UpdateLooterGuid(WorldObject* pLootedObject, bool ifneed)
{
    ...
    if (l_NewLooter)
    {
        if (oldLooterGUID != l_NewLooter->GetGUID())
        {
            SetLooterGuid(l_NewLooter->GetGUID());
            SendUpdate();
        }
    }
    else
...
GroupJoinBattlegroundResult Group::CanJoinBattlegroundQueue(Battleground const* p_BgOrTemplate, MS::Battlegrounds::BattlegroundType::Type p_BgQueueTypeId, uint32 MinPlayerCount)
{
    ...
    m_memberSlots.foreach_until([&](Group::MemberSlotPtr l_Member)
    {
        if (l_Member->player)
        {
            reference = l_Member->player;
            return true;
        }
        return false;
    });
    ...
    memberscount = 0;
    GroupJoinBattlegroundResult l_Result = ERR_BATTLEGROUND_NONE;
    m_memberSlots.foreach_until([&](Group::MemberSlotPtr l_Member) -> bool
    {
        memberscount++;
        Player* member = l_Member->player;

        auto l_Check = [&]() -> GroupJoinBattlegroundResult
        {
        ...
        l_Result = l_Check();
        if (l_Result != ERR_BATTLEGROUND_NONE)
            return true;
...
ItemDisenchantLootEntry const* Roll::GetItemDisenchantLoot(Player const* player) const
{
    ...
    BonusData bonusData;
    bonusData.Initialize(itemTemplate);

    for (auto l_BonusID : m_ItemBonuses)
    {
        ...
        if (l_Bonuses)
        {
            for (auto l_Bonus : *l_Bonuses)
            {
                ...
                bonusData.AddBonus(l_Bonus->Type, l_Bonus->Value);
            }
        }
    }

    uint32 itemLevel = Item::GetItemLevel(itemTemplate, bonusData, player->getLevel(), 0, 0, nullptr);
...
void Group::SetDungeonDifficultyID(Difficulty difficulty)
{
...
#ifndef CROSS
    if (!isBGGroup() && !isBFGroup() && difficulty != Difficulty::DifficultyMythicKeystone)
    {
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member) -> void
    {
        Player* player = l_Member->player;
        if (!player || !player->GetSession())
            return;
...
{
    return m_memberSlots.exist([&](Group::MemberSlotPtr l_Member) -> bool
    {
        Player* player = l_Member->player;
        if (player && !player->getAttackers().empty() && player->GetInstanceId() == instanceId && (player->GetMap()->IsRaidOrHeroicDungeon()))
            for (std::set<Unit*>::const_iterator i = player->getAttackers().begin(); i != player->getAttackers().end(); ++i)
                if ((*i) && (*i)->GetTypeId() == TYPEID_UNIT && (*i)->ToCreature()->GetFlagsExtra() & CREATURE_FLAG_EXTRA_INSTANCE_BIND)
                    return true;
...
Difficulty Group::GetDifficultyID(MapEntry const* p_MapEntry) const
{
    ...
    MapDifficultyEntry const* defaultDifficulty = GetDefaultMapDifficulty(p_MapEntry->MapID);
...
void Group::ResetInstances(uint8 method, bool isRaid, bool isLegacy, Player* SendMsgTo)
{
    ...
    m_boundInstances[diff].safe_foreach(false, [&](BoundInstancesMap::iterator* p_Itr)
    {
        InstanceSavePtr instanceSave = (*p_Itr)->second->save;
        ...
        if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND || method == INSTANCE_RESET_CHANGE_DIFFICULTY)
        {
            ...
            m_boundInstances[diff].getSource().erase((*p_Itr));
            (*p_Itr) = m_boundInstances[diff].getSource().begin();
...
InstanceGroupBindPtr Group::GetBoundInstance(Player* player)
{
...
InstanceGroupBindPtr Group::GetBoundInstance(Map* aMap)
{
    return GetBoundInstance(aMap->GetEntry());
}

InstanceGroupBindPtr Group::GetBoundInstance(MapEntry const* mapEntry)
{
    if (!mapEntry)
        return NULL;
...
InstanceGroupBindPtr Group::GetBoundInstance(Difficulty difficulty, uint32 mapId)
{
    // some instances only have one difficulty
...
InstanceGroupBindPtr Group::BindToInstance(InstanceSavePtr save, bool permanent, bool load)
{
    if (!save || isBGGroup() || isBFGroup())
        return NULL;

    InstanceGroupBindPtr bind = nullptr;

    auto l_Itr = m_boundInstances[save->GetDifficultyID()].find(save->GetMapId());
    if (l_Itr == m_boundInstances[save->GetDifficultyID()].end())
    {
        m_boundInstances[save->GetDifficultyID()][save->GetMapId()] = std::make_shared<InstanceGroupBind>();
...
void Group::UnbindInstance(uint32 p_MapID, uint8 p_DifficultyID, bool p_Unload)
{
    ...
    l_Instance->second->save->RemoveGroup(shared_from_this());                // save can become invalid
...
{
#ifndef CROSS
    if (player && !player->isGameMaster() && !player->IsInDraenorGarrison() && sMapStore.LookupEntry(player->GetMapId())->IsDungeon())
#else /* CROSS */
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        Player* pp = ObjectAccessor::FindPlayer(l_Member->guid);
        if (pp && pp->IsInWorld())
        {
...
void Group::ResetMaxEnchantingLevel()
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        pMember = ObjectAccessor::FindPlayer(l_Member->guid);
...
bool Group::IsGuildGroup() const
{
    ...
    return CountGuildMembers() >= CountNeededGuildMembers();
}

uint32 Group::CountGuildMembers() const
{
    uint32 l_GuildID = GetGroupGuildID();
    uint32 l_Counter = 0;

    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        if (Player* l_Player = l_Member->player)
        {
...
uint32 Group::CountNeededGuildMembers() const
{
    return CalculatePct(GetMembersCount(), 80);
}

uint32 Group::GetGroupGuildID() const
{
    ///      Guild   Count
    std::map<uint32, uint32> l_GuildMembersCount;

    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        if (Player* l_Player = l_Member->player)
        {
            if (uint32 l_GuildID = l_Player->GetGuildId())
            {
                auto const& l_It = l_GuildMembersCount.find(l_GuildID);
                if (l_It != l_GuildMembersCount.end())
                    (*l_It).second++;
                else
                    l_GuildMembersCount[l_GuildID] = 1;
            }
        }
    });
    ...
    uint32 l_Count = 0;
    uint32 l_Guild = 0;
    for (auto l_Iter : l_GuildMembersCount)
    {
        if (l_Iter.second > l_Count)
        {
            l_Count = l_Iter.second;
            l_Guild = l_Iter.first;
        }
    }
...
{
    return _getMemberCSlot(guid) != nullptr;
}
...
uint64 Group::GetMemberGUID(const std::string& name)
{
    ...
    m_memberSlots.foreach_until([&](Group::MemberSlotPtr l_Member) -> bool
    {
        if (l_Member->name == name)
        {
            l_Guid = l_Member->guid;
            return true;
        }
        return false;
    });
...
bool Group::SameSubGroup(uint64 guid1, MemberSlotPtr slot2) const
{
...
uint32 Group::GetActiveMarkers() const
{
    ...
    for (RaidMarkerPtr const& l_Marker : GetRaidMarkers())
    {
        if (l_Marker->Mask)
...
{
    auto const& l_RaidMarkers = GetRaidMarkers();
    ...
    for (RaidMarkerPtr const& l_Marker : l_RaidMarkers)
    {
        if (!l_Marker->Mask)
            continue;
...
void Group::AddRaidMarker(uint8 p_Slot, uint32 p_MapID, float p_X, float p_Y, float p_Z)
{
    ...
    RaidMarkerPtr l_RaidMarker = std::make_shared<RaidMarker>(RaidMarker());
...
void Group::RemoveRaidMarker(uint8 p_Slot)
{
    ...
    m_RaidMarkers[p_Slot] = std::make_shared<RaidMarker>(RaidMarker());
    SendRaidMarkersUpdate();
}
...
void Group::RemoveAllRaidMarkers()
{
    for (uint8 l_Slot = 0; l_Slot < eRaidMarkersMisc::MaxRaidMarkers; ++l_Slot)
        m_RaidMarkers[l_Slot] = std::make_shared<RaidMarker>(RaidMarker());
...
void Group::SetGroupMemberFlag(uint64 guid, bool apply, GroupMemberFlags flag)
{
    ...
    // Check if player is really in the raid
    Group::MemberSlotPtr slot = _getMemberWSlot(guid);
    if (slot == nullptr)
        return;
...
void Group::_initRaidSubGroupsCounter()
{
    ...
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member) -> void
    {
        ++m_subGroupsCounts[l_Member->group];
    });
}

Group::MemberSlotPtr const Group::_getMemberCSlot(uint64 p_Guid) const
{
    Group::MemberSlotPtr l_Slot = nullptr;
...
Group::MemberSlotPtr Group::_getMemberWSlot(uint64 p_Guid)
{
    Group::MemberSlotPtr l_Slot = nullptr;
...
void Group::RemoveUniqueGroupMemberFlag(GroupMemberFlags flag)
{
    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member)
    {
        if (l_Member->flags & flag)
            l_Member->flags &= ~flag;
    });
}

void Group::ToggleGroupMemberFlag(Group::MemberSlotPtr slot, uint8 flag, bool apply)
{
    if (apply)
        (slot)->flags |= flag;
    else
...
void Group::MemberLost(Player* player, uint32 againstMatchmakerRating, uint8 slot, int32 MatchmakerRatingChange)
{
    ...
    m_memberSlots.foreach_until([&](Group::MemberSlotPtr l_Member) -> bool
    {
        if (l_Member->guid == player->GetGUID())
        {
            // Update personal rating
...
void Group::WonAgainst(uint32 p_OwnMMRating, uint32 p_OpponentMMRating, int32& p_RatingChange, int32& p_MMRChange, uint8 p_Bracket)
{
    // Called when the team has won
    // Change in Matchmaker rating
    p_MMRChange = Arena::GetMatchmakerRatingMod(p_OwnMMRating, p_OpponentMMRating, true);

    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member) -> void
    {
        if (Player* l_Player = ObjectAccessor::FindPlayer(l_Member->guid))
        {
            // Change in Team Rating
            p_RatingChange = Arena::GetRatingMod(l_Player->GetArenaPersonalRating(p_Bracket), p_OpponentMMRating, true);

            if (l_Player->GetArenaPersonalRating(p_Bracket) < 1000)
                p_RatingChange = 96;

            if (l_Player->GetBattleground())
            {
                for (Battleground::BattlegroundScoreMap::const_iterator l_Itr2 = l_Player->GetBattleground()->GetPlayerScoresBegin(); l_Itr2 != l_Player->GetBattleground()->GetPlayerScoresEnd(); ++l_Itr2)
                {
                    if (l_Itr2->first == l_Member->guid)
                        l_Itr2->second->RatingChange = p_RatingChange;
                }
            }
...
void Group::HandleArenaDecay(uint8 p_Slot)
{
    ...
    auto const& l_Slot = l_WorldStates.find(p_Slot);
    if (l_Slot == l_WorldStates.end())
        return;

    CharacterWorldStates l_MatchCount = l_Slot->second;

    m_memberSlots.foreach([&](Group::MemberSlotPtr l_Member) -> void
    {
        if (Player* l_Player = ObjectAccessor::FindPlayer(l_Member->guid))
            l_Player->ModifyCharacterWorldState(l_MatchCount, 1);
    });
}
