...
enum GroupMemberOnlineStatus
{
    ...
    MEMBER_STATUS_VEHICLE   = 0x0200,                       // Lua_UnitInVehicl
};
...
enum PartyFlags
{
    PARTY_FLAG_NORMAL                   = 0x0000,
    PARTY_FLAG_BG                       = 0x0001,
    PARTY_FLAG_RAID                     = 0x0002,
    PARTY_FLAG_UNK_4                    = 0x0004,
    PARTY_FLAG_LFG                      = 0x0008,
    PARTY_FLAG_UNK_16                   = 0x0010,
    PARTY_FLAG_EVERYONE_IS_ASSISTANT    = 0x0040,
    PARTY_FLAG_UNK_128                  = 0x0080,
    PARTY_FLAG_GUILD_GROUP              = 0x0100,

    PARTY_FLAG_BGRAID                   = PARTY_FLAG_BG | PARTY_FLAG_RAID,
...
typedef std::shared_ptr<Roll> RollPtr;
...
typedef std::shared_ptr<InstanceGroupBind> InstanceGroupBindPtr;
...
class Group : public std::enable_shared_from_this<Group>
{
    public:
        struct MemberSlot
        {
            Player*     player; ///< can be nullptr if the player is offline or if its a creature
            uint64      guid;
            std::string name;
            uint8       group;
            uint8       flags;
        ...
        typedef std::shared_ptr<MemberSlot> MemberSlotPtr;
        typedef ACE_Based::LockedVector<MemberSlotPtr> MemberSlotList;
        ...
        typedef std::shared_ptr<RaidMarker> RaidMarkerPtr;

        typedef ACE_Based::RecursiveLockedMap< uint32 /*mapId*/, InstanceGroupBindPtr> BoundInstancesMap;
    protected:
        typedef MemberSlotList::iterator member_witerator;
        typedef std::set<uint64> InvitesList;

        typedef ACE_Based::LockedVector<RollPtr> Rolls;
    ...
    public:
        ...
        bool   AddCreatureMember(Creature* p_Creature);
        ...
        bool   RemoveCreatureMember(Creature* p_Creature);
        ...
        uint32 CountGuildMembers() const;
        uint32 CountNeededGuildMembers() const;
        uint32 GetGroupGuildID() const;
        ...
        bool SameSubGroup(uint64 guid1, MemberSlotPtr slot2) const;
        bool SameSubGroup(Player const* member1, Player const* member2) const;
        bool HasFreeSlotSubGroup(uint8 subgroup) const;

        void LinkPlayer(Player* p_Player);
        void UnlinkPlayer(Player* p_Player);

        MemberSlotList const& GetMemberSlots() const { return m_memberSlots; }
        MemberSlotList& GetMemberSlots() { return m_memberSlots; }
        uint32 GetMembersCount() const { return m_memberSlots.size(); }

        ACE_Based::LockedVector<RaidMarkerPtr> const& GetRaidMarkers() const { return m_RaidMarkers; }
        ...
        void SendUpdate();
        void SendUpdateToPlayer(uint64 playerGUID, MemberSlotPtr slot = nullptr);
        ...
        void HandleArenaDecay(uint8 p_Slot);
        ...
        void SendLootStartRoll(uint32 CountDown, uint32 mapid, RollPtr r);
        void SendLootStartRollToPlayer(uint32 countDown, uint32 mapId, Player* p, bool canNeed, RollPtr r);
        void SendLootRoll(uint64 SourceGuid, uint64 TargetGuid, uint8 RollNumber, uint8 RollType, RollPtr r);
        void SendLootRollWon(uint64 SourceGuid, uint64 TargetGuid, uint8 RollNumber, uint8 RollType, RollPtr r);
        void SendLootAllPassed(RollPtr roll);
        ...
        InstanceGroupBindPtr BindToInstance(InstanceSavePtr save, bool permanent, bool load = false);
        void UnbindInstance(uint32 mapid, uint8 difficulty, bool unload = false);
        InstanceGroupBindPtr GetBoundInstance(Player* player);
        InstanceGroupBindPtr GetBoundInstance(Map* aMap);
        InstanceGroupBindPtr GetBoundInstance(MapEntry const* mapEntry);
        InstanceGroupBindPtr GetBoundInstance(Difficulty difficulty, uint32 mapId);
        ...
        uint16 GetPartyFlags() const;
        uint8 GetPartyIndex() const;
        uint8 GetPartyType()  const;

        uint32 GetTeam() const { return m_Team; }

        uint64 m_ChallengeOwner;
        uint64 m_ChallengeItem;
    ...
    protected:
        ...
        Group::MemberSlotPtr const _getMemberCSlot(uint64 Guid) const;
        Group::MemberSlotPtr _getMemberWSlot(uint64 Guid);
        void SubGroupCounterIncrease(uint8 subgroup);
        void SubGroupCounterDecrease(uint8 subgroup);
        void ToggleGroupMemberFlag(Group::MemberSlotPtr slot, uint8 flag, bool apply);
        ...
        bool                 m_IsDisbanding;
        std::recursive_mutex m_DisbandingLock;

        ACE_Based::LockedVector<RaidMarkerPtr> m_RaidMarkers;
};
