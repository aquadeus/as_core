...
{
    {
        ...
        QueryResult l_Result = CharacterDatabase.Query("SELECT guid FROM group_member GROUP BY guid HAVING COUNT(guid) = 1");
        if (l_Result)
        {
            std::ostringstream l_Query;
            l_Query << "DELETE FROM groups WHERE guid IN (";

            do
            {
                Field* l_Fields = l_Result->Fetch();
                uint32 l_GroupGuid = l_Fields[0].GetUInt32();
            ...
            /// Remove last ","
            l_Query.seekp(-1, std::ios_base::end);

            l_Query << ");";
            CharacterDatabase.DirectExecute(l_Query.str().c_str());
        }
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Loading Group instance saves...");
    {
        ...
        do
        {
            ...
            InstanceSavePtr save = sInstanceSaveMgr->AddInstanceSave(mapEntry->MapID, fields[2].GetUInt32(), Difficulty(diff), time_t(fields[5].GetUInt32()), (bool)fields[6].GetUInt64(), true);
