...
size_t CinematicSequenceMgr::Load()
{
    ...
    for (uint32 l_SequenceID = 0; l_SequenceID < sCinematicSequencesStore.GetNumRows(); ++l_SequenceID)
    {
        ...
        if (!l_SequenceInfo || !l_SequenceInfo->cinematicCamera[0])
            continue;

        CinematicCameraEntry const* l_CameraEntry = sCinematicCameraStore.LookupEntry(l_SequenceInfo->cinematicCamera[0]);
        if (!l_CameraEntry || !l_CameraEntry->ModelFileDataID)
            continue;

        char l_Str[32];
        sprintf(l_Str, "FILE%08X.xxx", l_CameraEntry->ModelFileDataID);
        std::string l_ModelFileName = l_Str;
