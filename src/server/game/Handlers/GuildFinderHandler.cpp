...
void WorldSession::HandleGuildFinderAddRecruit(WorldPacket& p_Packet)
{
    ...
    MembershipRequestPtr l_Request = std::make_shared<MembershipRequest>(GetPlayer()->GetGUIDLow(), l_GuildLowGuid, l_Availability, l_ClassRoles, l_PlayStyle, l_Comment, time(NULL));
...
void WorldSession::HandleGuildFinderBrowse(WorldPacket& p_Packet)
{
    ...
    uint32 l_GuildVirtualRealm = g_RealmID;
    ...
    {
        LFGuildSettingsPtr l_GuildSettings = l_It->second;
    ...
    if (l_GuildCount == 0)
    {
        WorldPacket l_Response(SMSG_LF_GUILD_BROWSE);
        l_Response << uint32(0);
    ...
    WorldPacket l_Response(SMSG_LF_GUILD_BROWSE, 1024);
    ...
    for (LFGuildStore::const_iterator l_It = l_GuildList.begin(); l_It != l_GuildList.end(); ++l_It)
    {
        ...
        l_Response << uint32(l_GuildVirtualRealm);                                                          ///< GuildVirtualRealm
...
{
    std::list<MembershipRequestPtr> l_ApplicatedGuilds = sGuildFinderMgr->GetAllMembershipRequestsForPlayer(GetPlayer()->GetGUIDLow());

    WorldPacket l_Data(SMSG_LF_GUILD_APPLICATIONS);
    ...
    for (std::list<MembershipRequestPtr>::const_iterator l_It = l_ApplicatedGuilds.begin(); l_It != l_ApplicatedGuilds.end(); ++l_It)
    {
        Guild* l_Guild = sGuildMgr->GetGuildById((*l_It)->GetGuildId());
        if (!l_Guild)
            continue;

        LFGuildSettingsPtr l_GuildSettings = sGuildFinderMgr->GetGuildSettings((*l_It)->GetGuildId());
        MembershipRequestPtr l_Request = *l_It;
...
void WorldSession::HandleGuildFinderGetRecruits(WorldPacket& p_Packet)
{
    ...
    auto l_RecruitsList = sGuildFinderMgr->GetAllMembershipRequestsForGuild(m_Player->GetGuildId());

    WorldPacket l_Data(SMSG_LF_GUILD_RECRUITS, 10 * 1024);

    l_Data << uint32(l_RecruitsList.size());                            ///< Recruits Count
    l_Data << uint32(time(NULL));                                       ///< Update Time

    l_RecruitsList.foreach([&](MembershipRequestPtr l_Request)
    {
...
void WorldSession::HandleGuildFinderPostRequest(WorldPacket& /*p_Packet*/)
{
    ...
    LFGuildSettingsPtr l_Settings = sGuildFinderMgr->GetGuildSettings(m_Player->GetGuildId());

    WorldPacket l_Response(SMSG_LF_GUILD_POST);
    ...
    if (l_IsGuildMaster)
    {
        ...
        l_Response << uint32(l_Settings->GetInterests());            ///< PlayStyle
        l_Response << uint32(l_Settings->GetAvailability());         ///< Availability
        l_Response << uint32(l_Settings->GetClassRoles());           ///< ClassRoles
        l_Response << uint32(l_Settings->GetLevel());                ///< LevelRange
        l_Response << uint32(0);                                    ///< SecondsRemaining
...
void WorldSession::HandleGuildFinderSetGuildPost(WorldPacket& p_Packet)
{
    ...
    LFGuildSettingsPtr settings = std::make_shared<LFGuildSettings>(l_Active, m_Player->GetTeamId(), m_Player->GetGuildId(), l_ClassRoles, l_Availability, l_PlayStyle, l_LevelRange, l_Comment);
