...
void WorldSession::HandlePetAction(WorldPacket & p_RecvPacket)
{
    ...
    if (!l_Pet->isAlive())
    {
        ...
        if (!l_Pet->isAlive() && !(l_Spell->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD))
            return;
    }
    ...
    else
    {
        ...
        for (Unit::ControlList::iterator l_Itr = GetPlayer()->m_Controlled.begin(); l_Itr != GetPlayer()->m_Controlled.end(); ++l_Itr)
            if ((*l_Itr)->GetEntry() == l_Pet->GetEntry() && ((*l_Itr)->isAlive() || (l_Spell && l_Spell->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD)))
    ...
    switch (flag)
    {
        case ACT_COMMAND:                                   //0x07
            switch (spellid)
            {
                case COMMAND_STAY:                          //flat=1792  //STAY
                    ...
                    charmInfo->SetIsMoveTo(false);
                    charmInfo->SetIsMoveToStay(false);
                    ...
                    if (pet->getVictim() != TargetUnit || (pet->getVictim() == TargetUnit && !pet->GetCharmInfo()->IsCommandAttack()))
                    {
                        ...
                        if (Unit* owner = pet->GetOwner())
                        {
                            if (owner->getClass() == CLASS_DEATH_KNIGHT)
                            {
                                ...
                                {
                                    if ((*itr)->GetEntry() == DeathKnightPet::Gargoyle && !(*itr)->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE))
                                    {
                ...
                case COMMAND_MOVE_TO:
                    ...
                    pet->GetMotionMaster()->Clear(false);
                    pet->GetMotionMaster()->MoveIdle();
                    pet->GetMotionMaster()->MovePoint(pet->GetGUIDLow(), x, y, z);
            ...
            switch (spellid)
            {
                ...
                case REACT_HELPER:
                    if (pet->GetTypeId() == TYPEID_UNIT)
                    {
                        ...
                        if (charmInfo && charmInfo->GetCommandState() == COMMAND_MOVE_TO)
                            charmInfo->SetIsMoveToStay(false);
                    }
                    break;
            }
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (result == SPELL_CAST_OK)
            {
                ...
                {
                    // This is true if pet has no target or has target but targets differs.
                    if (pet->getVictim() != unit_target && spellInfo->HasAttribute(SpellAttr1::SPELL_ATTR1_MELEE_COMBAT_START))
                    {
                        if (pet->getVictim())
                            pet->AttackStop();
            ...
            else
            {
                if (pet->isPossessed() || pet->IsVehicle())
                    Spell::SendCastResult(GetPlayer(), spellInfo, Guid128(GUID_TYPE_CAST, 0, spellInfo->Id, 0), result);
                else
                    pet->SendPetCastFail(spellid, result, 0, spell->GetCastGuid());
...
void WorldSession::HandlePetRename(WorldPacket & p_RecvPacket)
{
    ...
    if (!l_Pet || !l_Pet->isPet() || ((Pet*)l_Pet)->getPetType()!= HUNTER_PET ||
        !l_Pet->HasByteFlag(UNIT_FIELD_SHAPESHIFT_FORM, UNIT_BYTES_2_OFFSET_PET_FLAGS, UNIT_CAN_BE_RENAMED) ||
    ...
    l_Pet->RemoveByteFlag(UNIT_FIELD_SHAPESHIFT_FORM, UNIT_BYTES_2_OFFSET_PET_FLAGS, UNIT_CAN_BE_RENAMED);
...
void WorldSession::HandlePetCastSpellOpcode(WorldPacket& p_RecvPacket)
{
    ...
    uint32 l_SpellXSpellVisualID = 0;
    ...
    p_RecvPacket >> l_SpellXSpellVisualID;
    ...
    if (l_SpellInfo->StartRecoveryCategory > 0) // Check if spell is affected by GCD
    {
        ...
        {
            l_Caster->SendPetCastFail(l_SpellID, SPELL_FAILED_NOT_READY, l_CastCount, Guid128(GUID_TYPE_CAST, 0, l_SpellInfo->Id, 0));
            return;
        }
    }
    ...
    std::list<AuraEffect*> const& l_TriggerByClients = l_Caster->GetAuraEffectsByType(AuraType::SPELL_AURA_PERIODIC_TRIGGER_BY_CLIENT);
    ...
    Spell* l_Spell = new Spell(l_Caster, l_SpellInfo, l_TriggerFlags);
        ...
        if (l_Caster->GetTypeId() == TYPEID_UNIT)
        {
            ...
            if (l_CreaturePet->isPet())
            {
                ...
                if (l_Pet->getPetType() == SUMMON_PET && (urand(0, 100) < 10))
                    ...
                    l_Pet->SendPetAIReaction(l_PetGUID);
            }
        }
    }
    else
    {
        l_Caster->SendPetCastFail(l_SpellID, l_Result, l_CastCount, l_Spell->GetCastGuid());
