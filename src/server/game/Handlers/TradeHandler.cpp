            ...
            if (myItems[i])
            {
                if (!AccountMgr::IsPlayerAccount(m_Player->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
                    sLog->outCommand(m_Player->GetSession()->GetAccountId(), "", m_Player->GetGUIDLow(), m_Player->GetName(),
                                    ...
                                    myItems[i]->GetTemplate()->Name1->Get(sWorld->GetDefaultDb2Locale()), myItems[i]->GetEntry(), myItems[i]->GetCount(),
            ...
            if (hisItems[i])
            {
                if (!AccountMgr::IsPlayerAccount(trader->GetSession()->GetSecurity()) && sWorld->getBoolConfig(CONFIG_GM_LOG_TRADE))
                {
                    sLog->outCommand(trader->GetSession()->GetAccountId(), "", trader->GetGUIDLow(), trader->GetName(),
                                    ...
                                    "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: %s (Account: %u)",
                                    trader->GetName(), trader->GetSession()->GetAccountId(),
                                    hisItems[i]->GetTemplate()->Name1->Get(sWorld->GetDefaultDb2Locale()), hisItems[i]->GetEntry(), hisItems[i]->GetCount(),
                                    m_Player->GetName(), m_Player->GetSession()->GetAccountId());
                }
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& /*p_RecvData*/)
{
    ...
    int64 l_MyMoney = m_Player->GetMoney();

    l_MyMoney   -= int64(l_MyTrade->GetMoney());
    l_MyMoney   += int64(l_HisTrade->GetMoney());

    int64 l_HisMoney = l_Trader->GetMoney();

    l_HisMoney  -= int64(l_HisTrade->GetMoney());
    l_HisMoney  += int64(l_HisTrade->GetMoney());

    if (l_MyMoney > int64(MAX_MONEY_AMOUNT))
    {
        m_Player->SendEquipError(InventoryResult::EQUIP_ERR_TOO_MUCH_GOLD, nullptr, nullptr);
        l_MyTrade->SetAccepted(false, true);
        l_MyTrade->SetAccepted(false, false);
        return;
    }

    if (l_HisMoney > int64(MAX_MONEY_AMOUNT))
    {
        l_Trader->SendEquipError(InventoryResult::EQUIP_ERR_TOO_MUCH_GOLD, nullptr, nullptr);
        l_HisTrade->SetAccepted(false, true);
        l_MyTrade->SetAccepted(false, false);
        return;
    }
    ...
    if (l_HisTrade->IsAccepted())
    {
        ...
        if (uint32 l_MySpellID = l_MyTrade->GetSpell())
        {
            ...
            l_MyTargets.SetCaster(m_Player);
        ...
        if (uint32 l_HisSpellID = l_HisTrade->GetSpell())
        {
            ...
            l_HisTargets.SetCaster(l_Trader);
        ...
        // Update money
        m_Player->ModifyMoney(-int64(l_MyTrade->GetMoney()), "HandleAcceptTradeOpcode " + std::to_string(l_Trader->GetGUIDLow()));
        m_Player->ModifyMoney(l_HisTrade->GetMoney(), "HandleAcceptTradeOpcode " + std::to_string(l_Trader->GetGUIDLow()));
        l_Trader->ModifyMoney(-int64(l_HisTrade->GetMoney()), "HandleAcceptTradeOpcode " + std::to_string(m_Player->GetGUIDLow()));
        l_Trader->ModifyMoney(l_MyTrade->GetMoney(), "HandleAcceptTradeOpcode " + std::to_string(m_Player->GetGUIDLow()));
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& p_RecvData)
{
    ...
    uint32 levelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ);
    ...
    if (GetPlayer()->getClass() == CLASS_DEATH_KNIGHT)
        levelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
    else if (GetPlayer()->getClass() == CLASS_DEMON_HUNTER)
        levelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);

    if (GetPlayer()->getLevel() < std::min(levelReq, (uint32)LevelLimit::MAX_LEVEL))
    {
        SendNotification(GetTrinityString(LANG_TRADE_REQ), levelReq);
        return;
    }
    ...
    uint32 otherLevelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ);
    switch (l_Player->getRace())
    {
        ...
        case Races::RACE_LIGHTFORGED_DRAENEI:
        case Races::RACE_HIGHMOUNTAIN_TAUREN:
            otherLevelReq += sWorld->getIntConfig(CONFIG_ALLIED_RACE_LEVEL_REQ_DIFF);
        default:
            break;
    }
    if (l_Player->getClass() == CLASS_DEATH_KNIGHT)
        otherLevelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
    else if (l_Player->getClass() == CLASS_DEMON_HUNTER)
        otherLevelReq = sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);

    if (l_Player->getLevel() < std::min(otherLevelReq, (uint32)LevelLimit::MAX_LEVEL))
    {
        SendNotification(GetTrinityString(LANG_TRADE_OTHER_REQ), otherLevelReq);
        return;
    }
