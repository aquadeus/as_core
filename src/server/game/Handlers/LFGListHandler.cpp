...
{
    uint32 l_NameLen, l_CommentLen, l_VoiceChatLen, l_ActivityId, l_UnkLegion;
    ...
    LFGListEntryPtr l_List = std::make_shared<LFGListEntry>();
...
void WorldSession::BuildLfgListRideTicket(WorldPacket* p_Data, LFGListEntryPtr const p_Entry)
{
...
void WorldSession::BuildLfgListApplicationRideTicket(WorldPacket* p_Data, LFGListEntry::LFGListApplicationEntryPtr const p_Entry)
{
...
void WorldSession::BuildLfgListEntryInfo(WorldPacket* p_Data, LFGListEntryPtr const p_Entry)
{
    ...
    *p_Data << uint8(0);         ///< Unk
    ...
    l_Members.foreach([&](Group::MemberSlotPtr l_Member)
    {
...
void WorldSession::BuildLfgListJoinRequest(WorldPacket* p_Data, LFGListEntryPtr const p_Entry)
{
    *p_Data << uint32(p_Entry->m_ActivityEntry->ID);
    *p_Data << float(p_Entry->m_RequiredItemLevel);
    *p_Data << uint32(0);   ///< UnkLegion
    ...
    p_Data->WriteBit(l_Unk1);
    p_Data->WriteBit(l_Unk2);
    p_Data->FlushBits();
...
void WorldSession::SendLfgListJoinResult(LFGListEntryPtr const p_Entry, uint8 p_Error)
{
...
void WorldSession::BuildLfgListQueueUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, bool p_Listed)
{
...
{
    uint32 l_NameLen, l_CommentLen, l_VoiceChatLen, l_ActivityId, l_ID, l_UnkLegion;
    float l_ItemLevel;

    LFGListEntryPtr l_Entry = ReadLfgListRideTicketInfo(&p_RecvData, &l_ID);
...
{
    uint32 l_ActivityCategory, l_ActivitySubCategory, l_Filter, l_UnkGuidLoop, l_PreferedFilter, l_BlacklistCount;

    uint32 l_FilterLen = p_RecvData.ReadBits(6);

    p_RecvData >> l_ActivityCategory;
    p_RecvData >> l_ActivitySubCategory;
    p_RecvData >> l_Filter;
    p_RecvData >> l_PreferedFilter;
    p_RecvData >> l_BlacklistCount;
    p_RecvData >> l_UnkGuidLoop;
...
void WorldSession::SendLfgSearchResponse(uint32 p_ActivityCategory, uint32 p_ActivitySubCategory, std::string p_FilterString)
{
    ...
    std::list<LFGListEntryPtr> l_LFGList = sLFGListMgr->GetFilteredList(p_ActivityCategory, p_ActivitySubCategory, p_FilterString, GetPlayer());
...
LFGListEntryPtr WorldSession::ReadLfgListRideTicketInfo(WorldPacket* p_Data, uint32* p_ID)
{
    uint64 l_Guid;
    ...
    *p_Data >> l_Timestamp;
    ...
    return sLFGListMgr->GetEntrybyGuidLow(l_ID);
}

LFGListEntry::LFGListApplicationEntryPtr WorldSession::ReadLfgListApplicantRideTicketInfo(WorldPacket* p_Data, LFGListEntryPtr p_ReferenceEntry, uint32* p_ID)
{
    uint64 l_Guid;
    uint32 l_Type, l_Timestamp, l_ID;
...
LFGListEntry::LFGListApplicationEntryPtr WorldSession::ReadLfgListApplicantRideTicketInfo(WorldPacket* p_Data, uint32* p_ID)
{
    uint64 l_Guid;
    uint32 l_Type, l_Timestamp, l_ID;
...
void WorldSession::HandleLfgListApplyForGroup(WorldPacket& p_RecvData)
{
    ...
    LFGListEntry::LFGListApplicationEntryPtr l_Application = std::make_shared<LFGListEntry::LFGListApplicationEntry>(GetPlayer()->GetGUIDLow(), l_Entry);
...
void WorldSession::SendLfgListApplyForGroupResult(LFGListEntryPtr const p_LFGEntry, LFGListEntry::LFGListApplicationEntryPtr const p_Application)
{
    ...
    l_Packet << uint32(p_Application->GetTimeoutTime());
...
void WorldSession::SendLfgListApplicantGroupInviteResponse(LFGListEntry::LFGListApplicationEntryPtr const p_Applicant)
{
    WorldPacket l_Data(SMSG_LFG_LIST_APPLICANT_GROUP_INVITE, 0x200);
    BuilfLfgListApplicantGroupInvite(&l_Data, p_Applicant);
    SendPacket(&l_Data);
}

void WorldSession::BuildLfgListApplicantListUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, LFGListEntry::LFGListApplicationEntryPtr const p_Applicant)
{
    BuildLfgListApplicantListUpdate(p_Data, p_Entry, std::list<LFGListEntry::LFGListApplicationEntryPtr>({ p_Applicant }));
}

void WorldSession::BuildLfgListApplicantListUpdate(WorldPacket* p_Data, LFGListEntryPtr const p_Entry, std::list<LFGListEntry::LFGListApplicationEntryPtr> p_ApplicantList)
{
    BuildLfgListRideTicket(p_Data, p_Entry);
    ...
    for (auto& l_Applicant : p_ApplicantList)
    {
        ...
        *p_Data << uint32(l_Player && l_Applicant->m_Listed ? 1 : 0);    ///< 1 Player in queue - could also by a group - NYI
        if (l_Player && l_Applicant->m_Listed)
        {
            ...
            *p_Data << uint32(l_Player->GetHonorLevel());
...
void WorldSession::BuilfLfgListApplicantGroupInvite(WorldPacket* p_Data, LFGListEntry::LFGListApplicationEntryPtr const p_Applicant)
{
...
{
    LFGListEntryPtr l_Entry = ReadLfgListRideTicketInfo(&p_RecvData, nullptr);
    LFGListEntry::LFGListApplicationEntryPtr l_Application = ReadLfgListApplicantRideTicketInfo(&p_RecvData, l_Entry, nullptr);
    p_RecvData.rfinish();
...
{
    LFGListEntry::LFGListApplicationEntryPtr l_Applicant = ReadLfgListApplicantRideTicketInfo(&p_RecvData, nullptr);
    if (!l_Applicant)
        return;
...
void WorldSession::HandleLfgListInvitationAnswer(WorldPacket& p_RecvData)
{
    ...
    sLFGListMgr->ChangeApplicantStatus(l_Applicant, l_Accept ? LFGListEntry::LFGListApplicationEntry::LFGListApplicationStatus::LFG_LIST_APPLICATION_STATUS_INVITE_ACCEPTED : LFGListEntry::LFGListApplicationEntry::LFGListApplicationStatus::LFG_LIST_APPLICATION_STATUS_INVITE_DECLINED);
}
