...
void WorldSession::HandleUseItemOpcode(WorldPacket& p_RecvPacket)
{
    ...
    p_RecvPacket.readGUID128(l_CastGUID);
    ...
    if (proto->Class == ITEM_CLASS_CONSUMABLE && !(proto->Flags & int32(ItemFlags::IGNORE_DEFAULT_ARENA_RESTRICTIONS)) && (pUser->InArena() || pUser->InRatedBattleGround()))
    {
    ...
    if (proto->Flags & int32(ItemFlags::NOT_USEABLE_IN_ARENA) && (pUser->InArena() || pUser->InRatedBattleGround()))
    {
        pUser->SendEquipError(EQUIP_ERR_NOT_DURING_ARENA_MATCH, pItem, NULL);
        return;
    }
    ...
    if (pItem->GetTemplate()->Bonding == BIND_WHEN_USE || pItem->GetTemplate()->Bonding == BIND_WHEN_PICKED_UP || pItem->GetTemplate()->Bonding == BIND_QUEST_ITEM)
    {
        if (!pItem->IsSoulBound())
        {
            pItem->SetState(ITEM_CHANGED, pUser);
            pItem->SetBinding(true);
            GetCollectionMgr()->AddItemAppearance(pItem);
        }
    }

    pItem->SetNotRefundable(m_Player);

    SpellCastTargets targets;

    targets.SetCaster(pUser);
    ...
    if (!sScriptMgr->OnItemUse(pUser, pItem, targets))
    {
        ...
        pUser->CastItemUseSpell(pItem, targets, l_CastGUID, l_Misc[0], l_Misc[1]);
    }
}
...
void WorldSession::HandleOpenItemOpcode(WorldPacket& p_Packet)
{
    ...
    if (!(l_ItemTemplate->Flags & int32(ItemFlags::HAS_LOOT)) && !l_Item->HasFlag(ITEM_FIELD_DYNAMIC_FLAGS, ITEM_FIELD_FLAG_WRAPPED))
    {
...
void WorldSession::HandleGameObjectUseOpcode(WorldPacket& recvData)
{
    ...
    {
        if (obj->GetGoType() != GameobjectTypes::GAMEOBJECT_TYPE_FISHINGNODE && !obj->IsWithinDistInMap(m_Player, GOB_INTERACTION_DISTANCE) && 
           (obj->GetEntry() != 268526 && !obj->IsWithinDistInMap(m_Player, GOB_INTERACTION_DISTANCE * 2)))
            return;
...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    if (!go->IsWithinDistInMap(m_Player, GOB_INTERACTION_DISTANCE))
        return;

    if (go->AI()->GossipHello(m_Player))
        return;

    m_Player->UpdateCriteria(CRITERIA_TYPE_USE_GAMEOBJECT, go->GetEntry());
}
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& p_RecvPacket)
{
    ...
    m_Player->SetCastingSpell(l_SpellID);

    m_LastSpellCasted.push_back(l_SpellID);
    ...
    bool l_IsScenarioSpell = false;
    if (Map* map = mover->GetMap())
    {
        if (Scenario* progress = sScenarioMgr->GetScenario(map->GetScenarioGuid()))
        {
            std::vector<ScenarioSpellData> const* scSpells = sObjectMgr->GetScenarioSpells(progress->GetScenarioId());
            if (scSpells)
            {
                for (std::vector<ScenarioSpellData>::const_iterator itr = scSpells->begin(); itr != scSpells->end(); ++itr)
                {
                    if ((*itr).StepId == progress->GetCurrentStep())
                    {
                        if (l_SpellID == (*itr).Spells)
                            l_IsScenarioSpell = true;
                    }
                }
            }
        }
    ...
    if (caster->IsPlayer()
        ...
        && !l_IsScenarioSpell
        && sSpellMgr->GetRespecializationIDBySpell(spellInfo->Id) == nullptr)
    {
        // GameObject Use
        ...
        else
        {
            ...
            {
#ifndef CROSS
                uint32 l_ConditionID = l_Unit->AI()->GetData(MS::Garrison::GDraenor::CreatureAIDataIDs::HasRecipe + l_SpellID);
#else /* CROSS */
    ...
    SpellInfo const* l_TempSpellInfo = caster->GetCastSpellInfo(spellInfo);
    if (!caster->HasSpell(l_TempSpellInfo->Id))
        l_TempSpellInfo = caster->Unit::GetCastSpellInfo(spellInfo);

    spellInfo = l_TempSpellInfo;
    l_SpellID = spellInfo->Id;
    ...
    std::list<AuraEffect*> const& l_TriggerByClients = caster->GetAuraEffectsByType(AuraType::SPELL_AURA_PERIODIC_TRIGGER_BY_CLIENT);
    for (auto l_Itr : l_TriggerByClients)
    {
        if (l_Itr->GetSpellEffectInfo()->TriggerSpell == l_SpellID)
        {
            l_TriggerFlags = TriggerCastFlags::TRIGGERED_FULL_MASK;
            break;
        }
    }

    uint32 l_Gcd = m_Player->GetGlobalCooldownMgr().GetGlobalCooldown(spellInfo);
    Spell* spell = new Spell(caster, spellInfo, l_TriggerFlags, 0, false);
    spell->m_FromClient = true;
    spell->m_Misc[0] = l_Misc[0];
    spell->m_Misc[1] = l_Misc[1];
    spell->SetCastGUID(l_CastGUID);
    spell->prepare(&targets, nullptr, l_Gcd);
}
...
void WorldSession::HandleCancelCastOpcode(WorldPacket& recvPacket)
{
    ...
    recvPacket.readGUID128(l_CastGUID);
    recvPacket >> spellId;
...
void WorldSession::HandleCancelChanneling(WorldPacket& recvData)
{
    ...
    /// Handle if it is a possess/charm spell
    /// Fox example: Mind Control - 605
    if (Spell* l_Spell = m_Player->GetCurrentSpell(CURRENT_CHANNELED_SPELL))
        if (l_Spell->GetSpellInfo()->HasAura(AuraType::SPELL_AURA_MOD_POSSESS))
            l_Mover = m_Player;
...
void WorldSession::HandleSelfResOpcode(WorldPacket& /*recvData*/)
{
    ...
    if (uint32 l_RessurectSpellID = m_Player->GetFirstDescriptorRessurectSpell())
    {
        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(l_RessurectSpellID);
        if (spellInfo)
            m_Player->CastSpell(m_Player, spellInfo, false, 0);

        m_Player->SetResurectSpell(0);
    }
}
...
void WorldSession::HandleMirrorImageDataRequest(WorldPacket& recvData)
{
    ...
    Unit* unit = sObjectAccessor->GetUnit(*m_Player, guid);
    if (!unit)
        return;
    ...
    Unit* l_SimulacrumTarget = sObjectAccessor->GetUnit(*creator, creator->m_SpellHelper.GetUint64(eSpellHelpers::SimulacrumTarget));
    if (l_SimulacrumTarget)
        creator = l_SimulacrumTarget;

    if (creator->m_SpellHelper.GetBool(eSpellHelpers::MirrorImage))
    {
        Unit* l_IllusionTarget = sObjectAccessor->GetUnit(*creator, creator->m_SpellHelper.GetUint64(eSpellHelpers::MirrorImage));
        if (l_IllusionTarget)
            creator = l_IllusionTarget;
    }
    ...
    if (creator->IsPlayer())
    {
        ...
        data << uint8(player->GetByteValue(PLAYER_FIELD_CUSTOM_DISPLAY_OPTION, PLAYER_CUSTOM_OPTOTIONS_FACIAL_STYLE));  ///< facialStyle
        data << uint8(player->GetByteValue(PLAYER_FIELD_HAIR_COLOR_ID, PLAYER_FIELD_HAIR_COLOR_ID_OFFSET_FACE_ID));                   ///< face
        data << uint8(player->GetByteValue(PLAYER_FIELD_HAIR_COLOR_ID, PLAYER_FIELD_HAIR_COLOR_ID_OFFSET_HAIR_STYLE_ID));             ///< hair
        data << uint8(player->GetByteValue(PLAYER_FIELD_HAIR_COLOR_ID, PLAYER_FIELD_HAIR_COLOR_ID_OFFSET_HAIR_COLOR_ID));             ///< haircolor
        data << uint8(player->GetByteValue(PLAYER_FIELD_HAIR_COLOR_ID, PLAYER_FIELD_HAIR_COLOR_ID_OFFSET_SKIN_ID));                   ///< skin

        for (int l_I = 0; l_I < PLAYER_CUSTOM_DISPLAY_SIZE; l_I++)
            data << uint8(player->GetByteValue(PLAYER_FIELD_CUSTOM_DISPLAY_OPTION, l_I));
        ...
        for (EquipmentSlots const* itr = itemSlots; *itr != EQUIPMENT_SLOT_END; ++itr)
        {
            if (*itr == EQUIPMENT_SLOT_HEAD && player->HasFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM))
                ...
                data << uint32(0);
            else if (Item* l_Item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, *itr))
                data << uint32(GetItemDisplayID(l_Item->GetVisibleEntry(player), l_Item->GetVisibleAppearanceModID(player)));
...
//////////////////////////////////////////////////////////////////////////
/// ToyBox
void WorldSession::HandleAddToyOpcode(WorldPacket& p_RecvData)
{
    uint64 l_ItemGuid = 0;
    ...
    if (!GetToyEntryFromItemId(l_Item->GetEntry()))
        return;
...
void WorldSession::HandleUseToyOpcode(WorldPacket& p_RecvPacket)
{
    std::string l_SrcTargetName;
    ...
    l_Spell->SetCastGUID(l_CastGUID);
