...
void WorldSession::HandleGuildUpdateMOTDTextOpcode(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_MotdText) > 128 || !sObjectMgr->CheckFakeColorProtection(l_MotdText, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void WorldSession::HandleGuildSetMemberNoteOpcode(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_Note) > 31 || !sObjectMgr->CheckFakeColorProtection(l_Note, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void WorldSession::HandleGuildAddRankOpcode(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_Name) > 15 || !sObjectMgr->CheckFakeColorProtection(l_Name, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void WorldSession::HandleGuildUpdateInfoTextOpcode(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_Info) > 500 || !sObjectMgr->CheckFakeColorProtection(l_Info, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void WorldSession::HandleGuildBankActivate(WorldPacket& p_Packet)
{
    ...
    if (Guild* l_Guild = _GetPlayerGuild(this))
    {
        m_Player->SetInteractionStatus(l_Banker, InteractionStatus::GuildBank);
...
void WorldSession::HandleGuildBankSwapItemsLegacy(WorldPacket& p_Packet)
{
    if (m_TimeLastMoveBankGuildItems > time(nullptr))
    {
        GetPlayer()->SendEquipError(EQUIP_ERR_INTERNAL_BAG_ERROR, NULL);
        return;
    }

    m_TimeLastMoveBankGuildItems = time(nullptr) + 1;
...
void WorldSession::HandleGuildBankMoveItemsPlayerBank(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsPlayerBank l_Data(p_RecvData);

    l_Data.Read();

    if (!m_Player->GetGameObjectIfCanInteractWith(l_Data.Banker, GameobjectTypes::GAMEOBJECT_TYPE_GUILD_BANK))
        return;

    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItemsWithInventory(m_Player, false, l_Data.BankTab, l_Data.BankSlot, l_Data.PlayerBag ? l_Data.PlayerBag : INVENTORY_SLOT_BAG_0, l_Data.PlayerSlot, 0);
}

void WorldSession::HandleGuildBankMoveItemsBankPlayer(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsBankPlayer l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItemsWithInventory(m_Player, true, l_Data.BankTab, l_Data.BankSlot, l_Data.PlayerBag ? l_Data.PlayerBag : INVENTORY_SLOT_BAG_0, l_Data.PlayerSlot, 0);
}

void WorldSession::HandleGuildBankMoveItemsBankBank(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsBankBank l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItems(m_Player, l_Data.BankTab, l_Data.BankSlot, l_Data.NewBankTab, l_Data.NewBankSlot, 0);
}

void WorldSession::HandleGuildBankMoveItemsPlayerBankCount(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsPlayerBankCount l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItemsWithInventory(m_Player, false, l_Data.BankTab, l_Data.BankSlot, l_Data.PlayerBag ? l_Data.PlayerBag : INVENTORY_SLOT_BAG_0, l_Data.PlayerSlot, l_Data.StackCount);
}

void WorldSession::HandleGuildBankMoveItemsBankPlayerCount(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsBankPlayerCount l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItemsWithInventory(m_Player, true, l_Data.BankTab, l_Data.BankSlot, l_Data.PlayerBag ? l_Data.PlayerBag : INVENTORY_SLOT_BAG_0, l_Data.PlayerSlot, l_Data.StackCount);
}

void WorldSession::HandleGuildBankMoveItemsBankPlayerAuto(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsAuto l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItemsWithInventory(m_Player, true, l_Data.BankTab, l_Data.BankSlot, InventorySlot::NULL_BAG, InventorySlot::NULL_SLOT, 0);
}

void WorldSession::HandleGuildBankMoveItemsBankBankCount(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMoveItemsBankBankCount l_Data(p_RecvData);
    ...
    if (Guild* l_Guild = m_Player->GetGuild())
        l_Guild->SwapItems(m_Player, l_Data.BankTab, l_Data.BankSlot, l_Data.NewBankTab, l_Data.NewBankSlot, l_Data.StackCount);
}

void WorldSession::HandleGuildBankMergeItemsPlayerBank(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMergeItemsPlayerBank l_Data(p_RecvData);
...
void WorldSession::HandleGuildBankMergeItemsBankPlayer(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMergeItemsBankPlayer l_Data(p_RecvData);
...
void WorldSession::HandleGuildBankMergeItemsBankBank(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankMergeItemsBankBank l_Data(p_RecvData);
...
void WorldSession::HandleGuildBankSwapItemsBankPlayer(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankSwapItemsBankPlayer l_Data(p_RecvData);
...
void WorldSession::HandleGuildBankSwapItemsBankBank(WorldPacket& p_RecvData)
{
    WorldPackets::Guild::GuildBankSwapItemsBankBank l_Data(p_RecvData);
...
void WorldSession::HandleGuildBankUpdateTab(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_Name) > 24 || !sObjectMgr->CheckFakeColorProtection(l_Name, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD) || utf8length(l_Icon) > 128)
        return;
...
void WorldSession::HandleSetGuildBankTabText(WorldPacket& p_Packet)
{
    ...
    if (utf8length(l_TabText) > 500 || !sObjectMgr->CheckFakeColorProtection(l_TabText, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_GUILD))
        return;
...
void WorldSession::HandleRequestGuildRewardsListOpcode(WorldPacket& p_Packet)
{
    ...
    {
        std::map<uint32, GuildReward> const& l_Rewards = sGuildMgr->GetGuildRewards();
        ...
        for (auto l_Iter : l_Rewards)
        {
            l_Data << uint32(l_Iter.second.Entry);                      ///< ItemID
            l_Data << uint32(0);                                        ///< Unk
            l_Data << uint32(l_Iter.second.AchievementId > 0 ? 1 : 0);  ///< AchievementsRequired
            l_Data << uint64(l_Iter.second.Racemask);                   ///< RaceMask
            l_Data << uint32(0);                                        ///< MinGuildLevel
            l_Data << uint32(l_Iter.second.Standing);                   ///< MinGuildRep
            l_Data << uint64(l_Iter.second.Price);                      ///< Cost

            if (l_Iter.second.AchievementId)
                l_Data << uint32(l_Iter.second.AchievementId);
        }
...
void WorldSession::HandleGuildSetAchievementTracking(WorldPacket& p_Packet)
{
    ...
    for (uint32 l_I = 0; l_I < l_Count; ++l_I)
    {
        uint32 l_Value;
        p_Packet >> l_Value;
        AchievementIDs.insert(l_Value);
    }

    if (Guild* l_Guild = GetPlayer()->GetGuild())
        l_Guild->HandleSetAchievementTracking(this, AchievementIDs);
}
