...
void WorldSession::HandleQuestgiverAcceptQuestOpcode(WorldPacket& recvData)
{
    ...
    if (l_Quest)
    {
        ...
        if (m_Player->CanAddQuest(l_Quest, true))
        {
            ...
            switch (object->GetTypeId())
            {
                ...
                case TYPEID_CONTAINER:
                {
                    ...
                    for (auto const& l_Objective : l_Quest->QuestObjectives)
                    {
                        ...
                        {
                            Item* l_Item = ((Item*)object);
                            ItemTemplate const* l_ItemTemplate = l_Item->GetTemplate();
                            if (l_Objective.ObjectID == (int32)l_Item->GetEntry())
                            {
                                destroyItem = false;
                                break;
                            }
                    ...
                    if (destroyItem)
                    {
                        if (!(((Item*)object)->GetTemplate()->Flags3 & (uint32)ItemFlagsEX2::DONT_DESTROY_ON_QUEST_ACCEPT))
                        {
                            l_IsItemDelete = true;
            ...
            /// If the questgiver still have a quest to give you or a quest you can complete, redirect to the main gossip instead close it
            if (!l_IsItemDelete)
            {
                if (Creature* l_QuestGiver = object->ToCreature())
                {
                    uint32 l_DialogStatus = getDialogStatus(m_Player, l_QuestGiver, 0);
                    if (l_DialogStatus != DIALOG_STATUS_UNAVAILABLE && l_DialogStatus != DIALOG_STATUS_INCOMPLETE)
                    {
                        if (!sScriptMgr->OnGossipHello(m_Player, l_QuestGiver))
                        {
                            m_Player->TalkedToCreature(l_QuestGiver->GetEntry(), l_QuestGiver->GetGUID());
                            m_Player->PrepareGossipMenu(l_QuestGiver, l_QuestGiver->GetCreatureTemplate()->GossipMenuId, true);
                            m_Player->SendPreparedGossip(l_QuestGiver);
                        }
                        l_QuestGiver->AI()->sGossipHello(m_Player);
                        return;
                    }
                }
            }
...
void WorldSession::HandleQuestQueryOpcode(WorldPacket& p_RecvData)
{
    if (!m_Player)
        return;
...
void WorldSession::HandleQuestgiverChooseRewardOpcode(WorldPacket& p_RecvData)
{
    ...
    {
        if (auto l_WorldQuestTemplate = MS::WorldQuest::Template::GetTemplate(l_Quest->GetQuestId()))
        {
            if (l_WorldQuestTemplate->GetWorldQuestType() == MS::WorldQuest::WorldQuestType::Bounty)
            {
                m_Player->GetWorldQuestManager()->Reward(l_Quest->GetQuestId(), l_LegacyRewardFound ? l_Slot : l_RewardEntry);
            }
            ...
            case TYPEID_PLAYER:
            {
                ...
                if (!l_CreatureQGiver || !(sScriptMgr->OnQuestReward(m_Player, l_CreatureQGiver, l_Quest, l_LegacyRewardFound ? l_Slot : l_RewardEntry)))
                {
                    ...
                    if (l_CreatureQGiver)
                    {
                        ...
                        /// If the questgiver still have a quest to give you or a quest you can complete, redirect to the main gossip instead close it
                        uint32 l_DialogStatus = getDialogStatus(m_Player, l_CreatureQGiver, 0);
                        if (l_DialogStatus != DIALOG_STATUS_UNAVAILABLE && l_DialogStatus != DIALOG_STATUS_INCOMPLETE)
                        {
                            if (!sScriptMgr->OnGossipHello(m_Player, l_CreatureQGiver))
                            {
                                m_Player->TalkedToCreature(l_CreatureQGiver->GetEntry(), l_CreatureQGiver->GetGUID());
                                m_Player->PrepareGossipMenu(l_CreatureQGiver, l_CreatureQGiver->GetCreatureTemplate()->GossipMenuId, true);
                                m_Player->SendPreparedGossip(l_CreatureQGiver);
                            }
                            l_CreatureQGiver->AI()->sGossipHello(m_Player);
                            return;
                        }
                    }
                }
...
void WorldSession::HandleQuestLogRemoveQuest(WorldPacket& recvData)
{
    ...
    if (slot < MAX_QUEST_LOG_SIZE)
    {
        if (uint32 questId = m_Player->GetQuestSlotQuestId(slot))
        {
            ...
            m_Player->RemoveCriteriaTimer(CRITERIA_TIMED_TYPE_QUEST, questId);
        }

        m_Player->SetQuestSlot(slot, 0);

        m_Player->UpdateCriteria(CRITERIA_TYPE_QUEST_ABANDONED, 1);
    }
}
...
uint32 WorldSession::getDialogStatus(Player* player, Object* questgiver, uint32 defstatus)
{
    ...
    for (QuestRelations::const_iterator i = qir.first; i != qir.second; ++i)
    {
        ...
        if (quest->IsLegendary() && status == QUEST_STATUS_COMPLETE && player->CanRewardQuest(quest, false))
            result2 = DIALOG_STATUS_LEGENDARY_REWARD;
            ...
            if (player->CanSeeStartQuest(quest))
            {
                if (player->SatisfyQuestLevel(quest, false))
                {
                    ...
                    else if (player->getLevel() <= ((player->GetQuestLevel(quest) == -1) ? player->getLevel() : player->GetQuestLevel(quest) + sWorld->getIntConfig(CONFIG_QUEST_LOW_LEVEL_HIDE_DIFF)))
                    {
                        ...
                        else if (quest->IsLegendary() && player->CanTakeQuest(quest, false))
                            result2 = DIALOG_STATUS_LEGENDARY_OFFER;
                        else
...
void WorldSession::HandleQuestgiverStatusMultipleQuery(WorldPacket& /*recvPacket*/)
{
    ...
    m_Player->m_clientGUIDs_lock.acquire_read();
...
void WorldSession::HandleRequestWorldQUestUpdate(WorldPacket& p_RecvData)
{
    GetPlayer()->SendWorldQuestUpdate();
}

void WorldSession::HandleQueryQuestRewards(WorldPacket& p_RecvData)
{
    uint32 l_QuestID;
    uint32 l_BountyQuestID;

    p_RecvData >> l_QuestID;
    p_RecvData >> l_BountyQuestID;

    auto l_Reward = GetPlayer()->GetWorldQuestManager()->GetReward(l_QuestID);
    if (!l_Reward)
        return;

    WorldPacket l_Data(SMSG_QUERY_QUEST_REWARD_RESPONSE, 200);
    l_Data << l_QuestID;
    l_Data << l_BountyQuestID;
    l_Data << uint32(l_Reward->m_ItemRewards.size());
    l_Data << uint32(l_Reward->m_CurrencyRewards.size());
    l_Data << uint64(l_Reward->m_Money);

    for (auto l_Currency : l_Reward->m_CurrencyRewards)
    {
        l_Data << uint32(l_Currency.m_ID);
        l_Data << uint32(l_Currency.m_Count);
    }

    for (auto l_Item : l_Reward->m_ItemRewards)
    {
        bool l_HaveBonuses = l_Item.m_ItemContext != 0 || l_Item.m_Bonuses.size() != 0;

        l_Data << uint32(l_Item.m_ID);
        l_Data << uint32(0);                ///< RandomPropSeed
        l_Data << uint32(0);                ///< RandomPropID
        l_Data.WriteBit(l_HaveBonuses);     ///< ItemBonus
        l_Data.WriteBit(false);             ///< Modifications

        /// Item Bonus Block
        if (l_HaveBonuses)
        {
            l_Data << uint8(l_Item.m_ItemContext);
            l_Data << uint32(l_Item.m_Bonuses.size());

            for (uint32 l_BonusID : l_Item.m_Bonuses)
        ...
        l_Data << uint32(l_Item.m_Count);   ///< Quantity
    }
