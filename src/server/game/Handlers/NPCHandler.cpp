...
{
    STABLE_ERR_MONEY        = 1,                        ///< "you don't have enough money"
    STABLE_ERR_INVALID_SLOT = 3,
    //STABLE_ERR_STABLE       = 6,                      ///< currently used in most fail cases
    STABLE_SUCCESS_STABLE   = 8,                        ///< stable success
    STABLE_SUCCESS_UNSTABLE = 9,                        ///< unstable/swap success
    STABLE_SUCCESS_BUY_SLOT = 10,                       ///< buy slot success
    STABLE_ERR_EXOTIC       = 11,                       ///< "you are unable to control exotic creatures"
    STABLE_ERR_STABLE       = 12                        ///< Internal pet error currently used in most fail cases
};

enum eCreatures
{
    ArchmageKhadgar         = 90417
};
...
void WorldSession::SendAdventureMapOpen(uint64 p_GUID)
{
    WorldPacket l_Data(SMSG_ADVENTURE_MAP_OPEN);
    l_Data.appendPackGUID(p_GUID);
    SendPacket(&l_Data);
}

void WorldSession::HandleAdventureMapPOIEnabledQuery(WorldPacket& p_RecvData)
{
    uint32 l_ScoutingMapPOI;
    p_RecvData >> l_ScoutingMapPOI;

    /// Adventure query feature opcode
    WorldPacket l_Data(SMSG_GARRISON_SCOUTING_MAP_RESULT);
    l_Data << uint32(l_ScoutingMapPOI);
    l_Data.WriteBit(m_Player->CanCompleteAdventurePOI(l_ScoutingMapPOI)); ///< Enabled
    l_Data.FlushBits();
    SendPacket(&l_Data);
}

void WorldSession::HandleAdventureMapStartQuest(WorldPacket& p_RecvData)
{
    uint32 l_QuestID;
    p_RecvData >> l_QuestID;

    for (uint32 l_ID = 0; l_ID < sAdventureMapPOIStore.GetNumRows(); ++l_ID)
    {
        AdventureMapPOIEntry const* l_Entry = sAdventureMapPOIStore.LookupEntry(l_ID);
        if (!l_Entry)
            continue;

        if (l_Entry->m_QuestID == l_QuestID)
        {
            if (m_Player->CanCompleteAdventurePOI(l_ID))
            {
                Quest const* l_QuestEntry = sObjectMgr->GetQuestTemplate(l_QuestID);
                m_Player->AddQuest(l_QuestEntry, nullptr);
            }
        }
    }
}
...
void WorldSession::HandleBankerActivateOpcode(WorldPacket& p_RecvData)
{
    ...
    m_Player->SetInteractionStatus(l_Guid, InteractionStatus::Bank);
    SendShowBank(l_Guid);
}
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& p_Packet)
{
    ...
    m_Player->ModifyMoney(-int64(nSpellCost), "HandleTrainerBuySpellOpcode");
...
void WorldSession::HandleGossipTalkOpcode(WorldPacket& recvData)
{
    uint64 guid;
    ...
    if (unit->GetEntry() != eCreatures::ArchmageKhadgar)
...
void WorldSession::SendSpiritResurrect()
{
    ...
    if (m_Player->HasCorpse())
    {
        corpseGrave = sObjectMgr->GetClosestGraveYard(corpseLocation.GetPositionX(), corpseLocation.GetPositionY(),
            corpseLocation.GetPositionZ(), corpseLocation.GetMapId(), m_Player->GetTeam());
    }
    ...
    if (corpseGrave)
    {
        ...
        if (corpseGrave != ghostGrave)
            m_Player->TeleportTo(corpseGrave->MapID, corpseGrave->x, corpseGrave->y, corpseGrave->z, m_Player->GetOrientation());
    ...
    if (p_QueryResult)
    {
        do
        {
            ...
            if (CreatureTemplate const* l_CreatureInfo = sObjectMgr->GetCreatureTemplate(l_CreatureID))
                l_DisplayID = l_CreatureInfo->Modelid[0] ? l_CreatureInfo->Modelid[0] : l_CreatureInfo->Modelid[1];

            if (m_Player->m_stablePetModels.find(l_DisplayID) == m_Player->m_stablePetModels.end())
                m_Player->m_stablePetModels[l_DisplayID] = l_CreatureID;
