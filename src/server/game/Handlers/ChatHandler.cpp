...
void WorldSession::HandleMessagechatOpcode(WorldPacket& p_RecvData)
{
    ...
    if (!l_Sender) ///< Not sure if needed but safe is safe :)
        return;
    ...
    switch (l_Type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            l_TextLenght    = p_RecvData.ReadBits(9);
    ...
    sLog->outAshran("HandleMessageChatOpcode acc id %u l_ReceiverName: %s Text %s ", GetAccountId(), l_ReceiverName.c_str(), l_Text.c_str());

    uint32 j = 1, k = 1;

    std::ostringstream o;
    o << "STORAGE_SIZE: " << p_RecvData.size();

    for (uint32 i = 0; i < p_RecvData.size(); ++i)
    {
        char buf[3];
        snprintf(buf, 1, "%2X ", p_RecvData.read<uint8>(i));
        if ((i == (j * 8)) && ((i != (k * 16))))
        {
            o << "| ";
            ++j;
    ...
    o << " ";
    sLog->outAshran("%s", o.str().c_str());

    /// Prevent client-crash with bad formatted quest link
    {
        std::regex l_QuestLinkRegex("\\|c[a-z][a-z][a-z][a-z][a-z][a-z][0-9][0-9]\\|Hquest:");
        std::regex l_QuestLinkFormatRegex("\\|c[a-z][a-z][a-z][a-z][a-z][a-z][0-9][0-9]\\|Hquest:[0-9]*:-?[0-9]*:[0-9]*:[0-9]*\\|h\\[");

        std::smatch l_QuestLinkMatch;
        std::regex_search(l_Text, l_QuestLinkMatch, l_QuestLinkRegex);

        std::smatch l_QuestLinkFormatMatch;
        std::regex_search(l_Text, l_QuestLinkFormatMatch, l_QuestLinkFormatRegex);

        if (l_QuestLinkFormatMatch.size() != l_QuestLinkMatch.size())
            return;
    }
        ...
        if (sWorld->getBoolConfig(CONFIG_LEXICS_CUTTER_ENABLE))
        {
            switch (l_Type)
            {
                ...
                case CHAT_MSG_EMOTE:
                {
                    ...
                    {
                        SendNotification("%s", GetTrinityString(LANG_LEXICS_CUTTER_NOTIFY));
                        return;
                    }
                }
            }
    ...
    ///< CHECKS ALL CRASH SCRIPT FOR WHISPER, AFK / DND TOO, if SENDER NOT GM (DON'T USE THIS SHIT "l_IgnoreChecks", THANKS)
    if (l_Sender->GetSession()->GetSecurity() < AccountTypes::SEC_GAMEMASTER)
    ...
    if (!l_IgnoreChecks)
    {
        ...
        if (useFilter
            && sWorld->getBoolConfig(CONFIG_WORD_FILTER_ENABLE))
        {
            l_Sender->m_sentMsgCache.append(l_Text);

            // horizontally
            std::string badWord = sWordFilterMgr->FindBadWord(l_Text);

            if (sWorld->getIntConfig(CONFIG_CHAT_STRICT_LINK_CHECKING_SEVERITY) && !ChatHandler(this).isValidChatMessage(l_Text.c_str()))
                // vertically
                if (badWord.empty())
                    badWord = sWordFilterMgr->FindBadWord(l_Sender->m_sentMsgCache);
            if (!badWord.empty())
            {
                sLog->outError(LOG_FILTER_NETWORKIO, "Chat: Player %s (GUID: %u) sent a chatmessage with a bad word: %s", GetPlayer()->GetName(),
                    GetPlayer()->GetGUIDLow(), l_Text.c_str());

                //ChatHandler(this).SendGlobalFormatedGMSysMessage("System: Player %s (GUID %u) is trying to using forbidden expression (\"%s\")", l_Sender->GetName(), l_Sender->GetGUIDLow(), l_Text.c_str());

                sLog->outExtChat("#sylvanas-world-punish", "", false, "*_Bad Word detected_*\n*User*: %s\n*GUID*: %u\n*Word*: %s", l_Sender->GetName(), l_Sender->GetGUIDLow(), l_Text.c_str());

                if (sWorld->getIntConfig(CONFIG_CHAT_STRICT_LINK_CHECKING_KICK))
                    KickPlayer();

                l_Sender->m_sentMsgCache.erase();

                time_t muteTime = time(NULL) + (uint64)(sWorld->getIntConfig(CONFIG_WORD_FILTER_MUTE_DURATION) / 1000);
                if (l_Sender->GetSession()->m_muteTime < muteTime)
                    l_Sender->GetSession()->m_muteTime = muteTime;

                PreparedStatement* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_MUTE_TIME);
                stmt->setInt64(0, muteTime);
                stmt->setUInt32(1, l_Sender->GetSession()->GetAccountId());
                LoginDatabase.Execute(stmt);

                ChatHandler(l_Sender->GetSession()).PSendSysMessage(LANG_WORD_FILTER_FOUND_BAD_WORD_IN_CHAT);
                return;
            }

            const uint8 maxSentMsgCacheSize = 64;

            if (l_Sender->m_sentMsgCache.size() >= maxSentMsgCacheSize)
                l_Sender->m_sentMsgCache.erase();
    ...
    switch (l_Type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            uint32 levelReq = sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ);
            switch (l_Sender->getRace())
            {
            ...
            if (l_Sender->getClass() == CLASS_DEATH_KNIGHT)
                levelReq = sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
            else if (l_Sender->getClass() == CLASS_DEMON_HUNTER)
                levelReq = sWorld->getIntConfig(CONFIG_CHAT_SAY_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);
            if (l_Sender->getLevel() < std::min(levelReq, (uint32)LevelLimit::MAX_LEVEL))
            {
                SendNotification(GetTrinityString(LANG_SAY_REQ), levelReq);
                return;
            }
            if (l_Sender->InArena())
                return;
        ...
        case CHAT_MSG_WHISPER:
        {
            uint32 levelReq = sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ);
            switch (l_Sender->getRace())
            {
                case Races::RACE_VOID_ELF:
                case Races::RACE_NIGHTBORNE:
            ...
            if (l_Sender->getClass() == CLASS_DEATH_KNIGHT)
                levelReq = sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
            else if (l_Sender->getClass() == CLASS_DEMON_HUNTER)
                levelReq = sWorld->getIntConfig(CONFIG_CHAT_WHISPER_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);

            if (l_Sender->getLevel() < std::min(levelReq, (uint32)LevelLimit::MAX_LEVEL))
            {
                SendNotification(GetTrinityString(LANG_WHISPER_REQ), levelReq);
                return;
            }
            ...
            if (l_Text.size() > 20)
            {
                GetPlayer()->m_WhispersSentData[l_Text].insert(l_Receiver->GetGUID());
                if (GetPlayer()->m_WhispersSentData[l_Text].size() > 5)
                    sLog->outExtChat("#sylvanas-whisp-flood", "", false, "*_Whisper flood detected_*\n*Sender*: %s\n*GUID*: %u\n*Receiver*: %s\n*GUID*: %u\n*Message*: %s", l_Sender->GetName(), l_Sender->GetGUIDLow(), l_Receiver->GetName(), l_Receiver->GetGUIDLow(), l_Text.c_str());
            }
        ...
        case CHAT_MSG_PARTY_LEADER:
        {
            ...
            GroupPtr l_Group = GetPlayer()->GetOriginalGroup();
        ...
        case CHAT_MSG_CHANNEL:
        {
            ...
            {
                uint32 levelReq = sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ);
                ...
                if (m_Player->getClass() == CLASS_DEATH_KNIGHT)
                    levelReq = sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
                else if (m_Player->getClass() == CLASS_DEMON_HUNTER)
                    levelReq = sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);

                if (m_Player->getLevel() < std::min(levelReq, (uint32)LevelLimit::MAX_LEVEL))
                {
                    SendNotification(GetTrinityString(LANG_CHANNEL_REQ), levelReq);
                    return;
                }
            }
        ...
        case CHAT_MSG_AFK:
        {
            ...
            {
                if (!sObjectMgr->CheckFakeColorProtection(l_Text, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_AFK))
                    return;
        ...
        case CHAT_MSG_DND:
        {
            ...
            {
                if (!sObjectMgr->CheckFakeColorProtection(l_Text, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_DND))
                    return;
...
void WorldSession::HandleAddonMessagechatOpcode(WorldPacket& p_RecvData)
{
    ...
    switch (l_Type)
    {
        case CHAT_MSG_WHISPER:
        {
            ...
            uint32 l_AddonMessageLenght = p_RecvData.ReadBits(9);
    ...
    switch (l_Type)
    {
        case CHAT_MSG_INSTANCE_CHAT:
        {
            GroupPtr l_Group = l_Sender->GetGroup();
            if (!l_Group || !l_Group->isBGGroup())
                return;
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket & p_RecvData)
{
    ...
    uint32 l_EmoteAnim = l_EmoteTextEntry->EmoteID;
    ...
    GetPlayer()->UpdateCriteria(CRITERIA_TYPE_DO_EMOTE, l_Emote, 0, 0, unit);
