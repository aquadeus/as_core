...
void WorldSession::HandleBattlePayStartPurchase(WorldPacket& p_RecvData)
{
    ...
    WebDatabase.AsyncQuery(l_Statement, m_Player->GetQueryCallbackMgr(), [l_AccountId, l_Product](PreparedQueryResult p_Result) -> void
    {
...
void WorldSession::HandleBattlePayConfirmPurchase(WorldPacket& p_RecvData)
{
    ...
    WebDatabase.AsyncQuery(l_Statement, m_Player->GetQueryCallbackMgr(), [l_AccountId, l_Purchase](PreparedQueryResult p_Result) -> void
    {
        /// Never buy points
        if (!p_Result)
            return;
