...
void WorldSession::HandleVoidStorageUnlock(WorldPacket & p_Packet)
{
    ...
    m_Player->ModifyMoney(-int64(VOID_STORAGE_UNLOCK_COST), "HandleVoidStorageUnlock");
...
void WorldSession::HandleVoidStorageTransfer(WorldPacket & p_Packet)
{
    ...
    if (!m_Player->HasEnoughMoney(uint64(l_Deposits.size() * VOID_STORAGE_STORE_ITEM_COST)))
    {
    ...
    for (std::vector<uint64>::iterator l_It = l_Deposits.begin(); l_It != l_Deposits.end(); ++l_It)
    {
        ...
        VoidStorageItem itemVS(sObjectMgr->GenerateVoidStorageItemId(), l_Item->GetEntry(), l_Item->GetGuidValue(ITEM_FIELD_CREATOR), l_Item->GetItemRandomPropertyId(), l_Item->GetItemSuffixFactor(), l_Item->GetAllItemBonuses(), l_Item->GetCustomFlags(), true);
    ...
    int64 l_Cost = l_DepositCountSecond * VOID_STORAGE_STORE_ITEM_COST;

    m_Player->ModifyMoney(-l_Cost, "HandleVoidStorageTransfer");
    ...
    for (std::vector<uint64>::iterator l_It = l_Withdrawals.begin(); l_It != l_Withdrawals.end(); ++l_It)
    {
        ...
        std::vector<uint32> l_Bonuses = l_VoidStorageItem->Bonuses;
        Item * l_Item = m_Player->StoreNewItem(l_Destination, l_VoidStorageItem->ItemEntry, true, l_VoidStorageItem->ItemRandomPropertyId, l_Bonuses);

        l_Item->SetGuidValue(ITEM_FIELD_CREATOR, MAKE_NEW_GUID(l_VoidStorageItem->CreatorGuid, 0, HIGHGUID_PLAYER));
        l_Item->SetCustomFlags(l_VoidStorageItem->CustomFlags);
