...
void WorldSession::SendTaxiMenu(Creature* p_Unit)
{
    ...
    TaxiNodesEntry const* l_TaxiNode = sTaxiNodesStore.LookupEntry(l_CurrentLocation);
    ...
    l_Data.FlushBits();
    l_Data << uint32(TaxiMaskSize);     ///< CanLandNodes
    l_Data << uint32(TaxiMaskSize);     ///< CanUseNodes
    ...
    if (l_TaxiNode->Flags & TaxiNodeFlags::TAXI_NODE_FLAG_ARGUS)
    {
        GetPlayer()->m_taxi.AppendArgusTaxiMaskTo(l_Data, GetPlayer()->isTaxiCheater(), false);     ///< CanLandNodes
        GetPlayer()->m_taxi.AppendArgusTaxiMaskTo(l_Data, GetPlayer()->isTaxiCheater(), true);     ///< CanUseNodes
    }
    else
    {
        GetPlayer()->m_taxi.AppendTaximaskTo(l_Data, GetPlayer()->isTaxiCheater());     ///< CanLandNodes
        GetPlayer()->m_taxi.AppendTaximaskTo(l_Data, GetPlayer()->isTaxiCheater());     ///< CanUseNodes
    }
...
void WorldSession::HandleActivateTaxiOpcode(WorldPacket& p_RecvPacket)
{
    ...
    uint32 l_GroundMountID = 0;
    uint32 l_FlyingMountID = 0;

    p_RecvPacket.readPackGUID(l_VendorGuid);
    p_RecvPacket >> l_PacketNode;
    p_RecvPacket >> l_GroundMountID;
    p_RecvPacket >> l_FlyingMountID;
    ...
    uint32 l_PreferredMountDisplay = 0;
    if (MountEntry const* p_Mount = sMountStore.LookupEntry(l_FlyingMountID))
    {
        if (GetPlayer()->HasSpell(p_Mount->SpellID))
        {
            for (uint32 l_Index = 0; l_Index < sMountXDisplayStore.GetNumRows(); ++l_Index)
            {
                MountXDisplayEntry const* p_MountDisplay = sMountXDisplayStore.LookupEntry(l_Index);
                if (p_MountDisplay != nullptr && p_MountDisplay->MountID == l_FlyingMountID)
                {
                    l_PreferredMountDisplay = p_MountDisplay->DisplayID;
                    break;
                }
            }
        }
    ...
    GetPlayer()->ActivateTaxiPathTo(nodes, l_Npc, 0, l_PreferredMountDisplay);
}
...
void WorldSession::HandleMoveSplineDoneOpcode(WorldPacket& p_RecvPacket)
{
    ...
    if (curDest)
    {
        ...
        for (uint32 l_I = 0; l_I < sGarrSiteLevelStore.GetNumRows(); ++l_I)
        {
            ...
            for (uint32 v : { 1153, 1152, 1159, 1158, 1331, 1330, 1464 })
            {
                if (l_Entry && l_Entry->SiteID != 0 && l_Entry->MapID == l_PlayerMapID && l_PlayerMapID == v)
                {
                    l_PlayerMapID = MS::Garrison::GDraenor::Globals::BaseMap;
                    break;
                }
            }
        }
    ...
    {
        sScriptMgr->OnMoveSplineDone(GetPlayer());
        sAnticheatMgr->HandleSplineDone(GetPlayer()->GetGUIDLow(), GetPlayer()->GetSession()->GetLatency());
        return;
    }
}
