...
void WorldSession::SendBattleGroundList(uint64 guid, BattlegroundTypeId bgTypeId)
{
    ...
    auto& l_Lock = sBattlegroundMgr->GetLock();
    std::lock_guard<std::recursive_mutex> l_Guard(l_Lock);

    WorldPacket data;
    MS::Battlegrounds::PacketFactory::List(&data, guid, m_Player, bgTypeId, 0, m_Player->GetDayWins((uint8)BattlegroundBracketType::RandomBattleground));
    SendPacket(&data);
}
    ...
    else
    {
        ...
        l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr) -> void
        {
            ...
            if (!l_InterRealmEnable)
            {
                ...
                WorldPacket l_Data; // send status packet (in queue)
                MS::Battlegrounds::PacketFactory::Status(&l_Data, l_BG, l_Member, l_QueueSlot, STATUS_WAIT_QUEUE, avgTime, ginfo->m_JoinTime, ginfo->m_ArenaType, false, false, true);
...
void WorldSession::HandleBattlefieldListOpcode(WorldPacket& p_Packet)
{
    ...
    {
        WorldPacket l_Data;
        MS::Battlegrounds::PacketFactory::List(&l_Data, 0, m_Player, BattlegroundTypeId(l_ListID), l_ListID, m_Player->GetDayWins((uint8)BattlegroundBracketType::RandomBattleground));
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket& p_Packet)
{
...
#ifndef CROSS
    bool l_InterRealmEnable = sWorld->getBoolConfig(CONFIG_INTERREALM_ENABLE) && !m_Player->IsWargameInvited();
    InterRealmSession* l_Tunnel = nullptr;
    ...
    uint32 l_Time = 0;
    uint32 l_Type = 0;                                  ///< type id from db2
    ...
    if (l_QueueSlotID > PLAYER_MAX_BATTLEGROUND_QUEUES)
    {
        ...
        printf("HandleBattleFieldPortOpcode queueSlot %u\r\n", l_QueueSlotID);
        return;
    }
    ...
    if (l_InterRealmEnable)
    {
        ...
        {
            printf("HandleBattleFieldPortOpcode: Accept invite (cross)\r\n");
            if (m_Player->GetInterRealmPlayerState() > InterRealmPlayerState::None)
            {
                ...
                printf("HandleBattleFieldPortOpcode: Try to transfer player to cross, but he is already in transfer\r\n");
                return;
            }

            m_Player->RemoveAurasByType(SPELL_AURA_MOUNTED);
            m_Player->ResetLeaveQueueCounter();
            ...
            GetPlayer()->SaveToDB(false, false, std::make_shared<MS::Utilities::Callback>([l_AccountID](bool p_Success) -> void
            {
                printf("HandleBattleFieldPortOpcode: Callback Save to db (cross)\r\n");
                ...
                if (InterRealmSession* ir_session = sWorld->GetInterRealmSession())
                {
                    ...
                    printf("HandleBattleFieldPortOpcode: Send IR_CMSG_BATTLEFIELD_PORT\r\n");
    ...
    switch (l_AcceptedInvite)
    {
        ...
        case 1:
            ...
            /// Set if the player queues for a random or a specific bg
            m_Player->SetInRandomBG(l_GroupQueueInfo.m_IsRandom);
        ...
        case 0:
            ...
            if (l_BG->GetStatus() == BattlegroundStatus::STATUS_WAIT_JOIN)
            {
                m_Player->HandleDeserterAfterLeaveQueue();
                if (l_BG->IsRatedBG() || (!l_BG->IsSkirmish() && l_BG->GetArenaType() == ArenaType::Arena2v2) || (!l_BG->IsSkirmish() && l_BG->GetArenaType() == ArenaType::Arena3v3) || (!l_BG->IsSkirmish() && l_BG->GetArenaType() == ArenaType::Arena1v1))
                {
                    uint32 l_PersonalRating = m_Player->GetArenaPersonalRating(static_cast<uint8>(l_BG->GetBattlegroundBracketType()));
                    if (l_PersonalRating <= 1500)
                    {
                        uint32 l_TeamA = l_BG->GetArenaMatchmakerRating(Team::ALLIANCE, static_cast<uint8>(l_BG->GetBattlegroundBracketType()));
                        uint32 l_TeamB = l_BG->GetArenaMatchmakerRating(Team::HORDE, static_cast<uint8>(l_BG->GetBattlegroundBracketType()));

                        ///Team A's Chance of Winning: 1 / (1+10(1580 - 1500)/400) = 0.38686
                        ///Team B's Chance of Winning: 1 / (1+10(1500 - 1580)/400) = 0.61314
                        uint32 TeamAChances = 1 / (1 + 10 * (l_TeamB - l_TeamA) / 400);
                        uint32 TeamBChances = 1 / (1 + 10 * (l_TeamA - l_TeamB) / 400);
                        ...
                        l_PersonalRating = m_Player->GetTeam() == ALLIANCE ? l_PersonalRating + -(l_PersonalRating + 24 * (1 - TeamAChances)) :
                            l_PersonalRating + -(l_PersonalRating + 24 * (1 - TeamBChances));
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket& p_Packet)
{
    ...
    /// If there is 2 healer in a 3 players group, the group can't join 3v3 Ranked Arena
    if (l_ArenaType == ArenaType::Arena3v3 && l_Group->GetMembersCount() == 3)
    {
        uint8 l_NumberOfHealer = 0;
        l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr) -> void
        {
            if (Player const* l_GroupMember = l_Itr->player)
            {
                if (l_GroupMember->GetRoleForGroup() == ROLE_HEALER)
                {
                    l_NumberOfHealer++;
                }
            }
        ...
        if (l_NumberOfHealer > 1)
        {
            ChatHandler(this).PSendSysMessage(LANG_ARENA_2_HEALERS_3v3);
            return;
        }
    }
    ...
    l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr) -> void
    {
        if (const Player * l_GroupMember = l_Itr->player)
        {
            l_ArenaRating       += l_GroupMember->GetArenaPersonalRating((uint8)GetBracketTypeByArenaType(l_ArenaType));
            l_MatchmakerRating  += l_GroupMember->GetArenaMatchMakerRating((uint8)GetBracketTypeByArenaType(l_ArenaType));
    ...
    l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr) -> void
    {
        ...
        if (!l_InterRealmEnable)
        {
            ...
            WorldPacket l_Data; // send status packet (in queue)
            MS::Battlegrounds::PacketFactory::Status(&l_Data, l_Battleground, l_Member, l_QueueSlot, STATUS_WAIT_QUEUE, l_AverageTime, l_GroupQueueInfo->m_JoinTime, l_ArenaType, false, false, true);
...
void WorldSession::HandleBattlemasterJoinArenaSkirmish(WorldPacket& p_Packet)
{
    ...
    ArenaType l_ArenaType = ArenaType::Arena2v2;

    /// Solo healer can't join 2v2 skirmish
    if (!l_JoinAsGroup && m_Player->GetRoleForGroup() == ROLE_HEALER)
        l_ArenaType = ArenaType::Arena3v3;
    else if (l_JoinAsGroup && l_Group)
    {
        /// group of 3 players join 3v3 skirmish (obviously)
        if (l_Group->GetMembersCount() == 3)
            l_ArenaType = ArenaType::Arena3v3;
        else
        {
            /// If there is a healer in a 2 players group, the group can't join 2v2 skrimish
    ...
    if (l_JoinAsGroup)
    {
        ...
        if (!l_GroupQueueInfo && !l_InterRealmEnable)
            return;
        ...
        l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
        {
            ...
            WorldPacket l_Data; // send status packet (in queue)
            MS::Battlegrounds::PacketFactory::Status(&l_Data, l_Battleground, l_Member, l_QueueSlot, STATUS_WAIT_QUEUE, l_AverageTime, l_GroupQueueInfo->m_JoinTime, l_ArenaType, true, false, true);
            l_Member->GetSession()->SendPacket(&l_Data);
        });
    }
    else
...
void WorldSession::HandleBattlemasterJoinRated(WorldPacket & /*p_Packet*/)
{
    ...
    l_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
    {
        ...
        {
            l_PersonalRating   += groupMember->GetArenaPersonalRating((uint8)BattlegroundBracketType::Battleground10v10);
            l_MatchmakerRating += groupMember->GetArenaMatchMakerRating((uint8)BattlegroundBracketType::Battleground10v10);
...
void WorldSession::HandleStartWarGame(WorldPacket& p_Packet)
{
    ...
    GroupPtr l_OpposingPartyGroup = l_OpposingPartyLeader->GetGroup();

    if (l_OpposingPartyGroup == nullptr)
        return;

    if (l_OpposingPartyGroup->GetLeaderGUID() != l_OpposingPartyLeader->GetGUID())
        return;
...
void WorldSession::HandleAcceptWarGameInvite(WorldPacket& p_Packet)
{
    ...
    GroupPtr l_OpposingGroup = l_OpposingPartyLeader->GetGroup();
    ...
    if (l_BGQueueTypeID == MS::Battlegrounds::BattlegroundType::AllArenas)
        l_BGQueueTypeID = static_cast<MS::Battlegrounds::BattlegroundType::Type>(urand(MS::Battlegrounds::BattlegroundType::TigersPeaks, MS::Battlegrounds::BattlegroundType::NagrandArena));

    if (l_BGQueueTypeID == MS::Battlegrounds::BattlegroundType::RandomBattleground)
        l_BGQueueTypeID = static_cast<MS::Battlegrounds::BattlegroundType::Type>(urand(MS::Battlegrounds::BattlegroundType::AlteracValley, MS::Battlegrounds::BattlegroundType::BattleForGilneas));
    ...
    auto l_PrepareGroupToWargame = [this, &l_Scheduler, &l_InviationMgr, l_BGQueueTypeID, l_ArenaType, l_BattlegroundTemplate, l_BattlegroundInstance, l_BracketEntry](GroupPtr p_Group, uint32 p_Team) -> void
    {
        ...
        p_Group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
        {
            Player* l_Member = l_Itr->player;
            ...
            WorldPacket l_Data; // send status packet (in queue)
            MS::Battlegrounds::PacketFactory::Status(&l_Data, l_BattlegroundInstance, l_Member, l_QueueSlot, STATUS_WAIT_QUEUE, l_AvgTime, l_GroupQueueInfo->m_JoinTime, l_GroupQueueInfo->m_ArenaType, false, false, true);
            l_Member->GetSession()->SendPacket(&l_Data);
        });
