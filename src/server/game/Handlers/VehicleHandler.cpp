    ...
    if (Vehicle* l_Vehicle = GetPlayer()->GetVehicle())
    {
        if (VehicleSeatEntry const* l_VehicleSeat = l_Vehicle->GetSeatForPassenger(GetPlayer()))
        {
            if (l_VehicleSeat->CanEnterOrExit())
                ...
                sLog->outError(LOG_FILTER_NETWORKIO, "Player %u tried to exit vehicle, but seatflags %u (ID: %u) don't permit that.", GetPlayer()->GetGUIDLow(), l_VehicleSeat->ID, l_VehicleSeat->m_flags);
        }
    }
}
