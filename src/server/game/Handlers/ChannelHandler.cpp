...
std::vector<uint32> GetTeams(WorldSession* p_Sesion)
{
    std::vector<uint32> l_Teams;
    l_Teams.push_back(p_Sesion->GetPlayer()->GetTeam());
    if (p_Sesion->GetSecurity())
        l_Teams.push_back(p_Sesion->GetPlayer()->GetTeam() == Team::HORDE ? Team::ALLIANCE : Team::HORDE);
...
void WorldSession::HandleJoinChannel(WorldPacket& recvPacket)
{
    ...
    std::string l_ChannelName = channelName;
    std::transform(l_ChannelName.begin(), l_ChannelName.end(), l_ChannelName.begin(), ::tolower);

    if (l_ChannelName == "gm" && GetSecurity() == 0)
        return;
    ...
    if (channelId)
    {
        ...
        AreaTableEntry const* current_zone = sAreaTableStore.LookupEntry(m_Player->GetZoneId());
        if (!current_zone)
            return;
    ...
    if (!sObjectMgr->CheckFakeColorProtection(channelName, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CHANNEL))
        return;

    for (uint32 l_Team : GetTeams(this))
    {
        if (ChannelMgr* cMgr = channelMgr(l_Team))
        {
            cMgr->team = l_Team;
...
void WorldSession::HandleLeaveChannel(WorldPacket& recvPacket)
{
    ...
    for (uint32 l_Team : GetTeams(this))
    {
        if (ChannelMgr* l_ChannelMgr = channelMgr(l_Team))
        {
...
void WorldSession::HandleChannelList(WorldPacket& p_Packet)
{
    ...
    if (!sObjectMgr->CheckFakeColorProtection(l_Channelname, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CHANNEL))
        return;
