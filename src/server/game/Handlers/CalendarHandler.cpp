...
void WorldSession::HandleCalendarGetCalendar(WorldPacket& /*p_RecvData*/)
{
    ...
    l_Data << uint32(MS::Utilities::WowTime::Encode(time(nullptr)));
    ...
    {
        m_Player->m_boundInstances[l_Iter].safe_foreach(true, [&](Player::BoundInstancesMap::iterator* p_Itr) -> void
        {
...
void WorldSession::HandleCalendarAddEvent(WorldPacket& p_RecvData)
{
    ...
    if (!sObjectMgr->CheckFakeColorProtection(l_Title, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CALENDAR) || !sObjectMgr->CheckFakeColorProtection(l_Desc, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CALENDAR))
        return;
...
void WorldSession::HandleCalendarUpdateEvent(WorldPacket& p_RecvData)
{
    ...
    if (!sObjectMgr->CheckFakeColorProtection(l_Title, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CALENDAR) || !sObjectMgr->CheckFakeColorProtection(l_Description, ColorFakeProtectionTypes::COLOR_FAKE_PROTECTION_CALENDAR))
        return;
...
void WorldSession::HandleCalendarEventInvite(WorldPacket& p_RecvData)
{
    ...
    else
    {
        ...
        l_Statement->setString(1, l_Name);
...
void WorldSession::HandleSetSavedInstanceExtend(WorldPacket& p_RecvData)
{
    ...
    uint32 l_DifficultyID = p_RecvData.read<uint32>();

    bool l_Extend = p_RecvData.ReadBit();

    InstancePlayerBindPtr l_InstanceBind = m_Player->GetBoundInstance(l_MapID, Difficulty(l_DifficultyID), l_Extend);
    if (!l_InstanceBind || !l_InstanceBind->save || !l_InstanceBind->perm)
        return;

    BindExtensionState l_NewState;
    if (!l_Extend || l_InstanceBind->extendState == BindExtensionState::EXTEND_STATE_EXPIRED)
        l_NewState = BindExtensionState::EXTEND_STATE_NORMAL;
    else
        l_NewState = BindExtensionState::EXTEND_STATE_EXTENDED;

    m_Player->BindToInstance(l_InstanceBind->save, true, l_NewState, false);
}

// ----------------------------------- SEND ------------------------------------
void WorldSession::SendCalendarRaidLockout(InstanceSavePtr save, bool add)
{
    uint32 currTime = time(nullptr);

    WorldPacket data(SMSG_CALENDAR_RAID_LOCKOUT_REMOVED, add ? 8 + 4 + 4 + 4 + 4 : 4 + 4);
    if (add)
    {
        data.SetOpcode(SMSG_CALENDAR_RAID_LOCKOUT_ADDED);
        data << uint64(save->GetInstanceId());
        data << uint32(MS::Utilities::WowTime::Encode(currTime));
    }
...
void WorldSession::SendCalendarRaidLockoutUpdated(InstanceSavePtr save)
{
}
#endif
