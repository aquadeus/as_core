...
SpellScript::TakePowersHandler::TakePowersHandler(SpellTakePowersFnType takePowersHandlerScript)
{
    _takePowersHandlerScript = takePowersHandlerScript;
}

void SpellScript::TakePowersHandler::Call(SpellScript* spellScript, Powers p_PowerType, int32& p_PowerCost)
{
    return (spellScript->*_takePowersHandlerScript)(p_PowerType, p_PowerCost);
}

SpellScript::CalculateThreatHandler::CalculateThreatHandler(SpellCalculateThreatFnType calculateThreatHandlerScript)
{
    _calculateThreatHandlerScript = calculateThreatHandlerScript;
}

void SpellScript::CalculateThreatHandler::Call(SpellScript* spellScript, Unit* p_Target, float& p_Threat)
{
    return (spellScript->*_calculateThreatHandlerScript)(p_Target, p_Threat);
}

SpellScript::InterruptHandler::InterruptHandler(SpellInterruptFnType interruptHandlerScript)
{
    _interruptHandlerScript = interruptHandlerScript;
}

void SpellScript::InterruptHandler::Call(SpellScript* spellScript, uint32 p_Time)
{
    return (spellScript->*_interruptHandlerScript)(p_Time);
}
...
bool SpellScript::_Validate(SpellInfo const* entry)
{
    for (std::list<EffectHandler>::iterator itr = OnEffectLaunch.begin(); itr != OnEffectLaunch.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectLaunch` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectHandler>::iterator itr = OnEffectLaunchTarget.begin(); itr != OnEffectLaunchTarget.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectLaunchTarget` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectHandler>::iterator itr = OnEffectHit.begin(); itr != OnEffectHit.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectHit` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectHandler>::iterator itr = OnEffectHitTarget.begin(); itr != OnEffectHitTarget.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectHitTarget` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
    ...
    for (std::list<ObjectAreaTargetSelectHandler>::iterator itr = OnObjectAreaTargetSelect.begin(); itr != OnObjectAreaTargetSelect.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnObjectAreaTargetSelect` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<ObjectTargetSelectHandler>::iterator itr = OnObjectTargetSelect.begin(); itr != OnObjectTargetSelect.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnObjectTargetSelect` of SpellScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
...
bool SpellScript::IsInTakePowersHook() const
{
    return m_currentScriptState == SPELL_SCRIPT_HOOK_TAKE_POWERS;
}

bool SpellScript::IsInCalculateThreatHook() const
{
    return m_currentScriptState == SPELL_SCRIPT_HOOK_CALCULATE_THREAT;
}

bool SpellScript::IsInInterruptHook() const
{
    return m_currentScriptState == SPELL_SCRIPT_HOOK_INTERRUPT;
}
...
int32 SpellScript::GetHitDamageForLeechEffect()
{
    if (!IsInTargetHook())
    {
        sLog->outError(LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::GetHitDamageForLeechEffect was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }
    if (!m_spell->GetSpellInfo()->HasEffect(SPELL_EFFECT_HEALTH_LEECH))
        return 0;

    return m_spell->damage;
}

int32 SpellScript::GetBaseHitDamage()
{
    if (!IsInTargetHook())
    {
        sLog->outError(LogFilterType::LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::GetBaseHitDamage was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return 0;
    }

    return m_spell->m_BaseDamage;
}
...
void SpellScript::SetHitDamageForLeechEffect(int32 p_Damage)
{
    if (!IsInTargetHook())
    {
        sLog->outError(LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::SetHitDamageForLeechEffect was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
        return;
    }
    if (!m_spell->GetSpellInfo()->HasEffect(SPELL_EFFECT_HEALTH_LEECH))
        return;

    m_spell->damage = p_Damage;
}
...
bool AuraScript::_Validate(SpellInfo const* entry)
{
    ...
    for (std::list<AuraDispelHandler>::iterator itr = OnDispel.begin(); itr != OnDispel.end();  ++itr)
        if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_2) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET) && !entry->HasAreaAuraEffect())
    ...
    for (std::list<EffectApplyHandler>::iterator itr = OnEffectApply.begin(); itr != OnEffectApply.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectApply` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectApplyHandler>::iterator itr = OnEffectRemove.begin(); itr != OnEffectRemove.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectRemove` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectApplyHandler>::iterator itr = AfterEffectApply.begin(); itr != AfterEffectApply.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectApply` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectApplyHandler>::iterator itr = AfterEffectRemove.begin(); itr != AfterEffectRemove.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectRemove` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectPeriodicHandler>::iterator itr = OnEffectPeriodic.begin(); itr != OnEffectPeriodic.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectPeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectPeriodicHandler>::iterator itr = AfterEffectPeriodic.begin(); itr != AfterEffectPeriodic.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectPeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectUpdatePeriodicHandler>::iterator itr = OnEffectUpdatePeriodic.begin(); itr != OnEffectUpdatePeriodic.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectUpdatePeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectCalcAmountHandler>::iterator itr = DoEffectCalcAmount.begin(); itr != DoEffectCalcAmount.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `DoEffectCalcAmount` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectCalcPeriodicHandler>::iterator itr = DoEffectCalcPeriodic.begin(); itr != DoEffectCalcPeriodic.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `DoEffectCalcPeriodic` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectCalcSpellModHandler>::iterator itr = DoEffectCalcSpellMod.begin(); itr != DoEffectCalcSpellMod.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `DoEffectCalcSpellMod` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectAbsorbHandler>::iterator itr = OnEffectAbsorb.begin(); itr != OnEffectAbsorb.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectAbsorb` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectAbsorbHandler>::iterator itr = AfterEffectAbsorb.begin(); itr != AfterEffectAbsorb.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectAbsorb` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectManaShieldHandler>::iterator itr = OnEffectManaShield.begin(); itr != OnEffectManaShield.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectManaShield` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectManaShieldHandler>::iterator itr = AfterEffectManaShield.begin(); itr != AfterEffectManaShield.end();  ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectManaShield` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectSplitDamageHandler>::iterator itr = OnEffectSplitDamage.begin(); itr != OnEffectSplitDamage.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectSplitDamage` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
    ...
    for (auto l_Itr = DoCheckEffectProc.begin(); l_Itr != DoCheckEffectProc.end(); ++l_Itr)
        if (!(*l_Itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `DoCheckEffectProc` of AuraScript won't be executed", entry->Id, (*l_Itr).ToString().c_str(), m_scriptName->c_str());

    for (auto l_Itr = DoCalcPPM.begin(); l_Itr != DoCalcPPM.end(); ++l_Itr)
        if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_2) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET) && !entry->HasAreaAuraEffect())
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` of script `%s` does not have apply aura effect - handler bound to hook `DoCalcPPM` of AuraScript won't be executed", entry->Id, m_scriptName->c_str());

    for (auto l_Itr = DoCalcProcChance.begin(); l_Itr != DoCalcProcChance.end(); ++l_Itr)
        if (!entry->HasEffect(SPELL_EFFECT_APPLY_AURA) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_2) && !entry->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET) && !entry->HasAreaAuraEffect())
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` of script `%s` does not have apply aura effect - handler bound to hook `DoCalcProcChance` of AuraScript won't be executed", entry->Id, m_scriptName->c_str());
    ...
    for (std::list<EffectProcHandler>::iterator itr = OnEffectProc.begin(); itr != OnEffectProc.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `OnEffectProc` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());

    for (std::list<EffectProcHandler>::iterator itr = AfterEffectProc.begin(); itr != AfterEffectProc.end(); ++itr)
        if (!(*itr).GetAffectedEffectsMask(entry))
            sLog->outError(LOG_FILTER_TSCR, "Spell `%u` Effect `%s` of script `%s` did not match db2 effect data - handler bound to hook `AfterEffectProc` of AuraScript won't be executed", entry->Id, (*itr).ToString().c_str(), m_scriptName->c_str());
...
AuraScript::CalcMaxDurationHandler::CalcMaxDurationHandler(AuraCalcMaxDurationFnType p_HandlerScript)
{
    m_HandlerScript = p_HandlerScript;
}

void AuraScript::CalcMaxDurationHandler::Call(AuraScript* p_AuraScript, int32& p_MaxDuration)
{
    (p_AuraScript->*m_HandlerScript)(p_MaxDuration);
}
...
AuraScript::EffectSplitDamageHandler::EffectSplitDamageHandler(AuraEffectSplitDamageFnType _pEffectHandlerScript, uint8 _effIndex)
    : AuraScript::EffectBase(_effIndex, SPELL_AURA_SPLIT_DAMAGE_PCT)
{
    pEffectHandlerScript = _pEffectHandlerScript;
}

void AuraScript::EffectSplitDamageHandler::Call(AuraScript* auraScript, AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& splitAmount)
{
...
bool AuraScript::CheckEffectProcHandler::Call(AuraScript* p_AuraScript, AuraEffect const* p_AurEff, ProcEventInfo& p_EventInfo)
{
    return (p_AuraScript->*_HandlerScript)(p_AurEff, p_EventInfo);
}

AuraScript::AuraProcPerMinuteHandler::AuraProcPerMinuteHandler(AuraProcPerMinuteFnType p_HandlerScript)
{
    _HandlerScript = p_HandlerScript;
}

void AuraScript::AuraProcPerMinuteHandler::Call(AuraScript* p_AuraScript, float & p_ProcPerMinute, ProcEventInfo& p_EventInfo)
{
    (p_AuraScript->*_HandlerScript)(p_ProcPerMinute, p_EventInfo);
}

AuraScript::AuraProcChanceHandler::AuraProcChanceHandler(AuraProcChanceFnType p_HandleScript)
{
    _HandlerScript = p_HandleScript;
}

void AuraScript::AuraProcChanceHandler::Call(AuraScript* p_AuraScript, float & p_Chance, ProcEventInfo & p_EventInfo)
{
    (p_AuraScript->*_HandlerScript)(p_Chance, p_EventInfo);
}
...
bool AuraScript::CanRefreshProcHandler::Call(AuraScript* auraScript, bool p_Decrease)
{
    return (auraScript->*_HandlerScript)(p_Decrease);
}
...
uint32 AuraScript::GetPeriodicHealDone() const
{
    if (m_currentScriptState != AURA_SCRIPT_HOOK_EFFECT_PERIODIC)
        return 0;

    return m_aura->GetPeriodicHealDone();
}
...
SpellMissInfo SpellScript::GetMissInfo()
{
    if (!IsInTargetHook())
    {
        sLog->outError(LOG_FILTER_TSCR, "Script: `%s` Spell: `%u`: function SpellScript::GetMissInfo was called, but function has no effect in current hook!", m_scriptName->c_str(), m_scriptSpellId);
