...
class AuraEffect
{
    ...
    public:
        ...
        void SetAmplitude(int32 newAmplitude) { m_amplitude = newAmplitude; m_AmplitudeForced = true; }
        ...
        void SetFloatAmount(float amount) { m_floatAmount = amount; }
        ...
        uint32 AbsorbBonusTaken(Unit* p_Caster, Unit* p_Target, int32 p_Amount);
        void CalculateFromDummyAmount(Unit* caster, Unit* target, int32 &amount);
        void CalculatePeriodic(Unit* caster, bool resetPeriodicTimer = true, bool load = false, bool p_Recalculation = false, bool p_IgnorePreviousModOnReapply = false);
        void CalculateSpellMod();
        void ChangeAmount(int32 newAmount, bool mark = true, bool onStackOrReapply = false, bool forceChangeAmount = false, float newFloatAmount = 0.0f);
        ...
        bool CanAuraEffectBeStacked() const { return (bool)m_spellInfo->Effects[m_effIndex].m_CanBeStacked; }
        bool AmountIsNotDependantOfStackModification() const { return bool(m_spellInfo->Effects[m_effIndex].m_AmountNotAlterByStacks);}

        int32 GetPeriodicHitDamage() const { return m_PeriodicHitDamage; }
        int32 GetPeriodicAmount() const { return m_PeriodicAmount; }
        bool IsPeriodicDynamicAmount(uint32 p_SpellID) const;
    private:
        Aura const* m_base;
        ...
        mutable int32 m_PeriodicHitDamage;
        mutable int32 m_PeriodicAmount;
    ...
    public:
        ...
        void HandleAuraFogOfWar(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        //  tracking
        ...
        void HandleAuraGainTalent(AuraApplication const* aurApp, uint8 mode, bool apply) const;
        //  movement
        ...
        void HandleAuraModMovementForceSpeed(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        //  immunity
        ...
        void HandleAuraAddModifier(AuraApplication const* aurApp, uint8 mode, bool apply) const;
        //   heal and energize
        ...
        void HandleSwitchTeam(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        ...
        void HandleAuraActivateSpellScene(AuraApplication const* aurApp, uint8 mode, bool apply) const;
        void HandleAuraContestedPvp(AuraApplication const* p_AuraApp, uint8 p_Mode, bool p_Apply) const;
        ...
        void HandleDisableMovementForce(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        void HandleAllowUsingGameobjectsWhileMounted(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        void HandleBattlegroundFlag(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        void HandleAuraDrowning(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        void HandleBonusLoot(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;

        // aura effect periodic tick handlers
        void HandlePeriodicDummyAuraTick(Unit* target, Unit* caster, SpellEffIndex p_EffIndex) const;
        ...
        void HandleModTimeRate(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
        void HandleModDebuffSpeed(AuraApplication const* p_AurApp, uint8 p_Mode, bool p_Apply) const;
