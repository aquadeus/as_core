...
void AuraApplication::_InitFlags(Unit* caster, uint32 effMask)
{
    ...
    switch (GetBase()->GetId())
    {
        ...
        case 7321:
            _flags |= AFLAG_TIMEMOD;
            break;
        case 12294: /// Mortal Strike
        case 246820: /// Mortal Strike
        ...
        case 157048:
            ...
            break;
        /// Spectral Owl
        /// It's because SpellInfo::IsPositiveEffect has 0-2 effect indexes now. There is needed to handle more effect indexes.
        case 242570:
            if (IsSelfcasted())
            {
...
void AuraApplication::BuildUpdatePacket(ByteBuffer & p_Data, bool p_Remove, uint32 p_OverrideSpellID) const
{
    ...
    uint32 l_SpellID = p_OverrideSpellID ? p_OverrideSpellID : l_Base->GetId();
    uint32 l_SpellXSpellVisualID = l_Base->GetSpellXSpellVisualID();
    uint8  l_Flags = GetFlags();

    std::list<AuraEffect*>const& l_VisualModifiers = l_Target->GetAuraEffectsByType(SPELL_AURA_CHANGE_VISUAL_EFFECT);
    for (AuraEffect* l_Effect : l_VisualModifiers)
    {
        if (l_Effect->GetMiscValue() == l_SpellID
            && l_Effect->GetMiscValueB())
        {
            l_SpellXSpellVisualID = sSpellMgr->GetSpellXVisualForSpellID(l_Effect->GetMiscValueB(), l_Target->GetMap()->GetDifficultyID(), l_SpellXSpellVisualID);
            break;
        }
    }

    // HACK: find the better way
    if (l_Base->GetCaster() == _target &&
        l_Base->GetSpellInfo()->Id == 205630) ///< Illidan's Grasp (Don't show stun effect on the caster)
    {
        l_SpellXSpellVisualID = 0;
    }

    if (l_Base->GetMaxDuration() > 0 && !(l_Base->GetSpellInfo()->AttributesEx5 & SPELL_ATTR5_HIDE_DURATION))
    {
        l_Flags |= AFLAG_DURATION;
        if (G3D::fuzzyNe(l_Target->GetFloatValue(EUnitFields::UNIT_FIELD_MOD_TIME_RATE), 1.00f))
            l_Flags |= AFLAG_TIMEMOD;
    }

    uint16 l_CasterLevel = 0;
    if (!l_Base->GetSpellInfo()->HasAttribute(SPELL_ATTR11_SCALES_WITH_ITEM_LEVEL))
        l_CasterLevel = l_Base->GetCasterLevel();
    else
        l_CasterLevel = uint16(l_Base->GetCastItemLevel());

    uint32 l_Mask = 0;
    uint8  l_Applications = l_Base->GetSpellInfo()->StackAmount ? l_Base->GetStackAmount() : l_Base->GetCharges();
    ...
    p_Data.appendGuid128(l_Base->GetCastGUID());                                                                ///< Cast GUID
    p_Data << uint32(p_OverrideSpellID ? p_OverrideSpellID : l_Base->GetId());                                  ///< SpellID
    p_Data << uint32(l_SpellXSpellVisualID);                                                                    ///< SpellVisuals
    ...
    p_Data.WriteBit((l_Flags & AFLAG_DURATION) != 0);                                                           ///< Has Duration
    p_Data.WriteBit((l_Flags & AFLAG_DURATION) != 0);                                                           ///< Has Remaining
    p_Data.WriteBit(l_Flags & AFLAG_TIMEMOD);                                                                   ///< Has TimeMod
    /// p_Data.WriteBits(l_PointsCount + (l_PointsCount && (l_Flags & AFLAG_UNK_20) ? 1 : 0), 6);               ///< Points Count
    /// p_Data.WriteBits(l_AsApModifier ? l_PointsCount : 0, 6);                                                ///< Estimated Points
    p_Data.WriteBits(l_Base->GetEffectCount(), 6);
    p_Data.WriteBits(0, 6);
    p_Data.WriteBit(false);                                                                                     ///< HasDebugData
    p_Data.FlushBits();
    ...
    if (l_Flags & AFLAG_TIMEMOD)
        p_Data << float(l_Target->GetFloatValue(EUnitFields::UNIT_FIELD_MOD_TIME_RATE));
    ...
    for (uint8 l_I = 0; l_I < l_Base->GetEffectCount(); ++l_I)
    {
        ...
        if (AuraEffect const* l_Effect = l_Base->GetEffect(l_I))
        {
            ...
            p_Data << float(l_Base->GetSpellInfo()->GetFloatAmount(l_Effect->GetAmount(), l_Effect->GetFloatAmount()));
        }
        else
            p_Data << float(0.0f);
    }
    ...
    ///}
    ///
    ///if (l_Flags & AFLAG_SCALABLE && l_AsApModifier)
    ///{
    ///    for (uint8 l_I = 0; l_I < m_EffectCount; ++l_I)
    ///    {
    ///        if (!HasEffect(l_I))
...
void AuraApplication::ClientUpdate(bool p_Remove)
{
    ...
    if (l_AuraBase == nullptr)
        return;

    if (l_AuraBase->GetSpellInfo()->Id == 197912 && sWorld->getBoolConfig(CONFIG_TOURNAMENT_ENABLE) && !(_target->IsPlayer() && _target->ToPlayer()->GetBattleground()))
        return;

    WorldPacket l_Data(SMSG_AURA_UPDATE);

    l_Data.WriteBit(false);                         ///< Update All
    l_Data.WriteBits(1, 9);                         ///< Aura count
    l_Data.FlushBits();
    ...
    if (l_AuraBase->GetSpellInfo()->Attributes & SPELL_ATTR0_HIDDEN_CLIENTSIDE || p_Remove)
        return;

    if (l_AuraBase->GetCaster() == _target && l_AuraBase->GetSpellInfo()->Id != 219812)
        return;
...
Aura* Aura::Create(SpellInfo const* spellproto, uint32 effMask, WorldObject* owner, Unit* caster, int32* baseAmount, Item* castItem, uint64 casterGUID, int32 castItemLevel)
{
    ...
    {
        case TYPEID_PLAYER:
            sAnticheatMgr->HandleAddAura(owner->GetGUIDLow(), spellproto->Id);
            /// no break intended
        case TYPEID_UNIT:
...
Aura::Aura(SpellInfo const* spellproto, WorldObject* owner, Unit* caster, Item* castItem, uint64 casterGUID, int32 castItemLevel) :
    ...
    m_lastProcAttemptTime(getMSTime() - 10 * IN_MILLISECONDS), m_lastProcSuccessTime(getMSTime() - 120 * IN_MILLISECONDS), m_customData(0), m_PeriodicHealDone(0),
    m_SpellXSpellVisualID(m_spellInfo->GetSpellXSpellVisualId(caster))
{
    m_ATForceGuid = 0;
    ...
    if (castItem && caster)
    {
        if (Player* l_Player = caster->ToPlayer())
        {
            if (m_spellInfo->AttributesEx11 & SPELL_ATTR11_SCALES_WITH_ITEM_LEVEL)
                m_casterLevel = castItem->GetItemLevel(l_Player);

        }
    }
    else
        m_casterLevel = caster ? caster->getLevelForTarget(owner) : m_spellInfo->SpellLevel;

    if (m_spellInfo->MaxScalingLevel && caster && caster->getLevelForTarget(m_owner) > m_spellInfo->MaxScalingLevel)
        m_casterLevel = m_spellInfo->MaxScalingLevel;
...
void Aura::_UnapplyForTarget(Unit* p_Target, Unit* p_Caster, AuraApplication * p_AuraApp)
{
    ...
    if (p_Caster && p_Caster->IsPlayer())
    {
        ...
        if (m_spellInfo->IsCooldownStartedOnEvent()                                     ///< IsCooldownEvent spell
            ...
            && m_spellInfo->Id != 200285)                                               ///< Gunpowder Charge should be usable only once per combat, so its cooldown is handled in UpdatePotionCooldown
...
void Aura::_Remove(AuraRemoveMode removeMode)
{
    ...
    {
        AuraApplication* l_AurApp = appItr->second;

        if (l_AurApp == nullptr)
            continue;

        Unit* target = l_AurApp->GetTarget();
        target->_UnapplyAura(l_AurApp, removeMode);

        if (target->IsPlayer())
            sAnticheatMgr->HandleRemoveAura(target->ToPlayer(), m_spellInfo->Id);
...
void Aura::UpdateTargetMap(Unit* caster, bool apply)
{
    ...
    m_updateTargetMapInterval = 1500;
    ...
    for (std::map<Unit*, uint32>::iterator itr = targets.begin(); itr != targets.end();)
    {
        ...
        else
        {
            ...
            if (itr->first->_CreateAuraApplication(this, itr->second) == nullptr)
                SetDuration(0);
...
void Aura::_ApplyEffectForTargets(uint8 effIndex)
{
    ...
    std::vector<AuraApplication*> l_AddedAuras;
    // apply effect to targets
    for (UnitList::iterator itr = targetList.begin(); itr != targetList.end(); ++itr)
    {
        if (AuraApplication* l_AuraApplication = GetApplicationOfTarget((*itr)->GetGUID()))
        {
            l_AddedAuras.push_back(l_AuraApplication);
            // owner has to be in world, or effect has to be applied to self
        ...
        else if (caster)
        {
            for (auto itr : m_spellInfo->SpellPowers)
            {
                ...
                if (int32 powerPerSecond = itr->CostPerSecond + int32(itr->CostPerSecondPercentage * caster->GetCreatePowers(powerType) / 100))
                {
                    ...
                    {
                        if (static_cast<int32>(caster->GetPower(powerType)) >= powerPerSecond)
                        {
                            if (GetId() == 117952)
                            {
                                if (AuraEffect* l_TheEmperorCapacitorEffect = caster->GetAuraEffect(235054, EFFECT_1))
                                    powerPerSecond = CalculatePct(powerPerSecond, 100 + l_TheEmperorCapacitorEffect->GetAmount());
                            }
                            if (GetId() != 193440 || GetOwner()->IsPlayer()) ///< Fix for Demonwrath consumming mana for all Pet
...
int32 Aura::CalcMaxDuration(Unit* caster)
{
    ...
    if (caster)
    {
        ...
        if (maxDuration != -1 && m_spellInfo->HasAttribute(SPELL_ATTR8_HASTE_AFFECT_DURATION_RECOVERY) && !m_spellInfo->IsChanneled())
        {
            Unit* l_Caster = caster->GetCharmerOrOwnerOrSelf();
            maxDuration = int32(maxDuration * std::max<float>(modOwner ? modOwner->GetFloatValue(UNIT_FIELD_MOD_CASTING_SPEED) : l_Caster->GetFloatValue(UNIT_FIELD_MOD_CASTING_SPEED), 0.5f));
        }
    }
    else
...
void Aura::SetDuration(int32 duration, bool withMods)
{
    if (withMods)
    {
        ...
        {
            // Calculate duration by haste.
            if (m_spellInfo->HasAttribute(SPELL_ATTR8_HASTE_AFFECT_DURATION_RECOVERY) && !m_spellInfo->IsChanneled())
                duration = int32(duration * std::max<float>(caster->GetFloatValue(UNIT_FIELD_MOD_CASTING_SPEED), 0.5f));
...
void Aura::RefreshDuration(bool withMods)
{
    if (withMods)
    {
        ...
        if (Unit* l_Caster = GetCaster())
        {
            // Calculate duration by haste.
            if (m_spellInfo->HasAttribute(SPELL_ATTR8_HASTE_AFFECT_DURATION_RECOVERY) && !m_spellInfo->IsChanneled())
                duration = int32(duration * std::max<float>(l_Caster->GetFloatValue(UNIT_FIELD_MOD_CASTING_SPEED), 0.5f));

            if (Player* l_ModOwner = l_Caster->GetSpellModOwner())
                l_ModOwner->ApplySpellMod(GetId(), SPELLMOD_DURATION, duration);
        }
...
void Aura::RefreshTimers()
{
    ...
    if (GetCaster() && GetCaster()->IsPlayer() && GetSpellInfo()->IsPandemicAffected())
    {
        for (uint8 i = 0; i < m_EffectCount; ++i)
        {
            if (AuraEffect const* l_Effect = GetEffect(i))
            {
                ...
                if (l_Effect->GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE || l_Effect->GetAuraType() == SPELL_AURA_PERIODIC_HEAL
                    || l_Effect->GetAuraType() == SPELL_AURA_PERIODIC_LEECH || l_Effect->GetAuraType() == SPELL_AURA_PERIODIC_SCHOOL_DAMAGE)
    ...
    if (l_IsAffectedByPandemic)
    {
        ...
        int32 l_OriginalMaxDuration = m_spellInfo->GetMaxDuration();

        // Calculate duration by haste.
        if (l_OriginalMaxDuration != -1 && m_spellInfo->HasAttribute(SPELL_ATTR8_HASTE_AFFECT_DURATION_RECOVERY))
            l_OriginalMaxDuration = int32(l_OriginalMaxDuration * std::max<float>(l_Caster->GetFloatValue(UNIT_FIELD_MOD_CASTING_SPEED), 0.5f));

        if (l_OriginalMaxDuration != -1 && l_ModOwner)
            l_ModOwner->ApplySpellMod(GetId(), SPELLMOD_DURATION, l_OriginalMaxDuration);

        /// Max duration of reapplied aura should be 130% of base max duration (so taken from spellInfo)
        int32 l_MaxAllowedDuration = CalculatePct(l_OriginalMaxDuration, 130);
        int32 l_AddedDuration = l_LeftDuration + l_OriginalMaxDuration;

        m_maxDuration = std::min(l_AddedDuration, l_MaxAllowedDuration);
    }
    ...
    for (uint8 i = 0; i < m_EffectCount; ++i)
        if (HasEffect(i))
            GetEffect(i)->CalculatePeriodic(caster, resetPeriodic, false, false, true);
}
...
uint8 Aura::CalcMaxCharges(Unit* caster) const
{
    ...
    switch (GetId())
    {
        ...
        case 79683:
            maxProcCharges = 3;
            break;
        case 253595: ///< Inexorable Assault
            maxProcCharges = std::min(10, (int32)GetStackAmount());
            break;
    }
...
bool Aura::ModCharges(int32 num, AuraRemoveMode removeMode)
{
    if (IsUsingCharges())
    {
        ...
        switch (GetId())
        {
            ...
            case 81340:     ///< Double Doom
            case 164545:    ///< Solar Empowerment
            case 145152:    ///< Bloodtalons
            case 203407:    ///< Resto Druid Honor Talent 'Revitalize' (hotfixed to have 2 procCharges max) 7.3.5 - Build 26365
            case 253595:    ///< Inexorable Assault
...
bool Aura::ModStackAmount(int32 num, AuraRemoveMode removeMode)
{
    ...
    int32 maxStackAmount = int32(m_spellInfo->StackAmount);
    if (Unit* l_Caster = GetCaster())
    {
    ...
    if (CallScriptCanRrefreshProcHandlers(num < 0) && stackAmount >= GetStackAmount())
    {
        RefreshSpellMods();
        RefreshTimers();
        ///< Fix Lunar and Solar Empowerment and Double Doom
        if (m_spellInfo->Id == 164545 || m_spellInfo->Id == 164547 || m_spellInfo->Id == 81340)
...
{
    std::vector<uint32> l_AurasID = sObjectMgr->GetAllCosmeticsID(eCosmetics::CosmeticAura);
    if (std::find(l_AurasID.begin(), l_AurasID.end(), GetId()) != l_AurasID.end())
        return false;

    if (sObjectMgr->ShouldntBeSaved(GetId()))
        return false;
    ...
    // Can't be saved - aura casted by areatrigger
    if (IsCastedByAreatrigger())
        return false;
...
void Aura::HandleAuraSpecificMods(AuraApplication const* aurApp, Unit* caster, bool apply, bool onReapply)
{
                ...
                for (std::vector<SpellLinked>::const_iterator itr = spellTriggered->begin(); itr != spellTriggered->end(); ++itr)
                {
                    ...
                    else if (_caster)
                    {
                        ...
                        if (itr->cooldown != 0 && _target->GetTypeId() == TYPEID_PLAYER)
                            _target->ToPlayer()->AddSpellCooldown(itr->effect, 0, itr->cooldown * TimeConstants::IN_MILLISECONDS);
                    }
                }
            }
                    ...
                    if (itr->effect < 0)
                    {
                        switch (itr->actiontype)
                        {
                            ...
                            case LINK_ACTION_SEND_COOLDOWN: // 3
                                _target->SendSpellCooldown(itr->effect, GetId());
                                break;
                            default:
                                break;
                        }
                    ...
                    else
                    {
                        ...
                        switch (itr->actiontype)
                        {
                            ...
                            case LINK_ACTION_REMOVE_MOVEMENT: //9
                                _target->RemoveMovementImpairingAuras();
                                break;
            ...
            for (std::vector<SpellLinked>::const_iterator itr = spellTriggered->begin(); itr != spellTriggered->end(); ++itr)
            {
                if (itr->effect > 0)
                {
                    ...
                    if (itr->cooldown != 0 && target->GetTypeId() == TYPEID_PLAYER)
                        target->ToPlayer()->AddSpellCooldown(itr->effect, 0, itr->cooldown * TimeConstants::IN_MILLISECONDS);
                }
            }
        }
    }
    ...
    if (apply)
    {
        ...
        if (forceData && !forceData->empty())
        {
            for (std::vector<AreaTriggerForce>::const_iterator itr = forceData->begin(); itr != forceData->end(); ++itr)
            {
                ...
                {
                    m_ATForceGuid = MAKE_NEW_GUID(sObjectMgr->GenerateLowGuid(HIGHGUID_AREATRIGGER), itr->CustomEntry, HIGHGUID_AREATRIGGER);
                    Position l_Direction = { itr->center.x, itr->center.y, itr->center.z, 0.0f };
                    player->SendApplyMovementForce(m_ATForceGuid, true, l_Direction, itr->windSpeed, itr->windType, itr->wind);
                }
            }
        }

        /// Sephuz's Secret
        /// Cannot handle success applying in HandleProc
        if (caster && caster != target &&
            (GetSpellInfo()->_IsLossOfControl() || GetSpellInfo()->_IsCrowdControl()))
        {
            if (Unit* l_Owner = caster->GetCharmerOrOwnerPlayerOrPlayerItself())
            {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            if (GetSpellInfo()->SpellFamilyFlags[0] & 0x40 && GetSpellInfo()->GetCategory() == 44)
            {
                ...
                if (target->HasAura(58039)) // Glyph of Blurred Speed ///< 7.0.3 deprecated
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            case 49206: // Summon Gargoyle debuff, need to find target
            {
                if (caster->getClass() == CLASS_DEATH_KNIGHT)
                {
                    ...
                    {
                        if ((*itr)->GetEntry() == DeathKnightPet::Gargoyle)
                        {
        ...
        if (Creature* l_Creature = target->ToCreature())
        {
            if (l_Creature->IsAIEnabled)
                l_Creature->AI()->OnAddAura(aurApp->GetBase(), onReapply);
        }

        if (target->IsPlayer())
        {
            if (InstanceScript* l_Instance = target->GetInstanceScript())
                l_Instance->OnPlayerAuraApplied(target->ToPlayer(), GetId());
        }
    }
    // mods at aura remove
    else
    {
        if (m_ATForceGuid)
        {
            G3D::Vector3 temp;
            if (Player* player = target->ToPlayer())
                player->SendApplyMovementForce(m_ATForceGuid, false, Position());
        }
        ...
        {
            if (l_Creature->IsAIEnabled)
                l_Creature->AI()->OnRemoveAura(GetId(), removeMode);
        }

        if (target->IsPlayer())
        {
            if (InstanceScript* l_Instance = target->GetInstanceScript())
                l_Instance->OnPlayerAuraRemoved(target->ToPlayer(), GetId(), removeMode);
        }
    }
    ...
    case SPELLFAMILY_ROGUE:
        // Stealth / Vanish
        if (GetSpellInfo()->SpellFamilyFlags[0] & 0x00400800 || GetSpellInfo()->Id == 185422) ///< Shadow dance
        {
            // Master of subtlety
            if (AuraEffect const* aurEff = target->GetAuraEffectOfRankedSpell(31223, 0))
            {
                if (apply)
                {
                    SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(31665);
                    if (!l_SpellInfo)
                        return;

                    int32 basepoints0 = l_SpellInfo->Effects[EFFECT_0].BasePoints;
                    ...
                    if (Aura* aur = target->GetAura(31665))
                        aur->SetAuraTimer(aurEff->GetAmount() * TimeConstants::IN_MILLISECONDS);
            }
            // Overkill
            if (target->HasAura(58426) && GetSpellInfo()->Id != 185422) ///< Shadow dance
            {
                if (apply)
                {
...
void Aura::HandleAuraSpecificPeriodics(AuraApplication const* aurApp, Unit* caster)
{
    ...
    for (uint8 i = 0; i < m_EffectCount; ++i)
    {
        ...
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            {
                GetEffect(i)->SetDonePct((static_cast<float>(caster->SpellDamageBonusDone(target, m_spellInfo, 100.0f, i, DOT, GetStackAmount()))/100.0f) * caster->SpellDamagePctDone(target, m_spellInfo, DOT, i, nullptr, true)); // Calculate % bonus damage done at application of DOT
                break;
            }
            case SPELL_AURA_PERIODIC_HEAL:
            case SPELL_AURA_OBS_MOD_HEALTH:
            {
                GetEffect(i)->SetDonePct(caster->SpellHealingPctDone(target, m_spellInfo, true)); // Calculate % bonus healing done at application of HOT
                break;
            }
            default:
                break;
...
bool Aura::CanStackWith(Aura const* existingAura) const
{
    ...
    ///< Skyfury Totem
    if (m_spellInfo->Id == 208963 && existingSpellInfo->Id == 208963 && !sameCaster)
        return false;

    ///< Paladin's blessing
    if ((m_spellInfo->Id == 203538 || m_spellInfo->Id == 203539) && m_spellInfo->Id == existingSpellInfo->Id)
        return false;

    ///< Vampiric Aura, Death Knight
    if (m_spellInfo->Id == 238698 && m_spellInfo->Id == existingSpellInfo->Id)
        return false;
    ...
    for (uint8 i = 0; i < l_MaxEffectCount; ++i)
    {
        ...
        if ((existingSpellInfo->EffectCount > i &&  existingSpellInfo->Effects[i].TriggerSpell == GetId())
            // prevent remove triggered aura by triggering aura refresh
            || (m_spellInfo->EffectCount > i && m_spellInfo->Effects[i].TriggerSpell == existingAura->GetId()))
            return true;
    }
    ...
    /// AreaTrigger debuffs don't stack with same ATs, adding exception for Imonar the Soulhunter - Shrapnel Blast
    if (m_spellInfo->HasCustomAttribute(SpellCustomAttributes::SPELL_ATTR0_CU_IS_CUSTOM_AOE_SPELL) && existingSpellInfo->Id == m_spellInfo->Id && m_spellInfo->Id != 247932)
        return false;
    ...
    if (!sameCaster)
    {
        ...
        for (uint32 i = 0; i < m_spellInfo->EffectCount; ++i)
        {
            ...
            switch (m_spellInfo->Effects[i].ApplyAuraName)
            {
                ...
                case SPELL_AURA_MOD_DAMAGE_FROM_CASTER_BY_SCHOOLMASK:              // Colossus smash-like auras
    ...
    {
        /// Dancing Rune Weapon
        /// It casts same spells with OriginalCaster as Owner
        if (GetCaster() && GetCaster() != existingAura->GetCaster() &&
            ((GetCaster()->GetEntry() == DeathKnightPet::DancingRuneWeapon) ||
            (existingAura->GetCaster() && existingAura->GetCaster()->GetEntry() == DeathKnightPet::DancingRuneWeapon)))
            return true;
        ...
        if (m_spellInfo->Id == 236181 && !sameCaster) ///< Guardian Druid Honor Talent 'Den Mother' +15% Max hp - Shouldn't stack between several druids. 7.1.5 Build 23420
            return false;

        // same spell with same caster should not stack
        if (!m_spellInfo->HasAura(SPELL_AURA_CONTROL_VEHICLE) && sameCaster)
            return false;
    }
    ...
    if (m_spellInfo->IconFileDataId == 0 || existingSpellInfo->IconFileDataId == 0)
    {
    ...
    if (m_spellInfo->IconFileDataId != existingSpellInfo->IconFileDataId &&
    ...
    if (m_spellInfo->IsAllwaysStackModifers() && !existingSpellInfo->IsAllwaysStackModifers())
    {
        for (int i = 0; i < 3; ++i)
        {
            if ((m_spellInfo->Effects[i].IsApplyingAura() || m_spellInfo->Effects[i].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID) &&
                (existingSpellInfo->Effects[i].IsApplyingAura() || existingSpellInfo->Effects[i].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID) &&
                m_spellInfo->Effects[i].ApplyAuraName == existingSpellInfo->Effects[i].ApplyAuraName)
            {
                ...
                case SPELL_AURA_MOD_RATING:
                    if (m_spellInfo->IconFileDataId == existingSpellInfo->IconFileDataId && m_spellInfo->Effects[i].MiscValue == existingSpellInfo->Effects[i].MiscValue)
                        return false;
                    break;
                default:
                    break;
...
bool Aura::IsSameRaidBuff(Aura const* p_ExistingAura) const
{
    ...
    {
        if (l_I >= l_SpellInfoExisting->EffectCount || l_I >= l_SpellInfoNew->EffectCount)
            break;
...
float Aura::CalcPPMProcChance(float procsPerMinute, Unit* actor) const
{
    ...
    float secondsSinceLastAttempt = std::min(float(currentTime - GetLastProcAttemptTime()) / IN_MILLISECONDS, 10.0f);
    float secondsSinceLastProc = std::min(float(currentTime - GetLastProcSuccessTime()) / IN_MILLISECONDS, 1000.0f);

    float chance = ppm * secondsSinceLastAttempt / 60.0f;
    chance *= std::max(1.0f, 1.0f + ((secondsSinceLastProc / averageProcInterval - 1.5f) * 3.0f));
    chance *= 100;
    RoundToInterval(chance, 0.0f, 100.0f);
    return chance;
}
...
void Aura::CallScriptCalcMaxDurationHandlers(int32& p_MaxDuration)
{
    for (std::list<AuraScript*>::iterator l_ScrItr = m_loadedScripts.begin(); l_ScrItr != m_loadedScripts.end(); ++l_ScrItr)
    {
        (*l_ScrItr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_CALC_MAX_DURATION);
        std::list<AuraScript::CalcMaxDurationHandler>::iterator l_HookItrEnd = (*l_ScrItr)->DoCalcMaxDuration.end(), l_HookItr = (*l_ScrItr)->DoCalcMaxDuration.begin();
        for (; l_HookItr != l_HookItrEnd; ++l_HookItr)
            (*l_HookItr).Call(*l_ScrItr, p_MaxDuration);
...
bool Aura::CallScriptDispel(DispelInfo* dispelInfo)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        if (!l_Prevented)
            l_Prevented = (*scritr)->_IsDefaultActionPrevented();
        (*scritr)->_FinishScriptCall();
    }

    CallDBScriptOnDispel(m_spellInfo->Id);
    return l_Prevented;
}
...
void Aura::CallScriptAfterDispel(DispelInfo* dispelInfo)
{
    ...
    CallDBScriptAfterDispel(m_spellInfo->Id);
}

bool Aura::CallScriptEffectApplyHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    UnsafeSpellOrAuraScript l_SafeGuard;
...
void Aura::CallScriptAfterEffectApplyHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    ...
    CallDBScriptAfterApply(m_spellInfo->Id, aurEff, aurApp, mode);
}
...
void Aura::CallScriptAfterEffectRemoveHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, AuraEffectHandleModes mode)
{
    ...
    CallDBScriptAfterRemove(m_spellInfo->Id, aurEff, aurApp, mode);
}
...
bool Aura::CallScriptEffectPeriodicHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp)
{
    ...
    if (!preventDefault)
        preventDefault = CallDBScriptOnPeriodic(m_spellInfo->Id, aurEff, aurApp);
...
void Aura::CallScriptAfterEffectPeriodicHandlers(AuraEffect const* p_AurEff, AuraApplication const* p_AurApp)
{
    ...
    CallDBScriptAfterPeriodic(m_spellInfo->Id, p_AurEff, p_AurApp);
}
...
void Aura::CallScriptEffectAbsorbHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo & dmgInfo, uint32 & absorbAmount, bool& defaultPrevented)
{
    ...
    CallDBScriptOnAbsorb(m_spellInfo->Id, aurEff, aurApp, dmgInfo, absorbAmount);
}
...
void Aura::CallScriptEffectAfterAbsorbHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo & dmgInfo, uint32 & absorbAmount)
{
    ...
    CallDBScriptAfterAbsorb(m_spellInfo->Id, aurEff, aurApp, dmgInfo, absorbAmount);
}
...
void Aura::CallScriptEffectSplitDamageHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo & dmgInfo, uint32 & splitAmount, bool& defaultPrevented)
{
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        (*scritr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_EFFECT_SPLIT_DAMAGE, aurApp);
        std::list<AuraScript::EffectSplitDamageHandler>::iterator effEndItr = (*scritr)->OnEffectSplitDamage.end(), effItr = (*scritr)->OnEffectSplitDamage.begin();
        for (; effItr != effEndItr; ++effItr)
        {
            if ((*effItr).IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
...
bool Aura::CallScriptCheckProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    return CallDBScritpCheckProc(m_spellInfo->Id, aurApp, eventInfo);
}
    ...
    for (auto scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        for (; hookItr != hookItrEnd; ++hookItr)
        {
            if (hookItr->IsEffectAffected(m_spellInfo, aurEff->GetEffIndex()))
                l_Result = hookItr->Call(*scritr, aurEff, eventInfo);
        }
...
void Aura::CallScriptCalcPPMRate(float & p_ProcPerMinute, ProcEventInfo & p_EventInfo)
{
    for (auto l_ScriptItr = m_loadedScripts.begin(); l_ScriptItr != m_loadedScripts.end(); ++l_ScriptItr)
    {
        (*l_ScriptItr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_CALC_PPM);

        auto l_HookItrEnd = (*l_ScriptItr)->DoCalcPPM.end();
        auto l_HookItr = (*l_ScriptItr)->DoCalcPPM.begin();

        for (; l_HookItr != l_HookItrEnd; ++l_HookItr)
            (*l_HookItr).Call(*l_ScriptItr, p_ProcPerMinute, p_EventInfo);

        (*l_ScriptItr)->_FinishScriptCall();
    }
}

void Aura::CallScriptCalcProcChance(float & p_Chance, ProcEventInfo & p_EventInfo)
{
    for (auto l_ScriptItr = m_loadedScripts.begin(); l_ScriptItr != m_loadedScripts.end(); ++l_ScriptItr)
    {
        (*l_ScriptItr)->_PrepareScriptCall(AURA_SCRIPT_HOOK_CALC_PROC_CHANCE);

        auto l_HookItrEnd = (*l_ScriptItr)->DoCalcProcChance.end();
        auto l_HookItr = (*l_ScriptItr)->DoCalcProcChance.begin();

        for (; l_HookItr != l_HookItrEnd; ++l_HookItr)
            (*l_HookItr).Call(*l_ScriptItr, p_Chance, p_EventInfo);
...
bool Aura::CallScriptPrepareProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    CallDBScriptPrepareProc(m_spellInfo->Id, aurApp, eventInfo);
...
void Aura::CallScriptProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    CallDBScriptProc(m_spellInfo->Id, aurApp, eventInfo);
}
...
void Aura::CallScriptAfterProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    CallDBScriptAfterProc(m_spellInfo->Id, aurApp, eventInfo);
}
...
bool Aura::CallScriptEffectProcHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    CallDBScriptEffectProc(m_spellInfo->Id, aurEff, aurApp, eventInfo);
...
void Aura::CallScriptAfterEffectProcHandlers(AuraEffect const* aurEff, AuraApplication const* aurApp, ProcEventInfo& eventInfo)
{
    ...
    CallDBScriptAfterEffectProc(m_spellInfo->Id, aurEff, aurApp, eventInfo);
}

bool Aura::CallScriptCanRrefreshProcHandlers(bool p_Decrease)
{
    ...
    for (std::list<AuraScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
        ...
        for (; effItr != effEndItr; ++effItr)
        {
            bool l_TempResult = (*effItr).Call(*scritr, p_Decrease);
...
void Aura::ChangeWindDirection(G3D::Vector3 p_NewDirection)
{
    if (!m_ATForceGuid || GetUnitOwner() == nullptr)
        return;

    Player* l_Target = GetUnitOwner()->ToPlayer();
    if (l_Target == nullptr)
        return;

    l_Target->SendApplyMovementForce(m_ATForceGuid, false, Position());

    std::vector<AreaTriggerForce> const* l_ForceData = sObjectMgr->GetAreaTriggerForce(GetId());
    if (l_ForceData && !l_ForceData->empty())
    {
        for (std::vector<AreaTriggerForce>::const_iterator itr = l_ForceData->begin(); itr != l_ForceData->end(); ++itr)
        {
            Position l_Direction = { itr->center.x, itr->center.y, itr->center.z, 0.0f };
            l_Target->SendApplyMovementForce(m_ATForceGuid, true, l_Direction, itr->windSpeed, itr->windType, p_NewDirection);
        }
    }
}

void Aura::InitCastGUID()
{
    m_CastGUID = Guid128(GUID_TYPE_CAST, 0, GetId(), Spell::GenerateNewSpellGUID());
}
    ...
    for (uint8 effIndex = 0; effIndex < m_EffectCount; ++effIndex)
    {
        ...
        else
        {
            ...
            if (!GetUnitOwner()->HasUnitState(UNIT_STATE_ISOLATED))
            {
                ...
                if (ConditionContainer* l_CondList = m_spellInfo->Effects[effIndex].ImplicitTargetConditions)
                {
                    targetList.remove_if([&](Unit* p_Target) -> bool
                    {
                        if (p_Target == GetUnitOwner())
                            return false;

                        ConditionSourceInfo l_SrcInfo = ConditionSourceInfo(p_Target, GetUnitOwner());
                        if (!sConditionMgr->IsObjectMeetToConditions(l_SrcInfo, *l_CondList))
                            return true;
...
void Aura::CallDBScriptOnDispel(uint32 p_SpellId)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::OnDispel)
            continue;

        PrepareAction(l_AuraScript, nullptr);
    }
}

void Aura::CallDBScriptAfterDispel(uint32 p_SpellId)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterDispel)
            continue;
...
void Aura::CallDBScriptAfterApply(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, AuraEffectHandleModes p_Modes)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterEffectApply)
            continue;

        if (l_AuraScript.EffectId != p_AuraEffect->GetEffIndex())
            continue;

        PrepareAction(l_AuraScript, p_AuraApplication);
    }
}

void Aura::CallDBScriptAfterRemove(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, AuraEffectHandleModes p_Modes)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterEffectRemove)
            continue;
...
bool Aura::CallDBScriptOnPeriodic(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication)
{
    bool l_DefaultActionPrevented = false;

    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::OnEffectPeriodic)
            continue;
    ...
    return l_DefaultActionPrevented;
}

void Aura::CallDBScriptAfterPeriodic(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterEffectPeriodic)
            continue;
...
void Aura::CallDBScriptOnAbsorb(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, DamageInfo& p_DamageInfo, uint32& p_AbsorbAmount)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::OnAbsorb)
            continue;
        ...
        Unit* l_ActionCaster = GetScriptActor(l_AuraScript.ActionCaster, p_AuraApplication);

        if (l_AuraScript.Action == eAuraScriptActions::ModAbsorb)
        {
            AuraEffect* l_AuraEffect = GetEffect(l_AuraScript.EffectId);
            if (!l_AuraEffect)
                return;

            uint32 l_Value = 0;
            if (l_AuraScript.ActionSpellId && l_AuraScript.DataEffectId != -1)
            {
                if (SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(l_AuraScript.ActionSpellId))
                    l_Value = l_SpellInfo->Effects[l_AuraScript.DataEffectId].BasePoints;
            }

            if (l_Value && l_AuraScript.ArtifactTraitId)
            {
                if (l_ActionCaster)
                {
                    if (Player* l_Player = l_ActionCaster->ToPlayer())
                        l_Value *= l_Player->GetRankOfArtifactPowerId(l_AuraScript.ArtifactTraitId);
                }
            }
            ...
            switch (l_AuraScript.CalculationType)
            {
                case eCalculationType::TypeCalcPct:
                    p_AbsorbAmount = CalculatePct(p_AbsorbAmount, l_Value);
                    break;
                case eCalculationType::TypeAddPct:
                    AddPct(p_AbsorbAmount, l_Value);
                    break;
                case eCalculationType::TypeRemovePct:
                    AddPct(p_AbsorbAmount, -int32(l_Value));
                    break;
                case eCalculationType::TypeAddFlat:
                    p_AbsorbAmount += l_Value;
                    break;
...
void Aura::CallDBScriptAfterAbsorb(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, DamageInfo& p_DamageInfo, uint32& p_AbsorbAmount)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterAbsorb)
            continue;
...
bool Aura::CallDBScritpCheckProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    bool l_AllowProc = true;

    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::CheckProc)
            continue;

        PrepareProcAction(l_AuraScript, p_AuraApplication, p_ProcEventInfo);

        if (l_AuraScript.Action == eAuraScriptActions::CheckingProc)
        {
            if (l_AuraScript.ActionSpellList.size())
            {
                if (DamageInfo* l_DamageInfo = p_ProcEventInfo.GetDamageInfo())
                {
                    ...
                    {
                        bool l_Found = false;
                        for (uint32 l_SpellID : l_AuraScript.ActionSpellList)
                        {
            ...
            Unit* l_ActionTarget = GetScriptActor(l_AuraScript.ActionTarget, p_AuraApplication, p_ProcEventInfo);
            if (!l_ActionTarget)
                return l_AllowProc;

            Unit* l_ActionOriginalCaster = GetScriptActor(l_AuraScript.ActionOriginalCaster, p_AuraApplication, p_ProcEventInfo);
            if (!l_ActionOriginalCaster)
                return l_AllowProc;

            for (uint32 l_Aura : l_AuraScript.AuraState)
            {
                if (!l_ActionTarget->HasAura(l_Aura, l_ActionOriginalCaster->GetGUID()))
                    return false;
            }

            for (uint32 l_Aura : l_AuraScript.ExcludeAuraState)
            {
                if (l_ActionTarget->HasAura(l_Aura, l_ActionOriginalCaster->GetGUID()))
                    return false;
            }
...
void Aura::CallDBScriptPrepareProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::PrepareProc)
            continue;
...
void Aura::CallDBScriptProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::OnProc)
            continue;
...
void Aura::CallDBScriptAfterProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterProc)
            continue;
...
void Aura::CallDBScriptEffectProc(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::OnEffectProc)
            continue;
...
void Aura::CallDBScriptAfterEffectProc(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    AuraScriptMapBounds l_Bounds = sSpellMgr->mAuraScriptMap.equal_range(p_SpellId);
    for (auto l_Itr = l_Bounds.first; l_Itr != l_Bounds.second; ++l_Itr)
    {
        AuraScriptEntry l_AuraScript = (*l_Itr).second;
        if (l_AuraScript.Hook != eAuraHooks::AfterEffectProc)
            continue;
...
void Aura::DoAction(AuraScriptEntry p_AuraScript, Unit* p_ActionCaster, Unit* p_ActionTarget, Unit* p_OriginalCaster)
{
    if (p_ActionCaster)
    {
        for (uint32 l_SpellId : p_AuraScript.AuraState)
        {
            if (!p_ActionCaster->HasAura(l_SpellId))
                return;
        }

        for (uint32 l_SpellId : p_AuraScript.ExcludeAuraState)
        {
            if (p_ActionCaster->HasAura(l_SpellId))
                return;
        }
    ...
    if (p_ActionTarget)
    {
        for (uint32 l_SpellId : p_AuraScript.TargetAuraState)
        {
            if (!p_ActionTarget->HasAura(l_SpellId))
                return;
        }

        for (uint32 l_SpellId : p_AuraScript.TargetExcludeAuraState)
        {
            if (p_ActionTarget->HasAura(l_SpellId))
                return;
        }
    ...
    switch (p_AuraScript.Action)
    {
        case eAuraScriptActions::TriggerSAI:
        {
            if (!p_ActionCaster || !p_ActionTarget)
                return;

            if (!p_ActionCaster->ToCreature() || !p_ActionCaster->ToCreature()->IsAIEnabled)
                return;

            switch (p_AuraScript.Hook)
            {
                case eAuraHooks::AfterEffectApply:
                    p_ActionCaster->ToCreature()->AI()->sOnAuraAddedToTarget(p_ActionTarget, GetSpellInfo()->Id);
                    break;
                case eAuraHooks::AfterEffectRemove:
                    p_ActionCaster->ToCreature()->AI()->sOnAuraRemovedFromTarget(p_ActionTarget, GetSpellInfo()->Id);
                    break;
                default:
                    break;
            }
        ...
        case eAuraScriptActions::CastSpell:
        {
            if (!p_ActionCaster || !p_ActionTarget)
                return;

            p_ActionCaster->CastSpell(p_ActionTarget, p_AuraScript.ActionSpellId, p_AuraScript.Triggered, nullptr, nullptr, p_OriginalCaster->GetGUID());

            break;
        }
        case eAuraScriptActions::RemoveAura:
        {
            if (!p_ActionTarget || !p_ActionCaster)
                return;

            p_ActionTarget->RemoveAurasDueToSpell(p_AuraScript.ActionSpellId, p_ActionCaster->GetGUID());

            break;
        }
        case eAuraScriptActions::ReduceCooldown:
        {
            if (!p_ActionCaster)
                return;

            Player* l_PlayerCaster = p_ActionCaster->ToPlayer();
            SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_AuraScript.TargetSpellId);
            if (!l_PlayerCaster || !l_SpellInfo)
                return;

            uint32 l_CooldownReduction = 0;
            if (p_AuraScript.ActionSpellId && p_AuraScript.DataEffectId != -1)
            {
                switch (p_AuraScript.DataSource)
                {
                    case eDataSources::SourceSpellInfo:
                    {
                        if (SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_AuraScript.ActionSpellId))
                            l_CooldownReduction = l_SpellInfo->Effects[p_AuraScript.DataEffectId].BasePoints;
                        break;
                    }
                    case eDataSources::SourceAura:
                    {
                        if (p_ActionTarget)
                        {
                            if (AuraEffect* l_SourceEffect = p_ActionTarget->GetAuraEffect(p_AuraScript.ActionSpellId, p_AuraScript.DataEffectId))
            ...
            if (l_SpellInfo->ChargeCategoryEntry != 0)
                ...
                l_PlayerCaster->ReduceSpellCooldown(p_AuraScript.TargetSpellId, l_CooldownReduction * IN_MILLISECONDS);

            break;
        }
        case eAuraScriptActions::RemoveCooldown:
        {
            if (!p_ActionCaster)
                return;
            ...
            if (l_SpellInfo->ChargeCategoryEntry != 0)
                ...
                l_PlayerCaster->RemoveSpellCooldown(p_AuraScript.TargetSpellId);

            break;
        }
        case eAuraScriptActions::ModAmount:
        {
            AuraEffect* l_AuraEffect = nullptr;

            if (p_AuraScript.TargetSpellId && p_AuraScript.TargetEffectId != -1)
            {
                if (Aura* l_Aura = GetCaster()->GetAura(p_AuraScript.TargetSpellId))
                    l_AuraEffect = l_Aura->GetEffect(p_AuraScript.TargetEffectId);
            }
            else
                l_AuraEffect = GetEffect(p_AuraScript.EffectId);
            ...
            if (p_AuraScript.ActionSpellId && p_AuraScript.DataEffectId != -1)
            {
                switch (p_AuraScript.DataSource)
                {
                    ...
                    {
                        if (SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(p_AuraScript.ActionSpellId))
                            l_Value = l_SpellInfo->Effects[p_AuraScript.DataEffectId].BasePoints;
                        break;
                    }
                    case eDataSources::SourceAura:
                    {
            ...
            if (l_Value && p_AuraScript.ArtifactTraitId)
            {
                if (p_ActionCaster)
                {
                    if (Player* l_Player = p_ActionCaster->ToPlayer())
                        l_Value *= l_Player->GetRankOfArtifactPowerId(p_AuraScript.ArtifactTraitId);
                }
            }
            ...
            switch (p_AuraScript.CalculationType)
            {
                case eCalculationType::TypeCalcPct:
        ...
        case eAuraScriptActions::SetAmount:
        {
            AuraEffect* l_AuraEffect = nullptr;
            ...
            switch (p_AuraScript.CalculationType)
            {
                case eCalculationType::TypeCalcPct:
                {
                    if (p_ActionTarget)
                    {
                        switch (p_AuraScript.DataStat)
                        {
                            case -1:
                                l_AuraEffect->SetAmount(CalculatePct(p_ActionTarget->GetMaxHealth(), l_Value ));
                                break;
                            default:
                                l_AuraEffect->SetAmount(CalculatePct(p_ActionTarget->GetMaxPower(Powers(p_AuraScript.DataStat)), l_Value ));
                                break;
                        }
                    }
                    break;
                ...
                case eCalculationType::TypeAddPct:
                {
                    if (p_ActionTarget)
                    {
                        ...
                        {
                            case -1:
                                l_AuraEffect->SetAmount(CalculatePct(p_ActionTarget->GetMaxHealth(), 100 + l_Value));
                                break;
                            default:
                                l_AuraEffect->SetAmount(CalculatePct(p_ActionTarget->GetMaxPower(Powers(p_AuraScript.DataStat)), 100 + l_Value));
                                break;
                        }
                    }
                    break;
        ...
        case eAuraScriptActions::ModDuration:
        {
            std::set<Aura*> l_AuraSet;

            if (p_AuraScript.TargetAuraType && GetCaster())
            {
                for (AuraEffect* l_AuraEffect : GetCaster()->GetAuraEffectsByType(AuraType(p_AuraScript.TargetAuraType)))
                {
                    if (Aura* l_Aura = l_AuraEffect->GetBase())
                        l_AuraSet.insert(l_Aura);
                }
            }
            else if (p_AuraScript.TargetSpellId && GetCaster())
            {
                if (Aura* l_Aura = GetCaster()->GetAura(p_AuraScript.TargetSpellId))
                    l_AuraSet.insert(l_Aura);
            }
            else
                l_AuraSet.insert(this);
        ...
        case eAuraScriptActions::SetDuration:
        {
            std::set<Aura*> l_AuraSet;

            if (p_AuraScript.TargetAuraType)
            {
                for (AuraEffect* l_AuraEffect : GetCaster()->GetAuraEffectsByType(AuraType(p_AuraScript.TargetAuraType)))
                {
                    if (Aura* l_Aura = l_AuraEffect->GetBase())
            ...
            else if (p_AuraScript.TargetSpellId)
            {
                if (Aura* l_Aura = GetCaster()->GetAura(p_AuraScript.TargetSpellId))
                    l_AuraSet.insert(l_Aura);
            }
                ...
                switch (p_AuraScript.CalculationType)
                {
                    case eCalculationType::TypeCalcPct:
                    {
                        if (p_ActionTarget)
                        {
                            ...
                            {
                                case -1:
                                    l_Aura->SetDuration((CalculatePct(p_ActionTarget->GetMaxHealth(), l_Value)) * IN_MILLISECONDS);
                                    break;
                                default:
                                    l_Aura->SetDuration((CalculatePct(p_ActionTarget->GetMaxPower(Powers(p_AuraScript.DataStat)), l_Value)) * IN_MILLISECONDS);
                                    break;
                            }
                        }
                        break;
                    ...
                    case eCalculationType::TypeAddPct:
                    {
                        if (p_ActionTarget)
                        {
                            ...
                            {
                                case -1:
                                    l_Aura->SetDuration((CalculatePct(p_ActionTarget->GetMaxHealth(), 100 + l_Value)) * IN_MILLISECONDS);
                                    break;
                                default:
                                    l_Aura->SetDuration((CalculatePct(p_ActionTarget->GetMaxPower(Powers(p_AuraScript.DataStat)), 100 + l_Value)) * IN_MILLISECONDS);
                                    break;
                            }
                        }
                        break;
...
void Aura::PrepareAction(AuraScriptEntry p_AuraScript, AuraApplication const* p_AuraApplication)
{
    Unit* l_ActionCaster = GetScriptActor(p_AuraScript.ActionCaster, p_AuraApplication);
    Unit* l_ActionTarget = GetScriptActor(p_AuraScript.ActionTarget, p_AuraApplication);
    Unit* l_ActionOriginalCaster = GetScriptActor(p_AuraScript.ActionOriginalCaster, p_AuraApplication);

    if (l_ActionCaster && l_ActionOriginalCaster && l_ActionTarget)
        DoAction(p_AuraScript, l_ActionCaster, l_ActionTarget, l_ActionOriginalCaster);
}

void Aura::PrepareProcAction(AuraScriptEntry p_AuraScript, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    Unit* l_ActionCaster = GetScriptActor(p_AuraScript.ActionCaster, p_AuraApplication, p_ProcEventInfo);
    Unit* l_ActionTarget = GetScriptActor(p_AuraScript.ActionTarget, p_AuraApplication, p_ProcEventInfo);
    Unit* l_ActionOriginalCaster = GetScriptActor(p_AuraScript.ActionOriginalCaster, p_AuraApplication, p_ProcEventInfo);

    if (l_ActionCaster && l_ActionOriginalCaster && l_ActionTarget)
        DoAction(p_AuraScript, l_ActionCaster, l_ActionTarget, l_ActionOriginalCaster);
}

Unit* Aura::GetScriptActor(uint32 p_ScriptActor, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo)
{
    switch (p_ScriptActor)
    {
        ...
        case eScriptActors::Target:
            return p_AuraApplication->GetTarget();
            break;
        case eScriptActors::ProcActor:
            return p_ProcEventInfo.GetActor();
            break;
        case eScriptActors::ProcTarget:
            return p_ProcEventInfo.GetActionTarget();
        default:
            break;
    }
...
Unit* Aura::GetScriptActor(uint32 p_ScriptActor, AuraApplication const* p_AuraApplication)
{
    switch (p_ScriptActor)
    {
        case eScriptActors::Caster:
...
void Aura::SetPeriodicHealDone(uint32 p_Heal)
{
    m_PeriodicHealDone = p_Heal;
}

uint32 Aura::GetPeriodicHealDone() const
{
    return m_PeriodicHealDone;
}

void Aura::SetLastProcAttemptTime(uint32 lastProcAttemptTime)
{
    if (GetUnitOwner()->IsPlayer())
        GetUnitOwner()->ToPlayer()->SetLastProcAttemptTime(GetId(), lastProcAttemptTime);
    else
        m_lastProcAttemptTime = lastProcAttemptTime;
}

void Aura::SetLastProcSuccessTime(uint32 lastProcSuccessTime)
{
    if (GetUnitOwner()->IsPlayer())
        GetUnitOwner()->ToPlayer()->SetLastProcSuccessTime(GetId(), lastProcSuccessTime);
    else
        m_lastProcSuccessTime = lastProcSuccessTime;
}

uint32 Aura::GetLastProcAttemptTime() const
{
    if (GetUnitOwner()->IsPlayer())
        return GetUnitOwner()->ToPlayer()->GetLastProcAttemptTime(GetId());
    return m_lastProcAttemptTime;
}

uint32 Aura::GetLastProcSuccessTime() const
{
    if (GetUnitOwner()->IsPlayer())
        return GetUnitOwner()->ToPlayer()->GetLastProcSuccessTime(GetId());
    return m_lastProcSuccessTime;
}
