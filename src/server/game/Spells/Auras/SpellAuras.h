...
class UnsafeSpellOrAuraScript
{
    public:
        UnsafeSpellOrAuraScript()
        {
            Unit::IncreaseUnsafeSpellOrAuraHookThreadLocal();
        }

        ~UnsafeSpellOrAuraScript()
        {
            Unit::ReduceUnsafeSpellOrAuraHookThreadLocal();
        }
};
...
class Aura
{
    ...
    public:
        ...
        void SetApplyTime(time_t p_Time) { m_applyTime = p_Time; }
        ...
        bool IsCastedByAreatrigger() const { return m_isCastedByAreatrigger; }
        void SetIsCastedByAreatrigger(bool set) { m_isCastedByAreatrigger = set; }
        ...
        AuraEffect* GetEffect(uint8 effIndex) const { ASSERT(effIndex < SpellEffIndex::MAX_EFFECTS); return m_EffectCount > effIndex ? m_effects[effIndex] : nullptr; }
        uint8 GetEffectCount() const { return m_EffectCount; }
        uint32 GetEffectMask() const { uint32 effMask = 0; for (uint8 i = 0; i < m_spellInfo->EffectCount; ++i) if (m_effects[i]) effMask |= 1 << i; return effMask; }
        ...
        void SetPeriodicHealDone(uint32 p_Heal);
        ...
        void SetLastProcAttemptTime(uint32 lastProcAttemptTime);
        void SetLastProcSuccessTime(uint32 lastProcSuccessTime);
        uint32 GetLastProcAttemptTime() const;
        uint32 GetLastProcSuccessTime() const;

        void Delink();

        // AuraScript
        void LoadScripts();
        void CallScriptCalcMaxDurationHandlers(int32& p_MaxDuration);
        bool CallScriptCheckAreaTargetHandlers(Unit* target);
        bool CallScriptDispel(DispelInfo* dispelInfo);
        ...
        void CallScriptEffectSplitDamageHandlers(AuraEffect* aurEff, AuraApplication const* aurApp, DamageInfo & dmgInfo, uint32 & splitAmount, bool & defaultPrevented);
        void SetScriptData(uint32 type, uint32 data);
        void SetScriptGuid(uint32 type, uint64 data);
        // Spell Proc Hooks
        bool CallScriptCheckProcHandlers(AuraApplication const* aurApp, ProcEventInfo& eventInfo);
        void CallScriptCalcPPMRate(float & p_ProcPerMinute, ProcEventInfo & p_EventInfo);
        void CallScriptCalcProcChance(float & p_Chance, ProcEventInfo & p_EventInfo);
        ...
        bool CallScriptCanRrefreshProcHandlers(bool p_Decrease);

        void CallDBScriptOnDispel(uint32 p_SpellId);
        void CallDBScriptAfterDispel(uint32 p_SpellId);
        void CallDBScriptAfterApply(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, AuraEffectHandleModes p_Modes);
        void CallDBScriptAfterRemove(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, AuraEffectHandleModes p_Modes);
        bool CallDBScriptOnPeriodic(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication);
        void CallDBScriptAfterPeriodic(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication);
        void CallDBScriptOnAbsorb(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, DamageInfo& p_DamageInfo, uint32& p_AbsorbAmount);
        void CallDBScriptAfterAbsorb(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, DamageInfo& p_DamageInfo, uint32& p_AbsorbAmount);
        bool CallDBScritpCheckProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void CallDBScriptPrepareProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void CallDBScriptProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void CallDBScriptAfterProc(uint32 p_SpellId, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void CallDBScriptEffectProc(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void CallDBScriptAfterEffectProc(uint32 p_SpellId, AuraEffect const* p_AuraEffect, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        void DoAction(AuraScriptEntry p_AuraScript, Unit* p_ActionCaster, Unit* p_ActionTarget, Unit* p_OriginalCaster);
        void PrepareAction(AuraScriptEntry p_AuraScript, AuraApplication const* p_AuraApplication);
        void PrepareProcAction(AuraScriptEntry p_AuraScript, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        Unit* GetScriptActor(uint32 p_ScriptActor, AuraApplication const* p_AuraApplication, ProcEventInfo& p_ProcEventInfo);
        Unit* GetScriptActor(uint32 p_ScriptActor, AuraApplication const* p_AuraApplication);

        void ChangeWindDirection(G3D::Vector3 p_NewDirection);

        void InitCastGUID();
        Guid128 const& GetCastGUID() const { return m_CastGUID; };
        ...
        void SetSpellXSpellVisualID(uint32 XspellVisualId) { m_SpellXSpellVisualID = XspellVisualId; }
