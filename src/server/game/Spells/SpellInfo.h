...
enum SpellCastTargetFlags
{
    ...
    TARGET_FLAG_ITEM_HEIRLOOM     = 0x00200000,               /// Item Heirloom
...
enum SpellSpecificType
{
    ...
    SpellSpecificAncestralVigor,
    SpellSpecificGreaterBlessing
};
...
enum SpellCustomAttributes
{
    ...
    SPELL_ATTR0_CU_SHARE_PLAYER_DAMAGE              = 0x04000000,
    SPELL_ATTR0_CU_ALLOW_INDOOR_MOUNT               = 0x08000000,
    SPELL_ATTR0_CU_NEGATIVE_AREATRIGGER             = 0x10000000,
    SPELL_ATTR0_CU_AREATRIGGER_BIG_OBJ              = 0x20000000,
    SPELL_ATTR0_CU_ALWAYS_CRITS                     = 0x40000000, ///< Spell will always crit. Handled in Unit::GetUnitSpellCriticalChance
...
public:
    ...
    /// Custom - only in core, handled by SpellMgr.cpp
    bool        m_CanBeStacked;
    bool        m_AmountNotAlterByStacks;
    ...
    bool IsAura() const;
    bool IsHealingEffect() const;
    bool IsShieldingEffect() const;
    bool IsHealingOrShieldingEffect() const;
    ...
    bool IsApplyingAura() const;
    bool IsDamagingEffect() const;
    bool IsPeriodicEffect() const;
    bool CanScale() const;

    int32 CalcValue(Unit const* p_Caster = nullptr, int32 const* p_BasePoints = nullptr, Unit const* p_Target = nullptr, Item const* p_CastItem = nullptr, bool p_Log = false, std::ostringstream* p_DamageLogStream = nullptr) const;
    ...
    float CalcRadius(Unit* caster = nullptr, Spell* = nullptr, bool Area = false) const;
...
static SpellEffectInfo* k_NullEffect = nullptr;

class SpellInfoEffect
{
public:
    SpellEffectInfo& operator[](uint32 p_Effect)
    {
        if (_effects.size() <= p_Effect)
        {
            if (!k_NullEffect)
                k_NullEffect = new SpellEffectInfo();
            return *k_NullEffect;
        }

        return _effects[p_Effect];
    }

    SpellEffectInfo const& operator[](uint32 p_Effect) const
    {
        if (_effects.size() <= p_Effect)
        {
            if (!k_NullEffect)
...
public:
    ...
    uint64 AuraInterruptFlags2;
    uint64 ChannelInterruptFlags;
    uint64 ChannelInterruptFlags2;
    uint32 ProcFlags;
    uint32 ProcChance;
    uint32 ProcCharges;
    uint32 ProcCooldown;
    ...
    LocalizedString* SpellName;
    LocalizedString* Rank;
    ...
    /// Update Criteria optimizations
    bool CanUpdateCriteriaBeSpellTarget;
    bool CanUpdateCriteriaBeSpellTarget2;
    bool CanUpdateCriteriaCastSpell;
    bool CanUpdateCriteriaCastSpell2;
    bool CanUpdateCriteriaTimedSpellTarget;
    bool CanUpdateCriteriaTimedSpellCaster;
    ...
    SpellInfo(SpellEntry const* spellEntry, uint32 difficulty, SpellVisualMap const& p_Visuals);
    ~SpellInfo();
    ...
    uint16 GetPrimaryProfessionSkill() const;
    ...
    bool CanPierceCloakOfShadows(Unit* p_Caster, Unit* p_Target, bool p_Triggered = false) const;
    ...
    bool HasEffectMechanic(Mechanics p_Mechanic) const
    {
        uint64 l_AllEffectMask = 0;
        for (int l_I = 0; l_I < EffectCount; ++l_I)
        {
            if (Effects[l_I].IsEffect() && Effects[l_I].Mechanic)
                l_AllEffectMask |= 1LL << Effects[l_I].Mechanic;
        }

        return l_AllEffectMask & (1LL << p_Mechanic);
    }
    ...
    SpellPowerEntry const* GetSpellPower(Powers p_PowerType) const;
    ...
    bool IsFeralDruidPeriodicDamage() const;
    bool IsUnstableAffliction() const;
    bool IsNonSnapshottingAura() const;
    ...
    bool IsNonPvP(Unit* p_Caster = nullptr, Unit* p_Target = nullptr) const;
    ...
    bool _IsCrowdControl(uint32 p_EffMask = 0, bool p_NoDamage = false) const;
    bool _IsLossOfControl() const;
