...
enum SpellCastFlags
{
    ...
    CAST_FLAG_DONT_RESET_PENDING = 0x00000040,
...
class DelayTriggerSpell : public BasicEvent
{
    public:

        explicit DelayTriggerSpell(Unit* p_Caster, uint64 p_TargetGUID, SpellCastTargets p_Targets, SpellInfo const* p_SpellInfo, CustomSpellValues p_Values, uint64 p_OriginalCasterGUID, Item* p_CastItem) : m_Caster(p_Caster), m_TargetGUID(p_TargetGUID), m_Targets(p_Targets), m_SpellInfo(p_SpellInfo), m_Values(p_Values), m_OriginalCasterGUID(p_OriginalCasterGUID), BasicEvent()
        {
            m_CastItemGuid = p_CastItem ? p_CastItem->GetGUID() : 0;
        }

        bool Execute(uint64 /*p_CurrTime*/, uint32 /*p_Diff*/)
        {
            Unit* l_Target = ObjectAccessor::GetUnit(*m_Caster, m_TargetGUID);

            if (l_Target == nullptr)
                return true;

            m_Targets.SetUnitTarget(l_Target);

            if (Player* l_Player = m_Caster->ToPlayer())
            {
                if (Item* l_CastItem = l_Player->GetItemByGuid(m_CastItemGuid))
                {
                    m_Caster->CastSpell(m_Targets, m_SpellInfo, &m_Values, TRIGGERED_FULL_MASK, l_CastItem, nullptr, m_OriginalCasterGUID);
                    return true;
                }
            }

            m_Caster->CastSpell(m_Targets, m_SpellInfo, &m_Values, TRIGGERED_FULL_MASK, nullptr, nullptr, m_OriginalCasterGUID);
            return true;
        }
...
public:
    ...
    void EffectCompleteWorldQuest(SpellEffIndex effIndex);
    ...
    void EffectCreateItemLoot(SpellEffIndex effIndex);
    ...
    void EffectPlaySpellScene(SpellEffIndex effIndex);
    ...
    void EffectEarnArtifactPower(SpellEffIndex p_EffIndex);
    void EffectEarnFishingArtifactPower(SpellEffIndex p_EffIndex);
    ...
    void EffectJumpDest2(SpellEffIndex p_EffIndex);
    void EffectCollectAppearance(SpellEffIndex p_EffIndex);
    void EffectLaunchQuestChoice(SpellEffIndex p_EffIndex);
    ...
    static uint64 GenerateNewSpellGUID();
    ...
    template<class SEARCHER> static void SearchTargets(SEARCHER& searcher, uint32 containerMask, Unit* referer, Position const* pos, float radius);
    ...
    void SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionContainer* condList, bool p_IgnoreCasterSize = false, bool p_IgnoreTargetSize = true);
    ...
    int32 CalculateDamage(uint8 i, Unit const* target, bool p_Log = false, std::ostringstream* p_LogDamageStream = nullptr) const
    {
        Unit* l_Caster = m_originalCaster ? m_originalCaster : m_caster;
        return l_Caster->CalculateSpellDamage(target, m_spellInfo, i, &m_spellValue->EffectBasePoints[i], m_CastItem, p_Log, p_LogDamageStream);
    }
    ...
    static void SendCastResult(Player* caster, SpellInfo const* spellInfo, Guid128 p_CastGuid, SpellCastResult result, SpellCustomErrors customError = SPELL_CUSTOM_ERROR_NONE);
    ...
    void SendInterrupted(uint32 result);
    ...
    void SetCastTime(int32 p_CastTime) { m_casttime = p_CastTime; };
    ...
    Unit* GetUnitTarget() const { return m_targets.GetUnitTarget() ? m_targets.GetUnitTarget() : unitTarget; }

    void SetPeriodicDamageModifier(float newModifier) { m_periodicDamageModifier = newModifier; }

    void AppendProcExFlag(ProcFlagsExLegacy p_Flag)
    {
        m_procEx |= p_Flag;
    };
    ...
    bool IsSingleEnnemyTargetSpell() const
    {
        int32 l_TargetCount = 0;
        for (TargetInfo const& l_TargetInfo : m_UniqueTargetInfo)
            if (l_TargetInfo.targetGUID != m_originalCasterGUID)
                ++l_TargetCount;
        return l_TargetCount == 1 ? true : false;
    }
    ...
    Guid128 GetCastGuid() const { return m_CastID; }
    void SetCastGUID(Guid128& p_CastGuid) { m_CastID = p_CastGuid; }

    SpellValue * const m_spellValue;

    void SetTriggerCastData(TriggerCastData p_TriggerCastData);

    TriggerCastData m_TriggerCastData;

    uint32 m_ArtifactKnowledgeLevelOfCastItem;

    void SetChanneledDuration(uint32 p_Duration) { m_channeledDuration = p_Duration; }
    uint32 GetChanneledDuration() const { return m_channeledDuration; }

    std::ostringstream* m_CurrentLogDamageStream;
    bool   m_ShoudLogDamage;
    uint32 m_MinDamageLog;

    WorldLocation* destTarget;

    Aura const* GetSpellAura() const { return m_spellAura; }
    WorldLocation const* GetDstTarget() const { return m_targets.GetDstPos(); }
    void SetDstTarget(Position* p_Target) { m_targets.SetDst(*p_Target); }

    uint32 GetSpellXSpellVisualID() const { return m_SpellVisualID; }

    bool IsPowerFullAura(SpellInfo const* p_SpellProto, Unit* caster, Unit* target);
...
protected:
    ...
    bool CanSpellProc(Unit* p_Target, uint32 p_Mask) const;

    void SendLoot(uint64 guid, LootType loottype);
    void UpdateSpellCastDataAmmo(uint32& p_AmmoDisplayID);

    bool BypassAuraCheckForAutoRepeatSpell();
    ...
    void AddEffectTarget(uint64 targetGuid) { m_effect_targets.insert(targetGuid); }
    void RemoveEffectTarget(uint64 targetGuid) { m_effect_targets.erase(targetGuid); }
    ...
    void CallScriptTakePowersHandlers(Powers p_PowerType, int32& p_PowerCost);
    void CallScriptCalculateThreatHandlers(Unit* p_Target, float& p_Threat);
    void CallScriptInterruptHandlers(uint32 p_Time);
    ...
    void CallDBScriptBeforeCast(uint32 p_SpellId);
    void CallDBScriptOnCast(uint32 p_SpellId);
    void CallDBScriptAfterCast(uint32 p_SpellId);
    void CallDBScriptOnPrepare(uint32 p_SpellId);
    SpellCastResult CallDBScriptCheckCast(uint32 p_SpellId);
    void CallDBScriptTakePowers(uint32 p_SpellId, Powers p_PowerType, int32& p_PowerCost);
    void CallDBScriptOnEffectHit(uint32 p_SpellId, SpellEffIndex p_EffectIndex);
    void CallDBScriptOnEffectHitTarget(uint32 p_SpellId, SpellEffIndex p_EffectIndex);
    void CallDBScriptBeforeHit(uint32 p_SpellId);
    void CallDBScriptOnHit(uint32 p_SpellId);
    void CallDBScriptAfterHit(uint32 p_SpellId);
    void DoAction(SpellScriptEntry p_SpellScript);
    Unit* GetScriptActor(SpellScriptEntry p_SpellScript, uint32 p_ScriptActor);
    bool IsAnEffectHook(SpellScriptEntry p_SpellScript);
    ...
    // effect helpers
    std::list<TempSummon*> SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numSummons);
...
namespace JadeCore
{
    ...
    {
        bool _ignoreSearcherSize;
        bool _ignoreTargetSize;
        float _range;
        Position const* _position;
    ...
    struct WorldObjectSpellWidthTargetCheck : public WorldObjectSpellAreaTargetCheck
    {
        ...
        WorldObjectSpellWidthTargetCheck(float p_Width, float p_Range, Unit* p_Caster, Unit* p_Referer,
