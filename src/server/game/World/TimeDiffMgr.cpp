...
void TimeDiffMgr::Update(uint32 diff)
{
    ...
    if (GetMSTimeDiffToNow(m_worldUpdate[INTERVAL_1_MINUTE]) >= MINUTE * IN_MILLISECONDS)
    {
        ...
        PreparedStatement* l_Statement = LogDatabase.GetPreparedStatement(LOG_INSERT_TIME_DIFF_LOG);
        l_Statement->setUInt32(0, m_prevLog[INTERVAL_1_MINUTE].Average);
        l_Statement->setUInt32(1, m_prevLog[INTERVAL_1_MINUTE].Max);
        l_Statement->setUInt32(2, m_prevLog[INTERVAL_1_MINUTE].Players);
