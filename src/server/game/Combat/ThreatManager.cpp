...
HostileReference* ThreatContainer::selectNextVictim(Creature* attacker, HostileReference* currentVictim)
{
    ...
    for (std::list<HostileReference*>::const_iterator iter = iThreatList.begin(); iter != iThreatList.end() && !found;)
    {
        ...
        if (attacker->canCreatureAttack(target) && attacker->ToCreature()->_IsTargetAcceptable(target))
        {
...
void ThreatManager::doAddThreat(Unit* p_Victim, float p_Threat)
{
    uint32 reducedThreadPercent = p_Victim->GetReducedThreatPercent();

    if (p_Threat > 0.0f)
    {
        if (Creature* creature = iOwner->ToCreature())
        {
            if (p_Victim->GetTypeId() == TYPEID_PLAYER && !iOwner->GetThreatTarget(p_Victim->GetGUID()))
            {
                iOwner->AddThreatTarget(p_Victim->GetGUID());

                if (creature->IsPersonal() && !creature->GetMap()->IsMythic())
    ...
    if (p_Threat > 0.0f && reducedThreadPercent)
    {
        Unit* redirectTarget = p_Victim->GetMisdirectionTarget();
        float reducedThreat = p_Threat * reducedThreadPercent / 100.0f;
        p_Threat -= reducedThreat;
        if (redirectTarget)
            _addThreat(redirectTarget, reducedThreat);
    }

    _addThreat(p_Victim, p_Threat);
}
