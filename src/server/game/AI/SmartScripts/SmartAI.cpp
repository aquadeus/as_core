...
void SmartAI::EndPath(bool fail)
{
    ...
    ObjectGuidList* targets = GetScript()->GetTargetList(SMART_ESCORT_TARGETS);
    if (targets && mEscortQuestID)
    {
        if (targets->size() == 1 && GetScript()->IsPlayer(ObjectAccessor::GetWorldObject(*me, (*targets->begin()))))
        {
            Player* player = ObjectAccessor::GetWorldObject(*me, (*targets->begin()))->ToPlayer();
            ...
            {
                for (auto& l_Member : group->GetMemberSlots())
                {
        ...
        {
            for (ObjectGuidList::iterator iter = targets->begin(); iter != targets->end(); ++iter)
            {
                if (GetScript()->IsPlayer(ObjectAccessor::GetWorldObject(*me, (*iter))))
                {
                    Player* player = ObjectAccessor::GetWorldObject(*me, (*iter))->ToPlayer();
                    if (!fail && player->IsAtGroupRewardDistance(me) && !player->HasCorpse())
...
bool SmartAI::IsEscortInvokerInRange()
{
    ...
    if (targets)
    {
        if (targets->size() == 1 && GetScript()->IsPlayer(ObjectAccessor::GetWorldObject(*me, ((*targets->begin())))))
        {
            ...
            if (GroupPtr group = player->GetGroup())
            {
                bool l_Exist = group->GetMemberSlots().exist([&](Group::MemberSlotPtr l_Itr)
                {
                    Player* groupGuy = l_Itr->player;

                    if (groupGuy && me->GetDistance(groupGuy) <= SMART_ESCORT_MAX_PLAYER_DIST)
                        return true;
        ...
        else
        {
            for (ObjectGuidList::iterator iter = targets->begin(); iter != targets->end(); ++iter)
            {
                if (GetScript()->IsPlayer(ObjectAccessor::GetWorldObject(*me, (*iter))))
                {
                    if (me->GetDistance(ObjectAccessor::GetWorldObject(*me, (*iter))->ToPlayer()) <= SMART_ESCORT_MAX_PLAYER_DIST)
                        return true;
                }
            }
        }
...
void SmartAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(SMART_ESCORT_ESCORTING))
    {
        if (me->GetMotionMaster()->GetMotionSlotType(MOTION_SLOT_ACTIVE) != POINT_MOTION_TYPE)  /// Continue instantly
        {
    ...
    else if (!me->GetOwner() || !me->GetOwner()->IsPlayer() || !me->GetVehicleKit() || me->GetVehicleKit() != me->GetOwner()->GetVehicle())
...
void SmartAI::AttackStart(Unit* who, bool p_Melee /*= false*/)
{
...
void SmartAI::DamageTaken(Unit* doneBy, uint32& damage, SpellInfo const*)
{
    ...
    if (mInvincibilityHpLevel && (damage >= me->GetHealth(doneBy) - mInvincibilityHpLevel))
    {
        damage = 0;
...
{
    GetScript()->ProcessEventsFor(SMART_EVENT_JUST_SUMMONED, summoner->IsUnit() ? summoner->ToUnit() : nullptr, 0, 0, false, nullptr, summoner->IsGameObject() ? summoner->ToGameObject() : nullptr);
}

void SmartAI::DamageDealt(Unit* doneTo, uint32& damage, DamageEffectType /*damagetype*/, SpellInfo const* /*p_SpellInfo*/)
{
...
void SmartAI::sOnAuraAddedToTarget(Unit* p_Unit, uint32 p_SpellId)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_ADD_AURA, p_Unit, p_SpellId);
}

void SmartAI::sOnAuraRemovedFromTarget(Unit* p_Unit, uint32 p_SpellId)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_REMOVE_AURA, p_Unit, p_SpellId);
}
...
class SmartEventObject : public EventObjectScript
{
    public:
        ...
        bool OnTrigger(Player* p_Player, EventObject* p_Event)
        {
            if (!p_Player->isAlive())
                return false;

            sLog->outDebug(LOG_FILTER_DATABASE_AI, "EventObject %u is using SmartEventObject script", p_Event->GetEntry());
            SmartScript l_Script;
            l_Script.OnInitialize(p_Event, nullptr);
            l_Script.ProcessEventsFor(SMART_EVENT_EVENTOBJECT_ONTRIGGER, p_Player, p_Event->GetEntry());
            return true;
        }
};
