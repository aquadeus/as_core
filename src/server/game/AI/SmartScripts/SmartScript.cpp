...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_TALK:
        {
            ...
            mUseTextTimer = true;
            if (e.action.talk.isPersonalText)
                sCreatureTextMgr->SendChat(talker, uint8(e.action.talk.textGroupID), mTextGUID, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_NORMAL, 0, TEAM_OTHER, false, nullptr, true);
            else
        ...
        case SMART_ACTION_ADD_QUEST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    ...
                    {
                        (*itr)->ToPlayer()->AddQuest(q, nullptr);
        ...
        case SMART_ACTION_CAST:
        {
            if (player)
            {
                player->CastSpell(player, e.action.cast.spell, (e.action.cast.flags & SMARTCAST_TRIGGERED) ? true : false);
                break;
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    {
                        if (e.action.cast.targetType)
                        {
                            me->AttackStop();
                            me->SetFacingTo(me->GetOrientation());
                            me->ToCreature()->SetReactState(ReactStates::REACT_PASSIVE);
        ...
        case SMART_ACTION_ADD_AURA:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    uint32 l_StackAmount = e.action.addaura.stackAmount ? e.action.addaura.stackAmount : 1;
                    if (Aura* l_Aura = (*itr)->ToUnit()->GetAura(e.action.addaura.spellId, (*itr)->ToUnit()->GetGUID()))
                        l_StackAmount += l_Aura->GetStackAmount();

                    (*itr)->ToUnit()->SetAuraStack(e.action.addaura.spellId, (*itr)->ToUnit(), l_StackAmount);
                    sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding aura %u to unit %u",
                        e.action.addaura.spellId, (*itr)->GetGUIDLow());
                }
            }
        ...
        {
            std::set<Player*> l_Players;
            std::set<uint64> l_Groups;
            if (me)
            {
                for (auto& l_Itr : me->m_LootContainers)
                {
                    uint64 l_Guid = l_Itr.first;

                    if (IS_GROUP(l_Guid))
                    {
                        if (l_Groups.count(l_Guid))
                            continue;

                        GroupPtr l_Group = sGroupMgr->GetGroupByGUID(l_Guid);


                        if (l_Group)
                        {
                            Player* l_First = nullptr;

                            l_Group->GetMemberSlots().foreach_until([&](Group::MemberSlotPtr l_Itr) -> bool
                            {
                                if (l_Itr->player)
                                {
                                    l_First = l_Itr->player;
                                    return true;
                                }
                            ...
                            if (l_First)
                            {
                                l_Players.insert(l_First);
                                l_Groups.insert(l_Guid);
                            }
                        }
            ...
            if (me && !l_Players.empty())
            {
                for (Player* l_Player : l_Players)
                    l_Player->RewardPlayerAndGroupAtEvent(e.action.killedMonster.creature, l_Player);
            }
        ...
        case SMART_ACTION_FORCE_DESPAWN:
        {
            ...
            for (auto& target : *targets)
            {
                if (target->IsCreature())
                    target->ToCreature()->DespawnOrUnsummon(e.action.forceDespawn.delay ? e.action.forceDespawn.delay : 100);
                else if (target->IsGameObject())
                {
                    if (auto _go = target->ToGameObject())
                    {
                        _go->SendObjectDeSpawnAnim(_go->GetGUID());
                        _go->SetRespawnTime(e.action.forceDespawn.delay);
                        _go->UpdateObjectVisibility();
                    }
                }
            }
            delete targets;
        ...
        case SMART_ACTION_MOD_CURRENCY:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (Player* player = (*itr)->ToPlayer())
                    player->ModifyCurrency(e.action.modifyCurrency.currencyId, 
                        e.action.modifyCurrency.increaseCount ? e.action.modifyCurrency.increaseCount : -(int32)e.action.modifyCurrency.decreaseCount, true, e.action.modifyCurrency.ignoreMultipliers != 0);
        ...
        case SMART_ACTION_SET_INVINCIBILITY_HP_LEVEL:
        {
            ...
            if (e.action.invincHP.percent)
                ...
                ai->SetInvincibilityHpLevel(uint64(e.action.invincHP.minHP));
            break;
        }
        ...
        case SMART_ACTION_SUMMON_FROM_TARGET:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (targets)
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    if (e.GetActionType() == SMART_ACTION_SUMMON_CREATURE && unit && unit->ToPlayer() && e.action.summonCreature.personalVisible)
                        summon = unit->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, unit->GetGUID());
                    else if (e.GetActionType() == SMART_ACTION_SUMMON_CREATURE)
                        summon = GetBaseObject()->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration);
                    else if (e.GetActionType() == SMART_ACTION_SUMMON_FROM_TARGET)
                        summon = (*itr)->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration);
            ...
            if (e.GetTargetType() == SMART_TARGET_RANDOM_POSITION)
            {
                ...
                if (e.GetActionType() == SMART_ACTION_SUMMON_CREATURE && unit && unit->ToPlayer() && e.action.summonCreature.personalVisible)
                    unit->SummonCreature(e.action.summonCreature.creature, pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ(), angle, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, unit->GetGUID());
                else
            ...
            if (e.GetTargetType() == SMART_TARGET_POSITION)
            {
                if (e.GetActionType() == SMART_ACTION_SUMMON_CREATURE && unit && unit->ToPlayer() && e.action.summonCreature.personalVisible)
                {
                    if (Creature* summon = unit->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, unit->GetGUID()))
                        if (unit && e.action.summonCreature.attackInvoker)
                            summon->AI()->AttackStart(unit);
                }
                else if (Creature* summon = GetBaseObject()->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
                    if (unit && e.action.summonCreature.attackInvoker)
                        summon->AI()->AttackStart(unit);
            }
            break;
        ...
        case SMART_ACTION_SUMMON_GO:
        {
            ...
            if (targets)
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    z += e.target.z;
                    o += e.target.o;
                    if (unit && unit->ToPlayer() && e.action.summonGO.personalVisible)
                        unit->SummonGameObject(e.action.summonGO.entry, x, y, z, o, 0, 0, 0, 0, e.action.summonGO.despawnTime, unit->GetGUID());
                    else
            ...
            if (unit && unit->ToPlayer() && e.action.summonGO.personalVisible)
                unit->SummonGameObject(e.action.summonGO.entry, e.target.x, e.target.y, e.target.z, e.target.o, 0, 0, 0, 0, e.action.summonGO.despawnTime, unit->GetGUID());
            else GetBaseObject()->SummonGameObject(e.action.summonGO.entry, e.target.x, e.target.y, e.target.z, e.target.o, 0, 0, 0, 0, e.action.summonGO.despawnTime);
            break;
        }
        case SMART_ACTION_KILL_UNIT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (!IsUnit(*itr) || ((*itr)->ToUnit()->IsPlayer() && (*itr)->ToUnit()->ToPlayer()->isGameMaster()))
                    continue;
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            if (e.GetTargetType() == SMART_TARGET_RANDOM_POSITION)
            {
                ...
                float l_ObjSize = me->GetObjectSize();
                float l_Dist = l_ObjSize + (e.target.randomPos.range - l_ObjSize) * (float)rand_norm();
                Position l_Pos;
                me->GetFirstCollisionPosition(l_Pos, l_Dist, l_Angle);
                me->GetMotionMaster()->MovePoint(e.action.MoveToPos.pointId, l_Pos);
                break;
            }
        ...
        case SMART_ACTION_MOVE_FORWARD:
        {
            ...
            me->SetWalk(e.action.moveAction.run == 0);
        ...
        case SMART_ACTION_RAND_TAKEOFF:
        {
            auto targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (auto& target : *targets)
            {
                if (auto unit = target->ToUnit())
                {
                    ...
                    auto f_rand = 0.f;
                    if (e.target.y)
                        f_rand = frand(e.target.x+0.1f, e.target.y);
                    unit->GetNearPosition(rPos, e.action.moveAction.range, f_rand ? f_rand : frand(0.f, M_PI));
                    unit->GetMotionMaster()->MoveTakeoff(1, rPos.GetPositionX(), rPos.GetPositionY(), rPos.GetPositionZ() + e.action.moveAction.rangeZ);
                }
            }
            delete targets;
            break;
        ...
        case SMART_ACTION_SEND_TARGET_TO_TARGET:
        {
            ...
            ObjectGuidList* storedTargets = GetTargetList(e.action.sendTargetToTarget.id);
            if (!storedTargets)
            {
                delete targets;
                break;
            ...
            ObjectList* l_ObjectList = new ObjectList();

            for (uint64 l_Guid : *storedTargets)
            {
                if (IS_GAMEOBJECT_GUID(l_Guid))
                {
                    if (GameObject* l_GameObject = HashMapHolder<GameObject>::Find(l_Guid))
                        l_ObjectList->push_back(l_GameObject);
                }
                else if (IS_CREATURE_GUID(l_Guid))
                {
                    if (Creature* l_Creature = HashMapHolder<Creature>::Find(l_Guid))
                        l_ObjectList->push_back(l_Creature);
                }
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                    if (SmartAI* ai = CAST_AI(SmartAI, (*itr)->ToCreature()->AI()))
                        ai->GetScript()->StoreTargetList(l_ObjectList, e.action.sendTargetToTarget.id);   // store a copy of target list
                    else
            ...
            if (ObjectList* targets = GetTargets(e, unit))
            {
                for (WorldObject* const obj : *targets)
                {
                    if (IsUnit(obj))
                    {
                        ...
                        sLog->outDebug(LOG_FILTER_SQL, "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_SOUND: target: %s (" UI64FMTD "), sound: %u, onlyself: %s",
        ...
        case SMART_ACTION_ADD_THREAT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                if (IsUnit(*itr))
                    (*itr)->ToUnit()->getThreatManager().addThreat((*itr)->ToUnit(), float(e.action.threatPCT.threatINC) - float(e.action.threatPCT.threatDEC));
            delete targets;
            break;
        }
        ...
        case SMART_ACTION_SEND_EVENT_SCENARIO:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    ...
                    uint32 l_ScenarioEventID = e.action.sendScenarioEvent.ScenarioEventID;

                    uint32 l_Count = std::max((uint32)1, e.action.sendScenarioEvent.count);

                    for (int l_I = 0; l_I < l_Count; l_I++)
                    {
                        l_Scenario->UpdateAchievementCriteria(CriteriaTypes::CRITERIA_TYPE_SEND_EVENT_SCENARIO, l_ScenarioEventID, 0, 0, l_Player, l_Player);
                        l_Scenario->UpdateAchievementCriteria(CriteriaTypes::CRITERIA_TYPE_SEND_EVENT, l_ScenarioEventID, 0, 0, l_Player, l_Player);
                    }
                }
            }
            delete targets;
        ...
        case SMART_ACTION_UPDATE_PLOT_INSTANCE:
        {
            ...
            if (l_GarrisonAI != nullptr && l_GarrisonAI->GetOwner() != nullptr && l_GarrisonAI->GetOwner()->GetGarrison() != nullptr)
                l_GarrisonAI->GetOwner()->GetGarrison()->GetDraenorGarrison()->UpdatePlot(l_GarrisonAI->GetPlotInstanceID());
#endif
            break;
        }
        case SMART_ACTION_PLAY_SCENE_OBJECT:
        {
            ...
            ObjectList* l_Targets = GetTargets(e, unit);
            if (!l_Targets)
                break;

            for (ObjectList::const_iterator l_Itr = l_Targets->begin(); l_Itr != l_Targets->end(); ++l_Itr)
            {
                if (Player* l_Player = (*l_Itr)->ToPlayer())
                {
        ...
        case SMART_ACTION_STOP_SCENE_OBJECT:
        {
            if (!GetBaseObject())
                break;
            ...
            for (ObjectList::const_iterator l_Itr = l_Targets->begin(); l_Itr != l_Targets->end(); ++l_Itr)
            {
                if (Player* l_Player = (*l_Itr)->ToPlayer())
                {
                    uint32 l_ID = l_Player->GetSceneInstanceIDByPackage(e.action.stopSceneObject.SceneID);

                    if (l_ID)
                        l_Player->CancelStandaloneScene(l_ID);
                }
            }
        ...
        case SMART_ACTION_BOSS_EVADE:
        {
            ...
            {
                if (uint32 l_BossID = me->GetBossID())
                    instance->SetBossState(l_BossID, NOT_STARTED);
            }

            me->SetPhaseMask(2, true);

            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_BOSS_EVADE: Creature %u EnterEvadeMode", me->GetGUIDLow());
            return;
        }
        case SMART_ACTION_BOSS_ANOUNCE:
        {
            ...
            LocaleConstant l_Locale;

            if (e.action.Announce.Locale)
                l_Locale = LocaleConstant(e.action.Announce.Locale);
            else
                l_Locale = DEFAULT_LOCALE;

            std::string l_Text = sObjectMgr->GetTrinityString(e.action.Announce.TextID, l_Locale);

            sWorld->SendWorldText(e.action.Announce.IDSample, l_Text.c_str());

            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_BOSS_ANOUNCE: Creature %u Announce", me->GetGUIDLow());
            return;
        }
        case SMART_ACTION_MOVE_Z:
        {
            ...
            me->GetPosition(l_X, l_Y, l_Z, l_O);

            l_X += float(e.action.MoveToZ.TargetX);
            l_Y += float(e.action.MoveToZ.TargetY);
            l_Z += float(e.action.MoveToZ.TargetZ);

            me->GetMotionMaster()->MovePoint(0, l_X, l_Y, l_Z);

            /// TODO: Resume path when reached jump location
            break;
        }
        case SMART_ACTION_SET_ID:
        {
            ...
            if (!l_Players.isEmpty())
            {
                for (Map::PlayerList::const_iterator l_Iter = l_Players.begin(); l_Iter != l_Players.end(); ++l_Iter)
                {
                    if (Player* l_Player = l_Iter->getSource())
                        me->GetMap()->ToInstanceMap()->PermBindAllPlayers(l_Player);
                }
            }

            break;
        }
        case SMART_ACTION_SET_SCENARIO_ID:
        {
            /// NYI
            break;
            if (!GetBaseObject())
            ...
            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SET_SCENATIO_ID: scenarioId %d", e.action.Scenario.ID);

            ObjectList* l_Targets = GetTargets(e, unit);
            if (!l_Targets)
                break;

            /*for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
            {
                if (Player* l_Player = (*l_Iter)->ToPlayer())
                    l_Player->SetScenarioId(e.action.Scenario.ID);
            }*/
        ...
        case SMART_ACTION_UPDATE_ACHIEVEMENT_CRITERIA:
        {
            ...
            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_UPDATE_ACHIEVEMENT_CRITERIA: type %d misc1 %d misc2 %d misc3 %d",
            ...
            {
                if (Player* l_Player = (*l_Iter)->ToPlayer())
                    l_Player->UpdateCriteria(CriteriaTypes(e.action.AchievementCriteria.Type), e.action.AchievementCriteria.Misc1, e.action.AchievementCriteria.Misc2, e.action.AchievementCriteria.Misc3, unit);
            }
        ...
        case SMART_ACTION_SUMMON_CONVERSATION:
        {
            ...
            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_CONVERSATION: Conversation %d", e.action.Conversation.ID);

            if (e.action.Conversation.TargetX || e.action.Conversation.TargetY || e.action.Conversation.TargetZ)
            {
                Position l_Pos;
                l_Pos.Relocate(float(e.action.Conversation.TargetX), float(e.action.Conversation.TargetY), float(e.action.Conversation.TargetZ), 0.0f);

                Conversation* l_Conversation = new Conversation;
                if (!l_Conversation->CreateConversation(sObjectMgr->GenerateLowGuid(HighGuid::HIGHGUID_CONVERSATION), e.action.Conversation.ID, GetBaseObject()->ToUnit(), nullptr, l_Pos))
                {
                    l_Conversation->CleanBeforeGC();
                    sGarbageCollector->Add(l_Conversation);
                }
            }
            else
            {
                ...
                for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
                {
                    if (Player* l_Player = (*l_Iter)->ToPlayer())
                    {
                        Position pos = *l_Player;

                        Conversation* l_Conversation = new Conversation;
                        if (!l_Conversation->CreateConversation(sObjectMgr->GenerateLowGuid(HighGuid::HIGHGUID_CONVERSATION), e.action.Conversation.ID, l_Player, nullptr, pos))
                        {
                            l_Conversation->CleanBeforeGC();
                            sGarbageCollector->Add(l_Conversation);
                        }
        ...
        case SMART_ACTION_SUMMON_ADD_PLR_PERSONAL_VISIBILITY:
        {
            ObjectList* l_Targets = GetTargets(e, unit);
            if (!me || !l_Targets)
                break;
            ...
            for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
            {
                if (Player* l_Player = (*l_Iter)->ToPlayer())
                {
                    if (!e.action.PersonnalOrHideVisibility.Visibility)
                    {
                    ...
                    {
                        if (!me->ShouldHideFor(l_Player->GetGUID()))
                        {
                            me->AddToHideList(l_Player->GetGUID());
                            me->DestroyForPlayer(l_Player, false);
                        }
                    }
                }
            }
        ...
        case SMART_ACTION_SUMMON_AREATRIGGER:
        {
            if (!GetBaseObject())
                break;

            sLog->outDebug(LogFilterType::LOG_FILTER_DATABASE_AI, "SmartScript::ProcessAction:: SMART_ACTION_SUMMON_AREATRIGGER: AreaTrigger %d", e.action.AreaTrigger.ID);

            if (e.action.AreaTrigger.TargetX || e.action.AreaTrigger.TargetY || e.action.AreaTrigger.TargetZ)
            {
                Position l_Pos;
                l_Pos.Relocate(float(e.action.AreaTrigger.TargetX), float(e.action.AreaTrigger.TargetY), float(e.action.AreaTrigger.TargetZ), 0.0f);

                AreaTrigger* l_AreaTrigger = new AreaTrigger;
                if (!l_AreaTrigger->CreateAreaTrigger(sObjectMgr->GenerateLowGuid(HighGuid::HIGHGUID_AREATRIGGER), e.action.AreaTrigger.ID, GetBaseObject()->ToUnit(), nullptr, nullptr, l_Pos, l_Pos))
                {
                    l_AreaTrigger->CleanBeforeGC();
                    sGarbageCollector->Add(l_AreaTrigger);
                }
                else if (e.action.AreaTrigger.Duration)
                    l_AreaTrigger->SetDuration(e.action.AreaTrigger.Duration);
            }
            else
            {
                ObjectList* l_Targets = GetTargets(e, unit);
        ...
        case SMART_ACTION_CHANGE_PHASE_ID:
        {
            ObjectList* l_Targets = GetTargets(e, unit);
            if (!l_Targets)
                break;
            ...
            for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
            {
                if ((*l_Iter)->GetTypeId() != TYPEID_PLAYER && e.action.SetPhaseID.ResetPhaseMask)
                    (*l_Iter)->SetPhaseMask(0, false);

                (*l_Iter)->SetInPhase(e.action.SetPhaseID.PhaseID, true, !e.action.SetPhaseID.RemovePhase);
            }

            delete l_Targets;
            break;
        }
        case SMART_ACTION_LEAP_BACK:
        {
            if (!me)
                break;

            me->JumpTo(e.action.LeapBack.SpeedXY, e.action.LeapBack.SpeedZ, e.action.LeapBack.Back != 0);
            break;
        }
        case SMART_ACTION_LEARN_SPELL:
        {
            ...
            for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
            {
                if (Player* l_Player = (*l_Iter)->ToPlayer())
                {
                    if (e.action.LearnSpell.Unlearn)
                    {
                        if (e.action.LearnSpell.Spell1)
                            l_Player->removeSpell(e.action.LearnSpell.Spell1);

                        if (e.action.LearnSpell.Spell2)
                            l_Player->removeSpell(e.action.LearnSpell.Spell2);
                    }
                    else
                    {
                        if (e.action.LearnSpell.Spell1)
                            l_Player->learnSpell(e.action.LearnSpell.Spell1, false);

                        if (e.action.LearnSpell.Spell2)
                            l_Player->learnSpell(e.action.LearnSpell.Spell2, false);
                    }
                }
            }
        ...
        case SMART_ACTION_PLAY_VISUAL_KIT:
        {
            ObjectList* l_Targets = GetTargets(e, unit);
            if (!l_Targets)
                break;
            ...
            for (ObjectList::const_iterator l_Iter = l_Targets->begin(); l_Iter != l_Targets->end(); ++l_Iter)
            {
                ...
                {
                    l_Unit->SendPlaySpellVisualKit(e.action.VisualKit.KitRecID, e.action.VisualKit.KitType, e.action.VisualKit.Duration);
                }
            }
        ...
        case SMART_ACTION_ADD_ACHIEVEMENT:
        {
            AchievementEntry const* l_AchievementEntry = sAchievementStore.LookupEntry(e.action.AddAchievement.AchievementID);
        ...
        case SMART_ACTION_QUEST_OBJECT_SATISFY:
        {
            ObjectList* l_Targets = GetTargets(e, unit);
            if (!l_Targets)
                break;
            ...
            for (auto l_Target : *l_Targets)
                if (Player* l_Player = l_Target->ToPlayer())
                    l_Player->QuestObjectiveSatisfy(e.action.QuestObjectiveSatisfy.ObjectiveID, e.action.QuestObjectiveSatisfy.Amount, e.action.QuestObjectiveSatisfy.Type);

            delete l_Targets;
            break;
        }
        case SMART_ACTION_ADD_MOVIE_DELAYED_TELEPORT:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                l_Player->AddMovieDelayedTeleport(e.action.DelayedMovieTP.MovieId, e.action.DelayedMovieTP.MapId, e.target.x, e.target.y, e.target.z, e.target.o);
            }

            delete targets;
            break;
        }
        case SMART_ACTION_MORPH_PLAYER_TO_ENTRY_OR_MODEL:
        {
            ObjectList* targets = GetTargets(e, unit);
            if (!targets)
                break;
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (!IsUnit(*itr) || !(*itr)->ToUnit()->IsPlayer())
                    continue;

                Player* l_Player = (*itr)->ToUnit()->ToPlayer();
                ...
                if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
                {
                    ...
                    if (e.action.morphOrMount.creature)
                    {
                        ...
                        {
                            uint32 display_id = sObjectMgr->ChooseDisplayId(0, ci);
                            l_Player->SetDisplayId(display_id);
                        }
                    }
                    //if no param1, then use value from param2 (modelId)
                    else
                    {
                        l_Player->SetDisplayId(e.action.morphOrMount.model);
                    }
                }
            }
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= nullptr*/)
{
    ...
    switch (e.GetTargetType())
    {
                ...
                if (Player* player = trigger->ToPlayer())
                {
                    if (GroupPtr group = player->GetGroup())
                    {
                        group->GetMemberSlots().foreach([&](Group::MemberSlotPtr l_Itr)
                        {
                            ...
                            if (GetBaseObject() && member->GetMap() != GetBaseObject()->GetMap())
                                return;
        ...
        {
            ObjectGuidListMap::iterator itr = mTargetStorage.find(e.target.stored.id);
            if (itr != mTargetStorage.end())
            {
                for (uint64 l_Guid : itr->second)
                {
                    if (WorldObject* l_Object = ObjectAccessor::GetWorldObject(*GetBaseObject(), l_Guid))
                        l->push_back(l_Object);
                }
        ...
        case SMART_TARGET_OWNER_OR_SUMMONER:
        {
            if (me)
            {
                ...
                if (!charmerOrOwnerGuid)
                    if (TempSummon* l_Ts = me->ToTempSummon())
                        charmerOrOwnerGuid = l_Ts->GetSummonerGUID();
    ...
    std::vector<WorldObject*> l_ToRemove;
    for (WorldObject* l_WorldObject : *l)
    {
        if (GetBaseObject() && l_WorldObject->GetMap() != GetBaseObject()->GetMap())
        {
            sLog->outExtChat("#jarvis-log", "danger", true, "SmartAI GetTargets for worldobject %u action %u is targeting a worldobject on another map", GetBaseObject()->GetEntry(), e.GetActionType());
            l_ToRemove.push_back(l_WorldObject);
        }
    }

    for (WorldObject* l_WorldObject : l_ToRemove)
        l->remove(l_WorldObject);
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    if (!sConditionMgr->IsObjectMeetingSmartEventConditions(e.entryOrGuid, e.event_id, e.GetScriptType(), unit, GetBaseObject()) && e.GetEventType() != SMART_EVENT_ON_FIND_OBJECT_IN_RANGE)
        return;
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_OOC_LOS:
        {
            ...
            if (e.event.los.onlyPlayer && unit->GetTypeId() != TYPEID_PLAYER)
                return;
        ...
        case SMART_EVENT_ADD_AURA:
        case SMART_EVENT_REMOVE_AURA:
        {
            if (e.event.raw.param1 && var0 != e.event.raw.param1)
                return;
        ...
        case SMART_EVENT_QUEST_OBJ_COPLETION:
        {
            if (e.event.objective.ObjectiveID != var0)
                return;
            ProcessAction(e, unit, var0);
            break;
        }
        case SMART_EVENT_ON_FIND_OBJECT_IN_RANGE:
        {
            auto obj = GetBaseObject();
            if (!obj || obj->IsEventObject())
                break;

            switch (e.event.objectInRange.type)
            {
                case 1: //player
                {
                    Player* target = nullptr;

                    JadeCore::NearestPlayerInObjectRangeCheck checker(obj, static_cast<float>(e.event.objectInRange.distance));
                    JadeCore::PlayerLastSearcher<JadeCore::NearestPlayerInObjectRangeCheck> searcher(obj, target, checker);
                    obj->VisitNearbyObject(static_cast<float>(e.event.objectInRange.distance), searcher);

                    if (target)
                    {
                        unit = target;
                        if (sConditionMgr->IsObjectMeetingSmartEventConditions(e.entryOrGuid, e.event_id, e.GetScriptType(), unit, obj))
                        {
                            if (e.event.objectInRange.entry && target->GetQuestStatus(e.event.objectInRange.entry) != QUEST_STATUS_INCOMPLETE)
                                return;

                            ProcessAction(e, unit, var0, var1, bvar, spell, gob);
                            RecalcTimer(e, e.event.objectInRange.cooldownTimer, e.event.objectInRange.cooldownTimer ? e.event.objectInRange.cooldownTimer : 1000);
                            return;
                        }
                    }
                    break;
                ...
                case 2: //creature
                {
                    if (e.event.objectInRange.entry != 0)
                    {
                        auto target = GetClosestCreatureWithEntry(GetBaseObject(), e.event.objectInRange.entry, static_cast<float>(e.event.objectInRange.distance), true);
                        if (target)
                        {
                            unit = target;
                            if (sConditionMgr->IsObjectMeetingSmartEventConditions(e.entryOrGuid, e.event_id, e.GetScriptType(), unit, obj))
                ...
                case 3: //gameobject
                {
                    if (e.event.objectInRange.entry != 0)
                    {
                        auto target = GetClosestGameObjectWithEntry(GetBaseObject(), e.event.objectInRange.entry, static_cast<float>(e.event.objectInRange.distance));
                        if (target)
                        {
                            if (sConditionMgr->IsObjectMeetingSmartEventConditions(e.entryOrGuid, e.event_id, e.GetScriptType(), unit, obj))
                            {
            ...
            RecalcTimer(e, e.event.objectInRange.repeatTimer, e.event.objectInRange.repeatTimer);
            break;
        }
        default:
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject* obj, AreaTriggerEntry const* at, Quest const* quest)
{
    if (e.empty())
    {
        ...
        if(quest)
            sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript: EventMap for Quest %u is empty but is using SmartScript.", quest->GetQuestId());
    ...
    for (SmartAIEventList::iterator i = e.begin(); i != e.end(); ++i)
    {
        ...
        if ((*i).event.event_spawnMask)
        {
            if (obj && ((1 << obj->GetMap()->GetSpawnMode()) & (*i).event.event_spawnMask))
                mEvents.push_back((*i));
            else
                continue;
        }
...
void SmartScript::GetScript()
{
    ...
    else if (mScriptType == SMART_SCRIPT_TYPE_QUEST)
    {
        e = sSmartScriptMgr->GetScript((int32)quest->GetQuestId(), mScriptType);
        FillScript(e, player, nullptr, nullptr);
    }
    else if (m_Event)
    {
        e = sSmartScriptMgr->GetScript(-static_cast<int32>(m_Event->GetDBTableGUIDLow()), mScriptType);
        if (e.empty())
            e = sSmartScriptMgr->GetScript(static_cast<int32>(m_Event->GetEntry()), mScriptType);
        FillScript(e, m_Event, nullptr);
    }
}

void SmartScript::OnInitialize(WorldObject* obj, AreaTriggerEntry const* at, Quest const* p_Quest)
{
    if (obj)//handle object based scripts
    {
        switch (obj->GetTypeId())
        {
            ...
            case TYPEID_EVENTOBJECT:
                ...
                m_Event = obj->ToEventObject();
                sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::OnInitialize: source is EventObject %u", m_Event->GetEntry());
                break;
            case TYPEID_PLAYER:
                mScriptType = SMART_SCRIPT_TYPE_QUEST; ///< Maybe that should be done more generically in case some events are implemented
                player = obj->ToPlayer();
                quest = p_Quest;
                sLog->outDebug(LOG_FILTER_DATABASE_AI, "SmartScript::OnInitialize: source is Quest: %u, Player: %lu", quest->GetQuestId(), player->GetGUID());
                break;
            default:
...
void SmartScript::SetScript9(SmartScriptHolder& e, uint32 entry)
{
    ...
    {
        sLog->outExtChat("#jarvis", "danger", true, "@wow_dev Entry " SI64FMTD " SourceType %u Event %u Action %u is trying to overwrite timed action list from a timed action, this is not allowed!.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
        return;
    }
...
{
    if (WorldObject* l_Object = GetBaseObject())
        return Unit::GetUnit(*l_Object, mLastInvoker);
