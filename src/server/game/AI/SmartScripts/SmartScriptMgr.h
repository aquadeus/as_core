...
enum SMART_EVENT
{
    ...
    SMART_EVENT_QUEST_OBJ_COPLETION      = 48,      // none
    ...
    SMART_EVENT_EVENTOBJECT_ONTRIGGER    = 78,      // EventObjec(0 any)

    /// Custom Firestorm Event
    SMART_EVENT_ADD_AURA                 = 79,      // spellId
    SMART_EVENT_REMOVE_AURA              = 80,      // spellId
    SMART_EVENT_QUEST_ABANDON            = 81,      // TODO

    SMART_EVENT_ON_FIND_OBJECT_IN_RANGE  = 82,      // On find nearest object in range. Distance, Type(1 - player, 2 - NPC, 3 - GO), Entry(npcId, goId), RepeatTimer
...
enum SMART_ACTION
{
    ...
    SMART_ACTION_TALK                               = 1,      // groupID from creature_text, duration to wait before TEXT_OVER event is triggered, useTalkTarget (0/1) - use target as talk target, IsPersonalText - text has be personal for target
    ...
    SMART_ACTION_CAST                               = 11,     // SpellId, CastFlags, targetType (This param need for cone spells, put delay in milliseconds, basically, it should be equal cast time + 500-1000ms))
    SMART_ACTION_SUMMON_CREATURE                    = 12,     // CreatureID, summonType, duration in ms, storageID, attackInvoker, personalVisible
    ...
    SMART_ACTION_MOVE_FORWARD                       = 46,     // distance, 0 , run
    SMART_ACTION_SET_VISIBILITY                     = 47,     // on/off
    SMART_ACTION_SET_ACTIVE                         = 48,     // on/off
    SMART_ACTION_ATTACK_START                       = 49,     //
    SMART_ACTION_SUMMON_GO                          = 50,     // GameObjectID, DespawnTime in ms, personalVisible
    ...
    SMART_ACTION_ADD_AURA                           = 75,     // spellid, stackamount
    ...
    SMART_ACTION_BOSS_ANOUNCE                       = 201,  ///< TextId from trinity_string
    SMART_ACTION_MOVE_Z                             = 202,  ///< TextId from trinity_string
    SMART_ACTION_SET_ID                             = 203,  ///< Set instance ID
    ...
    SMART_ACTION_SUMMON_ADD_PLR_PERSONAL_VISIBILITY = 207,  ///< Add Player In Personal Visibility
    SMART_ACTION_SUMMON_AREATRIGGER                 = 208,  ///< Summon AreaTrigger
    SMART_ACTION_RAND_TAKEOFF                       = 209,  ///< Radius, Range z. Target X and target Y is for frand angle of position
    SMART_ACTION_MOD_CURRENCY                       = 215,  ///< currency Id, increaseCount, decreaseCount, ignoreMultipliers (auras etc.)
    /// IDs custom for Firestorm (check structures to get params)
    SMART_ACTION_SEND_EVENT_SCENARIO                = 1001,
    ...
    SMART_ACTION_STOP_SCENE_OBJECT                  = 1007,
    SMART_ACTION_CHANGE_PHASE_ID                    = 1008,
    SMART_ACTION_LEAP_BACK                          = 1009,
    SMART_ACTION_SUMMON_FROM_TARGET                 = 1010,
    SMART_ACTION_LEARN_SPELL                        = 1011,
    SMART_ACTION_PLAY_VISUAL_KIT                    = 1012,
    SMART_ACTION_ADD_ACHIEVEMENT                    = 1013,
    SMART_ACTION_QUEST_OBJECT_SATISFY               = 1014,
    SMART_ACTION_ADD_MOVIE_DELAYED_TELEPORT         = 1023,  ///< MovieId, MapID
    SMART_ACTION_MORPH_PLAYER_TO_ENTRY_OR_MODEL     = 1044,  ///< Backported from BFA
...
struct SmartAction
{
    ...
    union
    {
        struct
        {
            ...
            uint32 decreaseCount;
            uint32 ignoreMultipliers;
        } modifyCurrency;
        ...
        {
            uint32 Visibility;
        } PersonnalOrHideVisibility;

        struct
        {
            uint32 PhaseID;
            uint32 ResetPhaseMask;
            bool RemovePhase;
        } SetPhaseID;
...
enum SmartScriptType
{
    ...
    SMART_SCRIPT_TYPE_LINKED_ID_CREATURE = 11,
...
const uint32 SmartAIEventMask[SMART_EVENT_END][2] =
{
    ...
    {SMART_EVENT_QUEST_OBJ_COPLETION,       SMART_SCRIPT_TYPE_MASK_QUEST },
    ...
    {SMART_EVENT_COUNTER_SET,               SMART_SCRIPT_TYPE_MASK_CREATURE },
    {SMART_EVENT_EVENTOBJECT_ONTRIGGER,     SMART_SCRIPT_TYPE_MASK_EVENTOBJECT },
    {SMART_EVENT_ADD_AURA,                  SMART_SCRIPT_TYPE_MASK_CREATURE },
    {SMART_EVENT_REMOVE_AURA,               SMART_SCRIPT_TYPE_MASK_CREATURE },
    {SMART_EVENT_QUEST_ABANDON,             SMART_SCRIPT_TYPE_MASK_QUEST },
    {SMART_EVENT_ON_FIND_OBJECT_IN_RANGE,   SMART_SCRIPT_TYPE_MASK_CREATURE + SMART_SCRIPT_TYPE_MASK_GAMEOBJECT },
};
...
typedef std::list<uint64> ObjectGuidList;
typedef std::unordered_map<uint32, ObjectList*> ObjectListMap;
typedef std::unordered_map<uint32, ObjectGuidList> ObjectGuidListMap;
...
class SmartAIMgr
{
    ...
    private:
        ...
        bool IsAnimKitValid(SmartScriptHolder const& e, uint32 entry)
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent AnimKit entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
                return false;
            }
            return true;
        }
