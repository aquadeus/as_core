...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    do
    {
        ...
        temp.entryOrGuid    = fields[l_Index++].GetInt32();
        temp.linkedId       = fields[l_Index++].GetString();
        SmartScriptType source_type = (SmartScriptType)fields[l_Index++].GetUInt8();
        ...
        if (temp.entryOrGuid >= 0)
        {
            switch (source_type)
            {
                case SMART_SCRIPT_TYPE_LINKED_ID_CREATURE:
                {
                    if (temp.linkedId == "" || !sObjectMgr->GetCreatureData(sObjectMgr->GetGUIDByLinkedId(temp.linkedId)))
                    {
                        sLog->outError(LOG_FILTER_SQL, "SmartAIMgr::LoadSmartAIFromDB: Creature linkedId (%s) does not exist, skipped loading.", temp.linkedId.c_str());
                        continue;
                    }
                    break;
                }
                ...
                case SMART_SCRIPT_TYPE_QUEST:
                {
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "SmartAIMgr::LoadSmartAIFromDB: Quest entry (%u) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                        continue;
                    }
                    break;
                }
                ...
                {
                    if (!sObjectMgr->GetEventObjectTemplate((uint32)temp.entryOrGuid))
                    {
                        sLog->outError(LOG_FILTER_SQL, "SmartAIMgr::LoadSmartAIFromDB: EventObject entry (%u) does not exist, skipped loading.", uint32(temp.entryOrGuid));
                        continue;
                    }
        ...
        if (source_type == (SmartScriptType)SMART_SCRIPT_TYPE_LINKED_ID_CREATURE)
        {
            source_type = SMART_SCRIPT_TYPE_CREATURE;
            temp.source_type = SMART_SCRIPT_TYPE_CREATURE;
            temp.entryOrGuid = -int32(sObjectMgr->GetGUIDByLinkedId(temp.linkedId));
        }

        temp.event_id = fields[l_Index++].GetUInt16();
        temp.link = fields[l_Index++].GetUInt16();
        temp.event.type = (SMART_EVENT)fields[l_Index++].GetUInt8();
        temp.event.event_phase_mask = fields[l_Index++].GetUInt8();
        temp.event.event_chance = fields[l_Index++].GetUInt8();
        temp.event.event_flags = fields[l_Index++].GetUInt8();
        temp.event.event_spawnMask = fields[l_Index++].GetUInt32();

        temp.event.raw.param1 = fields[l_Index++].GetUInt32();
        temp.event.raw.param2 = fields[l_Index++].GetUInt32();
        temp.event.raw.param3 = fields[l_Index++].GetUInt32();
        temp.event.raw.param4 = fields[l_Index++].GetUInt32();
        temp.event.raw.param5 = fields[l_Index++].GetUInt32();

        temp.action.type = (SMART_ACTION)fields[l_Index++].GetUInt16();
        temp.action.raw.param1 = fields[l_Index++].GetUInt32();
        temp.action.raw.param2 = fields[l_Index++].GetUInt32();
        temp.action.raw.param3 = fields[l_Index++].GetUInt32();
        temp.action.raw.param4 = fields[l_Index++].GetUInt32();
        temp.action.raw.param5 = fields[l_Index++].GetUInt32();
        temp.action.raw.param6 = fields[l_Index++].GetUInt32();

        temp.target.type = (SMARTAI_TARGETS)fields[l_Index++].GetUInt8();
        temp.target.raw.param1 = fields[l_Index++].GetUInt32();
        temp.target.raw.param2 = fields[l_Index++].GetUInt32();
        temp.target.raw.param3 = fields[l_Index++].GetUInt32();
        temp.target.x = fields[l_Index++].GetFloat();
        temp.target.y = fields[l_Index++].GetFloat();
        temp.target.z = fields[l_Index++].GetFloat();
        temp.target.o = fields[l_Index++].GetFloat();
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case SMART_EVENT_ACTION_DONE:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses invalid event id %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.doAction.eventId);
                    return false;
                }
                break;
            }
            case SMART_EVENT_FRIENDLY_HEALTH_PCT:
            {
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %u SourceType %u Event %u Action %u has pct value above 100, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                    return false;
                }
                ...
                switch (e.GetTargetType())
                {
                    ...
                    case SMART_TARGET_PLAYER_DISTANCE:
                        break;
                    default:
                        sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses invalid target_type %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.GetTargetType());
                        return false;
                }
                break;
            }
            ...
            {
                if (e.event.objectInRange.repeatTimer < 500)
                    e.event.objectInRange.repeatTimer = 500;
                break;
            }
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_PLAY_ANIMKIT:
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid AnimKit type %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.animKit.type);
                return false;
            }
            break;
        ...
        case SMART_ACTION_RANDOM_SOUND:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "SmartAIMgr: Entry %u SourceType %u Event %u Action %u does not have any non-zero sound", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                return false;
            }
        ...
        case SMART_ACTION_REMOVE_AURAS_BY_TYPE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Entry %u SourceType %u Event %u Action %u uses invalid data type %u (value range 0-TOTAL_AURAS), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.action.auraType.type);
                return false;
            }
            break;
        }
