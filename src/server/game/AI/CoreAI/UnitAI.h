...
class UnitAI
{
    ...
    public:
        ...
        virtual bool CanBypassResetRange() const
        {
            return false;
        }

        virtual void AttackStart(Unit* target, bool p_Melee = true);
        ...
        virtual void DoAction(int32 const p_Action, Creature* p_Creature)
        {
            UNUSED(p_Action);
            UNUSED(p_Creature);
        }
        ...
        virtual bool GetObjectData(uint64 const& p_Guid)
        {
            UNUSED(p_Guid);
            return false;
        }

        virtual uint64 GetData64(uint32 p_ID = 0)
        {
            UNUSED(p_ID);
            return 0;
        }
        ...
        virtual bool CanBeHitInTheBack() { return true; }

        virtual bool CanBeSelectedAsTarget(Unit const* p_Target, SpellInfo const* p_Spell) const
        {
            UNUSED(p_Target);
            UNUSED(p_Spell);
            return true;
        ...
        virtual bool CanBeAttacked(Unit const* p_Attacker, SpellInfo const* p_Spell) const
        {
            UNUSED(p_Attacker);
            UNUSED(p_Spell);
            return true;
        ...
        /// Called during spell target selection
        virtual bool IgnoreAoESpells() const { return false; }
        ...
        void SelectRangedTargetList(std::list<Player*>& p_PlayerList, bool p_AllowHeal = true, int32 p_CheckAura = 0) const;
        ...
        enum eTargetTypeMask
        {
            ...
            TypeMaskAllMelee    = eTargetTypeMask::TypeMaskMelee | eTargetTypeMask::TypeMaskTank | eTargetTypeMask::TypeMaskHealer,
            TypeMaskAllRanged   = eTargetTypeMask::TypeMaskRanged | eTargetTypeMask::TypeMaskHealer,
            TypeMaskAll         = eTargetTypeMask::TypeMaskAllMelee | eTargetTypeMask::TypeMaskAllRanged,
            TypeMaskNonTank     = eTargetTypeMask::TypeMaskAllRanged | eTargetTypeMask::TypeMaskMelee | eTargetTypeMask::TypeMaskHealer,
            TypeMaskAllDPS      = eTargetTypeMask::TypeMaskAll & ~(eTargetTypeMask::TypeMaskTank | eTargetTypeMask::TypeMaskHealer)
        };

        /// Select a player target, allowing melee, ranged, and/or tanks
        Player* SelectPlayerTarget(eTargetTypeMask p_TypeMask, std::vector<int32> p_ExcludeAuras = { }, float p_Dist = 0.0f, std::vector<uint64> p_ExcludeGuids = { }, bool IsFallback = false);

        Player* SelectMainTank() const;
        Player* SelectOffTank() const;

        // Called at any Damage to any victim (before damage apply)
        virtual void DamageDealt(Unit* p_Victim, uint32& p_Damage, DamageEffectType p_DamageType, SpellInfo const* p_SpellInfo)
        {
            UNUSED(p_Victim);
            UNUSED(p_Damage);
            UNUSED(p_DamageType);
        ...
        /// Called when Unit::BuildValuesUpdate is called
        /// Used to send different display IDs for players for a same unit
        virtual void OnSendDisplayID(uint32& p_DisplayID, Player* p_Target)
        {
            UNUSED(p_DisplayID);
            UNUSED(p_Target);
        }
        ...
        virtual void OnBuildDynFlags(uint32& p_DynFlags, Player* p_Player)
        {
            UNUSED(p_DynFlags);
            UNUSED(p_Player);
        }
        ...
        void DoCast(uint32 spellId, bool triggered = false);
        virtual void DoCast(Unit* victim, uint32 spellId, bool triggered = false);
        virtual void DoCast(Position p_Pos, uint32 p_SpellID, bool p_Triggered = false);
        void DoCastToAllHostilePlayers(uint32 spellid, bool triggered = false);
        void DoCastToAllPlayers(uint32 p_SpellID, bool p_Triggered = false);
        ...
        bool DoSpellAttackIfReady(uint32 spell, TriggerCastFlags triggerFlags = TRIGGERED_NONE);
        ...
        virtual void sOnAuraAddedToTarget(Unit* p_Unit, uint32 p_SpellId)
        {
            UNUSED(p_Unit);
            UNUSED(p_SpellId);
        }
        virtual void sOnAuraRemovedFromTarget(Unit* p_Unit, uint32 p_SpellId)
        {
            UNUSED(p_Unit);
            UNUSED(p_SpellId);
        }
