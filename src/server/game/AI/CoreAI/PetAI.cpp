...
void PetAI::UpdateAI(const uint32 diff)
{
    ...
    if (me->getVictim() && me->getVictim()->isAlive())
    {
        ...
        if (owner != nullptr)
        {
            ...
            if (me->getVictim() != owner->getVictim() && me->HasReactState(ReactStates::REACT_HELPER) && me->GetCharmInfo() && !me->GetCharmInfo()->IsCommandAttack() && !me->GetCharmInfo()->IsMoveTo() && me->IsValidAttackTarget(owner->getVictim()) && CanAttack(owner->getVictim()))
                AttackStart(owner->getVictim(), true);
        }
    ...
    else
    {
        if (me->GetCharmInfo())
        {
            if ((me->HasReactState(REACT_AGGRESSIVE) || me->GetCharmInfo()->IsAtStay()) && !me->GetCharmInfo()->IsMoveTo())
            {
                // Every update we need to check targets only in certain cases
                // Aggressive - Allow auto select if owner or pet don't have a target
                // Stay - Only pick from pet or owner targets / attackers so targets won't run by
                //   while chasing our owner. Don't do auto select.
                // All other cases (ie: defensive) - Targets are assigned by AttackedBy(), OwnerAttacked(), etc.
            ...
            else if (me->HasReactState(REACT_DEFENSIVE) && !me->GetCharmInfo()->IsMoveTo())
            {
                // Check owner attackers
                if (Unit* l_Charmer = me->GetCharmerOrOwner())
                {
                    if (Unit* l_OwnerAttacker = l_Charmer->getAttackerForHelper())
                    {
                        if (!l_OwnerAttacker->HasBreakableByDamageCrowdControlAura())
                            AttackStart(l_OwnerAttacker);
                    }
                }
            ...
            else if (!me->GetCharmInfo()->IsMoveTo())
                HandleReturnMovement();
        }
        else
            HandleReturnMovement();
    ...
    if (!me->HasUnitState(UNIT_STATE_CASTING))
    {
        ...
        if (!targetSpellStore.empty() && !me->HasUnitState(UNIT_STATE_STUNNED))
        {
...
void PetAI::KilledUnit(Unit* victim)
{
    ...
    {
        AttackStart(nextTarget);
        if (CharmInfo* l_CharmInfo = me->GetCharmInfo())
        {
            if (l_CharmInfo->GetCommandState() == COMMAND_MOVE_TO && !l_CharmInfo->IsMoveToStay())
                l_CharmInfo->SetIsMoveToStay(true);
        }
    }
    else
...
void PetAI::AttackStart(Unit* target, bool p_Melee /*= true*/)
{
    ...
    DoAttack(target, (!me->GetCharmInfo()->HasCommandState(COMMAND_STAY) || me->GetCharmInfo()->IsCommandAttack()) && !me->GetCharmInfo()->IsMoveTo() && !me->GetCharmInfo()->IsMoveToStay());
}
...
Unit* PetAI::SelectNextTarget(bool allowAutoSelect) const
{
    ...
    {
        if (!me->GetCharmInfo()->IsReturning() || me->GetCharmInfo()->IsFollowing() || me->GetCharmInfo()->IsAtStay() || me->GetCharmInfo()->IsMoveToStay())
...
void PetAI::HandleReturnMovement()
{
    ...
    /// Prevent from interrupting effects: Confused, Knocked Back, Jumping, Charging, Fleeing, Distracted.
    /// For example, using Ghoul's Leap it makes the ghoul jump to the target.
    /// The jump is interrupted without this fix.
    if (me->GetMotionMaster()->GetMotionSlotType(MovementSlot::MOTION_SLOT_CONTROLLED) == MovementGeneratorType::EFFECT_MOTION_TYPE)
        return;
    ...
    else if (me->GetCharmInfo()->HasCommandState(COMMAND_FOLLOW))// COMMAND_FOLLOW
    {
    ...
    else // COMMAND_MOVE_TO
    {
        if (!me->GetCharmInfo()->IsMoveToStay() && !me->GetCharmInfo()->IsReturning())
        {
            // Return to previous position where stay was clicked
            float x, y, z;
...
void PetAI::DoAttack(Unit* target, bool chase)
{
    ...
    if (me->Attack(target, true))
    {
        ...
        if (chase)
        {
            ...
            if (me->ToPet() && me->ToPet()->NeedToChase(target))
                me->GetMotionMaster()->MoveChase(target);
        }
        else // (Stay && ((Aggressive || Defensive) && In Melee Range)))
        {
            ClearCharmInfoFlags();
            if (me->GetCharmInfo()->HasCommandState(CommandStates::COMMAND_MOVE_TO))
                me->GetCharmInfo()->SetIsMoveToStay(true);
            else
...
void PetAI::MovementInform(uint32 moveType, uint32 data)
{
    ...
    switch (moveType)
    {
        case POINT_MOTION_TYPE:
        {
            ...
            if ((data == me->GetGUIDLow() && (me->GetCharmInfo()->IsReturning() || me->GetCharmInfo()->IsMoveTo())))
            {
                ClearCharmInfoFlags();
                if (me->GetCharmInfo()->HasCommandState(CommandStates::COMMAND_MOVE_TO))
                    me->GetCharmInfo()->SetIsMoveToStay(true);
...
bool PetAI::CanAttack(Unit* target)
{
    ...
    {
        if (!l_Owner->IsValidAttackTarget(target) || l_Owner->IsFriendlyTo(target))
            return false;
    }
    ...
    if (me->GetCharmInfo()->HasCommandState(COMMAND_STAY) || me->GetCharmInfo()->IsAtStay())
    ...
    if (me->getVictim() && me->getVictim() != target)
    {
        ...
        if (auto charmerOrOwner = me->GetCharmerOrOwner())
        {
            if (auto owner = charmerOrOwner->ToPlayer())
                ownerTarget = owner->GetSelectedUnit();
            else
                ownerTarget = charmerOrOwner->getVictim();

            if (ownerTarget && me->GetCharmInfo() && me->GetCharmInfo()->IsCommandAttack())
    ...
    if (me->GetOwnerGUID() && me->GetOwnerGUID() == player->GetGUID())
        switch (emote)
        {
            case TEXT_EMOTE_COWER:
                if (me->isPet() && me->ToPet()->IsPetGhoulOrAbomination())
...
void PetAI::ClearCharmInfoFlags()
{
    ...
    if (ci)
    {
        ...
        ci->SetIsMoveTo(false);
        ci->SetIsMoveToStay(false);
    }
}
