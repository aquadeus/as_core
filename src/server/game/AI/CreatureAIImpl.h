...
{
    public:
        EventMap() : _time(0), _phase(0), m_Stop(false) {}
        ...
        void StopEvents(bool p_Stop) { m_Stop = p_Stop; }
...
inline bool CreatureAI::UpdateVictimRanged()
{
    if (!me->isInCombat())
        return false;
...
inline bool CreatureAI::_EnterEvadeMode()
{
    ...
    if (me->GetEntry() != 46499 && me->GetEntry() != 62982 && me->GetEntry() != 67236 && me->GetEntry() != 59607 && !me->GetSpellModOwner()) // NPC_HOLY_GUARDIAN or Mindbender
...
inline void UnitAI::DoCast(Unit* victim, uint32 spellId, bool triggered /*= false*/)
{
...
inline void UnitAI::DoCast(Position p_Pos, uint32 p_SpellID, bool p_Triggered /*= false*/)
{
    if (me->HasUnitState(UNIT_STATE_CASTING) && !p_Triggered)
        return;

    me->CastSpell(p_Pos, p_SpellID, p_Triggered);
}
