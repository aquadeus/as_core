...
/// Constructor
GarrisonNPCAI::GarrisonNPCAI(Creature* p_Creature) : CosmeticAI(p_Creature), m_PlotInstanceLocation(nullptr), m_BuildingID(0), m_Owner(nullptr), m_SequenceSize(0), LastShipmentID(0) { }
...
void GarrisonNPCAI::SetupActionSequence(MS::Garrison::GDraenor::SequencePosition* p_CoordTable, uint8* p_SequenceTable, uint32 p_SequenceSize, uint32 p_FirstMovePointID)
{
...
/// @p_SummonType : Summon type
Creature* GarrisonNPCAI::SummonRelativeCreature(uint32 p_Entry, MS::Garrison::GDraenor::SequencePosition p_Position, TempSummonType p_SummonType)
{
...
void GarrisonNPCAI::SetData(uint32 p_ID, uint32 p_Value)
{
    switch (p_ID)
    {
        case MS::Garrison::GDraenor::CreatureAIDataIDs::PlotInstanceID:
        {
            m_PlotInstanceLocation = nullptr;

            for (uint32 l_I = 0; l_I < MS::Garrison::GDraenor::Globals::PlotInstanceCount; ++l_I)
            {
                if (MS::Garrison::GDraenor::gGarrisonPlotInstanceInfoLocation[l_I].PlotInstanceID == (p_Value & 0x0000FFFF) && MS::Garrison::GDraenor::gGarrisonPlotInstanceInfoLocation[l_I].SiteLevelID == ((p_Value >> 16) & 0x0000FFFF))
                {
                    m_PlotInstanceLocation = &MS::Garrison::GDraenor::gGarrisonPlotInstanceInfoLocation[l_I];
                    break;
                }
            }
        ...
        case MS::Garrison::GDraenor::CreatureAIDataIDs::BuildingID:
        {
            m_BuildingID = p_Value;
            OnSetBuildingID(m_BuildingID);
            break;
        }
        case MS::Garrison::GDraenor::CreatureAIDataIDs::DailyReset:
            OnDailyDataReset();
///                l_GarrisonMgr->UpdatePlot(GetPlotInstanceID()); ///< Disabled cause it causes troubles, m_Owner is not always available
            break;
        case MS::Garrison::GDraenor::CreatureAIDataIDs::DespawnData:
...
{
    if (p_Id == MS::Garrison::GDraenor::CreatureAIDataIDs::OwnerGuid)
...
{
    if ((p_ID & MS::Garrison::GDraenor::CreatureAIDataIDs::HasRecipe) != 0)
    {
        if (m_Recipes.empty())
            return (uint32)-1;

        uint32 l_RecipeID = p_ID & ~MS::Garrison::GDraenor::CreatureAIDataIDs::HasRecipe;
...
{
    if (p_Player->IsInDraenorGarrison())
    {
        MS::Garrison::Manager* l_GarrisonMgr = p_Player->GetGarrison();

        if (l_GarrisonMgr == nullptr || l_GarrisonMgr->GetDraenorGarrison())
            return;
        ...
        std::vector<MS::Garrison::GarrisonFollower> l_Followers = l_GarrisonMgr->GetDraenorGarrison()->GetWeeklyFollowerRecruits(p_Player);
        ...
        l_Data.WriteBit(l_GarrisonMgr->GetDraenorGarrison()->CanRecruitFollower());
...
void GarrisonNPCAI::SendRecruitmentFollowersGenerated(Player* p_Player, uint32 p_AbilityID, uint32 p_ErrorMessage, bool p_IsTrait)
{
    if (p_Player->IsInDraenorGarrison())
    {
        ...
        if (l_Followers.empty())
        {
            ...
            {
                l_Follower.Write(l_Data);
                l_GarrisonMgr->GetDraenorGarrison()->SetGarrisonWeeklyTavernData({ l_Follower.FollowerID, l_Follower.Abilities });
            }
        }
        else
        {
        ...
        if (!m_Recipes.empty())
        {
            ...
            for (uint32 l_I = 0; l_I < m_Recipes.size(); ++l_I)
            {
                if (m_Recipes.at(l_I).PlayerConditionID && !p_Player->EvalPlayerCondition(m_Recipes.at(l_I).PlayerConditionID).first)
                    ...
                    l_Data << uint32(0); ///< PlayerConditionID
            }
