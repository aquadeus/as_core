...
void FollowerAI::AttackStart(Unit* who, bool p_Melee /*= true*/)
{
    if (!who)
        return;
    ...
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->getVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            if (Player* player = GetLeaderForFollower())
            {
                ...
                if (GroupPtr group = player->GetGroup())
                {
                    bool l_Exist = group->GetMemberSlots().exist([&](Group::MemberSlotPtr l_Member)
                    {
...
Player* FollowerAI::GetLeaderForFollower()
{
    if (Player* player = Unit::GetPlayer(*me, m_uiLeaderGUID))
    {
        ...
        else
        {
            if (GroupPtr group = player->GetGroup())
            {
                Group::MemberSlotList::iterator l_Itr = group->GetMemberSlots().find_itr([&](Group::MemberSlotPtr l_Member) -> bool
                {
                    Player* member = l_Member->player;
                ...
                if (l_Itr != group->GetMemberSlots().end())
                    return (*l_Itr)->player;
            }
        }
    }
