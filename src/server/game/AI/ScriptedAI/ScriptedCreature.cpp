...
void SummonList::DespawnAllExceptEntries(std::vector<uint32> p_Exclude)
{
    for (SummonList::iterator l_Iter = begin(); l_Iter != end();)
    {
        Creature* summon = Unit::GetCreature(*me, *l_Iter);
        if (!summon)
            l_Iter = erase(l_Iter);
        else if (std::find(p_Exclude.begin(), p_Exclude.end(), summon->GetEntry()) == p_Exclude.end())
        {
            l_Iter = erase(l_Iter);
            summon->DespawnOrUnsummon();
        }
...
Creature* SummonList::GetCreatureWithEntry(uint32 entry)
{
...
ScriptedAI::ScriptedAI(Creature* creature) : CreatureAI(creature),
    ...
    m_CheckHomeTimer(5000)
{
    _difficulty = Difficulty(me->GetMap()->GetSpawnMode());
}

bool ScriptedAI::CheckHomeDistToEvade(uint32 p_Diff, float p_Dist /*= 0.0f*/, float p_X /*= 0.0f*/, float p_Y /*= 0.0f*/, float p_Z /*= 0.0f*/, bool p_OnlyZ /*= false*/)
{
    if (!me->isInCombat())
        return false;
    ...
    if (m_CheckHomeTimer <= p_Diff)
    {
        m_CheckHomeTimer = 3000;

        if (p_OnlyZ)
        {
            if ((me->GetPositionZ() > p_Z + p_Dist) || (me->GetPositionZ() < p_Z - p_Dist))
                l_Evade = true;
        }
        else if (p_X != 0.0f || p_Y != 0.0f || p_Z != 0.0f)
        {
            if (me->GetDistance(p_X, p_Y, p_Z) >= p_Dist)
                l_Evade = true;
        }
        else if (me->GetDistance(me->GetHomePosition()) >= p_Dist)
            l_Evade = true;
        ...
        m_CheckHomeTimer -= p_Diff;

    return false;
}

void ScriptedAI::DespawnOnRespawn(uint32 p_Time)
{
    me->DespawnOrUnsummon();
    me->Respawn(true, true, p_Time);
}
...
void ScriptedAI::AttackStart(Unit* victim, bool p_Melee)
{
    ...
    if (victim && me->Attack(victim, p_Melee))
    {
        if (p_Melee)
...
void ScriptedAI::DoCastSpell(Unit* target, SpellInfo const* spellInfo, bool triggered /*= false*/, bool p_GetInRangeIfNeeded /*= false*/)
{
    ...
    bool l_IsInRange = true;
    if (p_GetInRangeIfNeeded && !triggered)
    {
        uint32 l_Range = spellInfo->GetMaxRange(me->IsFriendlyTo(target));
        if (!me->IsWithinDist3d(target, l_Range))
        {
            l_IsInRange = false;
            me->GetMotionMaster()->MoveChase(target, l_Range * 0.9f, target->GetAngle(me));
        }
    }
...
void Scripted_NoMovementAI::AttackStart(Unit* target, bool p_Melee /*= true*/)
{
    if (!target)
        return;
...
BossAI::BossAI(Creature* creature, uint32 bossId) : ScriptedAI(creature),
    ...
    _bossId(bossId)
{
    creature->SetBossID(_bossId);

    /// If server crashes while boss is respawning (EncounterState == FAIL), force its respawn after loading
    if (creature->isDead() && instance && instance->instance && instance->instance->IsRaid())
        instance->HandleRespawnAtLoading(creature->GetGUID(), _bossId);
}
...
void BossAI::_EnterCombat()
{
    ...
    if (instance)
    {
        ...
        if (!sWorld->getBoolConfig(CONFIG_IS_TEST_SERVER) && !instance->CheckRequiredBosses(_bossId))
        {
            EnterEvadeMode();
            return;
        }
...
void BossAI::EnterEvadeMode()
{
    ...
    /// Generically triggers EncounterState FAIL in order to remove Bloodlust debuffs and player cooldowns
    if (InstanceScript* l_Instance = me->GetInstanceScript())
        l_Instance->SetBossState(_bossId, EncounterState::FAIL);

    /// Raid bosses have a 30 seconds respawn timer each time they evade
    if (me->GetMap()->IsRaid())
    {
        me->NearTeleportTo(me->GetHomePosition());
        me->Respawn(true, true, sWorld->getBoolConfig(CONFIG_IS_TEST_SERVER) ? 5 * TimeConstants::IN_MILLISECONDS : 30 * TimeConstants::IN_MILLISECONDS);
    }

    /// Adding a 5 sec despawn on dungeon bosses to prevent abuses
    if (me->GetMap()->IsNonRaidDungeon())
    {
        me->NearTeleportTo(me->GetHomePosition());
