...
void npc_escortAI::AttackStart(Unit* who, bool p_Melee /*= true*/)
{
    if (!who)
        return;

    if (me->GetFlagsExtra() & CREATURE_FLAG_EXTRA_CIVILIAN)
        return;
...
bool npc_escortAI::IsPlayerOrGroupInRange()
{
    if (Player* player = GetPlayerForEscort())
    {
        if (GroupPtr group = player->GetGroup())
        {
            return group->GetMemberSlots().exist([&](Group::MemberSlotPtr l_Member) -> bool
            {
                if (Player* member = l_Member->player)
