...
m_EscalatedStatus(TICKET_UNASSIGNED), m_Viewed(false), m_NeedResponse(false), m_HaveTicket(false)
{
    ...
    if (Player* l_Player = sObjectAccessor->FindPlayer(p_PlayerGuid))
        m_Locale = l_Player->GetSession()->GetSessionDbLocaleIndex();
    else
        m_Locale = LocaleConstant::LOCALE_enUS;
}
...
bool GmTicket::LoadFromDB(Field* p_Fields)
{
    ...
    m_Locale            = static_cast<LocaleConstant>(p_Fields[++l_Index].GetUInt8());

    if (m_Locale >= LocaleConstant::LOCALE_None)
        m_Locale = LocaleConstant::LOCALE_enUS;
...
void GmTicket::SaveToDB(SQLTransaction& p_Transaction) const
{
    ...
    l_Statement->setUInt8 (++l_Index, m_Locale);
...
void TicketMgr::LoadTickets()
{
    ...
    /// Delete old tickets (> 28 day)
    CharacterDatabase.PQuery("DELETE FROM gm_tickets WHERE createtime < UNIX_TIMESTAMP() - 60*60*24*7*4");
...
void TicketMgr::ShowList(ChatHandler& p_Handler, bool p_OnlineOnly, LocaleConstant p_Locale /*= LocaleConstant::LOCALE_None*/) const
{
    ...
    for (GmTicketList::const_iterator l_Iter = m_TicketList.begin(); l_Iter != m_TicketList.end(); ++l_Iter)
    {
        ...
        {
            LocaleConstant l_Locale = l_Iter->second->GetLocale();
            if (p_Locale != LocaleConstant::LOCALE_None && l_Locale != p_Locale)
                continue;

            std::string l_Lang = std::string(" - ") + localeNames[l_Locale];

            if (!p_OnlineOnly || l_Iter->second->GetPlayer())
                p_Handler.SendSysMessage(std::string(l_Iter->second->FormatMessageString(p_Handler) + l_Lang).c_str());
        }
    }
}
