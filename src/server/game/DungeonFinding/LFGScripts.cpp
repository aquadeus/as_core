...
void LFGPlayerScript::OnBindToInstance(Player* p_Player, Difficulty p_Difficulty, uint32 p_MapID, bool /*p_Permanent*/, uint8 /*p_ExtendState*/)
{
    MapEntry const* l_MapEntry = sMapStore.LookupEntry(p_MapID);

    if (l_MapEntry->IsDungeon() && p_Difficulty > Difficulty::DifficultyNormal)
        sLFGMgr->InitializeLockedDungeons(p_Player);
}
...
void LFGGroupScript::OnAddMember(GroupPtr group, uint64 guid)
{
    ...
    {
        if (Player* plrg = l_Member->player)
        {
...
void LFGGroupScript::OnRemoveMember(GroupPtr group, uint64 guid, RemoveMethod method, uint64 kicker, char const* reason)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayer(guid))
    {
        /// Disable it, that cause too much troubles
        //if (method == GROUP_REMOVEMETHOD_LEAVE && state != LFG_STATE_FINISHED_DUNGEON && player->getLevel() > 98)
