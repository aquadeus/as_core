...
enum LfgSlotRandomDungeonID
{
    ...
    LfgRandomLegionDungeon          = 1045,
    LfgRandomLegionHeroic           = 1046
};
...
enum LfgGroupType
{
    ...
    LfgGroupeTypeDungeonLegion           = 53,
    LfgGroupeTypeHeroicLegion            = 52
...
// Data needed by SMSG_LFG_UPDATE_PLAYER
...
class LFGMgr
{
    ...
    public:
        ...
        void Leave(Player* player, GroupPtr grp = NULL);
        ...
        uint32 GetLfgQueueJoinTime(uint64 guid)
        {
            ACE_Read_Guard<ACE_RW_Thread_Mutex> l_Lock(m_QueueInfoMapLock);
        ...
        LFGDungeonEntry const* GetLFGDungeon(uint32 mapId, Difficulty diff, uint32 team);

        /////////////////////////////////////////////
        /// LFR
        /////////////////////////////////////////////
        /// This function automatically send personal items to players
        void AutomaticLootDistribution(Creature* p_Creature, GroupPtr p_Group);
        /// This function automatically add personal loots of players on the creature
        void AutomaticLootAssignation(Creature* p_Creature, GroupPtr p_Group);
        ...
        void SetLfgGroup(uint32 p_GroupGuid, bool p_IsLfg);
        void SetPlayerGroup(uint32 p_PlayerGuid, uint32 p_GroupGuid);

        Position const* GetEntrancePosition(uint32 p_DungeonID) const;

        bool CheckGroupRoles(LfgRolesMap &groles, LfgCategory type, uint32 p_IterationCount, bool removeLeaderFlag = true);
    ...
    private:
        ...
        void GetCompatibleDungeons(LfgDungeonSet& dungeons, const std::set<uint64>& players, LfgLockPartyMap& lockMap);
        ...
        ACE_RW_Thread_Mutex m_QueueInfoMapLock;            ///< Queued groups lock
        ...
        ACE_Thread_Mutex m_PlayersLock;                 ///< Player data lock
        LfgGroupDataMap m_Groups;                          ///< Group data
        ACE_Thread_Mutex m_GroupsLock;                 ///< Group data lock

        std::recursive_mutex m_Lock;

        /// LFG Queue thread
        ACE_Based::Thread* m_LfgQueueUpdateRunnable;
        moodycamel::ConcurrentQueue<std::function<void()>> m_Tasks;
        moodycamel::ConcurrentQueue<std::function<void()>> m_QueueTasks;
        moodycamel::ConcurrentQueue<LfgProposal*> m_ProposalToProcess;
        std::map<uint32/*GroupGuid*/, bool/*IsLFG*/> m_LfgGroups;
        ACE_RW_Thread_Mutex m_LfgGroupsLock;
        std::map<uint32/*PlayerGuid*/, uint32/*GroupGuid*/> m_LfgPlayersGroup;
        ACE_RW_Thread_Mutex m_LfgPlayersGroupLock;
};
