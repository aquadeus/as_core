...
namespace MS { namespace Skill { namespace Archaeology
{
    ...
    void Manager::GenerateResearchProjects()
    {
        ...
        for (std::set<ResearchProjectEntry const*>::const_iterator l_It = sResearchProjectSet.begin(); l_It != sResearchProjectSet.end(); ++l_It)
        {
            ...
            auto l_SecondIt = std::find_if(m_ResearchProjects.begin(), m_ResearchProjects.end(), [l_ProjectEntry](uint32 const& p_ProjectID) -> bool
            {
                ...
                if (!l_Second || l_Second->ResearchBranchID != l_ProjectEntry->ResearchBranchID)
                    return false;
            ...
            if ((l_ProjectEntry->Rarity && !roll_chance_i(l_ProjectBaseChance)))
                continue;

            l_CandidatesProjectListPerBranch[l_ProjectEntry->ResearchBranchID].insert(l_ProjectEntry->ID);
        }
    ...
    uint32 Manager::GetSurveyBotEntry(float & p_Orientation)
    {
        ...
        m_Player->SummonGameObject(l_DigSite.LootGameObjectID, l_DigSite.LootGameObjectX, l_DigSite.LootGameObjectY, m_Player->GetMap()->GetHeight(l_DigSite.LootGameObjectX, l_DigSite.LootGameObjectY, MAX_HEIGHT), 0, 0, 0, 0, 0, 30000);
        m_LastArtifactGameObjectEntry = l_DigSite.LootGameObjectID;

        if (l_DigSite.LootGameObjectID)
            m_Player->UpdateCriteria(CRITERIA_TYPE_SURVEY_GAMEOBJECT, l_DigSite.LootGameObjectID, 1);

        if (l_DigSite.SiteLootCount < l_DigSite.SiteMaxLootCount - 1)
        {
        ...
        else
        {
            ...
            m_Player->UpdateCriteria(CRITERIA_TYPE_CLEAR_DIGSITE, 1);
        }
    ...
    bool Manager::SolveResearchProject(uint32 p_ProjectID)
    {
        ...
        m_Player->UpdateCriteria(CRITERIA_TYPE_COMPLETE_ARCHAEOLOGY_PROJECTS, p_ProjectID, 1);
        ...
        m_Player->UpdateSkill(SKILL_ARCHAEOLOGY, l_ResearchProjectEntry->Rarity ? 15 : 5);
        ...
        for (std::set<ResearchProjectEntry const*>::const_iterator l_It = sResearchProjectSet.begin(); l_It != sResearchProjectSet.end(); ++l_It)
        {
            if ((*l_It)->ResearchBranchID == l_ResearchProjectEntry->ResearchBranchID)
            {
                if (((*l_It)->Rarity && !roll_chance_i(l_Chance)))
                    continue;
    ...
    bool Manager::IsCompletedProject(uint32 p_ID) const
    {
        for (CompletedProjectMap::const_iterator l_Iter = m_CompletedProjects.cbegin(); l_Iter != m_CompletedProjects.cend(); ++l_Iter)
        {
            if ((*l_Iter).second.ProjectID == p_ID)
                return true;
        }
        ...
        for (ResearchLootVector::const_iterator l_It = l_Loot.begin(); l_It != l_Loot.end(); ++l_It)
        {
            ...
            if (p_Site.LootGameObjectID == 0)
            {
                switch (l_Entry.ResearchBranchID)
                {
                    ...
                    case 404: p_Site.LootGameObjectID = 246811; break;
                    case 406: p_Site.LootGameObjectID = 246804; break;
                    case 408: p_Site.LootGameObjectID = 246812; break;
