    ...
    do
    {
        ...
        if (reqSkillOrSpell > 0)                            // spell case
        {
                ...
                !reqSpellInfo->IsExplicitDiscovery())
            {
                if (reportedReqSpells.find(absReqSkillOrSpell) == reportedReqSpells.end())
                {
                    sLog->outError(LOG_FILTER_SQL, "Spell (ID: %u) not have MECHANIC_DISCOVERY (28) value in Mechanic field in spell.db2"
        ...
        else if (reqSkillOrSpell == 0)                      // skill case
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Spell (ID: %u) not listed in `SkillLineAbility.db2` but listed with `reqSpell`=0 in `skill_discovery_template` table", spellId);
                continue;
            }
