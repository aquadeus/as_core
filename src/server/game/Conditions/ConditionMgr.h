...
{                                                           // value1           value2         value3
    ...
    CONDITION_AURA                     = 1,                    // spell_id         effindex       stack amount       true if player has aura of spell_id with effect effindex (with at least value3 stacks if value3 != 0)
    ...
    CONDITION_IN_WATER                 = 41,                   // 0                0              0                  true if unit is in the water
    CONDITION_HAS_KNOWLEDGE_LEVEL      = 42,                   // Level                                              true if player has at least specified artifact knowledge level
    CONDITION_HAS_SUMMON               = 43,                   // NPC Id           ComparisonType Count              true if player has summon count matches given value
    ...
    CONDITION_CONTRIBUTION             = 49,                   // ID               STATE          0                  true if Contribution ID is in state STATE
    CONDITION_HAS_ORDER_HALL_TALENT    = 100,                  // ID               HasTalent      0                  true if player has talent, false if he hasn't
    CONDITION_MAX                                           // MAX
};
...
enum ConditionSourceType
{
    ...
    CONDITION_SOURCE_TYPE_TERRAIN_SWAP                   = 25, ///< NYI
    CONDITION_SOURCE_TYPE_PHASE                          = 26,

    CONDITION_SOURCE_TYPE_QUEST_SHOW_MARK                = 1000,
    CONDITION_SOURCE_TYPE_AREATRIGGER_ACTION             = 1002,
    CONDITION_SOURCE_TYPE_LOOT_ITEM                      = 1003,
    CONDITION_SOURCE_TYPE_PLAYERCHOICE_RESPONSE          = 1004,
...
class ConditionMgr
{
    ...
    public:
        ...
        ConditionContainer const* GetConditionsForAreaTriggerAction(uint32 areaTriggerId, uint32 actionId) const;
        ConditionContainer const* GetConditionsForItemLoot(uint32 p_Type, uint32 p_ItemId) const;
