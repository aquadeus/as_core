...
bool Condition::Meets(ConditionSourceInfo& sourceInfo) const
{
    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_AURA:
        {
            ...
            {
                AuraEffect const* l_AuraEffect = unit->GetAuraEffect(ConditionValue1, ConditionValue2);
                condMeets = l_AuraEffect && (ConditionValue3 ? l_AuraEffect->GetBase()->GetStackAmount() >= ConditionValue3 : true);
            }
            break;
        }
        case CONDITION_ITEM:
        ...
        {
            if (Player const* l_Player = object->ToPlayer())
            {
                if (MS::Garrison::Manager* l_GarrisonMgr = l_Player->GetGarrison())
                {
                    if (auto l_DraenorGarrison = l_GarrisonMgr->GetDraenorGarrison())
                    {
                        if (!ConditionValue2)
                            condMeets = l_DraenorGarrison->HasBuildingType(static_cast<MS::Garrison::GDraenor::Building::Type>(ConditionValue1));
                        else
                        {
                            MS::Garrison::GDraenor::GarrisonBuilding l_Building = l_DraenorGarrison->GetBuildingWithType(static_cast<MS::Garrison::GDraenor::Building::Type>(ConditionValue1));
                            ...
                            if (l_DraenorGarrison->GetBuildingLevel(l_Building) >= ConditionValue2)
                                condMeets = true;
                            else
                                condMeets = false;
                        }
        ...
        {
            if (Unit const* l_Unit = object->ToUnit())
                condMeets = l_Unit->IsInWater();
            break;
        }
        case CONDITION_HAS_KNOWLEDGE_LEVEL:
        {
            if (!object->IsPlayer())
                break;
            ...
            if (Player const* l_Player = object->ToPlayer())
            {
                uint32 l_PlayerArtifactKnowledge = l_Player->getLevel() >= 110 ? sWorld->GetCurrentArtifactKnowledge() : 0;
                condMeets = l_PlayerArtifactKnowledge >= ConditionValue1;
            }

            break;
        }
        case CONDITION_HAS_ORDER_HALL_TALENT:
        {
            if (Player const* l_Player = object->ToPlayer())
            {
                if (MS::Garrison::Manager const* l_Mgr = l_Player->GetGarrison())
                {
                    if (GarrTalentEntry const* l_Entry = sGarrTalentStore.LookupEntry(ConditionValue1))
                    {
                        if (ConditionValue2)
                            condMeets = l_Mgr->HasTalent(ConditionValue1);
                        else
                            condMeets = !l_Mgr->HasTalent(ConditionValue1);
                    }
                }
        ...
        case CONDITION_QUEST_OBJECTIVE_COMPLETE:
        {
            ...
            {
                Quest const* l_Quest = sObjectMgr->GetQuestTemplate(sObjectMgr->GetQuestObjectiveQuestId(ConditionValue1));
                if (!l_Quest)
                    break;

                QuestObjective const* l_Objective = l_Quest->GetQuestObjective(ConditionValue1);
                if (!l_Objective)
                    break;

                condMeets = (!player->GetQuestRewardStatus(l_Quest->GetQuestId()) && player->IsQuestObjectiveComplete(l_Quest, l_Objective));
            }
            break;
        }
        case CONDITION_CONTRIBUTION:
        {
            Contribution l_Contribution = sContributionMgr->GetContribution(ConditionValue1);
            if (l_Contribution)
            {
                if (l_Contribution.State == ConditionValue2)
                    condMeets = true;
            }
            break;
        }
        case CONDITION_HAS_SUMMON:
        {
            ...
            {
                uint32 count = 0;
                auto sumList = unit->GetSummonList(ConditionValue1);
                count = uint32(sumList->size());
                condMeets = CompareValues(static_cast<ComparisionType>(ConditionValue2), count, uint32(ConditionValue3));
            }
            break;
        }
        default:
...
bool ConditionMgr::IsObjectMeetToConditionList(ConditionSourceInfo& sourceInfo, ConditionContainer  const& conditions) const
{
    ...
    {
        sLog->outDebug(LOG_FILTER_CONDITIONSYS, "ConditionMgr::IsPlayerMeetToConditionList condType: %u val1: %lu", condition->ConditionType, condition->ConditionValue1);
...
ConditionContainer const* ConditionMgr::GetConditionsForAreaTriggerAction(uint32 areaTriggerId, uint32 actionId) const
{
    ...
    {
        ConditionsByEntryMap::const_iterator i = itr->second.find(actionId);
        if (i != itr->second.end())
            return &i->second;
    }
...
ConditionContainer const*  ConditionMgr::GetConditionsForItemLoot(uint32 p_Type, uint32 p_ItemId) const
{
    ItemLootConditionContainer::const_iterator itr = ItemLootConditionStore.find(p_Type);
    if (itr != ItemLootConditionStore.end())
    {
        ConditionsByEntryMap::const_iterator i = itr->second.find(p_ItemId);
        if (i != itr->second.end())
            return &i->second;
    }
    ...
    do
    {
        ...
        if (iConditionTypeOrReference < 0)//it has a reference
        {
            ...
            if (cond->ConditionValue1)
                sLog->outError(LOG_FILTER_SQL, "Condition %s %i has useless data in value1 (%lu)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue1);
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Condition %s %i has useless data in value2 (%lu)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Condition %s %i has useless data in value3 (%lu)!", rowType, iSourceTypeOrReferenceId, cond->ConditionValue3);
        ...
        if (CanHaveSourceGroupSet(cond->SourceType))
        {
            bool valid = false;
    ...
    for (uint32 l_Difficulty = Difficulty::DifficultyNone; l_Difficulty < Difficulty::MaxDifficulties; l_Difficulty++)
    {
        ...
        for (std::list<uint32>::iterator itr = sharedMasks.begin(); itr != sharedMasks.end(); ++itr)
        {
            ...
            if (uint32 commonMask = *itr & conditionEffMask)
            {
                ...
                for (; firstEffIndex < spellInfo->EffectCount; ++firstEffIndex)
                {
                    if ((1 << firstEffIndex) & *itr)
                        break;
                }

                if (firstEffIndex >= spellInfo->EffectCount)
                    return false;
...
bool ConditionMgr::isSourceTypeValid(Condition* cond) const
{
    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            if (!spellInfo)
            {
                sLog->outError(LOG_FILTER_SQL, "SourceEntry %u in `condition` table, does not exist in `spell.db2`, ignoring.", cond->SourceEntry);
                return false;
            }
            ...
            for (uint8 i = 0; i < spellInfo->EffectCount; ++i)
            {
                ...
                if (spellInfo->Effects[i].IsAreaAuraEffect())
                    continue;

                sLog->outError(LOG_FILTER_SQL, "SourceEntry %u SourceGroup %u in `condition` table - spell %u does not have implicit targets of types (_AREA_, _CONE_, _NEARBY_, _CYLINDER_) or area aura effect type (SPELL_EFFECT_APPLY_AREA_AURA_PARTY and others) for effect %u, SourceGroup needs correction, ignoring.", cond->SourceEntry, origGroup, cond->SourceEntry, uint32(i));
        ...
        case CONDITION_SOURCE_TYPE_LOOT_ITEM:
        {
            switch (cond->SourceGroup)
            {
                ...
                case LOOT_ITEM_TYPE_CURRENCY:
                {
                    CurrencyTypesEntry const* l_CurrencyEntry = sCurrencyTypesStore.LookupEntry(cond->SourceEntry);
                    if (!l_CurrencyEntry)
                    {
                        sLog->outError(LOG_FILTER_SQL, "SourceEntry %u in `condition` table, does not exist in `CurrencyTypesStore`, ignoring.", cond->SourceEntry);
                        return false;
                    }
                    break;
                }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond) const
{
    ...
    switch (cond->ConditionType)
    {
        case CONDITION_AURA:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Aura condition has non existing spell (Id: %lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2 > EFFECT_2)
            {
                sLog->outError(LOG_FILTER_SQL, "Aura condition has non existing effect index (%lu) (must be 0..2), skipped", cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_ITEM:
        {
            ...
            if (!proto)
            {
                sLog->outError(LOG_FILTER_SQL, "Item condition has non existing item (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (!cond->ConditionValue2)
            {
                sLog->outError(LOG_FILTER_SQL, "Item condition has 0 set for item count in value2 (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_ITEM_EQUIPPED:
        {
            ...
            if (!proto)
            {
                sLog->outError(LOG_FILTER_SQL, "ItemEquipped condition has non existing item (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "ItemEquipped condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "ItemEquipped condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_ZONEID:
        {
            ...
            if (!areaEntry)
            {
                sLog->outError(LOG_FILTER_SQL, "ZoneID condition has non existing area (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (areaEntry->ParentAreaID != 0)
            {
                sLog->outError(LOG_FILTER_SQL, "ZoneID condition requires to be in area (%lu) which is a subzone but zone expected, skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "ZoneID condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "ZoneID condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_REPUTATION_RANK:
        {
            ...
            if (!factionEntry)
            {
                sLog->outError(LOG_FILTER_SQL, "Reputation condition has non existing faction (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Reputation condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_TEAM:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Team condition specifies unknown team (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Team condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Team condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_SKILL:
        {
            ...
            if (!pSkill)
            {
                sLog->outError(LOG_FILTER_SQL, "Skill condition specifies non-existing skill (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2 < 1 || cond->ConditionValue2 > sWorld->GetConfigMaxSkillValue())
            {
                sLog->outError(LOG_FILTER_SQL, "Skill condition specifies invalid skill value (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Skill condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_QUESTSTATE:
        ...
        case CONDITION_QUEST_COMPLETE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Quest condition specifies non-existing quest (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_ACTIVE_EVENT:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "ActiveEvent condition has non existing event id (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "ActiveEvent condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "ActiveEvent condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_ACHIEVEMENT:
        {
            ...
            if (!achievement)
            {
                sLog->outError(LOG_FILTER_SQL, "Achievement condition has non existing achievement id (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Achievement condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Achievement condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_CLASS:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Class condition has non existing classmask (%lu), skipped", cond->ConditionValue1 & ~CLASSMASK_ALL_PLAYABLE);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Class condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Class condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_RACE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Race condition has non existing racemask (%lu), skipped", cond->ConditionValue1 & ~RACEMASK_ALL_PLAYABLE);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Race condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Race condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_GENDER:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Gender condition has invalid gender (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Gender condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Gender condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_MAPID:
        {
            ...
            if (!me)
            {
                sLog->outError(LOG_FILTER_SQL, "Map condition has non existing map (%lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Map condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Map condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_SPELL:
        {
            if (!sSpellMgr->GetSpellInfo(cond->ConditionValue1))
            {
                sLog->outError(LOG_FILTER_SQL, "Spell condition has non existing spell (Id: %lu), skipped", cond->ConditionValue1);
                return false;
            }

            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Spell condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Spell condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_LEVEL:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "Level condition has invalid option (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Level condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_DRUNKENSTATE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "DrunkState condition has invalid state (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2)
            {
                sLog->outError(LOG_FILTER_SQL, "DrunkState condition has useless data in value2 (%lu)!", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "DrunkState condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_NEAR_CREATURE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "NearCreature condition has non existing creature template entry (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "NearCreature condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_NEAR_GAMEOBJECT:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "NearGameObject condition has non existing gameobject template entry (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "NearGameObject condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_OBJECT_ENTRY:
        {
            switch (cond->ConditionValue1)
            {
                case TYPEID_UNIT:
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "ObjectEntry condition has non existing creature template entry (%lu), skipped", cond->ConditionValue2);
                        return false;
                    }
                    break;
                case TYPEID_GAMEOBJECT:
                    ...
                    {
                        sLog->outError(LOG_FILTER_SQL, "ObjectEntry condition has non existing game object template entry (%lu), skipped", cond->ConditionValue2);
                        return false;
                    }
                    break;
                case TYPEID_PLAYER:
                case TYPEID_CORPSE:
                    if (cond->ConditionValue2)
                        sLog->outError(LOG_FILTER_SQL, "ObjectEntry condition has useless data in value2 (%lu)!", cond->ConditionValue2);
                    break;
                default:
                    sLog->outError(LOG_FILTER_SQL, "ObjectEntry condition has wrong typeid set (%lu), skipped", cond->ConditionValue1);
                    return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "ObjectEntry condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_TYPE_MASK:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "TypeMask condition has invalid typemask set (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "TypeMask condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "TypeMask condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_RELATION_TO:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "RelationTo condition has invalid ConditionValue1(ConditionTarget selection) (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outError(LOG_FILTER_SQL, "RelationTo condition has ConditionValue1(ConditionTarget selection) set to self (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2 >= RELATION_MAX)
            {
                sLog->outError(LOG_FILTER_SQL, "RelationTo condition has invalid ConditionValue2(RelationType) (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "RelationTo condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_REACTION_TO:
        {
            if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
            {
                sLog->outError(LOG_FILTER_SQL, "ReactionTo condition has invalid ConditionValue1(ConditionTarget selection) (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outError(LOG_FILTER_SQL, "ReactionTo condition has ConditionValue1(ConditionTarget selection) set to self (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (!cond->ConditionValue2)
            {
                sLog->outError(LOG_FILTER_SQL, "mConditionValue2 condition has invalid ConditionValue2(rankMask) (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            break;
        }
        case CONDITION_DISTANCE_TO:
        {
            if (cond->ConditionValue1 >= cond->GetMaxAvailableConditionTargets())
            {
                sLog->outError(LOG_FILTER_SQL, "DistanceTo condition has invalid ConditionValue1(ConditionTarget selection) (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue1 == cond->ConditionTarget)
            {
                sLog->outError(LOG_FILTER_SQL, "DistanceTo condition has ConditionValue1(ConditionTarget selection) set to self (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3 >= COMP_TYPE_MAX)
            {
                sLog->outError(LOG_FILTER_SQL, "DistanceTo condition has invalid ComparisionType (%lu), skipped", cond->ConditionValue3);
                return false;
            }
            break;
        }
        ...
        {
            if (cond->ConditionValue1)
                sLog->outError(LOG_FILTER_SQL, "Alive condition has useless data in value1 (%lu)!", cond->ConditionValue1);
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Alive condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Alive condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_HP_VAL:
        {
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outError(LOG_FILTER_SQL, "HpVal condition has invalid ComparisionType (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "HpVal condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_HP_PCT:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "HpPct condition has too big percent value (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue2 >= COMP_TYPE_MAX)
            {
                sLog->outError(LOG_FILTER_SQL, "HpPct condition has invalid ComparisionType (%lu), skipped", cond->ConditionValue2);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "HpPct condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_AREAID:
        case CONDITION_INSTANCE_DATA:
        ...
        case CONDITION_WORLD_STATE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "World state condition has non existing world state in value1 (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "World state condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_PHASEMASK:
        {
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "Phasemask condition has useless data in value2 (%lu)!", cond->ConditionValue2);
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "Phasemask condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_TITLE:
        {
            ...
            if (!titleEntry)
            {
                sLog->outError(LOG_FILTER_SQL, "Title condition has non existing title in value1 (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        case CONDITION_SPAWNMASK:
        {
            if (cond->ConditionValue1 > SpawnMask::SpawnMaskRaidAll)
            {
                sLog->outError(LOG_FILTER_SQL, "Map Difficulty condition has non existing map difficulty in value1 (%lu), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
        ...
        {
            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "HasBuilding condition has useless data in value3 (%lu)!", cond->ConditionValue3);
            break;
        }
        case CONDITION_HAS_GARRISON_LEVEL:
        {
            if (cond->ConditionValue2)
                sLog->outError(LOG_FILTER_SQL, "HasGarrisonLevel condition has useless data in value3 (%lu)!", cond->ConditionValue2);

            if (cond->ConditionValue3)
                sLog->outError(LOG_FILTER_SQL, "HasGarrisonLevel condition has useless data in value3 (%lu)!", cond->ConditionValue3);

            break;
        }
        case CONDITION_QUEST_OBJECTIVE_COMPLETE:
        {
            Quest const* l_Quest = sObjectMgr->GetQuestTemplate(sObjectMgr->GetQuestObjectiveQuestId(cond->ConditionValue1));
            if (!l_Quest)
                return false;

            QuestObjective const* l_Objective = l_Quest->GetQuestObjective(cond->ConditionValue1);
            if (!l_Objective)
                return false;
        ...
        case CONDITION_CONTRIBUTION:
        {
            if (cond->ConditionValue2 <= 0 || cond->ConditionValue2 > eContributionStates::MaxState)
                return false;
        }
        default:
            break;
