    ...
    do
    {
        ...
        switch (type)
        {
            case DISABLE_TYPE_SPELL:
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Spell entry %u from `disables` doesn't exist in db2, skipped.", entry);
                    continue;
                }
            ...
            case DISABLE_TYPE_MAP:
            {
                ...
                if (!mapEntry)
                {
                    sLog->outError(LOG_FILTER_SQL, "Map entry %u from `disables` doesn't exist in db2, skipped.", entry);
                    continue;
                }
            ...
            case DISABLE_TYPE_BATTLEGROUND:
                ...
                {
                    sLog->outError(LOG_FILTER_SQL, "Battleground entry %u from `disables` doesn't exist in db2, skipped.", entry);
                    continue;
                }
                if (flags)
            ...
            case DISABLE_TYPE_GARRISON_MISSION:
            {
                ...
                if (!l_MissionEntry)
                {
                    sLog->outError(LOG_FILTER_SQL, "Garrison mission entry %u from `disables` doesn't exist in db2, skipped.", entry);
                    continue;
                }
                break;
            }
