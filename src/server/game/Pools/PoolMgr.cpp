...
void PoolGroup<Creature>::Despawn1Object(uint32 guid)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(guid))
    {
        ...
        if (Creature* creature = ObjectAccessor::GetObjectInWorld(MAKE_NEW_GUID(guid, data->id, HIGHGUID_UNIT), (Creature*)NULL))
            creature->AddDelayedEvent([creature]()->void { creature->AddObjectToRemoveList(); }, 0);
    }
}
...
void PoolGroup<GameObject>::Despawn1Object(uint32 guid)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(guid))
    {
        ...
        if (GameObject* pGameobject = ObjectAccessor::GetObjectInWorld(MAKE_NEW_GUID(guid, data->id, HIGHGUID_GAMEOBJECT), (GameObject*)NULL))
            pGameobject->AddDelayedEvent([pGameobject]()->void { pGameobject->AddObjectToRemoveList(); }, 0);
    }
}
...
void PoolGroup<Creature>::Spawn1Object(PoolObject* obj)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
    {
        ...
        uint32 l_Zone = data->zoneId;// sMapMgr->GetZoneId(data->mapid, data->posX, data->posY, data->posZ);
        if (Map* l_Map = sMapMgr->CreateBaseMap(data->mapid, l_Zone))
        {
            uint64 l_Guid = obj->guid;
            l_Map->AddTask([l_Map, data, l_Guid]()->void
            {
                ...
                if (!l_Map->Instanceable() && l_Map->IsGridLoaded(data->posX, data->posY))
                {
                    ...
                    if (!creature->LoadCreatureFromDB(l_Guid, l_Map))
                    {
...
void PoolGroup<GameObject>::Spawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
    {
        ...
        std::list<Map*> maps = sMapMgr->GetMapsPerZoneId(data->mapid, l_Zone);
        for (Map* l_Map : maps)
        {
            l_Map->AddTask([l_Map, data, l_Guid]()->void
            {
                ...
                if (!l_Map->Instanceable() && l_Map->IsGridLoaded(data->posX, data->posY))
                {
                    ...
                    if (!pGameobject->LoadGameObjectFromDB(l_Guid, l_Map, false))
                    {
...
void PoolGroup<Creature>::ReSpawn1Object(PoolObject* obj)
{
    if (CreatureData const* data = sObjectMgr->GetCreatureData(obj->guid))
    {
        ...
        {
            creature->GetMap()->AddTask([creature]()->void
            {
...
void PoolGroup<GameObject>::ReSpawn1Object(PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr->GetGOData(obj->guid))
    {
        ...
        {
            pGameobject->GetMap()->AddTask([pGameobject]()->void
            {
