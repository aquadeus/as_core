...
class InstanceSave
{
    ...
    public:
        ...
        void AddPlayer(Player* player) { std::lock_guard<std::recursive_mutex> l_Guard(m_PlayerLock); m_playerList.push_back(player); }
        bool RemovePlayer(Player* player);
        /* all groups bound to the instance */
        void AddGroup(GroupPtr group) { std::lock_guard<std::recursive_mutex> l_Guard(m_GroupLock); m_groupList.push_back(group); }
        bool RemoveGroup(GroupPtr group);
    ...
    private:
        ...
        std::recursive_mutex m_PlayerLock;
...
class InstanceSaveManager
{
    ...
    public:
        typedef std::unordered_map<uint32 /*InstanceId*/, InstanceSavePtr> InstanceSaveHashMap;
        ...
        time_t GetSubsequentResetTime(uint32 p_MapID, Difficulty p_Difficulty, time_t p_ResetTime) const;
        void ScheduleReset(bool add, time_t time, InstResetEvent event);

        void Update();

        InstanceSavePtr AddInstanceSave(uint32 mapId, uint32 instanceId, Difficulty difficulty, time_t resetTime,
            bool canReset, bool load = false);
        void RemoveInstanceSave(uint32 InstanceId);
        static void DeleteInstanceFromDB(uint32 instanceid);

        InstanceSavePtr GetInstanceSave(uint32 InstanceId);
    ...
    private:
        void _ResetOrWarnAll(uint32 mapid, Difficulty difficulty, bool warn, time_t resetTime, SQLTransaction& p_Trans);
