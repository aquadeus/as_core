...
using AreaPolygon = std::vector<G3D::Vector2>;
...
class PathGenerator
{
    public:
        ...
        bool CalculatePath(float destX, float destY, float destZ, bool forceDest = false, bool straightLine = false, Unit* l_Target = nullptr, bool useCombatReach = false);
        ...
        void UpdateNavMeshQuery();

        static void AddShortcutArea(uint32 p_MapId, AreaPolygon const p_Corners)
        {
            m_ShortcutAreas[p_MapId].push_back(p_Corners);
        }

        static bool NeedToUseShortcut(G3D::Vector3 const& p_StartPos, G3D::Vector3 const& p_EndPos, uint32 p_MapId);

    private:
        static std::map<uint32, std::vector<AreaPolygon>> m_ShortcutAreas;
        ...
        static bool IsInPolygon(G3D::Vector3 const& p_Check, std::vector<G3D::Vector2> const& p_Corners);
        bool NeedToUseShortcut(G3D::Vector3 const& p_StartPos, G3D::Vector3 const& p_EndPos);
