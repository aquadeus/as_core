...
void PointMovementGenerator<T>::DoInitialize(T* unit)
{
    ...
    if (m_FaceTargetGUID)
    {
        if (Unit const* l_FaceTarget = sObjectAccessor->GetUnit(*unit, m_FaceTargetGUID))
            init.SetFacing(l_FaceTarget);
    }
    if (m_SpellEffectExtraData)
        init.SetSpellEffectExtraData(*m_SpellEffectExtraData);
    init.Launch();
...
void PointMovementGenerator<T>::DoFinalize(T* unit)
{
    ...
    uint64 l_Guid = unit->GetGUID();

    unit->AddDelayedEvent([unit]() -> void
    {
        ...
        if (unit->GetVehicleBase())
        {
            ...
            while (!l_RecursivesVehicles.empty())
            {
                ...
                if (!l_Vehicle->movespline->Finalized())
    ...
    if (unit->movespline->Finalized())
    {
        ...
        if (m_CastAtArrival)
        {
            ...
            float l_Y = i_y;
            float l_Z = i_z;
            uint32 l_Spell = m_CastAtArrival;

            unit->AddDelayedEvent([l_X, l_Y, l_Z, l_Spell, unit]() -> void
            {
                unit->CastSpell(l_X, l_Y, l_Z, l_Spell, true);
            });
        }
    }
}
...
{
    /// HAVE to be delayed cast spell or that can cause crash
    if (_arrivalSpellId)
    {
        if (Unit* l_Target = ObjectAccessor::GetUnit(*unit, _arrivalSpellTargetGuid))
            unit->DelayedCastSpell(l_Target, _arrivalSpellId, true, 1);
        else
            unit->DelayedCastSpell(unit, _arrivalSpellId, true, 1);
    }
...
void EffectMovementGenerator::MovementInform(Unit* unit)
{
    ...
    switch (_id)
    {
        ...
        case specialSpells::ChainHurlJumpDest:
            ...
            unit->PlayOneShotAnimKitId(specialSpells::AnimChainHurl);
