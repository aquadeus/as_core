...
{
    protected:
        TargetedMovementGeneratorMedium(Unit* target, float offset, float angle, bool facingTargetWithAngle) :
            TargetedMovementGeneratorBase(target), i_path(NULL),
            i_recheckDistance(0), i_offset(offset), i_angle(angle),
            i_recalculateTravel(false), i_targetReached(false), i_teleportAtOwnerLanding(false),
            i_facingTargetWithAngle(facingTargetWithAngle)
        {
        }
    ...
    protected:
        ...
        bool i_teleportAtOwnerLanding;
        bool i_facingTargetWithAngle : 1;
};
...
class ChaseMovementGenerator : public TargetedMovementGeneratorMedium<T, ChaseMovementGenerator<T> >
{
    public:
        ...
        ChaseMovementGenerator(Unit* target, float offset, float angle, bool facingTargetWithAngle)
            : TargetedMovementGeneratorMedium<T, ChaseMovementGenerator<T> >(target, offset, angle, facingTargetWithAngle) { }
