...
void TargetedMovementGeneratorMedium<T, D>::_setTargetLocation(T* owner, bool updateDestination)
{
    ...
    if (updateDestination || !i_path)
    {
        ...
        else
        {
            ...
            if ((owner->isPet() && i_target->GetTypeId() == TYPEID_PLAYER) || (owner->GetOwner() && owner->GetOwner()->IsPlayer()))
            {
            ...
            if (i_target->IsWithinDistInMap(owner, dist) && !(owner->GetOwner() && owner->GetOwner()->IsPlayer()))
                return;
            ...
            ///else
            {
                ...
                /// Do not try to get the right angle if i_target is also chasing owner
                if ((i_target->GetTypeId() == TYPEID_UNIT || i_target->GetTypeId() == TYPEID_PLAYER)
                    && (owner->getAttackers().find(i_target->ToUnit()) != owner->getAttackers().end()))
                    i_target->GetClosePoint(x, y, z, size, i_offset, 0.0f);
                else
    ...
    if (i_teleportAtOwnerLanding)
    {
        Unit* l_PetOwner = owner->GetOwner();
        if (!l_PetOwner)
            i_teleportAtOwnerLanding = false;

        if (l_PetOwner && !l_PetOwner->IsFalling())
        {
            owner->NearTeleportTo(x, y, z, l_PetOwner->GetOrientation());
            i_teleportAtOwnerLanding = false;
        }
        return;
    }
    ...
    if (i_path->GetPathType() & PATHFIND_NOT_USING_PATH && forceDest)
    {
        /// Don't let the pet walk in the air to reach his owner while the owner is falling, we'll teleport the pet to the owner position when he will finish to fall
        Unit* l_PetOwner = owner->GetOwner();
        if (l_PetOwner && l_PetOwner->IsFalling())
        {
            i_teleportAtOwnerLanding = true;
            return;
        }
    }

    if (i_target.getTarget()->GetTransGUID())
        z = i_target.getTarget()->GetPositionZ() + owner->GetPositionH();
...
bool TargetedMovementGeneratorMedium<T, D>::DoUpdate(T* owner, uint32 time_diff)
{
    ...
    if (owner->HasUnitState(UnitState::UNIT_STATE_CASTING) && !owner->HasUnitState(UnitState::UNIT_STATE_MOVE_IN_CASTING))
    {
        for (uint8 l_I = CurrentSpellTypes::CURRENT_GENERIC_SPELL; l_I <= CurrentSpellTypes::CURRENT_CHANNELED_SPELL; ++l_I)
        {
            if (Spell const* l_Spell = owner->GetCurrentSpell(l_I))
            {
                if (l_Spell->m_spellInfo->IsChanneled() || l_Spell->m_spellInfo->InterruptFlags & SpellInterruptFlags::SPELL_INTERRUPT_FLAG_MOVEMENT)
                {
                    if (!owner->IsStopped())
                        owner->StopMoving();
    ...
    if (static_cast<D*>(this)->_lostTarget(owner))
    {
        ...
        if (!owner->HasUnitState(UnitState::UNIT_STATE_CASTING))
        {
            owner->AddDelayedEvent([owner]() -> void
            {
                if (Unit* l_Target = owner->getVictim())
                {
                    owner->GetMotionMaster()->Clear();
                    owner->GetMotionMaster()->MoveChase(owner->getVictim());
                }
            }, 1);
        }
    ...
    if (i_recheckDistance.Passed())
    {
        ...
        ///else
        {
            ...
            if (!owner->GetOwner() || !owner->GetOwner()->IsPlayer())
            {
            ...
            ///< Or Check Distance and Orientation For Player's Pets and guardians
            else if (!owner->GetVehicleKit() || (owner->GetVehicleKit() != owner->GetOwner()->GetVehicle())) ///< If owner of motionmaster is the vehicle in which the player is, the vehicle should not follow its owner
            {
                float x = 0.0f;
                float y = 0.0f;
                float z = 0.0f;
                ...
                if (owner->GetTypeId() == TYPEID_UNIT && owner->ToCreature()->CanFly())
                    targetMoved = !owner->IsWithinDist3d(x, y, z, 0.5f);
                else
                    targetMoved = !owner->IsWithinDist2d(x, y, 0.5f);
            }
        }
    ...
    if (owner->movespline->Finalized())
    {
        ...
        if ((i_angle != 0.0f || i_facingTargetWithAngle) && !owner->HasInArc(0.01f, i_target.getTarget()))
