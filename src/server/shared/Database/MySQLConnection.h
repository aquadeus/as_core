...
#define PREPARE_STATEMENT(a, b, c) PrepareStatement(a, b, CONNECTION_BOTH);
...
class MySQLConnection
{
    ...
    public:
        ...
        MySQLConnection(moodycamel::ConcurrentQueue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo);  //! Constructor for asynchronous connections.
        ...
        void SetIgnoreNoSuchTable(bool p_State)
        {
            m_IgnoreNoSuchTable = p_State;
        }
    ...
    private:
        moodycamel::ConcurrentQueue<SQLOperation*>* m_queue;      //! Queue shared with other asynchronous connections.
        std::unique_ptr<DatabaseWorker> m_worker;           //! Core worker task.
        MYSQL*                m_Mysql;                      //! MySQL Handle.
        MySQLConnectionInfo&  m_connectionInfo;             //! Connection info (used for logging)
        ConnectionFlags       m_connectionFlags;            //! Connection flags (for preparing relevant statements)
        std::mutex            m_Mutex;
        bool                  m_IgnoreNoSuchTable;
};
