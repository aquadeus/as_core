...
MySQLConnection::MySQLConnection(moodycamel::ConcurrentQueue<SQLOperation*>* queue, MySQLConnectionInfo& connInfo) :
m_reconnecting(false),
m_prepareError(false),
m_queue(queue),
m_Mysql(NULL),
...
{
    m_worker = std::unique_ptr<DatabaseWorker>(new DatabaseWorker(m_queue, this));
}

MySQLConnection::~MySQLConnection()
{
    if (m_worker)
        delete m_worker.release();

    for (size_t i = 0; i < m_stmts.size(); ++i)
        delete m_stmts[i].release();
...
uint32 MySQLConnection::Open()
{
    ...
    {
        sLog->outError(LOG_FILTER_SQL, "Could not connect to MySQL database at %s: %s", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        if (mysql_query(m_Mysql, sql))
        {
            ...
            sLog->outDebug(LOG_FILTER_SQL, "SQL: %s", sql);
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    uint32 index = stmt->m_index;
    {
        ...
        sLog->outDebug(LOG_FILTER_SQL, "SQL(p): %s", m_mStmt->getQueryString(m_queries[index].first).c_str());
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        ...
        printf("In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql);
    ...
    else
    {
        if (mysql_stmt_prepare(stmt, sql, static_cast<unsigned long>(strlen(sql))))
        {
            ...
            printf("In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql);
        ...
        {
            m_stmts[index] = std::unique_ptr<MySQLPreparedStatement>(new MySQLPreparedStatement(stmt));
        }
    }
}
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo)
{
    switch (errNo)
    {
        ...
        case ER_NO_SUCH_TABLE:
            if (errNo == ER_NO_SUCH_TABLE && m_IgnoreNoSuchTable)
                return true;
