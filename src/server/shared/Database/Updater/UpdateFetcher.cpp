...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            sLog->outTrace(LOG_FILTER_SQL, "Added locale file \"%s\".", itr->path().filename().generic_string().c_str());
            ...
            {
                sLog->outFatal(LOG_FILTER_SQL, "Duplicate filename \"%s\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            sLog->outWarn(LOG_FILTER_SQL, "DBUpdater: Given update include directory \"%s\" does not exist, skipped!", p.generic_string().c_str());
            continue;
        }
        ...
        sLog->outTrace(LOG_FILTER_SQL, "Added applied file \"%s\" from remote.", p.filename().generic_string().c_str());
...
std::string UpdateFetcher::ReadSQLUpdate(boost::filesystem::path const& file) const
{
    ...
    if (!in.is_open())
    {
        sLog->outInfo(LOG_FILTER_SQL, "Failed to open the sql update \"%s\" for reading! "
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
    ...
    {
        sLog->outDebug(LOG_FILTER_SQL, "Checking update \"%s\"...", availableQuery.first.filename().generic_string().c_str());
        ...
        if (iter != applied.end())
        {
            ...
            if (!redundancyChecks)
            {
                sLog->outDebug(LOG_FILTER_SQL, ">> Update is already applied, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
            ...
            {
                sLog->outDebug(LOG_FILTER_SQL, ">> Update is archived and marked as archived in database, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
        }
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    sLog->outWarn(LOG_FILTER_SQL, ">> It seems like the update \"%s\" \'%s\' was renamed, but the old file is still there! " \
                ...
                {
                    sLog->outInfo(LOG_FILTER_SQL, ">> Renaming update \"%s\" to \"%s\" \'%s\'.",
            ...
            {
                sLog->outInfo(LOG_FILTER_SQL, ">> Applying update \"%s\" \'%s\'...",
                    availableQuery.first.filename().string().c_str(), hash.substr(0, 7).c_str());
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            sLog->outInfo(LOG_FILTER_SQL, ">> Re-hashing update \"%s\" \'%s\'...", availableQuery.first.filename().string().c_str(),
        ...
        else
        {
            ...
            {
                sLog->outInfo(LOG_FILTER_SQL, ">> Reapplying update \"%s\" \'%s\' -> \'%s\' (it changed)...", availableQuery.first.filename().string().c_str(),
            ...
            else
            {
                ...
                {
                    sLog->outInfo(LOG_FILTER_SQL, ">> Updating the state of \"%s\" to \'%s\'...",
                ...
                sLog->outDebug(LOG_FILTER_SQL, ">> Update is already applied and matches the hash \'%s\'.", hash.substr(0, 7).c_str());
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            /// We dont need this output for the moment
            ///sLog->outWarn(LOG_FILTER_SQL, ">> The file \'%s\' was applied to the database, but is missing in" \
            ///" your update directory now!", entry.first.c_str());

            if (doCleanup)
                sLog->outInfo(LOG_FILTER_SQL, "Deleting orphaned entry \'%s\'...", entry.first.c_str());
        }
        ...
        {
            /// We dont need this output for the moment
            ///sLog->outError(LOG_FILTER_SQL, "Cleanup is disabled! There were %lu dirty files applied to your database, " \
