...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(LOGIN_SEL_LAST_ACCOUNT_NOTE, "SELECT id, note, date, notedby FROM account_note WHERE account_id = ? ORDER BY ID DESC LIMIT 1", CONNECTION_SYNCH);
    ...
    PREPARE_STATEMENT(LOGIN_INS_CHAR_SPELL, "INSERT INTO account_spell (accountId, spell, active, disabled, IsMountFavorite, groupRealmMask) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE groupRealmMask = groupRealmMask | ?, IsMountFavorite = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_CHARACTER_SPELL, "SELECT spell, active, disabled, IsMountFavorite, groupRealmMask FROM account_spell WHERE accountId = ?", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(LOGIN_SEL_TRANSFERS_EXP_LOAD, "SELECT `id`, `account`, `guid`, `dump`, `startrealm` FROM webshop_delivery_interexp_transfer WHERE `destrealm` = ? AND state = 1 LIMIT 50", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_UPD_TRANSFER_EXP, "UPDATE webshop_delivery_interexp_transfer SET state = 1, dump = ? WHERE id = ?", CONNECTION_ASYNC);
...
#undef PETBATTLE_FULL_FIELDS
    ...
    //////////////////////////////////////////////////////////////////////////
    /// Wardrobe
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_WARDROBES, "SELECT blobIndex, appearanceMask FROM account_wardrobes WHERE accountId = ? ORDER BY blobIndex DESC", CONNECTION_BOTH);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_WARDROBES, "INSERT INTO account_wardrobes (accountId, blobIndex, appearanceMask) VALUES (?, ?, ?) "
                                                   "ON DUPLICATE KEY UPDATE appearanceMask = appearanceMask | VALUES(appearanceMask)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_WARDROBE_FAVORITES, "SELECT itemModifiedAppearanceId FROM account_wardrobe_favorites WHERE accountId = ?", CONNECTION_BOTH);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_WARDROBE_FAVORITE, "INSERT INTO account_wardrobe_favorites (accountId, itemModifiedAppearanceId) VALUES (?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_WARDROBE_FAVORITE, "DELETE FROM account_wardrobe_favorites WHERE accountId = ? AND itemModifiedAppearanceId = ?", CONNECTION_ASYNC);
    //////////////////////////////////////////////////////////////////////////
    ...
    //////////////////////////////////////////////////////////////////////////
    /// Achievement
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_ACHIEVEMENTS, "SELECT achievement, date FROM account_legion_achievement WHERE account = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_CRITERIAPROGRESS, "SELECT criteria, counter, date FROM account_legion_achievement_progress WHERE account = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACHIEVEMENT_PROGRESS_BY_CRITERIA, "DELETE FROM account_legion_achievement_progress WHERE account = ? AND criteria = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_ACHIEVEMENT_PROGRESS, "INSERT INTO account_legion_achievement_progress (account, criteria, counter, date) VALUES (?, ?, ?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_ACHIEVEMENT_BY_ACHIEVEMENT, "DELETE FROM account_legion_achievement WHERE achievement = ? AND account = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_ACHIEVEMENT, "INSERT INTO account_legion_achievement (account, achievement, date) VALUES (?, ?, ?)", CONNECTION_ASYNC);
    //////////////////////////////////////////////////////////////////////////

    /////////////////////////////////////////////////////////////////////////
    /// Recruit A Friend
    PREPARE_STATEMENT(LOGIN_SEL_RAF_REQUEST_MAILS, "SELECT mail FROM recruit_a_friend_request", CONNECTION_BOTH)
    PREPARE_STATEMENT(LOGIN_INS_RAF_REQUEST, "INSERT INTO recruit_a_friend_request (mail, refererName, state, faction, message, refererAccountId, realmId) VALUES (?, ?, 1, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_SEL_RAF_ACC, "SELECT realmId, refererAccountId, recruitAccountId, faction, recruitCharacterGuid, startTime FROM recruit_a_friend WHERE refererAccountId = ? OR recruitAccountId = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_INS_RAF_ACC, "INSERT INTO recruit_a_friend (realmId, refererAccountId, recruitAccountId, faction) VALUE (?, ?, ?, ?) ON DUPLICATE KEY UPDATE recruitAccountId = recruitAccountId", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_UPD_RAF_CHARACTER, "UPDATE recruit_a_friend SET recruitCharacterGuid = ?, startTime = ? WHERE recruitAccountId = ?", CONNECTION_SYNCH)
    PREPARE_STATEMENT(LOGIN_SEL_RAF_ACC_BONUSES, "SELECT legendary, rewardMask, rewardAvailable, notificationsEnabled FROM recruit_a_friend_bonuses WHERE accountId = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_REP_RAF_ACC_BONUSES, "REPLACE INTO recruit_a_friend_bonuses(accountId, legendary, rewardMask, rewardAvailable, notificationsEnabled) VALUES (?, ?, ?, ?, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_UPD_RAF_ACC_BONUSES_REWARD, "UPDATE recruit_a_friend_bonuses SET rewardAvailable = rewardAvailable + 1 WHERE accountId = (SELECT refererAccountId FROM recruit_a_friend WHERE recruitCharacterGuid = ?)", CONNECTION_ASYNC)
    /////////////////////////////////////////////////////////////////////////
    ...
    PREPARE_STATEMENT(LOGIN_SEL_COSMETIC, "SELECT type, cosmetic_id FROM account_cosmetic WHERE account = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_INS_COSMETIC, "INSERT INTO account_cosmetic (account, type, cosmetic_id) VALUE (?, ?, ?)", CONNECTION_ASYNC)

    PREPARE_STATEMENT(LOGIN_SEL_ACCS_SECURITY, "SELECT id, gmlevel FROM account_access WHERE RealmID = ? OR RealmID = -1", CONNECTION_ASYNC)

    PREPARE_STATEMENT(LOGIN_INS_REAL_UPDATE_TIME, "INSERT INTO real_update_time (realm_id, timestamp, uptime, diff, players, queue) VALUE (?, CURRENT_TIMESTAMP(), ?, ?, ?, ?)", CONNECTION_ASYNC)

    PREPARE_STATEMENT(LOGIN_DEL_LAST_PLAYED_CHARACTER, "DELETE FROM account_last_played_character WHERE accountId = ? and realmId = ?", CONNECTION_ASYNC)
    PREPARE_STATEMENT(LOGIN_INS_LAST_PLAYED_CHARACTER, "INSERT INTO account_last_played_character(accountId, region, battlegroup, realmId, characterName, characterGUID, lastPlayedTime) VALUE (?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC)

    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_QUEST_REWARDED, "SELECT quest FROM account_queststatus_rewarded WHERE account_id = ?",  CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_INS_ACCOUNT_QUEST_REWARDED, "INSERT INTO account_queststatus_rewarded (account_id, quest) VALUE (?, ?)", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_DEL_ACCOUNT_QUEST_REWARDED, "DELETE FROM account_queststatus_rewarded WHERE account_id = ? AND quest = ?", CONNECTION_ASYNC);

    PREPARE_STATEMENT(LOGIN_SEL_AUTH_SESSION_ACCOUNT, "SELECT id, sessionkey, last_ip, locked, v, s, expansion, mutetime, locale, recruiter, os, username, UNIX_TIMESTAMP(joindate), service_flags, custom_flags, last_mac_addr FROM account WHERE username = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_AUTH_SESSION_GMLEVEL, "SELECT RealmID, gmlevel, animator FROM account_access WHERE id = ? AND(RealmID = ? OR RealmID = '-1')", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_AUTH_SESSION_BANNED,  "SELECT 1 FROM account_banned WHERE id = ? AND active = 1  UNION  SELECT 1 FROM ip_banned WHERE ip = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_AUTH_SESSION_PREMIUM, "SELECT premium_type FROM account_premium WHERE id = ? AND active = 1", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_AUTH_SESSION_TRUSTED, "SELECT id from account_trusted where id = ?", CONNECTION_ASYNC);
    PREPARE_STATEMENT(LOGIN_SEL_ACCOUNT_SESSIONKEY, "SELECT sessionkey FROM account WHERE id = ?", CONNECTION_ASYNC)
}
