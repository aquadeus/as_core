...
void WebDatabaseConnection::DoPrepareStatements()
{
    ...
    PREPARE_STATEMENT(WEB_INS_PURCHASE, "INSERT INTO `api_purchase` (AccountID, Expansion, Realm, CharacterGUID, ShopEntryData, Count, TypePrice, Price, IP, ExtraData) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?, \"\")", CONNECTION_ASYNC);
    ...
    PREPARE_STATEMENT(WEB_INS_FIRESTORM_POINTS, "INSERT INTO `api_points` (AccountID, Points, TypePoints, Data) VALUES (?, ?, 1, ?)", CONNECTION_ASYNC)
    PREPARE_STATEMENT(WEB_SEL_MIGRATION, "SELECT id, AccountID, Dump1, Dump2 FROM api_external_transfers WHERE state = 1 LIMIT 1", CONNECTION_ASYNC);
    PREPARE_STATEMENT(WEB_UPD_MIGRATION, "UPDATE api_external_transfers SET state = 3 WHERE id = ?", CONNECTION_ASYNC);
}
