...
{
    uint32 l_ExecuteErrorCode = m_conn->ExecuteTransaction(m_trans);
    ...
    if (m_conn->GetLastError() == 1213)
    {
        ...
        {
            l_ExecuteErrorCode = m_conn->ExecuteTransaction(m_trans);
            if (l_ExecuteErrorCode == 0)
                break;
        }
    }

    if (m_Callback != nullptr)
        m_Callback->m_State = l_ExecuteErrorCode == 0 ? MS::Utilities::CallBackState::Success : MS::Utilities::CallBackState::Fail;
