...
void Log::CreateAppenderFromConfig(const char* name)
{
    ...
    {
        fprintf(stderr, "Log::CreateAppenderFromConfig: Wrong Log Level %i for appender %s\n", level, name);
        return;
    }
    ...
    switch (type)
    {
        ...
        default:
            fprintf(stderr, "Log::CreateAppenderFromConfig: Unknown type %i for appender %s\n", type, name);
            break;
    }
}
...
class SlackOutputRunnable : public ACE_Based::Runnable
{
    public:
        void run()
        {
            /// curl call can cause SIGPIPE signal if the distant host close the socket while we write on it
            signal(SIGPIPE, &MS::SignalHandler::OnSignalReceive);
            MS::SignalHandler::SetFailureCountLimit(0xFFFFFFFF);
            MS::SignalHandler::EnableThrowExceptionAtFailure();

            while (true)
            {
                std::function<void()> l_Task;
                while (m_Queue.next(l_Task))
                    l_Task();
        ...
        ACE_Based::LockedQueue<std::function<void()>, ACE_Thread_Mutex> m_Queue;
};

static SlackOutputRunnable* g_SlackThread = nullptr;

/// Slack API Documentation : https://api.slack.com/docs/attachments
void Log::outExtChat(std::string const& p_Dest, std::string const& p_Color, bool p_IsAttachement, const char* p_Message, ...)
{
#ifndef WIN_32
    ...
    if (!g_SlackThread)
    {
        g_SlackThread = new SlackOutputRunnable;
        new ACE_Based::Thread(g_SlackThread, "SlackOutputRunnable");
    }
    ...
    g_SlackThread->m_Queue.add([=]() -> void
    {
        ...
        try
        {
            ...
            if (l_Curl)
            {
                ...
                if (p_IsAttachement)
                    l_PostData << "{\"attachments\": [{\"pretext\": \"*" << std::string(l_SlackAppName) << "*\", \"text\": \"" << std::string(l_Message) << "\", \"color\": \"" << std::string(p_Color) << "\", \"mrkdwn_in\": [\"pretext\", \"text\"]}], \"channel\": \"" << std::string(p_Dest) << "\"}";
                else
                    l_PostData << "{\"text\": \"" << std::string(l_Message) << "\", \"channel\": \"" << std::string(p_Dest) << "\"}";

                std::string l_DataTxt = l_PostData.str();

                struct curl_slist* l_HTTPHeaders = nullptr;
                l_HTTPHeaders = curl_slist_append(l_HTTPHeaders, "Content-Type: application/json");
                ...
                curl_easy_setopt(l_Curl, CURLOPT_HTTPHEADER, l_HTTPHeaders);

                // output to slack...
                const char* l_SlackWebhook = std::getenv("SLACK_WEBHOOK");
                if (l_SlackWebhook)
                {
                    curl_easy_setopt(l_Curl, CURLOPT_URL, l_SlackWebhook);
                ...
                // ...and output to mattermost at the same time
                const char* l_MattermostWebhook = std::getenv("MATTERMOST_WEBHOOK");
                if (l_MattermostWebhook)
                {
                    curl_easy_setopt(l_Curl, CURLOPT_URL, l_MattermostWebhook);
        ...
        {
            fprintf(stderr, "curl_easy_perform() failed (SIGPIPE)\n");
        }
    });
#endif
}
