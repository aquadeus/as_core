...
char* DBCFileLoader::AutoProduceData(const char* format, uint32& records, char**& indexTable, uint32 sqlRecordCount, uint32 sqlHighestIndex, char*& sqlDataTable)
{
    ...
    */
    ...
    if (i >= 0)
    {
        ...
        {
            int32 ind = getRecord(y).getInt(i);
            if (ind > maxi)
                maxi = ind;
        }

        // If higher index avalible from sql - use it instead of dbcs
        if ((int32)sqlHighestIndex > maxi)
    ...
    for (uint32 y = 0; y < recordCount; ++y)
    {
        if (i >= 0)
        {
            int32 l_Index = getRecord(y).getInt(i);

            /// This check is needed, because of some blizzards hacks in the DBCs
            if (l_Index >= 0)
                indexTable[l_Index] = &dataTable[offset];
        }
        else
