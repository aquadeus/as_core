...
bool utf8IsValid(const std::string& utf8str)
{
...
bool Utf8toWStr(char const* utf8str, size_t csize, wchar_t* wstr, size_t& wsize)
{
    try
    {
        ...
        std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
        auto wstrConverted = converter.from_bytes(utf8str);
        
        if (wstrConverted.size() > wsize)
        {
            wstr[0] = L'\0';
            wsize = 0;
            return false;
        ...
        wcsncpy(wstr, wstrConverted.c_str(), wstrConverted.size());
        wstr[wstrConverted.size()] = L'\0';
    }
    catch (std::exception)
    {
        if (wsize > 0)
...
bool Utf8toWStr(const std::string& utf8str, std::wstring& wstr)
{
    try
    {
        ...
        {
            std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>> converter;
            wstr = converter.from_bytes(utf8str);
        }
    }
    catch(std::exception)
    {
