                              ...
                              bool secure)
{
    ...
    if (!secure)
    {
        sLog->outTrace(LOG_FILTER_GENERAL, "Starting process \"%s\" with arguments: \"%s\".",
    ...
    {
        sLog->outInfo(LOG_FILTER_GENERAL, "%s", msg.c_str());
    });

    auto outError = MakeTCLogSink([&](std::string msg)
    {
        sLog->outError(LOG_FILTER_GENERAL, "%s", msg.c_str());
    });
    ...
    if (!secure)
    {
        sLog->outTrace(LOG_FILTER_GENERAL, ">> Process \"%s\" finished with return value %i.",
                 ...
                 std::string const& logger, std::string input_file, bool secure)
{
    return CreateChildProcess([](child& c) -> int
    {
        ...
        {
            printf("Exception ""%s"" when calling wait_for_exit", e.what());
            return EXIT_FAILURE;
        }
