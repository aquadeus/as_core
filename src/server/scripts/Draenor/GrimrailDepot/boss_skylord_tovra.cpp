                ...
                if (m_Count >= 3)
                {
                    ...
                    if (m_Instance != nullptr)
                    {
                        if (Creature* l_Dragon = m_Instance->instance->GetCreature(m_Instance->GetData64(GrimrailDepotData::DataSkyLordTovraDragon)))
                        {
                            ...
                            l_Dragon->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
...
public:
    ...
    void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time) override
    {
        if (Unit* l_Caster = p_AreaTrigger->GetCaster())
        {
            ...
            if (l_Diff <= p_Time)
            {
                if (Player* l_Player = p_AreaTrigger->FindNearestPlayer(2.0f))
                {
                    p_AreaTrigger->SetDuration(5);
