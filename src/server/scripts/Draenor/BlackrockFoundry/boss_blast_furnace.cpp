            ...
            void OnSpellCasted(SpellInfo const* p_SpellInfo) override
            {
                switch (p_SpellInfo->Id)
                {
                    case eSpells::Reanimate:
                    {
                        ...
                        me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
...
class spell_foundry_damage_shield : public SpellScriptLoader
{
    public:
        ...
        class spell_foundry_damage_shield_AuraScript : public AuraScript
        {
            ...
            void Register() override
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_foundry_damage_shield_AuraScript::AfterRemove, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN, AURA_EFFECT_HANDLE_REAL);
            }
        };
    ...
    public:
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 /*p_Time*/) override
        {
            if (Unit* l_Caster = p_AreaTrigger->GetCaster())
            {
                ...
                if (p_AreaTrigger->IsInPolygon(l_Caster))
                {
