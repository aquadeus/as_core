    ...
    public:
        ...
        struct iron_docks_nokgar_mob_dreadfangAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_NOT_SELECTABLE | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                ...
                if (p_Damage && p_Damage > 0)
                {
                    if (!m_Dead)
                    {
                        if (me->GetHealthPct() <= 12)
                        {
                            ...
                            me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | eUnitFlags::UNIT_FLAG_NOT_SELECTABLE);
               ...
               // if (me->HasUnitState(UnitState::UNIT_STATE_CASTING) || m_ShreddingStrikes)
                ...
                if (m_SavageMaullingDiff <= p_Diff)
                {
                    if (Unit* l_Target = SelectTarget(SelectAggroTarget::SELECT_TARGET_TOPAGGRO, 0, 50.0f, true))
                    {
                        ...
                        me->GetMotionMaster()->MoveJump(*l_Target, 8.0f, 4.0f, eMovementInformed::MovementInformDreadfangFranticMauling);
                    }
            ...
            void DoAction(int32 const p_Action) override
            {
                switch (p_Action)
                {
                    ...
                    case eIronDocksActions::ActionBurningArrowSingle:
                        if (Player* l_Target = me->FindNearestPlayer(200.0f))
