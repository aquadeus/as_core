    ...
    public:
        ...
        struct boss_skullocAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
            }
            ...
            void DoAction(int32 const p_Action) override
            {
                switch (p_Action)
                {
                    ...
                    case eIronDocksActions::ActionBombardBridge:
                        ...
                        {
                            VehicleAccessory l_Accessory(eCreatures::CreatureCannon, 0, true, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30 * TimeConstants::IN_MILLISECONDS, Position());
                            m_Vehicle->InstallAccessory(&l_Accessory);
                        }
                        break;
                    default:
                        break;
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                ...
                if (m_BombardmentReady)
                {
                    if (m_CanBeginStoppingaBombardment)
                    {
                        if (me->FindNearestPlayer(2.0f))
                        {
                            ...
                            events.ScheduleEvent(eSkullocEvents::EventCannonBarrage, 50 * TimeConstants::IN_MILLISECONDS);
                            me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS_2, eUnitFlags2::UNIT_FLAG2_DISABLE_TURN);
                            me->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                ...
                switch (events.ExecuteEvent())
                {
                    case eSkullocEvents::EventCannonBarrage:
                    {
                        ...
                        me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                        ...
                        {                                   
                            if (m_Instance != nullptr)
                            {
                                if (Creature* l_Zoggosh = m_Instance->instance->GetCreature(m_Instance->GetData64(eIronDocksDatas::DataZuggosh)))
                                {
                                    ...
                                    l_Zoggosh->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_NOT_SELECTABLE);
                ...
                if (Unit* l_Target = GetTarget())
                {
                    if (l_Target->GetTypeId() != TypeID::TYPEID_PLAYER)
                    {
                        if (Creature* l_TargetCreature = l_Target->ToCreature())
                        {
                            ...
                            l_TargetCreature->RemoveFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC | eUnitFlags::UNIT_FLAG_IMMUNE_TO_PC | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                        }
                    }
                }
            }
