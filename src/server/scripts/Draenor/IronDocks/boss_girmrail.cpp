    ...
    public:
        ...
        struct iron_docks_grimrail_mob_ogre_trapAI : public Scripted_NoMovementAI
        {
            ...
            void UpdateAI(uint32 const p_Diff) override
            {
                if (!m_Activated)
                {
                    if (Player* l_Nearest = me->FindNearestPlayer(0.6f))
                    {
                        m_Activated = true;
                        me->RemoveAllAuras();
...
class iron_docks_grimrail_spell_flaming_slash : public SpellScriptLoader
{
    ...
    class iron_docks_grimrail_spell_flaming_slash_SpellScript : public SpellScript
    {
        ...
        void HandleAfterCast()
        {
            ...
            GetCaster()->GetMotionMaster()->MoveCharge(static_cast<Position const*>(GetHitUnit()), 42.0f, eMovementInformeds::MovementInformFlamingSlash);
        }
