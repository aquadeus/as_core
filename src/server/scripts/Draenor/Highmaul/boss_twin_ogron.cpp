                            ...
                            if (l_Timer)
                            {
                                AddTimedDelayedOperation(l_Timer, [this, p_Pos, l_Dest]() -> void
                                {
                                    if (SpellInfo const* l_SpellInfo = sSpellMgr->GetSpellInfo(eSpells::BlazeFirstSpawn))
                                    {
                                        ...
                                        if (!l_AreaTrigger->CreateAreaTrigger(sObjectMgr->GenerateLowGuid(HighGuid::HIGHGUID_AREATRIGGER), l_SpellInfo->Effects[0].MiscValue, me, nullptr, l_SpellInfo, p_Pos, l_Dest))
...
class spell_highmaul_warming_up : public SpellScriptLoader
{
    public:
        ...
        class spell_highmaul_warming_up_AuraScript : public AuraScript
        {
            ...
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_highmaul_warming_up_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
