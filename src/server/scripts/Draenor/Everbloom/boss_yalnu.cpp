...
public:
    ...
    struct the_everbloom_yalnu_mob_feral_lasherAI : public ScriptedAI
    {
        ...
        {
            if (me->FindNearestPlayer(1.8f) && m_SleepMode)
            {
                me->Kill(me);
                m_SleepMode = false;
...
public:
    ...
    struct the_everbloom_yalnu_creature_teleport_to_bossAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            me->SetReactState(ReactStates::REACT_PASSIVE);
            me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS_2, eUnitFlags2::UNIT_FLAG2_DISABLE_TURN);
            me->SetFlag(EUnitFields::UNIT_FIELD_FLAGS, eUnitFlags::UNIT_FLAG_NON_ATTACKABLE | eUnitFlags::UNIT_FLAG_NOT_SELECTABLE | eUnitFlags::UNIT_FLAG_REMOVE_CLIENT_CONTROL | eUnitFlags::UNIT_FLAG_IMMUNE_TO_NPC);
        }

        void UpdateAI(const uint32 p_Diff) override
        {
            if (Player * l_Player = me->FindNearestPlayer(2.0f))
