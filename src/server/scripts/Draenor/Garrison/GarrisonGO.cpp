...
namespace MS { namespace Garrison { namespace GDraenor
{
    ...
    bool go_garrison_cache::OnGossipHello(Player* p_Player, GameObject* p_GameObject)
    {
        ...
        {
            p_Player->GetDraenorGarrison()->RewardGarrisonCache();
    ...
    {
        p_Player->CastSpell(p_Player, MS::Garrison::GDraenor::Spells::SPELL_RELIEVED);

        if (!p_Player->HasAchieved(Achievements::ACHIEVEMENT_STAYING_REGULAR))
            p_Player->CompletedAchievement(sAchievementStore.LookupEntry(Achievements::ACHIEVEMENT_STAYING_REGULAR));
    ...
    {
        if (!p_Player || !p_Player->GetDraenorGarrison())
            return false;

        auto l_Garrison                             = p_Player->GetDraenorGarrison();
            ...
            if (l_CanGetItems)
            {
                for (auto l_RewardItem : l_RewardItems)
                {
                    ...
                    if (l_Message == EQUIP_ERR_OK)
                    {
                        ...
                        {
                            p_Player->SendDisplayToast(l_RewardItem.first, l_ItemCount, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
                ...
                for (auto l_RewardCurrency : l_RewardedCurrencies)
                {
                    ...
                    {
                        p_Player->SendDisplayToast(l_RewardCurrency.first, 0, l_RewardCurrency.second, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_CURRENCY, false, false);
    ...
    bool go_garrison_herb::OnGossipHello(Player* p_Player, GameObject* p_GameObject)
    {
        ...
        std::vector<uint64> l_Creatures = l_Owner->GetDraenorGarrison()->GetBuildingCreaturesByBuildingType(GDraenor::Building::Type::Farm);
        ...
        for (uint64 l_CreatureGUID : l_Creatures)
        {
            ...
            if (l_Creature && l_Creature->AI())
                l_Creature->AI()->SetGUID(l_PackedValue, GDraenor::CreatureAIDataIDs::GatheredPos);
        }
    ...
    bool go_garrison_deposit::OnGossipHello(Player* p_Player, GameObject* p_GameObject)
    {
        ...
        std::vector<uint64> l_Creatures = l_Owner->GetDraenorGarrison()->GetBuildingCreaturesByBuildingType(GDraenor::Building::Type::Mine);
    ...
    void gob_IronTrap_Garrison::gob_IronTrap_GarrisonAI::Reset()
    {
        ...
        {
            case TEAM_ALLIANCE:
                if (l_Owner->IsQuestRewarded(MS::Garrison::GDraenor::Quests::Alliance_BreakingIntoTheTrapGame))
                    go->SetDisplayId(MS::Garrison::GDraenor::DisplayIDs::GobIronTrapDisplay);
                else
                {
                    go->SetDisplayId(MS::Garrison::GDraenor::DisplayIDs::InvisibleDisplay);
                    m_UpdateTimer = 1500;
                }
                break;
            case TEAM_HORDE:
                if (l_Owner->IsQuestRewarded(MS::Garrison::GDraenor::Quests::Horde_BreakingIntoTheTrapGame))
                    go->SetDisplayId(MS::Garrison::GDraenor::DisplayIDs::GobIronTrapDisplay);
                else
                {
                    go->SetDisplayId(MS::Garrison::GDraenor::DisplayIDs::InvisibleDisplay);
        ...
        if (m_AnimTimer)
        {
            if (m_AnimTimer <= p_Diff)
            {
                if (Player* l_Player = sObjectAccessor->FindPlayer(m_PlayerGuid))
                {
                    ...
                    /// legacy SMSG_ITEM_BONUS_DEBUG dont exist anymore
    ...
    bool go_garrison_deactivated_mage_portal::OnGameObjectSpellCasterUse(const GameObject* /*p_GameObject*/, Player* p_User) const
    {
        ...
        if (p_User->GetDraenorGarrison())
        {
            std::vector<GDraenor::GarrisonBuilding> l_Buildings = p_User->GetDraenorGarrison()->GetBuildings();

            for (GDraenor::GarrisonBuilding l_Building : l_Buildings)
            {
                switch (l_Building.BuildingID)
                {
                    case GDraenor::Building::ID::MageTower_SpiritLodge_Level1:
                        l_BuildingLevel = 1;
                        break;
                    case GDraenor::Building::ID::MageTower_SpiritLodge_Level2:
                        l_BuildingLevel = 2;
                        break;
                    case GDraenor::Building::ID::MageTower_SpiritLodge_Level3:
                        l_BuildingLevel = 3;
                        break;
                    default:
                        break;
...
{
    new MS::Garrison::GDraenor::go_garrison_anvil;
    new MS::Garrison::GDraenor::go_garrison_deactivated_mage_portal;
    new MS::Garrison::GDraenor::go_garrison_cache;
    new MS::Garrison::GDraenor::go_garrison_outhouse;
    new MS::Garrison::GDraenor::go_garrison_shipment_container;
    new MS::Garrison::GDraenor::go_garrison_herb;
    new MS::Garrison::GDraenor::go_garrison_deposit;
    new MS::Garrison::GDraenor::gob_IronTrap_Garrison;
    new MS::Garrison::GDraenor::go_garrison_timber;
    new MS::Garrison::GDraenor::go_garrison_essence_font;
}
#endif
