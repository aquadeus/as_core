...
namespace MS { namespace Garrison { namespace GDraenor
{
    ...
    /// Constructor
    npc_CommandTable::npc_CommandTable()
        : CreatureScript("npc_CommandTable")
    {

    }

    bool npc_CommandTable::OnGossipHello(Player* p_Player, Creature* p_Creature)
    {
        p_Player->GetSession()->SendGarrisonSetMissionNpc(p_Creature->GetGUID(), 1);
    ...
    {
        if (!p_Player->GetDraenorGarrison())
    ...
    {
        if (p_Player->getLevel() >= 88 && !p_Player->GetDraenorGarrison())
        {
            p_Player->PlayerTalkClass->SendCloseGossip();
            p_Player->CreateGarrison(GarrisonType::GarrisonDraenor);

            uint32 l_MovieID    = p_Player->GetDraenorGarrison()->GetGarrisonSiteLevelEntry()->MovieID;
            uint32 l_MapID      = p_Player->GetDraenorGarrison()->GetGarrisonSiteLevelEntry()->MapID;
    ...
    void npc_CallToArms::npc_CallToArmsAI::UpdateAI(const uint32 /*p_Diff*/)
    {
        ...
        if (m_Ranged)
        {
            if (me->IsWithinDist(me->getVictim(), 1.0f))
            {
    ...
    bool npc_garrison_atheeru_palestar::OnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 /*p_Sender*/, uint32 p_Action)
    {
        ...
        {
            p_Player->ModifyMoney(-10000, "npc_garrison_atheeru_palestar");
    ...
    bool npc_FleetCommandTable::OnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 /*p_Sender*/, uint32 /*p_Action*/)
    {
        ...
        p_Player->GetSession()->SendGarrisonSetMissionNpc(p_Creature->GetGUID(), eGarrisonFollowerTypeID::FollowerTypeIDShipyard);
        return true;
    }
    ...
    {
        if (p_Player->GetDraenorGarrison() == nullptr)
            return true;

        std::vector<uint64> l_CreatureGuids = p_Player->GetDraenorGarrison()->GetBuildingCreaturesByBuildingType(GDraenor::Building::Type::TradingPost);
            ...
            if (Creature* l_Creature = sObjectAccessor->GetCreature(*p_Player, *l_Itr))
            {
                if (l_Creature->ToGarrisonNPCAI())
                {
                    ...
                    if (l_GarrisonAI != nullptr && l_GarrisonAI->GetOwner() != nullptr)
                        l_GarrisonAI->GetOwner()->GetDraenorGarrison()->UpdatePlot(l_GarrisonAI->GetPlotInstanceID());
                }
            }
        }
    ...
    {
        auto l_GarrisonMgr = p_Player->GetDraenorGarrison();
        ...
        {
            GDraenor::GarrisonBuilding l_Building = l_GarrisonMgr->GetBuilding(l_BuildingPlotInstanceID);

            switch (l_Building.BuildingID)
            {
                /// Gem Boutique has custom handling
                case GDraenor::Building::ID::TailoringEmporium_TailoringEmporium_Level2:
                case GDraenor::Building::ID::TailoringEmporium_TailoringEmporium_Level3:
                {
                ...
                case GDraenor::Building::ID::TheTannery_TheTannery_Level2:
                case GDraenor::Building::ID::TheTannery_TheTannery_Level3:
                {
                    std::vector<RecipesConditions> l_Recipes;
                ...
                case GDraenor::Building::ID::TheForge_TheForge_Level2:
                case GDraenor::Building::ID::TheForge_TheForge_Level3:
                    ...
                    break;
                case GDraenor::Building::ID::AlchemyLab_AlchemyLab_Level2:
                case GDraenor::Building::ID::AlchemyLab_AlchemyLab_Level3:
                    ...
                    break;
                case GDraenor::Building::ID::EnchanterStudy_EnchanterStudy_Level2:
                case GDraenor::Building::ID::EnchanterStudy_EnchanterStudy_Level3:
                {
                    std::vector<RecipesConditions> l_Recipes =
                    {
                        { 173718, 0 },
...
///                        { 62948, 0 }, ///< not sure about this one
                ...
                case GDraenor::Building::ID::ScribesQuarters_ScribesQuarters_Level2:
                case GDraenor::Building::ID::ScribesQuarters_ScribesQuarters_Level3:
                {
                    std::vector<RecipesConditions> l_Recipes =
                    {
                        { 176513, 0 }
                    ...
                    if (Quest const* l_Quest = sObjectMgr->GetQuestTemplate(37270))
                    {
                        if (l_Player->CanTakeQuest(l_Quest, false) || l_Player->CanRewardQuest(l_Quest, false))
                        {
                            if (l_Player->GetQuestStatus(37270) == QUEST_STATUS_NONE)
                                l_Player->PlayerTalkClass->SendQuestGiverStatus(QuestGiverStatus::DIALOG_STATUS_AVAILABLE_REP, me->GetGUID());
                            else if (l_Player->GetQuestStatus(37270) == QUEST_STATUS_COMPLETE)
                                l_Player->PlayerTalkClass->SendQuestGiverStatus(QuestGiverStatus::DIALOG_STATUS_REWARD_REP, me->GetGUID());
                            else
                                l_Player->PlayerTalkClass->SendQuestGiverStatus(QuestGiverStatus::DIALOG_STATUS_NONE, me->GetGUID());
                        }
                    }
                }
    ...
    void npc_StablesTrainingMounts_Garr::npc_StablesTrainingMounts_GarrAI::PassengerBoarded(Unit* p_Passenger, int8 /*p_SeatID*/, bool p_Apply)
    {
        if (Player* l_Player = HashMapHolder<Player>::Find(m_SummonerGUID))
        {
            if (auto l_GarrisonMgr = l_Player->GetDraenorGarrison())
            {
                if (p_Apply)
                {
                    switch (l_GarrisonMgr->GetBuildingLevel(l_GarrisonMgr->GetBuildingWithType(GDraenor::Building::Type::Stable)))
                    {
                        case 2:
                        {
...
namespace GBrokenIsles {
    ...
    {
        BroadcastTextSoBF = 119001,
    };

    static void SendShipmentUI(Player* p_Player, Creature* p_Creature, uint32 p_ItemId = 0)
    {
        uint32 WorkOrderId = 0;
        if (p_Creature->GetEntry() == MS::Garrison::GBrokenIsles::eWorkOrderNPC::NPCNomi)
        {
            auto const& l_WorkOrderInfos = GBrokenIsles::s_WorkOrderInfos[0];
            auto l_Itr = l_WorkOrderInfos.find(p_ItemId);
            if (l_Itr != l_WorkOrderInfos.end())
                WorkOrderId = l_Itr->second.WorkOrderID;
        }
        else
        {
            auto const& l_WorkOrderInfos = GBrokenIsles::s_WorkOrderInfos[p_Player->getClass()];
            auto l_Itr = l_WorkOrderInfos.find(p_Creature->GetEntry());
            ...
            if (l_Itr != l_WorkOrderInfos.end())
            {
                ...
                WorkOrderId = l_Itr->second.WorkOrderID;
                while (l_TempItr != l_WorkOrderInfos.end() && !(*l_TempItr).second.RequiredGarrTalent && (*l_TempItr).first == p_Creature->GetEntry())
                    ++l_TempItr;

                if (l_TempItr != l_WorkOrderInfos.end() && l_Garrison->HasTalent((*l_TempItr).second.RequiredGarrTalent))
                    WorkOrderId = l_TempItr->second.WorkOrderID;
            }
        }
        ...
        for (auto i : MS::Garrison::GBrokenIsles::m_questStartShipments)
            if (p_Player->GetQuestStatus(i.first) && p_Player->GetQuestStatus(i.first) != QUEST_STATUS_REWARDED)
                WorkOrderId = i.second;

        WorldPacket l_Data(SMSG_OPEN_SHIPMENT_NPC_FROM_GOSSIP);
        l_Data.appendPackGUID(p_Creature->GetGUID());
        l_Data << uint32(WorkOrderId);

        p_Player->SendDirectMessage(&l_Data);
    }

    static bool HandleOnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 p_Sender, uint32 p_Action)
    {
        if (p_Action == GOSSIP_ACTION_INFO_DEF + p_Player->m_SpellHelper.GetUint32(eSpellHelpers::NPCWorkorderGossip))
        {
            SendShipmentUI(p_Player, p_Creature);
            return true;
        }
        else
            return false;
    ...
    static bool AddWorkorderMenuItem(Player* p_Player, Creature* p_Creature, uint32 p_BroadcastTextId)
    {
        if (p_Player->PlayerTalkClass->GetGossipMenu().GetMenuItemCount() == 0
            && p_Player->PlayerTalkClass->GetQuestMenu().GetMenuItemCount() == 0)
        {
            SendShipmentUI(p_Player, p_Creature);
            return true;
        }

        p_Player->m_SpellHelper.GetUint32(eSpellHelpers::NPCWorkorderGossip) = p_Player->PlayerTalkClass->GetGossipMenu().GetMenuItemCount();
        p_Player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GOSSIP_ICON_WORKORDER, p_BroadcastTextId, p_Player->GetSession()->GetSessionDbLocaleIndex(), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + p_Player->m_SpellHelper.GetUint32(eSpellHelpers::NPCWorkorderGossip), "", 0);
        return false;
    }

    class npc_class_hall_workorder_troop : public CreatureScript
    {
        public:
            npc_class_hall_workorder_troop() : CreatureScript("npc_class_hall_workorder_troop") { }

            bool OnGossipHello(Player* p_Player, Creature* p_Creature) override
            {
                p_Player->PrepareGossipMenu(p_Creature, 0, true);

                if (AddWorkorderMenuItem(p_Player, p_Creature, 0))
                    return true;
            ...
            {
                return HandleOnGossipSelect(p_Player, p_Creature, p_Sender, p_Action);
            }
    };

    class npc_class_hall_workorder_sobf : public CreatureScript
    {
        public:
            npc_class_hall_workorder_sobf() : CreatureScript("npc_class_hall_workorder_sobf") { }

            bool OnGossipHello(Player* p_Player, Creature* p_Creature) override
            {
                p_Player->PrepareGossipMenu(p_Creature, 0, true);

                if ((p_Player->GetCharacterWorldStateValue(CharacterWorldStates::GarrisonWeeklySealOfBrokenFateWorkorder) < 1)
                    && (p_Player->GetCharacterWorldStateValue(CharacterWorldStates::BrokenIslesWeeklyBonusRollTokenCount) < 3)
                    && (p_Player->GetCurrency(CurrencyTypes::CURRENCY_TYPE_SEAL_OF_BROKEN_FATE, true) < 6))
                {
                    if (AddWorkorderMenuItem(p_Player, p_Creature, eBroadcastTextID::BroadcastTextSoBF))
                        return true;
                }
    ...
    class npc_class_hall_workorder_nuke : public CreatureScript
    {
        public:
            npc_class_hall_workorder_nuke() : CreatureScript("npc_class_hall_workorder_nuke") { }
    ...
    class npc_class_hall_workorder_armament : public CreatureScript
    {
        public:
            npc_class_hall_workorder_armament() : CreatureScript("npc_class_hall_workorder_armament") { }
    ...
    class npc_nomi : public CreatureScript
    {
        public:
            npc_nomi() : CreatureScript("npc_nomi") { }

            bool OnGossipHello(Player* p_Player, Creature* p_Creature) override
            {
                p_Player->m_SpellHelper.GetUint64Map()[eSpellHelpers::CookingWorkorders].clear();

                p_Player->PrepareGossipMenu(p_Creature, 0, true);

                auto check = [](Player* player, uint32 c_item)
                {
                    if (c_item == eWorkorderComponenent::SilverMackerelQuest)
                    {
                        if (player->GetQuestStatus(40991) == QUEST_STATUS_INCOMPLETE && player->HasItemCount(c_item, 5, true, true))
                            return true;
                    }
                    else
                    {
                        if (player->IsQuestRewarded(40991))
                            return true;
                    }
                ...
                for (std::pair<uint32, uint32> const& l_ComponentInfo : s_CookingWorkorderComponenents)
                {
                    if (check(p_Player, l_ComponentInfo.first))
                    {
                        p_Player->m_SpellHelper.GetUint64Map()[eSpellHelpers::CookingWorkorders][p_Player->PlayerTalkClass->GetGossipMenu().GetMenuItemCount()] = l_ComponentInfo.first;
                        p_Player->m_SpellHelper.GetUint32(eSpellHelpers::NPCWorkorderGossip) = p_Player->PlayerTalkClass->GetGossipMenu().GetMenuItemCount();
                        p_Player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GOSSIP_ICON_WORKORDER, l_ComponentInfo.second, p_Player->GetSession()->GetSessionDbLocaleIndex(), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + p_Player->PlayerTalkClass->GetGossipMenu().GetMenuItemCount(), "", 0);
                    }
                }
            ...
            bool OnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 p_Sender, uint32 p_Action) override
            {
                std::map<uint64, uint64> const& l_CookingWorkorders = p_Player->m_SpellHelper.GetUint64Map()[eSpellHelpers::CookingWorkorders];
                if (l_CookingWorkorders.empty())
                    return false;

                uint32 l_LastWorkorderOption = GOSSIP_ACTION_INFO_DEF + p_Player->m_SpellHelper.GetUint32(eSpellHelpers::NPCWorkorderGossip);
                uint32 l_FirstWororderOption = l_LastWorkorderOption - l_CookingWorkorders.size() + 1;
                if (p_Action >= l_FirstWororderOption && p_Action <= l_LastWorkorderOption)
                {
                    std::map<uint64, uint64>::const_iterator l_Itr = l_CookingWorkorders.find(p_Action - GOSSIP_ACTION_INFO_DEF);
                    if (l_Itr == l_CookingWorkorders.end())
                        return false;

                    p_Player->m_SpellHelper.GetUint32(eSpellHelpers::CookingWorkorderItem) = l_Itr->second;

                    SendShipmentUI(p_Player, p_Creature, static_cast<uint32>(l_Itr->second));
                    return true;
                }
                else
                    return false;
    ...
    class PlayerScript_save_nomi_recipes : public PlayerScript
    {
        public:
            PlayerScript_save_nomi_recipes() : PlayerScript("PlayerScript_save_nomi_recipes") { }

            void OnItemLooted(Player* p_Player, Item* p_Item)
            {
                uint32 l_Entry = p_Item->GetEntry();
                if (s_CookingWorkorderRecipes.find(l_Entry) == s_CookingWorkorderRecipes.end())
                    return;

                p_Player->m_SpellHelper.GetUint64Set()[eSpellHelpers::NomiRecipes].insert(l_Entry);
            }
    };
...
{
    /// Class Hall
    new MS::Garrison::GBrokenIsles::npc_class_hall_workorder_troop;
    new MS::Garrison::GBrokenIsles::npc_class_hall_workorder_sobf;
    new MS::Garrison::GBrokenIsles::npc_class_hall_workorder_nuke;
    new MS::Garrison::GBrokenIsles::npc_class_hall_workorder_armament;

    /// Cooking
    new MS::Garrison::GBrokenIsles::npc_nomi;
    new MS::Garrison::GBrokenIsles::PlayerScript_save_nomi_recipes();

    /// Generic
    new MS::Garrison::GDraenor::npc_CommandTable;
    new MS::Garrison::GDraenor::npc_GarrisonFord;
    new MS::Garrison::GDraenor::npc_CallToArms;
    new MS::Garrison::GDraenor::npc_garrison_amperial_construct;
    new MS::Garrison::GDraenor::npc_garrison_atheeru_palestar;
    new MS::Garrison::GDraenor::npc_GarrisonStablesCreatures;
    new MS::Garrison::GDraenor::npc_follower_generic_script;
    new MS::Garrison::GDraenor::npc_StablesTrainingMounts_Garr;
    new MS::Garrison::GDraenor::npc_LeatherWorkingTent_Garr;
    new MS::Garrison::GDraenor::npc_InspiringBattleStandard;
    new MS::Garrison::GDraenor::npc_FearsomeBattleStandard;
    new MS::Garrison::GDraenor::npc_GarrisonWalter;
    new MS::Garrison::GDraenor::npc_GearshopWorkshopTurret_Garr;

    /// Alliance
    {
        new MS::Garrison::GDraenor::npc_GarrisonCartRope;
        new MS::Garrison::GDraenor::npc_AssistantBrightstone;
        new MS::Garrison::GDraenor::npc_ShellyHamby;
        new MS::Garrison::GDraenor::npc_BarosAlexsom;
        new MS::Garrison::GDraenor::npc_VindicatorMaraad;
        new MS::Garrison::GDraenor::npc_LunarfallLaborer;
        new MS::Garrison::GDraenor::npc_AncientTradingMechanism_Garr;

        /// Barracks
        new MS::Garrison::GDraenor::npc_JonathanStephens;

        /// Dwarven Bunker
        new MS::Garrison::GDraenor::npc_GussofForgefire;
        new MS::Garrison::GDraenor::npc_KristenStoneforge;

        /// The forge
        new MS::Garrison::GDraenor::npc_AuriaIrondreamer;
        new MS::Garrison::GDraenor::npc_YuliaSamras;
        new MS::Garrison::GDraenor::npc_DalanaClarke_Garr;

        /// Lunarfall excavation
        new MS::Garrison::GDraenor::npc_TimothyLeens;

        /// Fishing hut
        new MS::Garrison::GDraenor::npc_TharisStrongcast;
        new MS::Garrison::GDraenor::npc_Segumi;
        new MS::Garrison::GDraenor::npc_RonAshton;

        /// Trading post
        new MS::Garrison::GDraenor::npc_TraderJoseph;

        /// TailoringEmporium
        new MS::Garrison::GDraenor::npc_ChristopherMacdonald;
        new MS::Garrison::GDraenor::npc_KaylieMacdonald;

        /// Alchemy lab
        new MS::Garrison::GDraenor::npc_MaryKearie;
        new MS::Garrison::GDraenor::npc_PeterKearie;

        /// The tannery
        new MS::Garrison::GDraenor::npc_AndersLongstitch;
        new MS::Garrison::GDraenor::npc_MarianneLevine;

        /// Enchanter's Study
        new MS::Garrison::GDraenor::npc_AyadaTheWhite;
        new MS::Garrison::GDraenor::npc_Garm;

        /// Gem boutique
        new MS::Garrison::GDraenor::npc_CostanHighwall;
        new MS::Garrison::GDraenor::npc_KayaSolasen;

        /// Engineering works
        new MS::Garrison::GDraenor::npc_Zaren;
        new MS::Garrison::GDraenor::npc_HelaynWhent;

        /// Scribe quarters
        new MS::Garrison::GDraenor::npc_EricBroadoak;
        new MS::Garrison::GDraenor::npc_KurtBroadoak;

        /// Herb garden
        new MS::Garrison::GDraenor::npc_OllyNimkip;
        new MS::Garrison::GDraenor::npc_NaronBloomthistle;

        /// Barn
        new MS::Garrison::GDraenor::npc_HomerStonefield_Garr;
        new MS::Garrison::GDraenor::npc_HomerStonefield_Garr_Trap;

        /// Lumber Mill
        new MS::Garrison::GDraenor::npc_JustinTimberLord;

        /// Lunarfall Inn
        new MS::Garrison::GDraenor::npc_MadisonClark;
        new MS::Garrison::GDraenor::npc_lysa_serion_garr;

        /// Mage Tower
        new MS::Garrison::GDraenor::npc_ApprenticeVarNath;
        new MS::Garrison::GDraenor::npc_AncientWaygateProtector;

        /// Stables
        new MS::Garrison::GDraenor::npc_FannyFirebeard;
        new MS::Garrison::GDraenor::npc_KeeganFirebeard;

        /// Gladiator's Sanctum
        new MS::Garrison::GDraenor::npc_AltarOfBones;
        new MS::Garrison::GDraenor::npc_Kuros_Garr;

        /// Gnomish Gearworks
        new MS::Garrison::GDraenor::npc_Zee_Garrison;
    }

    /// Horde
    {
        new MS::Garrison::GDraenor::npc_FrostwallPeon("npc_FrostwallPeon_Dynamic");
        new MS::Garrison::GDraenor::npc_FrostwallPeon("npc_FrostwallPeon");
        new MS::Garrison::GDraenor::npc_Skaggit;
        new MS::Garrison::GDraenor::npc_LadySena;
        new MS::Garrison::GDraenor::npc_SergeantGrimjaw;
        new MS::Garrison::GDraenor::npc_SeniorPeonII;
        new MS::Garrison::GDraenor::npc_Gazlowe;

        /// War Mill
        new MS::Garrison::GDraenor::npc_GrunLek;
        new MS::Garrison::GDraenor::npc_FrostWallGrunt;
        new MS::Garrison::GDraenor::npc_FrostWallSmith;
        new MS::Garrison::GDraenor::npc_Magrish_Garr;

        /// The forge
        new MS::Garrison::GDraenor::npc_OrgekIronhand;
        new MS::Garrison::GDraenor::npc_Kinja;

        /// Trading post
        new MS::Garrison::GDraenor::npc_FaylaFairfeather;

        /// Tailoring Emporium
        new MS::Garrison::GDraenor::npc_WarraTheWeaver;
        new MS::Garrison::GDraenor::npc_Turga;

        /// Alchemy lab
        new MS::Garrison::GDraenor::npc_AlbertDeHyde;
        new MS::Garrison::GDraenor::npc_KeyanaTone;

        /// The tannery
        new MS::Garrison::GDraenor::npc_MurneGreenhoof;
        new MS::Garrison::GDraenor::npc_Yanney;

        /// Enchanter's Study
        new MS::Garrison::GDraenor::npc_YuklaGreenshadow;
        new MS::Garrison::GDraenor::npc_Garra;

        /// Gem boutique
        new MS::Garrison::GDraenor::npc_Dorogarr;
        new MS::Garrison::GDraenor::npc_ElrondirSurrion;

        /// Engineering works
        new MS::Garrison::GDraenor::npc_Pozzlow;
        new MS::Garrison::GDraenor::npc_GarbraFizzwonk;

        /// Scribe quarters
        new MS::Garrison::GDraenor::npc_Urgra;
        new MS::Garrison::GDraenor::npc_Yrogg;

        /// Herb garden
        new MS::Garrison::GDraenor::npc_Tarnon;
        new MS::Garrison::GDraenor::npc_NaliSoftOil;

        /// Barn
        new MS::Garrison::GDraenor::npc_FarmerLokLub;
        new MS::Garrison::GDraenor::npc_IronTrap;
        new MS::Garrison::GDraenor::npc_FarmerLokLub_Trap;

        /// Lumber Mill
        new MS::Garrison::GDraenor::npc_LumberLordOktron;

        /// Frostwall Mines
        new MS::Garrison::GDraenor::npc_Gorsol;

        /// Frostwall Tavern
        new MS::Garrison::GDraenor::npc_Murg;
        new MS::Garrison::GDraenor::npc_akanja_garr;

        /// Spirit Lodge
        new MS::Garrison::GDraenor::npc_Varsha;

        /// Stables
        new MS::Garrison::GDraenor::npc_Tormak;
        new MS::Garrison::GDraenor::npc_SagePaluna;

        /// Fishing Hut
        new MS::Garrison::GDraenor::npc_MakJin_Garr;

        /// Goblin Workshop
        new MS::Garrison::GDraenor::npc_Vee_Garrison;
    }

    /// General
    {
        /// Shipyard
        new MS::Garrison::GDraenor::npc_FleetCommandTable;
    }
}
#endif
