...
namespace MS { namespace Garrison { namespace GDraenor
{
    ...
    void spell_garrison_hearthstone::spell_garrison_hearthstone_SpellScript::HandleDummy(SpellEffIndex /*p_EffectIndex*/)
    {
        ...
        if (l_Player && l_Player->GetDraenorGarrison() && l_Player->GetDraenorGarrison()->GetGarrisonSiteLevelEntry())
        {
            auto* l_Garrison = l_Player->GetDraenorGarrison();

            uint32 l_Index = (l_Player->GetGarrison()->GetGarrisonFactionIndex() * MS::Garrison::GDraenor::Globals::MaxLevel) + (l_Garrison->GetGarrisonSiteLevelEntry()->Level - 1);
        ...
        public:
            ...
            class spell_garrison_stables_lasso_AuraScript : public AuraScript
            {
                ...
                void Register() override
                {
                    ...
                    AfterEffectRemove += AuraEffectRemoveFn(spell_garrison_stables_lasso_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
                }
            };
    ...
    class spell_garrison_well_rested : public SpellScriptLoader
    {
        public:
            ...
            class spell_garrison_well_rested_AuraScript : public AuraScript
            {
                ...
                void AfterAuraApply(AuraEffect const* /*p_AurEff*/, AuraEffectHandleModes /*p_Mode*/)
                {
                    Unit* l_Owner = GetUnitOwner();
                    if (l_Owner == nullptr)
                        return;

                    l_Owner->RemoveAura(eSpell::SpellWellRestedTrackingAura);
                }

                void Register() override
                {
                    AfterEffectApply += AuraEffectApplyFn(spell_garrison_well_rested_AuraScript::AfterAuraApply, EFFECT_0, SPELL_AURA_MOD_PERCENT_STAT, AURA_EFFECT_HANDLE_REAL);
                }
            };
        ...
        public:
            ...
            class spell_garrison_tent_spawn_SpellScript : public SpellScript
            {
                ...
                SpellCastResult CheckCast()
                {
                    ...
                    if (l_Caster->GetMapId() != 1116 && (l_Caster->GetTypeId() == TYPEID_PLAYER && !l_Caster->ToPlayer()->IsInDraenorGarrison()))
...
void AddSC_Garrison()
{
    new MS::Garrison::GDraenor::spell_garrison_rush_order();
    new MS::Garrison::GDraenor::spell_garrison_combine_scribe_items();
    new MS::Garrison::GDraenor::spell_aura_garrison_skyterror_falling();
    new MS::Garrison::GDraenor::spell_garrison_stables_lasso();
    new MS::Garrison::GDraenor::spell_garrison_hearthstone();
    new MS::Garrison::GDraenor::spell_garrison_portal();
    new MS::Garrison::GDraenor::spell_garrison_shipyard();
    new MS::Garrison::GDraenor::spell_aura_sticky_grenade();
    new MS::Garrison::GDraenor::spell_pneumatic_power_gauntlet();
    new MS::Garrison::GDraenor::spell_GarrisonRouseTrader();
    new MS::Garrison::GDraenor::spell_garrison_well_rested();
    new MS::Garrison::GDraenor::spell_garrison_tent_spawn();
}
#endif
