...
namespace MS { namespace Garrison { namespace GDraenor
{
        ...
        : CreatureAI(p_Creature)
    {
        ...
        if (m_IsDynamicScript && me->GetInstanceScript()->GetData(MS::Garrison::GDraenor::InstanceDataIDs::GARRISON_INSTANCE_DATA_PEON_ENABLED))
            SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_WOODCUTTING);
    }
                    ...
                    else if (p_ID == HordePeonData::MOVE_PATH_WOOD_CUTTING_A_END)
                    {
                        if (!m_BackToSpawnPos)
                        {
                            m_DelayedOperations.push([this]() -> void
                            {
                                ...
                                SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_WOODCUTTING_DISENGAGE);
    ...
    {
        if (m_IsDynamicScript && me->HasAura(MS::Garrison::GDraenor::Spells::SPELL_COMESTIC_SLEEP))
        {
            ...
            switch (m_Phase)
            {
                ...
                case HordePeonData::PHASE_WOODCUTTING_DISENGAGE:
                    ...
                    if (m_WoodCuttingRemainingTime < 0 && m_WoodCuttingCycleCount < HordePeonData::WOODCUTTING_MAX_CYLE_COUNT)
                        SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_WOODCUTTING_REENGAGE);
                    else if (m_WoodCuttingRemainingTime < 0)
                        SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_WOODCUTTING_DEPOSIT);
    ...
    {
        if (p_ID == GDraenor::CreatureAIDataIDs::Builder)
        {
            me->LoadEquipment(1, true);
            me->SetUInt32Value(UNIT_FIELD_EMOTE_STATE, EMOTE_STATE_WORK);
        }
        ...
        {
            if (p_ID == GDraenor::CreatureAIDataIDs::PeonWorking)
            {
    ...
    uint32 npc_FrostwallPeon::npc_FrostwallPeonAI::SelectRandomTree()
    {
        ...
        for (uint32 l_I = 0; l_I < 7; l_I++)
        {
            ...
            if (me->GetInstanceScript()->GetData(MS::Garrison::GDraenor::InstanceDataIDs::GARRISON_INSTANCE_DATA_THREE_FIRST + l_I))
                continue;
