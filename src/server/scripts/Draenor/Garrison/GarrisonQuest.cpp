...
namespace MS { namespace Garrison { namespace GDraenor
{
    ...
    void GarrisonQuestPlayerScript::OnItemLooted(Player* p_Player, Item* p_Item)
    {
        ...
        switch (p_Item->GetVisibleEntry(p_Player))
        {
...
//                p_Item->SetSpellCharges(0, p_Player->GetCharacterWorldStateValue(CharacterWorldStates::GarrisonWorkshopGearworksInventionCharges));
            ...
            case 119126:
            {
                ...
                p_Player->AddCriticalOperation([l_PlayerGuid]() -> void
                {
                    if (Player* l_Player = sObjectAccessor->FindPlayer(l_PlayerGuid))
    ...
    void playerScript_Garrison_Portals_Phases::OnLogin(Player* p_Player)
    {
        ...
        if (auto l_GarrisonMgr = p_Player->GetDraenorGarrison())
        {
            if (l_GarrisonMgr->GetBuildingWithType(GDraenor::Building::Type::TradingPost).BuildingID)
            {
...
{
    new MS::Garrison::GDraenor::GarrisonBuildingAuraPlayerScript;
    new MS::Garrison::GDraenor::GarrisonQuestPlayerScript;
    new MS::Garrison::GDraenor::playerScript_Garrison_TradingPost;
    new MS::Garrison::GDraenor::playerScript_Garrison_Portals_Phases;
    new MS::Garrison::GDraenor::playerScript_Garrison_Quests_Phases;
    new MS::Garrison::GDraenor::spell_learning_blueprint;
}
#endif
