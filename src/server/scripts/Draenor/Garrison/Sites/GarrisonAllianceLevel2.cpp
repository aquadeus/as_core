...
namespace MS { namespace Garrison { namespace GDraenor { namespace Sites
{
    ...
    void InstanceScript_GarrisonAllianceLevel2::OnOwnerEnter(Player* p_Owner)
    {
        ...
        if (p_Owner->GetTeamId() == TEAM_ALLIANCE && !p_Owner->HasAchieved(9100))
            p_Owner->CompletedAchievement(sAchievementStore.LookupEntry(9100));
    ...
    /// @p_Owner : Garrison owner
    bool InstanceScript_GarrisonAllianceLevel2::CanUseDraenorGarrisonCache(Player* /*p_Owner*/)
    {
        return true;
    }
    ...
    bool InstanceScript_GarrisonAllianceLevel2::CanUpgrade(Player* player, uint32 p_CurrentGarrisonLevel)
    {
        if (p_CurrentGarrisonLevel != 2)
            return false;

        if (!player->HasQuest(36615) || player->getLevel() < 100)
            return false;
                ...
                switch (p_BuildingID)
                {
                    ...
                    case GDraenor::Building::ID::Barn_Barn_Level3:
                        ...
                        if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemImprovedIronTrap))
                            p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
                        break;
                    case GDraenor::Building::ID::TradingPost_TradingPost_Level2:
                    {
...
{
    new MS::Garrison::GDraenor::Sites::InstanceMapScript_GarrisonAllianceLevel2();
}
#endif
