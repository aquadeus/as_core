...
namespace MS { namespace Garrison { namespace GDraenor { namespace Sites
{
    ...
    void InstanceScript_GarrisonHordeLevel1::OnOwnerEnter(Player* p_Owner)
    {
            ...
            if (!m_CreaturesPerEntry[NPCs::NPC_FROSTWALL_PEON_DYNAMIC].empty())
            {
                for (uint64 l_DynamicPeon : m_CreaturesPerEntry[NPCs::NPC_FROSTWALL_PEON_DYNAMIC])
                {
                    ...
                    if (l_Creature && l_Creature->AI())
                        l_Creature->AI()->SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_WOODCUTTING);
                }
            }
        }
        ...
        if (p_Owner->HasQuest(Quests::Horde_BuildYourBarracks))
        {
            ...
            if (p_Owner->GetDraenorGarrison() && p_Owner->GetDraenorGarrison()->GetBuildingWithType(GDraenor::Building::Type::Barracks).BuildingID)
            {
        ...
        if (p_Quest && p_Quest->GetQuestId() == Quests::QUEST_WHAT_WE_GOT)
        {
            ...
            if (!m_CreaturesPerEntry[NPCs::NPC_FROSTWALL_PEON_DYNAMIC].empty())
            {
                for (uint64 l_DynamicPeon : m_CreaturesPerEntry[NPCs::NPC_FROSTWALL_PEON_DYNAMIC])
                {
                    ...
                    if (l_Creature && l_Creature->AI())
                        l_Creature->AI()->SetData(GDraenor::CreatureAIDataIDs::PeonWorking, HordePeonData::PHASE_BACK_TO_HOME);
                }
            }
        }
    }
    ...
    uint32 InstanceScript_GarrisonHordeLevel1::GetPhaseMask(Player* p_Owner)
    {
        ...
        auto l_GarrisonMgr = p_Owner->GetDraenorGarrison();
        ...
        if (l_GarrisonMgr->HasBuildingType(GDraenor::Building::Type::MageTower))
        {
    ...
    /// @p_Owner : Garrison owner
    bool InstanceScript_GarrisonHordeLevel1::CanUseDraenorGarrisonCache(Player* p_Owner)
    {
        /// Horde
    ...
    {
        GarrSiteLevelEntry const* l_Entry = p_Owner->GetDraenorGarrison()->GetGarrisonSiteLevelEntry();
    ...
    {
        /// Build your Barracks quest
        if (p_BuildingID == GDraenor::Building::ID::Barracks_Barracks_Level1 && p_Owner->HasQuest(Quests::Horde_BuildYourBarracks))
    ...
    void InstanceScript_GarrisonHordeLevel1::OnPurchaseBuilding(Player* p_Owner, uint32 p_BuildingID)
    {
        ...
        else if (p_BuildingID == GDraenor::Building::ID::TradingPost_TradingPost_Level2)
        {
    ...
    void InstanceScript_GarrisonHordeLevel1::OnBuildingActivated(Player* p_Owner, uint32 p_BuildingID)
    {
        ...
        if (auto l_GarrisonMgr = p_Owner->GetDraenorGarrison())
        {
            uint8 l_BuildingLevel = l_GarrisonMgr->GetBuildingLevel(l_GarrisonMgr->GetBuilding(p_BuildingID));

            switch (p_BuildingID)
            {
                case GDraenor::Building::ID::LumberMill_LumberMill_Level1:
                case GDraenor::Building::ID::LumberMill_LumberMill_Level2:
                case GDraenor::Building::ID::LumberMill_LumberMill_Level3:
                    p_Owner->SetSkill(SkillType::SKILL_LOGGING, l_BuildingLevel, l_BuildingLevel, 75);
                    break;
                case GDraenor::Building::ID::Barn_Barn_Level2:
                    if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemIronTrap))
                        p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
                    break;
                case GDraenor::Building::ID::Barn_Barn_Level3:
                    if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemIronTrap))
                        p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
    ...
    void InstanceScript_GarrisonHordeLevel1::OnDeleteBuilding(Player* p_Owner, uint32 /*p_BuildingID*/, uint32 p_BuildingType, bool p_RemoveForUpgrade)
    {
        ...
        {
            case GDraenor::Building::Type::LumberMill:
                p_Owner->SetSkill(SkillType::SKILL_LOGGING, 0, 0, 0);
                break;
            case GDraenor::Building::Type::Barn:
                if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemIronTrap))
                    p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
                if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemImprovedIronTrap))
                    p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
...
{
    new MS::Garrison::GDraenor::Sites::InstanceMapScript_GarrisonHordeLevel1();
}
#endif
