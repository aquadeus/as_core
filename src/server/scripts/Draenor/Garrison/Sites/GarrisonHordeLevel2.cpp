...
namespace MS { namespace Garrison { namespace GDraenor { namespace Sites
{
    ...
    void InstanceScript_GarrisonHordeLevel2::OnOwnerEnter(Player* p_Owner)
    {
        ...
        if (p_Owner->GetTeamId() == TEAM_HORDE && !p_Owner->HasAchieved(9545))
            p_Owner->CompletedAchievement(sAchievementStore.LookupEntry(9545));
    ...
    /// @p_Owner : Garrison owner
    bool InstanceScript_GarrisonHordeLevel2::CanUseDraenorGarrisonCache(Player* /*p_Owner*/)
    {
        return true;
    }
    ...
    bool InstanceScript_GarrisonHordeLevel2::CanUpgrade(Player* player, uint32 p_CurrentGarrisonLevel)
    {
        if (p_CurrentGarrisonLevel != 2)
            return false;

        if (!player->HasQuest(36614) || player->getLevel() < 100)
            return false;
    ...
    {
        if (p_BuildingID == GDraenor::Building::ID::TradingPost_TradingPost_Level2)
        {
...
{
    new MS::Garrison::GDraenor::Sites::InstanceMapScript_GarrisonHordeLevel2();
}
#endif
