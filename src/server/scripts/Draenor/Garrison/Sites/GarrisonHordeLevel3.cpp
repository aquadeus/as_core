...
namespace MS { namespace Garrison { namespace GDraenor { namespace Sites
{
    ...
    void InstanceScript_GarrisonHordeLevel3::OnOwnerEnter(Player* p_Owner)
    {
        ...
        if (p_Owner->GetTeamId() == TEAM_HORDE && !p_Owner->HasAchieved(9546))
            p_Owner->CompletedAchievement(sAchievementStore.LookupEntry(9546));
    }
    ...
    uint32 InstanceScript_GarrisonHordeLevel3::GetPhaseMask(Player* p_Owner)
    {
        ...
        if (l_GarrisonMgr && l_GarrisonMgr->HasBuildingType(GDraenor::Building::Type::MageTower))
        {
    ...
    /// @p_Owner : Garrison owner
    bool InstanceScript_GarrisonHordeLevel3::CanUseDraenorGarrisonCache(Player* /*p_Owner*/)
    {
        return true;
    }
                ...
                switch (p_BuildingID)
                {
                    ...
                    case GDraenor::Building::ID::Barn_Barn_Level3:
                        ...
                        if (Item* l_Item = p_Owner->GetItemByEntry(Items::ItemImprovedIronTrap))
                            p_Owner->RemoveItem(l_Item->GetBagSlot(), l_Item->GetSlot(), true);
                        break;
                    case GDraenor::Building::ID::GemBoutique_GemBoutique_Level3:
                    {
                        for (uint32 l_I = 0; l_I < sGarrMissionStore.GetNumRows(); ++l_I)
                        {
                            ...
                            if (l_Entry->MissionType == GDraenor::Mission::Type::JewelCrafting)
...
{
    new MS::Garrison::GDraenor::Sites::InstanceMapScript_GarrisonHordeLevel3();
}
#endif
