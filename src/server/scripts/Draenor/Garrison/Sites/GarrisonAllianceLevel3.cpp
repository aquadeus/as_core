...
namespace MS { namespace Garrison { namespace GDraenor { namespace Sites
{
    ...
    void InstanceScript_GarrisonAllianceLevel3::OnOwnerEnter(Player* p_Owner)
    {
        ...
        if (p_Owner->GetTeamId() == TEAM_ALLIANCE && !p_Owner->HasAchieved(9101))
            p_Owner->CompletedAchievement(sAchievementStore.LookupEntry(9101));
    }
    ...
    /// @p_Owner : Garrison owner
    bool InstanceScript_GarrisonAllianceLevel3::CanUseDraenorGarrisonCache(Player* /*p_Owner*/)
    {
        return true;
    }
...
{
    new MS::Garrison::GDraenor::Sites::InstanceMapScript_GarrisonAllianceLevel3();
}
#endif
