...
{
    if (!sWorld->getBoolConfig(CONFIG_ENABLE_ASHRAN_PVP_ZONE))
        return false;

    RegisterZone(eAshranDatas::AshranMapID, eAshranDatas::AshranZoneID);

    if (Map* l_Map = sMapMgr->CreateBaseMap(eAshranDatas::AshranMapID, 0))
        sMapMgr->GetZoneMapsList(eAshranDatas::AshranMapID, 0)->push_back(l_Map);
...
{
    /// Prevent map switch if Ashran is disabled
    if (!sWorld->getBoolConfig(CONFIG_ENABLE_ASHRAN_PVP_ZONE))
        return;
...
void OutdoorPvPAshran::HandleKill(Player* p_Killer, Unit* p_Killed)
{
    ...
    if (l_SlayCount > 0)
    {
        ...
        /// SMSG_ITEM_BONUS_DEBUG dont exist anymore we need to rework the way to send informations
...
void OutdoorPvPAshran::FillCustomPvPLoots(Player* p_Looter, Loot& p_Loot, uint64 p_Container)
{
    ...
    p_Loot.Items.push_back(LootItem(l_StoreItem, ItemContext::None, &p_Loot, p_Looter));
...
void OutdoorPvPAshran::RewardHonorAndReputation(uint32 p_ArtifactCount, Player* p_Player)
{
    ...
    int32 l_HonorPoints = p_ArtifactCount * eArtifactsDatas::HonorConversionRate;
