...
bool OutdoorPvPSI::SetupOutdoorPvP()
{
    for (uint8 i = 0; i < OutdoorPvPSIBuffZonesNum; ++i)
        RegisterZone(1, OutdoorPvPSIBuffZones[i]);
    return true;
}
...
bool OutdoorPvPSI::HandleAreaTrigger(Player* player, uint32 trigger, bool p_Enter)
{
    switch (trigger)
    {
    case SI_AREATRIGGER_A:
        if (player->GetTeam() == ALLIANCE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_A >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetTrinityStringForDB2Locale(LANG_OPVP_SI_CAPTURE_A));
    ...
    case SI_AREATRIGGER_H:
        if (player->GetTeam() == HORDE && player->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            if (m_Gathered_H >= SI_MAX_RESOURCES)
            {
                ...
                sWorld->SendZoneText(OutdoorPvPSIBuffZones[0], sObjectMgr->GetTrinityStringForDB2Locale(LANG_OPVP_SI_CAPTURE_H));
