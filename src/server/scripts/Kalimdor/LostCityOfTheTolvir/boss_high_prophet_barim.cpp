...
enum eSpells
{
    ...
    SPELL_HEAVENS_FURY_DAMAGE                  = 81941,
    // Repentance
...
public:
    ...
    struct boss_high_prophet_barimAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            lSummons.DespawnAll();
            uiEventPhase = 0;
            m_SoulFragmentKilled = 0;
            Repentance = false;
            me->SetReactState(REACT_AGGRESSIVE);

            if (instance)
            {
                instance->SetBossState(DATA_BOSS_BARIM, NOT_STARTED);
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_BARIM, DONE);
                instance->SetData(DATA_HIGH_PROPHET_BARIM, DONE);

                if (Creature* blaze = Unit::GetCreature(*me, instance->GetData64(DATA_BLAZE)))
                    blaze->AI()->EnterEvadeMode();

                if (m_SoulFragmentKilled >= 3)
                    instance->DoCompleteAchievement(eAchievements::ACH_KILL_IT_WITH_FIRE);
            }
        }

        void SummonedCreatureDies(Creature* p_Summon, Unit* /*p_Killer*/) override
        {
            if (instance && p_Summon && p_Summon->GetEntry() == NPC_SOUL_FRAGMENT)
                ++m_SoulFragmentKilled;
        }
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_BARIM, IN_PROGRESS);
...
public:
    ...
    struct npc_blaze_of_the_heavensAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            GetCreatureListWithEntryInGrid(l_Creatures, me, 48907, 100.0f);
...
public:
    ...
    struct npc_soul_fragmentAI : public Scripted_NoMovementAI
    {
        ...
        void JustDied(Unit* p_Killer) override
        {
            ...
            if (instance)
                if (Creature* barim = Unit::GetCreature(*me, instance->GetData64(DATA_HIGH_PROPHET_BARIM)))
                    barim->AI()->SummonedCreatureDies(me, p_Killer);
        }
...
class npc_heavens_fury : public CreatureScript
{
public:
    npc_heavens_fury() : CreatureScript("npc_heavens_fury") { }

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_heavens_furyAI(creature);
    }

    struct npc_heavens_furyAI : public ScriptedAI
    {
        npc_heavens_furyAI(Creature* creature) : ScriptedAI(creature)
        {
            instance = creature->GetInstanceScript();
            me->SetReactState(REACT_PASSIVE);
        ...
        void Reset() override
        {
            me->CastSpell(me, SPELL_HEAVENS_FURY_VISUAL, false);
            me->CastSpell(me, SPELL_HEAVENS_FURY_DAMAGE, false);
        }
    };
};
...
public:
    ...
    class spell_soul_fragment_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_soul_fragment_SpellScript);
        ...
        void Register() override
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_soul_fragment_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_soul_fragment_SpellScript();
    }
};
