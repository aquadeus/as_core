...
public:
    ...
    struct boss_siamatAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_SIAMAT, NOT_STARTED);
        ...
        void EnterCombat(Unit* /*who*/) override
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_SIAMAT, IN_PROGRESS);
        ...
        void JustDied(Unit* killer) override
        {
            ...
            instance->SetBossState(DATA_BOSS_SIAMAT, DONE);
            std::list<Unit*> l_PartyMembers;
            killer->GetPartyMembers(l_PartyMembers);
            for (Unit* l_Member : l_PartyMembers)
                if (Aura* l_Aura = l_Member->GetAura(SPELL_LIGHTNING_CHARGE_AURA))
                    if (l_Aura->GetStackAmount() >= 3)
                        instance->DoCompleteAchievement(eAchievements::ACH_HEADED_SOUT);
        }
...
public:
    ...
    struct npc_servant_of_siamatAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* , uint32 &damage, SpellInfo const*  /*p_SpellInfo*/) override
        {
            if (!IsHeroic())
                return;
...
class spell_lightning_aura : public SpellScriptLoader
{
public:
    spell_lightning_aura() : SpellScriptLoader("spell_lightning_aura") { }

    class spell_lightning_aura_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_lightning_aura_SpellScript);
        ...
        void ApplyAura(SpellEffIndex effIndex)
        {
            ...
            if (Creature* l_creature = target->ToCreature())
                if (l_creature->GetEntry() == BOSS_SIAMAT)
                    l_creature->AddAura(SPELL_LIGHTNING_CHARGE_AURA, l_creature);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_lightning_aura_SpellScript::ApplyAura, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
        }
    };

    SpellScript *GetSpellScript() const override
    {
        return new spell_lightning_aura_SpellScript();
    }
};
