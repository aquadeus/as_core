...
public:
    ...
    struct boss_lockmawAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_LOCKMAW_AUGH, NOT_STARTED);
        ...
        void EnterCombat(Unit* /*pWho*/) override
        {
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_LOCKMAW_AUGH, IN_PROGRESS);
        ...
        {
            if (m_frenziedInARow == 0)
                events.ScheduleEvent(EVENT_RESTART_ACHIEVEMENT, 10000);

            ++m_frenziedInARow;

            if (instance && m_frenziedInARow >= 20)
                instance->DoCompleteAchievement(eAchievements::ACH_ACROPALYPSIS_NOW);
        }

        uint32 GetData(uint32 type) override
        {
            if (type == eData::DATA_KILLS_IN_A_ROW)
                return m_frenziedInARow;
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_SUMMON_RANDOM_AUGH_2:
                        ...
                        SummonAugh();
                        break;
                    case EVENT_RESTART_ACHIEVEMENT:
                        m_frenziedInARow = 0;
                        break;
                }
            }
...
public:
    ...
    struct npc_frenzied_crocAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* p_Killer) override
        {
            ...
            if (instance)
            {
                instance->DoStartCriteriaTimer(CRITERIA_TIMED_TYPE_CREATURE, 43658);
                if (Creature* lockmaw = Unit::GetCreature(*me, instance->GetData64(DATA_LOCKMAW)))
                    lockmaw->AI()->SummonedCreatureDies(me, p_Killer);
            }
        }
        ...
        uint32 GetData(uint32 type) override
        {
            if (type == eData::DATA_KILLS_IN_A_ROW)
            {
                ...
                if (instance)
                    if (Creature* lockmaw = Unit::GetCreature(*me, instance->GetData64(DATA_LOCKMAW)))
                        return lockmaw->GetAI()->GetData(eData::DATA_KILLS_IN_A_ROW);
            }
...
public:
    ...
    struct boss_aughAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*Killer*/) override
        {
            ...
            if (instance)
            {
                instance->SetBossState(DATA_BOSS_LOCKMAW_AUGH, DONE);
...
class spell_dust_flail: public SpellScriptLoader
{
    public:
        ...
        class spell_dust_flail_AuraScript : public AuraScript
        {
            ...
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_dust_flail_AuraScript::ExtraEffectRemove, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
...
/// Acrocalypse Now - 5291
/// criteriaId - 15992
class achievement_acrocalypse_now : public AchievementCriteriaScript
{
public:
    achievement_acrocalypse_now() : AchievementCriteriaScript("achievement_acrocalypse_now") { }
    ...
    bool OnCheck(Player* source, Unit* target) override
    {
        ...
        return false;
        /*return target->GetAI()->GetData(eData::DATA_KILLS_IN_A_ROW) >= 20;*/
    }
};
...
void AddSC_boss_lockmaw_augh()
{
    ...
    new boss_augh();
    new achievement_acrocalypse_now();
