            ...
            if (InstanceScript* pInstance = pCreature->GetInstanceScript())
            {
                switch (pCreature->GetEntry())
                {
                    ...
                    case NPC_TRAVEL_TO_MAELSTORM:
                        ...
                        break;
                    case NPC_TRAVEL_START:
                        if (pInstance->GetBossState(DATA_BLACKHORN) == DONE && pInstance->GetBossState(DATA_SPINE) == DONE)
                        {
                            pPlayer->NearTeleportTo(madnessdeathwingPos.GetPositionX(), madnessdeathwingPos.GetPositionY(), madnessdeathwingPos.GetPositionZ(), madnessdeathwingPos.GetOrientation());
                            break;
                        }
                        else if (pInstance->GetBossState(DATA_ULTRAXION) == DONE)
                        {
                            pPlayer->NearTeleportTo(skyfirePos.GetPositionX(), skyfirePos.GetPositionY(), skyfirePos.GetPositionZ(), skyfirePos.GetOrientation());
                            break;
                        }
                        else if (pInstance->GetBossState(DATA_YORSAHJ) == DONE && pInstance->GetBossState(DATA_ZONOZZ) == DONE)
                        {
                            pPlayer->NearTeleportTo(teleportPos[3].GetPositionX(), teleportPos[3].GetPositionY(), teleportPos[3].GetPositionZ(), teleportPos[3].GetOrientation());
                            break;
                        }
                        else if (pInstance->GetBossState(DATA_MORCHOK) == DONE)
                        {
                            pPlayer->NearTeleportTo(teleportPos[0].GetPositionX(), teleportPos[0].GetPositionY(), teleportPos[0].GetPositionZ(), teleportPos[0].GetOrientation());
                            break;
                        }
...
class npc_dragon_soul_fly_trigger_one : public CreatureScript
{
    public:
        npc_dragon_soul_fly_trigger_one() : CreatureScript("npc_dragon_soul_fly_trigger_one") { }

        CreatureAI* GetAI(Creature* pCreature) const
        {
            return new npc_dragon_soul_fly_trigger_oneAI(pCreature);
        }

        struct npc_dragon_soul_fly_trigger_oneAI : public ScriptedAI
        {
            npc_dragon_soul_fly_trigger_oneAI(Creature* pCreature) : ScriptedAI(pCreature)
            {
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                me->SetReactState(REACT_PASSIVE);
                me->SetCanFly(true);
                me->SetDisableGravity(true);
                events.ScheduleEvent(EVENT_CHECK, 200);
            }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_CHECK:
                            ...
                            {
                                pPlayer->CastSpell(pPlayer, 44795, true);
                                pPlayer->KnockbackFrom(-1656.0f, -2988.4f, 10, 10);
                            }

                            if (Player* pPlayer = me->FindNearestPlayer(40.0f))
                            {
                                if (pPlayer->GetPositionZ() < me->GetPositionZ() && !pPlayer->HasAura(44795))
                                {
                                    pPlayer->CastSpell(pPlayer, 44795, true);
                                    pPlayer->KnockbackFrom(-1656.0f, -2988.4f, 10, 10);
                                }
...
void AddSC_dragon_soul()
{
    ...
    new npc_dragon_soul_fly_trigger_one();
