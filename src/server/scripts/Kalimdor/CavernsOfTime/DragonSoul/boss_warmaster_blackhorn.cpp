...
enum Events
{
    ...
    EVENT_DECK_FIRE             = 37,
    EVENT_FIRE_BRIGADE          = 38,
    EVENT_FIND_DECK_FIRE        = 39,
    EVENT_OUT_FIRE              = 40,
...
class boss_warmaster_blackhorn: public CreatureScript
{
    public:
        ...
        struct boss_warmaster_blackhornAI : public BossAI
        {
            ...
            {
                if (instance)
                    instance->SetBossState(DATA_BLACKHORN, FAIL);
            ...
            void DoAction(const int32 action) override
            {
                if (action == ACTION_START_BATTLE)
                {
                    Talk(SAY_AGGRO);
                    DoZoneInCombat();
                    if (Creature* pGoriona = me->FindNearestCreature(NPC_GORIONA, 110.0f))
                    {
                        DoZoneInCombat(pGoriona);
                        pGoriona->AI()->DoAction(ACTION_GORIONA_MOVE);
                    }
                    if (Creature* pShip = me->FindNearestCreature(NPC_SKYFIRE, 100.0f))
                    {
                ...
                else if (action == ACTION_END_BATTLE)
                {
                    ...
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYFIRE_HARPOON_GUN, 100.0f);
                    if (creatures.size() == 2)
                    {
                    ...
                    creatures.clear();
                    me->GetCreatureListWithEntryInGrid(creatures, NPC_SKYFIRE_CANNON, 100.0f);
                    if (!creatures.empty())
                    {
                        for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                        {
            ...
            void JustDied(Unit* /*killer*/) override
            {
                ...
                if (Creature* pSwayze = me->FindNearestCreature(NPC_SKY_CAPTAIN_SWAYZE, 100.0f))
                {
                    if (pSwayze->AI())
            ...
            void DeckFireRemove()
            {
                ...
                GetCreatureListWithEntryInGrid(_controllers, me, NPC_DECK_FIRE_CONTROLLER, 100.0f);
                for (std::list<Creature*>::iterator itr = _controllers.begin(); itr != _controllers.end(); ++itr)
                {
                    if (*itr)
                    {
                        if (DynamicObject* DynObj = (*itr)->GetDynObject(109445))
                            DynObj->Remove();
                ...
                GetCreatureListWithEntryInGrid(_brigades, me, NPC_FIRE_BRIGADE, 100.0f);
                for (std::list<Creature*>::iterator itr = _brigades.begin(); itr != _brigades.end(); ++itr)
                {
                    if (*itr)
                        (*itr)->DespawnOrUnsummon();
                }
        ...
        private:
            ...
            uint8 phase;
            bool deck;
            std::list<Creature*> _controllers;
            std::list<Creature*> _brigades;
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SUMMON_ADDS:
                            if (Creature* pBlackhorn = me->FindNearestCreature(NPC_BLACKHORN, 100.0f))
    ...
    public:
        ...
        struct npc_warmaster_blackhorn_twilight_elite_dreadblade_slayerAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* /*owner*/)
            {
                ...
                events.ScheduleEvent(EVENT_BLADE_RUSH, 40000);
                    ...
                    switch (eventId)
                    {
                        case EVENT_BLADE_RUSH:
                            ...
                            if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, -18.0f, true))
                            {
                                ...
                                events.ScheduleEvent(EVENT_BLADE_RUSH, 40000);
                                events.ScheduleEvent(EVENT_CONTINUE, 2500);
                            }
                            break;
                        case EVENT_CONTINUE:
                            me->SetReactState(REACT_AGGRESSIVE);
...
class npc_warmaster_blackhorn_skyfire_harpoon_gun: public CreatureScript
{
    public:
        ...
        private:
            ...
            {
                std::list<Creature*> creatures;
                me->GetCreatureListWithEntryInGrid(creatures, entry, 100.0f);
                if (!creatures.empty())
                {
                    for (std::list<Creature*>::const_iterator itr = creatures.begin(); itr != creatures.end(); ++itr)
                    {
    ...
    public:
        ...
        struct npc_warmaster_blackhorn_skyfireAI : public  Scripted_NoMovementAI
        {
            ...
            void DamageTaken(Unit* attacker, uint32& damage, SpellInfo const*  /*p_SpellInfo*/) override
            {
                if (IsHeroic())
                {
                    if (!hp75 && me->HealthBelowPctDamaged(75, damage))
                    {
                        hp75 = true;
                        me->SummonCreature(NPC_DECK_FIRE_CONTROLLER, 13452.0f, -12133.0f, 151.2f, 0.0f, TEMPSUMMON_CORPSE_DESPAWN);
                        events.ScheduleEvent(EVENT_FIRE_BRIGADE, 20000);
                    }
                    if (!hp50 && me->HealthBelowPctDamaged(50, damage))
                    {
                        hp50 = true;
                        me->SummonCreature(NPC_DECK_FIRE_CONTROLLER, 13452.0f, -12133.0f, 151.2f, 0.0f, TEMPSUMMON_CORPSE_DESPAWN);
                        events.ScheduleEvent(EVENT_FIRE_BRIGADE, 20000);
            ...
            void UpdateAI(const uint32 diff) override
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                    case EVENT_FIRE_BRIGADE:
                        me->SummonCreature(NPC_FIRE_BRIGADE, 13452.0f, -12133.0f, 161.2f, 0.0f, TEMPSUMMON_DEAD_DESPAWN);
                        break;
                    }
                }
            }
...
class npc_warmaster_blackhorn_deck_fire_controller : public CreatureScript
{
public:
    npc_warmaster_blackhorn_deck_fire_controller() : CreatureScript("npc_warmaster_blackhorn_deck_fire_controller") { }

    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_warmaster_blackhorn_deck_fire_controllerAI(pCreature);
    }

    struct npc_warmaster_blackhorn_deck_fire_controllerAI : public  Scripted_NoMovementAI
    {
        npc_warmaster_blackhorn_deck_fire_controllerAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
        {
            bool nearest = false;
            me->SetReactState(REACT_PASSIVE);
            ...
            std::list<Creature*> _controllers;
            GetCreatureListWithEntryInGrid(_controllers, me, NPC_DECK_FIRE_CONTROLLER, 1.0f);
            for (std::list<Creature*>::iterator itr = _controllers.begin(); itr != _controllers.end(); ++itr)
            {
                if (*itr)
                {
            ...
            if (nearest || x > 13463.0f || x < 13421.0f || y < -12151.0f || y > -12115.0f)
                me->DespawnOrUnsummon();
            else
            {
                me->CastSpell(me, 109445, false);
                me->CastSpell(me, 110092, false);
                events.ScheduleEvent(EVENT_DECK_FIRE, 5000);
            }
        }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_DECK_FIRE:
                        ...
                        me->SummonCreature(NPC_DECK_FIRE_CONTROLLER, mX, mY, z, 0.0f, TEMPSUMMON_CORPSE_DESPAWN);

                        events.ScheduleEvent(EVENT_DECK_FIRE, 10000);
                        break;
                }
            }
        }
...
class npc_warmaster_blackhorn_fire_brigade : public CreatureScript
{
public:
    npc_warmaster_blackhorn_fire_brigade() : CreatureScript("npc_warmaster_blackhorn_fire_brigade") { }

    CreatureAI* GetAI(Creature* pCreature) const
    {
        return new npc_warmaster_blackhorn_fire_brigadeAI(pCreature);
    }

    struct npc_warmaster_blackhorn_fire_brigadeAI : public  ScriptedAI
    {
        npc_warmaster_blackhorn_fire_brigadeAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            me->SetReactState(REACT_PASSIVE);
            events.ScheduleEvent(EVENT_FIND_DECK_FIRE, 1000);
            me->SetCanFly(true);
            me->SetDisableGravity(true);
            me->SetSpeed(MOVE_FLIGHT, 2.0f, true);
        }
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_FIND_DECK_FIRE:
                        if (Creature* DeckFire = me->FindNearestCreature(NPC_DECK_FIRE_CONTROLLER, 100.0f, true))
                        {
                            Movement::MoveSplineInit init(me);
                            init.MoveTo(DeckFire->GetPositionX(), DeckFire->GetPositionY(), DeckFire->GetPositionZ() + 10.0f);
                            init.SetWalk(false);
                            init.Launch();
                        }
                        events.ScheduleEvent(EVENT_OUT_FIRE, 4000);
                        break;
                    case EVENT_OUT_FIRE:
                        if (Creature* DeckFire = me->FindNearestCreature(NPC_DECK_FIRE_CONTROLLER, 100.0f, true))
                        {
                            if (DynamicObject* DynObj = DeckFire->GetDynObject(109445))
                                DynObj->Remove();
                            DeckFire->DespawnOrUnsummon();
                        }
                        events.ScheduleEvent(EVENT_FIND_DECK_FIRE, 1000);
                        break;
                }
    ...
    public:
        ...
        class spell_warmaster_blackhorn_twilight_barrage_dmg_SpellScript : public SpellScript
        {
            ...
            void FilterTargetsDamage(std::list<WorldObject*>& targets)
            {
                ...
                if (targets.empty())
                {
                    if (Creature* pBlackhorn = GetCaster()->FindNearestCreature(NPC_BLACKHORN, 100.0f))
                        pBlackhorn->AI()->DoAction(ACTION_DECK);

                    if (Creature* pShip = GetCaster()->FindNearestCreature(NPC_SKYFIRE, 100.0f))
                    {
    ...
    public:
        ...
        class spell_warmaster_blackhorn_vengeance_AuraScript : public AuraScript
        {
            ...
            void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                uint32 val = uint32(100.0f - GetUnitOwner()->GetHealthPct());

                if (AuraEffect* aurEff = GetAura()->GetEffect(EFFECT_0))
                    GetAura()->GetEffect(EFFECT_0)->ChangeAmount(val, false, true);
            }
...
void AddSC_boss_warmaster_blackhorn()
{
    ...
    new npc_warmaster_blackhorn_deck_fire_controller();
    new npc_warmaster_blackhorn_fire_brigade();
