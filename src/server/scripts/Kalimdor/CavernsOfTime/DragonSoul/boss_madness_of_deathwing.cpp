...
class npc_dragon_soul_thrall_1 : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* pPlayer, Creature* pCreature) override
        {
            ...
            if (!pInstance || pPlayer->isInCombat() || pInstance->GetData(DATA_SPINE) != DONE)
                return false;
            /*
        ...
        bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*sender*/, uint32 action) override
        {
            ...
            {
                pInstance->SetData(DATA_MADNESS, IN_PROGRESS);
        ...
        struct npc_dragon_soul_thrall_1AI : public ScriptedAI
        {
            ...
            void DoAction(const int32 action) override
            {
                ...
                else if (action == ACTION_END_BATTLE)
                {
                    ...
                    if (pInstance)
                    {
                        pInstance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT, 0, me);
            ...
            void ResetBattle(bool done = false)
            {
                if (!done)
                {
                    ...
                    if (pInstance)
                    {
                        pInstance->SetBossState(DATA_MADNESS, FAIL);
