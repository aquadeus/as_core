                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_UPDATE_HEALTH:
                            if (me->isAlive())
                            {
                                scaleCoeff = float((double)me->GetHealth() / (double)me->GetMaxHealth());
                                if (scaleCoeff < 0.2f)
                                    scaleCoeff = 0.2f;
                                me->SetObjectScale(1.0f*scaleCoeff);
                                ...
                                if (Creature* pKohcrom = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_KOHCROM)))
                                {
                                    ...
                                    pKohcrom->SetObjectScale(1.0f*scaleCoeff);
                        ...
                        case EVENT_BLACK_BLOOD:
                            ...
                            break;
                        case EVENT_CONTINUE:
                            if (events.GetNextEventTime(EVENT_STOMP) >= events.GetNextEventTime(EVENT_RESONATING_CRYSTAL))
                            {
                                events.CancelEvent(EVENT_STOMP);
                                events.CancelEvent(EVENT_RESONATING_CRYSTAL);
                                events.ScheduleEvent(EVENT_STOMP, 12000);
                                events.ScheduleEvent(EVENT_RESONATING_CRYSTAL, 6000);
                            }
                            else
                            {
                                events.CancelEvent(EVENT_STOMP);
                                ...
                                events.ScheduleEvent(EVENT_RESONATING_CRYSTAL, 15000);
                            }
                            me->SetReactState(REACT_AGGRESSIVE);
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    case ACTION_KOHCROM_STOMP:
                        if (GetDifficulty() == Difficulty25N)
                            events.ScheduleEvent(EVENT_STOMP, 5000);
                        else
                            events.ScheduleEvent(EVENT_STOMP, 6000);
                        break;
                    case ACTION_KOHCROM_RESONATING_CRYSTAL:
                        if (GetDifficulty() == Difficulty25HC)
                            events.ScheduleEvent(EVENT_RESONATING_CRYSTAL, 5000);
                        else
                            events.ScheduleEvent(EVENT_RESONATING_CRYSTAL, 6000);
                        break;
    ...
    public:
        ...
        class spell_morchok_stomp_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                {
                    if (Unit* MorchokTarget = pMorchok->getVictim())
                    {
                        targets.sort(DistanceOrderPred(MorchokTarget));
