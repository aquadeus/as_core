...
class boss_warlord_zonozz: public CreatureScript
{
    public:
        ...
        struct boss_warlord_zonozzAI : public BossAI
        {
            ...
            {
                if (instance)
                    instance->SetBossState(DATA_ZONOZZ, FAIL);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_VOID_OF_THE_UNMAKING:
                            ...
                            events.ScheduleEvent(EVENT_PSYCHIC_DRAIN, 9000);
                            break;
                        ...
                        case EVENT_END_TANTRUM_2:
                            ...
                            timerVoid = events.GetNextEventTime(EVENT_VOID_OF_THE_UNMAKING) ? events.GetNextEventTime(EVENT_VOID_OF_THE_UNMAKING) - events.GetTimer() : 0;
                            if (timerVoid == 0)
                                timerVoid = IsHeroic() ? 15000 : 24000;
                            events.CancelEvent(EVENT_VOID_OF_THE_UNMAKING);
                            events.ScheduleEvent(EVENT_VOID_OF_THE_UNMAKING, timerVoid);
        ...
        class spell_warlord_zonozz_disrupting_shadows_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (GetCaster()->GetMap()->Is25ManRaid())
                    max_targets = RAND(7, 10);
                else
                    max_targets = RAND(2, 3);
