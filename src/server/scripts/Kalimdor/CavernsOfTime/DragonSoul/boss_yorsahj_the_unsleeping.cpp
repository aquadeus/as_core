...
class boss_yorsahj_the_unsleeping: public CreatureScript
{
    public:
        ...
        struct boss_yorsahj_the_unsleepingAI : public BossAI
        {
            ...
            {
                instance->SetBossState(DATA_YORSAHJ, FAIL);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CONTINUE:
                        {
                            ...
                            events.ScheduleEvent(EVENT_CALL_BLOOD_1, 39000);
                            break;
                        }
            ...
            void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Aura* aur = GetAura())
                {
                    if (aur->GetStackAmount() >= 5)
                    {
                        ...
                        {
                            case Difficulty10N:
                                spellId = SPELL_DEEP_CORRUPTION_DMG;
                                break;
                            case Difficulty25N:
                                spellId = SPELL_DEEP_CORRUPTION_DMG_25;
                                break;
                            case Difficulty10HC:
                                spellId = SPELL_DEEP_CORRUPTION_DMG_10H;
                                break;
                            case Difficulty25HC:
                                spellId = SPELL_DEEP_CORRUPTION_DMG_25H;
                                break;
                            default:
                                break;
                        }
