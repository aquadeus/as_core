    ...
    public:
                    ...
                    switch (eventId)
                    {
                        case EVENT_END_ENCOUNTER:
                            ...
                            if (pInstance)
                            {
                                ...
                                pInstance->SetData(DATA_SPINE, DONE);
                            }                            
                            break;
        ...
        private:
            ...
            void ResetBattle(bool done = false, bool despawn = true)
            {
                ...
                pInstance->DoRemoveAurasDueToSpellOnPlayers(SPELL_DEGRADATION);

                if (pInstance)
                {
                    pInstance->SetBossState(DATA_SPINE, NOT_STARTED);
                ...
                public:
                    ...
                    bool operator()(Player* u)
                    {
                        ...
                        if (_withAura)
                        {
                            ...
                            switch (_obj->GetMap()->GetDifficultyID())
                            {
                                case Difficulty25N:
...
class playerscript_remove_degredation : public PlayerScript
{
    public:
        playerscript_remove_degredation() : PlayerScript("playerscript_remove_degredation") { }

        void OnLogin(Player* p_Player) override
        {
            if (p_Player->HasAura(SPELL_DEGRADATION))
                p_Player->RemoveAura(SPELL_DEGRADATION);
        }
};
...
void AddSC_spine_of_deathwing()
{
    ...
    /// Player Script
    new playerscript_remove_degredation();
}
#endif
