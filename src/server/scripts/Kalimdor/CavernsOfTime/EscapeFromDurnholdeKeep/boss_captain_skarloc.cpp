...
struct boss_captain_skarloc : public ScriptedAI
{
    explicit boss_captain_skarloc(Creature* creature) : ScriptedAI(creature), summons(me) { }
    ...
    void MovementInform(uint32 type, uint32 id) override
    {
        ...
        {
            events2.RescheduleEvent(EVENT_INITIAL_TALK, 500);
            events2.RescheduleEvent(EVENT_START_FIGHT, 11000);
        }
    }
    ...
    void EnterCombat(Unit* /*who*/) override
    {
        ...
        events.RescheduleEvent(EVENT_SPELL_HOLY_LIGHT, 15000);
        events.RescheduleEvent(EVENT_SPELL_CLEANSE, 6000);
        events.RescheduleEvent(EVENT_SPELL_HAMMER, 20000);
        events.RescheduleEvent(EVENT_SPELL_HOLY_SHIELD, 10000);
        if (IsHeroic())
            events.RescheduleEvent(EVENT_SPELL_CONSECRATION, 1000);
    }
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        case EVENT_SPELL_HOLY_LIGHT:
            ...
            events.RescheduleEvent(EVENT_SPELL_HOLY_LIGHT, 20000);
            break;
        case EVENT_SPELL_CLEANSE:
            if (roll_chance_i(33))
                Talk(SAY_TAUNT);
            me->CastSpell(me, SPELL_CLEANSE, false);
            events.RescheduleEvent(EVENT_SPELL_CLEANSE, 10000);
            break;
        case EVENT_SPELL_HAMMER:
            DoCastVictim(SPELL_HAMMER_OF_JUSTICE, false);
            events.RescheduleEvent(EVENT_SPELL_HAMMER, 30000);
            break;
        case EVENT_SPELL_HOLY_SHIELD:
            me->CastSpell(me, SPELL_CLEANSE, false);
            events.RescheduleEvent(SPELL_HOLY_SHIELD, 30000);
            break;
        case EVENT_SPELL_CONSECRATION:
            me->CastSpell(me, SPELL_CONSECRATION, false);
            events.RescheduleEvent(EVENT_SPELL_CONSECRATION, 20000);
            break;
...
{
    RegisterCreatureAI(boss_captain_skarloc);
}
