...
public:
    ...
    struct instance_old_hillsbrad_InstanceMapScript : public InstanceScript
    {
        ...
        void CleanupInstance()
        {
            ...
            _events.RescheduleEvent(EVENT_INITIAL_BARRELS_FLAME, 0);
            _events.RescheduleEvent(EVENT_FINAL_BARRELS_FLAME, 0);

            if (_encounterProgress == ENCOUNTER_PROGRESS_BARRELS)
                _events.RescheduleEvent(EVENT_SUMMON_LIEUTENANT, 0);
            else
        ...
        void SetData(uint32 type, uint32 data) override
        {
            switch (type)
            {
                case DATA_THRALL_REPOSITION:
                    if (data > 1)
                        _events.RescheduleEvent(EVENT_THRALL_REPOSITION, data == 2 ? 0 : 10000);
                ...
                case DATA_BOMBS_PLACED:
                {
                    ...
                    if (_barrelCount == 5)
                    {
                        _events.RescheduleEvent(EVENT_INITIAL_BARRELS_FLAME, 4000);
                        _events.RescheduleEvent(EVENT_FINAL_BARRELS_FLAME, 12000);
                        _events.RescheduleEvent(EVENT_SUMMON_LIEUTENANT, 18000);
                    }
                    break;
                }
