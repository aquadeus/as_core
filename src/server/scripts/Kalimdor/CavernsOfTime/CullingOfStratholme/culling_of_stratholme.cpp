...
auto getBroadcastText = [](Player* player, uint32 id)
{
    if (!player)
        return "";
    auto entry = sBroadcastTextStore.LookupEntry(id);
    if (entry)
        return entry->MaleText->Get(player->GetSession()->GetSessionDbLocaleIndex());
    return "";
};
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32 action)
    {
        if (creature->HasFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP))
        {
            ...
            case GOSSIP_ACTION_INFO_DEF + 2:
                ...
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 27745), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        switch (pInstance->GetData(DATA_ARTHAS_EVENT))
        {
            case COS_PROGRESS_FINISHED_INTRO:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 27524), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
                player->SEND_GOSSIP_MENU(GOSSIP_MENU_ARTHAS_1, creature->GetGUID());
                break;
            case COS_PROGRESS_REACHED_TOWN_HALL:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 27744), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
                player->SEND_GOSSIP_MENU(GOSSIP_MENU_ARTHAS_2, creature->GetGUID());
                break;
            case COS_PROGRESS_KILLED_EPOCH:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 27602), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
                player->SEND_GOSSIP_MENU(GOSSIP_MENU_ARTHAS_4, creature->GetGUID());
                break;
            case COS_PROGRESS_LAST_CITY:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 28044), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 5);
                player->SEND_GOSSIP_MENU(GOSSIP_MENU_ARTHAS_5, creature->GetGUID());
                break;
            case COS_PROGRESS_BEFORE_MALGANIS:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, getBroadcastText(player, 28508), GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 6);
    ...
    struct npc_arthasAI : public npc_escortAI
    {
        ...
        EventMap cityGatesEvents;
        uint8 waveGroupId;
        uint8 waveKillCount;
        uint8 timeRiftId;
        GuidSet creatureToKill;
        ...
        {
            if (creatureToKill.find(cr->GetGUID()) != creatureToKill.end())
                creatureToKill.erase(cr->GetGUID());
        ...
        void DoAction(int32 param) override
        {
            if (param == ACTION_START_EVENT)
            {
                // Event
                eventInRun = true;
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE1, 0);
            }
            else if (param == ACTION_START_CITY)
            {
                Talk(SAY_PHASE201);
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE2, 12000);
                SetRun(false);
                eventInRun = true;
            ...
            else if (param == ACTION_KILLED_SALRAMM)
            {
                ...
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE2 + 9, 10000);
                cityGatesEvents.CancelEvent(EVENT_ACTION_SUMMON);
            }
            ...
            else if (param == ACTION_KILLED_MALGANIS)
            {
                EnterEvadeMode();
                eventInRun = true;
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE5 + 1, 22000);
                me->SetFacingTo(1.84f);

                if (!me->GetMap()->GetPlayers().isEmpty())
                    if (auto player = me->GetMap()->GetPlayers().getFirst()->getSource())
                        player->RewardPlayerAndGroupAtEvent(31006, player); // Malganis quest entry required
            }
            else if (param == ACTION_SKIP_INTRO)
            {
                SummonNextWave();
                me->SetReactState(REACT_PASSIVE);
                cityGatesEvents.RescheduleEvent(EVENT_ACTION_SUMMON, 2000);
            }
        }
        ...
        void Reset() override
        {
            ...
            if (pInstance)
            {
                ...
                if (pInstance->GetData(DATA_ARTHAS_EVENT) == COS_PROGRESS_FINISHED_CITY_INTRO || pInstance->GetData(DATA_ARTHAS_EVENT) == COS_PROGRESS_FINISHED_CITY_INTRO_SKIP)
                    cityGatesEvents.RescheduleEvent(EVENT_ACTION_SUMMON, 2000);
            }

            bool needClean = true;
            if (!creatureToKill.empty())
                for (auto g : creatureToKill)
                    if (ObjectAccessor::GetCreature(*me, g))
                    {
                        needClean = false;
                        break;
                    }
            if (needClean)
                creatureToKill.clear();
        }
        ...
        void WaypointReached(uint32 uiPointId) override
        {
            ...
            case 22:
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE3, 0);
                eventInRun = true;
                SetEscortPaused(true);
                break;
            ...
            case 36:
                SetRun(true);
                if (pInstance)
                    if (GameObject* pGate = pInstance->instance->GetGameObject(pInstance->GetData64(DATA_HIDDEN_DOOR)))
            ...
            case 55:
                Talk(SAY_PHASE502);
                actionEvents.RescheduleEvent(EVENT_ACTION_PHASE5, 7000);
                SetEscortPaused(true);
                eventInRun = true;
                break;
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            cityGatesEvents.Update(diff);

            while (auto eventIds = cityGatesEvents.ExecuteEvent())
            {
                switch (eventIds)
                {
                case EVENT_ACTION_SUMMON:
                {
                    if (roll_chance_i(50))
                    {
                        if (auto cre = pInstance->instance->SummonCreature(NPC_RISEN_ZOMBIE_ALT, zombiePos[urand(0, 2)]))
                        {
                            if (cre->IsAIEnabled)
                                cre->AI()->DoAction(ACTION_FORCE_CHANGE_LOCK);
                            cre->SetWalk(true);
                            cre->setActive(true);
                            cre->GetMotionMaster()->MovePoint(1, 2096.813f + frand(-1, 1), 1286.871 + frand(-1, 1), 138.675f);
                        }
                    }
                    else
                    {
                        if (auto cre = pInstance->instance->SummonCreature(NPC_RUNNING_CITIZEN, CitizenSpawn))
                        {
                            cre->setFaction(2075);
                            cre->setActive(true);
                            cre->CastSpell(cre, 52385, true);
                            //                  auto path = sObjectMgr->GetPathByCoordinatesListEntry(NPC_RUNNING_CITIZEN);
                            //                  cre->GetMotionMaster()->MoveSmoothPath(1, path, path.size(), false);
                        }
                    }
                    cityGatesEvents.RescheduleEvent(EVENT_ACTION_SUMMON, urand(3000, 12000));
                    break;
                }
                }
            }
            ...
            if (eventInRun)
            {
                ...
                case EVENT_ACTION_PHASE2 + 4:
                    if (Creature* malganis = me->SummonCreature(NPC_MAL_GANIS, EventPos[EVENT_SRC_MALGANIS], TEMPSUMMON_TIMED_DESPAWN, 60000))
                    {
                        ...
                        malganis->GetCreatureListWithEntryInGrid(unitList, 20.f, NPC_CITY_MAN);
                        malganis->GetCreatureListWithEntryInGrid(unitList, 20.f, NPC_CITY_MAN2);
                ...
                case EVENT_ACTION_PHASE3 + 6:
                    if (Creature* cr = GetEventNpc(NPC_CITY_MAN))
                    {
                        creatureToKill.insert(cr->GetGUID());
                        cr->UpdateEntry(NPC_INFINITE_HUNTER);
                        cr->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
                ...
                case EVENT_ACTION_PHASE3 + 7:
                    ...
                    {
                        creatureToKill.insert(cr->GetGUID());
                        cr->UpdateEntry(NPC_INFINITE_AGENT);
                        cr->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC | UNIT_FLAG_IMMUNE_TO_NPC);
                        cr->SetReactState(REACT_PASSIVE);
                    }
                    ScheduleNextEvent(currentEvent, 2000);
                ...
                case EVENT_ACTION_PHASE3 + 8:
                    ...
                    {
                        creatureToKill.insert(cr->GetGUID());
                        cr->UpdateEntry(NPC_INFINITE_ADVERSARY);
...
{
    actionEvents.PopEvent();
    actionEvents.RescheduleEvent(currentEvent + 1, time);
}
...
void npc_arthas::npc_arthasAI::EnterCombat(Unit* /*who*/)
{
    ...
    // Fight
    combatEvents.RescheduleEvent(EVENT_COMBAT_EXORCISM, 2000);
    combatEvents.RescheduleEvent(EVENT_COMBAT_HEALTH_CHECK, 2000);
}
...
void npc_arthas::npc_arthasAI::ReorderInstance(uint32 data)
{
    ...
    case COS_PROGRESS_KILLED_SALRAMM:
        ...
        if (data == COS_PROGRESS_FINISHED_CITY_INTRO || data == COS_PROGRESS_FINISHED_CITY_INTRO_SKIP)
        {
            if (data == COS_PROGRESS_FINISHED_CITY_INTRO_SKIP)
                me->SetReactState(REACT_PASSIVE);
            eventInRun = true;
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32  /*action*/)
    {
        ...
        else if (player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 11277)
        {
            if (InstanceScript* pInstance = creature->GetInstanceScript())
            {
                if (pInstance->GetData(DATA_ARTHAS_EVENT) == COS_PROGRESS_NOT_STARTED)
                {
                    pInstance->SetData(DATA_ARTHAS_EVENT, COS_PROGRESS_FINISHED_CITY_INTRO_SKIP);

                    if (Creature* arthas = ObjectAccessor::GetCreature(*creature, pInstance->GetData64(DATA_ARTHAS)))
                    {
                        arthas->AI()->Reset();
                        arthas->AI()->DoAction(ACTION_SKIP_INTRO);
                    }
                }
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 /*sender*/, uint32  /*action*/)
    {
        ...
        {
            if (player->PlayerTalkClass->GetGossipMenu().GetMenuId() == 9612 && creature->GetInstanceScript()->GetData(DATA_ARTHAS_EVENT) == COS_PROGRESS_CRATES_FOUND)
...
public:
    ...
    struct npc_cos_stratholme_citizienAI : public ScriptedAI
    {
        ...
        bool locked, allowMove;
        uint32 changeTimer;
        InstanceScript* pInstance;
        uint32 checkPosTimer;
        uint32 allowTimer;
        uint32 moveTimer = 0;
        ...
        void MoveInLineOfSight(Unit* who) override
        {
            if (!me->HasAura(52385))
            {
                ...
                if (!allowTimer && !locked && who->IsPlayer() && me->FindNearestCreature(NPC_ARTHAS, 80.f, true) && me->GetDistance(who) < 80.0f)
                {
                    InfectMe(1000);
                    if (allowMove)
                    {
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            if (moveTimer)
            {
                ...
                {
                    moveTimer = 0;
                    me->GetMotionMaster()->MovePoint(1, 2096.813f + frand(-1, 1), 1286.871 + frand(-1, 1), 138.675f);
                }
                else
                    moveTimer -= diff;
            }
            ...
            if (changeTimer)
            {
                ...
                {
                    me->UpdateEntry(NPC_RISEN_ZOMBIE);
                    me->SetReactState(REACT_AGGRESSIVE);
                    changeTimer = 20000;
                }
...
// 27745
struct npc_cos_lordaeron_footman : public ScriptedAI
{
    npc_cos_lordaeron_footman(Creature* creature) : ScriptedAI(creature) {}
    ...
    void EnterCombat(Unit* who) override
    {
        events.RescheduleEvent(1, urand(3000, 5000));
        events.RescheduleEvent(2, urand(7000, 12000));
    }

    void MoveInLineOfSight(Unit* who) override
    {
        if (who->ToCreature() && me->GetDistance(who) > 8.f)
            return;
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        if (auto eventId = events.ExecuteEvent())
        {
            ...
            {
                DoCastVictim(25710);
                events.RescheduleEvent(1, urand(5000, 8000));
                break;
            }
            case 2:
            {
...
{
    RegisterCreatureAI(npc_cos_lordaeron_footman);
    new npc_arthas();
