...
class spell_asira_dawnslayer_blade_barrier: public SpellScriptLoader
{
    public:
        ...
        class spell_asira_dawnslayer_blade_barrier_AuraScript : public AuraScript
        {
            ...
            {
                if (dmgInfo.GetAmount() < (uint32)GetSpellInfo()->Effects[EFFECT_0].BasePoints)
                    absorbAmount = dmgInfo.GetAmount() - 1;
                else
                    GetAura()->Remove();
            }
                    ...
                    bool operator()(WorldObject* unit)
                    {
                        ...
                        switch (unit->ToPlayer()->getClass())
                        {
                            ...
                            case CLASS_HUNTER:
                                return true;
                            case CLASS_DRUID:
                                //if (unit->ToPlayer()->GetActiveSpecializationID() == TALENT_TREE_DRUID_FERAL_COMBAT)
                                //    return true;
                                return false;
                            case CLASS_PALADIN:
                                //if (unit->ToPlayer()->GetActiveSpecializationID() == TALENT_TREE_PALADIN_HOLY)
                                //    return false;
                                return true;
                            default:
                                return false;
