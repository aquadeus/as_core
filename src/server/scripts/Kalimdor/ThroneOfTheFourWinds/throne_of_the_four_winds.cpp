...
struct npc_slipstream_raid : public ScriptedAI
{
    explicit npc_slipstream_raid(Creature* creature) : ScriptedAI(creature), isActive(true), linkedSlipstreamObject(NULL), linkedBoss(NULL), isUltimate(NULL)
    {
        if (!me->ToTempSummon())
    ...
    {
        if (me->ToTempSummon() || SlipstreamPosition >= DIR_ERROR || who->GetTypeId() != TYPEID_PLAYER || !isActive)
            return;
    ...
    {
        if (clicker)
            clicker->NearTeleportTo(-108.98f, 807.66f, 195.49f, 0.2f);
    }
...
{
    RegisterCreatureAI(npc_slipstream_raid);
