...
{
    explicit boss_alakir(Creature* pCreature) : BossAI(pCreature, DATA_ALAKIR_EVENT)
    {
    ...
    void Reset() override
    {
        ...
        if (instance->GetData(DATA_CONCLAVE_OF_WIND_EVENT) == DONE)
    ...
    {
        _JustDied();
        if (auto chest = instance->instance->GetGameObject(instance->GetData64(DATA_ALAKIR_CHEST)))
    ...
    void UpdateAI(uint32 diff) override
    {
        ...
        while (uint32 eventId = events.ExecuteEvent())
        {
            ...
            case EVENT_ACID_RAIN:
                DoCast(RAID_MODE(SPELL_ACID_RAIN_N_10, SPELL_ACID_RAIN_H_10, SPELL_ACID_RAIN_N_25, SPELL_ACID_RAIN_H_25));
...
struct npc_stormling : public ScriptedAI
{
    explicit npc_stormling(Creature* pCreature) : ScriptedAI(pCreature) { }
...
struct npc_tornado_moving : public ScriptedAI
{
    explicit npc_tornado_moving(Creature* creature) : ScriptedAI(creature) { }
...
{
    RegisterCreatureAI(boss_alakir);
    RegisterCreatureAI(npc_stormling);
    RegisterCreatureAI(npc_tornado_moving);
}