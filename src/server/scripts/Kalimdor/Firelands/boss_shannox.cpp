    ...
    public:
        ...
        struct boss_shannoxAI : public BossAI
        {
            ...
            void Reset() override
            {
                ...
                if (instance->GetBossState(DATA_SHANNOX) != DONE)
                    me->SetVisible(true);
                else
    ...
    public:
        ...
        struct npc_shannox_riplimbAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                _shannox = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_SHANNOX));
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (!UpdateVictim() && !bDead)
                {
                    if (!me->isInCombat())
                    {
                        ...
                        if (!_shannox->isInCombat() && me->GetDistance(_shannox) > 8.0f)
                            me->GetMotionMaster()->MoveChase(_shannox, 5.0f);
                    }
...
class spell_shannox_crystal_prison_trap : public SpellScriptLoader
{
    public:
        ...
        class spell_shannox_crystal_prison_trap_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                AfterEffectApply += AuraEffectApplyFn(spell_shannox_crystal_prison_trap_AuraScript::AfterApply, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
                AfterEffectRemove += AuraEffectRemoveFn(spell_shannox_crystal_prison_trap_AuraScript::AfterRemove, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
            }
        };
