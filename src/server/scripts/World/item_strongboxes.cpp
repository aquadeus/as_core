        ...
        bool OnOpen(Player* p_Player, Item* p_Item)
        {
            ...
            else
            {
                if (roll_chance_i(60))
                {
                    ...
                    p_Player->ModifyMoney(l_GoldAmount, "item_strongbox");
                ...
                else
                {
                    ...
                    p_Player->SendDisplayToast(CURRENCY_TYPE_HONOR_POINTS, l_HonorAmount, 0, DISPLAY_TOAST_METHOD_CURRENCY_OR_GOLD, TOAST_TYPE_NEW_CURRENCY, false, false);
                }
            }
...
/// champion strongbox (legion) - 138469 - 138474
class item_champion_strongbox : public ItemScript
{
    public:
        item_champion_strongbox() : ItemScript("item_champion_strongbox") { }
        ...
        bool OnOpen(Player* p_Player, Item* p_Item)
        {
            ...
            uint32 l_Random = urand(0, 100);

            if (l_Random < 30)  ///< 0-29, 30% chance to get class hall ressources
            {
                uint32 l_RessourcesCount = urand(20, 200);
                p_Player->ModifyCurrency(CURRENCY_TYPE_ORDER_RESSOURCES, l_RessourcesCount);
                p_Player->SendDisplayToast(CURRENCY_TYPE_ORDER_RESSOURCES, l_RessourcesCount, 0, DISPLAY_TOAST_METHOD_CURRENCY_OR_GOLD, TOAST_TYPE_NEW_CURRENCY, false, false);
            }
            else if (l_Random < 50) ///< 30-50 20% chance to get golds
            {
                uint32 l_Gold = urand(1, 30) * GOLD;
                p_Player->ModifyMoney(l_Gold, "item champion strongbox - legion");
                p_Player->SendDisplayToast(0, l_Gold, 0, DISPLAY_TOAST_METHOD_CURRENCY_OR_GOLD, TOAST_TYPE_MONEY, false, false);
            }
            else if (l_Random < 70) ///< 50 - 70 20% chance to get a random potions
            {
                std::array<uint32, 13> l_Potions = {{127834, 127835, 127836, 127837, 127838, 127839, 127840, 127841, 127846, 127843, 127844, 127845, 142117}};

                uint32 l_PotionID = l_Potions[rand() % 13];
                p_Player->AddItem(l_PotionID, 1);
                p_Player->SendDisplayToast(l_PotionID, 1, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
            }
            else ///< 70-100 30% chance to get honor marks
            {
                uint32 l_MarkCount = urand(1, 3);
                p_Player->AddItem(137642, l_MarkCount);
                p_Player->SendDisplayToast(137642, l_MarkCount, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
            }

            p_Player->CompleteLegendaryActivity(LegendaryActivity::Strongbox);
...
class item_runic_pouch : public ItemScript
{
    public:
        ...
        bool OnOpen(Player* p_Player, Item* p_Item)
        {
            ...
            p_Player->SendDisplayToast(l_ItemID, l_Count, 0, DISPLAY_TOAST_METHOD_LOOT, TOAST_TYPE_NEW_ITEM, false, false);
