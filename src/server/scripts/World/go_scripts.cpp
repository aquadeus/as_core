            ...
            bool GossipHello(Player* player)
            {
                ...
                if (player->HasItemCount(_stoneId, 1))
                {
                    if (SpellInfo const* spell = sSpellMgr->GetSpellInfo(_stoneSpell))
                        Spell::SendCastResult(player, spell, Guid128(GUID_TYPE_CAST, 0, spell->Id, 0), SPELL_FAILED_TOO_MANY_OF_ITEM);
                    return true;
                }
...
class go_cooking_fire : public GameObjectScript
{
public:
    go_cooking_fire() : GameObjectScript("go_cooking_fire") { }
    ...
    struct go_cooking_fire_setAI : public GameObjectAI
    {
        go_cooking_fire_setAI(GameObject* p_GameObject) : GameObjectAI(p_GameObject), m_Timer(1000) { }
        ...
        void UpdateAI(uint32 p_Diff) override
        {
            ...
            {
                go->CastSpell(nullptr, eSpells::CozyFire);
                m_Timer = 1000;
            }
        }
    };
    ...
    {
        return new go_cooking_fire_setAI(p_GameObject);
    }
};

/// Painted Egg - 235906
/// Update 7.1.5 Build 23420
class go_painted_egg : public GameObjectScript
{
    public:
        go_painted_egg() : GameObjectScript("go_painted_egg") { }

        struct go_painted_eggAI : public GameObjectAI
        {
            go_painted_eggAI(GameObject* p_GameObject) : GameObjectAI(p_GameObject) { }
        ...
        GameObjectAI* GetAI(GameObject* p_GameObject) const override
        {
            return new go_painted_eggAI(p_GameObject);
        }
};
