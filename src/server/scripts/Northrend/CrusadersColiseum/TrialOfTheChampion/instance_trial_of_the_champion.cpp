...
public:
    ...
    struct instance_trial_of_the_champion_InstanceMapScript : public InstanceScript
    {
        ...
        bool IsEncounterInProgress(int32 /*p_ExcludeBossID = -1*/) const
        {
        ...
        void SetData(uint32 type, uint32 data)
        {
            switch(type)
            {
                case BOSS_GRAND_CHAMPIONS:
                    ...
                    {
                        DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT_CHAMPIONS);
                ...
                case BOSS_ARGENT_CHALLENGE_E:
                    ...
                    if (data == DONE)
                    {
                        if (Creature* announcer = instance->GetCreature(uiAnnouncerGUID))
                        {
                            DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT_EADRIC);
                            announcer->GetMotionMaster()->MovePoint(0, 742.742f, 630.207f, 411.172f);
                            announcer->SetFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                ...
                case BOSS_ARGENT_CHALLENGE_P:
                    ...
                    if (data == DONE)
                    {
                        if (Creature* announcer = instance->GetCreature(uiAnnouncerGUID))
                        {
                            DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT_PALETRESS);
                            DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT_MEMORIES);
                            announcer->GetMotionMaster()->MovePoint(0, 742.742f, 630.207f, 411.172f);
                            announcer->SetFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                ...
                case BOSS_BLACK_KNIGHT:
                    ...
                    if (data == DONE)
                        DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_ACHIEVEMENT_BLACK_KNIGHT);
                    break;
