    ...
    struct boss_eadricAI : public ScriptedAI
    {
                ...
                if(spell->Id == SPELL_HAMMER_RIGHTEOUS_RET)
                    if(me->GetHealth() <= (uint32)spell->Effects[0].BasePoints)
                        if(InstanceScript* instance = me->GetInstanceScript())
                            instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_EADRIC_ACHIEVEMENT);
        }
        ...
        void DamageTaken(Unit* /*attacker*/, uint32 & damage, SpellInfo const*  /*p_SpellInfo*/)
        {
            ...
            if(damage >= me->GetHealth())
            {
                ...
                defeated = true;
                if (InstanceScript* instance = me->GetInstanceScript())
                    instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, 68575);
...
class spell_paletress_shield: public SpellScriptLoader
{
    public:
        ...
        class spell_paletress_shieldAuraScript : public AuraScript
        {
            ...
            {
                Unit* caster = GetCaster();
                Unit* attacker = dmgInfo.GetActor();
                ...
                caster->SendSpellNonMeleeDamageLog(attacker, GetSpellInfo()->Id, (uint32) (absorbAmount*0.25f), dmgInfo.GetSchoolMask(), 0, 0, true, 0, aurEff->GetBase()->GetSpellXSpellVisualID());
            }
    ...
    struct boss_paletressAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* /*attacker*/, uint32 & damage, SpellInfo const*  /*p_SpellInfo*/)
        {
            ...
            if(damage >= me->GetHealth())
            {
                ...
                defeated = true;
                if (InstanceScript* instance = me->GetInstanceScript())
                    instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, 68574);
                me->SetUnitMovementFlags(MOVEMENTFLAG_WALKING);
                me->GetMotionMaster()->MovePoint(1, me->GetHomePosition());
                me->SetTarget(0);
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
