...
struct npc_mounted_championAI : ScriptedAI
{
    ...
    bool isInMountedGauntlet()
    {
        ...
        {
            if (announcer->IsAIEnabled && (announcer->AI()->GetData(EVENT_INTRO) == IN_PROGRESS || announcer->AI()->GetData(EVENT_WAVES) == IN_PROGRESS))
                return true;
        }
...
class spell_toc5_ride_mount: public SpellScriptLoader
{
    public:
        ...
        class spell_toc5_ride_mount_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                AfterEffectApply += AuraEffectApplyFn(spell_toc5_ride_mount_AuraScript::HandleAfterEffect, EFFECT_0, SPELL_AURA_CONTROL_VEHICLE, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
                AfterEffectRemove += AuraEffectRemoveFn(spell_toc5_ride_mount_AuraScript::HandleAfterEffect, EFFECT_0, SPELL_AURA_CONTROL_VEHICLE, AURA_EFFECT_HANDLE_REAL);
            }
        };
