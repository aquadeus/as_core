            ...
            void MovementInform(uint32 type, uint32 id)
            {
                ...
                switch (id)
                {
                    case POINT_FESTERGUT:
                        if (instance->GetBossState(DATA_FESTERGUT) == NOT_STARTED)
                        ...
                        break;
                    case POINT_ROTFACE:
                        if (instance->GetBossState(DATA_ROTFACE) == NOT_STARTED)
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_FESTERGUT_GOO:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, NonTankTargetSelector(me)))
                            {
                                ...
                                {
                                    stalker->setFaction(FACTION_FRIENDLY);
                        ...
                        case EVENT_ROTFACE_DIES:
                            ...
                            if (GameObject *l_Obj = instance->instance->GetGameObject(instance->GetData(GameObjectsIds::GO_GREEN_PLAGUE_MONSTER_ENTRANCE)))
                                l_Obj->SetGoState(GOState::GO_STATE_READY);
    ...
    public:
        ...
        class spell_putricide_mutation_init_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckRequirement()
            {
                ...
                {
                    Spell::SendCastResult(GetExplTargetUnit()->ToPlayer(), GetSpellInfo(), Guid128(GUID_TYPE_CAST, 0, GetSpellInfo()->Id, 0), result, extension);
                    return result;
                }
            ...
            void HandleSummon(SpellEffIndex effIndex)
            {
                ...
                if (putricide->AI()->GetData(DATA_ABOMINATION))
                {
                    if (Player* player = caster->ToPlayer())
                        Spell::SendCastResult(player, GetSpellInfo(), Guid128(GUID_TYPE_CAST, 0, GetSpellInfo()->Id, 0), SPELL_FAILED_CUSTOM_ERROR, SPELL_CUSTOM_ERROR_TOO_MANY_ABOMINATIONS);
                    return;
                }
