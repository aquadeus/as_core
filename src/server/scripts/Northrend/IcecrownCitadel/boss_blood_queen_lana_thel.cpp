        ...
        struct boss_blood_queen_lana_thelAI : public BossAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (_creditBloodQuickening)
                {
                    ...
                    if (Creature* minchar = me->FindNearestCreature(NPC_INFILTRATOR_MINCHAR_BQ, 200.0f))
                    {
                        ...
                        minchar->RemoveByteFlag(UNIT_FIELD_ANIM_TIER, UNIT_BYTES_1_OFFSET_ANIM_TIER, UNIT_BYTE1_FLAG_ALWAYS_STAND);
            ...
            void DoAction(int32 const action)
            {
                ...
                else
                {
                    ...
                    me->SetByteFlag(UNIT_FIELD_ANIM_TIER, UNIT_BYTES_1_OFFSET_ANIM_TIER, UNIT_BYTE1_FLAG_ALWAYS_STAND);
                    me->SetCanFly(true);
            ...
            void JustReachedHome()
            {
                ...
                me->RemoveByteFlag(UNIT_FIELD_ANIM_TIER, UNIT_BYTES_1_OFFSET_ANIM_TIER, UNIT_BYTE1_FLAG_ALWAYS_STAND);
                me->SetCanFly(false);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_DELIRIOUS_SLASH:
                            if (Player* l_OffTank = Player::GetPlayer(*me, m_OffTankGuid))
                            {
                                if (!me->HasByteFlag(UNIT_FIELD_ANIM_TIER, UNIT_BYTES_1_OFFSET_ANIM_TIER, 0x03))
                        ...
                        case EVENT_PACT_OF_THE_DARKFALLEN:
                        {
                            ...
                            if (Is25ManRaid())
                                ++targetCount;
                            JadeCore::RandomResizeList<std::list<Player*>>(targets, targetCount);
                            if (targets.size() > 1)
                            {
                        ...
                        case EVENT_TWILIGHT_BLOODBOLT:
                        {
                            std::list<Player*> targets;
                            SelectRandomTarget(false, &targets);
                            JadeCore::RandomResizeList<std::list<Player*>>(targets, uint32(Is25ManRaid() ? 4 : 2));
                            for (std::list<Player*>::iterator itr = targets.begin(); itr != targets.end(); ++itr)
