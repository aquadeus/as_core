...
DoorData const doorData[] =
{
    ...
    {GO_GREEN_DRAGON_BOSS_EXIT,              DATA_VALITHRIA_DREAMWALKER, DOOR_TYPE_ROOM,       BOUNDARY_S   },
        ...
        struct instance_icecrown_citadel_InstanceMapScript : public InstanceScript
        {
            ...
            void OnGameObjectCreate(GameObject* go)
            {
                switch (go->GetEntry())
                {
                    ...
                    {
                        AddDoor(go, true);
                        if (instance->GetLegacyRaidType() == CriteriaLegacyRaidType::Heroic10 || instance->GetLegacyRaidType() == CriteriaLegacyRaidType::Heroic25 || instance->GetLegacyRaidType() == CriteriaLegacyRaidType::Normal10 || instance->GetLegacyRaidType() == CriteriaLegacyRaidType::Normal25)
                        {
                            if (GetData(DATA_VALITHRIA_DREAMWALKER) == NOT_STARTED || GetData(DATA_VALITHRIA_DREAMWALKER) == DONE)
                            {
                                HandleGameObject(0, true, go);
                                break;
                            }

                            if (GetData(DATA_VALITHRIA_DREAMWALKER) == IN_PROGRESS)
                            {
                    ...
                    case GO_CACHE_OF_THE_DREAMWALKER_25H:
                        ...
                        {
                            if (!valithria->GetMap()->GetPlayers().isEmpty())
                                go->SetLootRecipient(valithria->GetMap()->GetPlayers().begin()->getSource());
                        }
                            ...
                            case DONE:
                                if (GameObject* loot = instance->GetGameObject(DeathbringersCacheGUID))
                                {
                                    if (Creature* deathbringer = instance->GetCreature(DeathbringerSaurfangGUID))
                                        if (!deathbringer->GetMap()->GetPlayers().isEmpty())
                                            loot->SetLootRecipient(deathbringer->GetMap()->GetPlayers().begin()->getSource());
