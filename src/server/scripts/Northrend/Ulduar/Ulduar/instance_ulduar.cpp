            ...
            void OnGameObjectCreate(GameObject* gameObject)
            {
                switch (gameObject->GetEntry())
                {
                    ...
                    case GO_MIMIRON_TRAIN:
                        ...
                        /// Add unknown flag for gameobject transports, it prevents wow errors
                        gameObject->SetFlag(GAMEOBJECT_FIELD_FLAGS, GO_FLAG_MAP_OBJECT);
                ...
                switch (type)
                {
                    ...
                    case BOSS_HODIR:
                        ...
                        if (state == DONE)
                        {
                            if (GameObject* HodirRareCache = instance->GetGameObject(HodirRareCacheGUID))
                                if (GetData(DATA_HODIR_RARE_CACHE))
                                {
                                    if (Creature* hodir = instance->GetCreature(HodirGUID))
                                        if (!HodirRareCache->GetMap()->GetPlayers().isEmpty())
                                            HodirRareCache->SetLootRecipient(HodirRareCache->GetMap()->GetPlayers().begin()->getSource());
                            ...
                            {
                                if (Creature* hodir = instance->GetCreature(HodirGUID))
                                    if (!HodirChest->GetMap()->GetPlayers().isEmpty())
                                        HodirChest->SetLootRecipient(HodirChest->GetMap()->GetPlayers().begin()->getSource());
