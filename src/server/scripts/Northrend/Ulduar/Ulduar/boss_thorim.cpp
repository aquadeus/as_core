    ...
    public:
        ...
        struct boss_thorimAI : public BossAI
        {
            ...
            void EncounterPostProgress()
            {
                ...
                // Kill credit
                instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_THORIM_KILL_CREDIT);
                ...
                if (HardMode)
                {
                    ...
                    if (GameObject* go = me->SummonGameObject(RAID_MODE(GO_CACHE_OF_STORMS_HARDMODE_10, GO_CACHE_OF_STORMS_HARDMODE_25), 2134.58f, -286.908f, 419.495f, 1.55988f, 0.0f, 0.0f, 1.0f, 1.0f, WEEK))
            ...
            void EnterCombat(Unit* who)
            {
                ...
                for (uint8 n = 0; n < 7; n++)
                    ...
                    {
                        thunderOrb->SetDisplayId(thunderOrb->GetCreatureTemplate()->Modelid[1]);
                ...
                if (phase == PHASE_ARENA_ADDS && attacker && instance)
                {
                    ...
                    if (colossus && colossus->isDead() && giant && giant->isDead() && me->IsWithinDistInMap(attacker, 50.0f) && attacker->ToPlayer())
                    {
                        ...
                        if (EncounterTime <= MAX_HARD_MODE_TIME)
                        {
                            HardMode = true;
                            // Achievement
                            instance->DoUpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_THORIM_SIFFED_CREDIT);
                            // Summon Sif
...
class npc_sif : public CreatureScript
{
    public:
        ...
        struct npc_sifAI : public ScriptedAI
        {
            ...
            {
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_PACIFIED | UNIT_FLAG_REMOVE_CLIENT_CONTROL);
            }
