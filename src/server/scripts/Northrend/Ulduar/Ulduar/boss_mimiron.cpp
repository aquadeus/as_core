        ...
        struct boss_mimironAI : public BossAI
        {
            ...
            void EncounterPostProgress()
            {
                ...
                if (instance)
                {
                    ...
                    instance->DoUpdateCriteria(CRITERIA_TYPE_KILL_CREATURE, NPC_LEVIATHAN_MKII, 1);
                }
                _JustDied();
                                    ...
                                    if (instance)
                                    {
                                        if (Creature* VX_001 = me->SummonCreature(NPC_VX_001, 2744.65f, 2569.46f, 364.397f, 3.14159f, TEMPSUMMON_CORPSE_TIMED_DESPAWN, 10*IN_MILLISECONDS))
                                        {
                                            ...
                                            for (uint8 n = 5; n < 7; ++n)
                                            {
                                                ...
                                                {
                                                    Rocket->setFaction(FACTION_MONSTER);
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_ROCKET_STRIKE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 100.0f, true))
                            {
                                ...
                                {
                                    pTemp->SetDisplayId(pTemp->GetCreatureTemplate()->Modelid[1]);
...
class npc_mimiron_flame_spread : public CreatureScript
{
    public:
        ...
        struct npc_mimiron_flame_spreadAI : public ScriptedAI
        {
            ...
            {
                _instance = me->GetInstanceScript();
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_PACIFIED | UNIT_FLAG_REMOVE_CLIENT_CONTROL);
                me->SetReactState(REACT_PASSIVE);
                Reset();
            }
