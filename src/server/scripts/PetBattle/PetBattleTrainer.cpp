        ...
        bool OnGossipSelect(Player* p_Player, Creature* p_Creature, uint32 /*p_Sender*/, uint32 p_Action) override
        {
            ...
            if (p_Action == PetBattleTrainerFightActionID)
            {
                ...
                for (BattlePetNpcTeamMember l_Current : l_NpcTeam)
                {
                    ...
                    {
                        l_DisplayID = sObjectMgr->GetCreatureTemplate(sBattlePetSpeciesStore.LookupEntry(l_Current.Specie)->CreatureID)->Modelid[0];

                        if (!l_DisplayID)
                        {
                            l_DisplayID = sObjectMgr->GetCreatureTemplate(sBattlePetSpeciesStore.LookupEntry(l_Current.Specie)->CreatureID)->Modelid[1];

                            if (!l_DisplayID)
                            {
                                l_DisplayID = sObjectMgr->GetCreatureTemplate(sBattlePetSpeciesStore.LookupEntry(l_Current.Specie)->CreatureID)->Modelid[2];

                                if (!l_DisplayID)
                                {
                                    l_DisplayID = sObjectMgr->GetCreatureTemplate(sBattlePetSpeciesStore.LookupEntry(l_Current.Specie)->CreatureID)->Modelid[3];
                                }
                            }
                        }
                    }
