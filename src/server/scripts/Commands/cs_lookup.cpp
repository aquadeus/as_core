...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "ip",             SEC_GAMEMASTER,     SEC_GAMEMASTER,     true,  &HandleLookupPlayerIpCommand,        "", NULL },
            { "account",        SEC_GAMEMASTER,     SEC_GAMEMASTER,     true,  &HandleLookupPlayerAccountCommand,   "", NULL },
            { "email",          SEC_GAMEMASTER,     SEC_GAMEMASTER,     true,  &HandleLookupPlayerEmailCommand,     "", NULL },
            { NULL,             0,                  0,                  false, NULL,                                "", NULL }
        };
        static ChatCommand lookupArtifactCommandTable[] =
        {
            { NULL,             0,                  0,                  false, NULL,                                "", NULL }
        };
        static ChatCommand lookupCommandTable[] =
        {
            { "area",           SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleLookupAreaCommand,            "", NULL },
            { "creature",       SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupCreatureCommand,        "", NULL },
            { "event",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleLookupEventCommand,           "", NULL },
            { "faction",        SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupFactionCommand,         "", NULL },
            { "item",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupItemCommand,            "", NULL },
            { "itemset",        SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupItemSetCommand,         "", NULL },
            { "object",         SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupObjectCommand,          "", NULL },
            { "quest",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupQuestCommand,           "", NULL },
            { "player",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  NULL,                                "", lookupPlayerCommandTable },
            { "skill",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupSkillCommand,           "", NULL },
            { "spell",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupSpellCommand,           "", NULL },
            { "taxinode",       SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupTaxiNodeCommand,        "", NULL },
            { "tele",           SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleLookupTeleCommand,            "", NULL },
            { "title",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleLookupTitleCommand,           "", NULL },
            { "map",            SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleLookupMapCommand,             "", NULL },
            { "artifact",       SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  NULL,                                "", lookupArtifactCommandTable },
            { NULL,             0,                      0,                  false, NULL,                                "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "lookup",         SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  NULL,                                "", lookupCommandTable },
            { NULL,             0,                      0,                  false, NULL,                                "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleLookupAreaCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 areaflag = 0; areaflag < sAreaTableStore.GetNumRows(); ++areaflag)
        {
            ...
            if (areaEntry)
            {
                std::string name = areaEntry->AreaNameLang->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupCreatureCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (auto l_Iter = l_CreatureTemplates->begin(); l_Iter != l_CreatureTemplates->end(); ++l_Iter)
        {
            ...
            std::string name = l_Iter->second.Name[0];
            if (name.empty())
                continue;
    ...
    static bool HandleLookupFactionCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < sFactionStore.GetNumRows(); ++id)
        {
            if (FactionEntry const* factionEntry = sFactionStore.LookupEntry(id))
            {
                ...
                std::string name = factionEntry->NameLang->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            std::string name = itr->second.Name1->Get(handler->GetSessionDb2Locale());
            if (name.empty())
                continue;
    ...
    static bool HandleLookupItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        // Search in ItemSet.db2
        for (uint32 id = 0; id < sItemSetStore.GetNumRows(); id++)
        {
            ItemSetEntry const* set = sItemSetStore.LookupEntry(id);
            if (set)
            {
                std::string name = set->Name->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupSkillCommand(ChatHandler* handler, char const* args)
    {
        ...
        // Search in SkillLine.db2
        for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
        {
            SkillLineEntry const* skillInfo = sSkillLineStore.LookupEntry(id);
            if (skillInfo)
            {
                std::string name = skillInfo->DisplayName->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupSpellCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (uint32 id = 0; id < StoreSize; ++id)
        {
            ...
            if (spellInfo)
            {
                std::string name = spellInfo->SpellName->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupTaxiNodeCommand(ChatHandler* handler, const char * args)
    {
        ...
        // Search in TaxiNodes.db2
        for (uint32 id = 0; id < sTaxiNodesStore.GetNumRows(); id++)
        {
            TaxiNodesEntry const* nodeEntry = sTaxiNodesStore.LookupEntry(id);
            if (nodeEntry)
            {
                std::string name = nodeEntry->Name->Get(handler->GetSessionDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleLookupMapCommand(ChatHandler* /*handler*/, char const* args)
    {
        ...
        for (uint32 id = 0; id < sMapStore.GetNumRows(); id++)
        {
            ...
            if (MapInfo)
            {
                uint8 locale = handler->GetSession() ? handler->GetSession()->GetSessionDb2Locale() : sWorld->GetDefaultDb2Locale();
                ...
                if (!Utf8FitTo(name, wNamePart))
                {
                    ...
                    {
                        if (handler->GetSession() && locale == handler->GetSession()->GetSessionDb2Locale())
                            continue;
    ...
    static bool HandleLookupPlayerIpCommand(ChatHandler* handler, char const* args)
    {
        ...
        LoginDatabase.AsyncQuery(stmt, l_Player->GetQueryCallbackMgr(), [l_AccountId, limit](PreparedQueryResult const& p_Result) -> void
        {
            WorldSession* l_Session = sWorld->FindSession(l_AccountId);
            if (!l_Session || !l_Session->GetPlayer() || !l_Session->GetPlayer()->IsInWorld())
                return;

            ChatHandler l_ChatHandler(l_Session);
            LookupPlayerSearchCommand(p_Result, limit, l_ChatHandler);
        });
    ...
    static bool LookupPlayerSearchCommand(PreparedQueryResult result, int32 limit, ChatHandler& handler)
    {
        ...
        Player* l_Player = handler.GetSession() ? handler.GetSession()->GetPlayer() : nullptr;
        if (!l_Player)
            return false;
        ...
        // TODO: Need to check if the list is ordered user-friendly with limits.
        bool l_HasLimit = limit >= 0;
        auto l_LimitPtr = std::make_shared<std::atomic_int>(limit);
        ...
        do
        {
            ...
            uint32 l_GmAcc = handler.GetSession() ? handler.GetSession()->GetAccountId() : 0;

            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_SEL_CHAR_GUID_NAME_BY_ACC);
            stmt->setUInt32(0, accountId);
            CharacterDatabase.AsyncQuery(stmt, l_Player->GetQueryCallbackMgr(), [accountId, accountName, l_GmAcc, l_HasLimit, l_LimitPtr](PreparedQueryResult const& p_Result) -> void
            {
                WorldSession* l_Session = sWorld->FindSession(l_GmAcc);
                if (!l_Session || !l_Session->GetPlayer() || !l_Session->GetPlayer()->IsInWorld())
                    return;

                if (p_Result)
                {
                    ChatHandler(l_Session).PSendSysMessage(LANG_LOOKUP_PLAYER_ACCOUNT, accountName.c_str(), accountId);

                    do
                    {
                        int32 l_CurrentLimit = 0;
                        if (l_HasLimit)
                        {
                            l_CurrentLimit = l_LimitPtr->load(std::memory_order_relaxed);
                            if (l_CurrentLimit <= 0) break;
                        }

                        Field* characterFields = p_Result->Fetch();
                        uint32 guid = characterFields[0].GetUInt32();
                        std::string name = characterFields[1].GetString();

                        ChatHandler(l_Session).PSendSysMessage(LANG_LOOKUP_PLAYER_CHARACTER, name.c_str(), guid);

                        if (l_HasLimit)
                        {
                            while (!l_LimitPtr->compare_exchange_weak(l_CurrentLimit, l_CurrentLimit - 1, std::memory_order_relaxed))
                                if (l_CurrentLimit <= 0) break;
                        }
