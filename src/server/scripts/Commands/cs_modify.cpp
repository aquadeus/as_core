...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "fly",            SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyFlyCommand,           "", NULL },
            { "flyback",        SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyFlyBackCommand,       "", NULL },
            { "turn",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyTurnCommand,          "", NULL },
            { "pitch",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyPitchCommand,         "", NULL },
            { "all",            SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyASpeedCommand,        "", NULL },
            { "walk",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifySpeedCommand,         "", NULL },
            { "run",            SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyRunCommand,           "", NULL },
            { "backwalk",       SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyBWalkCommand,         "", NULL },
            { "swim",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifySwimCommand,          "", NULL },
            { "swimback",       SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifySwimBackCommand,      "", NULL },
            { "",               SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyASpeedCommand,        "", NULL },
            { NULL,             0,                      0,                  false, NULL,                              "", NULL }
        };
        static ChatCommand modifyCommandTable[] =
        {
            { "hp",             SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyHPCommand,            "", NULL },
            { "mana",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyManaCommand,          "", NULL },
            { "rage",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyRageCommand,          "", NULL },
            { "runicpower",     SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyRunicPowerCommand,    "", NULL },
            { "energy",         SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyEnergyCommand,        "", NULL },
            { "money",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyMoneyCommand,         "", NULL },
            { "scale",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyScaleCommand,         "", NULL },
            { "bit",            SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyBitCommand,           "", NULL },
            { "faction",        SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyFactionCommand,       "", NULL },
            { "spell",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifySpellCommand,         "", NULL },
            { "mount",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyMountCommand,         "", NULL },
            { "honor",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyHonorCommand,         "", NULL },
            { "reputation",     SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyRepCommand,           "", NULL },
            { "setreputation",  SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifySetRepCommand,        "", NULL },
            { "drunk",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleModifyDrunkCommand,         "", NULL },
            { "standstate",     SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyStandStateCommand,    "", NULL },
            { "phase",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleModifyPhaseCommand,         "", NULL },
            { "phaseid",        SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleModifyPhaseIDCommand,       "", NULL },
            { "gender",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyGenderCommand,        "", NULL },
            { "power",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyPowerCommand,         "", NULL },
            { "currency",       SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyCurrencyCommand,      "", NULL },
            { "artifactpower",  SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyArtifactPowerCommand, "", NULL },
            { "speed",          SEC_MODERATOR,          SEC_MODERATOR,      false, NULL,           "", modifyspeedCommandTable },
            { "arenarating",    SEC_GM_LEADER,          SEC_GM_LEADER,      false, &HandleModifyArenaRatingCommand,   "", NULL },
            { "arenammr",       SEC_GM_LEADER,          SEC_GM_LEADER,      false, &HandleModifyArenaMmrCommand,      "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                           "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "morph",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleModifyMorphCommand,          "", NULL },
            { "demorph",        SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleDeMorphCommand,              "", NULL },
            { "modify",         SEC_MODERATOR,          SEC_MODERATOR,      false, NULL,                 "", modifyCommandTable },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleModifyASpeedCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!target)
        {
            Unit* l_UnitTarget = handler->getSelectedUnit();
            ...
            l_UnitTarget->SetSpeed(MOVE_WALK,   ASpeed);
            l_UnitTarget->SetSpeed(MOVE_RUN,    ASpeed);
            l_UnitTarget->SetSpeed(MOVE_SWIM,   ASpeed);
            l_UnitTarget->SetSpeed(MOVE_FLIGHT, ASpeed);
            return true;
        }
    ...
    static bool HandleModifyMountCommand(ChatHandler* handler, const char* args)
    {
        ...
        data << uint32(target->m_movementCounter++);
        data << float(speed);
    ...
    static bool HandleModifyMoneyCommand(ChatHandler* handler, const char* args)
    {
        ...
        else
        {
            ...
            if (addmoney >=MAX_MONEY_AMOUNT)
                ...
                target->ModifyMoney(uint64(addmoney), "HandleModifyMoney from " + std::to_string(handler->GetSession()->GetPlayer()->GetGUIDLow()));
        }
    ...
    static bool HandleModifyHonorCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->GiveHonor(amount);

        handler->PSendSysMessage(LANG_COMMAND_MODIFY_HONOR, handler->GetNameLink(target).c_str(), target->GetHonor());
    ...
    static bool HandleModifySetRepCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        target->GetReputationMgr().SetReputationNoSpillOver(factionEntry, amount);
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_REP, factionEntry->NameLang, factionId,
            handler->GetNameLink(target).c_str(), target->GetReputationMgr().GetReputation(factionEntry));
        return true;
    }
    ...
    static bool HandleModifyPhaseIDCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->SetInPhase(phase, true, !target->HasInPhaseList(phase));
    ...
    static bool HandleModifyGenderCommand(ChatHandler* handler, const char* args)
    {
        ...
        target->SetByteValue(PLAYER_FIELD_INEBRIATION, PLAYER_BYTES_3_OFFSET_GENDER, gender);
...
//demorph player or unit
    ...
    static bool HandleModifyCurrencyCommand(ChatHandler* handler, const char* args)
    {
        ...
        char* l_AmountTxt = strtok(NULL, " ");
        if (!l_AmountTxt)
            return false;

        uint32 amount = atoi(l_AmountTxt);
        if (!amount)
            return false;
    ...
    static bool HandleModifyArtifactPowerCommand(ChatHandler* handler, const char* args)
    {
        if (!*args)
            return false;
        ...
        int64 amount = (uint64)atoll(args);

        MS::Artifact::Manager* l_Manager = target->GetCurrentlyEquippedArtifact();
        if (!l_Manager)
            return false;

        l_Manager->SetArtifactPower(amount);

        return true;
    }

    //Edit Player Arena Personal Rating
    static bool HandleModifyArenaRatingCommand(ChatHandler* p_Handler, const char* p_Args)
    {
        if (!*p_Args)
            return false;

        int32 l_ArenaRating = atoi((char*)p_Args);


        if (l_ArenaRating < 1 || l_ArenaRating > 3500)
        {
        ...
        if (p_Handler->HasLowerSecurity(l_Target, 0))
            return false;
        p_Handler->PSendSysMessage("You changed arena personal rating of %s to %i", p_Handler->GetNameLink(l_Target).c_str(), l_ArenaRating);
        if (p_Handler->needReportToTarget(l_Target))
            (ChatHandler(l_Target)).PSendSysMessage("Your arena personal rating has been changed by %s to %i ", p_Handler->GetNameLink().c_str(), l_ArenaRating);

        for (uint8 l_ArenaType = 0; l_ArenaType < static_cast<uint8>(BattlegroundBracketType::Max); l_ArenaType++)
        {
            l_Target->SetArenaPersonalRating(l_ArenaType, l_ArenaRating);
        }

        return true;
    }

    //Edit Player Arena MMR
    static bool HandleModifyArenaMmrCommand(ChatHandler* p_Handler, const char* p_Args)
    {
        if (!*p_Args)
            return false;

        int32 l_ArenaMMR = atoi((char*)p_Args);


        if (l_ArenaMMR < 1 || l_ArenaMMR > 3500)
        {
            p_Handler->SendSysMessage(LANG_BAD_VALUE);
            p_Handler->SetSentErrorMessage(true);
            return false;
        ...
        p_Handler->PSendSysMessage("You changed arena MMR of %s to %i", p_Handler->GetNameLink(l_Target).c_str(), l_ArenaMMR);
        if (p_Handler->needReportToTarget(l_Target))
            (ChatHandler(l_Target)).PSendSysMessage("Your arena MMR has been changed by %s to %i ", p_Handler->GetNameLink().c_str(), l_ArenaMMR);

        for (uint8 l_ArenaType = 0; l_ArenaType < static_cast<uint8>(BattlegroundBracketType::Max); l_ArenaType++)
        {
            l_Target->SetArenaMatchMakerRating(l_ArenaType, l_ArenaMMR);
        }
