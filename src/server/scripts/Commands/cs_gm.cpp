...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "chat",           SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleGMChatCommand,              "", NULL },
            { "fly",            SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleGMFlyCommand,               "", NULL },
            { "ingame",         SEC_PLAYER,             SEC_PLAYER,         true,  &HandleGMListIngameCommand,        "", NULL },
            { "list",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleGMListFullCommand,          "", NULL },
            { "visible",        SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleGMVisibleCommand,           "", NULL },
            { "",               SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleGMCommand,                  "", NULL },
            { NULL,             0,                      0,                  false, NULL,                              "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "gm",             SEC_MODERATOR,          SEC_MODERATOR,      false, NULL,                     "", gmCommandTable },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleGMFlyCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (strncmp(args, "on", 3) == 0)
        {
            ...
            sAnticheatMgr->HandleGmFly(target->GetGUIDLow(), true);
        }
        else if (strncmp(args, "off", 4) == 0)
        {
            target->SetCanFly(false);
            sAnticheatMgr->HandleGmFly(target->GetGUIDLow(), false);
        }
        else
        {
            handler->SendSysMessage(LANG_USE_BOL);
