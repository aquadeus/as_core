...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "add",            SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpAddCommand,                "", NULL },
            { "event",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpEventCommand,              "", NULL },
            { "load",           SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpLoadCommand,               "", NULL },
            { "modify",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpModifyCommand,             "", NULL },
            { "unload",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpUnLoadCommand,             "", NULL },
            { "reload",         SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleWpReloadCommand,             "", NULL },
            { "show",           SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpShowCommand,               "", NULL },
            { "start",          SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleWpStartCommand,              "", NULL },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand scriptWpCommandTable[] =
        {
            { "add",            SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, &HandleScriptWpAddCommand,          "", NULL },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "wp",             SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, NULL,               "",       wpCommandTable },
            { "script_wp",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     false, NULL,               "", scriptWpCommandTable },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        return commandTable;
    }
    ...
    */
    ...
    static bool HandleWpLoadCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!target || !target->GetCreatureData())
        {
        ...
        stmt->setUInt32(1, target->GetDBTableGUIDLow());
    ...
    static bool HandleWpUnLoadCommand(ChatHandler* handler, const char* /*args*/)
    {
        ...
        if (target->GetCreatureAddon())
        {
            if (target->GetCreatureAddon()->PathID != 0)
            {
                ...
                stmt->setString(0, target->GetCreatureData()->linkedId);
    ...
    static bool HandleWpStartCommand(ChatHandler* handler, const char* args)
    {
        char* path_number = NULL;
        uint32 pathid = 0;
        ...
        if (path_number)
        {
            ...
            auto result = WorldDatabase.PQuery("SELECT * FROM waypoints WHERE entry = %u", pathid);
            if (result)
            {
                auto result = WorldDatabase.PQuery("SELECT MAX(pointid) FROM waypoints WHERE entry = %u", pathid);
                if (result)
                {
                    uint32 maxpathid = result->Fetch()->GetInt32();
                    uint32 lastId = maxpathid + 1;

                    WorldDatabase.PExecute("REPLACE INTO waypoints (entry, pointid, position_x, position_y, position_z) VALUES (%u, %u, %f, %f, %f)",
                        pathid, lastId, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
                    handler->PSendSysMessage("[WAYPOINTS]: Point added. WP Entry: %u. Point: %u", pathid, lastId);
                }
            }
            else
            {
                WorldDatabase.PExecute("REPLACE INTO waypoints (entry, pointid, position_x, position_y, position_z) VALUES (%u, %u, %f, %f, %f)",
                    pathid, 1, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ());
                handler->PSendSysMessage("[WAYPOINTS]: New path started. WP Entry: %u", pathid);
            }
        }
