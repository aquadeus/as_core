...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "append",         SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketResponseAppendCommand,    "", NULL },
            { "appendln",       SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketResponseAppendLnCommand,  "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                    "", NULL }
        };
        static ChatCommand ticketCommandTable[] =
        {
            { "assign",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleGMTicketAssignToCommand,          "", NULL },
            { "close",          SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketCloseByIdCommand,         "", NULL },
            { "closedlist",     SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketListClosedCommand,        "", NULL },
            { "comment",        SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketCommentCommand,           "", NULL },
            { "complete",       SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketCompleteCommand,          "", NULL },
            { "delete",         SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleGMTicketDeleteByIdCommand,        "", NULL },
            { "escalate",       SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketEscalateCommand,          "", NULL },
            { "escalatedlist",  SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleGMTicketListEscalatedCommand,     "", NULL },
            { "list",           SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketListCommand,              "", NULL },
            { "onlinelist",     SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketListOnlineCommand,        "", NULL },
            { "reset",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleGMTicketResetCommand,             "", NULL },
            { "response",       SEC_MODERATOR,          SEC_MODERATOR,      true,  NULL,                                    "", ticketResponseCommandTable },
            { "togglesystem",   SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleToggleGMTicketSystem,             "", NULL },
            { "unassign",       SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleGMTicketUnAssignCommand,          "", NULL },
            { "setlang",        SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleGMTicketSetLangCommand,           "", NULL },
            { "viewid",         SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketGetByIdCommand,           "", NULL },
            { "viewname",       SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleGMTicketGetByNameCommand,         "", NULL },
            { "fsccreate",      SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketCreate,                "", NULL },
            { "fscupdate",      SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketUpdate,                "", NULL },
            { "fscappend",      SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketAppend,                "", NULL },
            { "fscend",         SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketEnd,                "", NULL },
            { "fscdelete",      SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketDelete,                "", NULL },
            { "fscticketget",   SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketGet,                   "", NULL },
            { "fscticketres",   SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMResponseResolve,             "", NULL },
            { "fscsetlang",     SEC_PLAYER,             SEC_PLAYER,         true,  &FSCHandleGMTicketSetLang,               "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                    "", NULL }
        };
    ...
    static bool FSCHandleGMTicketCreate(ChatHandler* p_Handler, char const* p_Args)
    {
        ...
        uint32 levelReq = sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ);
        ...
        if (l_Player->getClass() == CLASS_DEATH_KNIGHT)
            levelReq = sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DK_LEVEL_REQ_DIFF);
        else if (l_Player->getClass() == CLASS_DEMON_HUNTER)
            levelReq = sWorld->getIntConfig(CONFIG_TICKET_LEVEL_REQ) + sWorld->getIntConfig(CONFIG_DH_LEVEL_REQ_DIFF);

        if (l_Player->getLevel() < std::min(levelReq, (uint32)LevelLimit::MAX_LEVEL))
        {
            p_Handler->GetSession()->SendNotification(p_Handler->GetSession()->GetTrinityString(LANG_TICKET_REQ), levelReq);
            return true;
        }
    ...
    static bool FSCHandleGMTicketSetLang(ChatHandler* p_Handler, char const* p_Args)
    {
        // Don't accept tickets if the ticket queue is disabled. (Ticket UI is greyed out but not fully dependable)
        if (sTicketMgr->GetStatus() == GMTICKET_QUEUE_STATUS_DISABLED)
            return true;
        ...
        char* l_LocaleStr = strtok((char*)p_Args, " ");
        uint32 l_Locale = atoi(l_LocaleStr);

        if (l_Locale >= LocaleConstant::LOCALE_None)
            return true;

        if (GmTicket* l_Ticket = sTicketMgr->GetTicketByPlayer(l_Player->GetGUID()))
            l_Ticket->SetLocale(static_cast<LocaleConstant>(l_Locale));
    ...
    static bool HandleGMTicketAssignToCommand(ChatHandler* handler, char const* args)
    {
        ...
        static std::map<uint32, uint32> k_AccountSecurityCache;

        uint32 targetGmLevel = 0;

        auto l_Itr = k_AccountSecurityCache.find(targetAccountId);
        if (l_Itr == k_AccountSecurityCache.end())
        {
            targetGmLevel = AccountMgr::GetSecurity(targetAccountId, g_RealmID);
            k_AccountSecurityCache[targetAccountId] = targetGmLevel;
        }
        else
            targetGmLevel = l_Itr->second;
    ...
    {
        LocaleConstant l_Locale = LocaleConstant::LOCALE_None;
        std::string l_LocaleStr = (char*)args;
        if (l_LocaleStr == "en")
            l_Locale = LocaleConstant::LOCALE_enUS;
        else if (l_LocaleStr == "fr")
            l_Locale = LocaleConstant::LOCALE_frFR;
        else if (l_LocaleStr == "es")
            l_Locale = LocaleConstant::LOCALE_esES;
        else if (l_LocaleStr == "pt")
            l_Locale = LocaleConstant::LOCALE_ptPT;
        else if (l_LocaleStr == "ru")
            l_Locale = LocaleConstant::LOCALE_ruRU;

        sTicketMgr->ShowList(*handler, false, l_Locale);
        return true;
    }
    ...
    static bool HandleGMTicketUnAssignCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            security = sWorld->GetAccountSecurity(accountId);
        }
    ...
    static bool HandleGMTicketSetLangCommand(ChatHandler* p_Handler, char const* p_Args)
    {
        if (!*p_Args)
            return false;

        uint32 l_TicketId = atoi(strtok((char*)p_Args, " "));
        GmTicket* l_Ticket = sTicketMgr->GetTicket(l_TicketId);

        if (!l_Ticket)
        {
            p_Handler->SendSysMessage(LANG_COMMAND_TICKETNOTEXIST);
            return true;
        }

        char* l_LocaleStrChr = strtok(NULL, " ");
        if (!l_LocaleStrChr)
        {
            p_Handler->SendSysMessage("The language you provided isn't valid. Please use of of the following languages: fr | en | es | pt | ru");
            return true;
        }

        std::string l_LocaleStr = l_LocaleStrChr;

        LocaleConstant l_Locale = LocaleConstant::LOCALE_None;

        if (l_LocaleStr == "fr")
            l_Locale = LocaleConstant::LOCALE_frFR;
        else if (l_LocaleStr == "en")
            l_Locale = LocaleConstant::LOCALE_enUS;
        else if (l_LocaleStr == "es")
            l_Locale = LocaleConstant::LOCALE_esES;
        else if (l_LocaleStr == "pt")
        ...
        SQLTransaction l_Transaction = SQLTransaction(NULL);
        l_Ticket->SetLocale(l_Locale);
        l_Ticket->SaveToDB(l_Transaction);
        p_Handler->SendSysMessage("Language of the ticket changed.");
