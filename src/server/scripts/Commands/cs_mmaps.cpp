...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "loadedtiles", SEC_ADMINISTRATOR,     SEC_ADMINISTRATOR, false, &HandleMmapLoadedTilesCommand, "", NULL },
            { "loc",         SEC_ADMINISTRATOR,     SEC_ADMINISTRATOR, false, &HandleMmapLocCommand,         "", NULL },
            { "path",        SEC_ADMINISTRATOR,     SEC_ADMINISTRATOR, false, &HandleMmapPathCommand,        "", NULL },
            { "stats",       SEC_ADMINISTRATOR,     SEC_ADMINISTRATOR, false, &HandleMmapStatsCommand,       "", NULL },
            { "testarea",    SEC_ADMINISTRATOR,     SEC_ADMINISTRATOR, false, &HandleMmapTestArea,           "", NULL },
            { NULL,          0,                     0,                 false, NULL,                          "", NULL }
        };

        static ChatCommand commandTable[] =
        {
            { "mmap",   SEC_ADMINISTRATOR,          SEC_ADMINISTRATOR,  true,   NULL, "", mmapcommandTable },
            { NULL,     0,                          0,                  false,  NULL, "", NULL }
        };
    ...
    static bool HandleMmapLocCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        uint32 l_InstanceId = handler->GetSession()->GetPlayer()->GetInstanceId();
        if (!handler->GetSession()->GetPlayer()->GetMap()->Instanceable())
            l_InstanceId = handler->GetSession()->GetPlayer()->GetZoneId(true);

        // calculate navmesh tile location
        uint32 terrainMapId = player->GetMapId();
    ...
    static bool HandleMmapLoadedTilesCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        Player* player = handler->GetSession()->GetPlayer();
        uint32 terrainMapId = mapid;
        dtNavMesh const* navmesh = MMAP::MMapFactory::createOrGetMMapManager()->GetNavMesh(terrainMapId);
        dtNavMeshQuery const* navmeshquery = MMAP::MMapFactory::createOrGetMMapManager()->GetNavMeshQuery(terrainMapId, l_InstanceId);
        if (!navmesh || !navmeshquery)
        {
            handler->PSendSysMessage("NavMesh not loaded for current map.");
            return true;
    ...
    static bool HandleMmapTestArea(ChatHandler* handler, char const* /*args*/)
    {
        ...
        if (!creatureList.empty())
        {
            ...
            handler->PSendSysMessage("Generated %i paths in %u ms", paths, uPathLoadTime);
        }
        else
