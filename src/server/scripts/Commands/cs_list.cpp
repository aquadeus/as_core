...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "creature",       SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleListCreatureCommand,          "", NULL },
            { "item",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleListItemCommand,              "", NULL },
            { "object",         SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleListObjectCommand,            "", NULL },
            { "auras",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleListAurasCommand,             "", NULL },
            { "mail",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleListMailCommand,              "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "list",          SEC_ADMINISTRATOR,       SEC_ADMINISTRATOR,   true, NULL,                                 "", listCommandTable },
            { NULL,            0,                       0,                   false, NULL,                                "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleListAurasCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        {
            ...
            char const* name = aura->GetSpellInfo()->SpellName->Get(sWorld->GetDefaultDb2Locale());
                            ...
                            do
                            {
                                ...
                                if (result3)
                                {
                                    do
                                    {
                                        ...
                                        ItemTemplate const* l_ItemTemplate = sObjectMgr->GetItemTemplate(item_entry);
                                        if (l_ItemTemplate)
                                        {
                                            if (handler->GetSession())
                                            {
                                                char const* l_Name = l_ItemTemplate->Name1->Get(handler->GetSession()->GetSessionDbLocaleIndex());
                                                uint32 color = ItemQualityColors[l_ItemTemplate->Quality];
                                                std::ostringstream itemStr;
                                                itemStr << "|c" << std::hex << color << "|Hitem:" << item_entry << ":0:0:0:0:0:0:0:0:0|h[" << l_Name << "]|h|r";
                                                handler->PSendSysMessage(LANG_LIST_MAIL_INFO_ITEM, itemStr.str().c_str(), item_entry, item_guid, item_count);
                                            }
                                            else
                                            {
                                                char const* l_Name = l_ItemTemplate->Name1->Get(LocaleConstant::LOCALE_enUS);
                                                handler->PSendSysMessage(LANG_LIST_MAIL_INFO_ITEM, l_Name, item_entry, item_guid, item_count);
                                            }
                                        }
                                    }
