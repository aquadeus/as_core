...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "add",            SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleTeleAddCommand,             "", NULL },
            { "del",            SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleTeleDelCommand,             "", NULL },
            { "name",           SEC_MODERATOR,          SEC_MODERATOR,      true,  &HandleTeleNameCommand,            "", NULL },
            { "group",          SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleTeleGroupCommand,           "", NULL },
            { "loc",            SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleTeleLocCommand,             "", NULL },
            { "",               SEC_MODERATOR,          SEC_MODERATOR,      false, &HandleTeleCommand,                "", NULL },
            { NULL,             0,                      0,                  false, NULL,                              "", NULL }
        };
        static ChatCommand commandTable[] =
        {
            { "tele",           SEC_MODERATOR,          SEC_MODERATOR,      false, NULL,                   "", teleCommandTable },
            { NULL,             0,                      0,                  false, NULL,                               "", NULL }
        };
        return commandTable;
    }
    ...
    static bool HandleTeleNameCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (strcmp(teleStr, "$home") == 0)    // References target's homebind
        {
            if (target)
            {
                target->AddDelayedEvent([target]()->void
                {
    ...
    static bool HandleTeleLocCommand(ChatHandler* p_Handler, char const* p_Args)
    {
        char* l_LocStr = strtok((char*)p_Args, " ");
        if (l_LocStr == nullptr)
            return false;

        uint32 l_LocID = atoi(l_LocStr);
        if (sWorldSafeLocsStore.LookupEntry(l_LocID) == nullptr)
            return false;

        p_Handler->GetSession()->GetPlayer()->TeleportTo(l_LocID);
        return true;
    }
    ...
    static bool HandleTeleCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (me->isInCombat() && handler->GetSession()->GetSecurity() < SEC_GAMEMASTER)
        {
