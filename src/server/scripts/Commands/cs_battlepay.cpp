...
class battlepay_commandscript: public CommandScript
{
    public:
        ...
        ChatCommand* GetCommands() const
        {
            ...
            {
                { "dumpwebshopcategory",    SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR, false, &HandleDumpWebShopCategory,     "", nullptr },
                { "dumppack",               SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR, false, &HandleDumpWebShopPackCategory, "", nullptr },
                { "reload",                 SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR, false, &HandleReloadBattlePay,         "", nullptr },
                { "dumpiteminfo",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR, false, &HandleDumpItemInfo,            "", nullptr },
                { nullptr,                  0,                      0,                 false, nullptr,                        "", nullptr }
            };

            static ChatCommand g_CommandTable[] =
            {
                 { "battlepay",             SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,      true, nullptr, "", g_BattlepayCommandTable },
                 { nullptr,                 0,                      0,                      false, nullptr, "", nullptr }
            };
            ...
            do
            {
                ...
                if (l_FlagsFilter & 0x01)
                {
                    for (uint32 l_I = 0; l_I < sMountStore.GetNumRows(); ++l_I)
                    {
                        ...
                        l_CreatureDisplayInfoID = GetMountDisplayID(nullptr, l_I);
                ...
                if (l_FlagsFilter & 0x02)
                {
                    ...
                    for (size_t l_I = 0; l_I < sBattlePetSpeciesStore.GetNumRows(); ++l_I)
                    {
                        ...
                        l_CreatureDisplayInfoID = l_CreatureTemplate->Modelid[0];
                        break;
                    }
                }
        ...
        static bool HandleDumpItemInfo(ChatHandler* /*p_ChatHandler*/, char const* /*p_Args*/)
        {
            ...
            for (uint32 l_ItemID = 0; l_ItemID < sItemStore.GetNumRows(); l_ItemID++)
            {
                ...
                uint32 l_IconFileDataId = 0;
                ///if (g_ItemFileDataId.find(l_ItemID) != g_ItemFileDataId.end())
                ///    l_IconFileDataId = g_ItemFileDataId[l_ItemID];

                if (l_IconFileDataId == 0)
                {
                    ItemEntry const* l_ItemEntry = sItemStore.LookupEntry(l_ItemID);
                    if (l_ItemEntry != nullptr)
                        l_IconFileDataId = l_ItemEntry->IconFileDataId;
                }
                ...
                std::string l_IconBuffer = "TODO FD";// l_FileDataEntry->FileName;
