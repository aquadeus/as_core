...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
            { "load",           SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandlePDumpLoadCommand,                 "", NULL },
            { "write",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandlePDumpWriteCommand,                "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                    "", NULL }
        };
        static ChatCommand characterDeletedCommandTable[] =
        {
            { "list",           SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterDeletedListCommand,     "", NULL },
            { "delete",         SEC_CONSOLE,            SEC_CONSOLE,        true,  &HandleCharacterDeletedDeleteCommand,   "", NULL },
            { "restore",        SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleCharacterDeletedRestoreCommand,  "", NULL },
            { "old",            SEC_CONSOLE,            SEC_CONSOLE,        true,  &HandleCharacterDeletedOldCommand,      "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                   "", NULL }
        };

        static ChatCommand characterShowCommandTable[] =
        {
            { "item_logs",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     true, &HandleCharacterShowItemLogsCommand,     "", NULL },
            { NULL,             0,                      0,                  false, NULL,                                   "", NULL }
        };

        static ChatCommand characterCommandTable[] =
        {
            { "customize",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterCustomizeCommand,       "", NULL },
            { "changefaction",  SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterChangeFactionCommand,   "", NULL },
            { "changerace",     SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterChangeRaceCommand,      "", NULL },
            { "deleted",        SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  NULL,                                   "", characterDeletedCommandTable },
            { "erase",          SEC_CONSOLE,            SEC_CONSOLE,        true,  &HandleCharacterEraseCommand,           "", NULL },
            { "level",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleCharacterLevelCommand,           "", NULL },
            { "rename",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterRenameCommand,          "", NULL },
            { "reputation",     SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterReputationCommand,      "", NULL },
            { "titles",         SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterTitlesCommand,          "", NULL },
            { "getrename",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterGetrenameCommand,       "", NULL },
            { "borisitem",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  &HandleCharacterBorisItemCommand,       "", NULL },
            { "show",           SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  NULL,                                   "", characterShowCommandTable },
            { NULL,             0,                      0,                  false, NULL,                                   "", NULL }
        };

        static ChatCommand commandTable[] =
        {
            { "character",      SEC_GAMEMASTER,         SEC_GAMEMASTER,     true,  NULL,                                   "", characterCommandTable },
            { "levelup",        SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  false, &HandleLevelUpCommand,                  "", NULL },
            { "boostlevel",     SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  &HandleCharacterBoostLevelCommand,      "", NULL },
            { "pdump",          SEC_ADMINISTRATOR,      SEC_ADMINISTRATOR,  true,  NULL,                                   "", pdumpCommandTable },
            { NULL,             0,                      0,                  false, NULL,                                   "", NULL }
        };
        return commandTable;
    }
    ...
    */
    static void HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo, ChatHandler* handler)
    {
        ...
        if (PreparedQueryResult result = CharacterDatabase.Query(stmt))
            sWorld->AddCharacterInfo(delInfo.lowGuid, delInfo.name, delInfo.accountId, (*result)[2].GetUInt8(), (*result)[0].GetUInt8(), (*result)[1].GetUInt8(), (*result)[3].GetUInt8(), (*result)[4].GetUInt16());
#else /* CROSS */
        return;
#endif /* CROSS */
    }
    ...
    static bool HandleCharacterShowItemLogsCommand(ChatHandler* p_Handler, char const* p_Args)
    {
        if (!*p_Args)
        {
            p_Handler->PSendSysMessage("Wrong arguments. Please give an item id, and either select a player, or give his GUID as a second argument (this will trump any selected player)");
            return false;
        }

        char* l_CharItemID = strtok((char*)p_Args, " ");
        char* l_CharPlayerGUID = strtok(NULL, " ");

        if (!l_CharItemID)
        {
            p_Handler->PSendSysMessage("Wrong item ID");
            return false;
        }

        uint32 l_TargetGUID = 0;
        if (!l_CharPlayerGUID)
            l_TargetGUID = p_Handler->getSelectedPlayer() ? p_Handler->getSelectedPlayer()->GetGUIDLow() : 0;
        else
            l_TargetGUID = (uint32)atoi(l_CharPlayerGUID);

        if (l_TargetGUID == 0)
        {
            p_Handler->PSendSysMessage("Wrong GUID given or no proper selected target");
            return false;
        }

        uint32 l_ItemID = (uint32)atoi(l_CharItemID);
        if (l_ItemID == 0)
        {
            p_Handler->PSendSysMessage("Wrong item ID");
            return false;
        ...
        uint64 l_SenderGUID = p_Handler->GetSession()->GetPlayer()->GetGUID();

        PreparedStatement* l_Stmt = LogDatabase.GetPreparedStatement(LOG_SELECT_ITEM_GM_LOG);
        l_Stmt->setUInt32(0, l_TargetGUID);
        l_Stmt->setUInt32(1, l_ItemID);

        LogDatabase.AsyncQuery(l_Stmt, p_Handler->GetSession()->GetPlayer()->GetQueryCallbackMgr(), [l_SenderGUID, l_TargetGUID](PreparedQueryResult const& p_Result)
        {
            Player* l_Player = sObjectAccessor->FindPlayer(l_SenderGUID);
            if (!l_Player)
                return;

            if (!p_Result)
            {
                ChatHandler(l_Player->GetSession()).PSendSysMessage("No entries have been found with the specified Item Entry for Player GUID %u.", l_TargetGUID);
                return;
            }

            ChatHandler(l_Player->GetSession()).PSendSysMessage("Following Entries have been found for Player GUID %u", l_TargetGUID);

            uint8 l_AllowedRows = 0;
            uint8 l_Count = 0;

            if (p_Result->GetRowCount() > 50)
                l_AllowedRows = 50;
            else
                l_AllowedRows = p_Result->GetRowCount();

            do
            {
                Field* l_Data = p_Result->Fetch();

                uint32 l_ItemGUID = l_Data[0].GetUInt32();
                uint32 l_ItemID     = l_Data[2].GetUInt32();
                time_t l_Timestamp  = (time_t)(l_Data[3].GetUInt32());
                std::string l_Bonuses = l_Data[4].GetString();

                struct tm *l_TM = gmtime(&l_Timestamp);
                char l_Date[20];
                strftime(l_Date, sizeof(l_Date), "%Y-%m-%d", l_TM);
                sLog->outAshran("ITEM LOGS (GM) RESULT: Player GUID: %u - Item ID: %u - Item GUID: %u - Date: %s - Item Bonuses: %s", l_TargetGUID, l_ItemID, l_ItemGUID, l_Date, l_Bonuses.c_str());

                ChatHandler(l_Player->GetSession()).PSendSysMessage(TrinityStrings::ItemLogEntry, l_ItemID, l_Date);
                l_Count++;
            } while (p_Result->NextRow() && l_Count <= l_AllowedRows);
        });
    ...
    static bool HandleCharacterTitlesCommand(ChatHandler* handler, char const* args)
    {
        ...
        LocaleConstant loc = handler->GetSessionDb2Locale();
        char const* targetName = target->GetName();
        char const* knownStr = handler->GetTrinityString(LANG_KNOWN);

        // Search in CharTitles.db2
        for (uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
        {
            ...
            {
                std::string name = titleInfo->NameMale->Get(sWorld->GetDefaultDb2Locale());
                if (name.empty())
                    continue;
    ...
    static bool HandleCharacterBorisItemCommand(ChatHandler* handler, char const* args)
    {
        Player* target;
        uint64 targetGuid;
        std::string targetName;
        ...
        PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPD_ADD_AT_LOGIN_FLAG);
        stmt->setUInt16(0, uint16(AT_LOGIN_BORIS_ITEMS));
        if (target)
        {
            target->SetAtLoginFlag(AT_LOGIN_BORIS_ITEMS);
            stmt->setUInt32(1, target->GetGUIDLow());
        }
        else
        {
        ...
        handler->PSendSysMessage("Boris items will be given at character reconnection for guid %u", GUID_LOPART(targetGuid));
    ...
    static bool HandleCharacterReputationCommand(ChatHandler* handler, char const* args)
    {
        ...
        for (FactionStateList::const_iterator itr = targetFSL.begin(); itr != targetFSL.end(); ++itr)
        {
            ...
            char const* factionName = factionEntry ? factionEntry->NameLang->Get(sWorld->GetDefaultDb2Locale()) : "#Not found#";
    ...
    /**
    * Handles the '.character deleted list' command, which show all deleted characters which matches the given search string
    *
    ...
    */
    ...
    static bool HandlePDumpWriteCommand(ChatHandler* handler, char const* args)
    {
        ...
        switch (PlayerDumpWriter().WriteDump(fileStr, uint32(guid), accountId, Expansion::EXPANSION_LEGION))
        {
            case DUMP_SUCCESS:
    ...
    static bool HandleCharacterBoostLevelCommand(ChatHandler* p_Handler, char const* args)
    {
        Player* l_Player = p_Handler->GetSession()->GetPlayer();
        if (!l_Player)
            return false;

        l_Player->GiveLevel(110);
        return true;
    }
