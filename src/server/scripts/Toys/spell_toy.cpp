...
class sceneTrigger_blingtrons_circuit_design_tutorial : public SceneTriggerScript
{
    enum Constants
    {
        AchievementID = 10588
...
public:
    sceneTrigger_blingtrons_circuit_design_tutorial()
        : SceneTriggerScript("sceneTrigger_blingtrons_circuit_design_tutorial") {}

    bool OnTrigger(Player* p_Player, SpellScene const* p_Trigger, const std::string& p_Triggername) override
    {
        if (p_Triggername == "achievement")
            if (AchievementEntry const* l_Achievement = sAchievementStore.LookupEntry(AchievementID))
...
// toy 130251
class sceneTrigger_jewelcraft_game : public SceneTriggerScript
{
public:
    sceneTrigger_jewelcraft_game() : SceneTriggerScript("sceneTrigger_jewelcraft_game") {}

    bool OnTrigger(Player* player, SpellScene const* scene, const std::string& trigger) override
    {
        if (trigger == "achievement")
            player->UpdateCriteria(CRITERIA_TYPE_BE_SPELL_TARGET, 210630);
        return true;
    }
};
    ...
    public:
        ...
        class spell_item_toy_train_set_pulse_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                ...
                OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_item_toy_train_set_pulse_SpellScript::HandleTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ALLY);
            }
        };
...
void AddSC_spell_toy()
{
    ...
    new sceneTrigger_blingtrons_circuit_design_tutorial();
    new sceneTrigger_jewelcraft_game();
#ifndef CROSS
