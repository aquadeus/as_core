...
class areatrigger_rallying_banner : public AreaTriggerEntityScript
{
    public:
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 p_Time)
        {
            ...
            float l_Scale = p_AreaTrigger->GetFloatValue(OBJECT_FIELD_SCALE);
            ...
            {
                if (!l_Unit->ToCreature() || l_Unit->isInCombat())
                    continue;

                if (l_Caster->getVictim() == nullptr)
                    continue;

                if (l_Unit->ToCreature()->GetAI())
            ...
            if (m_GrowTime <= p_Time)
            {
                m_GrowTime = 1000;
                l_Scale += 2.0f;
                p_AreaTrigger->SetFloatValue(OBJECT_FIELD_SCALE, l_Scale);
            }
            else
                m_GrowTime -= p_Time;
        }
