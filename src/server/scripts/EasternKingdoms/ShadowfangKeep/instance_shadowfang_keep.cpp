    ...
    public:
        ...
        struct instance_shadowfang_keep_InstanceMapScript : public InstanceScript
        {
            ...
            void BeforePlayerEnter(Player* pPlayer)
            {
                ...
                if (GroupPtr pGroup = pPlayer->GetGroup())
                {
                    if (pGroup->isLFGGroup())
                    {
                        if (sLFGMgr->GetDungeon(pGroup->GetGUID()) == 288)
                        {
                            isLfg = true;
                        }
                    }
            ...
            void OnCreatureCreate(Creature* pCreature)
            {
                switch(pCreature->GetEntry())
                {
                    ...
                    case NPC_APOTHECARY_HUMMEL:
                        if (!isLfg)
                            pCreature->DespawnOrUnsummon(100);

                        hummerGUID = pCreature->GetGUID();
                        break;
                    case NPC_APOTHECARY_FRYE:
                        if (!isLfg)
                            pCreature->DespawnOrUnsummon(100);

                        fryeGUID = pCreature->GetGUID();
                        break;
                    case NPC_APOTHECARY_BAXTER:
                        if (!isLfg)
                            pCreature->DespawnOrUnsummon(100);

                        baxterGUID = pCreature->GetGUID();
                        break;
                }
            }
            ...
            uint64 GetData64(uint32 type)
            {
                switch (type)
                {
                    ...
                    case DATA_HUMMEL: return hummerGUID;
