                ...
                if (m_CheckPlayerTimer)
                {
                    if (m_CheckPlayerTimer <= p_Diff)
                    {
                        ...
                        for (Player* l_Player : l_PlayerList)
                        {
                            ...
                            if ((l_Player->GetQuestStatus(34446) != QUEST_STATUS_REWARDED && l_Player->GetTeamId() == TEAM_HORDE) ||
                                (l_Player->GetQuestStatus(35884) != QUEST_STATUS_REWARDED && l_Player->GetTeamId() == TEAM_ALLIANCE))
                                l_Player->TeleportTo(1265, 4066.5f, -2382.25f, 94.8536f, 1.5708f);
                            else
                            {
                                switch (l_Player->GetTeamId())
                                {
                                    case TEAM_ALLIANCE:
                                        l_Player->TeleportTo(1116, 2332.718f, 432.501f, 1.010f, 2.557f);
                                        break;
                                    case TEAM_HORDE:
                                        l_Player->TeleportTo(1116, 5535.01f, 5019.88f, 12.637f, 5.174f);
                                        break;
                                    default:
                                        break;
                                }
        ...
        struct npc_blasted_lands_zidormiAI : public ScriptedAI
        {
            ...
            void sGossipSelect(Player* p_Player, uint32 /*p_Sender*/, uint32 /*p_Action*/) override
            {
                if (p_Player->HasAura(eSpell::TimeTravelling))
                {
                    ...
                    {
                        p_Player->SwitchToPhasedMap(eMaps::NewBlastedLands);
                    });
                }
                else
                {
                    ...
                    p_Player->AddMapTask(eTaskType::TaskSwitchToPhasedMap, [p_Player]() -> void
                    {
                        p_Player->SwitchToPhasedMap(eMaps::EasternKingdoms);
                    });
                }
            ...
            if (p_Player->GetMapId() == BLASTED_LANDS_DRAENOR_PHASE || p_Player->GetMapId() == EASTERN_KINGDOM_MAP_ID)
            {
                if (p_NewZoneID != p_OldZoneID && (p_NewZoneID == BLASTER_LANDS_ZONE_ID || p_OldZoneID == BLASTER_LANDS_ZONE_ID))
                {
                    if (p_NewZoneID == BLASTER_LANDS_ZONE_ID && p_Player->GetMapId() == EASTERN_KINGDOM_MAP_ID
                        && !p_Player->HasAura(eSpell::TimeTravelling) && p_Player->getLevel() > 89)
                    {
                        p_Player->AddMapTask(eTaskType::TaskSwitchToPhasedMap, [p_Player]() -> void
                        {
