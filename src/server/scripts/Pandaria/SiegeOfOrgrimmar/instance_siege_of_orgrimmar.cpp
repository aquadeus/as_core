...
public:
    ...
    struct instance_siege_of_orgrimmar_InstanceMapScript : public InstanceScript
    {
        ...
        std::vector<uint64> elitesnipersGuids;
        ...
        void FillInitialWorldStates(ByteBuffer& packet) override
        {   
            packet << uint32(WS_SHOW_SOUTH_TOWER) << uint32(ShowSouthTower == IN_PROGRESS);
            packet << uint32(WS_SHOW_NORTH_TOWER) << uint32(ShowNorthTower == IN_PROGRESS);
            packet << uint32(WS_SHOW_CAPTURE_SOUTH_TOWER) << uint32(ShowSouthTower == SPECIAL);
            packet << uint32(WS_SHOW_CAPTURE_NORTH_TOWER) << uint32(ShowNorthTower == SPECIAL);
            packet << uint32(WS_SOUTH_TOWER) << uint32(SouthTowerCount);
            packet << uint32(WS_NORTH_TOWER) << uint32(NorthTowerCount);
            packet << uint32(WS_CAPTURE_SOUTH_TOWER) << uint32(SouthTowerCount);
            packet << uint32(WS_CAPTURE_NORTH_TOWER) << uint32(NorthTowerCount);
            packet << uint32(WS_SHOW_KORKRON_CANNON) << uint32(ShowCannon == IN_PROGRESS);
            packet << uint32(WS_SHOW_KORKRON_CANNON) << uint32(CannonCount);
            packet << uint32(WS_KORKRON_CANNON_COUNT) << uint32(CannonCount);
        }
        ...
        void OnCreatureCreate(Creature* creature) override
        {
            switch (creature->GetEntry())
            {
                ...
                case NPC_HORDE_CANNON:
                    ...
                    break;
                case NPC_ELITE_SNIPER:
                    elitesnipersGuids.push_back(creature->GetGUID());
                    break;
            }
        }
        ...
        void OnGameObjectCreate(GameObject* go) override
        {
            switch (go->GetEntry())
            {
                ...
                case GO_NORTH_DOOR:
                    ...
                    if (go->GetEntry() == GO_NORUSHEN_EX_DOOR && GetBossState(DATA_NORUSHEN) == DONE)
                        go->SetVisible(false);
                    break;
                ...
                case GO_RUSTY_BARS:
                    ...
                    if (GetBossState(DATA_KORKRON_D_SHAMAN) == DONE)
                        go->SetVisible(false);
                    break;
                ...
                case GO_NAZGRIM_EX_DOOR:
                    ...
                    if (GetBossState(DATA_GENERAL_NAZGRIM) == DONE)
                        go->SetVisible(false);
                    break;
                ...
                case GO_BLACKFUSE_ENT_DOOR:
                    ...
                    if (GetBossState(DATA_MALKOROK) == DONE)
                        go->SetVisible(false);
                    break;
                case GO_ENT_GATE:
                    AddDoor(go, true);
                ...
                case GO_KLAXXI_EX_DOOR:
                    ...
                    if (GetBossState(DATA_KLAXXI) == DONE)
                        go->SetVisible(false);
                    break;
        ...
        bool SetBossState(uint32 id, EncounterState state) override
        {
            ...
            case DATA_GALAKRAS:
            {
                ...
                case IN_PROGRESS:
                    ...
                    break;
                case DONE:
                    for (auto& guid : elitesnipersGuids)
                ...
                case DONE:
                    ...
                    for (std::vector<uint64>::const_iterator itr = klaxxilist.begin(); itr != klaxxilist.end(); itr++)
                    {
                        if (Creature* klaxxi = instance->GetCreature(*itr))
                        {
                            ...
                            klaxxi->RemoveFlag(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);
                        }
                    }
        ...
        void CreatureDies(Creature* creature, Unit* /*killer*/) override
        {
            ...
            {
                auto GetPosInRadiusWithRandOrient = [](Unit* unit, float dist, float& x, float& y)
                {
                    auto mod = frand(0.f, 6.f);
                    auto orientation = mod <= 5 ? mod + float(urand(1, 9)) / 10 : mod;
                ...
                if (Creature* kaztik = instance->GetCreature(GetData64(NPC_KAZTIK)))
                {
                    if (kaztik->isAlive() && kaztik->isInCombat())
                    {
                        ...
                        {
                            float x, y;
                            GetPosInRadiusWithRandOrient(ap, 50.0f, x, y);
            ...
            if (!PlayerList.isEmpty())
            {
                for (Map::PlayerList::const_iterator Itr = PlayerList.begin(); Itr != PlayerList.end(); ++Itr)
                {
                    if (Player* player = Itr->getSource())
                    {
                        ...
                        {
                            //player->SetFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_ALLOW_ONLY_ABILITY);
        ...
        {
            if (player && player->GetSession() && player->GetSession()->GetSecurity() > SEC_PLAYER)
                return true;
        ...
        bool IsRaidBoss(uint32 creature_entry) override
        {
        ...
        void Update(uint32 diff) override
        {
            if (kdshamansresetready)
            {
                if (kdshamansresetready <= diff)
                {
                    ...
                    for (uint8 n = 0; n < 2; n++)
                        if (auto kdsmaunt = instance->GetCreature(!n ? bloodclawGuid : darkfangGuid))
            ...
            while (uint32 eventId = Events.ExecuteEvent())
            {
                ...
                case EVENT_FINISH_1_H:
                    if (auto Lorthemar = instance->GetCreature(GetData64(NPC_LORTHEMAR_THERON_H)))
                        Lorthemar->AI()->Talk(7);
                    Events.ScheduleEvent(EVENT_FINISH_2_H, 2000);
                    break;
                case EVENT_FINISH_2_H:
                    if (auto Sylvana = instance->GetCreature(GetData64(NPC_LADY_SYLVANAS_WINDRUNNER_H)))
                        Sylvana->AI()->Talk(5);
                ...
                case EVENT_FINISH_1_A:
                    if (auto Jaina = instance->GetCreature(GetData64(NPC_LADY_JAINA_PROUDMOORE_A)))
                        Jaina->AI()->Talk(5);
                    Events.ScheduleEvent(EVENT_FINISH_2_A, 2000);
                    break;
                case EVENT_FINISH_2_A:
                    if (auto Varian = instance->GetCreature(GetData64(NPC_KING_VARIAN_WRYNN_A)))
                        Varian->AI()->Talk(7);
