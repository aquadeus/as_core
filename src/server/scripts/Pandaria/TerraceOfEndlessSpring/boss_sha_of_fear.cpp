...
class boss_sha_of_fear : public CreatureScript
{
    public:
        ...
        struct boss_sha_of_fearAI : public BossAI
        {
            ...
            void DamageDealt(Unit* /*victim*/, uint32& /*damage*/, DamageEffectType damageType, SpellInfo const* /*p_SpellInfo*/)
            {
...
class mob_terror_spawn : public CreatureScript
{
    public:
        ...
        struct mob_terror_spawnAI : public ScriptedAI
        {
            ...
            void CheckHitResult(MeleeHitOutcome& p_MeleeHitResult, SpellMissInfo& p_SpellInfo, Unit* p_Attacker, SpellInfo const* const /*p_SpellInfo*/) override
            {
                if (me->isInBack(p_Attacker))
                {
        ...
        class spell_ominous_cackle_cast_SpellScript : public SpellScript
        {
            ...
            void CorrectTargets(std::list<WorldObject*>& targets)
            {
                ...
                for (auto itr : targets)
                {
                    ...
                    if (!tankDone && player->GetRoleForGroup(player->GetActiveSpecializationID()) == Roles::ROLE_TANK)
                    {
                    ...
                    if (!healerDone && player->GetRoleForGroup(player->GetActiveSpecializationID()) == Roles::ROLE_HEALER)
                    {
                        healerDone = true;
                        targetsToAdd--;
                        realTargets.push_back(itr);
