...
DoorData const doorData[] =
{
    ...
    { GOB_DURUMU_EXIT_GATE,                 DATA_DURUMU_THE_FORGOTTEN,  DOOR_TYPE_PASSAGE,  BOUNDARY_S      },
...
public:
    ...
    struct instance_throne_of_thunder_InstanceMapScript : public InstanceScript
    {
        ...
        void RelocateResPosIfNeeded(float& p_X, float& p_Y, float& p_Z, float& p_O, uint32& p_MapID, Player* /*p_Player*/) override
        {
            ...
            if (GetBossState(eData::DATA_HORRIDON) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostHorridon);
            else if (GetBossState(eData::DATA_COUNCIL_OF_ELDERS) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostCouncil);
            else if (GetBossState(eData::DATA_TORTOS) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::TortosLair);
            else if (GetBossState(eData::DATA_MEGAERA) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostMegaera);
            else if (GetBossState(eData::DATA_JI_KUN) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostJiKun);
            else if (GetBossState(eData::DATA_DURUMU_THE_FORGOTTEN) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::DurumuLoc);
            else if (GetBossState(eData::DATA_PRIMORDIUS) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostPrimordius);
            else if (GetBossState(eData::DATA_DARK_ANIMUS) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostDarkAnimus);
            else if (GetBossState(eData::DATA_IRON_QON) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostIronQon);
            else if (GetBossState(eData::DATA_TWIN_CONSORTS) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostTwins);
            else if (GetBossState(eData::DATA_LEI_SHEN) == DONE)
                l_Loc = sWorldSafeLocsStore.LookupEntry(eSafeLocs::PostLeiShen);
