...
Position const g_RadenTeleportPosition = { 5592.302f, 4657.388f, -2.477f, 3.23f };
...
class spell_decapitate : public SpellScriptLoader
{
    public:
        ...
        class spell_decapitate_AuraScript : public AuraScript
        {
            ...
            void Register() override
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_decapitate_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
            }
        };
...
class areatrigger_thunder_trap_activator : public AreaTriggerScript
{
    public:
        ...
        bool OnTrigger(Player* p_Player, AreaTriggerEntry const* /*p_Trigger*/) override
        {
            if (!p_Player)
                return false;
...
class areatrigger_quarter_area : public AreaTriggerScript
{
    public:
        ...
        /// need to be rework
        /*bool OnTrigger(Player* p_Player, AreaTriggerEntry const* p_Trigger)
        {
            if (!p_Player)
                return false;
...
class go_displacement_pad : public GameObjectScript
{
    public:
        ...
        bool OnGossipHello(Player* p_Player, GameObject* /*p_GameObject*/) override
        {
            if (p_Player)
            {
                Position l_Pos = { 5710.45f, 4094.18f, 157.09f, 0.0f };

                Map* l_Map = p_Player->GetMap();
                if (!l_Map)
                    return false;

                if (l_Map->GetDifficultyID() == Difficulty10HC || l_Map->GetDifficultyID() == Difficulty25HC)
                {
                    if (p_Player->IsNearPosition(&l_Pos, 20.0f))
                    {
                        p_Player->NearTeleportTo(g_RadenTeleportPosition.m_positionX,
                            g_RadenTeleportPosition.m_positionY,
                            g_RadenTeleportPosition.m_positionZ,
                            g_RadenTeleportPosition.m_orientation);
    ...
    public:
        ...
        void OnUpdate(AreaTrigger* p_AreaTrigger, uint32 /*p_Time*/) override
        {
            ...
            for (Unit* l_Unit : l_TargetList)
            {
                ...
                if (!p_AreaTrigger->IsInPolygon(l_Unit))
                {
                    if (l_Unit->HasAura(SPELL_LIGHTNING_WHIP_HIT))
                    {
                        l_Unit->RemoveAura(SPELL_LIGHTNING_WHIP_HIT);
