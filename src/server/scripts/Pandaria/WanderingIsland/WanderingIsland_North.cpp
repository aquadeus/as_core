        ...
        struct mob_master_shang_xi_AI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (me->IsNearPosition(&l_Pos, 0.5f))
                    me->RemoveByteFlag(UNIT_FIELD_ANIM_TIER, UNIT_BYTES_1_OFFSET_ANIM_TIER, UNIT_STAND_STATE_SIT_MEDIUM_CHAIR);
...
public:
    ...
    struct mob_min_dimwindAI : public ScriptedAI
    {
        ...
        void DamageTaken(Unit* pDoneBy, uint32 &uiDamage, SpellInfo const* /*p_SpellInfo*/) override
        {
...
public:
    ...
    bool OnQuestAccept(Player* player, Creature* creature, Quest const* quest)
    {
        ...
        {
            Creature* tempSummon = creature->SummonCreature(54734, creature->GetPositionX() + 19, creature->GetPositionY() + 4, creature->GetPositionZ() - 1, creature->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN, 0, player->GetGUID());
            if (tempSummon)
            {
    ...
    public:
        ...
        class spell_huo_benediction_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                ...
                AfterEffectRemove += AuraEffectRemoveFn(spell_huo_benediction_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_INCREASE_SPEED, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            }
        };
