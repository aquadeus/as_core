...
/// Gleaming Crane Statue - 220903
class go_gleaming_crane_statue : public GameObjectScript
{
    public:
        go_gleaming_crane_statue() : GameObjectScript("go_gleaming_crane_statue") { }

        enum eSpells
        {
            CraneWings      = 144385,
            ForwardThrust   = 144391
        };

        bool OnGossipHello(Player* p_Player, GameObject* p_GameObject) override
        {
            //p_Player->KnockbackFrom(p_Player->GetPositionX(), p_Player->GetPositionY(), 0.f, 320.f, p_Player);

            Position l_Pos = { -704.33f, -5479.12f, 219.34f, 0.31f };
            p_Player->GetMotionMaster()->MoveJump(l_Pos, 20.f, 70.f);

            p_Player->AddAura(eSpells::CraneWings, p_Player);
    ...
    public:
        ...
        class spell_item_timeless_caches_SpellScript : public SpellScript
        {
            ...
            void Register() override
            {
                ...
                OnEffectHitTarget += SpellEffectFn(spell_item_timeless_caches_SpellScript::HandleCreateItem, EFFECT_0, SPELL_EFFECT_CREATE_LOOT);
            }
        };
