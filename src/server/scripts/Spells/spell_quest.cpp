...
class spell_q11396_11399_force_shield_arcane_purple_x3: public SpellScriptLoader
{
    public:
        ...
        class spell_q11396_11399_force_shield_arcane_purple_x3_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                AfterEffectApply += AuraEffectApplyFn(spell_q11396_11399_force_shield_arcane_purple_x3_AuraScript::HandleEffectApply, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
                AfterEffectRemove += AuraEffectRemoveFn(spell_q11396_11399_force_shield_arcane_purple_x3_AuraScript::HandleEffectRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
...
class spell_q39932_milling : public SpellScriptLoader
{
    public:
        spell_q39932_milling() : SpellScriptLoader("spell_q39932_milling") { }
        ...
        class spell_q39932_milling_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_q39932_milling_SpellScript);
            ...
            void HandleAfterCast()
            {
                ...
                {
                    l_Player->AreaExploredOrEventHappens(eQuest::QuestId);
                }
            }

            void Register()
            {
                AfterCast += SpellCastFn(spell_q39932_milling_SpellScript::HandleAfterCast);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_q39932_milling_SpellScript();
        }
};

class spell_pvp_wq_called_to_battle : public SpellScriptLoader
{
    public:
        spell_pvp_wq_called_to_battle() : SpellScriptLoader("spell_pvp_wq_called_to_battle") { }

        class spell_pvp_wq_called_to_battle_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_pvp_wq_called_to_battle_AuraScript);
            ...
            void HandleAfterEffectRemove(AuraEffect const* p_AuraEffect, AuraEffectHandleModes p_Modes)
            {
                if (GetTargetApplication()->GetRemoveMode() == AuraRemoveMode::AURA_REMOVE_BY_EXPIRE)
                {
                    ...
                    l_Target->ApplyModFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_IN_PVP, true);
                    l_Target->ApplyModFlag(PLAYER_FIELD_PLAYER_FLAGS, PLAYER_FLAGS_PVP_TIMER, false);
                    l_Target->UpdatePvP(true, true);
                }
            }

            void Register()
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_pvp_wq_called_to_battle_AuraScript::HandleAfterEffectRemove, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK);
            }
        };

         AuraScript* GetAuraScript() const
         {
            return new spell_pvp_wq_called_to_battle_AuraScript();
         }
};

/// 232210 - Crushing Blow
class spell_q46179_crushing_blow : public SpellScriptLoader
{
public:
    spell_q46179_crushing_blow() : SpellScriptLoader("spell_q46179_crushing_blow") {}

    class spell_q46179_crushing_blow_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_q46179_crushing_blow_SpellScript);
        ...
        void HandleAfterCast()
        {
            ...
            l_Caster->RemoveAura(eValues::CrushingBlow);

            if (Player* l_Player = l_Caster->ToPlayer())
                if (Unit* l_Target = l_Player->FindNearestCreature(eValues::FelCorruptor, 8.0f))
                {
                    l_Target->DestroyForPlayer(l_Player);
                    l_Player->KilledMonsterCredit(eValues::FelCorruptor);
                }
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_q46179_crushing_blow_SpellScript::HandleAfterCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_q46179_crushing_blow_SpellScript();
    }
};

// 37910 37962 36812 37968
class spell_gen_soaring_be : public SpellScriptLoader
{
public:
    spell_gen_soaring_be() : SpellScriptLoader("spell_gen_soaring_be") { }

    class spell_gen_soaring_be_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_soaring_be_AuraScript);

        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (auto owner = GetUnitOwner())
                owner->CastSpell(owner, 37108, true);
        }

        void Register()
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_gen_soaring_be_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_FEATHER_FALL, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_gen_soaring_be_AuraScript();
    }
};
...
void AddSC_quest_spell_scripts()
{
    ...
    new spell_q39932_milling();

    new spell_pvp_wq_called_to_battle();
    new spell_q46179_crushing_blow();
}
#endif
