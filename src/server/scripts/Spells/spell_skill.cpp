...
namespace MS { namespace Skill
{
                ...
                void HandleAfterCast()
                {
                    ...
                    if (l_SpellID != SpellIDs::ResearchWarbinderInkItem)
                    {
                        for (uint32 l_I = 0; l_I < sSkillLineAbilityStore.GetNumRows(); ++l_I)
                        {
                            ...
                            if (!l_Entry || l_Entry->SkillLine != SKILL_INSCRIPTION || l_Entry->spellId != l_SpellID)
                                continue;

                            if (l_CurrentSkillValue < l_Entry->MinSkillLineRank || l_CurrentSkillValue > l_Entry->TrivialSkillLineRankHigh)
                                continue;

                            uint32 l_NumSkillUp = l_Entry->NumSkillUps;

                            if (l_CurrentSkillValue >= l_Entry->TrivialSkillLineRankLow)
                                l_NumSkillUp = 1;
    ...
    /// Called by Mass Mill Aethril - 209658, Mass Mill Dreamleaf - 209659, Mass Mill Foxflower - 209660, Mass Mill Fjarnskaggl - 209661, Mass Mill Yseralline Seeds - 210116, Mass Mill Starlight Rose - 209662, Mass Mill Felwort - 209664
    /// Update 7.3.5 Build 23420
    class spell_inscription_mass_milling : public SpellScriptLoader
    {
        public:

            spell_inscription_mass_milling() : SpellScriptLoader("spell_inscription_mass_milling") { }

            class spell_inscription_mass_milling_SpellScript : public SpellScript
            {
                PrepareSpellScript(spell_inscription_mass_milling_SpellScript);

                enum eSpells
                {
                    MassMillAethril         = 209658,
                    MassMillDreamleaf       = 209659,
                    MassMillFoxflower       = 209660,
                    MassMillFjarnskaggl     = 209661,
                    MassMillYserallineSeeds = 210116,
                    MassMillStarlightRose   = 209662,
                    MassMillFelwort         = 209664
                };
                ...
                void HandleEffectHit(SpellEffIndex effIndex)
                {
                    ...
                    uint32 l_ReagentEntry = GetSpellInfo()->Reagent[0];
                    uint32 l_Count = GetSpellInfo()->ReagentCount[0] / 5;
                    if (l_ReagentEntry == 0 || l_Count < 1 || l_Count > 4)
                        return;

                    for (uint8 i = 0; i < l_Count; ++i)
                        l_Player->AutoStoreLoot(l_ReagentEntry, LootTemplates_Milling);
                }

                void Register()
                {
                    OnEffectHitTarget += SpellEffectFn(spell_inscription_mass_milling_SpellScript::HandleEffectHit, SpellEffIndex::EFFECT_0, SpellEffects::SPELL_EFFECT_CREATE_LOOT);
                }
            ...
            SpellScript* GetSpellScript() const override
            {
                return new spell_inscription_mass_milling_SpellScript();
            }
    };
        ...
        public:
            ...
            class spell_Skill_ResetSecondaryProperties_SpellScript : public SpellScript
            {
                ...
                bool HaveRequireIlevel(Item* p_ItemTarget)
                {
                    ...
                    if (p_ItemTarget->GetItemLevel(GetCaster()->ToPlayer()) < l_RequireIlevel)
                        return false;
                ...
                void ResetSecondaryProperties()
                {
                    ...
                    for (auto l_ItemBonusGroupID : l_ItemTarget->GetTemplate()->m_ItemBonusGroups)
                    {
                        ...
                        for (auto l_Bonus : l_ItemBonusGroup->Bonuses)
                    ...
                    for (auto l_ItemBonusGroupID : l_ItemTarget->GetTemplate()->m_ItemBonusGroups)
                    {
                        ...
                        std::vector<uint32> l_BonusList(l_ItemBonusGroup->Bonuses);
            ...
            public:
                ...
                class spell_Skill_GrowFromSkillLevel_SpellScript : public SpellScript
                {
                    ...
                    void Register() override
                    {
                        ...
                        OnEffectHitTarget += SpellEffectFn(spell_Skill_GrowFromSkillLevel_SpellScript::HandleCreateItem, EFFECT_0, SPELL_EFFECT_CREATE_LOOT);
                    }
                };
            ...
            public:
                ...
                class spell_Skill_Enchantment_TemporalCrystal_SpellScript : public SpellScript
                {
                    ...
                    void Register() override
                    {
                        ...
                        OnEffectHitTarget += SpellEffectFn(spell_Skill_Enchantment_TemporalCrystal_SpellScript::HandleCreateItem, EFFECT_0, SPELL_EFFECT_CREATE_LOOT);
                    }
                };
        ...
        class spell_skill_upgrade_armor_or_weapon_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckCast()
            {
                if (Item* l_ItemTarget = GetExplTargetItem())
                {
                    ...
                    {
                        if (l_ItemTarget->GetItemLevel(l_Player) >= (uint32)GetSpellInfo()->Effects[EFFECT_0].BasePoints)
    ...
    /// Darkmoon Card of Legion - 191659 192859 192890
    /// Update 7.1.5 Build 23420
    class spell_Skill_Darkmoon_Card_Of_Legion : public SpellScriptLoader
    {
        public:

            spell_Skill_Darkmoon_Card_Of_Legion() : SpellScriptLoader("spell_Skill_Darkmoon_Card_Of_Legion") { }

            class spell_Skill_Darkmoon_Card_Of_Legion_SpellScript : public SpellScript
            {
                PrepareSpellScript(spell_Skill_Darkmoon_Card_Of_Legion_SpellScript);

                void HandleOnCast()
                {
                    std::vector<uint32> l_RewardCard = {
                        128729, 128730, 128731, 128732, 128733, 128734, 128735, 128736, ///< Dominion
                        128737, 128738, 128739, 128740, 128741, 128742, 128743, 128744, ///< Hellfire
                        128721, 128722, 128723, 128724, 128725, 128726, 128727, 128728, ///< Promises
                        128713, 128714, 128715, 128716, 128717, 128718, 128719, 128720  ///< Immortality
                    };
                ...
                void Register()
                {
                    OnCast += SpellCastFn(spell_Skill_Darkmoon_Card_Of_Legion_SpellScript::HandleOnCast);
                    OnEffectHitTarget += SpellEffectFn(spell_Skill_Darkmoon_Card_Of_Legion_SpellScript::HandleEffectHit, SpellEffIndex::EFFECT_0, SpellEffects::SPELL_EFFECT_CREATE_LOOT);
                }
            ...
            SpellScript* GetSpellScript() const override
            {
                return new spell_Skill_Darkmoon_Card_Of_Legion_SpellScript();
            }
    };


    /// Last Update 7.3.5 build 26365
    /// Called By Mining - 2576, 3564, 10248, 29354, 50310, 74517, 102161, 158754, 195122
    class spell_skill_mining : public SpellScriptLoader
    {
        public:
            spell_skill_mining() : SpellScriptLoader("spell_skill_mining") { }

            class spell_skill_mining_SpellScript : public SpellScript
            {
                PrepareSpellScript(spell_skill_mining_SpellScript);

                enum eItems
                {
                    ItemPeonsMiningPick = 116913
                };

                enum eSpells
                {
                    PeonsMiningPick     = 172100
                };
                ...
                void HandleOnPrepare()
                {
                    ...
                    int32 l_CastTime = l_Spell->GetCastTime();
                    int32 l_ReduceTime = 0;
                    constexpr int32 l_MinCastTime = 200;

                    if (l_Player->HasItemCount(eItems::ItemPeonsMiningPick))
                    {
                        if (SpellInfo const* l_ItemSpellInfo = sSpellMgr->GetSpellInfo(eSpells::PeonsMiningPick))
                            if (l_ItemSpellInfo->CheckLocation(l_Player->GetMapId(), l_Player->GetZoneId(), l_Player->GetAreaId(), l_Player) == SpellCastResult::SPELL_CAST_OK)
                                l_ReduceTime += 1500;
                    }

                    l_Spell->SetCastTime(std::max(l_MinCastTime, l_CastTime - l_ReduceTime));
                }

                void Register() override
                {
                    OnPrepare += SpellOnPrepareFn(spell_skill_mining_SpellScript::HandleOnPrepare);
                }
            };

            SpellScript* GetSpellScript() const
            {
                return new spell_skill_mining_SpellScript();
            }
    };
...
void AddSC_spell_skill()
{
    ...
    new MS::Skill::spell_inscription_mass_milling();
    ...
    new MS::Skill::spell_Skill_Darkmoon_Card_Of_Legion();
    new MS::Skill::spell_skill_mining();
}
#endif
