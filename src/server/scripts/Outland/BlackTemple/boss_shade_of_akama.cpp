...
public:
    ...
    struct boss_shade_of_akamaAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            ResetTimer = 60000;
            DefenderTimer = 15000;
            m_HardResetTimer = 10 * TimeConstants::MINUTE * TimeConstants::IN_MILLISECONDS;
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            /// Boss shouldn't lasts this long, make him reset to prevent lags
            if (m_HardResetTimer <= diff)
                HasKilledAkama = true;
            else
                m_HardResetTimer -= diff;
            ...
            if (IsBanished)
            {
                ...
                if (DeathCount >= 6)
                {
                    me->RemoveAura(SPELL_SHADE_SOUL_CHANNEL_2);
                    if (AkamaGUID)
                    {
                        ...
                        if (Akama && Akama->isAlive())
                        {
                            IsBanished = false;
                            me->GetMotionMaster()->Clear(false);
                            me->GetMotionMaster()->MovePoint(1, Akama->GetPosition(), true);
