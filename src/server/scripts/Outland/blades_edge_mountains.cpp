...
class spell_quest_lashhan_channeling : public SpellScriptLoader
{
    public:
        ...
        class spell_quest_lashhan_channeling_AuraScript : public AuraScript
        {
            PrepareAuraScript(spell_quest_lashhan_channeling_AuraScript);
            ...
            void HandleEffectRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                ...
                GuidList* l_SummonGuids = l_Target->GetSummonList(eCreatures::KaliriTotem);

                std::vector<uint64> l_SummonGuidsCopy { l_SummonGuids->begin(), l_SummonGuids->end() };

                for (auto l_Guid : l_SummonGuidsCopy)
                {
                    if (Creature* l_Summon = l_Target->GetCreature(*l_Target, l_Guid))
                        l_Summon->DespawnOrUnsummon(1000);
                }
            }

            void Register() override
            {
                AfterEffectRemove += AuraEffectRemoveFn(spell_quest_lashhan_channeling_AuraScript::HandleEffectRemove, SpellEffIndex::EFFECT_0, AuraType::SPELL_AURA_DUMMY, AuraEffectHandleModes::AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const override
        {
            return new spell_quest_lashhan_channeling_AuraScript();
        }
};
        ...
        struct npc_simon_bunnyAI : public ScriptedAI
        {
            ...
            */
            ...
            void SpellHitTarget(Unit* target, const SpellInfo* spell)
            {
                ...
                // so player gets damage equal to calculated damage  db2 basepoints for SPELL_BAD_PRESS_DAMAGE (~50)
