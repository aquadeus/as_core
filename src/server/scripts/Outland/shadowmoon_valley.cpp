    ...
    struct npc_lord_illidan_stormrageAI : public ScriptedAI
    {
        ...
        void CheckEventFail()
        {
            ...
            if (GroupPtr EventGroup = player->GetGroup())
            {
                ...
                members.foreach([&](Group::MemberSlotPtr l_Member) -> void
                {
                    GroupMember = (Unit::GetPlayer(*me, l_Member->guid));
                    if (!GroupMember)
                        return;
...
enum eToLegionHoldQuest
{
    NPC_JOVAAN = 21633
};

class spell_crate_dusguise : public SpellScriptLoader
{
public:
    spell_crate_dusguise() : SpellScriptLoader("spell_crate_dusguise") { }

    class spell_crate_dusguise_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_crate_dusguise_AuraScript);

        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* target = GetTarget())
                if (!target->FindNearestCreature(NPC_JOVAAN, 50.0f, true))
                {
                    target->SetRooted(true);
                    target->SummonCreature(NPC_JOVAAN, -3311.00f, 2951.89f, 171.21f, 5.53f, TEMPSUMMON_TIMED_DESPAWN, 57000);
                }
        }
        ...
        void Register()
        {
            AfterEffectApply += AuraEffectRemoveFn(spell_crate_dusguise_AuraScript::OnApply, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(spell_crate_dusguise_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_TRANSFORM, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_crate_dusguise_AuraScript();
    }
};
