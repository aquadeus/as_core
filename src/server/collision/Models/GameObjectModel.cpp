...
bool GameObjectModel::initialize(const GameObject& go, const GameObjectDisplayInfoEntry& info)
{
    ...
    sLog->outInfo(LOG_FILTER_POINTERS, "Pointer 0x%" PRIxPTR " -> GameObjectModel linked to gob entry %u Guid: %u", std::uintptr_t(this), owner->GetEntry(), owner->GetDBTableGUIDLow());
