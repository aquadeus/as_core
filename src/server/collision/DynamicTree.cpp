...
float DynamicMapTree::getHeight(float x, float y, float z, float maxSearchDist, uint32 phasemask, bool fromUnder /*=false*/) const
{
    G3D::Vector3 v(x, y, z);
    G3D::Ray r(v, G3D::Vector3(0, 0, fromUnder ? 1 : -1));
    DynamicTreeIntersectionCallback callback(phasemask);
