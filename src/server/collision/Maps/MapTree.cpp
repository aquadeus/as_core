...
namespace VMAP
{
    ...
    class AreaInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                sLog->outDebug("maps", "AreaInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    class LocationInfoCallback
    {
        public:
            ...
            {
#ifdef VMAP_DEBUG
                sLog->outDebug("maps", "LocationInfoCallback: trying to intersect '%s'", prims[entry].name.c_str());
#endif
    ...
    */
    ...
    float StaticMapTree::getHeight(const Vector3& pPos, float maxSearchDist, bool fromUnder /*=false*/) const
    {
        float height = G3D::finf();
        Vector3 dir = Vector3(0, 0, fromUnder ? 1 : -1);
        G3D::Ray ray(pPos, dir);   // direction with length of 1
        float maxDist = maxSearchDist;
        if (getIntersectionTime(ray, maxDist, false))
        {
            height = fromUnder ? pPos.z + maxDist : pPos.z - maxDist;
        }
        return(height);
    }
    ...
    bool StaticMapTree::LoadMapTile(uint32 tileX, uint32 tileY, VMapManager2* vm)
    {
        ...
        auto l_Itr = iLoadedTiles.find(packTileID(tileX, tileY));

        /// Dont reload a already loaded tile
        if (l_Itr != iLoadedTiles.end() && l_Itr->second)
            return true;
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            ...
                            if (!iLoadedSpawns.count(referencedNode))
                                sLog->outError(LOG_FILTER_MAPS , "StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
